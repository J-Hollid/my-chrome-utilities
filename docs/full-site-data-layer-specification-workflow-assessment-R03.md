...
| Concurrent stale edits | Passed in model and focused browser observations. The rendered conflict review names pending/current/conflicting field counts and offers Reload current revision, Reapply pending edit, and Merge selected fields. Reapply preserved an externally saved description and the pending collection edit in one new revision; model coverage verifies per-field conflict choices. | Focused browser assertions; `test/data-layer-specification-repository-test.mjs` |
