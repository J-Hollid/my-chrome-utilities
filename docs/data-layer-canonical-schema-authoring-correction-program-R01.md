...
| C48 | Property facets leak into a long menu and conditions are detached from the rules they guard | Authoring 023–030 | A compact top-level layered menu opens one complete Definition form or nested conditional Rules; one ordinary-value field distinguishes expected from comma-separated allowed values | Viewport-level layered overlay host, canonical definition adapter, typed ordinary-value parser and formatter, conditional rule AST, compiler, validator, focus adapter, repository, and Undo | Six-surface menu fingerprints, trigger-relative geometry, background inertness, focus containment, one-pass definition persistence, bracket-free Table text, rule When and Then controls, three matching and non-matching examples, compatible composition, named conflicts, unchanged definitions, and reload | A, B, C, E | Operators complete ordinary property editing in one blocking Definition form; every conditional rule owns its condition and supersedes only matching definition facets |
| C49 | Rules require conditions, condition trees expose mechanics, example methods are inert, and property overlays can become unreachable after growth or resize | Authoring 023–033 and 037 | Definition uses zero-to-many Allowed values and a method-dependent example control; rules default to Always; condition rows are concise and directly editable; groups use one Add group choice; top-level overlay layers remain trigger-associated, remeasure after content or viewport changes, move upward into available space, and scroll the active layer only when it still cannot fit | Canonical value adapter, example control projection, optional conditional-rule AST, condition-tree projection, blocking overlay geometry, resize observation, and responsive layout | Empty, single, and multiple value persistence; three example methods; unconditional cardinality; concise human summary; add/remove predicate and All/Any/Not groups; absent redundant actions; low-row canonical and composed tables; dynamic growth and browser resize; upward repositioning before overflow; active-layer reachability; stable document and editor scroll; overflow removal; focus containment and restoration; reload and validation | A, B, C, E | An unconditional rule saves without a condition, example method changes the control, condition editing exposes no tree jargon, and an open layer uses empty upper viewport space before falling back to its own scrolling |
...
- one layered property menu whose single Definition form combines type, presence,
  ...
  each Rule has an optional When condition and required Then outcome; content growth
  and browser resizing move the open stack upward to use available viewport space,
  falling back to active-layer scrolling only when the complete layer still cannot
  fit;
