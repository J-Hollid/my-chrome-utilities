...
| C43 | Complex row detail exposes every facet form at once and differs between schema editors | Authoring 023 | One shared property context menu launches one focused section editor across all six contributor kinds | Shared menu, focused editor host, staged property session, focus adapter, and responsive presentation | Cross-surface menu and section fingerprints, absent embedded forms, single mounted editor, staged values, Cancel and Escape results, and origin focus | B, E | Opening a property never expands a giant row or mounts unrelated facet editors |
...
| C49 | Rules require conditions, condition trees expose mechanics, example methods are inert, and low-row overlays open outside view | Authoring 023–033 | Definition uses zero-to-many Allowed values and a method-dependent example control; rules default to Always; condition rows are concise and directly editable; groups use one Add group choice; active overlays scroll minimally into view and contain their controls | Canonical value adapter, example control projection, optional conditional-rule AST, condition-tree projection, overlay geometry and responsive layout | Empty, single, and multiple value persistence; three example methods; unconditional cardinality; concise human summary; add/remove predicate and All/Any/Not groups; absent redundant actions; 360-pixel bounding boxes; minimum page scroll; reload and validation | A, B, C, E | An unconditional rule saves without a condition, example method changes the control, condition editing exposes no tree jargon, and every active overlay is immediately operable |
