...
| C49 | Rules require conditions, condition trees expose mechanics, example methods are inert, and low-row overlays are constrained by the editor | Authoring 023–033 | Definition uses zero-to-many Allowed values and a method-dependent example control; rules default to Always; condition rows are concise and directly editable; groups use one Add group choice; top-level overlay layers remain trigger-anchored and use the browser viewport rather than the editor scroll region | Canonical value adapter, example control projection, optional conditional-rule AST, condition-tree projection, blocking overlay geometry and responsive layout | Empty, single, and multiple value persistence; three example methods; unconditional cardinality; concise human summary; add/remove predicate and All/Any/Not groups; absent redundant actions; low-row canonical and composed tables; stable editor geometry and scroll; viewport-bounded active-layer overflow; focus containment and restoration; reload and validation | A, B, C, E | An unconditional rule saves without a condition, example method changes the control, condition editing exposes no tree jargon, and a short editor never clips or scroll-constrains the active overlay |
...
- one layered property menu whose single Definition form combines type, presence,
  ...
  each Rule has an optional When condition and required Then outcome;
