...
| C42 | A Flow Page-frame card opens a Flow-specific editor instead of the shared schema-contribution workspace | Layering 021 | Both Page-frame entry points open the same complete editor used by Shared Profiles, Page Groups, and Pages while preserving Flow return state | Page-frame card and inline-action routing, shared editor host, sparse Page-instance command adapter, and Flow view state | Cross-surface control inventory, contributor and schema IDs, isolated record hashes, command scope, restored selection, viewport, expanded example, and focus | B, C, E | Contributor context changes identity and provenance but never substitutes a reduced or parallel editor |
