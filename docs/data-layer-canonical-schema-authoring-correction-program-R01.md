...
Every Page Group in that stack contributes its complete effective schema. Its
referenced Shared Profiles compose as parallel peers before its local Page Group
contribution, and those inherited properties continue through the Page, Flow Page
instance, validation, documentation, and export. A Shared Profile reached through
multiple selected groups or directly by the Page is deduplicated by stable identity
without losing its inheritance-route provenance. Excluding one group excludes only
profiles that have no remaining participating route.
...
| C59 | Page compilation drops Shared Profiles inherited by Page Groups | Structural authoring 007–009 | Each selected Page Group contributes its complete profile-plus-local effective schema through every downstream context | Transitive contributor graph, stable-identity deduplication, provenance, Page compiler, Fixture validator, and documentation | Two profiles on one group, repeated profile routes, applicability exclusion, Page and Page-instance rows, Fixture result, and documentation | C, D, E | No Shared Profile property stops at a Page Group or appears twice, and every surviving route remains explainable |
