...
| Compact reorderable editor controls | Schema projection, no-op, and compact-handle presentation corrections QA-integrated through `4d5c420f` after exact six-pack/774-task review evidence; reorderable item row-composition correction proposed for approval on 2026-08-23 | Preserve the settled handle and movement contracts. If approved, implement stable task `reorderable-item-row-composition-correction` from `docs/data-layer-compact-reorderable-editor-controls-R01.md`: make every migrated host place its grip beside the primary identity in one row, repair all four shared Documentation ordered-choice consumers, audit every original migration consumer, and replace defect-only aggregate geometry with per-consumer installed evidence. Run read-only ownership intent before product coding; preserve domain behavior and do not run the all-20 gate. |
