...
|---|---|---|
| Tealium Live | User-approved on 2026-09-09 for coder handoff and focused QA integration | Implement stable task `tealium-live` under `docs/tealium-live-R01.md` and `docs/tealium-live-verification-R01.md`, with the product/runtime pairs for `tealium-detection`, `tealium-live`, and `tealium-source-navigation`. Add Tealium beside Data Layer and Hotkeys, use Live sessions, detect custom-hosted and bundled tags, and open verified current-document sources through a small DevTools bridge. Classify manifest/background ownership before product coding; preserve exact host consumers and use required independent preparation. Experimental evidence is diagnostic only. Master promotion remains separate. |
