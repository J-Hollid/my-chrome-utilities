...
| Utility navigation icons | User-approved on 2026-09-10 for implementation and focused QA integration | Implement `utility-navigation-icons` under `docs/utility-navigation-icons-R01.md` and its product/runtime contracts. Use the approved preview: compact DL, HK, and Tealium T artwork with full accessible names and hover/focus labels. Preserve utility order, keyboard navigation, and retained sessions. Check exact ownership before coding; the host path forecast is 31 tasks, while the broad shell stylesheet selects 840. Report at 15 active coder minutes against a 30-minute allowance; no all-pack feature gate. |
