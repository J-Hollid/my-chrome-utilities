...
| Aggregate child failure routing | Required after incident `39b11f5e-e0f4-49c0-8709-b9bd6845df29` attributed a Flow-export child failure only to the Shell aggregate's missing result | Implement `aggregate-child-failure-routing` under `docs/verification-task-checkpoint-incident-repair-R01.md`: emit validated child outcomes and a failed parent completion, bind the exact legacy incident append-only to its canonical child task, preserve normal reliability rules, and resume the parked campsite candidate only from the reviewed QA descendant. |
