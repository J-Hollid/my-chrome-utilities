...
| Data Layer Live target permission recovery | Product paused before implementation; standing-authorized ownership preparation starts from exact QA `e54f411a0f` on 2026-08-24 | Under `docs/data-layer-live-target-permission-recovery-R01.md`, task `verification-slice-live-target-permission-recovery` must extract and prove a behavior-preserving Capture-owned coordination seam with Shell as its exact installed consumer, without narrowly assigning all of `src/side-panel.ts` or running all packs. Keep product scenarios 009 and runtime 002 unchanged and red, then automatically reissue `live-target-permission-recovery` from the exact preparation QA head. |
