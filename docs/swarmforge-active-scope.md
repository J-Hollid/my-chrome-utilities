...
| Flow nested-Event thumbnail containment correction | Evidence-contract candidate `565e217111` passes direct proof but its governed Shell review is blocked by stale public-path expectations in the unrelated layered Page-group installed consumer | Run standing-authorized `verification-repair-layered-page-group-display-paths` from current QA under `docs/data-layer-flow-table-documentation-export-program-R01.md`; align only the rendered Documentation matrix expectations with QA-integrated public root names, preserve canonical slash-path identity and every other leaf, then automatically reissue `verification-slice-flow-nested-event-thumbnail-containment` from the exact repair QA head before resuming the conserved product remainder. |
