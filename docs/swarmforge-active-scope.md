...
| Documentation property-row presentation | User-approved for coder handoff on 2026-08-22 | Under `docs/data-layer-flow-table-documentation-export-program-R01.md`, export root properties without a leading slash, object members with dot notation, and wildcard array members as `[x]`; preserve canonical paths for stored selection, lookup, diagnostics, provenance, and repairs; use stable task `documentation-property-display-path` with the focused `flow_export` boundary and no all-runnable-pack gate. |
