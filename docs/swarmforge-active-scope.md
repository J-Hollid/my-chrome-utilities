...
|---|---|---|
| Side-panel companion ordered prerequisites | User ordered Serena use assessment, module declaration ownership, and dialog decomposition through coder return `20260905T231226Z_000008_from_coder` | Start `serena-use-assessment` under `docs/side-panel-companion-ordered-prerequisites-R01.md` and `features/swarmforge-serena-use-assessment.feature`. After each reviewed QA integration, issue the next named stage in order. Preserve incomplete product draft `75be4ec6` as a recovery reference only. No all-runnable-pack feature gate or decomposition exception. |
| Side-panel companion brand correction | Approved; paused for the three ordered prerequisites; specification baseline `8e1ece9e39` | Resume stable task `side-panel-companion-brand-correction` from the reviewed prerequisite QA descendant under `docs/side-panel-companion-brand-correction-R01.md` and both companion feature files. Retain all seven views, contrast, navigation, one record per project, details, existing actions, and fresh installed runtime proof. Port only applicable draft changes; do not merge the incomplete candidate into a prerequisite. |
