...
| Nested checkpoint capability routing | Required after coherent routing candidate `397def8449` conserved the exact routing delta but its execution-checkpoint contract exposed both leaked inner synthetic protocol records and an undeclared real-CLI local-loopback prerequisite | Implement `nested-checkpoint-capability-routing-repair` from exact QA `2dcc24aa08`: route only the exact execution-checkpoint task through declared shared-loopback authority, keep nested synthetic protocol output inside its own receipt, and preserve production streaming and fail-closed protocol validation. Keep the routing candidate and failed receipt immutable. |
