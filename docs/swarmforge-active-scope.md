...
| Data Layer Live target permission recovery | User-approved for coder handoff on 2026-08-24 | Under `docs/data-layer-live-target-permission-recovery-R01.md`, reconcile a failed exact page probe into the selected target's access state, expose Request access in the current setup step, and recheck the same tab and history path after an exact-origin grant. Require read-only ownership intent before coding and no all-runnable-pack feature gate. |
