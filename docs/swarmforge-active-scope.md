...
| QA verification pack taxonomy evolution | Bounded focused evidence reaffirmed by the user on 2026-08-21; both mechanical genuinely-global declarations are audit-only and prohibited | Reissue `registry-derived-verification-packs` from the exact correction commit. Keep `scripts/verification-reliability-values.mjs` unchanged; retain `scripts/verification-pack-cardinality/` only under the Shell-owned slice with no registry pack consumers; prove executable synthetic registries and the specification-bound focused route; and do not request the same acceptance again while that scope is conserved. |
...
| Compact reorderable editor controls | Approved for immediate QA implementation on 2026-08-19 | Implement stable task `compact-reorderable-editor-controls` from the approved specification commit under `docs/data-layer-compact-reorderable-editor-controls-R01.md`; run read-only ownership intent before product coding and preserve every listed domain boundary. |
