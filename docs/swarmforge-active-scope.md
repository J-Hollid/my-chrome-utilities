...
| Legacy campsite satisfaction compatibility | Required after QA integration exposed the immutable taskless generation and cross-role handoff topology | Implement `legacy-campsite-satisfaction-compatibility` under `docs/campsite-implementation-prerequisite-gate-R01.md`: append an exact generation binding, resolve the QA-ready handoff through common project authority, persist beside the parked product, preserve every existing artifact byte, and then resume the conserved original remainder. Scenario 088 remains independent. |
