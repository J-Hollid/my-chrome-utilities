...
|---|---|---|
| Serena initial-instructions correction | User-approved follow-up active after companion QA integration `c573b9822b`; all four role queues and activity records were idle after handoff completion | Implement `serena-initial-instructions-repair` from the current QA specification descendant under `docs/serena-initial-instructions-follow-up-R01.md` and `features/swarmforge-serena-initial-instructions.feature`. Prove both filters, offline local configuration refresh, and the actual instruction-to-symbol sequence. Refresh connections only after the reviewed correction is ready and roles are idle. |
