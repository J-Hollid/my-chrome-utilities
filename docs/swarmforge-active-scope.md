...
| Flow Page-card effective-name correction | User-approved for coder handoff on 2026-08-22 | Add directional Flow scenario 050 and its runtime partner under `docs/data-layer-canvas-first-flow-workspace-program-R02.md` as task `flow-page-card-effective-name`; render exactly one visible effective Page name, prefer a stored Flow-specific override, remove visible source provenance and `Context-setting Page` card text, preserve source/context semantics in Details and accessibility, and begin with the focused `flow_graph` forecast with no all-runnable-pack gate. |
