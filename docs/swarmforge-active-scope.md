...
| QA verification granularity ratchet | Initial ratchet `066ea284de`; first-use mapping repair `8d3cf5012c`; durable-runtime staging repair `06222ff00f`; Flow schema-editor route seam QA-integrated at `4daea21df0` | Preserve the exact `layered_schema_flow_editor_route` slice and reviewed integrated-seam disposition; the stable scrolling product resumes from the documentation scorecard descendant without another assessment loop. |
...
| Flow instance schema-editor scrolling correction | Active for automatic reissue after verification prep `4daea21df0`; scenario 047 remains unimplemented | Reissue stable product task `flow-instance-schema-editor-scrolling` from the exact documentation scorecard descendant, consume `src/layered-schema/flow-editor-route-layout.ts`, run fresh exact changed-path planning, and do not run the all-20 gate. |
