...
| Tealium source targets | User-approved on 2026-09-10; metadata dependency QA-integrated at `0f72e53b93` | Implement stable task `tealium-source-targets` from the received descendant specification under `docs/tealium-source-targets-R01.md` and the updated source-navigation product/runtime contracts. Open a verified file when only its location is uncertain, use observed tag URLs and registered extension evidence, and provide Go to u.send plus Go to u.extend. Preserve unresolved file ambiguity and accepted metadata. Forecast 45 Tealium/Shell checks plus package; recompute coder intent and exact preflight. |
