...
|---|---|---|
| Tealium Live | User-approved; product paused for independent ownership preparation on 2026-09-09 | Implement stable task `tealium-live` under `docs/tealium-live-R01.md` and `docs/tealium-live-verification-R01.md`, with the product/runtime pairs for `tealium-detection`, `tealium-live`, and `tealium-source-navigation`. Add Tealium beside Data Layer and Hotkeys, use Live sessions, detect custom-hosted and bundled tags, and open verified current-document sources through a small DevTools bridge. Complete `ownership-prep-tealium-live` under `docs/ownership-prep-tealium-live-R01.md` with the staged sequence in `docs/ownership-prep-tealium-live-stages-R01.md`: integrate declaration support first, then settle structural preparation. Each stage needs independent review and implementation-bearing QA; Stage A alone cannot resume the product. Then reissue stable task `tealium-live` from that exact QA head. Preserve exact host consumers. Experimental evidence is diagnostic only. Master promotion remains separate. |
