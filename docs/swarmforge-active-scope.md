...
| Data Layer Live target permission recovery | Dormant coordinator preparation QA-integrated at `9f8a8b037b`; target-path apply-callback preparation QA-integrated at `80721743c8` after ten named focused tasks with no properties, unrelated pack arrays, or all-runnable-pack checkpoint | Automatically resume stable task `live-target-permission-recovery` from the exact QA descendant under `docs/data-layer-live-target-permission-recovery-R01.md`. Reconstruct accepted product candidate `1cf347a22c` and its slow-success repair as a patch reference without merging its old lineage. Leave the integrated broad side-panel callback unchanged; implement product scenario 009 and runtime scenario 002 through the bounded Capture seam, preserving valid active-tab access, exact-origin permission, same-tab path recheck, target/session identity, and focused feature verification. |
