...
|---|---|---|
| Side-panel companion brand correction | User-approved for coder handoff on 2026-09-06; baseline QA `3f3953dcf7` | Implement stable task `side-panel-companion-brand-correction` under `docs/side-panel-companion-brand-correction-R01.md` and its product/runtime feature pair. Correct contrast first, align all seven side-panel views with Studio, simplify navigation and project presentation, and preserve existing actions. Assess exact ownership before product edits; use a justified small preparation or conservative coverage with a recorded limitation. Keep the full product scope and use focused QA evidence only. |
