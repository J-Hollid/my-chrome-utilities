...
|---|---|---|
| Schema context JSON Schema export | User-approved for coder handoff on 2026-09-07 | Implement stable task `schema-context-json-schema-export` under `docs/schema-context-json-schema-export-R01.md` and its named product/runtime contracts. Expose preview, copy, and download for the displayed effective Draft or revision in every schema host. Add the export button to both existing Studio editor headers without a toolbar redesign. Use the program's read-only ownership intent and exact focused verification route; integrate only into QA. |
