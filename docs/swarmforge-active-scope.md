...
|---|---|---|
| Repository retrieval defaults and AGENTS.md | User-approved on 2026-09-08 for the coder queue after current multiple-observation-source work | Implement `repository-retrieval-defaults` under `docs/retrieval-defaults-and-repository-instructions-R01.md` and `features/repository-retrieval-defaults.feature`. Retain concise root `AGENTS.md` guidance; add bounded read-only discovery with explicit continuation and full-read access; verify actual instruction activation separately from observed use. Use read-only ownership intent and focused QA evidence. Preserve the current product task, role duties, MCP protocol, and complete required reads; no master promotion or all-runnable-pack feature checkpoint. |
| Project multiple observation sources | User-approved on 2026-09-08 for coder handoff and focused QA integration from `dce549f1c3` | Implement stable task `project-multiple-observation-sources` under `docs/data-layer-project-observation-sources-R01.md`, `features/data-layer-project-observation-sources.feature`, and `features/data-layer-project-observation-sources-runtime.feature`. Follow `docs/data-layer-project-observation-sources-verification-R01.md` for development focus, shared paths, ownership intent, and exact focused evidence. Observe multiple event arrays on one selected tab with independent status and source-labelled capture; preserve project persistence, source identity, and separate push destinations. Extract source coordination before extending the large Capture controller. No master promotion or all-runnable-pack feature checkpoint is authorized. |
