...
| Verification defect census and consolidated repair | User-directed correction active after the Event Library recovery repeatedly split one verification family into single-symptom prerequisites | Implement `docs/swarmforge-verification-defect-census-and-consolidated-repair-R01.md`: require one immutable bounded census before repair authority, one stable task for all in-family defects, same-family reopen without nested repair, and hard repair-handoff, QA-ready, integration, satisfaction, and resumption gates. Keep the Event Library remainder parked and issue no further single-defect unblocker while this correction is in flight. |
