...
| Flow Page-card effective-name correction | Product candidate `02ceabfd62` and refactorer candidate `8bbb93e6c6` paused after exposing a stale two-sided Flow evidence contract; independent repair is standing-authorized on 2026-08-22 | Run `verification-slice-flow-page-card-effective-name` from current QA under `docs/data-layer-canvas-first-flow-workspace-program-R02.md`; align only the `flow_graph` handler required-key range and Shell-owned complete-evidence fixture with runtime050 and prove its omission/falsification fails, preserve the Page-card product and property strengthening as a stacked remainder, then automatically resume stable task `flow-page-card-effective-name` from the exact repair QA head with the original focused product scope and no all-runnable-pack gate. |
