...
| Layered schema inherited exclusion and Example reconciliation | QA-integrated at `ad9b9276c2` after exact five-pack/113-task review evidence | Preserve direct stable-identity exclusion, atomic removal of same-contributor local facets, hidden unsupported actions, dependency and invariant safety, durable contained-occurrence behavior, one-step Undo, and selected/custom Example reconciliation. Rejected candidate `51aa431eb1` remains a patch reference only. |
