...
| QA verification granularity ratchet | Initial ratchet `066ea284de`; first-use mapping repair `8d3cf5012c`; durable-runtime staging repair `06222ff00f`; Flow schema-editor route assessment active from QA `28e7b2dded` | Implement `verification-slice-flow-instance-schema-editor-scrolling`, settle `src/data-layer-layered-schema-ui.ts` as an extracted route-layout seam or explicit parent fallback, then automatically reissue the product from the resulting QA head. |
...
| Flow instance schema-editor scrolling correction | Paused after read-only intent at QA `28e7b2dded`; no product coding or coherent candidate exists | QA-integrate `verification-slice-flow-instance-schema-editor-scrolling`, then automatically reissue stable product task `flow-instance-schema-editor-scrolling` from that exact QA head under directional Flow scenario 047, fresh exact changed-path planning, and no all-20 gate. |
