...
|---|---|---|
| Tealium Live | Stage A QA-integrated; additive utility isolation still unresolved; product paused | Implement stable task `tealium-live` under `docs/tealium-live-R01.md` and `docs/tealium-live-verification-R01.md`, with the product/runtime pairs for `tealium-detection`, `tealium-live`, and `tealium-source-navigation`. Add Tealium beside Data Layer and Hotkeys, use Live sessions, detect custom-hosted and bundled tags, and open verified current-document sources through a small DevTools bridge. Complete `ownership-prep-tealium-live` under `docs/ownership-prep-tealium-live-R01.md` under the current Stage B route and Stage A scorecard in `docs/ownership-prep-tealium-live-stage-b-R01.md`. Stage A passed 194 tasks in two packs. The 824-task Stage B intent is not an accepted execution route. Apply `docs/tealium-utility-isolation-correction-R01.md`: prove additive and private Tealium selection without unrelated Data Layer families, retaining exact shared host consumers. Final preparation still needs independent review and implementation-bearing QA. Then reissue stable task `tealium-live` from that exact QA head. Preserve exact host consumers. Experimental evidence is diagnostic only. Master promotion remains separate. |
