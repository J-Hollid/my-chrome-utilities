...
|---|---|---|
| Tealium Live | Activation and DevTools connection recovery QA-integrated through `ba51f3a225` | Preserve accepted task `tealium-live` and all terminal obligations. `docs/tealium-live-connection-recovery-qa-R01.md` records 46 passing Shell tasks including package and actual worker-stop recovery with its debugger detached. The original low-cost target remains unproved. Automatic metadata enrichment is the separately approved task in the next row. |
| Tealium Live idle connection recovery | User resumed the active scope and requested coder handoff on 2026-09-12 | Continue stable repair task `tealium-live` under `docs/tealium-live-idle-recovery-R01.md` and `features/tealium-connection-recovery.feature` / `features/tealium-connection-recovery-runtime.feature`. Repair quiet-connection retry exhaustion and recovery feedback through focused review and QA integration. The hold is lifted; master promotion remains separate. |
...
| QA-to-master integration after schema export | User-authorized on 2026-09-08; schema export is QA-integrated and release intake is complete | Proceed with `qa-master-after-schema-export` under `docs/qa-master-after-schema-export-R01.md` and its intake record. Freeze the exact recording head, route directly to the architect, and advance master only after exact final-ready evidence. No further routine approval is needed. |
| Schema context JSON Schema export | QA-integrated at `8cad26dabe` on 2026-09-08 after 1,121 passing review tasks | Preserve all 14 installed host/surface cases, exact Draft or revision preview, copy, and native download behavior under `docs/schema-context-json-schema-export-R01.md` and its product/runtime contracts. The user-approved cumulative master integration now follows `docs/qa-master-after-schema-export-R01.md`. |
