...
| Flow Documentation template instance content | QA-integrated at `a5290029c1` after exact two-pack/119-task review evidence | Preserve context-specific Page and Event examples, configured concept groups, private Page-instance visual bindings, renderer-local WebP conversion for Excel, immutable snapshot staleness, and fail-closed retryable visual-body loading. Cumulative promotion to `master` remains a separate user decision. |
| Layered schema inherited exclusion and Example reconciliation | QA-integrated at `ad9b9276c2` after exact five-pack/113-task review evidence; Page and Flow Page-instance selection UX correction proposed on 2026-08-23 and awaiting coder-handoff approval | Under `docs/data-layer-page-flow-inherited-property-selection-R01.md`, replace property-row/Definition exclusion discovery with first-class inherited-property selection for Pages and Flow Page-instances, omit exclusions from the effective Table and Tree, preserve sparse stable-identity storage and current-parent restoration, and leave Event occurrences and selected/custom Example reconciliation unchanged. Rejected candidate `51aa431eb1` remains a patch reference only. |
