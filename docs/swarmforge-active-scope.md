...
| Documentation template library | Guided Excel template authoring is QA-integrated at `a8ee95869b`; documentation-template recovery is QA-integrated at `08cbfe8fef` after exact 7-pack/210-task review evidence; Contract 3 area properties were approved by the user on 2026-08-23 | Implement `excel-template-area-properties`: one strict Properties cell combines generated-image fit, position, and padding, while Repeat `separator-area` emits a named trailing sub-area only between items. Preserve Contract 2 compatibility, fail-closed validation, immutable project data, the unchanged Rich contract, and terminal-deferred incidents. Cumulative promotion to `master` remains a separate user decision. |
