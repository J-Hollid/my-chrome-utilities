...
| Flow Page-card effective-name correction | Product candidate `02ceabfd62` and refactorer candidate `8bbb93e6c6` paused after exposing one stale shared Flow evidence fixture; independent repair is standing-authorized on 2026-08-22 | Run `verification-slice-flow-page-card-effective-name` from current QA under `docs/data-layer-canvas-first-flow-workspace-program-R02.md`; change only the Shell-owned complete-evidence fixture to require runtime001 through runtime050, preserve the product and its property strengthening as a stacked remainder, then automatically resume stable task `flow-page-card-effective-name` from the exact repair QA head with the original focused product scope and no all-runnable-pack gate. |
