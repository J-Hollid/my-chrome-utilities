...
|---|---|---|
| Serena development pilot | Approved; optional-tool ownership preparation is required before implementation resumes | Issue `verification-slice-serena-development-pilot` under `docs/serena-toolchain-ownership-preparation-R01.md`. Preserve the core checker, root lock, and global runtime ownership. After independent review and QA integration, reissue `serena-development-pilot` under `docs/serena-development-pilot-R01.md` from that exact QA head. No product delta exists; no comparison tasks or feature-mode all-pack run are authorized. |
