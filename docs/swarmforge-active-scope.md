...
| Nested checkpoint capability routing | Coherent four-path candidate `496777c33c` proves the exact local-loopback route and nested-output containment but is parked after its fresh exact evidence exposed a stale VTD-012 conservation inventory already present on its base | Keep candidate `496777c33c`, tree `1b3ff4d80d`, and failed diagnostic receipt immutable. After contract-conservation transition repair reaches QA, reissue only its exact four-path delta on that descendant and require fresh exact review evidence. |
| Verification contract-conservation transitions | Required because the immutable VTD-012 baseline still names four occurrences replaced by later approved contracts while its validator ignores current source digests and undeclared current leaves | Implement `verification-contract-conservation-transition-repair` from exact QA `486774f47d`: preserve the baseline, record four authority-bound transitions, inventory all current additions and owner-source digests in an append-only generation, make checking bidirectional and read-only, and permit only explicit deterministic reviewed refresh. No task-local exception or parked-candidate edit is allowed. |
