...
| Data Layer Live target permission recovery | First ownership preparation QA-integrated at `9f8a8b037b`; product stopped at coherent candidate `1cf347a22c` because the later behavior-bearing target-path `apply` callback makes exact planning all-20/890-task coarse-boundary | Issue standing-authorized task `verification-slice-live-target-permission-path-apply` from exact QA `59d9089c6d` under `docs/data-layer-live-target-permission-recovery-R01.md`. Install and prove the exact callback bridge while the Capture seam remains dormant, using only named causal focused tasks and no all-runnable-pack run. Preserve candidate `1cf347a22c` and its slow-success repair as the immutable product remainder, then automatically resume stable task `live-target-permission-recovery` from the preparation's exact QA head. |
