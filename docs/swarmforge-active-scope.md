...
| Flow instance schema-editor scrolling correction | Active by user approval on 2026-08-18 | Implement directional Flow scenario 047 and its runtime partner under `docs/data-layer-canvas-first-flow-workspace-program-R02.md` as task `flow-instance-schema-editor-scrolling`; make the Flow-launched Page-instance and Event-occurrence schema editor vertically scrollable with the bounded `flow_graph` and `layered_schema` pack forecast, exact changed-path planning, and no all-20 gate. |
