...
| QA verification granularity ratchet | Existing slices and dispositions are QA-integrated through the Flow schema-editor route seam `4daea21df0`; judgment-based deferral and a pre-promotion observation portfolio were approved on 2026-08-18 | Implement durable observations and explicit portfolio intake while preserving exact-pack closure, existing slices, conservative feature evidence, the mandatory all-pack ownership stop, and one final all-20 master gate. |
| SwarmForge outcome-bounded autonomy, priority unblockers, and stacked campsite ratchet | Core autonomy and campsite mechanics QA-integrated at `8f82a6a66f`; judgment-based campsite deferral approved on 2026-08-18 | Preserve semantic outcome boundaries, immutable authority validation, nested queue recovery, and conserved automatic resumption; add judgment outcomes and portfolio lifecycle without reopening product behavior. |
