...
| Data Layer Live target permission recovery | Product paused before implementation; standing-authorized ownership preparation starts from exact QA `e54f411a0f`; the user rejected the resulting mechanical all-20/889-task plan and approved a fail-closed causal focused bootstrap on 2026-08-24 | Under `docs/data-layer-live-target-permission-recovery-R01.md`, task `verification-slice-live-target-permission-recovery` must prove the dormant Capture-owned seam through its exact installed Shell consumer. Evidence is restricted to the declared QA-necessary tasks inside the causal `capture`, `event-library`, `schemas`, `defects`, and `shell` boundary; the broad side-panel and Capture-facade paths cannot pull unrelated pack arrays into feature verification. Any diff or observer drift invalidates the bootstrap instead of widening it. Keep product scenarios 009 and runtime 002 unchanged and red, then automatically reissue `live-target-permission-recovery` from the exact preparation QA head. |
