...
| Flow nested-Event thumbnail containment correction | User-approved for coder handoff on 2026-08-22 | Add directional Flow scenario 051 and its runtime partner under `docs/flow-concept-visual-asset-storage-portability-R01.md` as task `flow-nested-event-thumbnail-containment`; expand a visual-bearing nested Event below its ordinary content, expand its Page frame with existing child padding, derive anchors and canvas bounds from the enlarged frame, preserve compact mode and stored state, and forecast `flow_graph`, `flow_export`, `live_flow_testing`, and `property_set_flow_sections` with no all-runnable-pack gate. |
