...
| Flow nested-Event thumbnail containment correction | Evidence-contract candidate `565e217111` passes direct proof but its governed Shell review is blocked by stale public-path expectations in the unrelated layered Page-group installed consumer; repair candidate `da0a0f55ce` is exact and review-ready but cannot fast-forward after QA advanced independently to `938e6d9fa3` | Reconstruct standing-authorized `verification-repair-layered-page-group-display-paths` from exact current QA under `docs/data-layer-flow-table-documentation-export-program-R01.md`; align only the rendered Documentation matrix expectations with QA-integrated public root names, preserve canonical slash-path identity and every other leaf, obtain fresh exact-candidate review evidence, then automatically reissue `verification-slice-flow-nested-event-thumbnail-containment` from the exact repair QA head before resuming the conserved product remainder. |
...
| Layered schema inherited exclusion and Example reconciliation | Specification proposed from user-reported defects on 2026-08-23; awaiting handoff approval | Under `docs/data-layer-layered-schema-editing-repairs-R01.md`, let a specific contributor exclude one ordinary inherited effective property without mutating any parent, and let Allowed values edits reconcile a selected allowed-value Example rather than fail because of edit order while retaining custom examples and their warning contract. |
