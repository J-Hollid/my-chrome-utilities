...
| Flow nested-Event thumbnail containment correction | Independent Page-group display-path repair is QA-integrated at `522836f757`; runtime051 evidence-contract slice is QA-integrated at `f831dfc239` after exact `flow_graph`/`shell` review evidence; stopped product references `c5cacaf242` and `698d68d2` remain patch references | Reissue stable task `flow-nested-event-thumbnail-containment` from exact current QA, replay only the conserved product remainder, and obtain fresh exact product evidence without repeating either integrated verification repair. |
...
| Flow Documentation template instance content | User-approved for coder handoff on 2026-08-23; clean candidate `31b512c7d6` and its incident evidence are preserved, and the blocking Page-group display-path repair is QA-integrated at `522836f757` | Reissue stable task `flow-template-instance-content` from exact repaired QA and reconstruct the conserved product delta with fresh bound evidence. |
| Layered schema inherited exclusion and Example reconciliation | User-approved and handed to the coder on 2026-08-23; rejected candidate `51aa431eb1` is a patch reference, and prerequisite Page-group display-path repair is QA-integrated at `522836f757` | Reissue stable task `layered-schema-exclusion-example-repair` from exact repaired QA and close the recorded dependency-safety, durable runtime, contained-occurrence, Undo, and property-coverage gates without changing the independent repair. |
