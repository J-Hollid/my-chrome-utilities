...
| Verification contract-conservation transitions | Candidate `e4d7312be6` is rejected and stopped: ancestry-only authority, self-referential baseline validation, and free-form successor mappings leave three fail-open paths | Correct `verification-contract-conservation-transition-repair` from the newest specification QA descendant. Bind the baseline to original blob `a62bde42ab` and each transition to its exact historical Scenario 221 row; reject fake path/scenario, swapped rows, baseline mutation or reorder, and deletion-to-unrelated-successor mappings. Preserve every parked remainder and require fresh focused review evidence. |
