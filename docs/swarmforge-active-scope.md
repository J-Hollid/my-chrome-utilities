...
| Verification task-checkpoint incident repair | Candidate `ca1a21a6` implements the placement and campsite gates and quarantines `9273c9c903`, but its evidence task constructed undeclared production child plans of 6/441 and terminal 21/935 | Implement nested task `bounded-evidence-child-plan-containment` under `docs/verification-task-checkpoint-incident-repair-R01.md`. Reject every production runner invoked inside a registered parent task, replace production-runner contract probes with pure or isolated synthetic planning, retain `ca1a21a6` behavior, and run fresh bounded evidence only. |
