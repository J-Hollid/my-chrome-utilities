...
| Flow nested-Event thumbnail containment correction | Evidence-contract candidate `565e217111` passes direct proof but its governed Shell review is blocked by stale public-path expectations in the unrelated layered Page-group installed consumer; repair candidate `912437c5ec` is exact and review-ready but cannot fast-forward after QA advanced independently to `d13caa8402`; layered-schema candidate `51aa431eb1` encountered the same defect but its permissive normalization was rejected | Treat the standing repair as subsumed only when an exact current-QA candidate asserts the rendered no-leading-slash Documentation names, preserves every canonical slash-path identity, and receives fresh bound `layered_schema` evidence; otherwise reconstruct `verification-repair-layered-page-group-display-paths`, then automatically reissue `verification-slice-flow-nested-event-thumbnail-containment` from the exact repair QA head before resuming the conserved product remainder. |
...
| Flow Documentation template instance content | User-approved for coder handoff on 2026-08-23; clean candidate `31b512c7d6` passes direct product checks but its exact review is blocked by the same inherited Page-group display-path consumer | Preserve the product delta and incident evidence; after the exact display-path repair or an exact subsuming QA candidate is integrated, automatically reissue stable task `flow-template-instance-content` from that QA head with fresh bound evidence. |
| Layered schema inherited exclusion and Example reconciliation | User-approved and handed to the coder on 2026-08-23; candidate `51aa431eb1` was rejected in review for dependency safety, durable runtime, Undo, property-coverage, and public display-path evidence gaps | Reissue stable task `layered-schema-exclusion-example-repair` from exact corrected QA under `docs/data-layer-layered-schema-editing-repairs-R01.md`; preserve the stopped product delta only as a patch reference, close every named review gate, and subsume the separate display-path repair only with its exact no-leading-slash assertions and fresh evidence. |
