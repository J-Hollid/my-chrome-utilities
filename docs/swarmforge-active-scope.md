...
| Data Layer Live target permission recovery | Ownership preparation QA-integrated at `9f8a8b037b` after exact five-owner/nine-task focused evidence; no property task, unrelated pack array, or all-runnable-pack checkpoint ran | Automatically resume stable task `live-target-permission-recovery` from the exact QA descendant under `docs/data-layer-live-target-permission-recovery-R01.md`. Implement product scenario 009 and installed runtime scenario 002 through the new Capture-owned seam and bounded adapters, preserving valid active-tab access, exact-origin permission, same-tab path recheck, target/session identity, and the focused feature-mode verification boundary. A new broad side-panel composition change requires fresh causal scope classification rather than automatic widening. |
