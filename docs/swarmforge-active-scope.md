...
| Project Documentation workspace UX correction | Active by user approval on 2026-08-16 | Replace the stacked workspace with the approved Build, Preview, and Export modes under `docs/data-layer-project-documentation-workspace-program-R01.md` and its product/runtime feature pair; use the focused `flow_export` boundary. This does not resume an older rejected Project Documentation implementation lineage. |
