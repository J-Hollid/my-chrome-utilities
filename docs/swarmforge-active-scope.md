...
| Tealium Live automatic metadata | User-approved on 2026-09-10 for focused QA implementation after connection recovery | Implement stable task `tealium-live-metadata` under `docs/tealium-live-metadata-R01.md`, the appended detection/Live product and runtime scenarios, and `docs/tealium-live-metadata-verification-R01.md`. Read identity locally; try exact-version public names automatically; retain local/UID fallback on failure. No login or remote execution. Forecast 45 Tealium/Shell checks plus package; run coder intent and exact preflight. No shared host, manifest, or registry change is planned. |
