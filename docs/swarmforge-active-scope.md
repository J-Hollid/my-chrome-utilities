...
| QA verification pack taxonomy evolution | Bounded focused evidence reaffirmed by the user on 2026-08-21; the mechanical genuinely-global replay is disposed by a no-touch boundary | Reissue `registry-derived-verification-packs` from the exact correction commit. Keep `scripts/verification-reliability-values.mjs` unchanged, prove executable synthetic registries and the specification-bound focused route, and do not request the same acceptance again while that scope is conserved. |
