...
| Verification run-intent and deferred-incident corrections | QA-integrated through monotonic acceptance-session repair-preflight correction `03e4157b83` | Preserve the fail-closed distinction between preflight-only monotonic expansion and explicit task succession; inherited deferred incidents remain terminal obligations. |
...
| Documentation template library | Approved product ready to resume after preflight correction `03e4157b83`; clean candidate `00f3e45d` remains a patch reference with a bounded 10-pack/247-task plan | Reissue stable task `documentation-templates` from the scorecard commit after `03e4157b83`, use `00f3e45d` only as a patch reference, and require governed incident repair plus fresh exact evidence. |
...
`03e4157b83` now recognizes only verified monotonic expansion during repair
preflight and leaves direct succession and incident state unchanged. Candidate
`00f3e45d`, `e8e5fd48`, and `7f9c8a1121` remain patch references only. Stable
product task `documentation-templates` resumes automatically from the scorecard
commit immediately after `03e4157b83`. No all-20 feature checkpoint is
authorized.
