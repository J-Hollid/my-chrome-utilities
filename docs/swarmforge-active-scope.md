...
| Layered schema inherited exclusion and Example reconciliation | User-approved and handed to the coder on 2026-08-23; candidate `51aa431eb1` was rejected in review for dependency safety, durable runtime, Undo, property-coverage, and public display-path evidence gaps | Preserve the stopped product delta only as a patch reference; integrate independent task `verification-repair-layered-page-group-display-paths` first, then automatically reissue stable task `layered-schema-exclusion-example-repair` from that exact QA head and close every remaining named review gate. |
