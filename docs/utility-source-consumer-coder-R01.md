# Exact utility source-consumer correction

Task: `ownership-prep-tealium-live`. Specification and independently integrated
implementation base: `d5c876f4000994a8455d51b7e5242899c2e40d0c`.
This is the successor required by `utility-source-consumer-follow-up-R01.md`.
It preserves the intermediate declaration candidate and its separate evidence.
No product Tealium behavior is activated by this tooling and test correction.

## Implemented boundaries

The source consumer now selects
`browser:test/project-observation-source-host-browser-test.mjs`. It uses the
existing packaged Probe fixture and browser target session. The source seed
uses real project configuration and storage. The installed host and source
controller run against the existing controlled Chrome adapter and source arrays.
The observation waits for host readiness, source readiness, Probe readiness,
and each controlled event. It checks eight unique events from two sources while
Probe is visible and after return. It checks source labels, no lost or duplicate
rows, target 771, stable capture and utility sessions, unchanged channel keys
and objects, unchanged hook attach counts, and retained array wrappers. Target
closure must detach the subscriptions and must not silently select another tab.

It does not call the old broad source setup observer, which also tests filters.
The original 14-group source runner remains byte-identical under its source
...
Schema source checks still execute through the original full task.

The independent conservation check compares the extracted assertion blocks and
the complete remaining task with the immutable QA base. It also checks that
the full source browser program and group runner remain unchanged. The selection
...
real dependency edges. These are task times, not paired elapsed overhead.
...
What failed: earlier host selection used broad tests whose task names concealed
unrelated work. Refine the process by checking selected assertions and receipt
durations before accepting a utility host route.

This correction is not final Tealium runtime proof, QA integration, or master
