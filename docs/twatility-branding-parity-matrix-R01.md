...
| Focused property facets | O-CANON | property menu and one mounted Definition/Rules/Structure section | focused property editors | Preserve implementation | No competing Presence/Values/Conditions first layer; Cancel/Escape discards stage | Style existing layered overlay; do not restore stale mock tabs | V2 | T-CANON | C-CANON | preserved and verified — Slice 5 |
| Rules & conditions | O-CONDITION | stable rule rows; Then; optional When; shared nested condition tree | mock Rules and conditional disclosure | Structural improvement | Rule defaults Always; required Then; Add condition/group; typed All/Any/Not; stable IDs; one Undo | Unify clearer row hierarchy around authoritative shared renderer | V2 | T-CANON | C-CANON | implemented and verified — Slice 5 |
| Structure operations | O-CANON | children/cardinality/order/type/duplicate/remove | Structure editor | Branding/layout parity | Tree-derived paths, stable identities, impact review and Undo | Style rows and controls; preserve renderer | V1 | T-CANON | C-CANON | implemented and verified — Slice 5 |
| Ownership & provenance | O-CANON, O-COMPOSED | View/Edit/Remove local/Reset/Open source/Replace/conflict actions | ownership and provenance | Preserve implementation | Action legality by ownership; inherited bytes immutable; sparse reset | Status/provenance treatment only | V1 | T-CANON | C-CANON | preserved and verified — Slice 5 |
...
| Reusable rule editor | O-CANON, O-CONDITION | kind-first fields; Then/When; severity/message; save | reusable editor | Preserve implementation | Only applicable fields mount; invalid save has no command/write/Undo | Style production editor; omit mock generic expected-value model | V2 | T-CANON | C-CANON | preserved and verified — Slice 5 |
...
| Page Group membership | O-COMPOSED, O-STUDIO | searchable add; accessible reorder/remove; impact; focus | membership stack | Preserve implementation | Ordered general-to-specific list; lane placement separate; blocks in-use removal | Style current editor and provenance | V2 | T-CANON, T-STUDIO | C-STUDIO, C-CANON | preserved and verified — Slice 5 |

## Canonical, composed, Flow, and export workspaces

| Surface or workflow | Production owner | Production controls | Mock destination | Classification | Preserve contract | Proposed migration | Risk tier | Verification | Master collision risk | Decision and status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Canonical draft header | O-CANON, O-DURABLE | contributor/source/Draft token/provenance; Undo/Redo | canonical header | Branding/layout parity | Opaque token not operator revision; stable identity; page-memory history | Branded context/status bar | V1 | T-CANON | C-CANON | implemented and verified — Slice 5 |
| Property table/tree | O-CANON, O-COMPOSED | all properties; inline Description/Allowed values/Example; local overflow; tree | property table/tree | Branding/layout parity | Same canonical model; all effective rows; datasets/labels and editable-cell DOM order; one vertical owner at 360px | Responsive table/compact-row presentation that does not replace or reorder direct-edit cells | V2 | T-CANON | C-CANON | implemented and verified — Slice 5 |
| Quick Table cell transactions | O-CANON, O-COMPOSED | Enter; Tab; Shift+Tab; blur; Escape; inline diagnostic | no mock authority | Preserve implementation | One property command/Undo only on changed valid commit; unchanged no-op; invalid refocus; no duplicate blur; sparse inherited override; parent propagation; focus survives rerender | Style visible editable cells and diagnostics only; preserve `data-inline-schema-facet/path`, scope, and traversal order | V2 | T-CANON, T-A11Y | C-CANON | preserved and verified — Slice 5 |
| Property actions menu | O-CANON | Definition/Rules/Structure; source/override/reset/remove | property menu | Preserve implementation | One focused menu, ownership legality, exact origin focus | Restyle current menu/layers; omit stale extra first-level tabs | V2 | T-CANON | C-CANON | preserved and verified — Slice 5 |
| Definition editing | O-CANON | type; presence; comma-separated Allowed values; docs; example method/value | Definition layer | Structural improvement | Blank/Allowed value/Custom mount rules; typed validation; one command/Undo | Clearer progressive disclosure inside authoritative Definition layer | V2 | T-CANON | C-CANON | implemented and verified — Slice 5 |
| Rule inventory | O-CANON, O-CONDITION | stable rows; When/Then/severity/message/source/ownership; named actions | Rules layer | Structural improvement | Stable identity and legal View/Edit/Remove/Replace/Open source actions | Apply compact hierarchy and summaries without flattening | V2 | T-CANON | C-CANON | implemented and verified — Slice 5 |
| Add/edit rule | O-CONDITION | kind-first; required Then; optional Add When; review/save/cancel | mock rule builder | Structural improvement | Always default; invalid no-op; nested typed tree; focus restoration | Shared production builder with clearer row/layer presentation | V2 | T-CANON | C-CANON | implemented and verified — Slice 5 |
| Allowed values | O-CANON | zero/one/many typed comma-separated values | mock Allowed values | Structural improvement | No expected-vs-allowed distinction; legacy exact projects as one without mutation | Align all mounts with current Definition presentation | V2 | T-CANON | C-CANON | implemented and verified — Slice 5 |
| Structure editor | O-CANON | container/item/cardinality/unique/undeclared child/reorder | Structure layer | Branding/layout parity | Stable tree identity, valid structural commands and Undo | Branded structured rows | V1 | T-CANON | C-CANON | implemented and verified — Slice 5 |
| Composed contributor rows | O-COMPOSED | inherited/local/effective/shadowed/conflict/provenance; override/reset | composed workspace | Preserve implementation | Sparse facet ownership, live parent updates, conflict blocking | Style status/provenance and row hierarchy; keep production model | V2 | T-CANON | C-CANON | preserved and verified — Slice 5 |
...
| Canonical search IME/caret | O-CANON | same connected search input through typing/composition/clear | not represented in mock | Preserve implementation | No canonical command, persistence write, token or Undo | Styling must not remount or steal focus | V2 | T-CANON, T-A11Y | C-CANON | preserved and verified — Slice 5 |
| Nested overlay edge geometry | O-CONDITION | complete active layer at 360px; minimal nearest scroll; layer focus return | not represented in mock | Structural improvement | Every control remains in viewport; Escape closes one layer | Apply viewport-safe shared overlay presentation | V2 | T-CANON, T-A11Y | C-CANON | implemented and verified — Slice 5 |
...
## Slice 5 rules and conditions completion

Slice 5 completed the shared rules and condition-tree migration against finite
master cutoff `7edae41131a4e6a282d80f67a2fbcfbada52beb3`:

- canonical and composed focused condition mounts plus the compact side-panel
  editor now delegate to one production `CanonicalPredicate` presentation;
- Studio applicability uses the same nested visual grammar through an explicit
  adapter that preserves its distinct durable `Condition` AST exactly;
- All, Any, and Not groups, Add condition, Add group, typed predicate fields,
  stable DOM paths, rule Then/optional When, ownership, and legal rule actions
  remain driven by production models and commands;
- a scoped `schema-authoring-brand.css` layer presents canonical headers,
  property navigation, Definition/Rules/Structure layers, rule inventories,
  nested condition trees, composed rows, compact mounts, responsive nesting,
  focus, forced colours, and viewport containment;
- project condition Save remains one durable command, Undo restores the exact
  nested tree, reload preserves the restored project bytes, and Published
  revision remains unchanged;
- unit/property coverage owns both canonical and project condition renderers,
  while the registered packaged-Chrome adapter owns canonical, Studio, compact,
  1280/360 responsive, control-equivalence, accessibility, persistence, and
  runtime evidence; and
- the exact `layered_schema` acceptance pack and package command pass from
  generated source output. The first pack run exposed and led to restoration of
  the existing public `ensureComposedConditionIds` export; the complete rerun
  passed with no remaining failure.

The opening and closing master fetches found no upstream delta, so no master
merge or conflict resolution was required. Master was never checked out,
modified, committed to, merged into, or pushed. The remaining workflow/state
