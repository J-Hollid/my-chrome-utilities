...
(defn- assert-example! [example observed]
  (when-let [rule-type (support/example-value example "rule_type")]
    ...
    (when-let [comparison (support/example-value example "comparison")]
      (when-let [below (support/example-value example "below_limit_outcome")]
        (let [actual (mapv #(get-in observed [:outcomes (keyword (str rule-type ":" comparison ":" %)) :outcome]) [49 50 51])]
      ...
      (when-let [limit (and (support/example-value example "expected_constraint")
        ...
        (let [operator (if (= rule-type "Text length") "text-length" "item-count")]
          (support/assert! (some #(= {:operator operator :comparison comparison :limit (parse-long limit)} %)
                                 (:reopened observed))
          ...
          (support/assert! (= (support/require-example example "expected_constraint")
                              (get-in observed [:reopenedIssues (keyword (if (= rule-type "Text length") "/title" "/items")) :expected]))
    ...
    (when-let [legacy (support/example-value example "legacy_behavior")]
      (let [configuration (get-in observed [:legacy (if (= rule-type "Text length") :text :items)])]
