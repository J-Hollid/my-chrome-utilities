...
(defn- assert-runtime! [observed]
  (support/assert! (and (get-in observed [:created :empty]) (get-in observed [:created :workspace]) (= "Saved" (get-in observed [:created :status])) (str/includes? (get-in observed [:created :context]) "Production") (= 8 (count (get-in observed [:created :tree])))) "The actual project workspace did not create and render a durable blank project." observed)
