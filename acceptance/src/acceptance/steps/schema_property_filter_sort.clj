...
(defn- transition [world example _captures {:keys [text]}]
  (let [world (if (entry-steps text)
                (assoc world :schema-property-filter-sort (browser-observation!))
                world)
        observed (:schema-property-filter-sort world)]
    (support/assert! observed "Schema property filter/sort adapter was not executed." {:step text})
