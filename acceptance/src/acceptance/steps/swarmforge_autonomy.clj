...
(def step-patterns
  [#"^<mode_prerequisite>$"
   ...
   #"^(?:an immutable legacy campsite manifest predates its prerequisite task field and remains with the parked product role|the reviewed nested prerequisite has exact evidence and an architect-to-specifier QA-ready handoff in the common project authority|satisfaction is recorded after that implementation reaches the exact QA head|an append-only compatibility binding identifies the manifest digest, prerequisite task, and evidence base without rewriting the manifest|the evidence base descends from the manifest authority and is the exact reviewed specification ancestor of the implementation|the recorder validates the routed handoff and evidence across role worktrees before persisting satisfaction beside the parked product|the QA trigger starts from the original remainder and conserves the quarantined generation's task, commits, causal paths, and complete delta|a missing, ambiguous, superseded, mismatched, or role-local-only binding fails closed before resumption)$"
   #"^(?:regular development has one fixed repair family and discovery boundary|one bounded discovery pass lists every currently observed defect in that boundary|a verification-repair unblocker is validated|it requires one stable task, the repair family, the fixed boundary, a non-empty unique defect list, and a complete-discovery declaration|missing or incomplete structured repair fields are rejected before queue mutation|ordinary unblockers remain compatible without repair fields)$"
   #"^(?:one valid repair authorization is active|development encounters .+|the process takes .+|no finding silently enlarges the authorized product or safety outcome)$"
   #"^(?:one continuous repair addresses the initial and added in-boundary defects|the coherent candidate is handed to review|one final defect list identifies every repaired in-boundary finding|the smallest focused evidence observes the changed behavior and that final list|a known omission blocks review without creating another task, unblocker, census generation, or diagnostic architecture)$"
   #"^(?:defect consolidation is a three-task process-impact pilot|existing handoff, receipt, and Git timestamps are reviewed|they report discovery returns, repair authorizations, implementation passes, focused reruns, and process-versus-product time|no telemetry implementation or verification-platform expansion is added|lack of reduced stop-start cycles leads to simplification or removal instead of another hardening cycle)$"])
