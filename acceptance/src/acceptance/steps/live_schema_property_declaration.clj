...
(defn- assert-runtime! [{:keys [actions reviewCases saved validation separate published reloaded] :as observed}]
  ...
  (support/assert! (and (str/includes? (get-in reviewCases [:productName :text]) "/products/*/product_name")
                        ...
                        (every? #(and (:noValidationControls %) (:storageUnchanged %) (:guidedVisible %)
                                      (:nonzeroBounds %) (:inputAccepted %)) (vals reviewCases)))
                   "Declaration review was hidden, unusable, persisted early, crossed into validation configuration, or skipped a detected type." reviewCases)
