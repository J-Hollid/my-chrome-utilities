  ...
  (vec
   (concat
    ...
    (support/verified-feature-mode-handlers
     ["features/data-layer-page-group-structural-authoring.feature"]
     entry-modes :layered-schema-mode verify-model! validate-example!
     #(observe-target! page-group-observation "LAYERED_SCHEMA_PAGE_GROUP_TARGET" :layeredSchemaPageGroup)
     #(support/assert! (focused-evidence-valid? %) "Page Group browser evidence is incomplete." %)))))
