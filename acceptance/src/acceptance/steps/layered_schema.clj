...
(defn- assert-runtime! [evidence] (support/assert! (and (= #{:installedBoundary :humanSummary :completeEditor :structuredControl :saved :consequential :persistenceReload} (set (keys evidence))) (every? true? (vals evidence))) "Installed layered-schema evidence is incomplete." evidence))
