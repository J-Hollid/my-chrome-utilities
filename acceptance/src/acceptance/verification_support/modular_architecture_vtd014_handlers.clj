...
(defn- diagnostic-execution-handlers [example-values]
  [
   {:pattern #"^it executes (TARGET-A only|that case only|the setup boundary only and no target workflow|that exact task|no retry until the progress contract is repaired)$"
    :handler (fn [world example captures]
