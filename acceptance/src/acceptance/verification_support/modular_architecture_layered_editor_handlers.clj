...
(defn- multi-change-handlers [example-values verify-throughput!]
  [
   ...
   {:pattern #"^all 20 Layered Schema unit files, 13 property files, and the exact shared-profile feature and handler evidence remain selected$"
    :handler (fn [world _ _]
               (let [plan (:vtd005/plan world)]
                 (assert-vtd005! world (and (= [20 13] [(:unit plan) (:property plan)])
...
(defn- conservation-handlers [verify-throughput!]
  [
   ...
   {:pattern #"^the 53-task exact owner plan retains one build, 20 unit tasks, 13 property tasks, four browser sessions containing all eight logical targets, seven parses, seven generators, and one acceptance session$"
    :handler (fn [world _ _]
               (let [c (get-in world [:vtd005/evidence :conservation])]
                 (assert-vtd005! world (= [53 1 20 13 4 8 7 7 1]
