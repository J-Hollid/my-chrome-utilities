...
const project={
  ...
  documentationFlowGraphs:{
    "flow:checkout":{
      ...
      relationships:[
        {id:"relationship:legacy-cart-view",sourceEndpoint:{kind:"page-frame",id:"frame:cart"},targetEndpoint:{kind:"event-occurrence",id:"occurrence:view"},sourcePort:"right",targetPort:"left",kind:"expected_next"},
...
assert.deepEqual(
  ...
  [
    {id:"occurrence:view",kind:undefined,displayName:"Cart Cart page_view"},
    {id:"frame:confirmation-a",kind:"expected_next",displayName:"Success route"},
  ],
  "another unlinked event offers each contained interaction once, followed by outgoing related Pages",
);
assert.equal(new Set(liveFlowEventStepChoices(run,state,"live-102").choices.map(({id})=>id)).size,2);
...
cyclicState.project.documentationFlowGraphs["flow:checkout"].relationships=[
  ...cyclicState.project.documentationFlowGraphs["flow:checkout"].relationships,
  {id:"relationship:confirm-retry",sourceEndpoint:{kind:"page-frame",id:"frame:confirmation-a"},targetEndpoint:{kind:"page-frame",id:"frame:cart"},sourcePort:"top",targetPort:"bottom",kind:"alternative",label:"Retry cart"},
];
...
cyclicRun=linkLiveFlowEvent(cyclicRun,cyclicState,cyclicEvents[0],"frame:cart");
cyclicRun=linkLiveFlowEvent(cyclicRun,cyclicState,cyclicEvents.at(-1),"occurrence:view");
assert.deepEqual(cyclicRun.history.map(({stepId})=>stepId),["frame:cart","occurrence:view","frame:confirmation-a","frame:cart","occurrence:view"]);
assert.deepEqual(cyclicRun.history.at(-1).matchedPath.map(({stepId})=>stepId),["frame:cart","occurrence:view","frame:confirmation-a","frame:cart","occurrence:view"]);
