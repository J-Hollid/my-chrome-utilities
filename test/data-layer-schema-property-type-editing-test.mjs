...
import { applySchemaPropertyTypeEdit, inspectSchemaPropertyTypeEdit, schemaPropertyTypeImpactCanReplace, schemaPropertyTypeLabel, schemaPropertyTypeOwner } from "../dist/data-layer-schema-property-type-editing.js";
...
assert.deepEqual(inspectSchemaPropertyTypeEdit(schema,"/order_id","string"),{from:"Number",to:"String",compatible:["required membership","documentation","type mismatch treatment"],incompatible:["example value","rule range"]});
...
const warningItems=applySchemaPropertyTypeEdit(schema,{path:"/tags",type:"array",itemType:"string",treatment:"warning",removeIncompatible:false});
const itemIssues=validateWithSchema({sourceId:"history",eventName:"page",payload:{order_id:1,price:1,tags:["ok",2],products:[]}},warningItems,[warningItems]).issues;
assert.deepEqual(itemIssues.filter(({message})=>message==="Type mismatch").map(({instancePath,severity})=>[instancePath,severity]),[["/tags/1","warning"]]);
const nonArrayIssues=validateWithSchema({sourceId:"history",eventName:"page",payload:{order_id:1,price:1,tags:"bad",products:[]}},warningItems,[warningItems]).issues;
assert.deepEqual(nonArrayIssues.filter(({message})=>message==="Type mismatch").map(({instancePath,severity})=>[instancePath,severity]),[["/tags","warning"]]);
const ignoredItems=applySchemaPropertyTypeEdit(warningItems,{path:"/tags",type:"array",itemType:"string",treatment:"ignore",removeIncompatible:false});
assert.equal(validateWithSchema({sourceId:"history",eventName:"page",payload:{order_id:1,price:1,tags:["ok",2],products:[]}},ignoredItems,[ignoredItems]).issues.some(({message})=>message==="Type mismatch"),false);

const requiredPrice={...schema,attachedRules:[...(schema.attachedRules??[]),{id:"required-price",version:1,propertyPath:"/price",operator:"required",severity:"error"}]};
const ignoredRequiredPrice=applySchemaPropertyTypeEdit(requiredPrice,{path:"/price",type:"number",treatment:"ignore",removeIncompatible:false});
assert.equal(validateWithSchema({sourceId:"history",eventName:"page",payload:{order_id:1,tags:[],products:[]}},ignoredRequiredPrice,[ignoredRequiredPrice]).issues.some(({instancePath,message,severity})=>instancePath==="/price"&&message==="Required value"&&severity==="error"),true);
...
assert.deepEqual(inspectSchemaPropertyTypeEdit(productsSchema,"/products","array"),{from:"Array of Object",to:"Array",compatible:["type mismatch treatment"],incompatible:["descendant definitions","descendant required relationships","descendant documentation","descendant rules"]});
...
assert.equal(schemaPropertyTypeImpactCanReplace("conditional dependency order-condition","object"),false);
assert.throws(()=>applySchemaPropertyTypeEdit(dependent,{path:"/order_id",type:"object",treatment:"warning",removeIncompatible:false,resolutions:{"example value":{action:"remove"},"rule range":{action:"remove"},"conditional dependency order-condition":{action:"replace",value:"{}"}}}),/must be removed/);
...
const escaped={...schema,document:{type:"object",required:["a/b"],properties:{"a/b":{type:"number"}}},documentation:{properties:{"/a~1b":{displayName:"a/b",description:"Escaped",example:{value:1,selectionMethod:"custom"}}}},attachedRules:[]};
assert.deepEqual(inspectSchemaPropertyTypeEdit(escaped,"/a~1b","string").compatible,["required membership","documentation","type mismatch treatment"]);
assert.equal(applySchemaPropertyTypeEdit(escaped,{path:"/a~1b",type:"string",treatment:"error",removeIncompatible:true}).document.properties["a/b"].type,"string");
const base={...schema,id:"base",name:"Base",document:{type:"object",properties:{site_id:{type:"string"}}}};const middle={...schema,id:"middle",name:"Middle",parentSchemaId:"base",document:{type:"object",properties:{}}};const child={...schema,id:"child",name:"Child",parentSchemaId:"middle",document:{type:"object",properties:{}}};
assert.equal(schemaPropertyTypeOwner(child,"/site_id",[child,middle,base])?.name,"Base");
