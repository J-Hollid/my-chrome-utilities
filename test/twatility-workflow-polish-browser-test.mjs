...
try{
  ...
  await evaluate(studio,`(async()=>{const {openIndexedDbProjectRepository}=await import("./data-layer-durable-project-repository.js"),repo=await openIndexedDbProjectRepository(),root=()=>document.querySelector('[aria-label="Project Documentation workspace"]'),section=(kind)=>root()?.querySelector('[aria-label="Documentation section outline"] [data-section-kind="'+kind+'"]'),required=[["flow","Checkout journey"],["matrix","Data capture matrix"],["profile","Commerce foundation"]];for(const [kind,label] of required){if(section(kind))continue;const input=[...root().querySelectorAll('[data-studio-choice-contract="documentation.section-membership"]')].find((choice)=>choice.labels?.[0]?.textContent.trim()===label),before=(await repo.loadProject(${JSON.stringify(projectId)})).draftSequence;if(!input)throw new Error("Missing Documentation section membership choice: "+label);input.click();let durable=false;for(let attempt=0;attempt<200&&(!section(kind)||!durable);attempt+=1){await new Promise((resolve)=>setTimeout(resolve,25));durable=(await repo.loadProject(${JSON.stringify(projectId)})).draftSequence>before;}if(!section(kind)||!durable)throw new Error("Documentation section did not restore durably: "+label);}})()`);
