...
try{
  ...
  await evaluate(studio,`(async()=>{const root=()=>document.querySelector('[aria-label="Project Documentation workspace"]'),required=[["flow","Checkout journey"],["matrix","Data capture matrix"],["profile","Commerce foundation"]];for(const [kind,label] of required){if(root().querySelector('[aria-label="Documentation section outline"] [data-section-kind="'+kind+'"]'))continue;const input=[...root().querySelectorAll('[data-studio-choice-contract="documentation.section-membership"]')].find((choice)=>choice.labels?.[0]?.textContent.trim()===label);if(!input)throw new Error("Missing Documentation section membership choice: "+label);input.click();await new Promise((resolve)=>setTimeout(resolve,0));}})()`);
