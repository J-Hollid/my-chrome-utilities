...
try{
  ...
  await evaluate(studio,`(async()=>{const {openIndexedDbProjectRepository}=await import("./data-layer-durable-project-repository.js"),repo=await openIndexedDbProjectRepository();let last=-1,stable=0;for(let attempt=0;attempt<200;attempt+=1){const current=(await repo.loadProject(${JSON.stringify(projectId)})).draftSequence;if(current===last)stable+=1;else{last=current;stable=0;}if(stable>=6)return current;await new Promise((resolve)=>setTimeout(resolve,50));}throw new Error("Documentation native choice audit did not settle.");})()`);
  await studio.call("Page.navigate",{url:`${base}data-layer-specification-project.js`});
  await ready(studio,"document.readyState==='complete'","Documentation choice audit reset document");
  await evaluate(studio,`(async()=>{const durable=await import("./data-layer-durable-project-repository.js"),repo=await durable.openIndexedDbProjectRepository(),loaded=await repo.loadProject(${JSON.stringify(projectId)}),state=structuredClone(loaded.state),set=state.project.documentation.sets[0],flow=state.project.collections.flows.find(({name})=>name==="Checkout journey"),profile=state.project.collections.profiles.find(({name})=>name==="Commerce foundation"),required=[{id:"section:overview",kind:"overview",name:"Overview"},{id:"section:flow",kind:"flow",name:"Checkout journey",targetId:flow.id},{id:"section:matrix",kind:"matrix",name:"Data capture matrix"},{id:"section:profile",kind:"profile",name:"Commerce foundation",targetId:profile.id}];for(const fixture of required){const section=set.sections.find(({kind})=>kind===fixture.kind);if(section)section.selected=true;else set.sections.push({...fixture,selected:true});}const saved=await repo.saveDraft(durable.durableDraftCommand(loaded,state,{commandId:"restore-documentation-choice-audit-sections",label:"Restore Documentation choice audit sections"}));if(saved.status==="conflict")throw new Error("Documentation section fixture restoration conflicted.");})()`);
  await studio.call("Page.navigate",{url:`${base}specification-builder.html?project=${projectId}&view=documentation`});
  await ready(studio,"document.querySelector('[aria-label=\"Project Documentation workspace\"]')","restored Documentation workspace");
  await evaluate(studio,`(async()=>{let root=document.querySelector('[aria-label="Project Documentation workspace"]');[...root.querySelectorAll("button")].find(({textContent})=>textContent==="Add content").click();await new Promise((resolve)=>setTimeout(resolve,0));root=document.querySelector('[aria-label="Project Documentation workspace"]');[...root.querySelectorAll("button")].find(({textContent})=>textContent==="Document settings").click();})()`);
  await ready(studio,"document.querySelector('[aria-label=\"Project Documentation workspace\"] [data-studio-choice-contract=\"documentation.concept-subheadings\"]')","restored Documentation choice panels");
  await evaluate(studio,`(()=>{const scope=document.querySelector('[aria-label="Project Documentation workspace"] [aria-label="Documentation export scope"]');scope.value="selected";scope.dispatchEvent(new Event("change",{bubbles:true}));})()`);
  let restoredDocumentationKinds=[];
  for(let attempt=0;attempt<320;attempt+=1){restoredDocumentationKinds=await evaluate(studio,`[...document.querySelectorAll('[aria-label="Documentation section outline"] [data-section-kind]')].filter((row)=>row.querySelector(':scope > [data-reorder-item-row="true"] > button:not([data-reorder-trigger="true"])')).map((row)=>row.dataset.sectionKind)`);if(["flow","matrix","profile"].every((kind)=>restoredDocumentationKinds.includes(kind)))break;await wait(25);}
  assert.equal(["flow","matrix","profile"].every((kind)=>restoredDocumentationKinds.includes(kind)),true,`Documentation sections restored after native choice audit: ${JSON.stringify(restoredDocumentationKinds)}`);
  const documentationConfigurationChoices=await evaluate(studio,`(async()=>{
    ...
    for(const kind of["flow","matrix","profile"]){let button;for(let attempt=0;attempt<200&&!button;attempt+=1){button=root()?.querySelector('[aria-label="Documentation section outline"] [data-section-kind="'+kind+'"] [data-reorder-item-row="true"] > button:not([data-reorder-trigger="true"])');if(!button)await new Promise((resolve)=>setTimeout(resolve,25));}if(!button)throw new Error("Missing Documentation configuration section: "+kind);button.click();await new Promise((resolve)=>setTimeout(resolve,50));if(kind==="profile"){root().querySelector("[data-profile-concept]")?.click();await new Promise((resolve)=>setTimeout(resolve,50));}collect();}
    let overview;for(let attempt=0;attempt<200&&!overview;attempt+=1){overview=root()?.querySelector('[aria-label="Documentation section outline"] [data-section-kind="overview"] [data-reorder-item-row="true"] > button:not([data-reorder-trigger="true"])');if(!overview)await new Promise((resolve)=>setTimeout(resolve,25));}if(!overview)throw new Error("Missing Documentation overview section after configuration audit");overview.click();
  ...
  for(const kind of["flow","matrix","profile"]){
    await evaluate(studio,`document.querySelector('[aria-label="Documentation section outline"] [data-section-kind="${kind}"] [data-reorder-item-row="true"] > button:not([data-reorder-trigger="true"])').click()`);
    await wait(40);
  ...
  await evaluate(studio,`document.querySelector('[aria-label="Documentation section outline"] [data-section-kind="overview"] [data-reorder-item-row="true"] > button:not([data-reorder-trigger="true"])').click()`);
  await ready(studio,"document.querySelector('[data-studio-choice-contract=\"documentation.concept-subheadings\"]')","overview Documentation choices");
  ...
  await evaluate(studio,`document.querySelector('[aria-label="Documentation section outline"] [data-section-kind="flow"] [data-reorder-item-row="true"] > button:not([data-reorder-trigger="true"])').click()`);
  ...
  await evaluate(studio,`document.querySelector('[aria-label="Documentation section outline"] [data-section-kind="matrix"] [data-reorder-item-row="true"] > button:not([data-reorder-trigger="true"])').click()`);
  ...
  await evaluate(studio,`document.querySelector('[aria-label="Documentation section outline"] [data-section-kind="profile"] [data-reorder-item-row="true"] > button:not([data-reorder-trigger="true"])').click()`);
  ...
  await evaluate(studio,`document.querySelector('[aria-label="Documentation section outline"] [data-section-kind="overview"] [data-reorder-item-row="true"] > button:not([data-reorder-trigger="true"])').click()`);
  await evaluate(studio,`document.querySelector('[aria-label="Project Documentation workspace"] button[aria-controls="documentation-theme-panel"]').click()`);
  await ready(studio,"document.querySelector('[aria-label=\"Project Documentation workspace\"] [data-theme-group=\"Table\"]')","reopened Documentation theme controls");
  ...
  const documentationConservation=await evaluate(studio,`(async()=>{
    ...
    document.querySelector("#documentation-tab-export").click();let nextRoot,exportChoice,copy;for(let attempt=0;attempt<200&&(!exportChoice||!copy);attempt+=1){nextRoot=document.querySelector('[aria-label="Project Documentation workspace"]');exportChoice=nextRoot?.querySelector('[data-studio-choice-contract="documentation.export-section"]');copy=[...nextRoot?.querySelectorAll("button")??[]].find(({textContent})=>textContent==="Copy rich documentation");if(!exportChoice||!copy)await new Promise((resolve)=>setTimeout(resolve,25));}if(!exportChoice||!copy)throw new Error("Documentation export controls did not remount");globalThis.__choiceClipboard={html:"",plain:""};Object.defineProperty(navigator,"clipboard",{configurable:true,value:{write:async(items)=>{globalThis.__choiceClipboard.html=await(await items[0].getType("text/html")).text();globalThis.__choiceClipboard.plain=await(await items[0].getType("text/plain")).text();},writeText:async(value)=>{globalThis.__choiceClipboard.plain=value;}}});
