...
try{
  ...
  const analystPools=await evaluate(studio,`(async()=>{const {studioAnalystHintsForRoute}=await import("./specification-studio-technical-analyst-guidance.js"),parts=["Project overview","Shared Profiles","Pages","Page Groups","Events","Applicability","Flows","Fixtures","Assignments","Documentation"],pools=Object.fromEntries(parts.map((part)=>{const tips=studioAnalystHintsForRoute(part);return[part,{count:tips.length,distinct:new Set(tips.map(({id})=>id)).size,texts:tips.map(({text})=>text)}];})),flow=Object.fromEntries(studioAnalystHintsForRoute("Flows").map(({id,text})=>[id,text])),semantics={canvas:/Add Pages to the canvas first.*place interaction Events inside them/u.test(flow.flows),frames:/Page frames.*journey step/u.test(flow["flows-frames"]),containment:/Event occurrences inside their owning Page frame/u.test(flow["flows-occurrences"]),pageRelationships:/Page-to-Page relationships/u.test(flow["flows-relationships"])&&/Page frames/u.test(flow["flows-relationships"]),occurrencesAreNotEndpoints:!/connect(?:ing)? (?:Event )?occurrences|occurrence(?:s)? (?:as|for|to) relationship endpoints?/iu.test(flow["flows-relationships"]),documentation:/Documentation.*Flow's value map/u.test(flow["flows-documentation"])};return{pools,semantics};})()`);
  assert.equal(Object.keys(analystPools.pools).length,10);
  assert.equal(Object.values(analystPools.pools).every(({count,distinct,texts})=>count>=5&&count===distinct&&texts.every((text)=>text.length>20)),true);
