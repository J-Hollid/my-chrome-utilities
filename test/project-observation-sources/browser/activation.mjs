...
export async function observeUnavailableSource(value,eventName='A1') {
  ...
  const inputValue=Object.hasOwn(event,'history')?String(event.history):'missing';
  await c.start();
  ...
  return {value,inputValue,event:eventName,status,ids:c.captured().map(event=>event.id),names:c.captured().map(event=>event.name),
