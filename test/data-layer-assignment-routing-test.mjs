...
const project={
  ...
  collections:{
    pages:[{id:"page:cart",name:"Cart",propertySetIds:["group:checkout"],profileInheritanceRecipes:[{id:"recipe:sitewide",profileId:"profile:sitewide",targetId:"page:cart"}],canonicalSchema:{id:"schema:cart"},localSchemaContributions:[{path:"/cart_id",type:"string"}]}],
...
assert.deepEqual(assignmentConditionControl({path:"/items",type:"array"}),{path:"/items",type:"array",comparisons:["exists","does not exist"],valueType:"array"});
assert.deepEqual(assignmentConditionControl({path:"/metadata",type:"object"}),{path:"/metadata",type:"object",comparisons:["exists","does not exist"],valueType:"object"});
assert.deepEqual(assignmentConditionControl({path:"/nothing",type:"null"}),{path:"/nothing",type:"null",comparisons:["exists","does not exist"],valueType:"null"});
...
let sequence=0;const state={project:{id:"project:routing",name:"Routing",description:"",site:"example.test",environments:["Production"],namingConventions:{},publicationPolicy:{warningsBlock:false,fixturesRequired:false},collections:{profiles:[],propertySets:[],pages:project.collections.pages,events:project.collections.events,applicabilitySets:project.collections.applicabilitySets,flows:[],fixtures:[],assignments:project.collections.assignments},releases:[]},draft:{id:"draft:routing",status:"Saved",updatedAt:"2026-08-01T00:00:00Z"},history:{undo:[],redo:[]}},originalCondition=structuredClone(state.project.collections.applicabilitySets[0].condition),{applicabilitySetId:_sharedSet,...assignmentInput}=state.project.collections.assignments[0],saved=saveProjectAssignment(state,{...assignmentInput,eventName:"page_view",condition:{kind:"predicate",field:"pathname",operator:"equals",value:"/new"}},(kind)=>`${kind}:new:${sequence++}`),updated=saved.project.collections.assignments[0];
...
const cleared=saveProjectAssignment(state,{...assignmentInput,eventName:"page_view",clearApplicability:true},(kind)=>`${kind}:clear:${sequence++}`);assert.equal(cleared.project.collections.assignments[0].applicabilitySetId,undefined,"an explicit no-condition choice clears the prior restriction");assert.deepEqual(cleared.project.collections.applicabilitySets,state.project.collections.applicabilitySets,"clearing an Assignment does not mutate its formerly shared set");
