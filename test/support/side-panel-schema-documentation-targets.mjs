...
const schemaSpecificationBuilderExtendedRuntime = `(async()=>{
  ...
  const rect=node=>{const value=node.getBoundingClientRect();return{left:value.left,top:value.top,right:value.right,bottom:value.bottom,width:value.width,height:value.height};},headingInventory=Array.from(table.querySelectorAll('th[data-specification-column]')).map(cell=>{const band=cell.querySelector('[data-reorder-item-row]'),trigger=q('[data-reorder-trigger="true"]',cell),primary=band?.children[1],labels=Array.from(cell.querySelectorAll('button'),({textContent})=>textContent.trim()),targetRect=rect(trigger),primaryRect=primary?rect(primary):null,rowRect=band?rect(band):null;return{id:cell.dataset.specificationColumn,label:primary?.textContent??'',accessibleName:trigger.getAttribute('aria-label'),triggerCount:cell.querySelectorAll('[data-reorder-trigger="true"]').length,paired:labels.some(label=>/Move (earlier|later|up|down|left|right)/.test(label)),rowGeometry:primaryRect&&rowRect?{consumer:'specification table columns',host:rect(cell),gripTarget:targetRect,primaryContent:primaryRect,row:rowRect,valid:Math.min(targetRect.bottom,primaryRect.bottom)>Math.max(targetRect.top,primaryRect.top)&&Math.abs(targetRect.width-44)<=1&&Math.abs(targetRect.height-44)<=1}:null};}),dragBefore=headings();const transfer=new DataTransfer(),mandatory=q('th[data-specification-column="mandatory"]',table),type=q('th[data-specification-column="type"]',table),mandatoryHandle=q('[data-reorder-trigger="true"]',mandatory);mandatoryHandle.dispatchEvent(new DragEvent("dragstart",{bubbles:true,dataTransfer:transfer}));type.dispatchEvent(new DragEvent("dragover",{bubbles:true,cancelable:true,dataTransfer:transfer,clientY:type.getBoundingClientRect().top}));const indicator={before:type.classList.contains('reorder-drop-before'),after:type.classList.contains('reorder-drop-after'),border:getComputedStyle(type).borderBlockStartWidth};type.dispatchEvent(new DragEvent("drop",{bubbles:true,dataTransfer:transfer,clientY:type.getBoundingClientRect().top}));const dragged=headings();const boundaryDisabled=(heading,action)=>{const trigger=q('[data-reorder-trigger="true"]',heading);trigger.click();const disabled=Array.from(trigger.parentElement.querySelectorAll("button")).find(({textContent})=>textContent===action).disabled;trigger.click();return disabled;},headingCells=Array.from(table.querySelectorAll("th[data-specification-column]")),boundaries={firstLeft:boundaryDisabled(headingCells[0],"Move one position earlier"),lastRight:boundaryDisabled(headingCells.at(-1),"Move one position later")};Array.from(builder.querySelectorAll("button")).find(({textContent})=>textContent==="Reset column order").click();const reset=headings(),reorderEvidence={inventoryIds:headingInventory.map(({id})=>id).join('|'),inventoryLabels:headingInventory.map(({label})=>label).join('|'),inventoryAccessibleNames:headingInventory.map(({accessibleName})=>accessibleName).join('|'),inventoryTriggerCounts:headingInventory.map(({triggerCount})=>triggerCount).join('|'),inventoryLegacyPairs:headingInventory.map(({paired})=>paired).join('|'),rowGeometry:headingInventory.map(({rowGeometry})=>rowGeometry),beforeOrder:dragBefore.join('|'),indicator,movedOrder:dragged.join('|'),resetOrder:reset.join('|'),stableId:transfer.getData('application/x-reorderable-editor-item')};
...
const schemaPropertyCommentsRuntime = `(async () => {
  ...
  try {
    ...
    const headings = Array.from(builder.querySelectorAll("th[data-specification-column] [data-reorder-item-row] > :nth-child(2)"), ({ textContent }) => textContent);
