import { waitForSchemaCopyPresentation } from "./schema-copy-presentation.mjs";
...
const liveSchemaPropertyDeclarationSeedRuntime = `(() => {
  ...
  const payload={page_type:"product_detail",commerce:{currency:"EUR"},products:[{product_name:"Phone",product_id:42},{product_name:"Tablet",price:25}]};
...
const liveSchemaPropertyDeclarationRuntime = `(async () => {
  ...
  const addAll=q("#live-inspector-action-add-all-to-schema");const actions={addToSchema:Boolean(productName),addAllToSchema:Boolean(addAll),addValidation:Boolean(validation),destination:productName.ariaLabel,reachable:productName.getClientRects().length>0&&validation.getClientRects().length>0&&addAll.getClientRects().length>0};
  const reviewCases={};
  ...
  const bulkBefore=localStorage.getItem("my-chrome-utilities.schema-library.v1");q("#live-inspector-action-add-all-to-schema").click();await pause();let bulkDialog=q(".live-schema-bulk-review"),bulkDestination=bulkDialog.querySelector("#live-schema-bulk-destination");bulkDestination.value="new";bulkDestination.dispatchEvent(new Event("change"));const bulkName=bulkDialog.querySelector("#live-schema-bulk-name");bulkName.value="Observed checkout";bulkName.dispatchEvent(new Event("input"));const bulkRect=bulkDialog.getBoundingClientRect(),bulkText=bulkDialog.textContent;click(bulkDialog,"Cancel");await pause();const bulkCancel={visible:bulkRect.width>0&&bulkRect.height>0,bothItems:bulkText.includes("/products/*/price")&&bulkText.includes("/products/*/product_name"),destination:bulkDestination.value==="new"&&!bulkName.hidden,unchanged:bulkBefore===localStorage.getItem("my-chrome-utilities.schema-library.v1"),focus:document.activeElement?.id};
  q("#live-inspector-action-add-all-to-schema").click();await pause();bulkDialog=q(".live-schema-bulk-review");bulkDialog.dispatchEvent(new Event("cancel",{cancelable:true}));await pause();const bulkEscape={closed:!document.querySelector(".live-schema-bulk-review"),focus:document.activeElement?.id};
  q("#live-inspector-action-add-all-to-schema").click();await pause();bulkDialog=q(".live-schema-bulk-review");const bulkConfirm=Array.from(bulkDialog.querySelectorAll("button")).find(({textContent})=>textContent?.startsWith("Add observed properties"));if(!bulkConfirm)throw new Error("Missing bulk confirmation");bulkConfirm.focus();const keyboardFocus=document.activeElement===bulkConfirm;bulkConfirm.click();const bulkStored=(await globalThis.__waitForDurableSchemaObservation(([candidate])=>candidate?.workingDraft?.document?.properties?.products?.items?.properties?.price?.type==="number","bulk schema declaration"))[0];await frame();const bulk={cancel:bulkCancel,escape:bulkEscape,confirm:{keyboardFocus,priceType:bulkStored.workingDraft.document.properties.products.items.properties.price.type,priceExample:bulkStored.workingDraft.documentation.properties["/products/*/price"].example,version:bulkStored.version,workingDraft:Boolean(bulkStored.workingDraft),assignmentCount:bulkStored.workingDraft.assignments.length,ruleCount:bulkStored.workingDraft.attachedRules.length,liveUsable:Boolean(q("#live-event-inspector"))}};
  ...
  return {actions,bulk,reviewCases,saved,validation:validationEvidence,separate,published:{version:published.version,workingDraftAbsent:!published.workingDraft,productName:published.document.properties.products.items.properties.product_name,productId:published.document.properties.products.items.properties.product_id,rules:published.attachedRules}};
})()`;
...
const localRulePromotionReviewRuntime = `(async () => {
  const q=(selector,root=document)=>{const value=root.querySelector(selector);if(!value)throw new Error("Missing "+selector);return value;};
  ...
  await new Promise(requestAnimationFrame); const property=q('li[data-schema-property-canonical-path="/page_type"]'),action=q('.schema-attached-rule[data-rule-id="local-41"] .local-rule-promotion-action',property); action.focus({preventScroll:true});
  return {observation,cancelled:{focus:action.dataset.ruleId,open:q("details[data-attached-rules]",property).open,scroll:q("#schema-detail").scrollTop}};
})()`;
