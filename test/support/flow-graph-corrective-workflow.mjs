...
export const flowGraphRepeatedInstanceReadinessLimitMilliseconds = 30_000;

export function flowGraphRuntimeCanvasReady(state){
  return Boolean(state?.canvasConnected && state?.workspaceOpen === false &&
    state?.width > 0 && state?.height > 0);
}

export function flowGraphRuntimeDefinitionReady(state){
  return Boolean(state?.definitionConnected && state?.definitionDisabled === false &&
    state?.menuConnected && state?.workspaceConnected);
}

export function flowGraphRuntimeEditorReady(state){
  return Boolean(state?.editorConnected && state?.workspaceConnected);
}

export function flowGraphRuntimeInlineActionReady(state){
  return Boolean(state?.actionConnected && state?.actionDisabled === false &&
    state?.surfaceConnected);
}

export function flowGraphRuntimeReturnReady(state){
  return Boolean(state?.returnConnected && state?.returnDisabled === false &&
    state?.width > 0 && state?.height > 0);
}

export function flowGraphCorrectiveWorkflow(seeded, options){
  return flowWorkspaceR02Runtime(seeded, options);
...
export function flowGraphReloadEvidence(seeded){
  return `
(async()=>{
  const q=(selector,root=document)=>root?.querySelector(selector),all=(selector,root=document)=>root?[...root.querySelectorAll(selector)]:[],pause=(milliseconds=40)=>new Promise(resolve=>setTimeout(resolve,milliseconds)),waitFor=async(read,label)=>{for(let attempt=0;attempt<100;attempt+=1){const value=await read();if(value)return value;await pause();}throw new Error('Timed out waiting for '+label);},repository=await(await import('./data-layer-durable-project-repository.js')).openIndexedDbProjectRepository(),projectId=${JSON.stringify(seeded.projectId)},flowId=${JSON.stringify(seeded.flowId)},load=()=>repository.loadProject(projectId),snapshot=async()=>(await load()).state.project.documentationFlowGraphs[flowId],loaded=await load(),graph=loaded.state.project.documentationFlowGraphs[flowId],canvas=q('[aria-label="Interactive directional Flow canvas"]'),toolbar=q('[aria-label="Flow toolbar"]'),viewport=q('[aria-label="Flow canvas viewport"]'),surface=()=>all('[aria-label="Flow contextual surface"]').find(item=>!item.hidden),outlineButton=all('button',toolbar).find(({textContent})=>textContent.trim()==='Outline');outlineButton.click();await pause();const outline=q('[aria-label="Synchronized editable Flow outline"]'),stored=JSON.parse(sessionStorage.getItem('my-chrome-utilities.flow-view.v1:'+projectId+':'+flowId)??'{}'),evidence={
  ...
  const allVisualItems=[...graph.pageFrames.filter(({conceptVisual})=>conceptVisual&&conceptVisual.description!=='Off-screen visual').map(item=>({kind:'page-frame',item})),...graph.occurrences.filter(({conceptVisual})=>conceptVisual&&conceptVisual.description!=='Off-screen visual').map(item=>({kind:'occurrence',item}))],visualItems=allVisualItems.filter(({item})=>['Mobile Cart concept','Duplicate Cart visual','Payment shared concept'].includes(item.conceptVisual.description)),settlementVisualItems=allVisualItems.filter(({item})=>item.conceptVisual.description.includes('save-settlement visual'));
  if(settlementVisualItems.length){
    outlineButton.click();await waitFor(()=>!surface(),'closed reloaded Outline before settlement visual proof');const rows=[];
    for(const descriptor of settlementVisualItems){const selector=descriptor.kind==='page-frame'?'g[data-page-frame-id="'+CSS.escape(descriptor.item.id)+'"]:not([data-occurrence-id])':'[data-occurrence-id="'+CSS.escape(descriptor.item.id)+'"]',target=q(selector,canvas),asset=loaded.state.project.conceptVisualAssets?.find(({id})=>id===descriptor.item.conceptVisual.assetId),metadata=(await repository.listConceptVisualAssetMetadata(projectId)).filter(({id})=>id===asset?.id),body=asset&&await repository.loadConceptVisualAssetBody(projectId,asset.id),thumbnail=asset&&await repository.loadConceptVisualAssetThumbnail(projectId,asset.id);rows.push({kind:descriptor.kind,installedTarget:Boolean(target),attachment:Boolean(descriptor.item.conceptVisual.id&&asset?.digest),oneOriginal:metadata.length===1&&body?.size===asset?.byteLength,readyThumbnail:Boolean(thumbnail?.size)});}
    evidence.runtime049={reloadStable:settlementVisualItems.length===2&&rows.some(({kind})=>kind==='page-frame')&&rows.some(({kind})=>kind==='occurrence')&&rows.every(({installedTarget,attachment,oneOriginal,readyThumbnail})=>installedTarget&&attachment&&oneOriginal&&readyThumbnail),measurements:{reloadRows:rows}};
  }
  if(visualItems.length){
    if(!settlementVisualItems.length)outlineButton.click();await waitFor(()=>!surface(),'closed reloaded Outline before visual actions');
    const assets=loaded.state.project.conceptVisualAssets??[],assetIds=new Set(visualItems.map(({item})=>item.conceptVisual.assetId)),attachmentIds=new Set(visualItems.map(({item})=>item.conceptVisual.id)),descriptions=new Set(visualItems.map(({item})=>item.conceptVisual.description)),select=async({kind,item})=>{await waitFor(()=>!surface(),'closed contextual surface before '+item.conceptVisual.description);const selector=kind==='page-frame'?'g[data-page-frame-id="'+CSS.escape(item.id)+'"]:not([data-occurrence-id])':'[data-occurrence-id="'+CSS.escape(item.id)+'"]',target=q(selector,canvas);if(!target)throw new Error('Missing reloaded visual target '+kind+' '+item.id);target.dispatchEvent(new MouseEvent('click',{bubbles:true}));await pause();return target;},openAction=async(descriptor,label)=>{const target=await select(descriptor),trigger=all('button',viewport).find(({textContent})=>textContent.trim()==='Actions');if(!trigger)throw new Error('Missing Actions for reloaded visual '+descriptor.item.conceptVisual.description);trigger.click();const menu=await waitFor(()=>q('[role="menu"]',surface()),'reloaded visual menu '+descriptor.item.conceptVisual.description+' selected='+target.classList.contains('is-selected')+' expanded='+trigger.getAttribute('aria-expanded')),control=all('[role="menuitem"]',menu).find(({textContent})=>textContent.trim()===label);if(!control)throw new Error('Missing '+label+' for reloaded visual '+descriptor.item.conceptVisual.description);control.click();return control;};
    const original=visualItems.find(({item})=>item.conceptVisual.description==='Mobile Cart concept'),duplicate=visualItems.find(({item})=>item.conceptVisual.description==='Duplicate Cart visual'),event=visualItems.find(({item})=>item.conceptVisual.description==='Payment shared concept'),sharedAsset=assets.find(({id})=>id===original?.item.conceptVisual.assetId),originalAttachment=structuredClone(original?.item.conceptVisual),originalAsset=structuredClone(sharedAsset);
    for(const descriptor of[duplicate,event]){await openAction(descriptor,'Remove visual');await waitFor(async()=>!(await snapshot())[descriptor.kind==='page-frame'?'pageFrames':'occurrences'].find(({id})=>id===descriptor.item.id)?.conceptVisual,'removed shared visual');}
    const retainedLoaded=await load(),retainedGraph=retainedLoaded.state.project.documentationFlowGraphs[flowId],sharedRetained=Boolean(original&&retainedGraph.pageFrames.find(({id})=>id===original.item.id)?.conceptVisual?.assetId===sharedAsset?.id&&retainedLoaded.state.project.conceptVisualAssets?.some(({id})=>id===sharedAsset?.id));
    await openAction(original,'Remove visual');await waitFor(async()=>!(await load()).state.project.conceptVisualAssets?.some(({id})=>id===sharedAsset?.id),'last shared visual cleanup');const lastReferenceCleanup=!(await snapshot()).pageFrames.find(({id})=>id===original.item.id)?.conceptVisual;
    q('#undo-project').click();const restored=await waitFor(async()=>{const state=await load(),attachment=state.state.project.documentationFlowGraphs[flowId].pageFrames.find(({id})=>id===original.item.id)?.conceptVisual,asset=state.state.project.conceptVisualAssets?.find(({id})=>id===sharedAsset?.id);return attachment&&asset&&{attachment,asset};},'shared visual Undo restoration');
    evidence.runtime037={reloadExact:visualItems.length===3&&assetIds.size===1&&attachmentIds.size===3&&descriptions.size===3&&Boolean(sharedAsset&&!('bytes' in sharedAsset))&&sharedAsset.width===1024&&sharedAsset.height===640&&sharedAsset.byteLength>0,sharedRetained,lastReferenceCleanup,undoRestored:JSON.stringify(restored.attachment)===JSON.stringify(originalAttachment)&&JSON.stringify(restored.asset)===JSON.stringify(originalAsset)};
  }
  return evidence;
})()`;
}
...
export function flowGraphRepeatedInstanceEvidence(seeded,fixture){
  const readinessProgram=browserReadinessProgramSource({
    targetId:"FLOW_WORKSPACE_AUTHORING_TARGET",phase:"navigation",
    timeoutMs:flowGraphRepeatedInstanceReadinessLimitMilliseconds,pollIntervalMs:40,
    maximumSnapshotCharacters:400,
  });
  return `
(async()=>{
  const q=(selector,root=document)=>root?.querySelector(selector),all=(selector,root=document)=>root?[...root.querySelectorAll(selector)]:[],pause=(milliseconds=120)=>new Promise((resolve)=>setTimeout(resolve,milliseconds));${readinessProgram}const flowCanvasReady=${flowGraphRuntimeCanvasReady.toString()},definitionReady=${flowGraphRuntimeDefinitionReady.toString()},editorReady=${flowGraphRuntimeEditorReady.toString()},inlineActionReady=${flowGraphRuntimeInlineActionReady.toString()},returnReady=${flowGraphRuntimeReturnReady.toString()},repository=await(await import('./data-layer-durable-project-repository.js')).openIndexedDbProjectRepository(),documentationApi=await import('./data-layer-flow-documentation-snapshot.js'),projectId=${JSON.stringify(seeded.projectId)},flowId=${JSON.stringify(seeded.flowId)},fixture=${JSON.stringify(fixture)},load=()=>repository.loadProject(projectId),exampleValues=(state)=>fixture.frameIds.map((id)=>state.project.documentationFlowGraphs[flowId].pageFrames.find((frame)=>frame.id===id)?.localSchemaContributions?.find(({path})=>path==='/currency')?.allowedValues?.[0]),documentationValues=(state)=>{const contexts=documentationApi.flowDocumentationSnapshotFromState(state,flowId,'2026-08-04T00:00:00.000Z').contexts;return fixture.frameIds.map((id)=>contexts.find(({pageFrameId,kind})=>pageFrameId===id&&kind==='page-instance')?.compiled.properties['/currency']?.allowedValues?.[0]);};
  const flowCanvasState=()=>{const canvas=q('[aria-label="Interactive directional Flow canvas"]'),rect=canvas?.getBoundingClientRect(),workspace=q('.composed-schema-workspace'),workspaceRect=workspace?.getBoundingClientRect();return{canvas,canvasConnected:Boolean(canvas?.isConnected),workspaceOpen:Boolean(workspace?.isConnected&&(workspaceRect?.width??0)>0&&(workspaceRect?.height??0)>0),left:rect?.left??0,top:rect?.top??0,width:rect?.width??0,height:rect?.height??0};},flowCanvasSnapshot=({canvas,...state})=>state;
  const openInstanceSchema=async()=>{const boundary=await waitFor(()=>{const surface=q('[aria-label="Selected Page instance inline actions"]'),action=q('[data-flow-schema-contribution="true"]',surface);return{action,actionConnected:Boolean(action?.isConnected),actionDisabled:Boolean(action?.disabled),surfaceConnected:Boolean(surface?.isConnected)};},'actionable Page instance inline actions',inlineActionReady,({action,...state})=>state,50);boundary.action.click();};
  const actionableControl=async(read,label)=>{const boundary=await waitFor(()=>{const control=read(),rect=control?.getBoundingClientRect();return{control,connected:Boolean(control?.isConnected),disabled:Boolean(control?.disabled),width:rect?.width??0,height:rect?.height??0};},'actionable runtime024 '+label,state=>state.connected&&!state.disabled&&state.width>0&&state.height>0,({control,...state})=>state,50);return boundary.control;};
  flowEvidencePhase('runtime024');const contextualSurface=q('[aria-label="Flow contextual surface"]');if(contextualSurface&&!contextualSurface.hidden)all('button',contextualSurface).find(({textContent})=>textContent.trim()==='Close')?.click();await waitFor(flowCanvasState,'actionable runtime024 Flow canvas',flowCanvasReady,flowCanvasSnapshot,50);
  const setExpected=async(frameId,value)=>{let canvas=(await waitFor(flowCanvasState,'actionable runtime024 Flow canvas',flowCanvasReady,flowCanvasSnapshot,50)).canvas,frame=q('g[data-page-frame-id="'+CSS.escape(frameId)+'"]:not([data-occurrence-id])',canvas);frame.dispatchEvent(new MouseEvent('click',{bubbles:true}));await openInstanceSchema();const workspaceSelector='.composed-schema-workspace[data-schema-contributor-id="'+CSS.escape(frameId)+'"]';let workspace;const propertyBoundary=await waitFor(()=>{workspace=q(workspaceSelector);const rows=all('[data-flow-instance-effective-path]',workspace),control=q('[aria-label^="Property actions"]',rows.find(({dataset})=>dataset.flowInstanceEffectivePath==='/currency')),rect=control?.getBoundingClientRect();return{control,connected:Boolean(control?.isConnected),disabled:Boolean(control?.disabled),width:rect?.width??0,height:rect?.height??0,workspaceConnected:Boolean(workspace?.isConnected),availablePaths:rows.map(({dataset})=>dataset.flowInstanceEffectivePath),workspaceText:String(workspace?.textContent??'').slice(0,160)};},'actionable runtime024 Property actions',state=>state.connected&&!state.disabled&&state.width>0&&state.height>0,({control,...state})=>state,50),actions=propertyBoundary.control;actions.click();await waitFor(()=>q(':modal [data-property-context-menu="true"]'),'property menu');const definitionBoundary=await waitFor(()=>{const definition=q(':modal [data-section="definition"] button'),menu=q(':modal [data-property-context-menu="true"]');return{definition,definitionConnected:Boolean(definition?.isConnected),definitionDisabled:Boolean(definition?.disabled),menuConnected:Boolean(menu?.isConnected),workspaceConnected:Boolean(workspace?.isConnected)};},'actionable Definition section',definitionReady,({definition,...state})=>state,50);definitionBoundary.definition.click();const editorBoundary=await waitFor(()=>{const editor=all(':modal [data-focused-property-editor="true"]').at(-1);return{editor,editorConnected:Boolean(editor?.isConnected),workspaceConnected:Boolean(workspace?.isConnected),modalCount:all(':modal').length};},'actionable Definition editor',editorReady,({editor,...state})=>state),editor=editorBoundary.editor,expected=q('[name="ordinaryValue"]',editor);expected.value=value;expected.dispatchEvent(new Event('input',{bubbles:true}));(await actionableControl(()=>all('button',editor).find(({textContent})=>textContent.trim()==='Review changes'),'Review changes')).click();const confirm=await actionableControl(()=>all('button').find(({textContent})=>textContent.trim()==='Confirm changes'),'Confirm changes');confirm.click();await waitFor(async()=>{const state=(await load()).state,frameRecord=state.project.documentationFlowGraphs[flowId].pageFrames.find(({id})=>id===frameId);return frameRecord.localSchemaContributions?.find(({path})=>path==='/currency')?.allowedValues?.[0]===value;},'saved instance value');const returnBoundary=await waitFor(()=>{const returnToFlow=all('button').find((candidate)=>candidate.textContent.trim()==='Return to Flow'),rect=returnToFlow?.getBoundingClientRect();return{returnToFlow,returnConnected:Boolean(returnToFlow?.isConnected),returnDisabled:Boolean(returnToFlow?.disabled),width:rect?.width??0,height:rect?.height??0};},'actionable Return to Flow',returnReady,({returnToFlow,...state})=>state);returnBoundary.returnToFlow.click();await waitFor(flowCanvasState,'actionable runtime024 Flow canvas',flowCanvasReady,flowCanvasSnapshot,50);};for(let index=0;index<fixture.frameIds.length;index+=1)await setExpected(fixture.frameIds[index],fixture.values[index]);
  let loaded=await load();const graph=loaded.state.project.documentationFlowGraphs[flowId],beforeExamples=exampleValues(loaded.state),beforeDocumentation=documentationValues(loaded.state),documentationSnapshot=documentationApi.flowDocumentationSnapshotFromState(loaded.state,flowId,'2026-08-04T00:00:00.000Z'),frames=graph.pageFrames.filter(({id})=>fixture.frameIds.includes(id)),instanceRelationships=graph.relationships.filter(({id})=>fixture.relationshipIds.includes(id));let canvas=q('[aria-label="Interactive directional Flow canvas"]'),outline;(await actionableControl(()=>all('button',q('[aria-label="Flow toolbar"]')).find(({textContent})=>textContent.trim()==='Outline'),'Outline')).click();await pause();outline=q('[aria-label="Synchronized editable Flow outline"]');let rendered=true;for(const id of fixture.frameIds){const selector='[data-page-frame-id="'+CSS.escape(id)+'"]';rendered=rendered&&Boolean(q(selector,canvas))&&Boolean(q(selector,outline));}(await actionableControl(()=>all('button',q('[aria-label="Flow contextual surface"]')).find(({textContent})=>textContent.trim()==='Close'),'Close contextual surface')).click();const middleId=fixture.frameIds[1],middleGroup=q('g[data-page-frame-id="'+CSS.escape(middleId)+'"]:not([data-occurrence-id])',canvas);middleGroup.dispatchEvent(new MouseEvent('click',{bubbles:true}));await openInstanceSchema();const workspace=await waitFor(()=>q('.composed-schema-workspace[data-schema-contributor-id="'+CSS.escape(middleId)+'"]'),'middle schema workspace'),effectiveRow=q('[data-flow-instance-effective-path="/currency"]',workspace),effectiveContributors=fixture.frameIds.every((id)=>documentationSnapshot.contexts.find(({pageFrameId,kind})=>pageFrameId===id&&kind==='page-instance')?.compiled.properties['/currency']?.origins.some(({contributorId})=>contributorId===id))&&Boolean(effectiveRow);(await actionableControl(()=>all('button',workspace).find(({textContent})=>textContent.startsWith('Local changes ')),'Local changes')).click();(await actionableControl(()=>all('button',workspace).find(({textContent})=>textContent==='Reset /currency to parent'),'Reset currency')).click();(await actionableControl(()=>all('button',workspace).find(({textContent})=>textContent==='Confirm property reset'),'Confirm property reset')).click();for(let attempt=0;attempt<60;attempt+=1){await pause();loaded=await load();const middle=loaded.state.project.documentationFlowGraphs[flowId].pageFrames.find(({id})=>id===middleId);if(!middle.localSchemaContributions?.some(({path})=>path==='/currency'))break;}const afterExamples=exampleValues(loaded.state),afterDocumentation=documentationValues(loaded.state);all('button').find(({textContent})=>textContent==='Return to Flow')?.click();
...
(async()=>{
  ...
  return{beforeProject,occurrenceId:occurrence.id,relationshipId:relationship.id,frameId:frame.id,targetId:target.id,pageId:page.id,pageName:page.name,eventId:event.id,position:occurrence.position,relationship};
})()`;
}
...
(async()=>{
  ...
  const review=q('[aria-label="Flow Page-context migration review"]'),reviewText=review?.textContent??'',confirm=all('button',review).find(({textContent})=>textContent==='Confirm Page-context migration'),humanReview=Boolean(confirm&&reviewText.includes('Checkout journey')&&reviewText.includes(fixture.pageName)&&reviewText.includes('Legacy initial context')&&!reviewText.includes(fixture.occurrenceId));if(!confirm)throw new Error('Missing Page-context migration confirmation; heading='+q('#workspace-content h1')?.textContent+'; review='+reviewText+'; labelled='+all('[aria-label]').map((node)=>node.getAttribute('aria-label')).filter((label)=>/migration/i.test(label)).join('|'));
  confirm.click();let loaded,graph,migratedRelationship,page,event;for(let attempt=0;attempt<100;attempt+=1){await pause();loaded=await load();graph=loaded.state.project.documentationFlowGraphs[flowId];migratedRelationship=graph.relationships.find(({id})=>id===fixture.relationshipId);page=loaded.state.project.collections.pages.find(({id})=>id===fixture.pageId);event=loaded.state.project.collections.events.find(({id})=>id===fixture.eventId);if(!graph.occurrences.some(({id})=>id===fixture.occurrenceId)&&migratedRelationship?.sourceEndpoint?.kind==='page-frame'&&migratedRelationship.sourceEndpoint.id===fixture.frameId&&!Object.hasOwn(event,'role')&&!Object.hasOwn(page,'contextEventBindings')&&!JSON.stringify(graph).includes('contextBindingId'))break;}
...
export function flowGraphEventExampleSeed(seeded,mode){
  return `
(async()=>{
  const repository=await(await import('./data-layer-durable-project-repository.js')).openIndexedDbProjectRepository(),durable=await import('./data-layer-durable-project-repository.js'),flowId=${JSON.stringify(seeded.flowId)},mode=${JSON.stringify(mode)};
  for(let attempt=0;attempt<4;attempt+=1){const base=await repository.loadProject(${JSON.stringify(seeded.projectId)}),next=structuredClone(base.state),graph=next.project.documentationFlowGraphs[flowId],occurrence=graph.occurrences.find(({id})=>id===${JSON.stringify(seeded.occurrenceIds[0])}),page=next.project.collections.pages.find(({id})=>id===occurrence.pageId),event=next.project.collections.events.find(({id})=>id===occurrence.eventId);if(!occurrence||!page||!event)throw new Error('Missing runtime021 fixture target');
    ...
    const result=await repository.saveDraft(durable.durableDraftCommand(base,next,{commandId:'seed-runtime021-'+mode,label:'Seed '+mode+' Event example'}));if(result.status==='committed')return{occurrenceId:occurrence.id};if(result.status!=='conflict')throw new Error('Event example seed '+result.status);
  }
  throw new Error('Event example seed conflict after 4 fresh revisions');
})()`;
}

function flowExamplesWaitSource(){
  return `const waitFor=async(predicate,observe,timeoutMs=5000)=>{const started=performance.now(),deadline=started+timeoutMs;let last;while(true){last=observe();if(last)return last;const current=performance.now();if(current>=deadline){const snapshot=last instanceof Element?{tag:last.tagName,id:last.id,text:String(last.textContent??'').slice(0,120)}:last;throw new Error('FLOW_GRAPH_EXAMPLES_TARGET example compilation timed out waiting for '+predicate+' after '+Math.round(current-started)+'ms; last state '+JSON.stringify(snapshot).slice(0,400));}await new Promise((resolve)=>setTimeout(resolve,Math.min(25,deadline-current)));}};`;
}

export function flowGraphEventExampleIncompleteEvidence(seeded,fixture){
  return `
(async()=>{
  flowEvidencePhase('runtime021');const q=(selector,root=document)=>root?.querySelector(selector),all=(selector,root=document)=>root?[...root.querySelectorAll(selector)]:[];${flowExamplesWaitSource()}const repository=await(await import('./data-layer-durable-project-repository.js')).openIndexedDbProjectRepository(),projectId=${JSON.stringify(seeded.projectId)},flowId=${JSON.stringify(seeded.flowId)},fixture=${JSON.stringify(fixture)},flowNav=await waitFor('Flow navigation control',()=>q('[data-kind=flows]'));flowNav.click();const row=await waitFor('Checkout journey row',()=>all('.entity-row button').find(({textContent})=>textContent==='Checkout journey'));row.click();
  const details=()=>q('[data-event-example-for="'+CSS.escape(fixture.occurrenceId)+'"]',q('[aria-label="Interactive directional Flow canvas"]')),currentDetails=await waitFor('Event example details',details);currentDetails.open=true;let current=await waitFor('Event example JSON',()=>q('pre[data-readonly-example]',details())&&details()),missing=q('[data-example-issue-path="/runtime_note"]',current),json=JSON.parse(q('pre[data-readonly-example]',current).textContent),provenance=all('[data-example-path]',current),incomplete=current.dataset.exampleStatus==='Incomplete'&&!Object.hasOwn(json,'runtime_note')&&missing?.dataset.exampleIssueCode==='REQUIRED_EXAMPLE',provenanceExposed=provenance.length>=2&&provenance.some(({dataset})=>dataset.examplePath==='/quantity'),repair=q('a',missing),repairHref=repair?.getAttribute('href')?.includes('runtime_note');repair.click();const editor=q('#layered-schema-editor-host'),returnControl=await waitFor('Event example schema repair editor',()=>all('button',editor).find(({textContent})=>textContent==='Return to Flow')),selected=await waitFor('runtime_note repair target',()=>all('[data-property-id]',editor).find(({dataset})=>dataset.propertyId==='/runtime_note')??q('[data-flow-instance-effective-path="/runtime_note"]',editor)),repairFocusTarget=q('button,input,select,[tabindex]',selected)??selected;repairFocusTarget.focus({preventScroll:true});const repairOpened=Boolean(editor&&!editor.hidden&&q('#workspace-content')?.hidden),repairFocused=repairFocusTarget===document.activeElement;returnControl.click();current=(await waitFor('Event example after repair return',details));current.open=true;missing=await waitFor('runtime_note inline repair control',()=>q('[data-example-issue-path="/runtime_note"]',details()));const input=q('input',missing),save=all('button',missing).find(({textContent})=>textContent==='Save example');input.value='Ready';input.dispatchEvent(new Event('input',{bubbles:true}));save.click();current=await waitFor('complete Event example',()=>details()?.dataset.exampleStatus==='Complete'&&details());const loaded=await repository.loadProject(projectId),stored=loaded.state.project.documentationFlowGraphs[flowId].occurrences.find(({id})=>id===fixture.occurrenceId);
...
export function flowGraphEventExampleStateEvidence(seeded,fixture,expected,path,code){
  return `
(async()=>{flowEvidencePhase('runtime021');const q=(selector,root=document)=>root?.querySelector(selector),all=(selector,root=document)=>root?[...root.querySelectorAll(selector)]:[];${flowExamplesWaitSource()}const flowNav=await waitFor('Flow navigation control',()=>q('[data-kind=flows]'));flowNav.click();const row=await waitFor('Checkout journey row',()=>all('.entity-row button').find(({textContent})=>textContent==='Checkout journey'));row.click();const details=await waitFor('${expected} Event example',()=>{const node=q('[data-event-example-for="'+CSS.escape(${JSON.stringify(fixture.occurrenceId)})+'"]',q('[aria-label="Interactive directional Flow canvas"]'))??q('[data-event-example-for="'+CSS.escape(${JSON.stringify(fixture.occurrenceId)})+'"]');return node?.dataset.exampleStatus===${JSON.stringify(expected)}&&node;});return Boolean(q('[data-example-issue-path="'+${JSON.stringify(path)}+'"][data-example-issue-code="'+${JSON.stringify(code)}+'"]',details)??(${JSON.stringify(expected)}==='Blocked'?details:null));})()`;
}
...
export function flowGraphPageExampleIncompleteEvidence(seeded,fixture){
  return `
(async()=>{
  flowEvidencePhase('runtime025');const q=(selector,root=document)=>root?.querySelector(selector),all=(selector,root=document)=>root?[...root.querySelectorAll(selector)]:[],modal=()=>all(':modal').at(-1);${flowExamplesWaitSource()}const repository=await(await import('./data-layer-durable-project-repository.js')).openIndexedDbProjectRepository(),projectId=${JSON.stringify(seeded.projectId)},flowId=${JSON.stringify(seeded.flowId)},fixture=${JSON.stringify(fixture)},flowNav=await waitFor('Flow navigation control',()=>q('[data-kind=flows]'));flowNav.click();const row=await waitFor('Checkout journey row',()=>all('.entity-row button').find(({textContent})=>textContent==='Checkout journey'));row.click();
  const details=()=>q('[data-page-example-for="'+CSS.escape(fixture.frameId)+'"]'),currentDetails=await waitFor('Page example details',details);currentDetails.open=true;let current=await waitFor('Page example JSON',()=>q('pre[data-readonly-page-example]',details())&&details()),json=JSON.parse(q('pre[data-readonly-page-example]',current).textContent),missing=q('[data-example-issue-path="/page_name"]',current),provenance=all('[data-example-path]',current),incomplete=current.dataset.exampleStatus==='Incomplete'&&!Object.hasOwn(json,'page_name')&&missing?.dataset.exampleIssueCode==='REQUIRED_EXAMPLE',fullComposition=['/page_type','/form_step'].every((path)=>provenance.some(({dataset})=>dataset.examplePath===path)),repair=q('a',missing),repairHref=repair?.getAttribute('href')?.includes('page_name');repair.click();const editor=q('#layered-schema-editor-host');let effectiveRow=await waitFor('page_name effective repair row',()=>!editor.hidden&&q('[data-flow-instance-effective-path="/page_name"]',editor)),repairFocusTarget=q('[aria-label^="Property actions"]',effectiveRow)??effectiveRow;repairFocusTarget.focus({preventScroll:true});const repairOpened=Boolean(editor&&!editor.hidden&&effectiveRow),repairFocused=repairFocusTarget===document.activeElement,override=all('button',effectiveRow).find(({textContent})=>textContent==='Override here');if(override){override.click();effectiveRow=await waitFor('page_name local override row',()=>q('[data-flow-instance-effective-path="/page_name"]',editor));}const propertyActions=await waitFor('page_name Property actions',()=>q('[aria-label^="Property actions"]',effectiveRow));propertyActions.click();const definition=await waitFor('Page example Definition action',()=>q('[data-section="definition"] button',modal()));definition.click();let focused=await waitFor('focused Page property editor',()=>q('[data-focused-property-editor="true"]',modal())),method=q('[name="exampleMethod"]',focused);method.value='custom';method.dispatchEvent(new Event('change',{bubbles:true}));const exampleValue=await waitFor('Page example value input',()=>q('[name="exampleValue"]',modal()));focused=q('[data-focused-property-editor="true"]',modal());exampleValue.value='payment';exampleValue.dispatchEvent(new Event('input',{bubbles:true}));const reviewChanges=await waitFor('Page example Review changes',()=>all('button',focused).find(({textContent})=>textContent==='Review changes'));reviewChanges.click();const confirm=await waitFor('Page example Confirm changes',()=>all('button',modal()).find(({textContent})=>textContent==='Confirm changes'));confirm.click();const returnToFlow=await waitFor('Return to Flow control',()=>all('button',editor).find(({textContent})=>textContent==='Return to Flow'));returnToFlow.click();current=await waitFor('complete Page example',()=>{const value=details();value?.setAttribute('open','');return value?.dataset.exampleStatus==='Complete'&&value;});const loaded=await repository.loadProject(projectId),stored=loaded.state.project.documentationFlowGraphs[flowId].pageFrames.find(({id})=>id===fixture.frameId);
...
export function flowGraphPageExampleStateEvidence(fixture,expected,path,code){
  return `
(async()=>{flowEvidencePhase('runtime025');const q=(selector,root=document)=>root?.querySelector(selector),all=(selector,root=document)=>root?[...root.querySelectorAll(selector)]:[];${flowExamplesWaitSource()}const flowNav=await waitFor('Flow navigation control',()=>q('[data-kind=flows]'));flowNav.click();const row=await waitFor('Checkout journey row',()=>all('.entity-row button').find(({textContent})=>textContent==='Checkout journey'));row.click();const details=await waitFor('${expected} Page example',()=>{const node=q('[data-page-example-for="'+CSS.escape(${JSON.stringify(fixture.frameId)})+'"]');return node?.dataset.exampleStatus===${JSON.stringify(expected)}&&node;});return Boolean(q('[data-example-issue-path="'+${JSON.stringify(path)}+'"][data-example-issue-code="'+${JSON.stringify(code)}+'"]',details)??(${JSON.stringify(expected)}==='Blocked'?details:null));})()`;
}
