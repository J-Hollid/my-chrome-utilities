...
(async()=>{
  ...
  const visualItems=[...graph.pageFrames.filter(({conceptVisual})=>conceptVisual&&conceptVisual.description!=='Off-screen visual').map(item=>({kind:'page-frame',item})),...graph.occurrences.filter(({conceptVisual})=>conceptVisual&&conceptVisual.description!=='Off-screen visual').map(item=>({kind:'occurrence',item}))];
  if(visualItems.length){
    outlineButton.click();await waitFor(()=>!surface(),'closed reloaded Outline before visual actions');
