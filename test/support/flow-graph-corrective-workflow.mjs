...
(async()=>{
  ...
  const setExpected=async(frameId,value)=>{let canvas=(await waitFor(flowCanvasState,'actionable runtime024 Flow canvas',flowCanvasReady,flowCanvasSnapshot,50)).canvas,frame=q('g[data-page-frame-id="'+CSS.escape(frameId)+'"]:not([data-occurrence-id])',canvas);frame.dispatchEvent(new MouseEvent('click',{bubbles:true}));await pause();q('[data-flow-schema-contribution="true"]',q('[aria-label="Selected Page instance inline actions"]')).click();const workspace=await waitFor(()=>q('.composed-schema-workspace[data-schema-contributor-id="'+CSS.escape(frameId)+'"]'),'schema workspace');const row=q('[data-flow-instance-effective-path="/currency"]',workspace),actions=q('[aria-label^="Property actions"]',row);actions.click();await waitFor(()=>q(':modal [data-property-context-menu="true"]'),'property menu');const definitionBoundary=await waitFor(()=>{const definition=q(':modal [data-section="definition"] button'),menu=q(':modal [data-property-context-menu="true"]');return{definition,definitionConnected:Boolean(definition?.isConnected),definitionDisabled:Boolean(definition?.disabled),menuConnected:Boolean(menu?.isConnected),workspaceConnected:Boolean(workspace?.isConnected)};},'actionable Definition section',state=>state.definitionConnected&&!state.definitionDisabled&&state.menuConnected&&state.workspaceConnected,({definition,...state})=>state,50);definitionBoundary.definition.click();const editorBoundary=await waitFor(()=>{const editor=all(':modal [data-focused-property-editor="true"]').at(-1),definition=q(':modal [data-section="definition"] button');return{editor,editorConnected:Boolean(editor?.isConnected),definitionConnected:Boolean(definition?.isConnected),workspaceConnected:Boolean(workspace?.isConnected),modalCount:all(':modal').length};},'Definition editor',state=>state.editorConnected,({editor,...state})=>state),editor=editorBoundary.editor,expected=q('[name="ordinaryValue"]',editor);expected.value=value;expected.dispatchEvent(new Event('input',{bubbles:true}));all('button',editor).find(({textContent})=>textContent.trim()==='Review changes').click();const confirm=await waitFor(()=>all('button').find(({textContent})=>textContent.trim()==='Confirm changes'),'Confirm changes');confirm.click();await waitFor(async()=>{const state=(await load()).state,frameRecord=state.project.documentationFlowGraphs[flowId].pageFrames.find(({id})=>id===frameId);return frameRecord.localSchemaContributions?.find(({path})=>path==='/currency')?.allowedValues?.[0]===value;},'saved instance value');const returnToFlow=all('button').find((candidate)=>candidate.textContent.trim()==='Return to Flow'&&candidate.isConnected&&candidate.getBoundingClientRect().width>0);if(!returnToFlow)throw new Error('Missing visible Return to Flow for '+frameId);returnToFlow.click();await waitFor(flowCanvasState,'actionable runtime024 Flow canvas',flowCanvasReady,flowCanvasSnapshot,50);};for(let index=0;index<fixture.frameIds.length;index+=1)await setExpected(fixture.frameIds[index],fixture.values[index]);
