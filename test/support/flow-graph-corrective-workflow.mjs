...
export function flowGraphRepeatedInstanceSeed(seeded){
  return `
(async()=>{
  const repository=await(await import('./data-layer-durable-project-repository.js')).openIndexedDbProjectRepository(),durable=await import('./data-layer-durable-project-repository.js'),workspaceApi=await import('./data-layer-composed-schema-workspace.js'),graphApi=await import('./data-layer-flow-graph.js'),base=await repository.loadProject(${JSON.stringify(seeded.projectId)}),flowId=${JSON.stringify(seeded.flowId)},pageId=${JSON.stringify(seeded.pageIds[0])},sourceId=${JSON.stringify(seeded.frameIds[1])};let next=structuredClone(base.state),graph=next.project.documentationFlowGraphs[flowId],frames=graph.pageFrames.filter((frame)=>frame.pageId===pageId).slice(0,3),values=['instance-alpha','instance-beta','instance-gamma'];if(frames.length!==3)throw new Error('runtime024 needs exactly three reused Confirmation instances');for(const[index,frame]of frames.entries())next=workspaceApi.saveFlowComposedSchemaLocalFacets(next,flowId,frame.id,'/currency',{type:'string',expectedValue:values[index]});for(const frame of frames)next=graphApi.saveGraphRelationship(next,flowId,sourceId,{toStepId:frame.id,sourcePort:'top',targetPort:'bottom',label:'Instance route '+values[frames.indexOf(frame)]},(kind)=>kind+':runtime024:'+frames.indexOf(frame));const result=await repository.saveDraft(durable.durableDraftCommand(base,next,{commandId:'seed-runtime024-isolation',label:'Seed repeated Page-instance isolation evidence'}));if(result.status!=='committed')throw new Error('Repeated instance seed '+result.status);return{frameIds:frames.map(({id})=>id),values,pageId,sourceId,relationshipIds:frames.map((_,index)=>'flow-relationship:runtime024:'+index)};
})()`;
}
...
export function flowGraphRepeatedInstanceEvidence(seeded,fixture){
  return `
(async()=>{
  const q=(selector,root=document)=>root?.querySelector(selector),all=(selector,root=document)=>root?[...root.querySelectorAll(selector)]:[],pause=(milliseconds=120)=>new Promise((resolve)=>setTimeout(resolve,milliseconds)),repository=await(await import('./data-layer-durable-project-repository.js')).openIndexedDbProjectRepository(),examplesApi=await import('./flow-graph/page-example-derive.js'),documentationApi=await import('./data-layer-flow-documentation-snapshot.js'),projectId=${JSON.stringify(seeded.projectId)},flowId=${JSON.stringify(seeded.flowId)},fixture=${JSON.stringify(fixture)},load=()=>repository.loadProject(projectId),exampleValues=(state)=>fixture.frameIds.map((id)=>examplesApi.deriveFlowPageFrameExample(state.project,flowId,id).payload.currency),documentationValues=(state)=>{const contexts=documentationApi.flowDocumentationSnapshotFromState(state,flowId,'2026-08-04T00:00:00.000Z').contexts;return fixture.frameIds.map((id)=>contexts.find(({pageFrameId,kind})=>pageFrameId===id&&kind==='page-instance')?.compiled.properties['/currency']?.expectedValue);};
  ...
  let loaded=await load();const graph=loaded.state.project.documentationFlowGraphs[flowId];const beforeExamples=exampleValues(loaded.state);const beforeDocumentation=documentationValues(loaded.state);const documentationSnapshot=documentationApi.flowDocumentationSnapshotFromState(loaded.state,flowId,'2026-08-04T00:00:00.000Z');const frames=graph.pageFrames.filter(({id})=>fixture.frameIds.includes(id));const instanceRelationships=graph.relationships.filter(({id})=>fixture.relationshipIds.includes(id));let canvas=q('[aria-label="Interactive directional Flow canvas"]'),outline=q('[aria-label="Synchronized editable Flow outline"]');if(!outline){const outlineButton=all('button',q('[aria-label="Flow toolbar"]')).find(({textContent})=>textContent.trim()==='Outline');outlineButton?.click();await pause();outline=q('[aria-label="Synchronized editable Flow outline"]');}let rendered=true;for(const id of fixture.frameIds){const selector='[data-page-frame-id="'+CSS.escape(id)+'"]';rendered=rendered&&Boolean(q(selector,canvas))&&Boolean(q(selector,outline));}const middleId=fixture.frameIds[1],middleGroup=q('[data-page-frame-id="'+CSS.escape(middleId)+'"]',canvas);middleGroup?.dispatchEvent(new MouseEvent('click',{bubbles:true}));await pause();const actions=q('[aria-label="Selected Page instance inline actions"]'),openSchema=all('button',actions).find(({textContent})=>textContent.trim()==='Open schema contribution');if(!openSchema)throw new Error('Repeated Page contextual schema action missing.');openSchema.click();let workspace;for(let attempt=0;attempt<60&&!workspace;attempt+=1){await pause();workspace=q('.composed-schema-workspace[data-schema-contributor-id="'+CSS.escape(middleId)+'"]');}const effectiveRow=q('[data-flow-instance-effective-path="/currency"]',workspace);const effectiveContributors=fixture.frameIds.every((id)=>documentationSnapshot.contexts.find(({pageFrameId,kind})=>pageFrameId===id&&kind==='page-instance')?.compiled.properties['/currency']?.origins.some(({contributorId})=>contributorId===id))&&Boolean(effectiveRow);all('button',workspace).find(({textContent})=>textContent.startsWith('Local changes ')).click();await pause();const reset=all('button',workspace).find(({textContent})=>textContent==='Reset /currency to parent');reset.click();await pause();const confirm=all('button',workspace).find(({textContent})=>textContent==='Confirm property reset');confirm.click();for(let attempt=0;attempt<60;attempt+=1){await pause();loaded=await load();const middle=loaded.state.project.documentationFlowGraphs[flowId].pageFrames.find(({id})=>id===middleId);if(!middle.localSchemaContributions?.some(({path})=>path==='/currency'))break;}const afterExamples=exampleValues(loaded.state);const afterDocumentation=documentationValues(loaded.state);const returnFlow=all('button').find(({textContent})=>textContent==='Return to Flow');returnFlow?.click();
  return{threeIndependentFrames:frames.length===3&&frames.every(({pageId,id})=>pageId===fixture.pageId&&fixture.frameIds.includes(id))&&new Set(fixture.frameIds).size===3,distinctBranchTargets:instanceRelationships.length===3&&new Set(instanceRelationships.map(({targetEndpoint})=>targetEndpoint.id)).size===3&&instanceRelationships.every(({sourceEndpoint})=>sourceEndpoint.id===fixture.sourceId),isolatedExamples:JSON.stringify(beforeExamples)===JSON.stringify(fixture.values),effectiveContributors:Boolean(effectiveContributors),documentationIsolation:JSON.stringify(beforeDocumentation)===JSON.stringify(fixture.values),installedContextsRendered:Boolean(rendered),exactResetScope:afterExamples[0]===fixture.values[0]&&afterExamples[1]==='EUR'&&afterExamples[2]===fixture.values[2]&&afterDocumentation[0]===fixture.values[0]&&afterDocumentation[1]===undefined&&afterDocumentation[2]===fixture.values[2],sparseNoCopies:frames.every(({localSchemaContributions})=>localSchemaContributions?.filter(({path})=>path==='/currency').length===1)};
})()`;
}
