...
const schemaRenamingRetryReplayRuntime = `(async () => {
  ...
  const schemas=await __waitForDurableSchemaObservation((values)=>values.some(({id,workingDraft})=>id==="schema-page-view"&&workingDraft?.name==="Retry final"&&workingDraft?.canonicalSchema?.contributorName==="Retry final"),"the queued latest rename replay after Retry");await waitFor(()=>!q("#save-schema").disabled,"the queued latest rename durable settlement");const stored=schemas.find(({id})=>id==="schema-page-view"),replayed={name:stored.workingDraft.name,canonicalName:stored.workingDraft.canonicalSchema.contributorName,pending:stored.workingDraft.pendingChanges,visibleName:q("#schema-editor-name").value,publishReady:true};q("#close-storage-recovery").click();return{failure,result,replayed};
})()`;
