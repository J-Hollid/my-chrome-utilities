...

{
  ...
  for (const browserTargetId of browserTargetIds.length ? browserTargetIds : [null]) {
    ...
    if (activeBrowserTargetEnvironment.LIVE_SCHEMA_PROPERTY_DECLARATION_BROWSER_ADAPTER === "1") {
      ...
      assert.equal(observed.actions.addToSchema&&observed.actions.addValidation&&observed.actions.reachable,true);assert.equal(observed.actions.addAllToSchema,true);assert.match(observed.actions.destination,/Product detail/);
      assert.equal(observed.bulk.cancel.visible,true);assert.equal(observed.bulk.cancel.bothItems,true);assert.equal(observed.bulk.cancel.destination,true);assert.equal(observed.bulk.cancel.unchanged,true);assert.equal(observed.bulk.cancel.focus,"live-inspector-action-add-all-to-schema");
      assert.equal(observed.bulk.escape.closed,true);assert.equal(observed.bulk.escape.focus,"live-inspector-action-add-all-to-schema");assert.equal(observed.bulk.confirm.keyboardFocus,true);assert.equal(observed.bulk.confirm.priceType,"number");assert.equal(observed.bulk.confirm.priceExample.value,25);assert.equal(observed.bulk.confirm.priceExample.selectionMethod,"custom");assert.equal(observed.bulk.confirm.version,3);assert.equal(observed.bulk.confirm.workingDraft,true);assert.equal(observed.bulk.confirm.assignmentCount,1);assert.equal(observed.bulk.confirm.ruleCount,1);assert.equal(observed.bulk.confirm.liveUsable,true);
      for(const [key,path,type] of [["productName","/products/*/product_name","String"],["productId","/products/*/product_id","Number"]]){const review=observed.reviewCases[key];assert.match(review.text,new RegExp(path.replaceAll("*","\\*")));assert.match(review.text,new RegExp(type));assert.match(review.text,/Product detail revision 3/);assert.equal(review.noValidationControls&&review.storageUnchanged&&review.guidedVisible&&review.nonzeroBounds&&review.inputAccepted,true);}
