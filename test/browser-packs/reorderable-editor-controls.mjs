...
export async function runReorderableEditorControlsBrowser(environment=process.env){
  ...
  const membership=layered.membershipReorderEvidence,applications=layered.propertySetApplicationReorderEvidence,hierarchy=layered.hierarchyEvidence;
  const allLayeredAssertionsPassed=Object.values(layered).every(Boolean);
  const runtime001={
    "manual reproduction steps":defects.length===3&&defects.every(({completeControls,reorderEvidence})=>completeControls&&reorderEvidence.closedSemantics.accessibleName==="Reorder Click Bravo, position 2 of 3"&&reorderEvidence.closedSemantics.triggerDraggable&&!reorderEvidence.closedSemantics.rowDraggable),
    ...
    "canonical property tree":hierarchy.canonical.inventory.length>=3&&hierarchy.canonical.inventory.every(({triggers,primary,legacy})=>triggers===1&&primary===1&&!legacy)&&hierarchy.canonical.dragOrder.join("|")!==hierarchy.canonical.beforeOrder.join("|")&&hierarchy.canonical.dragExactUndo,
    "composed property tree":hierarchy.composed.inventory.length>=3&&hierarchy.composed.inventory.every(({triggers,primary,legacy})=>triggers===1&&primary===1&&!legacy)&&hierarchy.composed.dragOrder.join("|")!==hierarchy.composed.beforeOrder.join("|")&&hierarchy.composed.dragExactUndo,
    ...
    "guided array items":guided.inventory.length===3&&guided.inventory.every(({triggers,inputs,remove,legacy})=>triggers===1&&inputs===1&&remove&&!legacy)&&guided.before.join("|")==="1|2|3"&&guided.moved.join("|")==="2|1|3"&&guided.restored.join("|")==="1|2|3"&&guided.stableAfterMove==="Reorder Item 1, position 1 of 3",
