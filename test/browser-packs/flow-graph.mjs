...
try{
  ...
  const session=await evaluate(socket,`(async()=>{${helpers}
    ...
    evidence.runtime016={pointerTargetVisible,sourceHitsFrame,framePointerDown,pointerRegionAfter,pointerCoordinatesChanged,pointerIdentityStable,pointerReferencesStable,keyboardReturnedLeft,trustedFreePointerSequence:false,foreignPointerIgnored:false,pointerOwnerCommitted:false,trustedGroupedPointerSequence:false,groupedForeignPointerIgnored:false,groupedOwnerCompleted:false,protocolCancelGlobal:false,groupedPointerRejected,groupedKeyboardRejected,membershipGuidance,reloadRegions:false};
  ...
  const freeSetup=await evaluate(socket,`(async()=>{${helpers}
    await openFlow('Checkout journey');const frameId=${JSON.stringify(session.captures.freeLandingId)},frame=q('[data-free-page-frame-canvas="'+frameId+'"]',canvas());frame.focus();await pause();const rect=q('rect',frame).getBoundingClientRect(),target=q('[data-free-page-edge-target="after-lanes"]',canvas()).getBoundingClientRect();let source;for(let y=rect.top+3;y<rect.bottom&&!source;y+=8)for(let x=rect.left+3;x<rect.right;x+=8)if(document.elementFromPoint(x,y)?.closest('[data-free-page-frame-canvas]')===frame){source={x,y};break;}if(!source)throw new Error('No exposed trusted-pointer surface on free Page frame');window.__flowPointerEvents=[];for(const type of ['pointerdown','pointermove','pointerup','pointercancel'])window.addEventListener(type,(event)=>window.__flowPointerEvents.push({type:event.type,pointerId:event.pointerId,isTrusted:event.isTrusted,targetFrame:event.target?.closest?.('[data-free-page-frame-canvas]')?.dataset.freePageFrameCanvas??null}),true);return{frameId,source,target:{x:target.left+target.width/2,y:target.top+target.height/2}};
  })()`);
  const freeA=touchPoint(101,freeSetup.source.x,freeSetup.source.y),freeB=touchPoint(102,freeSetup.source.x+18,freeSetup.source.y+18),freeBEdge=touchPoint(102,freeSetup.target.x,freeSetup.target.y);
  await dispatchTouch("touchStart",[freeA]);
  const freeOwner=await evaluate(socket,`(()=>{${helpers}const frameId=${JSON.stringify(session.captures.freeLandingId)},frame=q('[data-free-page-frame-canvas="'+frameId+'"]',canvas()),events=window.__flowPointerEvents,owner=events.find(({type,targetFrame})=>type==='pointerdown'&&targetFrame===frameId);return{pointerId:owner?.pointerId,bytes:bytes(),revision:envelope().revision,stored:graphFor().graph.pageFrames.find(({id})=>id===frameId),transform:frame.getAttribute('transform'),status:q('[role="status"]',q('.documentary-flow')).textContent,focus:document.activeElement?.dataset.freePageFrameCanvas??null,targets:all('[data-free-page-edge-target]',canvas()).map(({dataset})=>dataset.freePageEdgeTarget).join('|'),captured:owner?frame.hasPointerCapture(owner.pointerId):false};})()`);
  await dispatchTouch("touchStart",[freeA,freeB]);
  await dispatchTouch("touchMove",[freeA,freeBEdge]);
  await dispatchTouch("touchEnd",[freeA]);
  const freeForeign=await evaluate(socket,`(()=>{${helpers}const frameId=${JSON.stringify(session.captures.freeLandingId)},frame=q('[data-free-page-frame-canvas="'+frameId+'"]',canvas()),events=window.__flowPointerEvents,ownerId=${JSON.stringify(freeOwner.pointerId)},foreign=events.find(({type,pointerId})=>type==='pointerdown'&&pointerId!==ownerId);return{foreignId:foreign?.pointerId,unchanged:bytes()===${JSON.stringify(freeOwner.bytes)}&&envelope().revision===${freeOwner.revision}&&JSON.stringify(graphFor().graph.pageFrames.find(({id})=>id===frameId))===${JSON.stringify(JSON.stringify(freeOwner.stored))}&&frame.getAttribute('transform')===${JSON.stringify(freeOwner.transform)}&&q('[role="status"]',q('.documentary-flow')).textContent===${JSON.stringify(freeOwner.status)}&&(document.activeElement?.dataset.freePageFrameCanvas??null)===${JSON.stringify(freeOwner.focus)}&&all('[data-free-page-edge-target]',canvas()).map(({dataset})=>dataset.freePageEdgeTarget).join('|')===${JSON.stringify(freeOwner.targets)}&&frame.hasPointerCapture(ownerId),events};})()`);
  const freeAEdge=touchPoint(101,freeSetup.target.x,freeSetup.target.y);
  await dispatchTouch("touchMove",[freeAEdge]);
  await dispatchTouch("touchEnd",[]);
  const freeFinished=await evaluate(socket,`(async()=>{${helpers}await pause();const frameId=${JSON.stringify(session.captures.freeLandingId)},frame=q('[data-free-page-frame-canvas="'+frameId+'"]',canvas()),stored=graphFor().graph.pageFrames.find(({id})=>id===frameId);return{committed:envelope().revision===${freeOwner.revision}+1&&bytes()!==${JSON.stringify(freeOwner.bytes)}&&stored.freePageRegion==='after-lanes'&&stored.id===frameId&&document.activeElement===frame,events:window.__flowPointerEvents};})()`);

  const cancelSetup=await evaluate(socket,`(async()=>{${helpers}const frameId=${JSON.stringify(session.captures.freeLandingId)},frame=q('[data-free-page-frame-canvas="'+frameId+'"]',canvas());frame.focus();await pause();const rect=q('rect',frame).getBoundingClientRect();window.__flowCancelEvents=[];window.addEventListener('pointercancel',(event)=>window.__flowCancelEvents.push({pointerId:event.pointerId,isTrusted:event.isTrusted}),true);return{frameId,source:{x:rect.left+30,y:rect.top+30},bytes:bytes(),revision:envelope().revision,stored:graphFor().graph.pageFrames.find(({id})=>id===frameId)};})()`);
  ...
  const cancelEvidence=await evaluate(socket,`(async()=>{${helpers}await pause();const frameId=${JSON.stringify(session.captures.freeLandingId)},frame=q('[data-free-page-frame-canvas="'+frameId+'"]',canvas()),events=window.__flowCancelEvents;return{globalCancel:events.length===2&&events.every(({isTrusted})=>isTrusted)&&new Set(events.map(({pointerId})=>pointerId)).size===2&&bytes()===${JSON.stringify(cancelSetup.bytes)}&&envelope().revision===${cancelSetup.revision}&&JSON.stringify(graphFor().graph.pageFrames.find(({id})=>id===frameId))===${JSON.stringify(JSON.stringify(cancelSetup.stored))}&&document.activeElement===frame};})()`);

  const groupedSetup=await evaluate(socket,`(async()=>{${helpers}const frameId=${JSON.stringify(session.captures.cartFrameId)},frame=q('[data-page-frame-id="'+frameId+'"]',canvas());frame.focus();await pause();const rect=q('rect',frame).getBoundingClientRect(),lanes=all('[data-lane-dropzone]',canvas()).map((lane)=>lane.getBoundingClientRect()),target=lanes.at(-1);window.__groupPointerEvents=[];for(const type of ['pointerdown','pointermove','pointerup','pointercancel'])window.addEventListener(type,(event)=>window.__groupPointerEvents.push({type:event.type,pointerId:event.pointerId,isTrusted:event.isTrusted,targetFrame:event.target?.closest?.('[data-page-frame-id]')?.dataset.pageFrameId??null}),true);return{frameId,source:{x:rect.left+30,y:rect.top+30},target:{x:target.left+target.width/2,y:rect.top+30},bytes:bytes(),revision:envelope().revision,stored:graphFor().graph.pageFrames.find(({id})=>id===frameId),transform:frame.getAttribute('transform'),status:q('[role="status"]',q('.documentary-flow')).textContent,focus:document.activeElement?.dataset.pageFrameId??null};})()`);
  const groupedA=touchPoint(105,groupedSetup.source.x,groupedSetup.source.y),groupedB=touchPoint(106,groupedSetup.source.x+18,groupedSetup.source.y+18),groupedBTarget=touchPoint(106,groupedSetup.target.x,groupedSetup.target.y);
  await dispatchTouch("touchStart",[groupedA]);
  const groupedOwnerId=await evaluate(socket,"window.__groupPointerEvents.find(({type})=>type==='pointerdown')?.pointerId");
  await dispatchTouch("touchStart",[groupedA,groupedB]);
  await dispatchTouch("touchMove",[groupedA,groupedBTarget]);
  await dispatchTouch("touchEnd",[groupedA]);
  const groupedForeign=await evaluate(socket,`(()=>{${helpers}const frameId=${JSON.stringify(session.captures.cartFrameId)},frame=q('[data-page-frame-id="'+frameId+'"]',canvas()),events=window.__groupPointerEvents,ownerId=${JSON.stringify(groupedOwnerId)},foreign=events.find(({type,pointerId})=>type==='pointerdown'&&pointerId!==ownerId);return{foreignId:foreign?.pointerId,unchanged:bytes()===${JSON.stringify(groupedSetup.bytes)}&&envelope().revision===${groupedSetup.revision}&&JSON.stringify(graphFor().graph.pageFrames.find(({id})=>id===frameId))===${JSON.stringify(JSON.stringify(groupedSetup.stored))}&&frame.getAttribute('transform')===${JSON.stringify(groupedSetup.transform)}&&q('[role="status"]',q('.documentary-flow')).textContent===${JSON.stringify(groupedSetup.status)}&&(document.activeElement?.dataset.pageFrameId??null)===${JSON.stringify(groupedSetup.focus)}&&frame.hasPointerCapture(ownerId),events};})()`);
  await dispatchTouch("touchEnd",[]);
  const groupedFinished=await evaluate(socket,`(async()=>{${helpers}await pause();const frameId=${JSON.stringify(session.captures.cartFrameId)};return{completed:bytes()===${JSON.stringify(groupedSetup.bytes)}&&envelope().revision===${groupedSetup.revision}&&JSON.stringify(graphFor().graph.pageFrames.find(({id})=>id===frameId))===${JSON.stringify(JSON.stringify(groupedSetup.stored))}&&document.activeElement?.dataset.pageFrameId===frameId,events:window.__groupPointerEvents};})()`);
  const freeRelevant=freeFinished.events.filter(({pointerId})=>[freeOwner.pointerId,freeForeign.foreignId].includes(pointerId)),groupedRelevant=groupedFinished.events.filter(({pointerId})=>[groupedOwnerId,groupedForeign.foreignId].includes(pointerId));
  ...
  session.evidence.runtime016.trustedGroupedPointerSequence=Boolean(groupedOwnerId&&groupedForeign.foreignId&&groupedOwnerId!==groupedForeign.foreignId&&groupedRelevant.length&&groupedRelevant.every(({isTrusted})=>isTrusted));
  session.evidence.runtime016.groupedForeignPointerIgnored=groupedForeign.unchanged;
  session.evidence.runtime016.groupedOwnerCompleted=groupedFinished.completed;
