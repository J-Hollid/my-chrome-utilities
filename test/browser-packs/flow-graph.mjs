...
const dragComponent=async(component,target,offsetX=0,offsetY=0)=>{target.scrollIntoView({block:'center',inline:'center'});await pause();const rect=target.getBoundingClientRect();flowNativePointer(JSON.stringify({type:'drag',x:rect.left+rect.width/2+offsetX,y:rect.top+rect.height/2+offsetY,component:{kind:component.dataset.componentKind,id:component.dataset.componentId}}));await nativeDone();await pause();};
const stableGraphCapture=(flowName)=>{const {stored,flow,graph}=graphFor(flowName);return{flowId:flow.id,laneOrder:[...graph.pageGroupIds],frames:graph.pageFrames.map(({id,pageId,pageGroupId,position})=>({id,pageId,pageGroupId,position})),occurrences:graph.occurrences.map(({id,pageFrameId,freePageFrameId,pageId,pageGroupId,contextBindingId,eventId,position})=>({id,pageFrameId,freePageFrameId,pageId,pageGroupId,contextBindingId,eventId,position})),relationships:graph.relationships.map(({id,sourceNodeId,targetNodeId,kind,group,label,documentationCondition,expectation})=>({id,sourceNodeId,targetNodeId,kind,group,label,documentationCondition,expectation})),selectedItem:graph.selectedItem,viewport:graph.viewport??{x:0,y:0,zoom:1},entityIds:{groups:stored.project.collections.pageGroups.map(({id})=>id),pages:stored.project.collections.pages.map(({id})=>id),events:stored.project.collections.events.map(({id})=>id)}};};
`;
...
try{
  ...
  socket.on("Runtime.bindingCalled",async({name,payload})=>{const action=JSON.parse(payload);try{if(name==="flowNativeKey"){const key=action.key,code=key===" "?"Space":key,windowsVirtualKeyCode=key==="Enter"?13:key==="Escape"?27:key.startsWith("Arrow")?{ArrowLeft:37,ArrowUp:38,ArrowRight:39,ArrowDown:40}[key]:key.charCodeAt(0);await socket.call("Input.dispatchKeyEvent",{type:"keyDown",key,code,windowsVirtualKeyCode,nativeVirtualKeyCode:windowsVirtualKeyCode});await socket.call("Input.dispatchKeyEvent",{type:"keyUp",key,code,windowsVirtualKeyCode,nativeVirtualKeyCode:windowsVirtualKeyCode});}else if(action.type==="drag"){const dragData={items:[{mimeType:"application/x-flow-component",data:JSON.stringify(action.component)}],dragOperationsMask:1};for(const type of ["dragEnter","dragOver","drop"])await socket.call("Input.dispatchDragEvent",{type,x:action.x,y:action.y,data:dragData});}else await socket.call("Input.dispatchMouseEvent",{type:action.type,x:action.x,y:action.y,button:action.type==="mouseMoved"?"none":"left",buttons:action.type==="mouseReleased"?0:1,clickCount:1});}catch(error){console.error("Native input failed",error);}});
  ...
  const session=await evaluate(socket,`(async()=>{${helpers}
    ...
    for(const name of ['Cart','Payment','Shipping','Thank you','Landing'])await add('pages',name);
    ...
    const landing=stored.project.collections.pages.find(({name})=>name==='Landing'),landingBinding=landing.contextEventBindings[0];
    await dragComponent(catalogButton('Pages','Landing'),q('[data-ungrouped-dropzone]',canvas()));
    ({graph}=graphFor());const freeLanding=graph.occurrences.find(({freePageFrame,pageId})=>freePageFrame&&pageId===landing.id);
    const exactFreeFrame=Boolean(freeLanding)&&freeLanding.pageId===landing.id&&freeLanding.contextBindingId===landingBinding.id&&!freeLanding.pageGroupId&&!freeLanding.pageFrameId;
    await dragComponent(catalogButton('Events','route_view'),q('[data-free-page-frame-id="'+freeLanding.id+'"]'));
    ({graph}=graphFor());const freeInteraction=graph.occurrences.find(({freePageFrameId})=>freePageFrameId===freeLanding.id);
    const freeInteractionExact=Boolean(freeInteraction)&&freeInteraction.eventId===events.route_view.id&&freeInteraction.pageId===landing.id&&!freeInteraction.pageGroupId;
    ...
    evidence.runtime007={exactFreeFrame,freeInteractionExact,freeRelationshipStarted,landingWrongDrop};
    ...
    return{evidence,captures:{checkout:stableGraphCapture('Checkout journey'),minimal:stableGraphCapture('Minimal journey'),paymentOccurrenceId:paymentOccurrence.id,minimalSelectedId:minimalPayment.id},installedBoundary:location.protocol==='chrome-extension:'&&chrome.runtime.id===location.hostname};
  })()`);
  ...
  const reloadEvidence=await evaluate(socket,`(async()=>{${helpers}
    ...
    return{checkoutStable,minimalStable};
  })()`);
  ...
  for(let runtime=1;runtime<=15;runtime+=1){const key=`runtime${String(runtime).padStart(3,"0")}`;assert.equal(Object.values(session.evidence[key]).every(Boolean),true,`${key}: ${JSON.stringify(session.evidence[key])}`);}
