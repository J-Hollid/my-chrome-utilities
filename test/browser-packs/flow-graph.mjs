...
try{
  ...
  const session=await evaluate(socket,`(async()=>{${helpers}
    ...
    q('[data-free-page-frame-canvas="'+freeLanding.id+'"]',canvas()).focus();await nativeKey('ArrowLeft');({graph}=graphFor());const landingAfterKeyboard=graph.pageFrames.find(({id})=>id===freeLanding.id),landingCanvas=q('[data-free-page-frame-canvas="'+freeLanding.id+'"]',canvas()),landingTransform=landingCanvas.getAttribute('transform'),keyboardReturnedLeft=landingAfterKeyboard.freePageRegion==='before-lanes'&&document.activeElement?.dataset.freePageFrameCanvas===freeLanding.id&&landingTransform.startsWith('translate('+(landingAfterKeyboard.position.x-10)+' ');
    ...
    evidence.runtime016={pointerTargetVisible,sourceHitsFrame,framePointerDown,pointerRegionAfter,pointerCoordinatesChanged,pointerIdentityStable,pointerReferencesStable,keyboardReturnedLeft,groupedPointerRejected,groupedKeyboardRejected,membershipGuidance,reloadRegions:false};
  ...
  const legacyFixture=await evaluate(socket,`(()=>{${helpers}
    const stored=envelope(),flow=stored.project.collections.flows.find(({name})=>name==='Checkout journey'),graph=stored.project.documentationFlowGraphs[flow.id],pageView=stored.project.collections.events.find(({name})=>name==='page_view'),routeView=stored.project.collections.events.find(({name})=>name==='route_view'),pageViewOccurrence=graph.occurrences.find(({eventId,role})=>eventId===pageView.id&&role==='context-setting'),routeViewOccurrence=graph.occurrences.find(({eventId,role,pageFrameId})=>eventId===routeView.id&&role==='context-setting'&&pageFrameId===pageViewOccurrence.pageFrameId),cart=stored.project.collections.pages.find(({id})=>id===pageViewOccurrence.pageId),pageBindingId='binding:legacy-page-view',routeBindingId='binding:legacy-route-view';
    cart.name='Cart';
    ...
    const topology=()=>JSON.stringify({frames:graph.pageFrames,occurrences:graph.occurrences.map(({id,pageFrameId,pageId,pageGroupId,position,obligation,minimum,maximum,optional})=>({id,pageFrameId,pageId,pageGroupId,position,obligation,minimum,maximum,optional})),relationships:graph.relationships});
    return{preProject:stored.project,topology:topology(),flowId:flow.id,pageId:cart.id,pageViewId:pageView.id,routeViewId:routeView.id,occurrenceIds:[pageViewOccurrence.id,routeViewOccurrence.id],bindingIds:[pageBindingId,routeBindingId]};
  })()`);
  await reload(socket);
  session.evidence.runtime017=await evaluate(socket,`(async()=>{${helpers}
    const fixture=${JSON.stringify(legacyFixture)};
    await openFlow('Checkout journey');
    const review=q('[aria-label="Flow Page-context migration review"]'),reviewText=review.textContent,rows=all('li',review).map(({textContent})=>textContent),humanReview=rows.length===2&&rows.some((text)=>text.includes('Cart / page_view')&&text.includes('Initial load')&&text.includes('affected occurrence'))&&rows.some((text)=>text.includes('Cart / route_view')&&text.includes('SPA route change')&&text.includes('affected occurrence')),noRawIds=[...fixture.occurrenceIds,...fixture.bindingIds,fixture.pageId,fixture.pageViewId,fixture.routeViewId].every((id)=>!reviewText.includes(id));
    ...
    return{humanReview,noRawIds,topologyPreserved,directOccurrences,canonicalBindingFree,undoRestored};
  })()`);
