...
try {
    ...
    const evaluateSeed = async (expression) => {
      await evaluate(`(()=>{globalThis.__flowSeedState={status:'pending'};Promise.resolve(${expression})
        .then(value=>{globalThis.__flowSeedState={status:'passed',value};},error=>{
          globalThis.__flowSeedState={status:'failed',message:String(error?.message??error)};
        });return true;})()`, "seed start");
      const settled = await observeBrowserReadiness({targetId,phase:"fixture",
        predicateDescription:"detached Flow seed promise settlement",timeoutMs:120000,
        pollIntervalMs:20,maximumSnapshotCharacters:500,
        observe:async()=>evaluate("globalThis.__flowSeedState", "seed settlement"),
        ready:state=>state?.status!=="pending",snapshot:state=>state});
      await evaluate("delete globalThis.__flowSeedState", "seed cleanup");
      if (settled.status === "failed") throw new Error(settled.message);
