...
const project={
  ...
  documentationFlowGraphs:{"flow:checkout":{
    ...
    relationships:[
      {id:"relationship:cart-confirm",sourceEndpoint:{kind:"page-frame",id:"frame:cart"},targetEndpoint:{kind:"page-frame",id:"frame:confirmation-a"},sourcePort:"right",targetPort:"left",kind:"expected_next",label:"Success route"},
      {id:"relationship:cart-confirm-b",sourceEndpoint:{kind:"page-frame",id:"frame:cart"},targetEndpoint:{kind:"page-frame",id:"frame:confirmation-b"},sourcePort:"top",targetPort:"bottom",kind:"alternative",label:"Skip route"},
...
const outlineEvents=[
  {type:"observed",id:"live-100",name:"page_view",sourceId:"history",sourceName:"Event history",sourceKind:"Data layer",timestamp:"2026-07-23T10:00:00.000Z",pageUrl:"https://shop.test/payment",payload:{site:"shop",currency:"EUR",payment_id:"payment-before"}},
  ...
  {type:"observed",id:"live-102",name:"pageview",sourceId:"history",sourceName:"Event history",sourceKind:"Data layer",timestamp:"2026-07-23T10:00:02.000Z",pageUrl:"https://shop.test/payment",payload:{site:"shop",currency:"EUR",payment_id:"payment-1",oForm:{formStepName:"review"}}},
  {type:"observed",id:"live-103",name:"add_payment_info",sourceId:"history",sourceName:"Event history",sourceKind:"Data layer",timestamp:"2026-07-23T10:00:03.000Z",pageUrl:"https://shop.test/payment",payload:{site:"shop",currency:"EUR",payment_id:"payment-1",event_name:"add_payment_info",method:"card"}},
];
...
const primaryObservation=await runRenderedWorkflow("live-flow-testing",`${workflowPreamble}
...
const next=q('#live-flow-step-selector'),outgoingOnly=[...next.options].slice(1).map(option=>option.value).join('|')==='occurrence:view|frame:confirmation-a|frame:confirmation-b'&&q('#live-flow-event-link').textContent.includes('contained Event or an outgoing related Page');
next.value='occurrence:view';click('Link event to Flow step');await until(()=>q('#live-flow-event-link')?.textContent.includes('Recorded graph step occurrence:view'));
const createDefectActions=[...q('#live-event-inspector').querySelectorAll('button')].filter(button=>button.textContent==='Create defect report'),earlierCaptureLinked=Boolean(q('#live-event-validation-issues li')&&q('#live-event-validation-issues').textContent.includes('Manual Flow test')&&createDefectActions.length===1&&createDefectActions[0].id==='live-inspector-action-create-defect-report'&&!document.querySelector('#live-manual-flow-validation')&&!document.querySelector('.live-flow-create-defect'));
const clipboard={plain:[]};Object.defineProperty(navigator,'clipboard',{configurable:true,value:{writeText:async(value)=>clipboard.plain.push(value)}});q('#live-inspector-action-create-defect-report').click();await until(()=>[...document.querySelectorAll('button')].some(button=>button.textContent==='Save defect'));
...
const branch=q('#live-flow-step-selector'),branchChoices=[...branch.options].slice(1).map(option=>option.value).join('|'),cursorStable=branchChoices==='occurrence:view|frame:confirmation-a|frame:confirmation-b';
...
const primaryEarlierCaptureLinked=${JSON.stringify(primaryObservation.evidence.earlierCaptureLinked)};
...
q('[data-event-id="live-102"]').click();await until(()=>q('#live-flow-step-selector'));selector=q('#live-flow-step-selector');const relationshipRows=[...selector.options].slice(1).filter(option=>option.value.startsWith('frame:')).map(option=>{const [name,relationship_kind,display_name,id]=option.textContent.split(' · ');return{relationship_kind,next_step:id.startsWith('frame:')?name+' Page frame':name+' Event occurrence',label_state:display_name==='Cart to '+name?'no label':'label '+display_name,display_name};});
...
q('#back-to-events').click();q('[data-event-id="live-103"]').click();await until(()=>q('#live-flow-step-selector'));const outgoingBeforeDefect=[...q('#live-flow-step-selector').options].slice(1).map(option=>option.value).join('|');q('#back-to-events').click();q('[data-event-id="live-102"]').click();await until(()=>q('#live-flow-event-link')?.textContent.includes('frame:payment'));
...
const flowIssuePresentation=q('#live-event-validation-issues')?.textContent??'',flowContextPresentation=q('#live-flow-event-link')?.textContent??'',flowCreateActions=[...q('#live-event-inspector').querySelectorAll('button')].filter(button=>button.textContent==='Create defect report'),singleDefectPath=flowIssuePresentation.includes('expected "payment"')&&flowIssuePresentation.includes('actual "review"')&&flowContextPresentation.includes('Recorded graph step frame:payment')&&flowCreateActions.length===1&&flowCreateActions[0].id==='live-inspector-action-create-defect-report'&&!document.querySelector('#live-manual-flow-validation')&&!document.querySelector('.live-flow-create-defect');
q('#live-inspector-action-create-defect-report').click();await until(()=>q('#live-event-inspector h4')?.textContent==='Defect report: Checkout journey · Payment · pageview');
...
const ordinaryBuilderControls=Boolean(q('#defect-issue-formStepName')?.checked&&q('[aria-label="Report sections"]')&&q('[aria-label="Final report preview"]')?.textContent.includes('"oForm"')&&q('[aria-label="Final report preview"]')?.textContent.includes('"review"')&&['Copy for Jira Cloud','Save defect','Save defect and copy'].every(text=>[...document.querySelectorAll('button')].some(button=>button.textContent===text)));
const reportMeaning=relationshipPreview.includes('pageview does not satisfy Payment in Checkout journey')&&relationshipPreview.includes('does not satisfy the linked Payment Flow-step expectation')&&!/Flow definition (?:is )?defective|Checkout journey (?:failed|executed)/i.test(relationshipPreview);
const relationshipDefectUi=singleDefectPath&&relationshipIssue.includes('observed "review"')&&relationshipIssue.includes('expected "payment"')&&relationshipIssue.includes('rule EXPECTED_VALUE')&&relationshipAssistance.includes('Use Payment Flow-step expectation')&&relationshipAssistance.includes('effective schema revision')&&!/generic constraint|manually selected Flow step/i.test(relationshipIssue+relationshipAssistance)&&relationshipPreview.includes('path Cart to Payment')&&ordinaryBuilderControls&&reportMeaning;
const flowExpectationControl=[...q('[aria-label="formStepName expected-result assistance"]').querySelectorAll('input')].find(input=>input.dataset.responseSource?.startsWith('Use Payment Flow-step expectation'));flowExpectationControl.click();await until(()=>q('[aria-label="Final report preview"]').textContent.includes('formStepName: "payment"'),'exact Flow expectation correction');
const correctedPreview=q('[aria-label="Final report preview"]'),exactCorrectionPreview=Boolean(correctedPreview.querySelector('[data-operation="replace"][data-json-pointer="/oForm/formStepName"]')&&correctedPreview.querySelector('[data-json-pointer="/oForm/formStepName"][style*="background-color:#d9f7d9"]')&&correctedPreview.textContent.includes('formStepName: "payment"')&&correctedPreview.textContent.includes('source Payment Flow-step expectation · effective schema revision'));
...
const relationshipStored=JSON.parse(localStorage.getItem('my-chrome-utilities.defect-library.v1')).defects[0].report,relationshipSnapshot=relationshipStored.event.flowContext,storedCorrection=relationshipStored.expected.corrections.find(correction=>correction.pointer==='/oForm/formStepName'),relationshipDurable=relationshipSnapshot.flowId==='flow:checkout'&&relationshipSnapshot.selectedStepId==='frame:payment'&&relationshipSnapshot.eventId==='live-102'&&relationshipSnapshot.eventStepLink.eventId==='live-102'&&relationshipSnapshot.linkEvidence.label==='path Cart to Payment'&&relationshipSnapshot.linkEvidence.relationshipIds?.includes('relationship:cart-payment')&&relationshipSnapshot.effectiveTarget.name==='Payment'&&relationshipSnapshot.effectiveSchemaRevision>0&&relationshipSnapshot.provenance.some(item=>item.scope==='Flow Page-instance')&&relationshipStored.expected.payload.oForm.formStepName==='payment'&&storedCorrection?.operation==='replace'&&storedCorrection?.response==='payment'&&storedCorrection?.marker==='+'&&storedCorrection?.responseSource==='Payment Flow-step expectation · effective schema revision '+relationshipSnapshot.effectiveSchemaRevision&&outlineClipboard[0].includes('path Cart to Payment')&&outlineClipboard[0].includes('live-102')&&outlineClipboard[0].includes(relationshipSnapshot.effectiveSchemaRevisionIdentity)&&outlineClipboard[0].includes('formStepName: "payment"')&&outlineClipboard[0].includes('source Payment Flow-step expectation · effective schema revision');
...
q('[data-event-id="live-103"]').click();await until(()=>q('#live-flow-step-selector'));selector=q('#live-flow-step-selector');const outgoingAfterDefect=[...selector.options].slice(1).map(option=>option.value).join('|'),outgoingStable=outgoingAfterDefect===outgoingBeforeDefect&&outgoingAfterDefect==='occurrence:add-payment';selector.value='occurrence:add-payment';click('Link event to Flow step');await until(()=>q('#live-flow-event-link')?.textContent.includes('occurrence:add-payment'));
