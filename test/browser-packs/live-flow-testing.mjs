...
q('[data-event-id="live-102"]').click();await until(()=>q('#live-flow-step-selector'));selector=q('#live-flow-step-selector');const relationshipRows=[...selector.options].slice(1).map(option=>{const [name,relationship_kind,display_name,id]=option.textContent.split(' · ');return{relationship_kind,next_step:id.startsWith('frame:')?name+' Page frame':name+' Event occurrence',label_state:display_name==='Cart to '+name?'no label':'label '+display_name,display_name};});
...
const relationshipStored=JSON.parse(localStorage.getItem('my-chrome-utilities.defect-library.v1')).defects[0].report,relationshipSnapshot=relationshipStored.event.flowContext,relationshipDurable=relationshipSnapshot.flowId==='flow:checkout'&&relationshipSnapshot.selectedStepId==='frame:payment'&&relationshipSnapshot.eventId==='live-102'&&relationshipSnapshot.eventStepLink.eventId==='live-102'&&relationshipSnapshot.linkEvidence.label==='path Cart to Payment'&&relationshipSnapshot.effectiveTarget.name==='Payment'&&relationshipSnapshot.effectiveSchemaRevision>0&&relationshipSnapshot.provenance.some(item=>item.scope==='Flow Page-instance')&&outlineClipboard[0].includes('path Cart to Payment')&&outlineClipboard[0].includes('live-102')&&outlineClipboard[0].includes(relationshipSnapshot.effectiveSchemaRevisionIdentity);
