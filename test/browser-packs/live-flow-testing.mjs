...
const host=q('#live-flow-test'),guidance=host.textContent.includes('No Flow, event, or Assignment is selected automatically.'),flow=q('#live-flow-selector'),onlyActive=[...flow.options].map(option=>option.textContent).join('|')==='Choose Flow|Checkout journey'&&!flow.value;
...
const pageResult=host.textContent.includes('revision 2 · Valid');click('Cart to Cart page_view · expected_next');
...
const invalid=host.textContent.includes('Confirmation · live-103')&&host.textContent.includes('revision 2 · Invalid'),defectAction=[...host.querySelectorAll('button')].filter(button=>button.textContent==='Create defect report').at(-1);
...
const restored=q('#live-flow-test').textContent.includes('Completed selected path')&&q('#live-flow-test').textContent.includes('restored read-only; matching was not resumed')&&!q('#live-flow-test').querySelector('select')&&/defect defect:/.test(q('#live-flow-test').textContent),stored=JSON.parse(localStorage.getItem('my-chrome-utilities.saved-session-library.v1')),savedSummary=stored.sessions[0].flowTests[0],durable=savedSummary.history.at(-1).effectiveSchemaRevision===2&&savedSummary.history.at(-1).issues.map(issue=>issue.path).sort().join('|')==='/currency|/site'&&savedSummary.history.at(-1).defectId?.startsWith('defect:')&&savedSummary.history.every(entry=>!('contributors' in entry));
...
const evidence={installed,guidance,onlyActive,starts,manualCandidates,pageResult,reasons,occurrence,invalid,defectPath,defectDurable,linked,completed,restored,durable,unchanged};return{passed:Object.values(evidence).every(Boolean),width:innerWidth,overflow:document.documentElement.scrollWidth>innerWidth,evidence};`,{preload,fullPanel:true});
