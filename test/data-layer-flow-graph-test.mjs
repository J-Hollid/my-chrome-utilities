...
import {addGraphOccurrence,addInteractionOccurrenceToPage,addUngroupedPageFrame,applyFlowPageGroupLaneSelection,documentaryFlowGraph,flowOccurrenceEventSchema,flowOutline,flowRelationshipText,inspectFlowGraph,inspectOccurrenceContainmentMove,inspectUngroupedPageDrop,moveGraphOccurrence,projectFlowGraph,removeGraphOccurrence,reorderGraphOccurrence,saveGraphRelationship,setFlowPageGroupLanes,updateGraphOccurrence} from "../dist/data-layer-flow-graph.js";
...
const landingPage=laneAdd("pages",{name:"Landing Page",contextEventBindings:[{id:"context-binding:landing",name:"page_view",eventId:laneRoute.id}]});const lanesBeforeFree=structuredClone(documentaryFlowGraph(laneState.project,laneFlow.id).pageGroupIds),groupsBeforeFree=structuredClone(laneState.project.collections.pageGroups);laneState=addUngroupedPageFrame(laneState,laneFlow.id,{name:"Landing Page entry",pageId:landingPage.id,contextBindingId:"context-binding:landing",y:70},laneId);const freeFrame=documentaryFlowGraph(laneState.project,laneFlow.id).occurrences.at(-1);assert.equal(freeFrame.freePageFrame,true);assert.equal("pageGroupId" in freeFrame,false);assert.equal("lane" in freeFrame,false);assert.deepEqual(documentaryFlowGraph(laneState.project,laneFlow.id).pageGroupIds,lanesBeforeFree);assert.equal("pageGroupIds" in laneState.project.collections.flows.find(({id})=>id===laneFlow.id),false);assert.deepEqual(laneState.project.collections.pageGroups,groupsBeforeFree);const beforeCrossRegionMove=laneState;assert.equal(moveGraphOccurrence(laneState,laneFlow.id,freeFrame.id,{pageGroupId:checkoutGroup.id,y:190}),beforeCrossRegionMove,"free Page frames cannot move into named Page Group lanes");laneState=moveGraphOccurrence(laneState,laneFlow.id,freeFrame.id,{y:90});laneState=updateGraphOccurrence(laneState,laneFlow.id,freeFrame.id,{name:"Landing Page entry",freePageFrame:true,pageId:landingPage.id,contextBindingId:"context-binding:landing",obligation:"Required",minimum:1,maximum:1,y:90});const editedFreeFrame=documentaryFlowGraph(laneState.project,laneFlow.id).occurrences.find(({id})=>id===freeFrame.id);assert.deepEqual(editedFreeFrame.position,{y:90});assert.equal(editedFreeFrame.freePageFrame,true);assert.equal("lane" in editedFreeFrame,false);assert.equal("x" in editedFreeFrame.position,false);laneState=addInteractionOccurrenceToPage(laneState,laneFlow.id,{name:"Landing Purchase",freePageFrameId:freeFrame.id,pageId:landingPage.id,eventId:lanePurchase.id,obligation:"Required",minimum:1,maximum:1,y:190},laneId);const landingPurchase=documentaryFlowGraph(laneState.project,laneFlow.id).occurrences.at(-1);laneState=saveGraphRelationship(laneState,laneFlow.id,landingPurchase.id,{toStepId:purchaseOccurrence.id,kind:"expected-next"},laneId);assert.equal(projectFlowGraph(laneState.project,laneFlow.id).graph.relationships.at(-1).sourceNodeId,landingPurchase.id);const rejectedDrop=inspectUngroupedPageDrop(laneState.project,laneFlow.id,landingPage.id,checkoutGroup.id);assert.equal(rejectedDrop.rejected,true);assert.match(rejectedDrop.guidance,/kind=pages.*entity=.*field=pageGroupIds/);
