...
state=saveGraphRelationship(state,flow.id,context.id,{toStepId:interaction.id,sourcePort:"right",targetPort:"left",group:"checkout",label:"continue",documentationCondition:"checkout is open",expectation:"payment follows context",condition:{kind:"predicate",field:"forbidden",operator:"equals",value:"x"}},id);
...
assert.deepEqual(migratedRelationship,{id:"relationship:legacy-parallel",sourceEndpoint:{kind:"event-occurrence",id:context.id},targetEndpoint:{kind:"event-occurrence",id:interaction.id},sourcePort:"top",targetPort:"bottom",kind:"alternative",group:"legacy",documentationCondition:"documented",expectation:"manual"});
...
assert.equal(saveGraphRelationship(canvasState,canvasFlow.id,canvasContext.id,{toStepId:canvasContext.id,sourcePort:"right",targetPort:"left"},canvasId),canvasState,"self-connections are atomic no-ops");
...
for(const [source,target] of [[customerFrame.id,paymentFrame.id],[customerFrame.id,eventB.id],[eventA.id,paymentFrame.id],[eventA.id,eventB.id]])topologyState=saveGraphRelationship(topologyState,topologyFlow.id,source,{toStepId:target,sourcePort:"right",targetPort:"left"},topologyId);
