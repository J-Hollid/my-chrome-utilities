...
import {writeProjectDocumentationWorkbookWithTemplates} from "../dist/documentation-templates/excel-renderer.js";
...
for(let sample=0;sample<160;sample+=1){const left=2+templateRandom()%12,top=2+templateRandom()%30,width=2+templateRandom()%5,height=2+templateRandom()%5,right=left+width-1,bottom=top+height-1,range=`${columnLetters(left)}${top}:${columnLetters(right)}${bottom}`,prototype={kind:"matrix",contractVersion:2,worksheetName:"Template",cells:[{address:`${columnLetters(left)}${top}`,value:"{{row.property}}"}],areas:[{name:"RowPattern",type:"repeat",source:"matrix.rows",direction:"down",range}],merges:[]},valid=(merge)=>validateExcelTemplatePrototype({...prototype,merges:[merge]}).valid;assert.equal(valid(range),true,"a merge wholly contained by a repeat remains valid");assert.equal(valid(`${columnLetters(right+2)}${top}:${columnLetters(right+3)}${bottom}`),true,"a merge disjoint from a repeat remains valid");assert.equal(valid(`${columnLetters(left-1)}${top}:${columnLetters(left)}${bottom}`),false,"a merge crossing a repeat edge is rejected");assert.equal(valid(`${columnLetters(left-1)}${top-1}:${columnLetters(right+1)}${bottom+1}`),false,"a merge enclosing a repeat is rejected");}
for(let sample=0;sample<24;sample+=1){const start=2+templateRandom()%8,rowCount=2+templateRandom()%4,cellCount=2+templateRandom()%4,repeatedHeight=20+templateRandom()%40,repeatedWidth=20+templateRandom()%40,staticHeight=20+templateRandom()%40,staticWidth=20+templateRandom()%40,book=new ExcelJS.Workbook(),sheet=book.addWorksheet("Template"),guide=book.addWorksheet("Template Guide");sheet.getCell(`A${start}`).value="{{row.property}}";sheet.getCell(`B${start}`).value="{{cell.value}}";sheet.getRow(start+1).height=repeatedHeight;sheet.getColumn(3).width=repeatedWidth;sheet.getRow(start+3).height=staticHeight;sheet.getColumn(6).width=staticWidth;book.definedNames.add(`'Template'!$A$${start}:$C$${start+1}`,"RowPattern");book.definedNames.add(`'Template'!$B$${start}:$C$${start}`,"CellPattern");guide.addTable({name:"TemplateSettings",ref:"A1",headerRow:true,columns:[{name:"Setting"},{name:"Value"}],rows:[["Contract",2],["Kind","matrix"]]});guide.addTable({name:"TemplateAreas",ref:"A6",headerRow:true,columns:[{name:"Area"},{name:"Type"},{name:"Source"},{name:"Direction"}],rows:[["RowPattern","Repeat","matrix.rows","Down"],["CellPattern","Repeat","row.cells","Across"]]});const bytes=new Uint8Array(await book.xlsx.writeBuffer()),digest=`property:${sample}`,record={id:digest,name:"Property",format:"excel",kind:"matrix",contractVersion:2,digest,body:{assetId:digest,digest,byteLength:bytes.length},validation:{valid:true,findings:[]}},rows=Array.from({length:rowCount},(_,row)=>({property:`/${row}`,concept:"",cells:Array.from({length:cellCount},(_,column)=>({columnKey:String(column),heading:String(column),value:`${row}:${column}`}))})),snapshot={projectId:"project",projectName:"Shop",projectPurpose:"",projectWebsite:"shop.example",generatedAt:"2026-08-19T00:00:00.000Z",title:"Set",incomplete:false,snapshotHash:"snapshot",sourceRevisions:{},set:{id:"set",name:"Set",themeId:"theme",sections:[{id:"matrix",name:"Matrix",kind:"matrix",selected:true}],templateAssignments:{"excel:matrix":record.id}},theme:{id:"theme",name:"Theme",clientName:"",logo:"",headerText:"",footerText:"",typography:{family:"Arial",headingSize:16,bodySize:11},colors:{heading:"#000000",accent:"#000000",stripe:"#ffffff"},density:"comfortable",borders:true,striping:true,highlightedHeadings:true,columnWidths:{}},templateDigests:{"excel:matrix":digest},templates:[record],tables:[{id:"matrix",title:"Matrix",headings:[],rows:[],templateData:{columns:[],rows,concepts:[],legend:""}}],diagnostics:[]},output=await writeProjectDocumentationWorkbookWithTemplates(snapshot,{scope:"current",currentSectionId:"matrix"},async()=>new Blob([bytes])),rendered=new ExcelJS.Workbook();await rendered.xlsx.load(output);const result=rendered.worksheets[0];for(let row=0;row<rowCount;row+=1)assert.equal(result.getRow(start+1+row*2).height,repeatedHeight,"blank row height is conserved through every outer repeat");for(let column=0;column<cellCount;column+=1)assert.equal(result.getColumn(3+column*2).width,repeatedWidth,"blank column width is conserved through every nested repeat");assert.equal(result.getRow(start+1+rowCount*2).height,staticHeight,"blank static row height shifts after repeated expansion");assert.equal(result.getColumn(6).width,staticWidth,"blank static column width is conserved outside repeated expansion");}
...
const propertyConfiguration={selectedPaths:["/unsafe"],metadata:["type","description"],headingParts:{step:true,page:true,event:true}};
for(let sample=0;sample<120;sample+=1){
  const head=`group_${sample}`,tail=`property_${templateRandom()%10000}`,canonicalPaths=[`/${head}/${tail}`,`/${head}.${tail}`,`/${head}/*/${tail}`,`/${head}[x]/${tail}`],properties=Object.fromEntries(canonicalPaths.map((path,index)=>[path,{presence:"required",expectedValue:`value:${sample}:${index}`,documentation:`metadata:${sample}:${index}`,origins:[{contributorId:"profile",contributorName:"Profile",scope:"Shared Profile"}],superseded:[]}])) ,collisionSnapshot=compileFlowDocumentationSnapshot({projectId:`project:${sample}`,projectName:"Shop",flowId:`flow:${sample}`,flowName:"Collision",graphRevision:sample,sourceState:"published",generatedAt:"2026-07-20T00:00:00.000Z",contexts:[{id:`context:${sample}`,kind:"page-instance",pageFrameId:`frame:${sample}`,pageName:"Page",eventName:"page_view",stepLabel:"1",effectiveRevision:1,compiled:{status:"ready",properties,conflicts:[],provenance:[],exclusions:[]}}]}),selection=[canonicalPaths[3],canonicalPaths[0],canonicalPaths[2],canonicalPaths[1]],rows=configureFlowDocumentationTable(collisionSnapshot,"values",{selectedPaths:selection,metadata:["description"]}).rows;
  assert.deepEqual(rows.map((row)=>row.slice(1)),selection.map((path)=>{const index=canonicalPaths.indexOf(path);return[`metadata:${sample}:${index}`,`value:${sample}:${index}`];}),"generated dotted and wildcard display collisions conserve canonical row identity");
  assert.deepEqual(rows.map(([display])=>display),[`${head}[x].${tail}`,`${head}.${tail}`,`${head}[x].${tail}`,`${head}.${tail}`],"actual wildcards and wildcard-looking literal segments share only their presentation");
}
