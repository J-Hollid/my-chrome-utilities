...
const support=add("propertySets",{name:"Support"});state={...state,project:{...state.project,collections:{...state.project.collections,pages:state.project.collections.pages.map((page)=>page.id===cart.id?{...page,pageGroupIds:[checkout.id]}:page.id===returns.id?{...page,pageGroupIds:[support.id]}:page)}}};
...
assert.deepEqual({pageFrameId:direct.pageFrameId,pageId:direct.pageId,pageGroupId:direct.pageGroupId,eventId:direct.eventId,role:direct.role,trigger:direct.trigger},{pageFrameId:cartFrame.id,pageId:cart.id,pageGroupId:checkout.id,eventId:pageView.id,role:undefined,trigger:"Initial load"});
