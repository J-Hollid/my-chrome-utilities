...
state.project.collections.propertySets.push(
...
state.project.collections.pages.push({id:"page:cart",name:"Cart",profileId:"profile:sitewide",propertySetApplications:[{propertySetId:"group:checkout"},{propertySetId:"group:retail"}],schemaConstraints:[{path:"/funnel_step",expectedValue:"2"}]});
...
assert.equal(storedCanonicalPolicy.canonicalSchema.revision,0,"the policy toggle preserves the publication-only Schema revision");
assert.equal(Object.hasOwn(storedCanonicalPolicy.canonicalSchema,"changes"),false,"the policy toggle stores no canonical command journal");
...
const parentlessCanonicalState=createSpecificationProject({name:"Admitted canonical",site:"legacy.example",id:(kind)=>`${kind}:admitted`}),legacyNode={id:"property:legacy-status",name:"legacy_status",order:0,type:"string",presence:{mode:"optional"},allowedValues:[],rules:[],documentation:{displayText:"legacy_status",description:"",comments:"",example:{method:"blank"}},provenance:[{source:"created"}],overrideReferences:[]},parentlessPage={id:"page:admitted",name:"Admitted Page"};
parentlessCanonicalState.project.collections.pages.push(parentlessPage);
delete parentlessPage.profileId;delete parentlessPage.profileIds;delete parentlessPage.profileInheritanceRecipes;delete parentlessPage.propertySetApplications;delete parentlessPage.schemaConstraints;delete parentlessPage.localSchemaContributions;
parentlessPage.canonicalSchema={id:"canonical:legacy-page",contributorId:parentlessPage.id,contributorName:parentlessPage.name,revision:2,state:"Draft",rootIds:[legacyNode.id],nodes:{[legacyNode.id]:legacyNode}};
const admittedProjection=composedCanonicalSchema(parentlessCanonicalState,parentlessPage,"Page");
assert.deepEqual(admittedProjection,parentlessPage.canonicalSchema,"a parentless admitted canonical contributor mounts its stored publication revision without synthesizing an opaque token");
const admittedEdit=applyCanonicalCommand(admittedProjection,{kind:"set",baseRevision:2,propertyId:legacyNode.id,patch:{documentation:{...legacyNode.documentation,description:"Restored legacy status"}}});
assert.equal(admittedEdit.status,"applied");
const admittedSaved=saveComposedCanonicalDocument(parentlessCanonicalState,"pages",parentlessPage.id,admittedEdit.document),admittedStored=admittedSaved.project.collections.pages[0];
assert.equal(admittedStored.canonicalSchema.revision,2,"editing a parentless admitted contributor preserves its publication Schema revision");
assert.equal(admittedStored.canonicalSchema.nodes[legacyNode.id].documentation.description,"Restored legacy status","the canonical edit persists through the composed save boundary");
assert.equal(Object.hasOwn(admittedStored.canonicalSchema,"changes"),false,"the composed save boundary does not recreate a command journal");
assert.equal(Object.hasOwn(admittedStored,"localSchemaContributions"),false,"a parentless canonical contributor is not demoted to sparse composed storage");
...
assert.equal(storedCompactPolicy.canonicalSchema.revision,4,"the compact Event policy preserves its Published Schema revision");
...
assert.equal(storedOccurrence.canonicalSchema.revision,2,"the compact occurrence policy preserves its Published Schema revision");
...
const nestedStructuredPage=saveComposedSchemaLocalFacetsAndStructures(structuredPage,"pages","page:cart","/page_name/locale",{type:"string"},[{kind:"add-child",path:"/page_name/locale",name:"language"}],(kind)=>`${kind}:nested`);
assert.equal(nestedStructuredPage.project.collections.pages[0].localSchemaContributions.find(({path})=>path==="/page_name/locale").definitionId,"property:workspace","editing an owned structure preserves its stable definition identity");
assert.equal(nestedStructuredPage.project.collections.pages[0].localSchemaContributions.find(({path})=>path==="/page_name/locale/language").definitionId,"property:nested");
...
ownershipState.project.collections.propertySets.push({id:"group:ownership",name:"Ownership group",profileId:"profile:ownership",localSchemaContributions:[{path:"/owned",allowedValues:["parent"],allowedValueIds:["value:group"],allowedValueProvenance:[{id:"value:group",state:"overridden",source:"group"}],rules:[{id:"rule:group",kind:"custom",severity:"warning",message:"Group rule",provenance:{source:"created",state:"local"}}]}]});
ownershipState.project.collections.pages.push({id:"page:ownership",name:"Ownership page",profileId:"profile:ownership",propertySetApplications:[{propertySetId:"group:ownership"}],localSchemaContributions:[{path:"/owned",rules:[{id:"rule:page",kind:"custom",severity:"error",message:"Page rule",provenance:{source:"created",state:"local"}}]}]});
const ownershipGroup=ownershipState.project.collections.propertySets[0],groupProjection=composedCanonicalSchema(ownershipState,ownershipGroup,"Property Set"),groupOwned=Object.values(groupProjection.nodes).find((node)=>canonicalPropertyPath(groupProjection,node.id)==="/owned");
assert.deepEqual(groupOwned.rules.map(({id,provenance})=>({id,state:provenance?.state,contributorId:provenance?.contributorId})),[{id:"rule:parent",state:"inherited",contributorId:"profile:ownership"},{id:"rule:group",state:"local",contributorId:"group:ownership"}],"Property Set compact projection retains per-rule ownership and the actual contributor");
assert.deepEqual(groupOwned.allowedValues.map(({id,provenance})=>({id,state:provenance?.[0]?.state,contributorId:provenance?.[0]?.contributorId})),[{id:"value:group",state:"overridden",contributorId:"group:ownership"}],"Property Set compact projection retains stable value identity and overridden ownership");
...
blocked.project.collections.propertySets.push({id:"group:partner",name:"Partner Checkout",schemaConstraints:[{path:"/funnel_name",type:"number"},{path:"/funnel_step",type:"number"}]});
blocked.project.collections.pages[0].propertySetApplications.push({propertySetId:"group:partner"});
...
clarity.project.collections.propertySets.push({id:"group:clarity",name:"Checkout",profileId:"profile:clarity"});
const emptyCheckout=clarity.project.collections.propertySets[0],emptyWorkspace=composedSchemaWorkspace(clarity,emptyCheckout,"Property Set");
assert.equal(emptyWorkspace.status,"ready","a Property Set with one complete Profile and no local contribution compiles Ready");
...
const clarityWorkspace=composedSchemaWorkspace(clarity,emptyCheckout,"Property Set"),customerDecision=clarityWorkspace.rows.find(({path})=>path==="/customer_status"),orderDecision=clarityWorkspace.rows.find(({path})=>path==="/order_total");
...
const repaired=resetComposedSchemaLocalFacet(clarity,"propertySets",emptyCheckout.id,"/customer_status","type"),repairedCheckout=repaired.project.collections.propertySets[0];
...
assert.equal(composedSchemaWorkspace(repaired,repairedCheckout,"Property Set").conflictSummary,"1 property needs a decision before validation and developer export.");
...
completeIssues.project.collections.propertySets[0].localSchemaContributions=[{path:"/customer_status",allowedValues:[30],maximum:5}];
const completeDecision=composedSchemaWorkspace(completeIssues,completeIssues.project.collections.propertySets[0],"Property Set").rows.find(({path})=>path==="/customer_status");
...
assert.match(composedSchemaWorkspace(completeIssues,completeIssues.project.collections.propertySets[0],"Property Set").conflictSummary,/1 property needs a decision/);
const completeAfterRange=resetComposedSchemaLocalFacet(completeIssues,"propertySets","group:clarity","/customer_status","minimum"),completeFinal=composedSchemaWorkspace(completeAfterRange,completeAfterRange.project.collections.propertySets[0],"Property Set");
...
inventoryState.project.collections.propertySets.push({id:"group:inventory",name:"Checkout",profileId:"profile:inventory",localSchemaContributions:[{path:"/customer_status",documentation:"Checkout description",allowedValues:["closed","archived"],rules:[{id:"rule:checkout-account",name:"Checkout account",kind:"pattern",pattern:"^[0-9]+$",replacesRuleId:"rule:parent-account"}]},{path:"/order_total",presence:"required"},{path:"/checkout_note",type:"string",documentation:"Checkout only"}]});
const inventoryGroup=inventoryState.project.collections.propertySets[0],inventoryWorkspace=composedSchemaWorkspace(inventoryState,inventoryGroup,"Property Set");
...
const resetInventoryProperty=resetComposedSchemaLocalProperty(inventoryState,"propertySets",inventoryGroup.id,"/customer_status");
assert.deepEqual(resetInventoryProperty.project.collections.propertySets[0].localSchemaContributions.map(({path})=>path),["/order_total","/checkout_note"],"property reset removes only that property's local items in one command");
const resetInventoryAll=resetComposedSchemaLocalChanges(inventoryState,"propertySets",inventoryGroup.id);
assert.deepEqual(resetInventoryAll.project.collections.propertySets[0].localSchemaContributions,[],"reset all removes every target-owned sparse contribution");
...
const groupedCleanupState=structuredClone(inventoryState),groupedCleanup=groupedCleanupState.project.collections.propertySets[0],groupedConstraint={path:"/grouped_cleanup",allowedValues:["local"],allowedValueIds:["allowed:local"],allowedValueProvenance:[{id:"allowed:local",state:"local",source:"group"}],presence:"required",condition:{kind:"all",children:[],id:"condition:local"},minimum:3,maximum:8,minItems:2,maxItems:4,documentation:"keep me"};groupedCleanup.localSchemaContributions=[structuredClone(groupedConstraint)];groupedCleanup.schemaConstraints=[structuredClone(groupedConstraint)];
const cleanedFacet=(facet)=>{const reset=resetComposedSchemaLocalFacet(groupedCleanupState,"propertySets",groupedCleanup.id,"/grouped_cleanup",facet),entity=reset.project.collections.propertySets[0];return[entity.localSchemaContributions[0],entity.schemaConstraints[0]];};
for(const constraint of cleanedFacet("allowedValues"))assert.deepEqual(constraint,{path:"/grouped_cleanup",presence:"required",condition:{kind:"all",children:[],id:"condition:local"},minimum:3,maximum:8,minItems:2,maxItems:4,documentation:"keep me"},"ordinary Allowed values reset removes companion IDs and provenance from every local store");
for(const constraint of cleanedFacet("presence"))assert.deepEqual(constraint,{path:"/grouped_cleanup",allowedValues:["local"],allowedValueIds:["allowed:local"],allowedValueProvenance:[{id:"allowed:local",state:"local",source:"group"}],minimum:3,maximum:8,minItems:2,maxItems:4,documentation:"keep me"},"ordinary Presence reset removes its condition from every local store");
for(const facet of["minimum","maximum"])for(const constraint of cleanedFacet(facet)){assert.equal(Object.hasOwn(constraint,"minimum"),false,"ordinary Range reset from "+facet+" removes minimum");assert.equal(Object.hasOwn(constraint,"maximum"),false,"ordinary Range reset from "+facet+" removes maximum");assert.equal(constraint.documentation,"keep me","ordinary Range reset from "+facet+" preserves unrelated bytes");}
for(const facet of["minItems","maxItems"])for(const constraint of cleanedFacet(facet)){assert.equal(Object.hasOwn(constraint,"minItems"),false,"ordinary Cardinality reset from "+facet+" removes minItems");assert.equal(Object.hasOwn(constraint,"maxItems"),false,"ordinary Cardinality reset from "+facet+" removes maxItems");assert.equal(constraint.documentation,"keep me","ordinary Cardinality reset from "+facet+" preserves unrelated bytes");}
const canonicalInventoryState=structuredClone(inventoryState),canonicalInventoryGroup=canonicalInventoryState.project.collections.propertySets[0],canonicalInventoryDocument=composedCanonicalSchema(canonicalInventoryState,canonicalInventoryGroup,"Property Set");canonicalInventoryGroup.canonicalSchema=canonicalInventoryDocument;delete canonicalInventoryGroup.localSchemaContributions;delete canonicalInventoryGroup.schemaConstraints;
assert.equal(composedSchemaWorkspace(canonicalInventoryState,canonicalInventoryGroup,"Property Set").localChangeCount,5,"canonical-backed contributors inventory only their explicitly local facets, rules, and property");
const canonicalFacetReset=resetComposedSchemaLocalFacet(canonicalInventoryState,"propertySets",canonicalInventoryGroup.id,"/customer_status","documentation"),canonicalFacetGroup=canonicalFacetReset.project.collections.propertySets[0],canonicalFacetWorkspace=composedSchemaWorkspace(canonicalFacetReset,canonicalFacetGroup,"Property Set");
...
const canonicalValuesReset=resetComposedSchemaLocalFacet(canonicalInventoryState,"propertySets",canonicalInventoryGroup.id,"/customer_status","allowedValues"),canonicalValuesWorkspace=composedSchemaWorkspace(canonicalValuesReset,canonicalValuesReset.project.collections.propertySets[0],"Property Set"),canonicalValuesNode=Object.values(canonicalValuesReset.project.collections.propertySets[0].canonicalSchema.nodes).find(({name})=>name==="customer_status");
...
const explicitParentTypeState=structuredClone(canonicalInventoryState),explicitParentTypeGroup=explicitParentTypeState.project.collections.propertySets[0],explicitParentTypeNode=Object.values(explicitParentTypeGroup.canonicalSchema.nodes).find(({name})=>name==="customer_status");explicitParentTypeNode.type="number";explicitParentTypeNode.localDefinitionFacets=[...new Set([...(explicitParentTypeNode.localDefinitionFacets??[]),"type"])];
const explicitParentTypeReset=resetComposedSchemaLocalFacet(explicitParentTypeState,"propertySets",explicitParentTypeGroup.id,"/customer_status","type"),explicitParentTypeResetGroup=explicitParentTypeReset.project.collections.propertySets[0],explicitParentTypeResetNode=Object.values(explicitParentTypeResetGroup.canonicalSchema.nodes).find(({name})=>name==="customer_status"),explicitParentTypeRow=composedSchemaWorkspace(explicitParentTypeReset,explicitParentTypeResetGroup,"Property Set").rows.find(({path})=>path==="/customer_status");
assert.equal(explicitParentTypeResetNode.type,"string","canonical Type reset restores an explicit parent Type in the stored canonical document");
assert.equal(explicitParentTypeRow.effective.type,"string","composed derivation exposes the restored explicit parent Type");
const absentParentTypeState=structuredClone(explicitParentTypeState),absentParentTypeGroup=absentParentTypeState.project.collections.propertySets[0],absentParentTypeNode=Object.values(absentParentTypeGroup.canonicalSchema.nodes).find(({name})=>name==="customer_status");delete absentParentTypeState.project.collections.profiles[0].schemaConstraints[0].type;delete absentParentTypeNode.inheritedDefinition.type;
const absentParentTypeReset=resetComposedSchemaLocalFacet(absentParentTypeState,"propertySets",absentParentTypeGroup.id,"/customer_status","type"),absentParentTypeResetGroup=absentParentTypeReset.project.collections.propertySets[0],absentParentTypeResetNode=Object.values(absentParentTypeResetGroup.canonicalSchema.nodes).find(({name})=>name==="customer_status"),absentParentTypeRow=composedSchemaWorkspace(absentParentTypeReset,absentParentTypeResetGroup,"Property Set").rows.find(({path})=>path==="/customer_status");
assert.equal(Object.hasOwn(absentParentTypeResetNode,"type"),false,"canonical Type reset removes the concrete local Type when the parent omits Type");
assert.equal(absentParentTypeResetNode.localDefinitionFacets.includes("type"),false,"absent-parent Type reset clears canonical ownership with the concrete value");
assert.equal(absentParentTypeRow.effective.type,undefined,"composed derivation does not expose the stale local Type after an absent-parent reset");
...
derivedFacetState.project.collections.propertySets.push({id:"group:derived-reset",name:"Child",profileId:"profile:derived-reset",localSchemaContributions:[{path:"/code",patterns:["^local$"]},{path:"/amount",minimum:3,maximum:8},{path:"/items",minItems:2,maxItems:4}]});
const derivedGroup=derivedFacetState.project.collections.propertySets[0];derivedGroup.canonicalSchema=composedCanonicalSchema(derivedFacetState,derivedGroup,"Property Set");delete derivedGroup.localSchemaContributions;
const resetDerived=(path,facet)=>{const reset=resetComposedSchemaLocalFacet(derivedFacetState,"propertySets",derivedGroup.id,path,facet),row=composedSchemaWorkspace(reset,reset.project.collections.propertySets[0],"Property Set").rows.find((candidate)=>candidate.path===path);return{reset,row,node:Object.values(reset.project.collections.propertySets[0].canonicalSchema.nodes).find(({name})=>path.endsWith(name))};};
...
const invariantState=structuredClone(derivedFacetState),invariantGroup=invariantState.project.collections.propertySets[0],invariantNode=Object.values(invariantGroup.canonicalSchema.nodes).find(({name})=>name==="amount"),invariantRule={id:"rule:parent-invariant",name:"Fraud ceiling",kind:"range",maximum:100,severity:"error",message:"Stable fraud ceiling",enforcement:"invariant",provenance:{source:"path-constraint",sourceId:"profile:derived-reset",state:"inherited"}};invariantNode.rules.push(structuredClone(invariantRule));const invariantBefore=structuredClone(invariantNode.rules.find(({id})=>id===invariantRule.id)),invariantReset=resetComposedSchemaLocalFacet(invariantState,"propertySets",invariantGroup.id,"/amount","maximum"),invariantAfter=Object.values(invariantReset.project.collections.propertySets[0].canonicalSchema.nodes).find(({name})=>name==="amount").rules;
assert.deepEqual(invariantAfter.find(({id})=>id===invariantRule.id),invariantBefore,"Range reset preserves every byte and the identity of an unrelated inherited invariant rule");
assert.equal(invariantAfter.some(({id})=>id==="rule:"+invariantNode.id+":range"),false,"Range reset removes only the locally owned synthetic derived range");
assert.deepEqual({minimum:composedSchemaWorkspace(invariantReset,invariantReset.project.collections.propertySets[0],"Property Set").rows.find(({path})=>path==="/amount").effective.minimum,maximum:composedSchemaWorkspace(invariantReset,invariantReset.project.collections.propertySets[0],"Property Set").rows.find(({path})=>path==="/amount").effective.maximum},{minimum:1,maximum:10},"live parent composition supplies the effective Range after the sparse local facet is removed");
...
const canonicalRuleId=canonicalInventoryDocument.nodes[Object.values(canonicalInventoryDocument.nodes).find(({name})=>name==="customer_status").id].rules.find(({name})=>name==="Checkout account").id,canonicalRuleReset=resetComposedSchemaLocalRule(canonicalInventoryState,"propertySets",canonicalInventoryGroup.id,"/customer_status",canonicalRuleId);
assert.equal(composedSchemaWorkspace(canonicalRuleReset,canonicalRuleReset.project.collections.propertySets[0],"Property Set").localChangeCount,4,"canonical-backed exact rule reset removes the advertised local rule");
const canonicalAllReset=resetComposedSchemaLocalChanges(canonicalInventoryState,"propertySets",canonicalInventoryGroup.id);
assert.equal(composedSchemaWorkspace(canonicalAllReset,canonicalAllReset.project.collections.propertySets[0],"Property Set").localChangeCount,0,"canonical-backed reset all removes every target-owned item");
...
const canonicalAllEffective=composedSchemaWorkspace(canonicalAllReset,canonicalAllReset.project.collections.propertySets[0],"Property Set").rows.find(({path})=>path==="/customer_status").effective,canonicalParent=composedSchemaWorkspace(inventoryState,inventoryGroup,"Property Set").rows.find(({path})=>path==="/customer_status").inherited;
...
assert.equal(Object.hasOwn(canonicalAllReset.project.collections.propertySets[0],"canonicalSchema"),false,"reset all removes the canonical owning storage instead of retaining stale local values behind cleared markers");
...
sharedAdditionState.project.collections.propertySets.push({id:"group:recipe-aware",name:"Recipe aware",profileInheritanceRecipes:[recipe("recipe:first","profile:first"),recipe("recipe:second","profile:second")]});
const recipeAwareIncluded=includeComposedSchemaParentAdditionSelections(sharedAdditionState,"propertySets","group:recipe-aware",[{recipeId:"recipe:first",propertyIds:["property:shared-addition"]}]),recipeAwareRecipes=recipeAwareIncluded.project.collections.propertySets[0].profileInheritanceRecipes;
...
parallelContext.project.collections.propertySets.push({id:"group:parallel-context",name:"Checkout context",profileIds:["profile:parallel-a","profile:parallel-b"]});
const parallelGroup=parallelContext.project.collections.propertySets[0],parallelWorkspace=composedSchemaWorkspace(parallelContext,parallelGroup,"Property Set"),parallelDecision=parallelWorkspace.rows.find(({path})=>path==="/customer_status");
...
const contextualized=applyComposedSchemaContextualFacet(parallelContext,"propertySets",parallelGroup.id,"/customer_status",contextualRepair.facet,contextualRepair.value),contextualizedGroup=contextualized.project.collections.propertySets[0],contextualizedWorkspace=composedSchemaWorkspace(contextualized,contextualizedGroup,"Property Set");
...
crossFacetContext.project.collections.propertySets.push({id:"group:cross-facet-context",name:"Checkout",profileIds:["profile:cross-allowed","profile:cross-expected"]});
const crossFacetGroup=crossFacetContext.project.collections.propertySets[0],crossFacetRow=composedSchemaWorkspace(crossFacetContext,crossFacetGroup,"Property Set").rows.find(({path})=>path==="/customer_status");
...
const crossFacetAllowedRepair=crossFacetRow.repairs.find(({kind,contributorId})=>kind==="use-contextual"&&contributorId==="profile:cross-allowed"),crossFacetResolved=applyComposedSchemaContextualFacet(crossFacetContext,"propertySets",crossFacetGroup.id,"/customer_status",crossFacetAllowedRepair.facet,crossFacetAllowedRepair.value);
const crossFacetResolvedWorkspace=composedSchemaWorkspace(crossFacetResolved,crossFacetResolved.project.collections.propertySets[0],"Property Set"),crossFacetEffective=crossFacetResolvedWorkspace.rows.find(({path})=>path==="/customer_status").effective;
...
namedRules.project.collections.propertySets.push({id:"group:named-rules",name:"Checkout",profileId:"profile:named-rules",localSchemaContributions:[
...
const overriddenRule=overrideComposedSchemaLocalRule(namedRules,"propertySets",namedGroup.id,"/customer_status",patternRepair.ruleId,patternRepair.sourceRuleId),overriddenStored=overriddenRule.project.collections.propertySets[0].localSchemaContributions.find(({path})=>path==="/customer_status").rules[0];
...
assert.equal(composedSchemaWorkspace(overriddenRule,overriddenRule.project.collections.propertySets[0],"Property Set").rows.find(({path})=>path==="/customer_status").validationState,"ready","the named replacement recompiles without the superseded Pattern conflict");
...
multiplePatternRules.project.collections.propertySets.push({id:"group:exact-pattern",name:"Checkout",profileId:"profile:exact-pattern",localSchemaContributions:[{path:"/account_code",rules:[{id:"rule:digits-pattern",name:"Digits only",kind:"pattern",pattern:"^[0-9]+$"}]}]});
const exactPatternGroup=multiplePatternRules.project.collections.propertySets[0],exactPatternRow=composedSchemaWorkspace(multiplePatternRules,exactPatternGroup,"Property Set").rows.find(({path})=>path==="/account_code"),exactPatternRepair=exactPatternRow.repairs.find(({kind})=>kind==="override-rule");
...
const exactPatternOverridden=overrideComposedSchemaLocalRule(multiplePatternRules,"propertySets",exactPatternGroup.id,"/account_code",exactPatternRepair.ruleId,exactPatternRepair.sourceRuleId),exactPatternEffective=composedSchemaWorkspace(exactPatternOverridden,exactPatternOverridden.project.collections.propertySets[0],"Property Set").rows[0].effective;
assert.equal(composedSchemaWorkspace(exactPatternOverridden,exactPatternOverridden.project.collections.propertySets[0],"Property Set").status,"ready");
...
duplicateNames.project.collections.propertySets.push({id:"group:duplicate-names",name:"Checkout",profileId:"profile:source-shared",localSchemaContributions:[{path:"/protected",type:"number"}]});
const duplicateDecision=composedSchemaWorkspace(duplicateNames,duplicateNames.project.collections.propertySets[0],"Property Set").rows.find(({path})=>path==="/protected");
...
policyConflict.project.collections.propertySets.push({id:"group:policy-conflict",name:"Checkout",profileIds:["profile:closed","profile:open"]});
const policyWorkspace=composedSchemaWorkspace(policyConflict,policyConflict.project.collections.propertySets[0],"Property Set");
