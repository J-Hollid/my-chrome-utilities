...
const explicitParentTypeState=structuredClone(canonicalInventoryState),explicitParentTypeGroup=explicitParentTypeState.project.collections.pageGroups[0],explicitParentTypeNode=Object.values(explicitParentTypeGroup.canonicalSchema.nodes).find(({name})=>name==="customer_status");explicitParentTypeNode.type="number";explicitParentTypeNode.localDefinitionFacets=[...new Set([...(explicitParentTypeNode.localDefinitionFacets??[]),"type"])];
const explicitParentTypeReset=resetComposedSchemaLocalFacet(explicitParentTypeState,"pageGroups",explicitParentTypeGroup.id,"/customer_status","type"),explicitParentTypeResetGroup=explicitParentTypeReset.project.collections.pageGroups[0],explicitParentTypeResetNode=Object.values(explicitParentTypeResetGroup.canonicalSchema.nodes).find(({name})=>name==="customer_status"),explicitParentTypeRow=composedSchemaWorkspace(explicitParentTypeReset,explicitParentTypeResetGroup,"Page Group").rows.find(({path})=>path==="/customer_status");
...
const absentParentTypeState=structuredClone(explicitParentTypeState),absentParentTypeGroup=absentParentTypeState.project.collections.pageGroups[0],absentParentTypeNode=Object.values(absentParentTypeGroup.canonicalSchema.nodes).find(({name})=>name==="customer_status");delete absentParentTypeState.project.collections.profiles[0].schemaConstraints[0].type;delete absentParentTypeNode.inheritedDefinition.type;
const absentParentTypeReset=resetComposedSchemaLocalFacet(absentParentTypeState,"pageGroups",absentParentTypeGroup.id,"/customer_status","type"),absentParentTypeResetGroup=absentParentTypeReset.project.collections.pageGroups[0],absentParentTypeResetNode=Object.values(absentParentTypeResetGroup.canonicalSchema.nodes).find(({name})=>name==="customer_status"),absentParentTypeRow=composedSchemaWorkspace(absentParentTypeReset,absentParentTypeResetGroup,"Page Group").rows.find(({path})=>path==="/customer_status");
...
const invariantState=structuredClone(derivedFacetState),invariantGroup=invariantState.project.collections.pageGroups[0],invariantNode=Object.values(invariantGroup.canonicalSchema.nodes).find(({name})=>name==="amount"),invariantRule={id:"rule:parent-invariant",name:"Fraud ceiling",kind:"range",maximum:100,severity:"error",message:"Stable fraud ceiling",enforcement:"invariant",provenance:{source:"path-constraint",sourceId:"profile:derived-reset",state:"inherited"}};invariantNode.rules.push(structuredClone(invariantRule));const invariantBefore=structuredClone(invariantNode.rules.find(({id})=>id===invariantRule.id)),invariantReset=resetComposedSchemaLocalFacet(invariantState,"pageGroups",invariantGroup.id,"/amount","maximum"),invariantAfter=Object.values(invariantReset.project.collections.pageGroups[0].canonicalSchema.nodes).find(({name})=>name==="amount").rules;
...
assert.deepEqual({minimum:composedSchemaWorkspace(invariantReset,invariantReset.project.collections.pageGroups[0],"Page Group").rows.find(({path})=>path==="/amount").effective.minimum,maximum:composedSchemaWorkspace(invariantReset,invariantReset.project.collections.pageGroups[0],"Page Group").rows.find(({path})=>path==="/amount").effective.maximum},{minimum:1,maximum:10},"live parent composition supplies the effective Range after the sparse local facet is removed");
