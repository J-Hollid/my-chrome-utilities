...
const project={
  ...
  collections:{
    ...
    assignments:[
      {id:"assignment:retail",name:"Retail",targetId:"profile:retail",targetKind:"Shared Profile",applicabilitySetId:"app:retail",eventId:"event:purchase",sourceId:"event-history",target:"payload",priority:10},
      {id:"assignment:trade",name:"Trade",targetId:"profile:trade",targetKind:"Shared Profile",applicabilitySetId:"app:trade",eventId:"event:purchase",sourceId:"event-history",target:"payload",priority:10},
    ],
  },
};
...
const legacyProject=structuredClone(project);legacyProject.collections.schemaDrafts=[{id:"schema:retail",name:"Retail schema",version:3,published:true,document:{type:"object"}}];
const migration=migrateCanonicalProject({projectEnvelope:createCanonicalProjectEnvelope(legacyProject,"legacy"),schemaLibrary:[legacySchema]});
...
assert.equal(compiled.status,"compiled");assert.equal(Object.isFrozen(compiled.plan),true);assert.equal(compiled.plan.schemas["assignment:retail"].schemaId,"profile:retail");assert.equal(compiled.plan.schemas["assignment:retail"].revision,1);
...
assert.deepEqual(compiled.plan.schemas["assignment:retail"].required,["event","ecommerce"]);
assert.deepEqual(compiled.plan.provenance["assignment:retail:/ecommerce/transaction_id"].map(({profileId})=>profileId),["profile:retail"]);

const compatibleProject=structuredClone(project);compatibleProject.collections.profiles[1].requirements.push({path:"/ecommerce/currency",type:"string",required:true,allowedValues:["EUR","USD"],documentation:"ISO currency",examples:["EUR"],rules:[{id:"rule:uppercase",operator:"regular-expression",parameters:"^[A-Z]{3}$"}]},{path:"/ecommerce/coupon",type:"string",forbidden:true},{path:"/ecommerce/items",type:"array",rules:[{id:"rule:items",operator:"item-count",parameters:"1,10",severity:"error",message:"Use 1 through 10 items"}]});
const compatible=compileSpecificationProject(createCanonicalProjectEnvelope(compatibleProject,"draft:compatible"));assert.equal(compatible.status,"compiled");const currency=compatible.plan.schemas["assignment:retail"].document.properties.ecommerce.properties.currency;assert.deepEqual(currency.enum,["EUR","USD"]);assert.equal(currency.description,"ISO currency");assert.deepEqual(currency.examples,["EUR"]);assert.equal(currency["x-rules"][0].id,"rule:uppercase");
...
assert.deepEqual(first,second);assert.equal(first.winner.assignmentId,"assignment:retail");assert.equal(first.winner.schemaRevision,1);assert.deepEqual(first.effectiveProfiles,["profile:retail"]);assert.deepEqual(first.issues,[]);assert.equal(first.candidates.find(({assignmentId})=>assignmentId==="assignment:trade").rejectionReasons.includes("flowId"),true);
assert.equal(first.planContentIdentity,compiled.plan.contentIdentity);assert.match(first.resultIdentity,/^result:/);

const assignmentOwned=structuredClone(project);assignmentOwned.collections.assignments=[{...assignmentOwned.collections.assignments[0],name:"Browser raw Retail",sourceId:"browser",target:"raw input"}];
const assignmentOwnedCompiled=compileSpecificationProject(createCanonicalProjectEnvelope(assignmentOwned,"draft:assignment-owned"));assert.equal(assignmentOwnedCompiled.status,"compiled");
assert.equal(assignmentOwnedCompiled.plan.assignments[0].sourceId,"browser");assert.equal(assignmentOwnedCompiled.plan.assignments[0].validationTarget,"raw input");
const rawObservation={sourceId:"browser",eventName:"purchase",flowId:"flow:retail",payload:{event:"purchase"},rawInput:{event:"purchase",ecommerce:{transaction_id:"T-raw",value:7}}};
const rawResult=evaluateSpecificationObservation(assignmentOwnedCompiled.plan,rawObservation);assert.equal(rawResult.winner.assignmentId,"assignment:retail");assert.deepEqual(rawResult.issues,[],"raw-input Assignment validates rawInput rather than payload");assert.equal(rawResult.candidates[0].event.accepted,true);assert.equal(rawResult.candidates[0].applicability.accepted,true);assert.equal(rawResult.candidates[0].resolution,"winner");assert.equal(rawResult.routingSummary,"Browser raw Retail is the sole winner");
const wrongSource=evaluateSpecificationObservation(assignmentOwnedCompiled.plan,{...rawObservation,sourceId:"server"});assert.equal(wrongSource.winner,undefined);assert.deepEqual(wrongSource.candidates[0].rejectionReasons,["source"]);assert.match(wrongSource.candidates[0].event.evidence,/Source rejected/);
const wrongEvent=evaluateSpecificationObservation(assignmentOwnedCompiled.plan,{...rawObservation,eventName:"refund"});assert.equal(wrongEvent.winner,undefined);assert.deepEqual(wrongEvent.candidates[0].rejectionReasons,["Event"]);
const wrongPath=evaluateSpecificationObservation(assignmentOwnedCompiled.plan,{...rawObservation,flowId:"flow:trade"});assert.equal(wrongPath.winner,undefined);assert.deepEqual(wrongPath.candidates[0].rejectionReasons,["flowId"]);assert.match(wrongPath.candidates[0].applicability.evidence,/flowId/);
const tiedProject=structuredClone(assignmentOwned);tiedProject.collections.assignments.push({...tiedProject.collections.assignments[0],id:"assignment:alternative",name:"Browser raw alternative"});const tiedCompiled=compileSpecificationProject(createCanonicalProjectEnvelope(tiedProject,"draft:tied"));assert.equal(tiedCompiled.status,"compiled");const tiedResult=evaluateSpecificationObservation(tiedCompiled.plan,rawObservation);assert.equal(tiedResult.winner,undefined);assert.deepEqual(tiedResult.ties,["assignment:retail","assignment:alternative"]);assert.match(tiedResult.routingSummary,/ambiguous.*Browser raw Retail.*Browser raw alternative/i);
const priorityProject=structuredClone(tiedProject);priorityProject.collections.assignments[0].priority=20;const priorityCompiled=compileSpecificationProject(createCanonicalProjectEnvelope(priorityProject,"draft:priority"));assert.equal(priorityCompiled.status,"compiled");const priorityResult=evaluateSpecificationObservation(priorityCompiled.plan,rawObservation);assert.equal(priorityResult.winner.assignmentId,"assignment:retail");assert.equal(priorityResult.candidates.find(({assignmentId})=>assignmentId==="assignment:alternative").resolution,"lower priority");
