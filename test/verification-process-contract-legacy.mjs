...
assert.deepEqual(focusedAcceptanceOptions([
  ...
  "--timeout-regression", "unit:test/verification-contracts/reliability-run-intent-contract-test.mjs",
...
const focusedSelectorOptions = focusedAcceptanceOptions([
  "--pack", "shell", "--focused-task", "unit:test/verification-contracts/reliability-run-intent-contract-test.mjs",
]);
assert.deepEqual(focusedSelectorOptions.focusedTaskKeys,
  ["unit:test/verification-contracts/reliability-run-intent-contract-test.mjs"],
...
try {
  ...
  for (const relativePath of extractedCliFiles) {
    const destination = path.join(cliContentionRepository, relativePath);
  ...
  // This fixture exercises checkpoint contention, not loopback authority. Keep
  // its copied registry canonical while removing unrelated host capabilities.
  for (const registryPath of ["verification/packs.base.json", "verification/packs.json"]) {
    const absoluteRegistryPath = path.join(cliContentionRepository, registryPath);
    const fixtureRegistry = JSON.parse(await readFile(absoluteRegistryPath, "utf8"));
    for (const pack of fixtureRegistry) delete pack.executionPrerequisites;
    await writeFile(absoluteRegistryPath, `${JSON.stringify(fixtureRegistry, null, 2)}\n`);
  }
  ...
  const fixtureChangePath = path.join(cliContentionRepository,
    "scripts/verification-registry/compiler.mjs");
  await writeFile(fixtureChangePath, `${await readFile(fixtureChangePath, "utf8")}\n`);
  await exec("git", ["add", "scripts/verification-registry/compiler.mjs"], {
    cwd:cliContentionRepository,
  });
  await exec("git", ["commit", "-qm", "cli contention fixture"], { cwd:cliContentionRepository });
  assert.equal(await exec("git", ["status", "--porcelain"], { cwd:cliContentionRepository }), "",
    "the isolated CLI fixture must be clean before evidence execution");

  const packIds = ["verification_process"];
...
assert.match(await readFile(new URL("../scripts/verification-evidence/core.mjs", import.meta.url), "utf8"),
...
try {
  ...
  assert.doesNotThrow(() => timeoutRepairFocusedTaskPlan(legacyBoundaryIncident,
    ["scripts/run-focused-acceptance.mjs"], "unit:test/verification-contracts/reliability-run-intent-contract-test.mjs",
...
`);
  ...
  await captureRepairRejection("limitOnly", validateTimeoutRepairProposal(first, {
    ...
    regression:{ key:"unit:test/verification-contracts/reliability-run-intent-contract-test.mjs", status:"passed", commit:"repair-commit" },
  ...
  await captureRepairRejection("unchangedCandidate", validateTimeoutRepairProposal(first, {
    ...
    causalCategory:"artifact/process locking", causalExplanation:"stale lock ownership",
    checkpoint:{ baseCommit:"approved-base", evidenceTask:"vtd014" },
    regression:{ key:"unit:test/verification-contracts/reliability-run-intent-contract-test.mjs", status:"passed", commit:"failed-commit" },
    focusedReceipt:{ status:"passed", commit:"failed-commit", provenance:"fresh" },
  ...
  const validRepairProposal = {
    ...
    checkpoint:{ baseCommit:"approved-base", evidenceTask:"vtd014" },
    regression:{ key:"unit:test/verification-contracts/reliability-run-intent-contract-test.mjs", status:"passed",
  ...
  const acceptedBasePackIds = allPackIds.filter((id) =>
    acceptedBasePacks.some((pack) => pack.id === id));
  ...
  vtd014Evidence = {
    ...
    conservation:{ changedFiles,
      ...
      currentTaskDigest:verificationDigest(currentConservationPlan.tasks.filter(({key,packId})=>
  ...
  const approvedTaskSuccessionConservesVtd014 = taskSuccessionEvidence.conserved &&
    taskSuccessionEvidence.exactIdentities && taskSuccessionEvidence.registryGuard;
  assert.ok(vtd014Evidence.conservation.currentTaskDigest ===
      vtd014Evidence.conservation.acceptedBaseTaskDigest ||
      approvedTaskSuccessionConservesVtd014,
  "VTD-014 task conservation is either identity-stable or registry-succession conserved");
  assert.ok(vtd014Evidence.conservation.currentPackContractDigest ===
      vtd014Evidence.conservation.acceptedBasePackContractDigest ||
      approvedTaskSuccessionConservesVtd014,
  "VTD-014 pack conservation is either identity-stable or registry-succession conserved");
} finally {
...
const acceptedTerminalIdentities = baseTerminalPlan.tasks
...
for (const { key } of planVerification(packs, {
  packIds:["verification_process"], includeProperties:true,
}).tasks) {
  if (key !== "build:dist") approvedVerificationTaskKeys.add(key);
}
...
const vtd004CaptureAcceptance = {
  ...
  conservation:{evidenceProfile:captureEvidenceProfile,exactTaskCount:exactCapturePlan.tasks.length,
    unitCount:exactCapturePlan.unitTasks.length,propertyCount:12,featureCount:66,handlerCount:25,
...
const observedModularPolicyAuditEdges = new Set();
for (const edge of crossPackCodeEdges) {
  ...
  const flowStyleAuditRead = edge.requiringPath === "test/verification-process-contract-legacy.mjs" &&
    approvedFlowStyleAuditPaths.has(edge.requiredPath);
  const modularPolicyAuditEdge = `${edge.requiringPath} -> ${edge.requiredPath}`;
  const modularPolicyAuditRead = edge.requiringOwner === "shell" &&
      edge.requiredOwner === "verification_process" &&
      edge.requiredPath.startsWith("scripts/verification") ||
    edge.requiringOwner === "verification_process" && edge.requiredOwner === "shell" &&
      edge.verificationConsumerPath === "test/verification-process-contract-legacy.mjs";
  if (modularPolicyAuditRead) observedModularPolicyAuditEdges.add(modularPolicyAuditEdge);
  if (flowStyleAuditRead) observedFlowStyleAuditPaths.add(edge.requiredPath);
  if (!globalStylesheetRead && !flowStyleAuditRead && !modularPolicyAuditRead &&
...
assert.ok([...observedModularPolicyAuditEdges].some((edge) =>
  edge.includes(" -> scripts/verification")) &&
  [...observedModularPolicyAuditEdges].some((edge) =>
    edge.includes("test/verification-process-contract-legacy.mjs") ||
    edge.includes("test/support/")),
"the modular policy audit accounts for both directions of verification-only cross-owner inputs");
...
for (const modularPolicyPath of ["scripts/verification-evidence.mjs", "verification/packs.json"]) {
  assert.equal(verificationOwner(packs, modularPolicyPath), "verification_process",
    `modular policy owner for ${modularPolicyPath}`);
}
...
expectedTerminalIdentities({tasks:vtd009ExactBase.tasks.filter(({key}) =>
