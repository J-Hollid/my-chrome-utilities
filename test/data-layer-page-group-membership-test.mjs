...
state={...state,project:{...state.project,collections:{...state.project.collections,pages:[cart],propertySets:[checkout,retail,trade,delivery],flows:[{id:"flow:checkout",name:"Checkout journey"}]},documentationFlowGraphs:{"flow:checkout":{pageGroupIds:[checkout.id,retail.id,delivery.id],pageFrames:[],occurrences:[],relationships:[]}}}};

assert.deepEqual(orderedPageGroupIds(state.project,cart.id),[checkout.id],"Page-owned order is authoritative when present");
const canonicalFixture=state,divergentFixture={...state,project:{...state.project,collections:{...state.project.collections,propertySets:[checkout,retail,{...trade,pageIds:[cart.id]},delivery]}}},divergentBytes=JSON.stringify(divergentFixture);
...
assert.deepEqual(state.project.collections.propertySets,beforeOrderProject.collections.propertySets,"reorder does not rewrite Property Set projections");
...
const legacyState={...canonicalFixture,project:{...canonicalFixture.project,collections:{...canonicalFixture.project.collections,propertySets:[checkout,retail,{...trade,pageIds:[cart.id]},delivery]}}};
...
assert.equal(migrated.project.collections.propertySets.some(({pageIds})=>pageIds?.includes(cart.id)),false,"confirmed migration removes the competing legacy source");
assert.equal(migrated.history.undo.at(-1).label,"Migrate ordered Property Set membership for Cart");
...
assert.throws(()=>confirmPageGroupMembershipMigration(missingState,missing),/missing Property Set/);

console.log("data-layer Property Set membership tests passed");
