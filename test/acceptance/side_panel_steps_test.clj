...
(deftest maps-package-identity-to-the-strong-branded-display-name
  (is (= "TWAtility Belt"
         (side-panel/extension-name-for-project "my-chrome-utilities")))
  (is (not= "my-chrome-utilities"
            (side-panel/extension-name-for-project "my-chrome-utilities")))
  (is (= "Another extension"
         (side-panel/extension-name-for-project "Another extension"))))
  ...
  (is (= [{:kind :command-registry :path "src/commands.ts"}
         ...
         (side-panel/forbidden-scope-findings
          {"src/commands.ts" "export const commandRegistry = new Map();"
           ...
           "src/database.ts" "const request = indexedDB.open('projects');"}))))
