...
(deftest browser-evidence-completeness-is-shared-across-pack-handlers
  (let [complete {:installedBoundary true :runtime001 {:ready true}}
        required #{:installedBoundary :runtime001}
        runtime #{:runtime001}]
    (is (support/all-values-true? (:runtime001 complete)))
    (is (support/complete-browser-evidence? complete required runtime))
    (is (not (support/complete-browser-evidence?
              (assoc-in complete [:runtime001 :ready] false) required runtime)))))
