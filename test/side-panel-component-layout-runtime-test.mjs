...
const schemaSpecificationContainerDefaultsRuntime = `(()=>{
  ...
  const controls=()=>Array.from(list.querySelectorAll('input[data-path]')),paths=()=>Array.from(table.querySelectorAll("tbody tr"),({dataset})=>dataset.propertyPath),control=(path)=>q('input[data-path="'+path+'"]',list);const initialControls=controls(),initialAll=initialControls.every(({checked})=>checked),initialPaths=paths();
  ...
  const search=q('[aria-label="Search properties"]',builder);search.value="duration";search.dispatchEvent(new Event("input",{bubbles:true}));search.value="";search.dispatchEvent(new Event("input",{bubbles:true}));const sort=q('[aria-label="Preview order"]',builder);sort.value="name";sort.dispatchEvent(new Event("change",{bubbles:true}));q('[aria-label^="Move Property name later"]',builder).click();const example=q('[data-specification-example-path="/products/*/product_name"]',builder);example.value="Retained example";example.dispatchEvent(new Event("input",{bubbles:true}));const copyMode=q('[aria-label="Copy mode"]',builder);copyMode.value="spreadsheet";copyMode.dispatchEvent(new Event("change",{bubbles:true}));Array.from(builder.querySelectorAll('input[type="checkbox"]')).find(({parentElement})=>parentElement?.textContent?.includes("Include headings")).click();const retained={products:control("/products").checked,duration:control("/products/*/duration").checked};
  const source=q("#schema-specification-source",builder);source.value="historical:2";source.dispatchEvent(new Event("change",{bubbles:true}));const reset={all:controls().every(({checked})=>checked),paths:paths(),available:controls().map(({dataset})=>dataset.path)};click(builder,"Clear selection");const cleared={checked:controls().filter(({checked})=>checked).length,rows:paths().length};click(builder,"Select all");const selected={all:controls().every(({checked})=>checked),rows:paths().length,controls:controls().length};
  return{initial:{all:initialAll,available:initialControls.map(({dataset})=>dataset.path),paths:initialPaths,unique:new Set(initialPaths).size===initialPaths.length},afterContainer,afterDescendant,retained,reset,cleared,selected,unchanged:before===localStorage.getItem("my-chrome-utilities.schema-library.v1"),runtimeErrors:globalThis.__sidePanelRuntimeErrors??[]};
})()`;
...
try {
  ...
  for (const width of componentWidths) {
    ...
    if(process.env.SCHEMA_SPECIFICATION_CONTAINER_DEFAULTS_BROWSER_ADAPTER==="1"){
      ...
      assert.equal(observed.initial.all,true);assert.equal(observed.initial.unique,true);assert.equal(observed.initial.available.some((path)=>path.endsWith("/*")),false);assert.deepEqual(observed.initial.paths,observed.initial.available);assert.equal(observed.initial.available.includes("/products")&&observed.initial.available.includes("/products/*/duration")&&observed.initial.available.includes("/site_id"),true);assert.equal(observed.afterContainer.container,false);assert.equal(observed.afterContainer.descendant,true);assert.equal(observed.afterContainer.paths.includes("/products"),false);assert.equal(observed.afterContainer.paths.includes("/products/*/duration"),true);assert.deepEqual(observed.afterDescendant,{container:false,descendant:false});assert.deepEqual(observed.retained,{products:false,duration:false});assert.equal(observed.reset.all,true);assert.deepEqual([...observed.reset.paths].sort(),[...observed.reset.available].sort());assert.deepEqual(observed.cleared,{checked:0,rows:0});assert.equal(observed.selected.all,true);assert.equal(observed.selected.rows,observed.selected.controls);assert.equal(observed.unchanged,true);assert.deepEqual(observed.runtimeErrors,[]);socket.close();continue;
    }
