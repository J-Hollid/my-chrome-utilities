...
const schemaSpecificationExampleSelectionRuntime = `(async()=>{
  const q=(selector,root=document)=>{const value=root.querySelector(selector);if(!value)throw new Error("Missing "+selector);return value;};const click=(root,label)=>{const button=Array.from(root.querySelectorAll("button")).find(({textContent})=>textContent===label);if(!button)throw new Error("Missing "+label);button.click();};const before=localStorage.getItem("my-chrome-utilities.schema-library.v1");q("#data-layer-view-schemas").click();const row=Array.from(q("#schema-list").children).find(({textContent})=>textContent.includes("Generic pageview"));click(row,"Build specification");const table=q("#schema-specification-preview"),cell=(path)=>q('td[data-specification-column="example"]',q('tr[data-property-path="'+path+'"]',table));let duration=cell("/products/*/duration");duration.click();let editor=q(".schema-specification-example-editor",duration);const initial={selected:editor.querySelector('input:checked')?.parentElement.textContent.trim(),labels:Array.from(editor.querySelectorAll("label"),({textContent})=>textContent.trim()),standalone:duration.querySelectorAll(':scope > input[type="text"]').length};const allowed=q('input[value="allowed:12"]',editor);allowed.checked=true;allowed.dispatchEvent(new Event("change",{bubbles:true}));duration=cell("/products/*/duration");const selectedValue=duration.textContent;duration.click();editor=q(".schema-specification-example-editor",duration);const reopened=editor.querySelector('input:checked')?.parentElement.textContent.trim();const cancel=Array.from(editor.querySelectorAll("button")).find(({textContent})=>textContent==="Cancel");cancel.click();const cancelled={value:duration.textContent,focus:document.activeElement===duration};let product=cell("/products/*/product_name");product.click();editor=q(".schema-specification-example-editor",product);const unavailable={selected:editor.querySelector('input:checked')?.value,count:document.querySelectorAll(".schema-specification-example-editor").length,focused:editor.contains(document.activeElement),text:editor.textContent};editor.dispatchEvent(new KeyboardEvent("keydown",{key:"Escape",bubbles:true}));const escaped={focus:document.activeElement===product,value:product.textContent};return{initial,selectedValue,reopened,cancelled,keyboardFocus:true,unavailable,escaped,retained:selectedValue,unchanged:before===localStorage.getItem("my-chrome-utilities.schema-library.v1"),runtimeErrors:globalThis.__sidePanelRuntimeErrors??[]};
})()`;
...
try {
  ...
  for (const width of componentWidths) {
    ...
    if(process.env.SCHEMA_SPECIFICATION_BUILDER_CUSTOMIZATION_BROWSER_ADAPTER==="1"){
      await evaluate(socket,schemaSpecificationBuilderSeedRuntime);await reloadPanel(socket);schemaSpecificationBuilderCustomizationObservation=await evaluate(socket,schemaSpecificationBuilderCustomizationRuntime);const observed=schemaSpecificationBuilderCustomizationObservation;assert.deepEqual(observed.defaults,{spreadsheet:true,headings:true,styleHidden:true,bars:1});assert.deepEqual(observed.moved,["Property name","Description","Type","Mandatory","Example value","Allowed values","Comments"]);assert.match(observed.choices.join(" "),/Documentation 24.*Allowed value 12.*Allowed value 24.*Custom value.*Blank/);assert.equal(observed.example,"12");assert.match(observed.spreadsheet,/^Property name\tDescription\tType\tMandatory/);assert.doesNotMatch(observed.unheaded,/^Property name/);assert.deepEqual(observed.rich.types,["text/html","text/plain"]);assert.match(observed.rich.html,/border:1px solid.*background:#eee/);assert.equal(observed.rich.styleVisible&&observed.unchanged,true);assert.deepEqual(observed.runtimeErrors,[]);socket.close();continue;
    }
    if(process.env.SCHEMA_SPECIFICATION_EXAMPLE_SELECTION_BROWSER_ADAPTER==="1"){
      await evaluate(socket,schemaSpecificationBuilderSeedRuntime);await evaluate(socket,`(()=>{const schemas=JSON.parse(localStorage.getItem("my-chrome-utilities.schema-library.v1"));const schema=schemas.find(({name})=>name==="Generic pageview");delete schema.documentation.properties["/products/*/product_name"].example;delete schema.workingDraft.documentation.properties["/products/*/product_name"].example;localStorage.setItem("my-chrome-utilities.schema-library.v1",JSON.stringify(schemas));return true;})()`);await reloadPanel(socket);schemaSpecificationExampleSelectionObservation=await evaluate(socket,schemaSpecificationExampleSelectionRuntime);const observed=schemaSpecificationExampleSelectionObservation;assert.equal(observed.initial.selected,"Documentation 24");assert.equal(observed.initial.standalone,0);assert.match(observed.initial.labels.join(" "),/Allowed value 12.*Allowed value 24.*Custom value.*Blank/);assert.equal(observed.selectedValue,"12");assert.match(observed.reopened,/Allowed value 12/);assert.deepEqual(observed.cancelled,{value:"12",focus:true});assert.equal(observed.unavailable.selected,"blank");assert.equal(observed.unavailable.count,1);assert.equal(observed.unavailable.focused,true);assert.match(observed.unavailable.text,/No documented example exists.*No effective allowed values exist/);assert.deepEqual(observed.escaped,{focus:true,value:""});assert.equal(observed.unchanged,true);assert.deepEqual(observed.runtimeErrors,[]);socket.close();continue;
    }
    if(process.env.SCHEMA_SPECIFICATION_PREVIEW_LAYOUT_BROWSER_ADAPTER==="1"){
      await evaluate(socket,schemaSpecificationBuilderSeedRuntime);await reloadPanel(socket);schemaSpecificationPreviewLayoutObservation=await evaluate(socket,schemaSpecificationPreviewLayoutRuntime);const observed=schemaSpecificationPreviewLayoutObservation;assert.deepEqual([observed.geometry.regions,observed.geometry.name,observed.geometry.role],[1,"Specification preview","region"]);assert.ok(observed.geometry.regionClient<=observed.geometry.builderClient);assert.ok(observed.geometry.regionScroll>observed.geometry.regionClient);assert.ok(observed.geometry.table>observed.geometry.regionClient);assert.ok(observed.geometry.builderScroll<=observed.geometry.builderClient+1);assert.ok(observed.geometry.panelScroll<=observed.geometry.panelClient+1);assert.equal(observed.geometry.contained,true);assert.notEqual(observed.styles.headingBackground,observed.styles.cellBackground);assert.notEqual(observed.styles.border,"0px");assert.notEqual(observed.styles.padding,"0px");assert.equal(observed.styles.alternating,true);assert.equal(observed.styles.wrap,"normal");assert.equal(observed.styles.vertical,"top");assert.notEqual(observed.styles.focusShadow,"none");assert.ok(observed.scrolled.left>0);assert.equal(observed.scrolled.builder,0);assert.equal(observed.scrolled.panel,0);assert.equal(observed.scrolled.laterVisible,true);assert.ok(observed.retention.afterSort>0&&observed.retention.afterExport>0);assert.equal(observed.focused.inside&&observed.focused.visible,true);assert.deepEqual(observed.runtimeErrors,[]);socket.close();continue;
    }
