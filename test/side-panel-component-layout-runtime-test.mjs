...
const assetServer = createServer(async (request, response) => {
  ...
  if(pathname==="/observation-target.html"){response.writeHead(200,{"Content-Type":"text/html"}).end('<!doctype html><title>Retail confirmation</title><script>globalThis.dataLayer=[]</script>');return;}
...
async function openSpecificationBuilder(port, width, height = 900, pageUrl = `http://127.0.0.1:${assetPort}/specification-builder.html`) {
  ...
  const diagnosis=await socket.call("Runtime.evaluate",{expression:"({url:location.href,title:document.title,body:document.body?.innerText,error:document.documentElement?.outerHTML?.slice(0,1000)})",returnByValue:true});
  throw new Error(`Specification Builder DOM did not finish loading. ${JSON.stringify(diagnosis.result.value)}`);
}
