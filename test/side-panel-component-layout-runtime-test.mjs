...
const allowedValuesRuleMigrationCoverageRuntime = `(async()=>{
  const verification=await import("/data-layer-schema-verification.js"),picker=await import("/data-layer-schema-property-rule-picker.js"),guided=await import("/data-layer-guided-rule-parameter-integrity.js");
  ...
  const copied=[];globalThis.ClipboardItem=class{constructor(data){this.data=data;this.types=Object.keys(data);}};Object.defineProperty(navigator,"clipboard",{configurable:true,value:{write:async(items)=>copied.push({kind:"rich",item:items[0]}),writeText:async(text)=>copied.push({kind:"plain",text})}});const builder=q("#schema-specification-builder");click(builder,"Copy specification table");await new Promise((resolve)=>setTimeout(resolve,0));const copyMode=q('[aria-label="Copy mode"]',builder);copyMode.value="spreadsheet";copyMode.dispatchEvent(new Event("change",{bubbles:true}));click(builder,"Copy specification table");await new Promise((resolve)=>setTimeout(resolve,0));const plain=copied.find(({kind})=>kind==="plain").text;
  ...
  return{validation:{valid:valid.issues.length,invalid:invalid.issues.map(({expected})=>expected)},authoring:{picker:pickerRule,guided:guidedRule,authored},inheritance:{values:parent.attachedRules[0].allowedValues,invalid:inheritedInvalid.issues.map(({expected})=>expected)},invalidMigration:{unsafe:unsafe.attachedRules[0],canonical:unsafe.attachedRules[1]},copyImport:{plain,imported:imported.attachedRules[0]}};
})()`;
