...
try {
  ...
  for (const width of componentWidths) {
    ...
    if (process.env.SCHEMA_PROPERTY_TYPE_EDITING_BROWSER_ADAPTER === "1") {
      ...
      assert.deepEqual(observed.controls,{valueTypes:["String","Number","Boolean","Object","Array"],treatments:["Error","Warning","Ignore"],defaultTreatment:"Error",itemHidden:true});assert.deepEqual(observed.arrayControls,{itemTypes:["Any item type","String","Number","Boolean","Object"],initial:"String"});assert.match(observed.review,/Number changing to String/);assert.match(observed.review,/example value.*rule range.*conditional dependency order-condition/);assert.deepEqual(observed.impactChoices,{remove:"Remove incompatible schema data",cancel:true,blocked:true});assert.match(observed.descendantImpact.review,/descendant definitions.*descendant required relationships.*descendant documentation.*descendant rules/);assert.deepEqual({blocked:observed.descendantImpact.blocked,unchanged:observed.descendantImpact.unchanged,focus:observed.descendantImpact.focus},{blocked:true,unchanged:true,focus:true});assert.equal(observed.unchangedBeforeConfirm,true);assert.deepEqual(observed.orderSaved,{published:"number",draft:"string",required:["order_id"],description:"Order identifier"});assert.equal(observed.tagsItems,null);assert.deepEqual(observed.warning,[["/price","warning"]]);assert.equal(observed.ignored,false);assert.equal(observed.persistedTreatment,"ignore");assert.deepEqual(observed.remainingRules,["product-name-required"]);assert.equal(observed.reusableUnchanged,true);assert.deepEqual(observed.publication,{version:4,draftAbsent:true,current:{order:"string",tags:{type:"array",typeMismatchTreatment:"error"},priceTreatment:"ignore"},historical:{order:"number",tags:{type:"array",items:{type:"string"}},priceTreatment:"error"}});assert.deepEqual(observed.runtimeErrors,[]);
