...
const componentWidths = process.env.LOCAL_RULE_PROMOTION_BROWSER_ADAPTER === "1" ? [320]
  ...
  : process.env.SCHEMA_PROPERTY_TYPE_EDITING_BROWSER_ADAPTER === "1" ? [720]
  ...
  : process.env.DEFECT_REPORT_COMPONENT_OPTIONS_BROWSER_ADAPTER === "1" ? [720]
...
const schemaPropertyTypeEditingSeedRuntime = `(() => {
  ...
  const documentation={properties:{"/order_id":{displayName:"Order",description:"Order identifier",example:{value:42,selectionMethod:"custom"}},"/products/*/name":{displayName:"Name",description:"Product name"}}};const rules=[{id:"product-name",version:1,propertyPath:"/products/*/name",operator:"required",severity:"error"}];
  const parent={id:"base-event",name:"Base event",version:2,published:true,document:{type:"object",properties:{site_id:{type:"string"}}},assignments:[]};const schema={id:"schema-page-view",name:"Page view",version:3,published:true,parentSchemaId:"base-event",document,assignments:[],documentation,attachedRules:rules,workingDraft:{baseVersion:3,sourceVersion:3,parentSchemaId:"base-event",document:structuredClone(document),assignments:[],documentation:structuredClone(documentation),attachedRules:structuredClone(rules),pendingChanges:[]}};
  localStorage.setItem("my-chrome-utilities.schema-library.v1",JSON.stringify([schema,parent]));localStorage.setItem("my-chrome-utilities.schema-rule-library.v1","[]");return true;
})()`;
...
const schemaPropertyTypeEditingRuntime = `(async()=>{
  ...
  const property=(path)=>q('[data-schema-property-canonical-path="'+path+'"]');const inheritedAction=Array.from(property("/site_id").querySelectorAll("button")).find(({textContent})=>textContent.startsWith("Type owned by")).textContent;const storedBefore=localStorage.getItem("my-chrome-utilities.schema-library.v1");let order=property("/order_id");click(order,"Edit type");const editor=q(".schema-property-type-editor",order),value=q('[aria-label="Value type for /order_id"]',editor),treatment=q('[aria-label="Type mismatch treatment for /order_id"]',editor);const controls={valueTypes:Array.from(value.options).map(({textContent})=>textContent),treatments:Array.from(treatment.options).map(({textContent})=>textContent),defaultTreatment:treatment.selectedOptions[0].textContent,itemHidden:q('[aria-label="Item type for /order_id"]',editor).parentElement.hidden};setSelect(value,"string");click(editor,"Review type change");const review=q(".schema-property-type-review",editor).textContent,replacement=q('[aria-label="Replace example for /order_id"]',editor);replacement.value="A-42";replacement.dispatchEvent(new Event("input",{bubbles:true}));const unchangedBeforeConfirm=storedBefore===localStorage.getItem("my-chrome-utilities.schema-library.v1");click(editor,"Confirm type change");await pause();
  ...
  tags=property("/tags");click(tags,"Edit type");tagsEditor=q(".schema-property-type-editor",tags);setSelect(q('[aria-label="Item type for /tags"]',tagsEditor),"string");setSelect(q('[aria-label="Type mismatch treatment for /tags"]',tagsEditor),"warning");click(tagsEditor,"Review type change");click(tagsEditor,"Confirm type change");await pause();let arrayStored=JSON.parse(localStorage.getItem("my-chrome-utilities.schema-library.v1"))[0],arraySchema={...arrayStored,...arrayStored.workingDraft};delete arraySchema.workingDraft;const verificationArray=await import("/data-layer-schema-verification.js"),arrayEvent={sourceId:"history",eventName:"page",payload:{order_id:"A",price:19.95,tags:["x",2],products:[]}};const arrayWarning=verificationArray.validateWithSchema(arrayEvent,arraySchema,[arraySchema]).issues.filter(({message})=>message==="Type mismatch").map(({instancePath,severity})=>[instancePath,severity]);tags=property("/tags");click(tags,"Edit type");tagsEditor=q(".schema-property-type-editor",tags);setSelect(q('[aria-label="Type mismatch treatment for /tags"]',tagsEditor),"ignore");click(tagsEditor,"Review type change");click(tagsEditor,"Confirm type change");await pause();arrayStored=JSON.parse(localStorage.getItem("my-chrome-utilities.schema-library.v1"))[0];arraySchema={...arrayStored,...arrayStored.workingDraft};delete arraySchema.workingDraft;const arrayIgnored=verificationArray.validateWithSchema(arrayEvent,arraySchema,[arraySchema]).issues.some(({instancePath,message})=>instancePath==="/tags/1"&&message==="Type mismatch");tags=property("/tags");click(tags,"Edit type");tagsEditor=q(".schema-property-type-editor",tags);setSelect(q('[aria-label="Item type for /tags"]',tagsEditor),"");click(tagsEditor,"Review type change");click(tagsEditor,"Confirm type change");await pause();
  let products=property("/products");click(products,"Edit type");const productsEditor=q(".schema-property-type-editor",products);setSelect(q('[aria-label="Item type for /products"]',productsEditor),"");click(productsEditor,"Review type change");const productReview=q(".schema-property-type-review",productsEditor).textContent;const resolutionInputs=Array.from(productsEditor.querySelectorAll("[data-type-impact-resolution]"));resolutionInputs[0].click();const partialBlocked=Array.from(productsEditor.querySelectorAll("button")).find(({textContent})=>textContent==="Confirm type change").disabled;resolutionInputs.slice(1).forEach((input)=>input.click());const fullyResolved=!Array.from(productsEditor.querySelectorAll("button")).find(({textContent})=>textContent==="Confirm type change").disabled;const productBeforeCancel=localStorage.getItem("my-chrome-utilities.schema-library.v1");click(productsEditor,"Cancel");const cancel={unchanged:productBeforeCancel===localStorage.getItem("my-chrome-utilities.schema-library.v1"),focus:document.activeElement?.getAttribute("aria-label")};
  let price=property("/price");click(price,"Edit type");let priceEditor=q(".schema-property-type-editor",price),priceTreatment=q('[aria-label="Type mismatch treatment for /price"]',priceEditor);setSelect(priceTreatment,"warning");click(priceEditor,"Review type change");const beforeFailure=localStorage.getItem("my-chrome-utilities.schema-library.v1"),originalSet=Storage.prototype.setItem;Storage.prototype.setItem=function(key,value){if(key==="my-chrome-utilities.schema-library.v1")throw new Error("storage unavailable");return originalSet.call(this,key,value);};click(priceEditor,"Confirm type change");Storage.prototype.setItem=originalSet;const failure={unchanged:beforeFailure===localStorage.getItem("my-chrome-utilities.schema-library.v1"),review:q(".schema-property-type-review",priceEditor).textContent};click(priceEditor,"Confirm type change");await pause();stored=JSON.parse(localStorage.getItem("my-chrome-utilities.schema-library.v1"))[0];const effective={...stored,...stored.workingDraft};delete effective.workingDraft;const verification=await import("/data-layer-schema-verification.js");const event={sourceId:"history",eventName:"page",payload:{order_id:"A",price:"19.95",tags:["x",2],products:[]}};const warning=verification.validateWithSchema(event,effective,[effective]).issues.filter(({message})=>message==="Type mismatch").map(({instancePath,severity})=>[instancePath,severity]);
  ...
  const tagsItems=stored.workingDraft.document.properties.tags.items??null,persistedTreatment=stored.workingDraft.document.properties.price.typeMismatchTreatment;q("#save-schema").click();await pause();q("#confirm-schema-revision").click();await pause();stored=JSON.parse(localStorage.getItem("my-chrome-utilities.schema-library.v1"))[0];const historical=stored.revisionHistory.find(({version})=>version===3);const currentValidation=verification.validateWithSchema(event,stored,[stored]).issues.some(({instancePath,message})=>instancePath==="/price"&&message==="Type mismatch");const publication={version:stored.version,draftCleared:!stored.workingDraft,order:stored.document.properties.order_id.type,tagsItems:stored.document.properties.tags.items??null,treatment:stored.document.properties.price.typeMismatchTreatment,historical:{order:historical.document.properties.order_id.type,tags:historical.document.properties.tags.items.type,treatment:historical.document.properties.price.typeMismatchTreatment??"error"},currentTypeMismatch:currentValidation};
  return{controls,arrayControls,inheritedAction,arrayItem:{warning:arrayWarning,ignored:arrayIgnored},review,unchangedBeforeConfirm,orderSaved,tagsItems,impact:{review:productReview,count:resolutionInputs.length,partialBlocked,fullyResolved},cancel,failure,warning,ignored,persistedTreatment,publication,runtimeErrors:globalThis.__sidePanelRuntimeErrors??[]};
})()`;

const schemaSpecificationBuilderSeedRuntime = `(() => {
  localStorage.clear();
  const parent={id:"base-event",name:"Base event",version:3,published:true,document:{type:"object",required:["site_id"],properties:{site_id:{type:"string"},legacy_flag:{type:"boolean"}}},assignments:[],documentation:{properties:{"/site_id":{displayName:"site_id",description:"Site identifier",example:{value:"otelo",selectionMethod:"custom"}},"/legacy_flag":{displayName:"legacy_flag",description:"Legacy flag"}}},attachedRules:[{id:"site-values",version:1,propertyPath:"/site_id",operator:"allowed-values",allowedValues:["otelo","hollandsnieuwe","ben"]},{id:"legacy-required",version:1,propertyPath:"/legacy_flag",operator:"required"}]};
  const historical={id:"schema-generic-pageview",name:"Generic pageview",version:2,published:true,parentSchemaId:"base-event",document:{type:"object",properties:{legacy:{type:"string"}}},assignments:[],documentation:{properties:{"/legacy":{displayName:"legacy",description:"Historical property",example:{value:"old",selectionMethod:"custom"}}}}};
  ...
  const documentation={properties:{"/page_type":{displayName:"page_type",description:"Page classification",example:{value:"product_detail",selectionMethod:"custom"}},"/commerce/currency":{displayName:"commerce.currency",description:"Transaction currency",example:{value:"EUR",selectionMethod:"custom"}},"/products":{displayName:"products",description:"Products in the event"},"/products/*/product_name":{displayName:"products[].product_name",description:"Displayed product name",example:{value:"Phone",selectionMethod:"custom"}},"/products/*/duration":{displayName:"products[].duration",description:"Contract duration in months",example:{value:24,selectionMethod:"custom"}},"/payment_method":{displayName:"payment_method",description:"Payment method",example:{value:"card",selectionMethod:"custom"}},"/conflicting_method":{displayName:"conflicting_method",description:"Conflicting method"},"/tracking_context":{displayName:"tracking_context",description:"Tracking integration context"},"/unsafe_text":{displayName:"unsafe_text",description:"Unsafe <tag & \\"quote\\">\\tcontinued\\nline",example:{value:"safe",selectionMethod:"custom"}},"/alpha":{displayName:"alpha",description:"Alphabetical property",example:{value:1,selectionMethod:"custom"}}}};
  ...
  const schema={id:"schema-generic-pageview",name:"Generic pageview",version:4,published:true,parentSchemaId:"base-event",inheritedRuleOverrides:{legacy_flag:"disabled"},document,assignments:[],documentation,attachedRules:rules,revisionHistory:[historical],workingDraft:{baseVersion:4,sourceVersion:4,document:{...document,properties:{...document.properties,draft_only:{type:"boolean"}}},assignments:[],parentSchemaId:"base-event",inheritedRuleOverrides:{legacy_flag:"disabled"},documentation:{properties:{...documentation.properties,"/draft_only":{displayName:"draft_only",description:"Draft property"}}},attachedRules:rules,pendingChanges:["Add draft_only"]}};
...
const schemaSpecificationBuilderCustomizationRuntime=`(async()=>{const pause=()=>new Promise((resolve)=>setTimeout(resolve,0));const q=(selector,root=document)=>{const value=root.querySelector(selector);if(!value)throw new Error("Missing "+selector);return value;};const copied=[];let failRich=false;globalThis.ClipboardItem=class{constructor(data){this.data=data;this.types=Object.keys(data);}};Object.defineProperty(navigator,"clipboard",{configurable:true,value:{write:async(items)=>{if(failRich)throw new Error("rich unavailable");copied.push({kind:"rich",item:items[0]});},writeText:async(plain)=>copied.push({kind:"plain",plain})}});const before=localStorage.getItem("my-chrome-utilities.schema-library.v1");q("#data-layer-view-schemas").click();const row=Array.from(q("#schema-list").children).find(({textContent})=>textContent.includes("Generic pageview"));Array.from(row.querySelectorAll("button")).find(({textContent})=>textContent==="Build specification").click();const builder=q("#schema-specification-builder"),bar=q("#schema-specification-export-bar"),style=q('[aria-label="Table style"]',bar),headings=q('input[type="checkbox"]',bar);const defaults={spreadsheet:q('input[value="spreadsheet"]',bar).checked,headings:headings.checked,styleHidden:style.parentElement.hidden,bars:builder.querySelectorAll("#schema-specification-export-bar").length};const type=q('th[data-column="type"]'),mandatory=q('th[data-column="mandatory"]'),transfer=new DataTransfer();type.dispatchEvent(new DragEvent("dragstart",{bubbles:true,dataTransfer:transfer}));mandatory.dispatchEvent(new DragEvent("drop",{bubbles:true,dataTransfer:transfer}));const moved=Array.from(builder.querySelectorAll("th > span"),({textContent})=>textContent);const duration=q('tr[data-property-path="/products/*/duration"] td[data-column="example"]');duration.click();const editor=q(".schema-specification-example-editor",duration),choices=Array.from(editor.querySelectorAll("label"),({textContent})=>textContent.trim());Array.from(editor.querySelectorAll("label")).find(({textContent})=>textContent.includes("Allowed value 12")).querySelector("input").click();const example=q('tr[data-property-path="/products/*/duration"] td[data-column="example"]').textContent;Array.from(builder.querySelectorAll("button")).find(({textContent})=>textContent==="Copy specification table").click();await pause();const spreadsheet=copied.at(-1).plain;headings.click();Array.from(builder.querySelectorAll("button")).find(({textContent})=>textContent==="Copy specification table").click();await pause();const unheaded=copied.at(-1).plain;q('input[value="rich"]',bar).click();headings.click();style.value="highlighted";style.dispatchEvent(new Event("change",{bubbles:true}));Array.from(builder.querySelectorAll("button")).find(({textContent})=>textContent==="Copy specification table").click();await pause();const rich=copied.at(-1).item;const html=await rich.data["text/html"].text(),plain=await rich.data["text/plain"].text();failRich=true;Array.from(builder.querySelectorAll("button")).find(({textContent})=>textContent==="Copy specification table").click();await pause();const fallback={plain:copied.at(-1).plain,feedback:q("#schema-specification-copy-feedback").textContent};const source=q("#schema-specification-source");source.value="historical:2";source.dispatchEvent(new Event("change",{bubbles:true}));source.value="published:4";source.dispatchEvent(new Event("change",{bubbles:true}));const resetExample=q('tr[data-property-path="/products/*/duration"] td[data-column="example"]').textContent,preserved={rich:q('input[value="rich"]',bar).checked,headings:headings.checked,style:style.value,columns:Array.from(builder.querySelectorAll("th > span"),({textContent})=>textContent)};const layout={width:innerWidth,barWidth:bar.getBoundingClientRect().width,tableWidth:q("#schema-specification-preview").scrollWidth,copyVisible:Array.from(builder.querySelectorAll("button")).find(({textContent})=>textContent==="Copy specification table").getClientRects().length>0};return{defaults,moved,choices,example,spreadsheet,unheaded,rich:{types:rich.types,html,plain,styleVisible:!style.parentElement.hidden},fallback,resetExample,preserved,layout,unchanged:before===localStorage.getItem("my-chrome-utilities.schema-library.v1"),runtimeErrors:globalThis.__sidePanelRuntimeErrors??[]};})()`;

const schemaSpecificationBuilderProvenanceRuntime=`(()=>{const q=(selector,root=document)=>{const value=root.querySelector(selector);if(!value)throw new Error("Missing "+selector);return value;};const labels=(path)=>{const cell=q('tr[data-property-path="'+path+'"] td[data-column="example"]');cell.click();return Array.from(q(".schema-specification-example-editor",cell).querySelectorAll("label"),({textContent})=>textContent.trim());};return{conditional:labels("/products/*/duration"),inherited:labels("/site_id")};})()`;

const schemaPropertyCommentsRuntime=`(async()=>{const pause=()=>new Promise((resolve)=>setTimeout(resolve,0));const q=(selector,root=document)=>{const value=root.querySelector(selector);if(!value)throw new Error("Missing "+selector);return value;};const copied=[];globalThis.ClipboardItem=class{constructor(data){this.data=data;this.types=Object.keys(data);}};Object.defineProperty(navigator,"clipboard",{configurable:true,value:{write:async(items)=>copied.push(items[0]),writeText:async(plain)=>copied.push({plain})}});const original=JSON.parse(localStorage.getItem("my-chrome-utilities.schema-library.v1")).find(({name})=>name==="Generic pageview"),publishedBefore=JSON.stringify(original.documentation);q("#data-layer-view-schemas").click();const row=Array.from(q("#schema-list").children).find(({textContent})=>textContent.includes("Generic pageview"));Array.from(row.querySelectorAll("button")).find(({textContent})=>textContent==="Edit working draft").click();let property=q('[data-schema-property-canonical-path="/products/*/product_name"]');property.querySelector(".schema-property-documentation-control").click();const comments=q('[id^="schema-documentation-comments-"]',property);comments.value="  Sent by checkout"+String.fromCharCode(10)+"Do not derive from position  ";comments.dispatchEvent(new Event("input",{bubbles:true}));property.querySelector('input[value="Save documentation"]').click();await pause();const stored=JSON.parse(localStorage.getItem("my-chrome-utilities.schema-library.v1")).find(({name})=>name==="Generic pageview"),saved=stored.workingDraft.documentation.properties["/products/*/product_name"].comments,publishedUnchanged=JSON.stringify(stored.documentation)===publishedBefore;property=q('[data-schema-property-canonical-path="/products/*/product_name"]');property.querySelector(".schema-property-documentation-control").click();const reopened=q('[id^="schema-documentation-comments-"]',property).value;q("#build-specification").click();const builder=q("#schema-specification-builder"),headings=Array.from(builder.querySelectorAll("th > span"),({textContent})=>textContent),specRow=q('tr[data-property-path="/products/*/product_name"]',builder),cells=Array.from(specRow.children,({textContent})=>textContent);builder.querySelector('input[value="rich"]').click();Array.from(builder.querySelectorAll("button")).find(({textContent})=>textContent==="Copy specification table").click();await pause();const item=copied[0],html=await item.data["text/html"].text(),plain=await item.data["text/plain"].text();return{saved,reopened,publishedUnchanged,headings,cells,clipboard:{html,plain},runtimeErrors:globalThis.__sidePanelRuntimeErrors??[]};})()`;
...
try {
  ...
  for (const width of componentWidths) {
    ...
    if(process.env.SCHEMA_SPECIFICATION_BUILDER_CUSTOMIZATION_BROWSER_ADAPTER==="1"){
      await evaluate(socket,schemaSpecificationBuilderSeedRuntime);await reloadPanel(socket);schemaSpecificationBuilderCustomizationObservation=await evaluate(socket,schemaSpecificationBuilderCustomizationRuntime);const observed=schemaSpecificationBuilderCustomizationObservation;const provenance=await evaluate(socket,schemaSpecificationBuilderProvenanceRuntime);observed.provenance=provenance;assert.deepEqual(observed.defaults,{spreadsheet:true,headings:true,styleHidden:true,bars:1});assert.deepEqual(observed.moved,["Property name","Description","Type","Mandatory","Example value","Allowed values"]);assert.match(observed.choices.join(" "),/Documentation.*Allowed value 12.*Allowed value 24.*Blank.*Custom value/);assert.match(provenance.conditional.join(" "),/Allowed value 12.*when price_monthly exists/);assert.match(provenance.inherited.join(" "),/Allowed value otelo.*inherited/);assert.equal(observed.example,"12");assert.match(observed.spreadsheet,/^Property name\tDescription\tType\tMandatory/);assert.doesNotMatch(observed.unheaded,/^Property name/);assert.deepEqual(observed.rich.types,["text/html","text/plain"]);assert.match(observed.rich.html,/border:1px solid.*background:#eee/);assert.match(observed.rich.plain,/^Property name\tDescription\tType\tMandatory/);assert.match(observed.fallback.plain,/^Property name\tDescription\tType\tMandatory/);assert.match(observed.fallback.feedback,/only plain text|copied plain text/i);assert.equal(observed.resetExample,"24");assert.deepEqual(observed.preserved,{rich:true,headings:true,style:"highlighted",columns:["Property name","Description","Type","Mandatory","Example value","Allowed values"]});assert.equal(observed.layout.width,320);assert.equal(observed.layout.copyVisible,true);assert.ok(observed.layout.barWidth<=320);assert.equal(observed.rich.styleVisible&&observed.unchanged,true);assert.deepEqual(observed.runtimeErrors,[]);socket.close();continue;
    }
    if(process.env.SCHEMA_PROPERTY_COMMENTS_BROWSER_ADAPTER==="1"){
      await evaluate(socket,schemaSpecificationBuilderSeedRuntime);await reloadPanel(socket);schemaPropertyCommentsObservation=await evaluate(socket,schemaPropertyCommentsRuntime);const observed=schemaPropertyCommentsObservation;assert.equal(observed.saved,"Sent by checkout\nDo not derive from position");assert.equal(observed.reopened,observed.saved);assert.equal(observed.publishedUnchanged,true);assert.deepEqual(observed.headings,["Property name","Description","Mandatory","Type","Example value","Allowed values","Comments"]);assert.equal(observed.cells[6],observed.saved);assert.match(observed.clipboard.html,/Comments[\s\S]*Sent by checkout<br>Do not derive from position/);assert.match(observed.clipboard.plain,/Allowed values\tComments/);assert.equal(observed.clipboard.plain.split("\n").some((line)=>line.endsWith("Sent by checkout Do not derive from position")),true);assert.deepEqual(observed.runtimeErrors,[]);socket.close();continue;
    }
    if (process.env.SCHEMA_PROPERTY_TYPE_EDITING_BROWSER_ADAPTER === "1") {
      ...
      assert.deepEqual(observed.controls,{valueTypes:["String","Number","Boolean","Object","Array"],treatments:["Error","Warning","Ignore"],defaultTreatment:"Error",itemHidden:true});assert.deepEqual(observed.arrayControls,{itemTypes:["Any item type","String","Number","Boolean","Object"],initial:"String"});assert.match(observed.review,/Number changing to String.*example value/);assert.equal(observed.unchangedBeforeConfirm,true);assert.deepEqual(observed.orderSaved,{published:"number",draft:"string",required:["order_id"],description:"Order identifier",example:"A-42"});assert.equal(observed.tagsItems,null);
      assert.match(observed.inheritedAction,/Type owned by Base event/);assert.deepEqual(observed.arrayItem.warning,[["/tags/1","warning"]]);assert.equal(observed.arrayItem.ignored,false);assert.match(observed.impact.review,/descendant definitions.*descendant documentation.*descendant rules/);assert.equal(observed.impact.count,3);assert.equal(observed.impact.partialBlocked&&observed.impact.fullyResolved,true);assert.equal(observed.cancel.unchanged,true);assert.match(observed.cancel.focus,/Edit type for \/products/);assert.equal(observed.failure.unchanged,true);assert.match(observed.failure.review,/not saved.*storage unavailable/);assert.deepEqual(observed.warning,[["/price","warning"]]);assert.equal(observed.ignored,false);assert.equal(observed.persistedTreatment,"ignore");assert.deepEqual(observed.publication,{version:4,draftCleared:true,order:"string",tagsItems:null,treatment:"ignore",historical:{order:"number",tags:"string",treatment:"error"},currentTypeMismatch:false});assert.deepEqual(observed.runtimeErrors,[]);
      await reloadPanel(socket);const reloaded=await evaluate(socket,`(()=>{const q=(selector,root=document)=>{const value=root.querySelector(selector);if(!value)throw new Error("Missing "+selector);return value;};q("#data-layer-view-schemas").click();const row=Array.from(q("#schema-list").children).find(({textContent})=>textContent.includes("Page view"));Array.from(row.querySelectorAll("button")).find(({textContent})=>textContent==="Edit working draft").click();const order=q('[data-schema-property-canonical-path="/order_id"]');const price=q('[data-schema-property-canonical-path="/price"]');const metadata={order:order.querySelector(".schema-property-metadata").textContent,priceAction:Array.from(price.querySelectorAll("button")).find(({textContent})=>textContent.startsWith("Edit type")).textContent};Array.from(row.querySelectorAll("button")).find(({textContent})=>textContent==="Build specification").click();const specRows=Array.from(q("#schema-specification-preview").querySelectorAll("tbody tr")).map((tr)=>Array.from(tr.children).map(({textContent})=>textContent));return{...metadata,spec:specRows.find(([name])=>name==="order_id")};})()`);assert.match(reloaded.order,/type string/);assert.match(reloaded.priceAction,/Number/);assert.equal(reloaded.spec[3],"String");schemaPropertyTypeEditingObservation.reloaded=reloaded;
      socket.close();continue;
    }
  ...
  if(process.env.SCHEMA_SPECIFICATION_BUILDER_CUSTOMIZATION_BROWSER_ADAPTER==="1")console.log(JSON.stringify({schemaSpecificationBuilderCustomization:schemaSpecificationBuilderCustomizationObservation}));
