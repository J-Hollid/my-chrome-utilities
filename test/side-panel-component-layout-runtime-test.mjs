      ...
      specificationProjectObservation=await evaluate(socket,`(async()=>{
        ...
        return{created,search,bulk,afterUndo,preflight,release,stored:{name:stored.project.name,collections:Object.fromEntries(Object.entries(stored.project.collections).map(([kind,items])=>[kind,items.length])),releases:stored.project.releases.length,draft:Boolean(stored.draft)},layout};
	      ...
	      specificationProjectObservation.corrections=await evaluate(socket,`(async()=>{
	        ...
	        q('#project-tree button[data-kind="flows"]').click();q("#workspace-content .entity-row button").click();let stored=JSON.parse(localStorage.getItem("my-chrome-utilities.specification-project.v1")),retailFlow=stored.project.collections.flows[0],event=stored.project.collections.events[0],page=stored.project.collections.pages[0];const addStep=async({name,pageId="",eventId="",minimum=1,maximum=1,optional=false,branch="",from="",to=""})=>{set("#flow-step-name",name);set("#flow-step-page",pageId);set("#flow-step-event",eventId);set("#flow-step-minimum",minimum);set("#flow-step-maximum",maximum);q("#flow-step-optional").checked=optional;set("#flow-step-branch",branch);set("#flow-step-from",from);set("#flow-step-to",to);q("#add-flow-step-form").requestSubmit();await pause();};await addStep({name:"Product",pageId:page.id,eventId:event.id,maximum:5,branch:"retail-checkout"});await addStep({name:"Upsell",pageId:page.id,optional:true,branch:"upsell-or-checkout",from:"product",to:"confirmation"});await addStep({name:"Retail confirmation",pageId:page.id,eventId:event.id,from:"upsell",to:"confirmation"});
	        await add("flows","Trade checkout");await addStep({name:"Trade account",pageId:page.id,branch:"trade-checkout"});await addStep({name:"Trade confirmation",pageId:page.id,eventId:event.id,from:"account",to:"confirmation"});for(let index=3;index<=50;index+=1)await add("flows","Benchmark flow "+index);
	        await add("profiles","Retail confirmation requirements");await add("profiles","Trade account requirements");stored=JSON.parse(localStorage.getItem("my-chrome-utilities.specification-project.v1"));const retailProfile=stored.project.collections.profiles[1],tradeProfile=stored.project.collections.profiles[2],tradeFlow=stored.project.collections.flows[1];set("#project-schema-name","Sitewide page context");set("#project-schema-id","schema-sitewide");set("#project-schema-revision",1);set("#project-schema-description","Shared event envelope");q("#create-project-schema-draft").requestSubmit();await pause();stored=JSON.parse(localStorage.getItem("my-chrome-utilities.specification-project.v1"));const publishedSchemaBytes=JSON.stringify(stored.project.collections.schemaDrafts[0].publishedRevision),saveAssignment=async({name,schemaId,selector,policy="pinned"})=>{set("#project-assignment-name",name);set("#project-assignment-schema",schemaId);set("#project-assignment-source","event-history");set("#project-assignment-event","purchase");set("#project-assignment-priority",10);q("#project-assignment-version-policy").value=policy;set("#project-assignment-revision",1);set("#project-assignment-path","flowId");set("#project-assignment-value",selector);set("#project-assignment-not-path","");set("#project-assignment-not-value","");q("#save-project-assignment").requestSubmit();await pause();};await saveAssignment({name:"Retail",schemaId:retailProfile.id,selector:"retail checkout"});await saveAssignment({name:"Trade",schemaId:tradeProfile.id,selector:"trade checkout",policy:"follow latest"});
	        set("#project-assignment-search","retail");const assignmentSearch={count:q("#project-assignment-count").textContent,rows:Array.from(q("#project-assignment-list").querySelectorAll("button"),({textContent})=>textContent),empty:!q("#project-assignment-empty").hidden};q("#project-assignment-list button").click();const retailId=q("#project-assignment-id").value,conditionBefore=JSON.parse(localStorage.getItem("my-chrome-utilities.specification-project.v1")).project.collections.assignments.find(({id})=>id===retailId).condition;set("#project-assignment-priority",20);q("#save-project-assignment").requestSubmit();await pause();stored=JSON.parse(localStorage.getItem("my-chrome-utilities.specification-project.v1"));const editedRetail=stored.project.collections.assignments.find(({id})=>id===retailId),assignmentLifecycle={search:assignmentSearch,ids:stored.project.collections.assignments.map(({id})=>id),stableId:editedRetail.id===retailId,conditionPreserved:JSON.stringify(editedRetail.condition)===JSON.stringify(conditionBefore),pinnedRevision:editedRetail.schemaRevision,publishedUnchanged:JSON.stringify(stored.project.collections.schemaDrafts[0].publishedRevision)===publishedSchemaBytes};q("#project-assignment-id").value="";set("#project-assignment-name","Blank placeholder");set("#project-assignment-schema","");set("#project-assignment-source","");set("#project-assignment-event","");q("#save-project-assignment").noValidate=true;q("#save-project-assignment").requestSubmit();q("#save-project-assignment").noValidate=false;await pause();assignmentLifecycle.blankExcluded=JSON.parse(localStorage.getItem("my-chrome-utilities.specification-project.v1")).project.collections.assignments.length===2;assignmentLifecycle.blankMessage=q("#project-assignment-conflicts").textContent;
	        ...
	        stored=JSON.parse(localStorage.getItem("my-chrome-utilities.specification-project.v1"));const module=await import("./data-layer-specification-project.js"),retail0=module.startFlowInstance(stored.project,retailFlow.id,"retail-tab"),retail1=module.advanceFlowInstance(stored.project,retail0,{eventId:event.id,pageId:page.id}),trade0=module.startFlowInstance(stored.project,tradeFlow.id,"trade-tab"),trade1=module.advanceFlowInstance(stored.project,trade0,{pageId:page.id}),retailFinal=module.resolveApplicability(stored.project,{pathname:"/checkout/confirmation",eventName:"purchase",flowId:retail1.selector,payload:{}}),tradeFinal=module.resolveApplicability(stored.project,{pathname:"/checkout/confirmation",eventName:"purchase",flowId:trade1.selector,payload:{}});localStorage.setItem("my-chrome-utilities.flow-instances.v1",JSON.stringify([retail1,trade1]));const decisive={retail:{selector:retail1.selector,winner:retailFinal.winner?.id,schemaId:stored.project.collections.assignments.find(({id})=>id===retailFinal.winner?.id)?.schemaId},trade:{selector:trade1.selector,winner:tradeFinal.winner?.id,schemaId:stored.project.collections.assignments.find(({id})=>id===tradeFinal.winner?.id)?.schemaId},markerPresent:false,ambiguous:retailFinal.ties.length!==1||tradeFinal.ties.length!==1};
