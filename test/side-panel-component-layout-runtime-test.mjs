...
const schemaPropertyTypeEditingSeedRuntime = `(() => {
  ...
  const schema={id:"schema-page-view",name:"Page view",version:3,published:true,document,assignments:[],documentation,attachedRules:rules,workingDraft:{baseVersion:3,sourceVersion:3,document:structuredClone(document),assignments:[],documentation:structuredClone(documentation),attachedRules:structuredClone(rules),pendingChanges:[]}};
  localStorage.setItem("my-chrome-utilities.schema-library.v1",JSON.stringify([schema]));localStorage.setItem("my-chrome-utilities.schema-rule-library.v1",JSON.stringify([{id:"range",name:"Reusable range",kind:"Numeric range",version:7,operator:"numeric-range",parameters:"1,99",attachments:["schema-page-view"]}]));return true;
})()`;
...
try {
  ...
  for (const width of componentWidths) {
    ...
    if (process.env.SCHEMA_PROPERTY_TYPE_EDITING_BROWSER_ADAPTER === "1") {
      ...
      assert.deepEqual(observed.controls,{valueTypes:["String","Number","Boolean","Object","Array"],treatments:["Error","Warning","Ignore"],defaultTreatment:"Error",itemHidden:true});assert.deepEqual(observed.arrayControls,{itemTypes:["Any item type","String","Number","Boolean","Object"],initial:"String"});assert.match(observed.review,/Number changing to String/);assert.match(observed.review,/example value.*rule range.*conditional dependency order-condition/);assert.equal(observed.impactChoices.count,3);assert.deepEqual(observed.impactChoices.options["example value"],["Choose resolution","Remove artifact","Replace artifact"]);assert.deepEqual(observed.impactChoices.options["rule range"],["Choose resolution","Remove artifact"]);assert.deepEqual(observed.impactChoices.options["conditional dependency order-condition"],["Choose resolution","Remove artifact","Replace artifact"]);assert.deepEqual({cancel:observed.impactChoices.cancel,blocked:observed.impactChoices.blocked,resolved:observed.impactChoices.resolved},{cancel:true,blocked:true,resolved:true});assert.match(observed.descendantImpact.review,/descendant definitions.*descendant required relationships.*descendant documentation.*descendant rules/);assert.deepEqual({blocked:observed.descendantImpact.blocked,unchanged:observed.descendantImpact.unchanged,focus:observed.descendantImpact.focus},{blocked:true,unchanged:true,focus:true});assert.match(observed.persistenceFailure.message,/Simulated persistence failure/);assert.deepEqual({storedUnchanged:observed.persistenceFailure.storedUnchanged,inMemoryType:observed.persistenceFailure.inMemoryType,resolutions:observed.persistenceFailure.resolutions},{storedUnchanged:true,inMemoryType:"Number",resolutions:["replace","remove","replace"]});assert.equal(observed.unchangedBeforeConfirm,true);assert.deepEqual(observed.orderSaved,{published:"number",draft:"string",required:["order_id"],description:"Order identifier",example:"ORDER-42",condition:{type:"string",value:"ORDER-42"}});assert.equal(observed.tagsItems,null);assert.deepEqual(observed.warning,[["/price","warning"]]);assert.equal(observed.ignored,false);assert.equal(observed.persistedTreatment,"ignore");assert.deepEqual(observed.remainingRules,["product-name-required","order-condition"]);assert.equal(observed.reusableUnchanged,true);assert.deepEqual(observed.publication,{version:4,draftAbsent:true,current:{order:"string",tags:{type:"array",typeMismatchTreatment:"error"},priceTreatment:"ignore"},historical:{order:"number",tags:{type:"array",items:{type:"string"}},priceTreatment:"error"}});assert.deepEqual(observed.runtimeErrors,[]);
      await reloadPanel(socket);const reloaded=await evaluate(socket,`(()=>{const q=(selector,root=document)=>{const value=root.querySelector(selector);if(!value)throw new Error("Missing "+selector);return value;};q("#data-layer-view-schemas").click();const row=Array.from(q("#schema-list").children).find(({textContent})=>textContent.includes("Page view"));Array.from(row.querySelectorAll("button")).find(({textContent})=>textContent==="Build specification").click();const specRows=Array.from(q("#schema-specification-preview").querySelectorAll("tbody tr")).map((tr)=>Array.from(tr.children).map(({textContent})=>textContent));const stored=JSON.parse(localStorage.getItem("my-chrome-utilities.schema-library.v1"))[0];return{version:stored.version,draftAbsent:!stored.workingDraft,order:stored.document.properties.order_id.type,priceTreatment:stored.document.properties.price.typeMismatchTreatment,spec:specRows.find(([name])=>name==="order_id")};})()`);assert.deepEqual({version:reloaded.version,draftAbsent:reloaded.draftAbsent,order:reloaded.order,priceTreatment:reloaded.priceTreatment,specType:reloaded.spec[3]},{version:4,draftAbsent:true,order:"string",priceTreatment:"ignore",specType:"String"});schemaPropertyTypeEditingObservation.reloaded=reloaded;
      socket.close();continue;
    }
