  ...
  for (const width of componentWidths) {
    ...
    if (process.env.DEFECT_REPORT_COMPONENT_OPTIONS_BROWSER_ADAPTER === "1") {
      defectReportComponentOptionsObservation = await evaluate(socket, `(async () => {
        ...
        const currencyResponse=q('input[name="defect-response-currency"][value="EUR"]',root);currencyResponse.checked=true;currencyResponse.dispatchEvent(new Event("change",{bubbles:true}));
        ...
        const order=q("#defect-issue-order_id",root);order.checked=false;order.dispatchEvent(new Event("change",{bubbles:true}));order.checked=true;order.dispatchEvent(new Event("change",{bubbles:true}));
        click(root,"Generate pathname steps");const add=q('button[aria-label^="Add step to "]',root);add.click();click(root,"Custom step");const custom=q('[data-reproduction-field="customText"]',root);custom.value="Confirm order summary";custom.dispatchEvent(new Event("input",{bubbles:true}));click(root,"Add step");
        const afterRefresh={states:controls.map(({checked})=>checked),headings:headings(preview),focus:document.activeElement===controls[2],scroll:root.scrollTop};
        ...
        return {initial,combinations,evidence:{rules:(evidenceText.match(/Allowed currency v2/g)||[]).length,schema:evidenceText.includes("Checkout version 4"),severity:evidenceText.includes("error"),pointer:evidenceText.includes("/commerce/currency"),violation:evidenceText.includes("Value is not allowed"),constraint:evidenceText.includes("one of EUR or USD"),actual:evidenceText.includes("GBP"),capture:["purchase","Event history","https://shop.example/checkout","2026-07-15T12:00:00Z"].every((value)=>evidenceText.includes(value)),noProvenance:!evidenceText.includes("response source")},withoutDifferences,restoredDifferences,componentControlRefresh,afterRefresh,stored:{components:defect.report.components,preview:reopenedBefore,afterEdit:reopenedAfter,rich:headings(Object.assign(document.createElement("section"),{innerHTML:rich[0].html})),recopy:headings(Object.assign(document.createElement("section"),{innerHTML:recopyRich[0].html})),structured:Boolean(defect.report.actual.differences.length&&defect.report.evidence.validation.length),notes:defect.notes},legacy:{headings:legacyHeadings,copy:headings(Object.assign(document.createElement("section"),{innerHTML:legacyCopy[0].html})),unchanged:legacyBefore===JSON.stringify(legacy),newDefaults},structuredBefore};
      })()`);
      ...
      assert.deepEqual(defectReportComponentOptionsObservation.componentControlRefresh,{focus:true,scroll:43});
