...
try {
  ...
  for (const width of componentWidths) {
    ...
    const capturedContinuationSeed=`(()=>{const event={id:"capture:purchase",name:"purchase",sourceId:"event-history",sourceName:"Event history",captureTime:"2026-07-18T16:00:00Z",pageUrl:"https://shop.example/confirmation",payload:{currency:"EUR",order_id:"o-1"},rawInput:[],validation:"Valid"},session={id:"session:capture",name:"Retail capture",pageScope:"https://shop.example/confirmation",startedAt:"2026-07-18T16:00:00Z",endedAt:"2026-07-18T16:01:00Z",events:[event],immutable:true},view={view:"Live",status:"Paused",pageUrl:session.pageScope,sources:[{id:"event-history",name:"Event history",status:"Saved session"}],events:[event],listVisible:true},project={id:"project:retail-trade",name:"Retail and Trade",description:"",site:"shop.example",environments:["Production"],namingConventions:{property:"snake_case",event:"snake_case"},publicationPolicy:{warningsBlock:false,fixturesRequired:true},collections:{profiles:[{id:"profile:retail",name:"Retail",requirements:[]}],pages:[{id:"page:confirmation",name:"Checkout confirmation"}],pageGroups:[],events:[{id:"event:purchase",name:"Purchase",eventName:"purchase",sourceId:"event-history",target:"payload"}],applicabilitySets:[],flows:[{id:"flow:retail",name:"Retail checkout",steps:[{id:"step:confirmation",name:"Confirmation",eventId:"event:purchase"}]}],fixtures:[],schemaDrafts:[{id:"schema:purchase",name:"Purchase",version:4,published:true,document:{type:"object",properties:{currency:{type:"string"},order_id:{type:"string"}}},assignments:[],workingDraft:{name:"Purchase",baseVersion:4,sourceVersion:4,document:{type:"object",properties:{currency:{type:"string"},order_id:{type:"string"}}},assignments:[],pendingChanges:[]}}],assignments:[]},releases:[]},state={project,draft:{id:"draft:retail-trade",status:"Saved",updatedAt:"2026-07-18T16:00:00Z"},history:{undo:[],redo:[]}};localStorage.setItem("my-chrome-utilities.specification-project.v1",JSON.stringify(state));localStorage.setItem("my-chrome-utilities.saved-session-library.v1",JSON.stringify({sessions:[session]}));localStorage.setItem("my-chrome-utilities.saved-session-live-feed.v1",JSON.stringify({version:1,sessionId:session.id,currentView:view,savedView:view,currentScrollTop:0,savedScrollTop:0,backgroundEventCount:0}));localStorage.setItem("my-chrome-utilities.schema-validation-records.v1",JSON.stringify([{eventId:event.id,eventName:event.name,state:"Valid",checkedAt:"2026-07-18T16:01:00Z",schemaId:"schema:purchase",schemaName:"Purchase",schemaVersion:4,target:"payload",issueCodes:[]} ]));Object.defineProperty(globalThis,"open",{configurable:true,value:(url)=>{globalThis.__capturedBuilderUrl=String(url);return{};}});})()`;
    const socket = capturedContinuationScenario ? await openPanelWithInitialization(port,width,capturedContinuationSeed) : process.env.SPECIFICATION_PROJECT_BROWSER_ADAPTER === "1" ? await openSpecificationBuilder(port,width) : await openPanel(port, width);
    if (process.env.SPECIFICATION_PROJECT_BROWSER_ADAPTER === "1") {
      if(capturedContinuationScenario){
        specificationProjectObservation=await evaluate(socket,`(async()=>{const q=(selector)=>{const value=document.querySelector(selector);if(!value)throw new Error("Missing "+selector);return value;},pause=()=>new Promise((resolve)=>setTimeout(resolve,0));q("#data-layer-view-schemas").click();await pause();const continueButton=Array.from(q("#schema-validation-record-list").querySelectorAll("button")).find(({textContent})=>textContent==="Continue in project");if(!continueButton)throw new Error("Missing Continue in project action: "+q("#schema-validation-record-list").textContent);continueButton.click();await pause();const dialog=Array.from(document.querySelectorAll("dialog[open]")).find(({textContent})=>textContent.includes("Continue captured validation in project"));if(!dialog)throw new Error("Missing captured continuation review");const labels=Array.from(dialog.querySelectorAll("label"),({textContent})=>textContent),eventNames=Array.from(dialog.querySelectorAll("label select")).find((select)=>select.parentElement.textContent.includes("Event"))?[...Array.from(dialog.querySelectorAll("label select")).find((select)=>select.parentElement.textContent.includes("Event")).options].map(({textContent})=>textContent):[];Array.from(dialog.querySelectorAll("button")).find(({textContent})=>textContent.includes("Save Fixture"))?.click();await pause();const stored=JSON.parse(localStorage.getItem("my-chrome-utilities.specification-project.v1")),fixture=stored.project.collections.fixtures[0],navigation=JSON.parse(localStorage.getItem("my-chrome-utilities.specification-project-navigation.v1"));return{corrections:{capturedContinuation:{reviewOpen:true,labels,eventNames,provenance:fixture.provenance.kind,assertionCount:fixture.assertions.length,observationName:fixture.observations[0].eventName,fixtureId:fixture.id,navigationId:navigation.id,openedBuilder:String(globalThis.__capturedBuilderUrl).includes("specification-builder.html?kind=fixtures")}}};})()`);
        const continuation=specificationProjectObservation.corrections.capturedContinuation;assert.deepEqual(continuation.eventNames,["Purchase"]);assert.equal(continuation.provenance,"captured-validation");assert.equal(continuation.assertionCount,2);assert.equal(continuation.fixtureId,continuation.navigationId);assert.equal(continuation.openedBuilder,true);socket.close();continue;
      }
