...
const schemaPropertyTypeEditingSeedRuntime = `(() => {
  ...
  const schema={id:"schema-page-view",name:"Page view",version:3,published:true,document,assignments:[],documentation:{properties:{"/order_id":{displayName:"Order",description:"Order identifier"}}},workingDraft:{baseVersion:3,sourceVersion:3,document:structuredClone(document),assignments:[],documentation:{properties:{"/order_id":{displayName:"Order",description:"Order identifier"}}},pendingChanges:[]}};
  localStorage.setItem("my-chrome-utilities.schema-library.v1",JSON.stringify([schema]));localStorage.setItem("my-chrome-utilities.schema-rule-library.v1","[]");return true;
})()`;
...
const schemaPropertyTypeEditingRuntime = `(async()=>{
  ...
  return{controls,arrayControls,review,unchangedBeforeConfirm,orderSaved,tagsItems:stored.workingDraft.document.properties.tags.items??null,warning,ignored,persistedTreatment:stored.workingDraft.document.properties.price.typeMismatchTreatment,runtimeErrors:globalThis.__sidePanelRuntimeErrors??[]};
})()`;
...
try {
  ...
  for (const width of componentWidths) {
    ...
    if (process.env.SCHEMA_PROPERTY_TYPE_EDITING_BROWSER_ADAPTER === "1") {
      ...
      assert.deepEqual(observed.controls,{valueTypes:["String","Number","Boolean","Object","Array"],treatments:["Error","Warning","Ignore"],defaultTreatment:"Error",itemHidden:true});assert.deepEqual(observed.arrayControls,{itemTypes:["Any item type","String","Number","Boolean","Object"],initial:"String"});assert.match(observed.review,/Number changing to String/);assert.equal(observed.unchangedBeforeConfirm,true);assert.deepEqual(observed.orderSaved,{published:"number",draft:"string",required:["order_id"],description:"Order identifier"});assert.equal(observed.tagsItems,null);assert.deepEqual(observed.warning,[["/price","warning"]]);assert.equal(observed.ignored,false);assert.equal(observed.persistedTreatment,"ignore");assert.deepEqual(observed.runtimeErrors,[]);
      await reloadPanel(socket);const reloaded=await evaluate(socket,`(()=>{const q=(selector,root=document)=>{const value=root.querySelector(selector);if(!value)throw new Error("Missing "+selector);return value;};q("#data-layer-view-schemas").click();const row=Array.from(q("#schema-list").children).find(({textContent})=>textContent.includes("Page view"));Array.from(row.querySelectorAll("button")).find(({textContent})=>textContent==="Edit working draft").click();const order=q('[data-schema-property-canonical-path="/order_id"]');const price=q('[data-schema-property-canonical-path="/price"]');q("#build-specification").click();const specRows=Array.from(q("#schema-specification-preview").querySelectorAll("tbody tr")).map((tr)=>Array.from(tr.children).map(({textContent})=>textContent));return{order:order.querySelector(".schema-property-metadata").textContent,priceAction:Array.from(price.querySelectorAll("button")).find(({textContent})=>textContent.startsWith("Edit type")).textContent,spec:specRows.find(([name])=>name==="order_id")};})()`);assert.match(reloaded.order,/type string/);assert.match(reloaded.priceAction,/Number/);assert.equal(reloaded.spec[3],"String");schemaPropertyTypeEditingObservation.reloaded=reloaded;
      socket.close();continue;
    }
