  ...
  for (const width of componentWidths) {
    ...
    if (process.env.SPECIFICATION_PROJECT_BROWSER_ADAPTER === "1") {
      if(capturedContinuationScenario){
        ...
        const continuation=specificationProjectObservation.corrections.capturedContinuation;assert.deepEqual(profileContinuation.paths,["/currency","/order_id"]);assert.ok(profileContinuation.evidence.every((identity)=>identity===continuation.evaluationResultIdentity));assert.equal(profileContinuation.navigation.kind,"profiles");await wait(500);const browserTargets=await fetch(`http://127.0.0.1:${port}/json/list`).then((response)=>response.json());continuation.openedBuilder=browserTargets.some(({url})=>url.includes("specification-builder.html?kind=fixtures"));assert.deepEqual(continuation.eventNames,["Purchase"]);assert.equal(continuation.provenance,"captured-validation");assert.equal(continuation.assertionCount,2);assert.match(continuation.evaluationResultIdentity,/^result:/);assert.match(continuation.reviewText,/Evaluated result result:/);assert.match(continuation.reviewText,/Profile requirements: \/currency/);assert.match(continuation.reviewText,/\/order_id \(string, required\)/);assert.equal(continuation.fixtureId,continuation.navigationId);assert.equal(continuation.openedBuilder,true,JSON.stringify(browserTargets.map(({url})=>url)));socket.close();continue;
      }
