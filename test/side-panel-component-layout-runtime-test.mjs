  ...
  for (const width of componentWidths) {
    ...
    if (process.env.SPECIFICATION_PROJECT_BROWSER_ADAPTER === "1") {
      if(capturedContinuationScenario){
        ...
        await socket.call("Page.addScriptToEvaluateOnNewDocument",{source:`(()=>{const messages=[],channels=new Map(),api={__emitCapture(rawValue){const channelId=channels.get(101);if(!channelId)throw new Error("No production capture channel");for(const listener of [...messages])listener({type:"my-chrome-utilities.data-layer-history-entry",channelId,rawValue,timestamp:new Date().toISOString()},{tab:{id:101}});},runtime:{onMessage:{addListener(listener){messages.push(listener);},removeListener(listener){const at=messages.indexOf(listener);if(at>=0)messages.splice(at,1);}}},tabs:{query:async()=>[{id:101,windowId:1,url:"https://shop.example/confirmation",title:"Retail confirmation",active:true}],onUpdated:{addListener(){},removeListener(){}},onRemoved:{addListener(){},removeListener(){}}},permissions:{onRemoved:{addListener(){},removeListener(){}}},scripting:{executeScript:async(details)=>{if(details.world==="MAIN"&&details.args?.length===3){channels.set(101,details.args[1]);return[{result:{installed:true,rawValues:[]}}];}if(details.args?.length===1){const root={},parts=String(details.args[0]).split(".").filter(Boolean);let current=root;parts.forEach((part,index)=>{current[part]=index===parts.length-1?[]:{};current=current[part];});return[{result:root}];}return[{result:undefined}];}}};if(globalThis.chrome)Object.assign(globalThis.chrome,api);else Object.defineProperty(globalThis,"chrome",{configurable:true,value:api});})()`});
        await socket.call("Page.navigate",{url:`http://127.0.0.1:${assetPort}/side-panel.html`});
      ...
      specificationProjectObservation=await evaluate(socket,`(async()=>{
        ...
        return{created,search,bulk,afterUndo,preflight,release,stored:{name:stored.project.name,collections:Object.fromEntries(Object.entries(stored.project.collections).map(([kind,items])=>[kind,items.length])),releases:stored.project.releases.length,draft:Boolean(stored.draft)},layout};
	      ...
	      const scenarioId=process.env.SPECIFICATION_PROJECT_SCENARIO_ID??"";
