...
const {announceReorderCompletion,renderReorderControl,renderReorderableItemRow}=await import("../dist/reorderable-editor/control.js");
const order=[{id:"alpha",label:"Alpha"},{id:"bravo",label:"Bravo"},{id:"charlie",label:"Charlie"}];
const compositionDocument=new FakeDocument(),compositionControl=compositionDocument.createElement("span"),
  compositionPrimary=compositionDocument.createElement("label"),compositionSecondary=compositionDocument.createElement("p"),
  compositionAction=compositionDocument.createElement("button");
...
assert.equal(composition.style.maxWidth,"100%");
...
assert.equal(compositionAction.style.gridColumn,"3","an intrinsic trailing action remains in the primary row");
assert.deepEqual(composition.children,[compositionControl,compositionPrimary,compositionAction,compositionSecondaryBand]);
assert.deepEqual(compositionSecondaryBand.children,[compositionSecondary]);
...
assert.equal(trigger.style.background,"color-mix(in srgb, currentColor 14%, Canvas 86%)","the hover surface uses muted theme-derived contrast");
