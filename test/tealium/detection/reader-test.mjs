...
const identity=runtime('21','Local');identity.cfg={utid:'shop/main/202609100601',account:'wrong',profile:'wrong',env:'qa',v:'ut4.51.2026',path:'https://first.party/custom/'};
let identified=observe(identity).tags[0];
assert.equal(identified.account,'shop');assert.equal(identified.profileName,'main');assert.equal(identified.publishId,'202609100601');
assert.equal(identified.profile,'default');assert.equal(identified.environment,'qa');assert.equal(identified.libraryVersion,null);
identity.cfg={utid:'shop/checkout/202609100600',path:'https://tags.shop.example/utag/shop/checkout/dev/',template:'ut4.51'};
