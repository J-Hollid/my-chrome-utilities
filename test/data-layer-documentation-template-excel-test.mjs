...
import {documentationRasterDimensions,writeDocumentationTemplateStarter,writeProjectDocumentationWorkbookWithTemplates} from "../dist/documentation-templates/excel-renderer.js";

globalThis.ExcelJS=ExcelJS;

assert.deepEqual(parseExcelAreaProperties("image","fit: scale-down; position: center; padding: 8px"),{
  fit:"scale-down",position:{horizontal:"center",vertical:"center"},padding:{top:8,right:8,bottom:8,left:8},
},"one Contract 3 Properties cell normalizes combined image declarations");
assert.deepEqual(imageLayoutInArea({width:100,height:60},{width:40,height:20},parseExcelAreaProperties("image","fit: scale-down; position: center; padding: 8px")),{width:40,height:20,left:30,top:20});
assert.deepEqual(imageLayoutInArea({width:100,height:60},{width:20,height:20},parseExcelAreaProperties("image","fit: contain; position: right bottom; padding: 4px 8px 12px 16px")),{width:44,height:44,left:48,top:4});
assert.deepEqual(imageLayoutInArea({width:100,height:60},{width:40,height:20},parseExcelAreaProperties("image","position: 25% 75%")),{width:40,height:20,left:15,top:30});
for(const [properties,pattern] of [
  ["fit: cover",/unsupported image fit cover/u],
  ["padding: 8px; padding: 4px",/padding.*more than once/u],
  ["margin: 8px",/unsupported image property margin/u],
  ["separator-area: LogoGap",/separator-area cannot be used for an Image/u],
])assert.throws(()=>parseExcelAreaProperties("image",properties),pattern);
assert.throws(()=>parseExcelAreaProperties("repeat","position: center"),/position cannot be used for a Repeat/u);
...
const separated={kind:"flow",contractVersion:3,worksheetName:"Template",cells:[
  {address:"A1",value:"{{page.pageName}}"},{address:"B1",value:">>"},{address:"C1",value:"Later content"},
],areas:[{name:"PageStep",type:"repeat",source:"flow.pages",direction:"across",range:"A1:B1",properties:{separatorArea:{name:"PageSeparator",range:"B1:B1"}}}],merges:[]};
const separatedGrid=renderExcelTemplateGrid(separated,{section:{name:"Flow",kind:"flow"},flow:{pages:[{pageName:"Cart"},{pageName:"Shipping"},{pageName:"Payment"}]}});
assert.deepEqual(separatedGrid.cells.map(({address,value})=>[address,value]),[["A1","Cart"],["B1",">>"],["C1","Shipping"],["D1",">>"],["E1","Payment"],["F1","Later content"]],"separator sub-area renders only between repeat items and shifts later content exactly");
for(const count of [0,1]){const grid=renderExcelTemplateGrid(separated,{section:{name:"Flow",kind:"flow"},flow:{pages:Array.from({length:count},(_,index)=>({pageName:`Page ${index+1}`}))}});assert.equal(grid.cells.filter(({value})=>value===">>").length,0,`${count} repeat items emit no separator`);}
const separatedDown={...separated,cells:[{address:"A1",value:"{{page.pageName}}"},{address:"A2",value:"---"},{address:"A3",value:"Later content"}],areas:[{name:"PageStep",type:"repeat",source:"flow.pages",direction:"down",range:"A1:A2",properties:{separatorArea:{name:"PageSeparator",range:"A2:A2"}}}]},separatedDownGrid=renderExcelTemplateGrid(separatedDown,{section:{name:"Flow",kind:"flow"},flow:{pages:[{pageName:"Cart"},{pageName:"Shipping"},{pageName:"Payment"}]}});assert.deepEqual(separatedDownGrid.cells.map(({address,value})=>[address,value]),[["A1","Cart"],["A2","---"],["A3","Shipping"],["A4","---"],["A5","Payment"],["A6","Later content"]],"Down repeats use the complete bottom edge as an inter-item separator");

const contextualImagePrototype={...prototype,cells:[...prototype.cells,{address:"A3",value:"{{concept.name}}"}],areas:[{name:"PageCard",type:"repeat",source:"flow.pages",direction:"across",range:"A2:D8"},{name:"ConceptRows",type:"repeat",source:"page.concepts",direction:"down",range:"A3:B4"},{name:"PageVisual",type:"image",source:"page.visual.image",range:"C3:D7"}]};
assert.equal(validateExcelTemplatePrototype(contextualImagePrototype).valid,true,"Page concept groups and contextual visuals are valid only inside their owning Page repeat");
assert.match(validateExcelTemplatePrototype({...contextualImagePrototype,areas:[...contextualImagePrototype.areas.slice(0,-1),{name:"PageVisual",type:"image",source:"page.visual.image",range:"E3:F7"}]}).findings.map(({message})=>message).join("\n"),/PageVisual.*flow\.pages/u,"a contextual visual cannot escape its Page repeat");
const nestedVisualPrototype={...contextualImagePrototype,cells:[...prototype.cells,{address:"B3",value:"{{concept.name}}"}],areas:[{name:"PageCard",type:"repeat",source:"flow.pages",direction:"across",range:"A2:D8"},{name:"ConceptRows",type:"repeat",source:"page.concepts",direction:"down",range:"B3:D7"},{name:"PageVisual",type:"image",source:"page.visual.image",range:"C4:D6"}]};
assert.match(validateExcelTemplatePrototype(nestedVisualPrototype).findings.map(({message})=>message).join("\n"),/PageVisual.*nearest repeat.*flow\.pages/u,"a contextual visual cannot be copied once per nested concept and borrow another Page's image");
const enclosingVisualPrototype={...contextualImagePrototype,cells:[...prototype.cells,{address:"C4",value:"{{concept.name}}"}],areas:[{name:"PageCard",type:"repeat",source:"flow.pages",direction:"across",range:"A2:F8"},{name:"ConceptRows",type:"repeat",source:"page.concepts",direction:"down",range:"C4:D6"},{name:"PageVisual",type:"image",source:"page.visual.image",range:"B3:E7"}]};
assert.match(validateExcelTemplatePrototype(enclosingVisualPrototype).findings.map(({message})=>message).join("\n"),/PageVisual.*nested repeat/u,"a contextual visual cannot enclose a nested repeat within its Page owner");
assert.match(validateExcelTemplatePrototype({...contextualImagePrototype,cells:[...contextualImagePrototype.cells,{address:"C3",value:"{{page.visual.image}}"}]}).findings.map(({message})=>message).join("\n"),/cannot use page\.visual\.image/u,"image bytes cannot be emitted through an ordinary cell binding");
const flowGuide=excelTemplateGuideFor("flow");assert.equal(flowGuide.collections.find(({path})=>path==="flow.pages").nestedCollections.includes("page.concepts"),true);assert.equal(flowGuide.collections.some(({path})=>path==="event.concepts"),true);assert.equal(flowGuide.areaExamples.some(({type,source,parent})=>type==="Image"&&source==="page.visual.image"&&parent==="PageCard"),true,"the Flow guide advertises the scoped contextual image source");
...
const nestedContext={section:{name:"Coverage",kind:"matrix"},matrix:{rows:[
  {property:"products[x].product_name",cells:[{value:"Mandatory"},{value:"Optional"},{value:"Blocked"}]},
  {property:"groups[x].products[x].product_id",cells:[{value:"Optional"},{value:"Mandatory"},{value:"Not defined"}]},
]}};
...
assert.deepEqual(nestedRendered.cells.filter(({value})=>String(value).includes("[x]")).map(({address,value})=>[address,value]),[["A2","products[x].product_name"],["A4","groups[x].products[x].product_id"]]);
...
assert.deepEqual(["products[x].product_name","groups[x].products[x].product_id"].map(value=>generatedAddresses.get(value)),["A3","A5"]);
...
const alternateVisual="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=",contextualFlowTable={...flowTable,templateData:{...flowTable.templateData,pages:[{stepLabel:"1",pageName:"Cart",sourcePageName:"Cart",eventName:"pageview",heading:"Cart",rows:[],concepts:[{name:"Commerce",rows:[{}]},{name:"Funnel",rows:[{}]}],events:[{eventName:"view_cart"},{eventName:"add_shipping"}],visual:{image:logo,description:"Cart state",caption:"Review",sourceReference:"https://example.test/inert"}},{stepLabel:"2",pageName:"Confirmation",sourcePageName:"Cart",eventName:"pageview",heading:"Confirmation",rows:[],concepts:[{name:"Commerce",rows:[{}]},{name:"Funnel",rows:[{}]}],events:[{eventName:"purchase"},{eventName:"receipt"}],visual:{image:alternateVisual,description:"Confirmation state"}},{stepLabel:"3",pageName:"Payment",sourcePageName:"Payment",eventName:"pageview",heading:"Payment",rows:[],concepts:[{name:"Funnel",rows:[{}]}],events:[{eventName:"payment"}]}]}},contextualOutput=await writeProjectDocumentationWorkbookWithTemplates({...flowSnapshot,theme:{...flowSnapshot.theme,logo:""},tables:[contextualFlowTable]},{scope:"current",currentSectionId:"flow"},async()=>flowBlob),contextualBook=new ExcelJS.Workbook();await contextualBook.xlsx.load(contextualOutput);const contextualImages=contextualBook.worksheets[0].getImages();assert.equal(contextualImages.length,2,"multiple nested items do not duplicate Page visuals and a Page without a visual leaves its area empty");const contextualZip=await JSZip.loadAsync(contextualOutput),mediaBodies=await Promise.all(Object.values(contextualZip.files).filter(({name})=>name.startsWith("xl/media/")&&!name.endsWith("/")).map(file=>file.async("base64")));assert.deepEqual(new Set(mediaBodies),new Set([logo.slice(logo.indexOf(",")+1),alternateVisual.slice(alternateVisual.indexOf(",")+1)]),"each image target owns the exact visual from its Page repeat");const relationshipBodies=await Promise.all(Object.values(contextualZip.files).filter(({name})=>name.endsWith(".rels")).map(file=>file.async("string")));assert.equal(relationshipBodies.some(value=>/TargetMode=["']External["']/u.test(value)),false,"contextual visuals create no external relationship");
const webpBytes=new Uint8Array(30);webpBytes.set(new TextEncoder().encode("RIFF"),0);webpBytes.set(new TextEncoder().encode("WEBPVP8X"),8);new DataView(webpBytes.buffer).setUint32(16,10,true);const webpWidth=320,webpHeight=200;webpBytes.set([webpWidth-1&255,(webpWidth-1)>>8&255,(webpWidth-1)>>16&255,webpHeight-1&255,(webpHeight-1)>>8&255,(webpHeight-1)>>16&255],24);const webpVisual=`data:image/webp;base64,${Buffer.from(webpBytes).toString("base64")}`;assert.deepEqual(documentationRasterDimensions(webpVisual),{width:webpWidth,height:webpHeight},"WebP intrinsic dimensions are read without changing the saved body");const webpPages=contextualFlowTable.templateData.pages.map((page,index)=>index<2?{...page,visual:{image:webpVisual,description:`WebP ${index+1}`}}:{...page}),webpTable={...contextualFlowTable,templateData:{...contextualFlowTable.templateData,pages:webpPages}};let conversions=0;const webpOutput=await writeProjectDocumentationWorkbookWithTemplates({...flowSnapshot,theme:{...flowSnapshot.theme,logo:""},tables:[webpTable]},{scope:"current",currentSectionId:"flow"},async()=>flowBlob,{convertWebpToPng:async source=>{conversions+=1;assert.equal(source,webpVisual);return logo;}}),webpBook=new ExcelJS.Workbook();await webpBook.xlsx.load(webpOutput);assert.equal(conversions,1,"one bounded renderer-side conversion is shared for repeated identical WebP bodies");assert.equal(webpBook.worksheets[0].getImages().length,2,"each Page retains its own image anchor after WebP conversion and an absent Page remains empty");const webpZip=await JSZip.loadAsync(webpOutput),webpMedia=Object.values(webpZip.files).filter(({name})=>name.startsWith("xl/media/")&&!name.endsWith("/"));assert.equal(webpMedia.every(({name})=>name.endsWith(".png")),true,"Excel receives compatible PNG parts while the template snapshot retains WebP");assert.equal(webpPages.every((page,index)=>index===2||page.visual.image===webpVisual),true,"renderer conversion does not mutate stored or snapshot WebP bodies");

const positionedSource=new ExcelJS.Workbook();await positionedSource.xlsx.load(flowStarter);const positionedTemplate=positionedSource.getWorksheet("Template"),positionedGuide=positionedSource.getWorksheet("Template Guide");positionedTemplate.getColumn(3).width=50/7;positionedTemplate.getColumn(4).width=50/7;positionedTemplate.getRow(1).height=22.5;positionedTemplate.getRow(2).height=22.5;positionedGuide.getCell("E14").value="fit: contain; position: right bottom; padding: 4px 8px 12px 16px";const positionedBytes=await positionedSource.xlsx.writeBuffer(),positionedBlob=new Blob([positionedBytes]),positionedValidation=await validateExcelTemplateWorkbook(positionedBlob,"flow");assert.equal(positionedValidation.valid,true,positionedValidation.findings.map(({location,message})=>`${location}: ${message}`).join("\n"));assert.match(positionedValidation.inspection.areas.find(({name})=>name==="ThemeLogo").properties,/fit: contain.*position: right bottom.*padding: 4px 8px 12px 16px/u);const positionedRecord={...flowTemplate,contractVersion:3,digest:"sha256:positioned",body:{assetId:"asset:positioned",digest:"sha256:positioned",byteLength:positionedBlob.size},validation:positionedValidation},positionedOutput=await writeProjectDocumentationWorkbookWithTemplates({...flowSnapshot,set:{...flowSnapshot.set,templateAssignments:{"excel:flow":positionedRecord.id}},templateDigests:{"excel:flow":positionedRecord.digest},templates:[positionedRecord]},{scope:"current",currentSectionId:"flow"},async()=>positionedBlob),positionedBook=new ExcelJS.Workbook();await positionedBook.xlsx.load(positionedOutput);const positionedAnchor=positionedBook.worksheets[0].getImages()[0].range;assert.ok(Math.abs(positionedAnchor.ext.width-44)<0.01&&Math.abs(positionedAnchor.ext.height-44)<0.01,"Contract 3 contain geometry enlarges the image inside the padded rectangle");assert.ok(Math.abs(positionedAnchor.tl.nativeColOff/9525-48)<0.01&&Math.abs(positionedAnchor.tl.nativeRowOff/9525-4)<0.01,"Contract 3 right-bottom placement is stored in the generated OOXML anchor");

const invalidPropertiesBook=new ExcelJS.Workbook();await invalidPropertiesBook.xlsx.load(positionedBytes);invalidPropertiesBook.getWorksheet("Template Guide").getCell("E14").value="fit: cover";const invalidProperties=await validateExcelTemplateWorkbook(new Blob([await invalidPropertiesBook.xlsx.writeBuffer()]),"flow");assert.equal(invalidProperties.valid,false);assert.match(invalidProperties.findings.map(({location,message})=>`${location}: ${message}`).join("\n"),/TemplateAreas ThemeLogo Properties: ThemeLogo unsupported image fit cover/u);
const consumedPaddingBook=new ExcelJS.Workbook();await consumedPaddingBook.xlsx.load(positionedBytes);consumedPaddingBook.getWorksheet("Template Guide").getCell("E14").value="padding: 51px";const consumedPadding=await validateExcelTemplateWorkbook(new Blob([await consumedPaddingBook.xlsx.writeBuffer()]),"flow");assert.equal(consumedPadding.valid,false);assert.match(consumedPadding.findings.map(({location,message})=>`${location}: ${message}`).join("\n"),/ThemeLogo padding leaves no room for its image/u);

const separatorBook=new ExcelJS.Workbook(),separatorTemplate=separatorBook.addWorksheet("Template"),separatorGuide=separatorBook.addWorksheet("Template Guide");separatorTemplate.getCell("A1").value="{{page.pageName}}";separatorTemplate.getCell("B1").value=">>";separatorTemplate.getCell("C1").value="Later content";separatorBook.definedNames.add("'Template'!$A$1:$B$1","PageStep");separatorBook.definedNames.add("'Template'!$B$1:$B$1","PageSeparator");separatorGuide.addTable({name:"TemplateSettings",ref:"A1",headerRow:true,columns:[{name:"Setting"},{name:"Value"}],rows:[["Contract",3],["Kind","flow"]]});separatorGuide.addTable({name:"TemplateAreas",ref:"A6",headerRow:true,columns:[{name:"Area"},{name:"Type"},{name:"Source"},{name:"Direction"},{name:"Properties"}],rows:[["PageStep","Repeat","flow.pages","Across","separator-area: PageSeparator"]]});const separatorBytes=await separatorBook.xlsx.writeBuffer(),separatorBlob=new Blob([separatorBytes]),separatorValidation=await validateExcelTemplateWorkbook(separatorBlob,"flow");assert.equal(separatorValidation.valid,true,separatorValidation.findings.map(({location,message})=>`${location}: ${message}`).join("\n"));const separatorRecord={...flowTemplate,contractVersion:3,digest:"sha256:separator",body:{assetId:"asset:separator",digest:"sha256:separator",byteLength:separatorBlob.size},validation:separatorValidation},separatorOutput=await writeProjectDocumentationWorkbookWithTemplates({...flowSnapshot,set:{...flowSnapshot.set,templateAssignments:{"excel:flow":separatorRecord.id}},templateDigests:{"excel:flow":separatorRecord.digest},templates:[separatorRecord],tables:[contextualFlowTable]},{scope:"current",currentSectionId:"flow"},async()=>separatorBlob),separatorOutputBook=new ExcelJS.Workbook();await separatorOutputBook.xlsx.load(separatorOutput);assert.deepEqual(["A1","B1","C1","D1","E1","F1"].map(address=>separatorOutputBook.worksheets[0].getCell(address).value),["Cart",">>","Confirmation",">>","Payment","Later content"],"actual Contract 3 workbook emits named separators only between ordered Page items");
