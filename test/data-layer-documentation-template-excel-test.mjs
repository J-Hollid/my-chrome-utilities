...
const contextualFlowTable={...flowTable,templateData:{...flowTable.templateData,pages:[{stepLabel:"1",pageName:"Cart",sourcePageName:"Cart",eventName:"pageview",heading:"Cart",rows:[],concepts:[],events:[],visual:{image:logo,description:"Cart state",caption:"Review",sourceReference:"https://example.test/inert"}},{stepLabel:"2",pageName:"Payment",sourcePageName:"Payment",eventName:"pageview",heading:"Payment",rows:[],concepts:[],events:[]}]}},contextualOutput=await writeProjectDocumentationWorkbookWithTemplates({...flowSnapshot,theme:{...flowSnapshot.theme,logo:""},tables:[contextualFlowTable]},{scope:"current",currentSectionId:"flow"},async()=>flowBlob),contextualBook=new ExcelJS.Workbook();await contextualBook.xlsx.load(contextualOutput);assert.equal(contextualBook.worksheets[0].getImages().length,1,"each Page repeat embeds only its own available contextual visual and an absent visual leaves its area empty");const contextualZip=await JSZip.loadAsync(contextualOutput),relationshipBodies=await Promise.all(Object.values(contextualZip.files).filter(({name})=>name.endsWith(".rels")).map(file=>file.async("string")));assert.equal(relationshipBodies.some(value=>/TargetMode=["']External["']/u.test(value)),false,"contextual visuals create no external relationship");
