...
function emitStartupLoadBudgetRepair() {
  ...
  if(preRepair.ready||!repaired.ready||!installedObservationSource().includes('+20000')) {
...
const installedObservationSource=()=>observeTwoInstalledSources.toString()
  .replace('performance.now()+10000','performance.now()+20000');
...
try {
  const retained=(width)=>({pagePath:'side-panel.html',
    ...
    run:async({socket,evaluate})=>{await socket().call('Emulation.setDeviceMetricsOverride',{width,height:900,deviceScaleFactor:1,mobile:false});return evaluate(null,`await (${installedObservationSource()})(); return await (${observeRetainedUtility.toString()})(${observeProbeUtility.toString()})`);}});
  const definitions={UTILITY_RETAINED_PAGE_360:retained(360),UTILITY_RETAINED_PAGE_800:retained(800),
    ...
    UTILITY_STARTUP_EXCEPTION:{pagePath:'side-panel.html',beforeExpression:()=>`localStorage.clear();localStorage.setItem('probe.crash','true');await (${seedObservationProject.toString()})();return true;`,
      expression:()=>`await (${installedObservationSource()})();return await (${observeUtilityStartupException.toString()})();`},
