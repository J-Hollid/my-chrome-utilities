...
const anotherLaterProduct=node("property:product-stock","in_stock","boolean",3,{parentId:product.id,concept:"ecommerce"}),twiceGrownDocument={...grownTreeDocument,nodes:{...grownTreeDocument.nodes,[anotherLaterProduct.id]:anotherLaterProduct}},twoParentAdditions=profileInheritanceParentAdditions(twiceGrownDocument,appliedPartialEcommerce),includedOneOfTwo=includeProfileInheritanceParentAdditions(twiceGrownDocument,appliedPartialEcommerce,[laterProduct.id]);
assert.deepEqual(twoParentAdditions.map(({propertyId})=>propertyId),[laterProduct.id,anotherLaterProduct.id],"both post-application identities remain independently reviewable");
assert.deepEqual(profileInheritanceParentAdditions(twiceGrownDocument,includedOneOfTwo).map(({propertyId})=>propertyId),[anotherLaterProduct.id],"including one addition preserves every unchecked addition for a later review");
