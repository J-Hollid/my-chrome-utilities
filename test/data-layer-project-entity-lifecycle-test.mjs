...
const examples={profiles:"Sitewide",pageGroups:"Checkout",pages:"Cart",events:"Purchase",applicabilitySets:"Retail checkout",flows:"Checkout journey",assignments:"Retail Purchase",fixtures:"Valid purchase"};
for(const [kind,name] of Object.entries(examples))state=createProjectCollectionEntity(state,kind,name,id,kind==="assignments"?{targetKind:"Shared Profile",targetId:state.project.collections.profiles[0].id}:kind==="pages"?{eventName:"pageview"}:{});
...
for(const kind of["profiles","pageGroups","pages"]){const entity=state.project.collections[kind][0];assert.equal(entity.canonicalSchema.contributorId,entity.id,`${kind} creation initializes its directly editable canonical contribution`);assert.equal(entity.canonicalSchema.contributorName,entity.name);}
...
const expectedCreationFields={profiles:["Profile purpose"],pageGroups:["Description"],pages:["Description","Page-view event name","Exact URL path"],events:["Canonical event name"],applicabilitySets:["Priority","Fallback"],flows:["Correlation field"],fixtures:["Test case type","Event","Page"],assignments:["Contributor kind","Contributor target","Event","Applicability Set","Priority"]};
...
assert.deepEqual(projectCollectionCreationFields.pageGroups,[{key:"description",label:"Description",control:"textarea",guidance:"Explain the shared Page context and intended use of this Page Group."}],"Page Group creation exposes one guided optional multiline description");
assert.equal(projectCollectionDefinitions.fixtures.overview,"Test cases");
assert.equal(projectCollectionDefinitions.fixtures.singular,"Test case");
assert.equal(state.project.collections.fixtures[0].testType,"event-validation");
assert.equal("releasePolicy" in state.project.collections.fixtures[0],false);
assert.throws(()=>createProjectCollectionEntity(state,"pages","Missing event identity",id),/Page-view event name is required/);
const configuredPageState=createProjectCollectionEntity(state,"pages","Product detail",id,{description:" Product detail context ",eventName:"pageview",pathname:"/products/:product_id",pageGroupIds:[state.project.collections.pageGroups[0].id]}),configuredPage=configuredPageState.project.collections.pages.find(({name})=>name==="Product detail");
assert.equal(configuredPage.description,"Product detail context");assert.equal(configuredPage.eventName,"pageview");assert.equal(configuredPage.pathname,"/products/:product_id");assert.deepEqual(configuredPage.pageGroupIds,[],"Page Group membership is added from the searchable Page workspace picker");
const configuredGroupState=createProjectCollectionEntity(state,"pageGroups","Regional checkout",id,{description:"  Checkout Pages used by the regional storefronts  ",environment:"Production",matcher:"checkout"}),configuredGroup=configuredGroupState.project.collections.pageGroups.find(({name})=>name==="Regional checkout");
assert.equal(configuredGroup.description,"Checkout Pages used by the regional storefronts");assert.equal("environment" in configuredGroup,false);assert.equal("matcher" in configuredGroup,false);
...
assert.equal(configuredEvent.eventName,"route_view");assert.equal(configuredEvent.role,undefined);
const configuredAssignmentState=createProjectCollectionEntity(configuredEventState,"assignments","Route assignment",id,{targetKind:"Event",targetId:configuredEvent.id,eventId:configuredEvent.id,applicabilitySetId:state.project.collections.applicabilitySets[0].id,priority:25}),configuredAssignment=configuredAssignmentState.project.collections.assignments.find(({name})=>name==="Route assignment");
assert.equal(configuredAssignment.eventName,"route_view");assert.equal(configuredAssignment.targetId,configuredEvent.id);assert.equal(configuredAssignment.targetKind,"Event");assert.equal(configuredAssignment.priority,25);assert.equal("schemaDraftId" in configuredAssignment,false);assert.equal("schemaId" in configuredAssignment,false);assert.equal("schemaDrafts" in configuredAssignmentState.project.collections,false);
...
const landingState=createProjectCollectionEntity(state,"pages","Landing",id,{eventName:"pageview"}),landing=landingState.project.collections.pages.find(({name})=>name==="Landing"),review=inspectProjectEntityRemoval(landingState,"pages",landing.id);
