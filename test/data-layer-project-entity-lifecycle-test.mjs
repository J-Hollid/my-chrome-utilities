...
const examples={profiles:"Sitewide",propertySets:"Checkout",pages:"Cart",events:"Purchase",applicabilitySets:"Retail checkout",flows:"Checkout journey",assignments:"Retail Purchase",fixtures:"Valid purchase"};
for(const [kind,name] of Object.entries(examples))state=createProjectCollectionEntity(state,kind,name,id,kind==="assignments"?{targetKind:"Shared Profile",targetId:state.project.collections.profiles[0].id}:{});
...
for(const kind of["profiles","propertySets","pages"]){const entity=state.project.collections[kind][0];assert.equal(entity.canonicalSchema.contributorId,entity.id,`${kind} creation initializes its directly editable canonical contribution`);assert.equal(entity.canonicalSchema.contributorName,entity.name);}
...
const expectedCreationFields={profiles:["Profile purpose"],propertySets:["Description"],pages:["Description"],events:["Canonical event name"],applicabilitySets:["Priority","Fallback"],flows:["Correlation field"],fixtures:["Test case type","Event","Page"],assignments:["Contributor kind","Contributor target","Event","Applicability Set","Priority"]};
...
assert.deepEqual(projectCollectionCreationFields.propertySets,[{key:"description",label:"Description",control:"textarea",guidance:"Explain the reusable schema component and intended use of this Property Set."}],"Property Set creation exposes one guided optional multiline description");
...
const configuredPageState=createProjectCollectionEntity(state,"pages","Product detail",id,{description:" Product detail context ",eventName:"pageview",pathname:"/products/:product_id",propertySetIds:[state.project.collections.propertySets[0].id]}),configuredPage=configuredPageState.project.collections.pages.find(({name})=>name==="Product detail");
assert.equal(configuredPage.description,"Product detail context");assert.equal("eventName" in configuredPage,false);assert.equal("pathname" in configuredPage,false);assert.deepEqual(configuredPage.propertySetApplications,[],"Property Set applications are added from the searchable Page workspace picker");
const configuredGroupState=createProjectCollectionEntity(state,"propertySets","Regional checkout",id,{description:"  Checkout Pages used by the regional storefronts  ",environment:"Production",matcher:"checkout"}),configuredGroup=configuredGroupState.project.collections.propertySets.find(({name})=>name==="Regional checkout");
