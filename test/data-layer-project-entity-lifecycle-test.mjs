...
const expectedCreationFields={profiles:["Profile purpose"],pageGroups:["Description"],pages:["Description"],events:["Canonical event name"],applicabilitySets:["Priority","Fallback"],flows:["Correlation field"],fixtures:["Test case type","Event","Page"],assignments:["Contributor kind","Contributor target","Event","Applicability Set","Priority"]};
...
assert.deepEqual(projectCollectionCreationFields.pageGroups,[{key:"description",label:"Description",control:"textarea",guidance:"Explain the reusable schema component and intended use of this Property Set."}],"Property Set creation exposes one guided optional multiline description");
...
assert.equal(configuredPage.description,"Product detail context");assert.equal("eventName" in configuredPage,false);assert.equal("pathname" in configuredPage,false);assert.deepEqual(configuredPage.pageGroupIds,[],"Property Set applications are added from the searchable Page workspace picker");
