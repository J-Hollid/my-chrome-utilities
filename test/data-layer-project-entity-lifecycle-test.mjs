...
const expectedCreationFields={profiles:["Profile purpose"],pageGroups:["Description"],pages:["Observed event name","Path matcher","Page Groups"],events:["Canonical event name"],applicabilitySets:["Priority","Fallback"],flows:["Correlation field"],fixtures:["Test case type","Event","Page"],assignments:["Contributor kind","Contributor target","Event","Applicability Set","Priority"]};
...
assert.throws(()=>createProjectCollectionEntity(state,"pages","Missing event identity",id),/Observed event name is required/);
const configuredPageState=createProjectCollectionEntity(state,"pages","Product detail",id,{eventName:"pageview",pathname:"/products/:product_id",pageGroupIds:[state.project.collections.pageGroups[0].id]}),configuredPage=configuredPageState.project.collections.pages.find(({name})=>name==="Product detail");
assert.equal(configuredPage.eventName,"pageview");assert.equal(configuredPage.pathname,"/products/:product_id");assert.deepEqual(configuredPage.pageGroupIds,[state.project.collections.pageGroups[0].id]);
