...
const customer=add("pages",{name:"Customer details"}),payment=add("pages",{name:"Payment"}),route=add("events",{name:"route_view",eventName:"route_view",role:"context-setting"}),verify=add("events",{name:"verify_identity",eventName:"verify_identity",role:"interaction"}),group=add("pageGroups",{name:"Checkout"}),flow=add("flows",{name:"Checkout journey",purpose:"Document checkout",steps:[]});
...
state=saveGraphRelationship(state,flow.id,customerFrame.id,{toStepId:verification.id,sourcePort:"top",targetPort:"bottom",group:"identity",documentationCondition:"identity required",expectation:"verification follows details"},id);
...
assert.equal(flowRelationshipDeletionAccessibleName(undefined,"Customer details","ID verification"),"Delete relationship Customer details to ID verification","an unlabelled relationship is named by both endpoints");
