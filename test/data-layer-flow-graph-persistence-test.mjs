...
import {addGraphOccurrence,addUngroupedPageFrame,documentaryFlowGraph,projectFlowGraph,saveGraphRelationship,setFlowPageGroupLanes,updateGraphOccurrence} from "../dist/data-layer-flow-graph.js";
...
let laneSequence=0,laneState=createSpecificationProject({name:"Lane persistence",site:"shop.example",id:(kind)=>`${kind}:lane-persist-${++laneSequence}`});const laneId=(kind)=>`${kind}:lane-persist-${++laneSequence}`,laneAdd=(kind,entity)=>{laneState=addProjectEntity(laneState,kind,entity,laneId);return laneState.project.collections[kind].at(-1);},lanePage=laneAdd("pages",{name:"Checkout"}),laneEvent=laneAdd("events",{name:"Purchase",schemaId:"schema:purchase"}),laneGroup=laneAdd("pageGroups",{name:"Checkout",pageIds:[lanePage.id]}),laneFlow=laneAdd("flows",{name:"Checkout lanes",steps:[]});laneState=setFlowPageGroupLanes(laneState,laneFlow.id,[laneGroup.id]);laneState=addGraphOccurrence(laneState,laneFlow.id,{name:"Purchase",pageGroupId:laneGroup.id,pageId:lanePage.id,eventId:laneEvent.id,obligation:"Required",minimum:1,maximum:1,y:170},laneId);const freePage=laneAdd("pages",{name:"Landing",contextEventBindings:[{id:"binding:landing",name:"page_view",eventId:laneEvent.id}]});laneState=addUngroupedPageFrame(laneState,laneFlow.id,{name:"Landing entry",pageId:freePage.id,contextBindingId:"binding:landing",y:70},laneId);const laneValues=new Map(),laneStorage={getItem:(key)=>laneValues.get(key)??null,setItem:(key,value)=>laneValues.set(key,value)};assert.equal(commitCanonicalProjectState(laneStorage,laneState).status,"committed");const laneReload=restoreCanonicalProjectState(laneValues.values().next().value),laneDocumentary=documentaryFlowGraph(laneReload.project,laneFlow.id),laneStored=laneDocumentary.occurrences[0],freeStored=laneDocumentary.occurrences[1],laneProjected=projectFlowGraph(laneReload.project,laneFlow.id);assert.deepEqual(laneDocumentary.pageGroupIds,[laneGroup.id]);assert.equal("pageGroupIds" in laneReload.project.collections.flows[0],false);assert.deepEqual({pageGroupId:laneStored.pageGroupId,position:laneStored.position},{pageGroupId:laneGroup.id,position:{y:170}});assert.deepEqual({pageId:freeStored.pageId,contextBindingId:freeStored.contextBindingId,freePageFrame:freeStored.freePageFrame},{pageId:freePage.id,contextBindingId:"binding:landing",freePageFrame:true});assert.deepEqual(laneProjected.lanes.map(({id,name})=>[id,name]),[[laneGroup.id,"Checkout"]]);assert.equal(laneProjected.graph.nodes[0].layout.x,30);assert.equal(laneProjected.graph.nodes[1].layout.lane,"Ungrouped entry pages");
