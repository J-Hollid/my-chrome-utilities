...
const upgradeProjection=workflow.pendingUpgradeState();
assert.deepEqual(upgradeProjection,{id:rule.id,schemaIds:["schema:one"]});
upgradeProjection.schemaIds=[];
assert.deepEqual(workflow.pendingUpgradeState(),{id:rule.id,schemaIds:["schema:one"]},
  "callers cannot mutate the attachment upgrade through its projection");
assert.equal(workflow.pendingUpgrade,undefined,"the attachment upgrade transaction is private");workflow.confirmUpgrade();

assert.equal(schemas[0].attachedRules[0].version,2);

assert.equal(workflow.pendingUpgradeState(),undefined);
...
assert.equal(workflow.pendingSyncState(),undefined);
assert.equal(workflow.pendingSync,undefined,"the attachment sync transaction is private");
