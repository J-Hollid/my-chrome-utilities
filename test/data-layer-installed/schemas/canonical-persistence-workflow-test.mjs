...
workflow.renderContext();workflow.render();workflow.queueLibraryPersistence("schema:one");assert.deepEqual(calls.slice(0,5),["context","render",["queue","schema:one",1],"persist"]);
assert.equal(workflow.beginSettlement("schema:one"),7);await workflow.settle({type:"saved",schemaId:"schema:one"});assert.equal(calls.some((entry)=>Array.isArray(entry)&&entry[0]==="clear"),true);
