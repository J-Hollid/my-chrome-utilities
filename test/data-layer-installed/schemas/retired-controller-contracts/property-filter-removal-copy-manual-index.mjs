  ...
  "checks": [
    {
      ...
      "method": "equal",
      "owner": "test/data-layer-installed/schemas/retired-property-controller-contract-test.mjs",
    ...
    },
    {
      ...
      "expected": "\"\"",
      "binding": "assert.equal(elements.get(\"#schema-property-filter\").value, \"\");"
    },
    {
      ...
      "expected": "0",
      "binding": "assert.equal(propertyToggle.listenerCount(), 0, \"property selection rerender disposes the replaced row listeners\");"
    },
    {
      ...
      "expected": "true",
      "binding": "assert.equal(schemaMasterPanel.hidden, true);"
    },
    {
      ...
      "expected": "false",
      "binding": "assert.equal(schemaRulesPanel.hidden, false);"
    },
    {
      ...
      "expected": "true",
      "binding": "assert.equal(elements.get(\"#schema-property-removal-dialog\").open, true);"
    },
    {
      ...
      "binding": "assert.match(elements.get(\"#schema-property-removal-summary\").textContent, /Documentation entries: \\/title/);"
    },
    {
      ...
      "expected": "undefined",
      "binding": "assert.equal(uiController.schemas().find(({ id }) => id === uiController.state().activeSchemaId).workingDraft.document.properties.title, undefined);"
    },
    {
      ...
      "expected": "\"string\"",
      "binding": "assert.equal(uiController.schemas().find(({ id }) => id === uiController.state().activeSchemaId).workingDraft.document.properties.title.type, \"string\", \"Undo restores the exact property definition\");"
    },
    {
      ...
      "expected": "\"string\"",
      "binding": "assert.equal(documentationDraft.document.properties.title.type, \"string\");"
    },
    {
      ...
      "expected": "undefined",
      "binding": "assert.equal(documentationDraft.documentation.properties, undefined, \"documentation-only removal leaves the schema property intact\");"
    },
    {
      ...
      "expected": "true",
      "binding": "assert.equal(elements.get(\"#schema-property-copy-dialog\").open, true);"
    },
    {
      ...
      "expected": "\"boolean\"",
      "binding": "assert.equal(uiController.schemas().find(({ id }) => id === destinationId).workingDraft.document.properties.checkout.type, \"boolean\");"
    },
    {
      ...
      "expected": "undefined",
      "binding": "assert.equal(uiController.schemas().find(({ id }) => id === destinationId).workingDraft, undefined, \"property-copy undo restores the complete destination schema state\");"
    },
    {
      ...
      "expected": "false",
      "binding": "assert.equal(elements.get(\"#confirm-schema-specific-index\").disabled, false);"
    },
    {
      ...
      "expected": "false",
      "binding": "assert.equal(elements.get(\"#schema-specific-index-dialog\").open, false);"
    },
    {
      ...
      "expected": "true",
      "binding": "assert.equal(elements.get(\"#schema-property-rule-picker\").open, true, \"an accepted specific index transitions directly into the controller-owned rule picker\");"
    },
    {
      ...
      "expected": "\"items.2\"",
      "binding": "assert.equal(uiController.rulePickerState().path, \"items.2\");"
    },
    {
      ...
      "expected": "/items\\/2/",
      "binding": "assert.match(elements.get(\"#schema-property-rule-picker\").dataset.conditionPreview, /items\\/2/, \"the dotted controller path retains the accepted canonical array index\");"
    },
    {
      ...
      "observable": "uiController.rulePickerState().path",
      "expected": "undefined",
      "binding": "assert.equal(uiController.rulePickerState().path, undefined, \"closing the owned rule picker clears its transition state without an external callback\");"
    },
    {
      ...
      "binding": "assert.match(elements.get(\"#schema-manual-property-preview\").textContent, /checkout.total is number/);"
    },
    {
      ...
      "expected": "\"number\"",
      "binding": "assert.equal(uiController.schemas().find(({ id }) => id === uiController.state().activeSchemaId) .workingDraft.document.properties.checkout.properties.total.type, \"number\");"
    }
  ]
};
