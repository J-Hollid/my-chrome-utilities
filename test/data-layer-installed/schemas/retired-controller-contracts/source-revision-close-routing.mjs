...
export const checks = [
  {
    ...
    "expected": "/Parent v2/",
    "binding": "assert.match(elements.get(\"#schema-inheritance-provenance\").textContent, /Parent v2/);"
  },
  {
  ...
  },
  {
    ...
    "binding": "assert.match(elements.get(\"#schema-inherited-rule-groups\").children[0].children[0].textContent, /Active inherited \\(1\\)/);"
  },
  {
    ...
    "expected": "false",
    "binding": "assert.equal(uiController.schemas().find(({ id }) => id === \"schema:page\").workingDraft.document.additionalProperties, false);"
  },
  {
    ...
    "expected": "true",
    "binding": "assert.equal(elements.get(\"#schema-revision-review\").open, true, \"saving opens the controller-owned revision review\");"
  },
  {
    ...
    "expected": "2",
    "binding": "assert.equal(uiController.schemas()[0].version, 2);"
  },
  {
    ...
    "binding": "assert.equal(uiController.schemas()[0].name, \"Page checkout\");"
  },
  {
    ...
    "binding": "assert.equal(uiController.schemas()[0].documentation.description, \"Checkout payload\");"
  },
  {
  ...
  },
  {
    ...
    "expected": "true",
    "binding": "assert.equal(elements.get(\"#schema-editor\").hidden, true, \"stored-schema publication clears editor state and returns to the list\");"
  },
  {
    ...
    "binding": "assert.deepEqual(elements.get(\"#schema-revision-selector\").children.map(({ value, textContent }) => ({ value, textContent })), [{ value:\"1\", textContent:\"Revision 1\" }], \"opening a published schema renders its historical revision choices\");"
  },
  {
  ...
  },
  {
    ...
    "expected": "undefined",
    "binding": "assert.equal(uiController.schemas().find(({ id }) => id === \"schema:page\").workingDraft, undefined, \"requesting historical restoration does not mutate the current schema before confirmation\");"
  },
  {
    ...
    "observable": "uiController.schemas().find(({ id }) => id === \"schema:page\").workingDraft",
    "expected": "undefined",
    "binding": "assert.equal(uiController.schemas().find(({ id }) => id === \"schema:page\").workingDraft, undefined, \"cancelling historical restoration leaves the current schema unchanged\");"
  },
  {
    ...
    "expected": "1",
    "binding": "assert.equal(uiController.schemas().find(({ id }) => id === \"schema:page\").workingDraft.sourceVersion, 1, \"confirming historical restoration creates a working draft from the selected revision\");"
  },
  {
    ...
    "binding": "assert.equal(relationshipActions.at(-1), \"build:schema:page:historical:1\");"
  },
  {
    ...
    "expected": "3",
    "binding": "assert.equal(uiController.schemas().length, 3, \"revision duplication remains schema-controller behavior\");"
  },
  {
  ...
  },
  {
    ...
    "binding": "assert.equal(fixture.schemaStorageWrites, storedDraftWrites);"
  },
  {
    ...
    "binding": "assert.equal(uiValues.get(\"my-chrome-utilities.schema-library.v1\"), storedDraftStorage);"
  },
  {
    ...
    "expected": "truthy",
    "binding": "assert.ok(uiController.schemas().find(({ id }) => id === lifecycleSchemaId).workingDraft, \"close retains the stored working draft without another persistence write\");"
  },
  {
  ...
  },
  {
    ...
    "binding": "assert.equal(fixture.schemaStorageWrites, abandonWrites);"
  },
  {
    ...
    "observable": "uiController.schemas().find(({ id }) => id === lifecycleSchemaId).workingDraft",
    "expected": "truthy",
    "binding": "assert.ok(uiController.schemas().find(({ id }) => id === lifecycleSchemaId).workingDraft, \"discard-schema-draft abandons editor state without discarding the stored working draft\");"
  },
  {
    ...
    "binding": "assert.equal(fixture.schemaStorageWrites, discardStoredWrites + 1);"
  },
  {
    ...
    "observable": "uiController.schemas().find(({ id }) => id === lifecycleSchemaId).workingDraft",
    "expected": "undefined",
    "binding": "assert.equal(uiController.schemas().find(({ id }) => id === lifecycleSchemaId).workingDraft, undefined, \"discard-working-schema-draft is the distinct operation that mutates the stored library\");"
  }
];
