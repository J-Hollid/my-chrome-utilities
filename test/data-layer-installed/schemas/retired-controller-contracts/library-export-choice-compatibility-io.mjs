  ...
  "checks": [
    {
      ...
      "method": "equal",
      "owner": "test/data-layer-installed/schemas/retired-library-export-controller-contract-test.mjs",
      "contract": "uiController.openExportChoices() => true",
      "observable": "uiController.openExportChoices()",
      "expected": "true",
      "binding": "assert.equal(uiController.openExportChoices(), true);"
    },
    {
      ...
      "expected": "true",
      "binding": "assert.equal(elements.get(\"#schema-export-choices\").open, true);"
    },
    {
      ...
      "binding": "assert.equal(elements.get(\"#schema-export-choices\").children[0].textContent, \"Export Schema Library\");"
    },
    {
      ...
      "binding": "assert.equal(elements.get(\"#schema-export-choices\").children[3].textContent, \"JSON Schema Draft 2020-12 bundle\");"
    },
    {
      ...
      "binding": "assert.equal(elements.get(\"#schema-export-choices\").children[4].textContent, \"For third-party standards-based validation; not extension configuration.\");"
    },
    {
      ...
      "expected": "[]",
      "binding": "assert.deepEqual(schemaDownloads, [], \"cancelled export produces no download\");"
    },
    {
      ...
      "expected": "true",
      "binding": "assert.equal(elements.get(\"#schema-export-compatibility-review\").open, true);"
    },
    {
      ...
      "binding": "assert.equal(elements.get(\"#schema-export-compatibility-review\").children[0].textContent, \"JSON Schema Draft 2020-12 compatibility review\");"
    },
    {
      ...
      "binding": "assert.equal(elements.get(\"#schema-export-compatibility-review\").children[2][\"aria-label\"], \"Standard export conversions\");"
    },
    {
      ...
      "binding": "assert.equal(uiController.omittedRuleStatus(1), \"1 omitted rule\");"
    },
    {
      ...
      "binding": "assert.match(schemaDownloads[0], /schema.*\\.json/, \"confirmed standard export crosses the typed download port\");"
    },
    {
      ...
      "binding": "assert.equal(elements.get(\"#schema-result\").textContent,`Exported JSON Schema Draft 2020-12 bundle · ${publishedExportCount} schemas · 2 omitted rules.`);"
    },
    {
      ...
      "binding": "assert.deepEqual(elements.get(\"#export-schema\").focusOptions,{preventScroll:true},\"export completion restores trigger focus without scrolling\");"
    },
    {
      ...
      "binding": "assert.equal(elements.get(\"#schema-result\").textContent,`Exported Extension backup · ${uiController.schemas().length} schemas and ${uiController.rules().length} rules.`);"
    },
    {
      ...
      "binding": "assert.equal(elements.get(\"#schema-result\").textContent,`Exported Extension schema package · ${exportedSchema.name} revision ${exportedSchema.version}.`);"
    },
    {
      ...
      "binding": "assert.equal(elements.get(\"#schema-result\").textContent,`Exported JSON Schema Draft 2020-12 · ${exportedSchema.name} revision ${exportedSchema.version} · 0 omitted rules.`);"
    }
  ]
};
