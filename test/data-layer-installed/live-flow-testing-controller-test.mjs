...
const controller = createLiveFlowTestingInstalledController({ root:{querySelector:()=>null}, activeProject:async()=>undefined,
  events:()=>[], saveSummary(){}, subscribe:() => () => { removed += 1; }, createUi:(options)=>{uiOptions=options;return ui;} });
...
uiOptions.saveSummary({id:"completed:from-ui"});
assert.deepEqual(controller.state().completed, [{id:"completed:from-ui"}], "Live Flow retains evidence completed by its UI");
