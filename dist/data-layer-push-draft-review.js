...
export function createPushDraftReview(editor, target) {
    ...
    const identityAndExecution = [
        ...(saved.name === reviewedEditor.template.name ? [] : [["Template name", `${saved.name} → ${reviewedEditor.template.name}`]]),
        ...(saved.eventName === reviewedEditor.template.eventName ? [] : [["Event name", `${saved.eventName} → ${reviewedEditor.template.eventName}`]]),
        ...(saved.destination === reviewedEditor.template.destination ? [] : [["Destination", `${saved.destination} → ${reviewedEditor.template.destination}`]]),
    ];
    return {
