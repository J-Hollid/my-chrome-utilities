...
export function createFlowSectionAroundFrames(state, flowId, input, id) {
    ...
    if (!name)
        throw new Error("A Flow Section requires a name.");
    if (!frameIds.size)
        throw new Error("Wrap selection requires at least one Page frame.");
    if ([...frameIds].some((frameId) => !known.has(frameId)))
        throw new Error("Wrap selection contains an unknown Page frame.");
...
export function addFlowPageFrameAtPosition(state, flowId, pageId, position, sectionId, id) {
    const graph = sectionGraph(state.project, flowId), page = state.project.collections.pages.find(({ id }) => id === pageId), section = sectionId ? graph.sections?.find(({ id }) => id === sectionId) : undefined;
    if (!page)
        throw new Error(`Unknown Page ${pageId}.`);
    if (sectionId && !section)
...
export function addFlowPageFrameAndRelationship(state, flowId, input, id) {
    ...
    if (!source)
        throw new Error(`Unknown Page frame ${input.sourceId}.`);
    if (!page)
        throw new Error(`Unknown Page ${input.pageId}.`);
    if (!kind)
        throw new Error("The selected Page ports do not define a documentary relationship kind.");
...
const legacyApplicabilitySetId = (group) => typeof group.applicabilitySetId === "string" && group.applicabilitySetId.length > 0 ? group.applicabilitySetId : undefined;
...
const expectedApplications = (before, page) => { const legacy = legacyCollections(before).pageGroups ?? [], stored = Array.isArray(page.pageGroupIds) ? page.pageGroupIds.map(String) : [], members = legacy.filter((group) => (group.pageIds ?? []).map(String).includes(page.id)).map(({ id }) => id); return unique([...stored, ...members]).map((propertySetId) => { const group = legacy.find(({ id }) => id === propertySetId), applicabilitySetId = legacyApplicabilitySetId(group); return { name: group.name, propertySetId, ...(applicabilitySetId ? { applicabilitySetId } : {}) }; }); };
...
export function upgradePageGroupsToPropertySets(state, id) {
    ...
    return transactProject(state, "Upgrade Page Groups to Property Sets and Flow Sections", (project) => {
        const legacySets = legacyCollections(project).pageGroups ?? [], propertySets = legacySets.map((group) => { const next = clone(group); delete next.pageIds; delete next.applicabilitySetId; return next; }), applicabilityBySet = new Map(legacySets.map((group) => [group.id, legacyApplicabilitySetId(group)])), legacyMembers = new Map(legacySets.map((group) => [group.id, new Set((group.pageIds ?? []).map(String))]));
