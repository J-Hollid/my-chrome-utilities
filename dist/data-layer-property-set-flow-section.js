...
const legacyFrameWidth = (graph, frame) => Math.max(190, ...(graph.occurrences ?? []).filter((occurrence) => occurrence.pageFrameId === frame.id).map((occurrence) => Number(occurrence.position?.x ?? 24) + 190));
const normalizeLegacyFramePlacement = (graph, frame, sectionId, laneOffset, namedWidth) => {
    const legacy = frame, x = Number(frame.position.x ?? 24), absoluteX = legacy.freePageRegion === "before-lanes" ? x : legacy.freePageRegion === "after-lanes" ? laneOffset + namedWidth + x : laneOffset + x, next = { ...legacy, ...(sectionId ? { sectionId } : {}), position: { x: absoluteX, y: frame.position.y } };
