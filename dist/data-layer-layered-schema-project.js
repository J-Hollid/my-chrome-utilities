...
export function layeredContributorsForPath(state, path, observation) {
    ...
    const pageApplications = page?.propertySetApplications ?? [], groupIds = path.pageGroupIds ?? (path.pageGroupId ? [path.pageGroupId] : []), groupRecords = groupIds.flatMap((groupId) => propertySets(state).filter(({ id }) => id === groupId).map((group) => { const contributor = contributionFor(group, "Property Set"), application = pageApplications.find(({ propertySetId }) => propertySetId === groupId), applicabilityId = application?.applicabilitySetId ?? group.applicabilitySetId, applicability = (state.project.collections.applicabilitySets ?? []).find(({ id }) => id === applicabilityId), conditional = Boolean(applicability), evaluated = observation !== undefined, active = !evaluated || conditionMatches(applicability?.condition, observation); return { group, active, contributor: { ...contributor, ...(conditional ? { applicabilityConditional: true, applicabilitySetId: applicability.id, applicabilitySetName: applicability.name, applicabilityCondition: structuredClone(applicability.condition), ...(evaluated ? { active, ...(!active ? { exclusionReason: `${applicability.name} did not match` } : {}) } : {}) } : {}) } }; }));
...
export function assignmentContributorTargets(state) {
    ...
    return [...collection(state.project.collections.profiles, "Shared Profile"), ...collection((state.project.collections.propertySets ?? state.project.collections.propertySets), "Property Set"), ...collection(state.project.collections.pages, "Page"), ...collection(state.project.collections.events, "Event"), ...frames];
}
export function compileAssignmentContributorTarget(state, assignment, context, observation = {}) {
    ...
    const source = target.kind === "Shared Profile" ? state.project.collections.profiles.find(({ id }) => id === target.id) : target.kind === "Property Set" ? state.project.collections.propertySets.find(({ id }) => id === target.id) : target.kind === "Page" ? state.project.collections.pages.find(({ id }) => id === target.id) : target.kind === "Event" ? state.project.collections.events.find(({ id }) => id === target.id) : flowPageFrameContributor(state, target.flowId, target.id), scope = target.kind === "Flow Page instance" ? "Flow Page-instance" : target.kind;
    if (!source)
