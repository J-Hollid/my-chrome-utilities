...
export function layeredContributorsForPath(state, path, observation) {
    ...
    const groupIds = path.pageGroupIds ?? (path.pageGroupId ? [path.pageGroupId] : []), groups = groupIds.flatMap((groupId) => state.project.collections.pageGroups.filter(({ id }) => id === groupId).map((group) => { const contributor = contributionFor(group, "Page Group"), applicability = (state.project.collections.applicabilitySets ?? []).find(({ id }) => id === group.applicabilitySetId), conditional = Boolean(applicability), evaluated = observation !== undefined, active = evaluated ? conditionMatches(applicability?.condition, observation) : false; return { ...contributor, ...(conditional ? { applicabilityConditional: true, applicabilitySetId: applicability.id, applicabilitySetName: applicability.name, applicabilityCondition: structuredClone(applicability.condition), ...(evaluated ? { active, ...(!active ? { exclusionReason: `${applicability.name} did not match` } : {}) } : {}) } : {}) }; }));
    return [...one(state.project.collections.profiles, path.profileId, "Shared Profile"), ...groups, ...one(state.project.collections.pages, path.pageId, "Page"), ...(pageFrame ? [contributionFor(pageFrame, "Flow Page-instance")] : []), ...one(state.project.collections.events, path.eventId, "Event"), ...(occurrence ? [contributionFor(occurrence, occurrence.freePageFrame ? "Flow Page-instance" : "Event-occurrence")] : [])];
}
