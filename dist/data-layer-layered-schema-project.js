...
const propertySets = (state) => (state.project.collections.propertySets ?? state.project.collections.pageGroups);
const orderedPropertySetIds = (state, pageId) => {
    if (!pageId)
        return [];
    const page = state.project.collections.pages.find(({ id }) => id === pageId), applications = page?.propertySetApplications;
    return Array.isArray(applications) ? applications.map((application) => String(application.propertySetId)) : orderedPageGroupIds(state.project, pageId);
};
...
export function layeredContributorsForPath(state, path, observation) {
    const graph = path.flowId ? state.project.documentationFlowGraphs?.[path.flowId] : undefined, pageFrame = graph?.pageFrames?.find(({ id }) => id === path.pageFrameId), occurrence = graph?.occurrences?.find(({ id }) => id === path.occurrenceId), page = state.project.collections.pages.find(({ id }) => id === path.pageId), flowFrame = path.flowId && path.pageFrameId ? flowPageFrameContributor(state, path.flowId, path.pageFrameId) : undefined;
    const one = (entities, id, scope) => id ? entities.filter((entity) => entity.id === id).map((entity) => contributionFor(entity, scope)) : [];
    const pageApplications = page?.propertySetApplications ?? [], groupIds = path.pageGroupIds ?? (path.pageGroupId ? [path.pageGroupId] : []), groupRecords = groupIds.flatMap((groupId) => propertySets(state).filter(({ id }) => id === groupId).map((group) => { const contributor = contributionFor(group, "Page Group"), application = pageApplications.find(({ propertySetId }) => propertySetId === groupId), applicabilityId = application?.applicabilitySetId ?? group.applicabilitySetId, applicability = (state.project.collections.applicabilitySets ?? []).find(({ id }) => id === applicabilityId), conditional = Boolean(applicability), evaluated = observation !== undefined, active = !evaluated || conditionMatches(applicability?.condition, observation); return { group, active, contributor: { ...contributor, ...(conditional ? { applicabilityConditional: true, applicabilitySetId: applicability.id, applicabilitySetName: applicability.name, applicabilityCondition: structuredClone(applicability.condition), ...(evaluated ? { active, ...(!active ? { exclusionReason: `${applicability.name} did not match` } : {}) } : {}) } : {}) } }; }));
    const directProfileIds = [...new Set([...(path.profileIds ?? []), ...(path.profileId ? [path.profileId] : [])])], routes = new Map(), downstreamNames = [...(page ? [page.name] : []), ...(flowFrame ? [flowFrame.name] : [])], addRoute = (profileId, names) => { const profile = state.project.collections.profiles.find(({ id }) => id === profileId); if (!profile)
        return; const route = [profile.name, ...names.filter((name) => name !== profile.name)].join(" → "), known = routes.get(profileId) ?? []; if (!known.includes(route))
        routes.set(profileId, [...known, route]); };
    ...
    for (const profileId of directProfileIds)
        addRoute(profileId, directTargetNames);
    for (const { group, active } of groupRecords)
        if (active)
            for (const profileId of referencedProfileIds(group))
    ...
    for (const record of groupRecords) {
        if (record.active)
            for (const profileId of referencedProfileIds(record.group)) {
                const profile = profileContributor(profileId);
                if (profile)
                    contributors.push(profile);
            }
        contributors.push(record.contributor);
    }
...
export function assignmentContributorTargets(state) {
    ...
    return [...collection(state.project.collections.profiles, "Shared Profile"), ...collection((state.project.collections.propertySets ?? state.project.collections.pageGroups), "Property Set"), ...collection(state.project.collections.pages, "Page"), ...collection(state.project.collections.events, "Event"), ...frames];
}
export function compileAssignmentContributorTarget(state, assignment, context, observation = {}) {
    const assignmentKind = assignment.targetKind === "Page Group" ? "Property Set" : assignment.targetKind, target = assignmentContributorTargets(state).find(({ id, kind }) => id === assignment.targetId && kind === assignmentKind);
    if (!target)
        throw new Error(`Assignment ${assignment.name} has an unavailable contributor target.`);
    const source = target.kind === "Shared Profile" ? state.project.collections.profiles.find(({ id }) => id === target.id) : target.kind === "Property Set" || target.kind === "Page Group" ? (state.project.collections.propertySets ?? state.project.collections.pageGroups).find(({ id }) => id === target.id) : target.kind === "Page" ? state.project.collections.pages.find(({ id }) => id === target.id) : target.kind === "Event" ? state.project.collections.events.find(({ id }) => id === target.id) : flowPageFrameContributor(state, target.flowId, target.id), scope = target.kind === "Flow Page instance" ? "Flow Page-instance" : target.kind === "Property Set" ? "Page Group" : target.kind;
    if (!source)
