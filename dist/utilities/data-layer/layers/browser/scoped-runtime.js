...
export function mountScopedDataLayerAdapter(root, panelIds, storage) {
    ...
    if (panelIds.includes("data-layer-panel-library")) {
        ...
        const persist = () => storage?.setItem(EVENT_TEMPLATE_LIBRARY_STORAGE_KEY, serializeEventTemplateLibrary(templates));
        persist();
        bind("#save-template-revision", () => { const editor = updateDraftJson(openPropertyEditor(templates[0] ?? original), JSON.stringify({ event: "checkout", step: 2 })); templates = [saveDraftRevision(editor).template]; persist(); const result = root.querySelector("#event-template-result"); if (result)
            result.textContent = `Revision ${templates[0]?.version ?? 0} saved`; });
        bind("#export-event-library", () => { const serialized = JSON.stringify(eventLibraryExport(templates)), imported = eventLibraryImport(serialized); templates = replaceImportedTemplates([], imported.templates); persist(); const result = root.querySelector("#event-library-transfer-result"); if (result) {
            result.dataset.transferTemplates = String(templates.length);
            result.dataset.transferBytes = String(serialized.length);
            result.textContent = `${templates.length} template exported and imported with ${templates[0]?.revisionHistory?.length ?? 0} revisions`;
        } });
    }
    ...
    if (panelIds.includes("data-layer-panel-schemas")) {
        ...
        bind("#recheck-schema-validation", () => { const validation = validateWithSchema({ sourceId: "scoped", eventName: "checkout", payload: {}, rawInput: {} }, schema, [schema]); const result = root.querySelector("#schema-result"); if (result) {
            result.dataset.validationState = validation.state;
            result.textContent = `Validation complete: ${validation.state}`;
        } });
        bind("#export-schema", () => { const archive = createSchemaLibraryExport([schema], []), serialized = JSON.stringify(archive); const result = root.querySelector("#schema-result"); if (result) {
            result.dataset.transferBytes = String(serialized.length);
    ...
    if (panelIds.includes("data-layer-panel-defects")) {
        ...
        if (panel && ownerDocument) {
            ...
            const onSave = () => { const library = restoreDefectLibrary(storage?.getItem(DEFECT_LIBRARY_STORAGE_KEY) ?? null); const saved = addDefect(library, defect, true); storage?.setItem(DEFECT_LIBRARY_STORAGE_KEY, serializeDefectLibrary(saved.library)); status.dataset.saveStatus = "saved"; status.textContent = "Defect saved"; };
            const onCopy = async () => { const copied = await copyText(`${report.summary}\n\n${report.description}\nExpected: ${report.expectedEvent}`); status.dataset.copyStatus = copied ? "copied" : "failed"; status.textContent = copied ? "Defect report copied for Jira" : "Copy failed"; };
            save.addEventListener("click", onSave);
            copy.addEventListener("click", onCopy);
            cleanups.push(() => { save.removeEventListener("click", onSave); copy.removeEventListener("click", onCopy); save.remove(); copy.remove(); status.remove(); });
        }
    }
