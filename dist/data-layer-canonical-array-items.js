...
export function canonicalArrayScopeIssue(scope) {
    if (!scope)
        return undefined;
    const positioned = scope.boundaries.filter(({ mode }) => mode === "position");
    if (positioned.length > 1)
        return "Choose Item at position for at most one array boundary.";
    if (positioned.some(({ position }) => !Number.isInteger(position) || Number(position) < 1))
        return "Enter a whole position of 1 or greater.";
    return undefined;
}
export function canonicalArrayScopeSummary(boundaries) {
...
export function canonicalArrayBoundaries(document, propertyId) {
    const ancestors = [];
    let node = document.nodes[propertyId];
    while (node?.parentId) {
...
const schemaForNode = (document, node) => {
    ...
    if (required.length) {
        if (node.type === "object")
            definition.required = required;
        if (node.type === "array" && (node.itemSchema?.type ?? node.itemType) === "object")
            definition.items.required = required;
    }
    return definition;
};
...
//# sourceMappingURL=data-layer-canonical-array-items.js.map