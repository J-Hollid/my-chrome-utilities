...
export function applySchemaPropertyTypeEdit(schema, edit) {
    ...
    if (impact.incompatible.length && !edit.removeIncompatible)
        throw new Error("Resolve incompatible schema data before saving the type change");
    ...
    const attachedRules = (schema.attachedRules ?? []).filter((rule) => !(impact.incompatible.includes(`rule ${rule.id}`)
