...
export const projectCollectionDefinitions = {
    ...
    pageGroups: { kind: "pageGroups", overview: "Property Sets", singular: "Property Set", addAction: "Add Property Set", purpose: "reusable complete schema components applied to Pages", example: "Checkout base", prerequisites: ["optional Shared Profiles"], consumers: ["Page Property composition", "Assignments"] },
...
export const projectCollectionCreationFields = {
    ...
    pageGroups: [{ key: "description", label: "Description", control: "textarea", guidance: "Explain the reusable schema component and intended use of this Property Set." }],
...
function assignmentTargetExists(project, targetKind, targetId) { const collection = targetKind === "Shared Profile" ? project.collections.profiles : targetKind === "Property Set" ? (project.collections.propertySets ?? project.collections.pageGroups) : targetKind === "Page" ? project.collections.pages : targetKind === "Event" ? project.collections.events : targetKind === "Flow Page instance" ? Object.values(project.documentationFlowGraphs ?? {}).flatMap((graph) => (graph.pageFrames ?? [])) : []; return collection.some(({ id }) => id === targetId); }
export function createProjectCollectionEntity(state, kind, name, id, values = {}) { const clean = name.trim(); if (!clean)
    ...
    throw new Error(`${projectCollectionDefinitions[kind].singular} name must be unique in this project.`); const event = state.project.collections.events[0], eventId = event?.id, pageId = firstId(state.project, "pages"), applicabilitySetId = firstId(state.project, "applicabilitySets"), attributes = projectCreationAttributes(state, kind, values), canonicalSchema = ["profiles", "pageGroups", "pages"].includes(kind) ? createCanonicalSchema({ id: id("canonical-schema"), contributorId: "", contributorName: clean }) : undefined; const canonicalDefault = canonicalSchema ? { canonicalSchema } : {}, defaults = { profiles: { requirements: [], ...canonicalDefault }, pageGroups: { pageIds: [], ...canonicalDefault }, pages: { pageGroupIds: [], ...canonicalDefault }, events: { eventName: clean.toLowerCase().replace(/[^a-z0-9]+/g, "_"), sourceId: "event-history", target: "payload" }, applicabilitySets: { priority: 0, condition: { kind: "all", conditions: [] } }, flows: { steps: [] }, fixtures: { testType: "event-validation", input: {}, inputGuidance: { kind: "authoring-guidance" }, sourceProvenance: { kind: "manual", id: id("manual-source"), revision: "draft" }, reviewedExpectations: {}, status: "Blocked", differences: [], ...(eventId ? { eventId } : {}), ...(pageId ? { pageId } : {}) }, assignments: { ...(eventId ? { eventId, eventName: event?.eventName ?? event?.name } : {}), ...(applicabilitySetId ? { applicabilitySetId } : {}), sourceId: "event-history", target: "payload", priority: 10 } }; return addProjectEntity(state, kind, { name: clean, ...defaults[kind], ...attributes }, id); }
