...
import { pruneUnreferencedFlowConceptVisualAssets } from "./flow-graph/concept-visual-references.js";
export const projectCollectionDefinitions = {
    profiles: { kind: "profiles", overview: "Shared Profiles", singular: "Shared Profile", addAction: "Add Shared Profile", purpose: "reusable complete schemas for generic or event-specific variables", example: "Sitewide", prerequisites: ["project Draft"], consumers: ["Pages", "Property Sets", "Events", "Flows"] },
    propertySets: { kind: "propertySets", overview: "Property Sets", singular: "Property Set", addAction: "Add Property Set", purpose: "reusable complete schema components applied to Pages", example: "Checkout base", prerequisites: ["optional Shared Profiles"], consumers: ["Page Property composition", "Assignments"] },
    pages: { kind: "pages", overview: "Pages", singular: "Page", addAction: "Add Page", purpose: "reusable schema context and specific requirements", example: "Cart", prerequisites: ["optional Property Sets"], consumers: ["Flows", "Test cases", "Assignments"] },
    events: { kind: "events", overview: "Events", singular: "Event", addAction: "Add Event", purpose: "reusable interaction events within Pages", example: "Purchase", prerequisites: ["optional Shared Profiles"], consumers: ["Flows", "Assignments", "Test cases"] },
    ...
    flows: { kind: "flows", overview: "Flows", singular: "Flow", addAction: "Add Flow", purpose: "documentary journey topology", example: "Checkout journey", prerequisites: ["Pages", "Events"], consumers: ["documentation"] },
    fixtures: { kind: "fixtures", overview: "Test cases", singular: "Test case", addAction: "Add Test case", purpose: "saved input plus reviewed expectations rerunnable against the current Draft", example: "Valid purchase", prerequisites: ["a Page or Event", "effective input guidance"], consumers: ["advisory preflight evidence"] },
...
export const projectCollectionCreationFields = {
    ...
    propertySets: [{ key: "description", label: "Description", control: "textarea", guidance: "Explain the reusable schema component and intended use of this Property Set." }],
    pages: [{ key: "description", label: "Description", control: "textarea", guidance: "Explain the reusable Page schema context and intended use." }],
    ...
    fixtures: [{ key: "testType", label: "Test case type", control: "select", options: [{ value: "event-validation", label: "Event validation test" }, { value: "page-context", label: "Page context test" }], defaultValue: "event-validation" }, { key: "eventId", label: "Event", control: "select", collection: "events" }, { key: "pageId", label: "Page", control: "select", collection: "pages" }],
    assignments: [{ key: "targetKind", label: "Contributor kind", control: "select", options: [{ value: "Shared Profile", label: "Shared Profile" }, { value: "Property Set", label: "Property Set" }, { value: "Page", label: "Page" }, { value: "Event", label: "Event" }, { value: "Flow Page instance", label: "Flow Page instance" }], defaultValue: "Shared Profile" }, { key: "targetId", label: "Contributor target", control: "select" }, { key: "eventId", label: "Event", control: "select", collection: "events" }, { key: "applicabilitySetId", label: "Applicability Set", control: "select", collection: "applicabilitySets" }, { key: "priority", label: "Priority", control: "number", defaultValue: 10 }],
};
...
export function inspectProjectEntityRemoval(state, kind, identity) { const entity = state.project.collections[kind].find(({ id }) => id === identity); if (!entity)
    throw new Error(`Unknown ${projectCollectionDefinitions[kind].singular} ${identity}.`); const dependencies = projectEntityDependencies(state.project, kind, identity), graphs = state.project.documentationFlowGraphs, ownedTopology = kind === "flows" && Boolean(graphs?.[identity]); return { kind, id: identity, name: entity.name, dependencies, blocked: Boolean(dependencies.length), summary: `${entity.name} · one ${projectCollectionDefinitions[kind].singular} removal${ownedTopology ? " with its complete owned Flow topology" : ""} · ${dependencies.length} dependent reference${dependencies.length === 1 ? "" : "s"} · project remains Draft and evidence becomes stale.` }; }
export function removeProjectCollectionEntity(state, kind, identity) { const review = inspectProjectEntityRemoval(state, kind, identity); if (review.blocked)
    throw new Error(`Remove ${review.name} is blocked by ${review.dependencies.map(({ name }) => name).join(", ")}.`); return transactProject(state, `Remove ${review.name}`, (project) => { const currentGraphs = project.documentationFlowGraphs, documentationFlowGraphs = kind === "flows" && currentGraphs ? Object.fromEntries(Object.entries(currentGraphs).filter(([flowId]) => flowId !== identity)) : currentGraphs; return pruneUnreferencedFlowConceptVisualAssets({ ...project, collections: { ...project.collections, [kind]: project.collections[kind].filter(({ id }) => id !== identity) }, ...(documentationFlowGraphs ? { documentationFlowGraphs } : {}) }); }); }
...
function assignmentTargetExists(project, targetKind, targetId) { const collection = targetKind === "Shared Profile" ? project.collections.profiles : targetKind === "Property Set" ? project.collections.propertySets : targetKind === "Page" ? project.collections.pages : targetKind === "Event" ? project.collections.events : targetKind === "Flow Page instance" ? Object.values(project.documentationFlowGraphs ?? {}).flatMap((graph) => (graph.pageFrames ?? [])) : []; return collection.some(({ id }) => id === targetId); }
export function createProjectCollectionEntity(state, kind, name, id, values = {}) { const clean = name.trim(); if (!clean)
    ...
    throw new Error(`${projectCollectionDefinitions[kind].singular} name must be unique in this project.`); const event = state.project.collections.events[0], eventId = event?.id, pageId = firstId(state.project, "pages"), applicabilitySetId = firstId(state.project, "applicabilitySets"), attributes = projectCreationAttributes(state, kind, values), canonicalSchema = ["profiles", "propertySets", "pages"].includes(kind) ? createCanonicalSchema({ id: id("canonical-schema"), contributorId: "", contributorName: clean }) : undefined; const canonicalDefault = canonicalSchema ? { canonicalSchema } : {}, defaults = { profiles: { requirements: [], ...canonicalDefault }, propertySets: { ...canonicalDefault }, pages: { propertySetApplications: [], ...canonicalDefault }, events: { eventName: clean.toLowerCase().replace(/[^a-z0-9]+/g, "_"), sourceId: "event-history", target: "payload" }, applicabilitySets: { priority: 0, condition: { kind: "all", conditions: [] } }, flows: { steps: [] }, fixtures: { testType: "event-validation", input: {}, inputGuidance: { kind: "authoring-guidance" }, sourceProvenance: { kind: "manual", id: id("manual-source"), revision: "draft" }, reviewedExpectations: {}, status: "Blocked", differences: [], ...(eventId ? { eventId } : {}), ...(pageId ? { pageId } : {}) }, assignments: { ...(eventId ? { eventId, eventName: event?.eventName ?? event?.name } : {}), ...(applicabilitySetId ? { applicabilitySetId } : {}), sourceId: "event-history", target: "payload", priority: 10 } }; return addProjectEntity(state, kind, { name: clean, ...defaults[kind], ...attributes }, id); }
