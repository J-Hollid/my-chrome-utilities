...
export function renderMissingEventDefectReportBuilder(root, visits, schemas, options) {
    ...
    if (options.initialVisitId)
        draft = changeMissingEventScope(draft, options.initialVisitId);
    ...
    const render = () => {
        ...
        evidence.setAttribute("aria-label", "Missing event evidence");
        evidence.append(element("h5", "Reproduction and supporting timeline"));
        const manualLabel = element("label", "Manual reproduction steps, one per line");
        const manual = element("textarea");
        manual.value = manualSteps;
        manual.addEventListener("input", () => { manualSteps = manual.value; });
        manualLabel.append(manual);
        evidence.append(manualLabel);
        for (const event of draft.scope.events) {
            const label = element("label", `${event.captureTime ?? "Time unavailable"} · ${event.name} · observed event`);
            const checkbox = element("input");
            checkbox.type = "checkbox";
            checkbox.checked = timelineIds.has(event.id);
            checkbox.addEventListener("change", () => checkbox.checked ? timelineIds.add(event.id) : timelineIds.delete(event.id));
        ...
        preview.setAttribute("aria-label", "Final missing-event report preview");
        ...
        if (completed) {
            ...
            preview.append(element("p", "Confirm and verify the expected event to generate the report."));
