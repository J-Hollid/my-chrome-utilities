...
export function saveComposedSchemaLocalFacetsAndStructures(state, kind, entityId, path, facets, commands, id) { const sparse = Object.fromEntries(Object.entries(facets).filter(([, value]) => value !== undefined && value !== "")); return updateEntity(state, kind, entityId, `Save ${commands.length + 1} ${kind === "pages" ? "Page" : "Page Group"} schema changes`, (entity) => { const existing = entity.localSchemaContributions ?? [], structured = applyLayerConstraintStructures(existing, commands, id), next = [...structured.filter((constraint) => constraint.path !== path), ...(Object.keys(sparse).length ? [{ path, ...clone(sparse) }] : [])]; return { ...entity, localSchemaContributions: next, compiledTargetsStale: true }; }); }
