...
export function saveComposedEventCanonicalDocument(state, entityId, document) { const entity = state.project.collections.events.find(({ id }) => id === entityId); if (!entity)
    throw new Error(`Event ${entityId} is unavailable.`); const workspace = composedSchemaWorkspace(state, entity, "Event"), parents = new Map(workspace.rows.map(({ path, inherited }) => [path, inherited])), constraints = canonicalConstraints(document).map((constraint) => sparseAgainst(constraint, parents.get(constraint.path))).filter((constraint) => Object.keys(constraint).length > 1); return updateEntity(state, "events", entityId, `Save effective canonical schema for ${entity.name}`, (current) => ({ ...current, localSchemaContributions: constraints, compiledTargetsStale: true })); }
function saveFlowComposedCanonicalDocument(state, flowId, entityId, field, scope, document) {
    ...
    const workspace = composedSchemaWorkspace(state, entity, scope, undefined, flowId), parents = new Map(workspace.rows.map(({ path, inherited }) => [path, inherited])), constraints = canonicalConstraints(document).map((constraint) => sparseAgainst(constraint, parents.get(constraint.path))).filter((constraint) => Object.keys(constraint).length > 1);
    return transactProject(state, `Save effective canonical schema for ${entity.name}`, (project) => { const graphs = project.documentationFlowGraphs, current = graphs[flowId], entries = current[field] ?? []; return { ...project, documentationFlowGraphs: { ...graphs, [flowId]: { ...current, [field]: entries.map((candidate) => { if (candidate.id !== entityId)
                    return candidate; const next = { ...candidate, localSchemaContributions: constraints, compiledTargetsStale: true }; delete next.canonicalSchema; delete next.schemaConstraints; return next; }) } } }; });
}
...
export function saveComposedSchemaLocalFacetsAndStructures(state, kind, entityId, path, facets, commands, id) { const sparse = Object.fromEntries(Object.entries(facets).filter(([, value]) => value !== undefined && value !== "")), retainFocusedFacets = !structureDeletesPath(commands, path) && Object.keys(sparse).length > 0; return updateEntity(state, kind, entityId, `Save ${commands.length + 1} ${kind === "pages" ? "Page" : "Page Group"} schema changes`, (entity) => { const existing = entity.localSchemaContributions ?? [], structured = applyLayerConstraintStructures(existing, commands, id), next = [...structured.filter((constraint) => constraint.path !== path), ...(retainFocusedFacets ? [{ path, ...clone(sparse) }] : [])]; return { ...entity, localSchemaContributions: next, compiledTargetsStale: true }; }); }
