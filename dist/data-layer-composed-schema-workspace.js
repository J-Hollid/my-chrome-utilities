...
const storedCanonicalDraft = (entity) => { const document = entity.canonicalSchema; return Boolean(document && Object.keys(document.nodes).length > 0 && !entity.localSchemaContributions && !entity.schemaConstraints && !entity.requirements); };
const saveStoredCanonicalDraft = (entity, document) => { const next = { ...entity, canonicalSchema: journalFreeCanonicalData(document), compiledTargetsStale: true }; delete next.onlyDefinedFields; return next; };
export function saveComposedCanonicalDocument(state, kind, entityId, document) { const entity = state.project.collections[kind].find(({ id }) => id === entityId); if (!entity)
    throw new Error(`${kind === "pages" ? "Page" : "Property Set"} ${entityId} is unavailable.`); const scope = kind === "pages" ? "Page" : "Property Set", workspace = composedSchemaWorkspace(state, entity, scope), parents = new Map(workspace.rows.map(({ path, inherited }) => [path, inherited])), constraints = sparseCanonicalConstraints(document, parents); return updateEntity(state, kind, entityId, `Save effective canonical schema for ${entity.name}`, (current) => { if (storedCanonicalDraft(current))
    return saveStoredCanonicalDraft(current, document); const next = { ...current, localSchemaContributions: constraints, onlyDefinedFields: document.onlyDefinedFields === true, compiledTargetsStale: true }; delete next.canonicalSchema; delete next.schemaConstraints; return next; }); }
export function saveComposedEventCanonicalDocument(state, entityId, document) { const entity = state.project.collections.events.find(({ id }) => id === entityId); if (!entity)
    throw new Error(`Event ${entityId} is unavailable.`); const workspace = composedSchemaWorkspace(state, entity, "Event"), parents = new Map(workspace.rows.map(({ path, inherited }) => [path, inherited])), constraints = sparseCanonicalConstraints(document, parents); return updateEntity(state, "events", entityId, `Save effective canonical schema for ${entity.name}`, (current) => storedCanonicalDraft(current) ? saveStoredCanonicalDraft(current, document) : { ...current, localSchemaContributions: constraints, onlyDefinedFields: document.onlyDefinedFields === true, compiledTargetsStale: true }); }
function saveFlowComposedCanonicalDocument(state, flowId, entityId, field, scope, document) {
    ...
    return transactProject(state, `Save effective canonical schema for ${entity.name}`, (project) => { const graphs = project.documentationFlowGraphs, current = graphs[flowId], entries = current[field] ?? []; return { ...project, documentationFlowGraphs: { ...graphs, [flowId]: { ...current, [field]: entries.map((candidate) => { if (candidate.id !== entityId)
                    return candidate; if (storedCanonicalDraft(candidate))
                    return saveStoredCanonicalDraft(candidate, document); const next = { ...candidate, localSchemaContributions: constraints, onlyDefinedFields: document.onlyDefinedFields === true, compiledTargetsStale: true }; delete next.canonicalSchema; delete next.schemaConstraints; return next; }) } } }; });
}
