...
export function saveComposedSchemaPolicy(state, kind, entityId, onlyDefinedFields) { return updateEntity(state, kind, entityId, `Change defined-fields policy at ${kind === "pages" ? "Page" : "Page Group"}`, (entity) => entityWithSchemaPolicy(entity, onlyDefinedFields)); }
export function saveComposedEntitySchemaPolicy(state, kind, entityId, onlyDefinedFields) { return updateEntity(state, kind, entityId, `Change defined-fields policy at ${kind === "pages" ? "Page" : kind === "pageGroups" ? "Page Group" : "Event"}`, (entity) => entityWithSchemaPolicy(entity, onlyDefinedFields)); }
...
} return Object.keys(next).length > 1 ? [next] : []; })); return { ...entity, localSchemaContributions: withoutFacet(entity.localSchemaContributions), schemaConstraints: withoutFacet(entity.schemaConstraints), compiledTargetsStale: true }; }); }
