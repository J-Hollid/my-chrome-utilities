...
export function validateFlowVisualMetadata(value) {
    ...
    if (!Number.isInteger(value.width) || !Number.isInteger(value.height) || value.width < 1 || value.height < 1 || value.width > FLOW_VISUAL_LIMITS.dimension || value.height > FLOW_VISUAL_LIMITS.dimension || value.width * value.height > FLOW_VISUAL_LIMITS.pixels)
        throw new DOMException(`Visual ${value.id} dimensions exceed the supported limit.`, "DataError");
    if (!Number.isInteger(value.byteLength) || value.byteLength < 1 || value.byteLength > FLOW_VISUAL_LIMITS.sourceBytes)
...
const ascii = (bytes, start, length) => new TextDecoder().decode(bytes.slice(start, start + length));
const u16be = (bytes, offset) => new DataView(bytes.buffer, bytes.byteOffset + offset, 2).getUint16(0, false);
const u24le = (bytes, offset) => bytes[offset] | (bytes[offset + 1] << 8) | (bytes[offset + 2] << 16);
const u32be = (bytes, offset) => new DataView(bytes.buffer, bytes.byteOffset + offset, 4).getUint32(0, false);
const u32le = (bytes, offset) => new DataView(bytes.buffer, bytes.byteOffset + offset, 4).getUint32(0, true);
function pngDimensions(bytes) {
    if (bytes.length < 24 || ![137, 80, 78, 71, 13, 10, 26, 10].every((value, index) => bytes[index] === value) || ascii(bytes, 12, 4) !== "IHDR")
        return undefined;
...
async function structurallyComplete(body, mediaType) { if (mediaType === "image/jpeg") {
    const suffix = new Uint8Array(await body.slice(-2).arrayBuffer());
    return suffix[0] === 0xff && suffix[1] === 0xd9;
} if (mediaType === "image/webp") {
    const header = new Uint8Array(await body.slice(0, 12).arrayBuffer());
    return header.length === 12 && u32le(header, 4) + 8 === body.size;
} let offset = 8, sawImage = false; while (offset + 12 <= body.size) {
    const header = new Uint8Array(await body.slice(offset, offset + 8).arrayBuffer()), length = u32be(header, 0), kind = ascii(header, 4, 4), end = offset + 12 + length;
    if (end > body.size)
        return false;
    if (kind === "IDAT")
        sawImage = true;
...
async function requireCompleteDecode(body, mediaType) {
    if (typeof createImageBitmap === "function") {
        let bitmap;
        try {
            bitmap = await createImageBitmap(body);
            if (!bitmap.width || !bitmap.height)
                throw new Error("empty decode");
            return;
        }
        catch {
            throw new DOMException(`Visual body is not a completely decodable ${mediaType.replace("image/", "").toUpperCase()} image.`, "DataError");
        }
        finally {
            bitmap?.close();
        }
    }
    if (!await structurallyComplete(body, mediaType))
        throw new DOMException(`Visual body is not a complete ${mediaType.replace("image/", "").toUpperCase()} image.`, "DataError");
}
export async function inspectFlowVisualBody(body, mediaType) {
    ...
    if (!dimensions)
        throw new DOMException(`Visual body is not a valid ${mediaType.replace("image/", "").toUpperCase()} image.`, "DataError");
    return dimensions;
}
