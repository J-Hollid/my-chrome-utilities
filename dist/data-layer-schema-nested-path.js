...
export function canonicalNestedPath(path) {
    return `/${pathSegments(path).join("/")}`;
}
export function canonicalPathForTargetIntent(intent) {
    const normalized = intent.trim().toLowerCase();
    const indexed = normalized.match(/^(.+) item at zero-based index (\d+)$/);
    if (indexed)
        return `/${indexed[1]?.trim().replaceAll(" ", "/")}/${indexed[2]}`;
    const everyItem = normalized.match(/^(.+) in every (.+) item$/);
    if (everyItem)
        return `/${everyItem[2]?.trim().replaceAll(" ", "/")}/*/${everyItem[1]?.trim().replaceAll(" ", "/")}`;
    return `/${normalized.replace(/^nested\s+/, "").replaceAll(" ", "/")}`;
}
function missingMatch(templatePath, concrete, remaining) {
    if (remaining.includes("*"))
        return [];
    return [{ templatePath, concretePath: `/${[...concrete, ...remaining].join("/")}`, value: undefined, exists: false }];
}
export function resolveNestedValues(value, path) {
    const segments = pathSegments(path);
    const templatePath = `/${segments.join("/")}`;
    const visit = (current, remaining, concrete) => {
        const [segment, ...rest] = remaining;
        if (segment === undefined)
            return [{ templatePath, concretePath: `/${concrete.join("/")}`, value: current, exists: true }];
        if (segment === "*") {
...
export function nestedTargetChoices(value, concretePath) {
    ...
    if (arrayIndex < 0)
        return [{ label: "Nested property", path: canonical, matchedValueCount: resolveNestedValues(value, canonical).filter(({ exists }) => exists).length }];
...
export function validateNestedRuleTarget(document, path) {
    const segments = pathSegments(path);
    const canonicalPath = `/${segments.join("/")}`;
    if (path.includes("/-") || segments.some((segment) => /^-\d+$/.test(segment)))
        return { result: "blocked", assistance: "Enter a non-negative array index", canonicalPath };
    let schema = document;
    ...
    for (const segment of segments) {
        if (segment === "*" || /^\d+$/.test(segment)) {
            if (schema?.type !== "array")
                return { result: "blocked", assistance: `${containerName} is not an array`, canonicalPath };
            usedWildcard ||= segment === "*";
            if (/^\d+$/.test(segment))
                usedIndex = Number(segment);
            schema = schema.items;
            continue;
        }
        if (schema?.type === "array") {
            if (schema.items?.type !== "object")
                return { result: "blocked", assistance: `${containerName} items cannot contain property ${segment}`, canonicalPath };
            schema = schema.items.properties?.[segment];
            containerName = segment;
            continue;
        }
        if (schema?.type !== "object")
            return { result: "blocked", assistance: `${containerName} cannot contain child properties`, canonicalPath };
        schema = schema.properties?.[segment];
        containerName = segment;
    }
    if (!schema?.type)
        return { result: "blocked", assistance: "Target path is not present in the schema model", canonicalPath };
...
export function ensureNestedSchemaPath(document, path, targetType) {
    const segments = pathSegments(path);
    const createdNodes = [];
    const add = (schema, remaining, prefix) => {
        const [segment, ...rest] = remaining;
        if (segment === undefined)
            return schema;
        if (segment === "*" || /^\d+$/.test(segment)) {
            const marker = "*";
            const items = schema.items ?? (rest.length ? { type: "object" } : { type: targetType });
            if (!schema.items)
                createdNodes.push(`/${[...prefix, marker].join("/")}`);
            return { ...schema, type: "array", items: rest.length ? add(items, rest, [...prefix, marker]) : items };
        }
        const properties = schema.properties ?? {};
        const next = rest[0];
        const child = properties[segment] ?? (rest.length ? (next === "*" || /^\d+$/.test(next ?? "") ? { type: "array" } : { type: "object" }) : { type: targetType });
        if (!properties[segment])
            createdNodes.push(`/${[...prefix, segment].join("/")}`);
...
//# sourceMappingURL=data-layer-schema-nested-path.js.map