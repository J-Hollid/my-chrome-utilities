...
function synchronizeCanonicalRules(base, current, next) {
    const baseById = new Map(base.map((rule) => [rule.id, rule])), currentById = new Map(current.map((rule) => [rule.id, rule])), nextById = new Map(next.map((rule) => [rule.id, rule])), ids = [...next.map(({ id }) => id), ...current.map(({ id }) => id).filter((id) => !nextById.has(id))];
    return ids.flatMap((id) => { const oldRule = baseById.get(id), localRule = currentById.get(id), newRule = nextById.get(id); if (!oldRule)
        return [clone(localRule ?? newRule)]; if (!localRule)
        return []; if (!newRule)
...
function canonicalSourceChangeIds(base, next) {
    const paths = (document) => document.sourceContent?.pathsByNodeId ?? Object.fromEntries(Object.values(document.nodes).map((node) => [node.id, canonicalPropertyPath(document, node.id)])), basePaths = paths(base), nextPaths = paths(next), baseByPath = new Map(Object.entries(basePaths).map(([id, path]) => [path, base.nodes[id]])), nextByPath = new Map(Object.entries(nextPaths).map(([id, path]) => [path, next.nodes[id]]));
    const normalized = (document, node, pathById) => { const { id: _id, ...value } = clone(node), definition = document.sourceContent?.definitionsByNodeId?.[node.id], propertyPath = (id) => pathById[id] ?? id, predicate = (entry) => entry.kind === "predicate" ? { ...entry, propertyId: propertyPath(entry.propertyId) } : { ...entry, children: entry.children.map(predicate) }; if (value.parentId)
        value.parentId = propertyPath(value.parentId); value.provenance = value.provenance.map(({ revision: _revision, ...entry }) => entry); value.allowedValues = value.allowedValues.map(({ id: _allowedId, ...entry }) => entry); value.rules = value.rules.map((rule) => ({ ...rule, id: rule.id.replace(node.id, propertyPath(node.id)), ...(rule.condition ? { condition: predicate(rule.condition) } : {}), ...(rule.provenance ? { provenance: (({ revision: _revision, ...entry }) => entry)(rule.provenance) } : {}) })); return { node: value, definition: clone(definition) }; }, sourcePaths = new Set([...baseByPath.keys(), ...nextByPath.keys()]);
...
function preserveCanonicalFacets(base, current, next, nextSource, affectedPropertyIds) {
    ...
    const sourcePaths = (document) => document.sourceContent?.pathsByNodeId ?? Object.fromEntries(Object.values(document.nodes).map((node) => [node.id, canonicalPropertyPath(document, node.id)])), baseSourcePaths = sourcePaths(base), baseIds = new Map(Object.entries(baseSourcePaths).map(([id, path]) => [id, currentBySourcePath.get(path) ?? (current.nodes[id] ? id : currentByPath.get(path) ?? id)]));
    const remapNode = (node, ids) => { const propertyId = (id) => ids.get(id) ?? id, stableId = propertyId(node.id), embeddedId = (id) => id.replace(node.id, stableId), predicate = (value) => value.kind === "predicate" ? { ...clone(value), propertyId: propertyId(value.propertyId) } : { ...clone(value), children: value.children.map(predicate) }; return { ...clone(node), id: stableId, ...(node.parentId ? { parentId: propertyId(node.parentId) } : {}), presence: { ...clone(node.presence), ...(node.presence.condition ? { condition: predicate(node.presence.condition) } : {}) }, allowedValues: node.allowedValues.map((entry) => ({ ...clone(entry), id: embeddedId(entry.id) })), rules: node.rules.map((rule) => ({ ...clone(rule), id: embeddedId(rule.id), ...(rule.condition ? { condition: predicate(rule.condition) } : {}) })) }; }, alignedBase = Object.fromEntries(Object.values(base.nodes).map((node) => { const aligned = remapNode(node, baseIds); return [aligned.id, aligned]; })), nextSourcePaths = sourcePaths(nextSource), generatedByPath = new Map(Object.values(next.nodes).map((node) => [canonicalPropertyPath(next, node.id), nextIds.get(node.id) ?? node.id])), sourceIds = new Map(Object.entries(nextSourcePaths).map(([id, path]) => [id, currentBySourcePath.get(path) ?? currentByPath.get(path) ?? generatedByPath.get(path) ?? id])), alignedSource = Object.fromEntries(Object.values(nextSource.nodes).map((node) => { const aligned = remapNode(node, sourceIds); return [aligned.id, aligned]; })), nodes = {};
    const facets = ["name", "parentId", "order", "type", "itemType", "presence", "allowedValues", "documentation", "expectedValue", "enforcement", "target", "overrideReferences", "provenance"];
    for (const generatedNode of Object.values(next.nodes)) {
        const nextNode = remapNode(generatedNode, nextIds), currentNode = current.nodes[nextNode.id], baseNode = alignedBase[nextNode.id], sourceNode = alignedSource[nextNode.id];
        if (!currentNode) {
            nodes[nextNode.id] = nextNode;
            continue;
        }
        const merged = clone(nextNode);
        for (const facet of facets) {
            const value = baseNode && sourceNode ? synchronizeDocument(baseNode[facet], currentNode[facet], sourceNode[facet]) : clone(currentNode[facet]);
            if (value === undefined)
                delete merged[facet];
            else
                merged[facet] = value;
        }
        merged.rules = baseNode && sourceNode ? synchronizeCanonicalRules(baseNode.rules, currentNode.rules, sourceNode.rules) : synchronizeCanonicalRules([], currentNode.rules, sourceNode?.rules ?? nextNode.rules);
...
export function commitSavedSchemaSynchronization(state, review) {
    return transactProject(state, `Synchronize saved schema ${review.schemaId} to revision ${review.toRevision}`, (project) => {
        ...
        let baseSequence = 0;
        const base = savedSchemaCanonicalDocument({ id: review.schemaId, name: adopted.name, version: review.fromRevision, document: clone(priorContent.document), attachedRules: clone(priorContent.rules), documentation: clone(priorContent.documentation) }, (kind) => `${adopted.id}:base:${review.fromRevision}:${kind}:${++baseSequence}`, { id: canonical.id, contributorId: adopted.id, contributorName: adopted.name });
        let sourceSequence = 0;
        const nextSource = savedSchemaCanonicalDocument({ id: review.schemaId, name: adopted.name, version: review.toRevision, document: clone(review.source.document), attachedRules: clone(review.source.rules ?? []), ...(review.source.documentation === undefined ? {} : { documentation: clone(review.source.documentation) }) }, (kind) => `${adopted.id}:source:${review.toRevision}:${kind}:${++sourceSequence}`, { id: canonical.id, contributorId: adopted.id, contributorName: adopted.name }), affectedPropertyIds = canonicalSourceChangeIds(base, nextSource);
        let sequence = 0;
        ...
        const synchronized = preserveCanonicalFacets(base, canonical, generated, nextSource, affectedPropertyIds);
...
function assignmentTarget(project, kind, identity) { if (kind === "Shared Profile")
    return project.collections.profiles.find(({ id }) => id === identity); if (kind === "Property Set")
    return project.collections.propertySets.find(({ id }) => id === identity); if (kind === "Page")
