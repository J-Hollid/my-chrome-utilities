...
function revealLiveProperty(path) {
    ...
    for (const disclosure of Array.from(propertyList?.querySelectorAll("details[data-property-path]") ?? [])) {
        ...
        if (concretePrefixes.has(disclosurePath) || wildcardPrefixes.has(disclosurePath)
            || (affectedBase && (path === affectedBase || path.startsWith(`${affectedBase}/`))))
...
function renderPropertyNode(node, addValidation, schemaDocumentation, expandAllowedValue, addToSchema, declaration) {
    ...
    if (node.expression)
        item.dataset.propertyExpression = node.expression;
    ...
    evaluationList.replaceChildren(...node.evaluations.map((evaluation) => {
        const item = document.createElement("li");
        item.textContent = `${evaluation.status}: ${evaluationText(evaluation)}`;
        if (expandAllowedValue && allowedValueExpansionAvailability(evaluation).available) {
            const add = document.createElement("button");
            add.type = "button";
            add.className = "live-allowed-value-expansion";
            ...
            add.dataset.ruleId = evaluation.ruleId ?? "";
            add.dataset.ruleVersion = String(evaluation.ruleVersion);
            add.addEventListener("click", () => expandAllowedValue(evaluation, add));
            item.append(add);
        }
        return item;
    }));
    ...
    if (documentation) {
        ...
        information.className = "live-property-documentation-control";
        information.textContent = "Information";
        const documentationId = `${item.id}-documentation`;
        const previewId = `${documentationId}-preview`;
        information.setAttribute("aria-label", `Information for ${documentation.displayName || node.name}`);
        information.setAttribute("aria-describedby", previewId);
        information.setAttribute("aria-expanded", "false");
        const documentationPreview = document.createElement("div");
        documentationPreview.id = previewId;
        documentationPreview.className = "live-property-documentation-preview";
        documentationPreview.setAttribute("role", "tooltip");
        documentationPreview.hidden = true;
        documentationPreview.textContent = documentation.description;
        const persistent = document.createElement("section");
        persistent.id = documentationId;
        persistent.className = "live-property-documentation-details";
        persistent.hidden = true;
        persistent.tabIndex = -1;
        ...
        comments.className = "live-property-documentation-comments";
        comments.textContent = documentation.comments ? `Comments: ${documentation.comments}` : "";
        comments.hidden = !documentation.comments;
        ...
        const hideDocumentationPreview = () => { if (document.activeElement !== information)
            documentationPreview.hidden = true; };
        information.addEventListener("pointerenter", showDocumentationPreview);
        information.addEventListener("pointerleave", () => globalThis.setTimeout(hideDocumentationPreview, 0));
        information.addEventListener("focus", showDocumentationPreview);
        information.addEventListener("blur", () => globalThis.setTimeout(hideDocumentationPreview, 0));
        ...
        information.addEventListener("click", () => { persistent.hidden = !persistent.hidden; documentationPreview.hidden = true; information.setAttribute("aria-expanded", String(!persistent.hidden)); if (!persistent.hidden)
            persistent.focus({ preventScroll: true }); });
        information.addEventListener("keydown", (event) => { if (event.key === "Escape") {
            closePersistent();
            event.preventDefault();
        } });
        persistent.addEventListener("keydown", (event) => { if (event.key === "Escape") {
            closePersistent();
            event.preventDefault();
        } });
        row.append(information);
        item.append(documentationPreview, persistent);
    }
    if (addValidation) {
        const add = document.createElement("button");
        add.type = "button";
    ...
    if (node.specificItems?.length) {
        ...
        const list = document.createElement("ul");
        list.replaceChildren(...node.specificItems.map((child) => renderPropertyNode(child, addValidation, schemaDocumentation, expandAllowedValue, addToSchema, declaration)));
...
export function renderLiveInspector(elements, event, actionHandlers, presentation = {}) {
    ...
    const priorOpenPaths = new Set(Array.from(elements.eventInspector.querySelectorAll("#live-validation-properties details[open][data-property-path]")).map(({ dataset }) => dataset.propertyPath ?? ""));
    ...
    if (event.validationDetails?.documentation?.description)
        appendSummaryItem(summary, "Schema description", `${event.validationDetails.documentation.description} · ${event.validationDetails.documentation.descriptionOrigin?.name ?? event.validationDetails.schema?.name ?? "Schema"} revision ${event.validationDetails.documentation.descriptionOrigin?.version ?? event.validationDetails.schema?.version ?? "current"}`);
    ...
    const addToSchema = actionHandlers.addPropertyToSchema ? (path, trigger) => actionHandlers.addPropertyToSchema?.(event, path, trigger) : undefined;
    const declaration = actionHandlers.propertyDeclaration ? (path) => actionHandlers.propertyDeclaration?.(event, path) ?? {} : undefined;
    const expandAllowedValue = actionHandlers.expandAllowedValue ? (evaluation, trigger) => actionHandlers.expandAllowedValue?.(event, evaluation, trigger) : undefined;
    ...
    const applyPropertySearch = () => {
        ...
        const rows = Array.from(propertyList.querySelectorAll(".live-validation-property"));
        const matches = query ? rows.filter((row) => {
            if (row.closest(".live-property-specific-items"))
                return false;
        ...
        for (const match of matches) {
            let parent = match.parentElement?.closest(".live-validation-property");
            while (parent) {
                visible.add(parent);
                parent = parent.parentElement?.closest(".live-validation-property");
            }
        }
        for (const row of rows)
            row.hidden = Boolean(query) && !visible.has(row);
        for (const disclosure of disclosures) {
            if (query)
    ...
    for (const disclosure of Array.from(propertyList.querySelectorAll("details[data-property-path]")))
        disclosure.toggleAttribute("open", priorOpenPaths.has(disclosure.dataset.propertyPath ?? ""));
    if (priorFocusedPropertyPath) {
        const target = Array.from(propertyList.querySelectorAll(".live-validation-property[data-property-path]")).find(({ dataset }) => dataset.propertyPath === priorFocusedPropertyPath);
        if (target) {
            let ancestor = target.parentElement?.closest("details");
            while (ancestor) {
                ancestor.open = true;
