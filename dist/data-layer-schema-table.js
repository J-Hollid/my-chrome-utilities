...
export function ensureSchemaTablePropertyEditorContainmentStyle(dom) {
    if (dom.getElementById(schemaTablePropertyEditorContainmentStyleId))
        return;
    const style = dom.createElement("style");
    style.id = schemaTablePropertyEditorContainmentStyleId;
    style.textContent = schemaTablePropertyEditorContainmentCss;
...
export function applySchemaTablePropertyEditorAllocation(heading, cell, action) {
    ...
    if (action) {
        action.dataset.schemaTablePropertyEditorAction = "true";
...
export function schemaTableRuleOutcomeSummary(rule) {
    ...
    if (rule.kind === "value" && rule.operator !== undefined)
        return valueRuleRequirement(rule.operator, rule.expectedValue);
    if (rule.kind === "value" || rule.kind === "allowed-values")
