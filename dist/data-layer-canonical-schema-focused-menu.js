...
export function renderCanonicalFocusedMenu(node, context) {
    const path = canonicalPropertyPath(context.current(), node.id), state = context.sourceState(node), actions = focusedOwnershipActions({ inherited: state === "inherited", local: state === "local", overridden: state === "overridden", invariant: node.enforcement === "invariant", conflict: false, replaceable: node.enforcement !== "invariant" });
