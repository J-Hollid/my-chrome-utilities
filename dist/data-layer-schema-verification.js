...
function issuesFor(value, schema, path, schemaPath, result, metadata, onlyDeclaredProperties, undeclaredPropertyExceptions, templatePath = "") {
    ...
    if (schema.type === "object" && value && typeof value === "object" && !Array.isArray(value)) {
        ...
        if (onlyDeclaredProperties && !undeclaredPropertyExceptions.has(templatePath || "/"))
        ...
        for (const [property, child] of Object.entries(schema.properties ?? {}))
            if (property in record)
                issuesFor(record[property], child, `${path}/${property}`, `${schemaPath}/properties/${property}`, result, metadata, onlyDeclaredProperties, undeclaredPropertyExceptions, `${templatePath}/${property}`);
    }
    ...
    if (schema.type === "array" && Array.isArray(value) && schema.items)
        value.forEach((item, index) => issuesFor(item, schema.items, `${path}/${index}`, `${schemaPath}/items`, result, metadata, onlyDeclaredProperties, undeclaredPropertyExceptions, `${templatePath}/*`));
}
...
function typedConfiguredValue(rule, value) {
    if (rule.applicableType === "number")
        return Number(value);
    if (rule.applicableType === "boolean")
        return value === "true";
    return value;
}
function valueChoiceEvaluationEvidence(rule, schema, actualValue) {
    const operator = rule.operator?.replaceAll("_", "-").replaceAll(" ", "-").toLowerCase();
    if (operator === "allowed-values")
        return { ruleId: rule.id, operator, schemaId: schema.id, actualValue, allowedValues: configuredAllowedValues(rule) };
    if (operator === "exact-value" && rule.parameters !== undefined)
        return { ruleId: rule.id, operator, schemaId: schema.id, actualValue, allowedValues: [typedConfiguredValue(rule, rule.parameters)] };
    return {};
}
