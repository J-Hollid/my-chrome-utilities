...
export class FlowVisualSha256 {
    ...
    update(bytes) {
        ...
        if (this.pendingBytes) {
            const count = Math.min(64 - this.pendingBytes, bytes.length);
            this.pending.set(bytes.subarray(0, count), this.pendingBytes);
            this.pendingBytes += count;
            offset = count;
            if (this.pendingBytes === 64) {
                compressFlowVisualSha256(this.state, this.pending);
                this.pendingBytes = 0;
            }
        }
        while (offset + 64 <= bytes.length) {
            compressFlowVisualSha256(this.state, bytes.subarray(offset, offset + 64));
            offset += 64;
        }
        if (offset < bytes.length) {
            this.pending.set(bytes.subarray(offset), 0);
            this.pendingBytes = bytes.length - offset;
        }
    }
    hex() {
        if (this.finished)
        ...
        const bits = BigInt(this.totalBytes) * 8n, view = new DataView(tail.buffer);
        view.setUint32(tail.length - 8, Number((bits >> 32n) & 0xffffffffn));
        view.setUint32(tail.length - 4, Number(bits & 0xffffffffn));
        for (let offset = 0; offset < tail.length; offset += 64)
            compressFlowVisualSha256(this.state, tail.subarray(offset, offset + 64));
        return Array.from(this.state, value => value.toString(16).padStart(8, "0")).join("");
    }
}
//# sourceMappingURL=flow-visual-sha256.js.map