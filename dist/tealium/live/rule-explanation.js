const show = (value) => value === undefined ? 'undefined' : JSON.stringify(value) ?? String(value);
const all = (steps) => steps.some(step => step.result === false) ? false :
    steps.every(step => step.result === true) ? true : null;
const any = (steps) => steps.some(step => step.result === true) ? true :
...
function definedValue(row, key) {
    if (row.dataKeys.includes(key))
        return true;
    return row.dataComplete ? false : null;
}
function conditionStep(condition, row) {
    const data = row.data;
    ...
    if (operator === 'is_defined')
        result = definedValue(row, key);
    else if (operator === 'is_not_defined') {
        const exists = definedValue(row, key);
        result = exists === null ? null : !exists;
    }
        ...
        if (operator.includes('ignore_case')) {
            ...
            if (operator.startsWith('equals'))
                result = a === b;
            if (operator.startsWith('does_not_equal'))
                result = a !== b;
            if (operator.startsWith('contains'))
                result = a.includes(b);
            if (operator.startsWith('does_not_contain'))
                result = !a.includes(b);
        }
...
function split(source, symbol) {
    ...
    for (let i = 0; i < source.length; i++) {
        ...
        if (char === ')' || char === ']')
            depth--;
        if (!depth && source.slice(i, i + 2) === symbol) {
...
function codeStep(source, row) {
    const data = row.data;
    const value = unwrap(source);
    for (const [symbol, label, combine] of [['||', 'Any of', any], ['&&', 'All of', all]]) {
        const parts = split(value, symbol);
        if (parts.length > 1) {
            const children = parts.map(part => codeStep(part, row));
            return { label, result: combine(children), children };
        }
    }
    ...
    if (comparison) {
        ...
        const left = ignoreCase ? String(actual).toLowerCase() : String(actual);
        const right = ignoreCase ? expected.toLowerCase() : expected;
        let result = actual === undefined ? null : null;
        if (actual !== undefined) {
            if (op === '==' || op === '===')
                result = left === right;
            if (op === '!=' || op === '!==')
...
export function explainRule(row) {
    if (row.conditions?.length) {
        const groups = row.conditions.map(group => {
            const children = group.map(item => conditionStep(item, row));
...
//# sourceMappingURL=rule-explanation.js.map