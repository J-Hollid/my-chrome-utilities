            ...
            try {
                ...
                if (!environment && identity) {
                    try {
                        ...
                        if (/^https?:$/.test(path.protocol) && parts?.[1] === account && parts[2] === profileName)
                            environment = parts[3];
                    }
                ...
                try {
                    ...
                    if (object(rawData))
                        for (const key of dataKeys.slice(0, 300)) {
                            try {
                    ...
                    if (object(conditions))
                        for (const id of Object.keys(conditions).filter(key => /^\d+$/.test(key)).slice(0, 200)) {
                            try {
                ...
                for (const uid of ids) {
                    ...
                    let assigned;
                    try {
                        assigned = own(tag, 'loadrule') ?? own(tag, 'loadRule');
                    }
                    catch {
                        assigned = null;
                    }
                    ...
                    try {
                        ...
                        if (Array.isArray(extensions)) {
                            ...
                            if (values.every(value => typeof value === 'function'))
                                extensionSources = values.map(value => Function.prototype.toString.call(value));
                        }
                    }
