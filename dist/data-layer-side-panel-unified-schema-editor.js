...
export function compactCanonicalCommandPolicy(kind, semanticSavePending) {
...
const valuesWithStableIds = (current, next, id) => {
...
const remapPredicateProperties = (predicate, propertyId) => predicate.kind === "predicate" ? { ...predicate, propertyId: propertyId(predicate.propertyId) } : { ...predicate, children: predicate.children.map((child) => remapPredicateProperties(child, propertyId)) };
const predicateWithoutIds = (predicate) => { const { id: _id, ...value } = predicate; return predicate.kind === "predicate" ? value : { ...value, children: predicate.children.map(predicateWithoutIds) }; };
const stablePredicateIds = (prior, next) => {
    if (prior.kind !== next.kind)
        return next;
    if (next.kind === "predicate")
        return { ...next, ...(prior.id ? { id: prior.id } : {}) };
    const priorGroup = prior;
    return { ...next, ...(priorGroup.id ? { id: priorGroup.id } : {}), children: next.children.map((child, index) => priorGroup.children[index] ? stablePredicateIds(priorGroup.children[index], child) : child) };
};
const comparableRule = (rule) => { const { id: _id, name: _name, revision: _revision, provenance: _provenance, reusableRuleId: _reusableRuleId, reusableOutcome: _reusableOutcome, replacesRuleId: _replacesRuleId, enforcement: _enforcement, arrayScope: _arrayScope, condition, ...semantic } = rule; return { ...semantic, ...(condition ? { condition: predicateWithoutIds(condition) } : {}) }; };
const rulesWithStableConditions = (current, next, propertyId) => {
    const claimed = new Set();
    return next.map((rule) => {
        ...
        if (prior)
            claimed.add(prior.id);
        if (prior && same(comparableRule(prior), comparableRule(stable)))
            return clone(prior);
        if (!prior)
            return stable;
        const merged = { ...prior, ...stable };
        if (prior.reusableRuleId === undefined && stable.reusableRuleId === stable.id)
            delete merged.reusableRuleId;
        if (prior.provenance)
            merged.provenance = clone(prior.provenance);
        if (prior.condition && !stable.condition)
            merged.condition = clone(prior.condition);
        else if (prior.condition && stable.condition)
            merged.condition = stablePredicateIds(prior.condition, stable.condition);
        return merged;
    });
};
