...
const workbookPrototype = (workbook, expectedKind) => {
    ...
    for (const finding of geometry.findings)
        findings.push({ location: finding.cell ? `Template ${finding.cell}` : `named area ${finding.area ?? "setup"}`, message: finding.repair ? `${finding.message} ${finding.repair}` : finding.message });
