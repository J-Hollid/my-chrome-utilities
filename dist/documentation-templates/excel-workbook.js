...
import { parseExcelTemplateDirective, validateExcelTemplatePrototype } from "./excel-template.js";
...
const commentDirectives = (texts) => [...texts].filter(([name]) => /xl\/comments\d*\.xml$/u.test(name)).flatMap(([, text]) => [...text.matchAll(/<comment\b([^>]*)>([\s\S]*?)<\/comment>/gu)].flatMap(match => { const ref = attributes(match[1]).ref, value = xmlText(match[2]); return ref && value.startsWith("tw:") ? [{ cell: ref, text: value }] : []; }));
const worksheetCells = (text, shared) => [...text.matchAll(/<c\b([^>]*)>([\s\S]*?)<\/c>/gu)].flatMap(match => { const attrs = attributes(match[1]), cell = attrs.r; if (!cell)
    return []; const body = match[2], raw = attrs.t === "s" ? shared[Number(/<v>([^<]*)<\/v>/u.exec(body)?.[1] ?? -1)] ?? "" : xmlText(body); return raw.includes("{{") ? [{ address: cell, value: raw }] : []; });
export async function validateExcelTemplateWorkbook(source, expectedKind) {
    const findings = [];
    if (source.size > 10 * 1024 * 1024)
        return { valid: false, findings: [{ location: "workbook", message: "The Excel template is too large." }], entryCount: 0, unpackedBytes: 0 };
    let entries;
    try {
        entries = await directory(source);
    }
    catch (error) {
        return { valid: false, findings: [{ location: "workbook", message: error instanceof Error ? error.message : String(error) }], entryCount: 0, unpackedBytes: 0 };
    }
    ...
    const workbook = texts.get("xl/workbook.xml") ?? "", sheetCount = (workbook.match(/<sheet\b/gu) ?? []).length;
    if (sheetCount !== 1)
        findings.push({ location: "xl/workbook.xml", message: "Keep exactly one prototype worksheet." });
    const directives = commentDirectives(texts), templateDirectives = directives.flatMap(source => { try {
        const parsed = parseExcelTemplateDirective(source.text);
        return parsed.kind === "template" ? [parsed] : [];
    }
    catch (error) {
        findings.push({ location: `worksheet ${source.cell}`, message: error instanceof Error ? error.message : String(error) });
        return [];
    } }), markers = templateDirectives.map(({ templateKind }) => templateKind);
    if (markers.length !== 1)
        findings.push({ location: "worksheet Note", message: `Declare one ${expectedKind ? expectedKind[0].toUpperCase() + expectedKind.slice(1) : "documentation"} contract-version-1 template.` });
    const kind = markers[0];
    if (kind && expectedKind && kind !== expectedKind)
        findings.push({ location: "worksheet Note", message: `The workbook declares ${kind}, not ${expectedKind}.` });
    const worksheetEntry = [...texts.keys()].find(name => /^xl\/worksheets\/sheet[^/]*\.xml$/u.test(name)), worksheet = worksheetEntry ? texts.get(worksheetEntry) ?? "" : "", sharedText = texts.get("xl/sharedStrings.xml") ?? "", shared = [...sharedText.matchAll(/<si\b[^>]*>([\s\S]*?)<\/si>/gu)].map(match => xmlText(match[1])), cells = worksheetCells(worksheet, shared), merges = [...worksheet.matchAll(/<mergeCell\b[^>]*ref=["']([^"']+)["'][^>]*\/?>(?:<\/mergeCell>)?/gu)].map(match => match[1]);
    if (kind) {
        const prototype = { kind, contractVersion: 1, worksheetName: "Prototype", cells, directives, merges }, geometry = validateExcelTemplatePrototype(prototype);
        for (const finding of geometry.findings)
            findings.push({ location: `worksheet ${finding.cell ?? "geometry"}`, message: finding.message });
    }
    return { valid: findings.length === 0, ...(kind ? { kind, contractVersion: 1 } : {}), findings, entryCount: entries.length, unpackedBytes: entries.reduce((sum, { size }) => sum + size, 0) };
}
