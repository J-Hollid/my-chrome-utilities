...
import { excelTemplateGuideFor, imageLayoutInArea, parseExcelAreaProperties, renderExcelTemplateGrid } from "./excel-template.js";
...
const starterPrototype = (kind) => {
    if (kind === "overview")
        return { kind, contractVersion: 3, worksheetName: "Template", cells: [{ address: "A1", value: "{{section.name}}" }, { address: "A3", value: "{{field.label}}" }, { address: "B3", value: "{{field.value}}" }], areas: [{ name: "FieldRow", type: "repeat", source: "overview.fields", direction: "down", range: "A3:B3" }], merges: [] };
    if (kind === "flow")
        return { kind, contractVersion: 3, worksheetName: "Template", cells: [{ address: "A1", value: "{{section.name}}" }, { address: "A3", value: "{{page.stepLabel}}" }, { address: "B3", value: "{{page.pageName}}" }, { address: "A5", value: "{{event.eventName}}" }], areas: [{ name: "PageCard", type: "repeat", source: "flow.pages", direction: "across", range: "A3:D8" }, { name: "EventRow", type: "repeat", source: "page.events", direction: "down", range: "A5:B5" }, { name: "PageVisual", type: "image", source: "page.visual.image", range: "C5:D7", properties: parseExcelAreaProperties("image", "") }, { name: "ThemeLogo", type: "image", source: "theme.logo", range: "C1:D2", properties: parseExcelAreaProperties("image", "") }], merges: [] };
    if (kind === "matrix")
        return { kind, contractVersion: 3, worksheetName: "Template", cells: [{ address: "A1", value: "{{section.name}}" }, { address: "A3", value: "{{row.property}}" }, { address: "B3", value: "{{cell.value}}" }], areas: [{ name: "RowPattern", type: "repeat", source: "matrix.rows", direction: "down", range: "A3:C4" }, { name: "CellPattern", type: "repeat", source: "row.cells", direction: "across", range: "B3:C3" }], merges: [] };
    return { kind, contractVersion: 3, worksheetName: "Template", cells: [{ address: "A1", value: "{{section.name}}" }, { address: "A3", value: "{{concept.name}}" }, { address: "A4", value: "{{row.property}}" }, { address: "B4", value: "{{cell.value}}" }], areas: [{ name: "ConceptPattern", type: "repeat", source: "profile.concepts", direction: "down", range: "A3:D6" }, { name: "RowPattern", type: "repeat", source: "concept.rows", direction: "down", range: "A4:D4" }, { name: "CellPattern", type: "repeat", source: "row.cells", direction: "across", range: "B4:C4" }], merges: [] };
};
const absoluteNamedRange = (range) => `'Template'!${range.split(":").map(cell => `$${cell.replace(/([A-Z]+)(\d+)/u, "$1$$$2")}`).join(":")}`;
export async function writeDocumentationTemplateStarter(kind) { const workbook = new (excelJs().Workbook)(), prototype = starterPrototype(kind), worksheet = workbook.addWorksheet("Template"), guide = workbook.addWorksheet("Template Guide"), catalogue = excelTemplateGuideFor(kind); for (const cell of prototype.cells) {
    ...
    workbook.definedNames.add(absoluteNamedRange(area.range), area.name); guide.getCell("A1").value = "Guided Excel template"; guide.getCell("A2").value = "Use exact {{value.paths}} for single values. Contract 3 Properties combine Image layout, Repeat separators, and one optional finite Output background."; guide.getCell("A3").value = "Properties keys: Image fit scale-down or contain; position left/center/right plus top/center/bottom or percentages; padding one to four px values. Repeat separator-area names a complete trailing edge. Output accepts only background-fill: #RRGGBB with blank Source and Direction; keep it finite and within 250000 generated cells. Blank keeps Contract 2 defaults."; guide.addTable?.({ name: "TemplateSettings", ref: "A5", headerRow: true, columns: [{ name: "Setting" }, { name: "Value" }], rows: [["Contract", 3], ["Kind", kind]] }); guide.addTable?.({ name: "TemplateAreas", ref: "A10", headerRow: true, columns: [{ name: "Area" }, { name: "Type" }, { name: "Source" }, { name: "Direction" }, { name: "Properties" }], rows: prototype.areas.map(area => [area.name, area.type === "repeat" ? "Repeat" : "Image", area.source, area.type === "repeat" ? (area.direction === "across" ? "Across" : "Down") : "", ""]) }); guide.getCell("G1").value = "Single values"; catalogue.values.forEach((entry, index) => { guide.getCell(index + 2, 7).value = `${entry.placeholder} — ${entry.meaning} — example ${entry.example} — ${entry.available}`; }); const collectionStart = catalogue.values.length + 3; guide.getCell(collectionStart, 7).value = "Repeatable data"; catalogue.collections.forEach((entry, index) => { guide.getCell(collectionStart + index + 1, 7).value = `${entry.path} — ${entry.meaning}; item prefix ${entry.itemPrefix}; fields ${entry.fields.join(", ") || "none"}; nested ${entry.nestedCollections.join(", ") || "none"}; ${entry.directions.join(" or ")}; ${entry.emptyResult}; ${entry.copyBehavior}; Template Guide example ${entry.example}`; }); const examplesStart = collectionStart + catalogue.collections.length + 2; guide.getCell(examplesStart, 7).value = "Copyable TemplateAreas examples"; catalogue.areaExamples.forEach((entry, index) => { guide.getCell(examplesStart + index + 1, 7).value = `${entry.area} | ${entry.type} | ${entry.source} | ${entry.direction} | ${entry.range} | parent ${entry.parent ?? "none"}`; }); guide.getCell("H1").value = "Contract 3 Properties grammar and copyable examples"; catalogue.propertyGuidance.forEach((entry, index) => { guide.getCell(index + 2, 8).value = entry; }); const contract3Start = catalogue.propertyGuidance.length + 3; guide.getCell(contract3Start, 8).value = "Copyable Contract 3 TemplateAreas rows"; catalogue.propertyExamples.forEach((entry, index) => { guide.getCell(contract3Start + index + 1, 8).value = `${entry.area} | ${entry.type} | ${entry.source} | ${entry.direction} | ${entry.properties} | named range ${entry.range}`; }); guide.getColumn(7).width = 120; guide.getColumn(8).width = 120; return new Uint8Array(await workbook.xlsx.writeBuffer()); }
const tableRows = (sheet, name) => { const table = sheet.getTable?.(name).table; if (table?.rows)
    return table.rows; const ref = /^([A-Z]+)(\d+):([A-Z]+)(\d+)$/u.exec(table?.tableRef ?? ""); if (!ref)
    return []; const columnNumber = (letters) => [...letters].reduce((value, letter) => value * 26 + letter.charCodeAt(0) - 64, 0), left = columnNumber(ref[1]), right = columnNumber(ref[3]); return Array.from({ length: Number(ref[4]) - Number(ref[2]) }, (_, rowOffset) => Array.from({ length: right - left + 1 }, (_, columnOffset) => sheet.getCell(Number(ref[2]) + 1 + rowOffset, left + columnOffset).value)); };
function prototypeFromWorkbook(workbook, kind) {
    ...
    const named = new Map(workbook.definedNames.model.map(item => [item.name.toLocaleLowerCase("en-US"), item.ranges[0]?.replace(/^'?Template'?!/u, "").replaceAll("$", "")])), settings = new Map(tableRows(guide, "TemplateSettings").map(row => [String(row[0] ?? ""), String(row[1] ?? "")])), contractVersion = settings.get("Contract") === "3" ? 3 : 2, areas = tableRows(guide, "TemplateAreas").flatMap(row => { const name = String(row[0] ?? ""), type = String(row[1] ?? "").toLowerCase(), source = String(row[2] ?? ""), direction = String(row[3] ?? "").toLowerCase(), rawProperties = contractVersion === 3 ? String(row[4] ?? "") : "", range = named.get(name.toLocaleLowerCase("en-US")); if (!range)
        return []; if (type === "repeat" && (direction === "across" || direction === "down")) {
        const parsed = parseExcelAreaProperties("repeat", rawProperties), separatorRange = parsed.separatorAreaName ? named.get(parsed.separatorAreaName.toLocaleLowerCase("en-US")) : undefined;
        return [{ name, type: "repeat", source, direction, range, ...(parsed.separatorAreaName && separatorRange ? { properties: { separatorArea: { name: parsed.separatorAreaName, range: separatorRange } } } : {}) }];
    } if (type === "output" && !source && !direction)
        return [{ name, type: "output", source: "", range, properties: parseExcelAreaProperties("output", rawProperties) }]; return type === "image" && (source === "theme.logo" || source === "page.visual.image") ? [{ name, type: "image", source, range, ...(contractVersion === 3 ? { properties: parseExcelAreaProperties("image", rawProperties) } : {}) }] : []; });
    ...
    return { kind, contractVersion, worksheetName: "Template", cells: [...cells.values()], areas, merges: [...(worksheet.model?.merges ?? [])] };
}
...
const generatedImageRange = (target, layout) => ({ tl: { nativeRow: target.row - 1, nativeCol: target.column - 1, nativeRowOff: Math.round(layout.top * 9525), nativeColOff: Math.round(layout.left * 9525) }, ext: { width: layout.width, height: layout.height }, editAs: "oneCell" });
async function copyImages(source, sourceSheet, targetBook, targetSheet, rendered, prototype, context, convertWebpToPng, imageCache) {
    ...
    for (const area of prototype.areas) {
        ...
        if (area.source === "theme.logo") {
            ...
            if (!image || !intrinsic)
                continue;
            const imageId = targetBook.addImage(image), layout = imageLayoutInArea(bounds, intrinsic, area.properties);
            for (const target of targets)
                targetSheet.addImage(imageId, generatedImageRange(target, layout));
            continue;
        }
        ...
        for (const [index, target] of targets.entries()) {
            ...
            if (!image)
                continue;
            const layout = imageLayoutInArea(bounds, intrinsic, area.properties);
            targetSheet.addImage(targetBook.addImage(image), generatedImageRange(target, layout));
        }
    }
}
async function renderCustomInto(output, body, snapshot, table, worksheetName, convertWebpToPng, imageCache) { const section = snapshot.set.sections.find(({ id }) => id === table.id); const validation = await validateExcelTemplateWorkbook(body, section.kind); if (!validation.valid)
    ...
    for (let row = first.row; row <= last.row; row += 1)
        for (let column = first.column; column <= last.column; column += 1) {
            ...
            if (!cell.fill || cell.fill.type === undefined || cell.fill.type === "pattern" && (cell.fill.pattern === undefined || cell.fill.pattern === "none"))
                cell.fill = fill;
        }
