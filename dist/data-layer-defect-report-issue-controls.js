...
export function appendIssueControls(issues, expectedControls, state, selectedChoices) {
    for (const reportIssue of state.report().issues) {
        ...
        for (const value of assistance.schemaValues) {
            ...
            schemaValue.name = `defect-response-${reportIssue.id}`;
            schemaValue.value = String(value);
        ...
        if (/forbidden/i.test(reportIssue.constraint)) {
            ...
            rule.type = "radio";
            rule.name = `defect-response-${reportIssue.id}`;
            rule.value = "remove";
            ...
            ruleLabel.append(rule, "Remove property — validation rule");
            group.append(ruleLabel);
        }
        const customLabel = document.createElement("label");
        const custom = document.createElement("input");
        custom.type = "radio";
        custom.name = `defect-response-${reportIssue.id}`;
        custom.dataset.responseSource = "Custom value or response";
        const response = document.createElement("input");
        response.placeholder = "Custom value or response";
        response.hidden = true;
        const warning = document.createElement("output");
        warning.dataset.customResponseWarning = reportIssue.id;
        warning.setAttribute("aria-live", "polite");
        const keep = document.createElement("button");
        keep.type = "button";
        keep.textContent = "Keep custom override";
        keep.hidden = true;
        const replace = document.createElement("button");
        replace.type = "button";
        replace.textContent = "Replace custom override";
        replace.hidden = true;
        hideCustomResponse = () => {
            response.hidden = true;
            warning.textContent = "";
        ...
        const exampleType = reportIssue.example
            ? reportIssue.example.value === null ? "null" : typeof reportIssue.example.value
            : undefined;
        const responseValue = () => exampleType ? exampleValueFromInput(response.value, exampleType)?.value : response.value;
        ...
        custom.addEventListener("change", () => {
            ...
            if (!customInitialized && reportIssue.example) {
                response.value = String(reportIssue.example.value);
        ...
        replace.addEventListener("click", () => {
            selectedChoices.delete(reportIssue.id);
            response.value = "";
            warning.textContent = "Enter a replacement value.";
            keep.hidden = true;
            replace.hidden = true;
            response.focus({ preventScroll: true });
            state.refresh();
...
//# sourceMappingURL=data-layer-defect-report-issue-controls.js.map