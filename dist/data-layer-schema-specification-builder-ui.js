...
export function renderSchemaSpecificationBuilder(root, schema, close, writeClipboard) {
    root.replaceChildren();
    const heading = document.createElement("h4");
    heading.textContent = `Build specification · ${schema.name}`;
    const source = document.createElement("output");
    source.textContent = schema.workingDraft ? `Source: working draft based on revision ${schema.version}` : `Source: published revision ${schema.version}`;
    ...
    summary.textContent = `${rows.length} selected properties`;
    ...
    labels.forEach((label) => { const th = document.createElement("th"); th.textContent = label; tr.append(th); });
    head.append(tr);
    const body = table.querySelector("tbody");
    rows.forEach((row) => { const tr = document.createElement("tr"); [row.propertyName, row.description, row.mandatory, row.type, row.example ?? "", row.allowedValues.join(" | ")].forEach((value) => { const td = document.createElement("td"); td.textContent = value; tr.append(td); }); body.append(tr); });
