...
export function renderSchemaSpecificationBuilder(root, current, allSchemas, initialSurfaceKey, close, clipboardPort) {
    ...
    const renderPreview = () => {
        ...
        headRow.replaceChildren(...columns.map((column) => {
            ...
            cell.append(renderReorderControl({ focusScopeId: "schema-specification-columns", itemId: column, itemLabel: specificationColumnLabels[column], completeOrder: columns.map(id => ({ id, label: specificationColumnLabels[id] })), dropTarget: cell, preserveTargetSemantics: true, onMove: ({ itemId, toIndex }) => { columns = reorderValues(columns, itemId, toIndex, value => value); renderPreview(); return true; } }));
            return cell;
        }));
