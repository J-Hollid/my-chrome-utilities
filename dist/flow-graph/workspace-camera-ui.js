...
export function installFlowCamera(options) {
    ...
    const apply = (camera) => {
        ...
        canvas.querySelectorAll("[data-flow-visual-thumbnail]").forEach((item) => item.style.setProperty("display", thumbnailPixels ? "block" : "none", "important"));
        canvas.querySelectorAll("[data-flow-visual-fallback]").forEach((item) => item.style.setProperty("display", thumbnailPixels ? "none" : "", "important"));
