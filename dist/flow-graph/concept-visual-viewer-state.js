...
const clamp = (value, minimum, maximum) => { const clamped = Math.max(minimum, Math.min(maximum, value)); return clamped === 0 ? 0 : clamped; };
export const clampedFlowConceptVisualViewerState = ({ raster, viewport }, state) => { const scale = clamp(state.scale, flowConceptVisualFitScale(raster, viewport), FLOW_CONCEPT_VISUAL_MAX_VIEWER_SCALE), limit = flowConceptVisualViewerPanLimit({ raster, viewport }, scale); return { scale, x: clamp(state.x, -limit.x, limit.x), y: clamp(state.y, -limit.y, limit.y) }; };
