...
export function flowConceptVisualThumbnailViewport(itemWidth) {
...
const activateVisualIndicator = (indicator, activate) => {
    ...
    indicator.addEventListener("keydown", event => { if (event.key !== "Enter" && event.key !== " ")
        return; event.preventDefault(); event.stopPropagation(); activate(); });
};
