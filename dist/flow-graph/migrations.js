import { graphIndex, legacyBindingOccurrence, relationshipEndpoint, relationshipPorts, saveStoredGraph, storedGraph } from "../data-layer-flow-graph.js";
import { transactProject } from "../utilities/data-layer/schemas.js";
export function reviewLegacyFlowContextMigration(project, flowId) { void flowId; const items = [], blockers = []; for (const [candidateFlowId, graph] of Object.entries(graphIndex(project))) {
...
export function migrateLegacyFlowRelationshipKinds(state, flowId) { const graph = storedGraph(state.project, flowId); if (!graph.relationships.some((relationship) => relationship.kind === "parallel"))
