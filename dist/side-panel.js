...
const schemaAssignmentPolicy = document.querySelector("#schema-assignment-policy");
...
const closeSchemaEditorReview = document.querySelector("#close-schema-editor-review");
const closeSchemaEditorReviewHeading = document.querySelector("#close-schema-editor-review-heading");
const closeSchemaEditorReviewSummary = document.querySelector("#close-schema-editor-review-summary");
...
function renderSchemas() {
    ...
    if (schemaList)
        schemaList.replaceChildren(...visible.map((schema) => {
            ...
            const updateRules = document.createElement("button");
            ...
            revise.type = updateRules.type = duplicate.type = remove.type = "button";
            revise.textContent = "Edit as new version";
            updateRules.textContent = "Update pinned rules";
            ...
            updateRules.addEventListener("click", () => {
                ...
                if (updated.every((attachment, index) => attachment.ruleId === attachments[index]?.ruleId && attachment.version === attachments[index]?.version)) {
                    if (schemaResult)
                        schemaResult.textContent = `${schema.name} already uses the latest pinned rules.`;
                    return;
                }
            ...
            remove.addEventListener("click", () => { if (!globalThis.confirm(`Delete ${schema.name} version ${schema.version}? Validation history remains available.`))
                return; schemas = schemas.filter(({ id }) => id !== schema.id); persistSchemaLibrary(); renderSchemas(); });
            item.append(revise, updateRules, duplicate, remove);
            return item;
        }));
...
function renderSchemaRules() {
    ...
    schemaRuleList?.replaceChildren(...visible.map((rule) => {
        ...
        remove.addEventListener("click", () => {
            ...
            if (pinnedBy.length > 0) {
                if (schemaResult)
                    schemaResult.textContent = `Cannot delete ${rule.name} v${rule.version}; pinned by ${pinnedBy.map((schema) => `${schema.name} v${schema.version}`).join(", ")}.`;
                return;
            }
            if (!globalThis.confirm(`Delete ${rule.name}?`))
                return;
    ...
    if (schemaRuleCount)
        schemaRuleCount.textContent = `${visible.length} rules`;
}
function renderSchemaAssignments() {
...
function renderSchemaDraft() {
    ...
    if (schemaAttachedRules)
        schemaAttachedRules.textContent = attachments.length === 0 ? "No pinned reusable rules" : `Pinned rules: ${attachments.map((attachment) => `${reusableRules.find((rule) => rule.id === attachment.ruleId && rule.version === attachment.version)?.name ?? attachment.ruleId} v${attachment.version}`).join(", ")}`;
...
function defineSchemaProperty(document, path, property, required) {
    const [name, ...rest] = path;
    if (!name)
        return document;
    const properties = document.properties ?? {};
    if (rest.length === 0)
        return { ...document, type: document.type ?? "object", ...(required ? { required: [...new Set([...(document.required ?? []), name])] } : {}), properties: { ...properties, [name]: property } };
...
addSchemaRuleButton?.addEventListener("click", () => {
    ...
    if (selectedRule && applicable.length > 0 && !applicable.includes("any") && property.type && !applicable.includes(property.type)) {
        if (schemaResult)
            schemaResult.textContent = `${selectedRule.name} does not apply to ${property.type} at ${path.join(".")}.`;
        return;
    }
...
saveSchemaButton?.addEventListener("click", () => {
    if (!schemaDraft || saveSchemaButton.disabled)
        return;
    if (!globalThis.confirm(`Save ${schemaDraft.name} version 1?`))
        return;
...
saveAndCloseSchemaButton?.addEventListener("click", () => {
    if (!schemaDraft || !schemaDraft.name.trim() || Object.keys(schemaDraft.document.properties ?? {}).length === 0)
        return;
    const saved = { ...schemaDraft, id: `schema:${schemaDraft.name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}:1`, assignments: [] };
    schemaHistory[saved.name] = [...new Set([...(schemaHistory[saved.name] ?? []), saved.version])].sort((left, right) => left - right);
    schemas = [...schemas, saved];
    schemaDraft = undefined;
    persistSchemaLibrary();
    hideDialog(closeSchemaEditorReview);
    renderSchemaDraft();
    renderSchemas();
    if (schemaResult)
        schemaResult.textContent = `Saved ${saved.name} version 1.`;
...
saveSchemaAssignmentButton?.addEventListener("click", () => {
    const schema = schemas.find((candidate) => candidate.id === schemaAssignmentSchema?.value);
    const sourceId = schemaAssignmentSource?.value.trim();
    const eventName = schemaAssignmentEvent?.value.trim();
    const target = schemaAssignmentTarget?.value === "raw input" ? "raw input" : schemaAssignmentTarget?.value === "payload" ? "payload" : undefined;
