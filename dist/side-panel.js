...
function openCompactCanonicalPropertyActions(path, trigger) {
    ...
    let ownershipSession = { inherited: !focusedStructureOwned(original) || ["inherited", "overridden"].includes(focusedSourceState(original)), local: ["local", "overridden"].includes(focusedSourceState(original)), structureOwned: focusedStructureOwned(original), invariant: original.enforcement === "invariant", activated: [] };
