...
const patternDomain = (pattern) => /\[a-z\]/iu.test(pattern) ? "letters" : /\[0-9\]|\\d/u.test(pattern) ? "digits" : undefined;
const conflictingPeerRulePair = (source, local, kind) => { const sourceRules = peerRules(source, kind), localRules = peerRules(local, kind), conflicts = (sourceRule, localRule) => { if (kind === "pattern") {
    const sourceDomain = typeof sourceRule.pattern === "string" ? patternDomain(sourceRule.pattern) : undefined, localDomain = typeof localRule.pattern === "string" ? patternDomain(localRule.pattern) : undefined;
    return Boolean(sourceDomain && localDomain && sourceDomain !== localDomain);
} if (kind === "range")
    return typeof sourceRule.maximum === "number" && typeof localRule.minimum === "number" && localRule.minimum > sourceRule.maximum || typeof sourceRule.minimum === "number" && typeof localRule.maximum === "number" && localRule.maximum < sourceRule.minimum; if (kind === "cardinality")
    return typeof sourceRule.maxItems === "number" && typeof localRule.minItems === "number" && localRule.minItems > sourceRule.maxItems || typeof sourceRule.minItems === "number" && typeof localRule.maxItems === "number" && localRule.maxItems < sourceRule.minItems; return false; }; for (const sourceRule of sourceRules)
    for (const localRule of localRules)
        if (conflicts(sourceRule, localRule))
            return { source: sourceRule, local: localRule }; return { ...(sourceRules[0] ? { source: sourceRules[0] } : {}), ...(localRules[0] ? { local: localRules[0] } : {}) }; };
...
const matchesIssue = (resolution, issue, constraint) => { const sourceFacet = issue.details.sourceFacet ?? issue.keys[0], localFacet = issue.details.localFacet ?? issue.keys[0], sourceId = issue.details.sourceContributorId, localId = issue.details.localContributorId, rejectsSource = resolution.rejectedContributorId === sourceId, rejectsLocal = resolution.rejectedContributorId === localId, rejectsAuthority = rejectsSource && (issue.details.sourceFacetProtected || issue.details.sourceRuleInvariant) || rejectsLocal && (issue.details.localFacetProtected || issue.details.localRuleInvariant); return !rejectsAuthority && ownsFacet(constraint, resolution.selectedFacet) && ((resolution.selectedContributorId === sourceId && resolution.selectedFacet === sourceFacet && rejectsLocal && resolution.rejectedFacet === localFacet) || (resolution.selectedContributorId === localId && resolution.selectedFacet === localFacet && rejectsSource && resolution.rejectedFacet === sourceFacet)); };
...
const peerFacetIssues = (left, right) => {
    const issues = [], details = (facet, section, sourceValue, localValue, message, kind, sourceFacet, localFacet) => { const sourceAuthority = authorityConstraint(left), localAuthority = authorityConstraint(right), pair = kind ? conflictingPeerRulePair(sourceAuthority, localAuthority, kind) : {}, sourceRule = pair.source, localRule = pair.local; return { facet, section, sourceContributor: left.contributor.name, sourceContributorId: left.contributor.id, sourceValue: clone(sourceValue), ...(sourceFacet ? { sourceFacet } : {}), ...(protectedFacet(sourceAuthority, sourceFacet) ? { sourceFacetProtected: true } : {}), localContributor: right.contributor.name, localContributorId: right.contributor.id, localValue: clone(localValue), ...(localFacet ? { localFacet } : {}), ...(protectedFacet(localAuthority, localFacet) ? { localFacetProtected: true } : {}), contributorIds: [left.contributor.id, right.contributor.id], message, ...(sourceRule?.id ? { sourceRuleId: String(sourceRule.id) } : {}), ...(invariantFacet(sourceAuthority, sourceFacet, sourceRule) ? { sourceRuleInvariant: true } : {}), ...(localRule?.id ? { localRuleId: String(localRule.id) } : {}), ...(invariantFacet(localAuthority, localFacet, localRule) ? { localRuleInvariant: true } : {}) }; };
    ...
    if (leftConstraint.type && rightConstraint.type && leftConstraint.type !== rightConstraint.type)
        issues.push({ keys: ["type"], details: details("Type", "Definition", leftConstraint.type, rightConstraint.type, "the parallel definitions use incompatible Types", undefined, "type", "type") });
    if (leftConstraint.presence && rightConstraint.presence && new Set([leftConstraint.presence, rightConstraint.presence]).has("required") && new Set([leftConstraint.presence, rightConstraint.presence]).has("forbidden"))
        issues.push({ keys: ["presence"], details: details("Presence", "Definition", leftConstraint.presence, rightConstraint.presence, "the parallel definitions require and exclude the same property", undefined, "presence", "presence") });
    if (leftConstraint.allowedValues?.length && rightConstraint.allowedValues?.length && !leftConstraint.allowedValues.some((value) => rightConstraint.allowedValues.some((candidate) => same(value, candidate))))
        issues.push({ keys: ["allowedValues"], details: details("Allowed values", "Definition", leftConstraint.allowedValues, rightConstraint.allowedValues, "the available choices do not overlap", undefined, "allowedValues", "allowedValues") });
    if (leftConstraint.expectedValue !== undefined && rightConstraint.expectedValue !== undefined && !same(leftConstraint.expectedValue, rightConstraint.expectedValue))
        issues.push({ keys: ["expectedValue"], details: details("Expected value", "Definition", leftConstraint.expectedValue, rightConstraint.expectedValue, "the parallel expected values differ", undefined, "expectedValue", "expectedValue") });
    if (leftConstraint.patterns?.length && rightConstraint.patterns?.length && leftConstraint.patterns.some((leftPattern) => rightConstraint.patterns.some((rightPattern) => { const leftDomain = patternDomain(leftPattern), rightDomain = patternDomain(rightPattern); return Boolean(leftDomain && rightDomain && leftDomain !== rightDomain); })))
        issues.push({ keys: ["patterns"], details: details("Pattern rule", "Rules", leftConstraint.patterns, rightConstraint.patterns, "the rules cannot both match", "pattern", "patterns", "patterns") });
    ...
    if (leftConstraint.itemSchema && rightConstraint.itemSchema && !same(leftConstraint.itemSchema, rightConstraint.itemSchema))
        issues.push({ keys: ["itemSchema"], details: details("Array item definition", "Structure", leftConstraint.itemSchema, rightConstraint.itemSchema, "the parallel item definitions are incompatible", undefined, "itemSchema", "itemSchema") });
...
export function compileLayeredSchema(contributors, context) {
    ...
    for (const [key, entries] of peerEntries) {
        const peerGroup = key.split("\u0000")[0], policyConflict = conflictingPolicyGroups.has(peerGroup), algebra = entries.map(({ contributor, constraint }) => ({ contributor, constraint: peerConstraintForCompile(constraint), authorityConstraint: constraint })), facetIssues = algebra.flatMap((left, leftIndex) => algebra.slice(leftIndex + 1).flatMap((right) => peerFacetIssues(left, right)));
        let incompatible = policyConflict || facetIssues.length > 0 || peerSetMismatch(algebra.map(({ constraint }) => constraint));
