...
export function pageGroupMembers(project, pageGroupId) {
    const legacyIds = project.collections.propertySets.find(({ id }) => id === pageGroupId)?.pageIds ?? [];
...
function writeMemberships(state, pageId, pageGroupIds, label) {
    return transactProject(state, label, (project) => ({ ...project, collections: { ...project.collections, pages: project.collections.pages.map((page) => page.id === pageId ? { ...page, pageGroupIds: [...pageGroupIds] } : page), propertySets: project.collections.propertySets.map((group) => { if (!Array.isArray(group.pageIds) || group.pageIds.every((id) => id !== pageId))
...
export function addPageGroupMembership(state, pageId, pageGroupId) {
    ...
    if (!group)
        throw new Error(`Unknown Property Set ${pageGroupId}.`);
...
export function inspectPageGroupMembershipMove(project, pageId, pageGroupId, delta) {
    const page = project.collections.pages.find(({ id }) => id === pageId);
    if (!page)
        throw new Error(`Unknown Page ${pageId}.`);
    const pageGroup = project.collections.propertySets.find(({ id }) => id === pageGroupId);
    if (!pageGroup)
        throw new Error(`Unknown Property Set ${pageGroupId}.`);
...
export function stagePageGroupMembershipMigration(project, pageId) {
    ...
    const known = new Set(project.collections.propertySets.map(({ id }) => id));
...
export function mountPageGroupMembershipEditor(host, options) {
    let pending;
    const doc = host.ownerDocument;
    const focusTrigger = (id) => queueMicrotask(() => host.querySelector(`[data-page-group-membership-id="${CSS.escape(id)}"] [data-reorder-trigger="true"]`)?.focus({ preventScroll: true }));
    const button = (label, action) => { const control = doc.createElement("button"); control.type = "button"; control.textContent = label; control.addEventListener("click", action); return control; };
    const render = () => {
        ...
        section.setAttribute("aria-label", "Page Group memberships");
        heading.textContent = "Page Group memberships";
        list.setAttribute("aria-label", `${page.name} Page Group membership stack`);
        ...
        if (pending) {
            ...
            queueMicrotask(() => { if (typeof dialog.showModal === "function")
                ...
                dialog.setAttribute("open", ""); confirm.focus(); });
        }
