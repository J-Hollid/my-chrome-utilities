...
const upgradeSeparatedProjectState = (state) => { let sequence = 0; if (state.draft)
    return upgradePageGroupsToPropertySets(state, (kind) => `${kind}:${state.project.id}:separation:${++sequence}`); const upgraded = upgradePageGroupsToPropertySets({ ...state, draft: { id: `draft:${state.project.id}:separation`, status: "Saved", updatedAt: "1970-01-01T00:00:00.000Z" } }, (kind) => `${kind}:${state.project.id}:separation:${++sequence}`), result = { ...upgraded }; delete result.draft; return result; };
const upgradeSeparatedState = (state) => { const upgraded = upgradeSeparatedProjectState(state), releases = state.project.releases.map((release) => { const project = { ...clone(state.project), collections: clone(release.snapshot), releases: [] }; delete project.currentRelease; const snapshotState = { project, draft: { id: `draft:${state.project.id}:release:${release.revision}:separation`, status: "Saved", updatedAt: "1970-01-01T00:00:00.000Z" }, history: { undo: [], redo: [] } }, snapshot = upgradeSeparatedProjectState(snapshotState).project.collections; return { ...clone(release), snapshot }; }); return { ...upgraded, project: { ...upgraded.project, releases } }; };
...
const externalLineageKeys = new Set(["sourceLineage", "externalLineage"]), immutableContentKeys = new Set(["richBlocks"]), identityMapContainers = new Set(["nodes", "documentationFlowGraphs", "labels"]), identityValueContainers = new Set(["templateAssignments"]);
...
function projectIdentityMapping(source, targetProjectId) {
    ...
    const collect = (value, external = false, parentKey = "") => { if (Array.isArray(value)) {
        ...
        const outside = external || externalLineageKeys.has(key) || immutableContentKeys.has(key);
...
function remapProjectReferences(value, mapping, external = false, parentKey = "") {
    ...
    if (!record(value))
        return value;
    return Object.fromEntries(Object.entries(value).map(([key, entry]) => { const outside = external || externalLineageKeys.has(key) || immutableContentKeys.has(key), mappedKey = !outside && identityMapContainers.has(parentKey) ? mapping.get(key) ?? key : key; if (typeof entry === "string" && !outside && (referenceField(key) || identityValueContainers.has(parentKey)))
...
export class DurableProjectRepository {
    ...
    async saveDraft(command) {
        ...
        const stores = ["projectMetadata", "projectRoots", "projectEntityMetadata", "projectEntities", "flowGraphs", "fixtures", "releases", "visualAssetMetadata", "visualAssetBodies", "changeFeed"], result = await this.backend.transaction(stores, "readwrite", async (transaction) => {
            ...
            for (const item of assetBodies) {
                await transaction.put("visualAssetBodies", item.key, { digest: item.identity.digest, body: item.body });
                changed.set(`visualAssetBodies/${item.key}`, { store: "visualAssetBodies", key: item.key });
            }
