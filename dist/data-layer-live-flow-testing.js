...
function frameEventName(state, frame) { return String(state.project.collections.pages.find(({ id }) => id === frame?.pageId)?.eventName ?? "pageview"); }
...
export function selectLiveFlow(run, state, flowId) {
    ...
    const frameEntities = new Map((graphs(state)[flowId]?.pageFrames ?? []).map((frame) => [frame.id, frame])), sections = new Map(current.sections.map((section) => [section.id, section.name]));
    const startChoices = current.pageFrames.map((frame) => { const page = state.project.collections.pages.find(({ id }) => id === frame.pageId), lane = sections.get(frame.sectionId ?? "") ?? "Outside Sections", storedName = frameEntities.get(frame.id)?.name, instanceName = storedName === `${page?.name} frame` ? page?.name : storedName; return { id: frame.id, name: instanceName || page?.name || frame.id, kind: "Page", pageId: frame.pageId, lane, stableFrameId: frame.id, recommended: !incoming.has(frame.id) }; }).sort((left, right) => Number(right.recommended) - Number(left.recommended));
...
export function liveFlowEventStepChoices(run, state, eventId) {
    ...
    const currentGraph = graphs(state)[run.flowId], contained = (currentGraph?.occurrences ?? [])
        ...
        .map((occurrence) => { const target = step(state, run.flowId, occurrence.id); return { id: target.id, name: target.name, stepKind: "Event", root: false, recorded: false, displayName: `${source.name} ${target.name}` }; });
