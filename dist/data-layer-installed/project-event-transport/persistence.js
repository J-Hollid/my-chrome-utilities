...
export function createInstalledTransportPersistence(ports) {
    async function commit(next) {
        ...
        if (!base || !envelope || base.project.id !== next.project.id)
            throw new Error("Open project");
    ...
    async function saveSources(projectId, sources) {
        await ports.durable.settled();
        ...
        if (result.status === "conflict")
            throw new Error("Observation sources changed in a newer Draft. Reload and retry.");
    ...
    const sources = {
        ...
        async load() {
            ...
            if (loaded.state.project.eventTransport?.observationSources === undefined) {
                ...
                if (loaded.state.project.eventTransport?.observationSources === undefined)
                    throw new Error("Observation source migration could not be verified");
            }
    ...
    return {
        ...
        async savePaths(paths) {
            const state = ports.currentProject();
            if (!state)
                throw new Error("Open project");
