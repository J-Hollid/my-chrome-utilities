...
export function createGuidedPublicOperations(ports, projections) {
    ...
    return { ...projections,
        ...
        liveValidationAvailable: (event) => {
            const schemas = ports.schemas(), manual = schemas.find(({ id }) => id === validation.manualSchemaId(event.id));
        ...
        validateLive: (event) => {
            ...
            const manual = schemas.find(({ id }) => id === validation.manualSchemaId(event.id));
