...
export class SchemaCanonicalContextTableView {
    ...
    renderContext() {
        ...
        if (c.pendingCommand) {
            ...
            const compareLatest = () => { c.reviewVisible = true; const base = c.pendingBase, latest = adapter.load(); c.commandFeedback = `Comparing command base revision ${base?.revision ?? "unknown"} with latest revision ${latest.revision}.`; rerender(); }, retryAction = () => c.retryCommand(), rejectAction = () => c.rejectCommand();
