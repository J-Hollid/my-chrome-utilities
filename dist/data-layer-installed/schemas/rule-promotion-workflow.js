...
export class SchemaRulePromotionWorkflow {
    ...
    restore(ruleId, rerender = true) {
        ...
        if (this.pending)
            this.focusReturn = { propertyPath: this.pending.propertyPath, ruleId: ruleId ?? this.pending.sourceRuleId, detailScroll: this.pending.detailScroll };
        ...
        restore();
        p.scheduleFrame(() => { restore(); p.scheduleFrame(() => { Array.from(p.root.querySelectorAll("button[data-rule-id]")).find(({ dataset }) => dataset.ruleId === focus.ruleId && dataset.propertyPath === focus.propertyPath)?.focus({ preventScroll: true }); restore(); p.detail?.removeEventListener("scroll", restore); }); });
    }
    open(propertyPath, sourceRuleId) {
        ...
        p.promotionDialog.open({ review, cancel: () => { if (this.pending?.generation === generation)
                ...
                return p.commitPromotion(result.schema.id, previousSchemas, previousRules, nextSchemas, nextRules).then(async () => { await p.settleCanonical?.(result.schema.id); const focus = () => Array.from(p.root.querySelectorAll("button[data-rule-id]")).find(({ dataset }) => dataset.ruleId === result.replacementRuleId && dataset.propertyPath === propertyPath)?.focus({ preventScroll: true }); p.scheduleFrame(() => p.scheduleFrame(focus)); return () => { if (this.pending?.generation === generation) {
                    p.result(`Promoted ${sourceRuleId} to reusable rule ${result.replacementRuleId}.`);
