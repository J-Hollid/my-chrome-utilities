...
export class SchemaRuleAttachmentWorkflow {
    ...
    requestSync(id) { const rule = this.ports.stored(id), p = this.ports.behavior(); if (!rule)
        return false; const review = this.review(rule); this.pendingSync = { rule: structuredClone(rule), review }; p.presentation.showSync(review); return true; }
    confirmSync() { const pending = this.pendingSync, p = this.ports.behavior(); if (!pending)
        return; const rule = this.ports.stored(pending.rule.id); if (!rule)
        throw new Error("The reusable rule was removed after review"); const review = this.review(rule); if (JSON.stringify(review) !== JSON.stringify(pending.review))
        throw new Error("The attached schemas changed after review"); p.replaceSchemas(publishReusableRuleSync(p.schemas(), rule, review)); this.pendingSync = undefined; p.persistLibrary(); p.renderAll(); p.presentation.close("sync"); }
