...
export class SchemaLibraryController {
    ...
    configure(behavior) { this.#operations = new SchemaLibraryOperations(this, behavior); }
    ...
    openImportFile() { this.#operations?.openImportFile(); }
    reviewImport(serialized) { this.#operations?.reviewImport(serialized); }
    async readImportFile() { await this.#operations?.readImportFile(); }
    replaceImport() { this.#operations?.replaceImport(); }
    appendImport() { this.#operations?.appendImport(); }
    cancelImport() { this.#operations?.cancelImport(); }
    requestDeletion(id) { return this.#operations?.requestDeletion(id) ?? false; }
    confirmDeletion() { this.#operations?.confirmDeletion(); }
    cancelDeletion() { this.#operations?.cancelDeletion(); }
    openExportChoices(trigger, schema) { this.#operations?.openExportChoices(trigger, schema); }
    requestExport() { this.#operations?.requestExport(); }
    omittedStatus(count) { return this.#operations?.omittedStatus(count) ?? `${count} omitted ${count === 1 ? "rule" : "rules"}`; }
    resetBehaviorState() { this.#operations?.reset(); }
}
