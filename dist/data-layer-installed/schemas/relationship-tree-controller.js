...
export function createSchemaRelationshipTreeController(ports) {
    ...
    const controller = {
        ...
        render(options) {
            ...
            const filtered = controller.project(options.projectId, options.nodes), rows = [], document = ports.list?.ownerDocument;
            const savedRow = (node, level) => {
                const schema = options.schemas.find(({ id }) => `saved:${id}` === node.targetKey);
                if (!schema || !document)
                    return;
                ...
                item.setAttribute("aria-selected", String(options.activeSchemaId === schema.id));
            ...
            const visit = (node, level) => {
                ...
                if (node.targetKey) {
                    ...
                    item.setAttribute("aria-selected", String(options.invokingReference === node.key));
                    ...
                    controller.listen(studio, "click", () => options.openContributorInStudio(node.targetKey));
                    ...
                    controller.listen(toggle, "click", () => { controller.toggle(node.key); options.rerender(); });
                    item.append(toggle);
                }
                rows.push(item);
                const expanded = node.expanded || controller.isExpanded(node.key);
            ...
            if (ports.emptyState)
                ports.emptyState.hidden = resultCount > 0;
            if (ports.count) {
                ports.count.textContent = `${resultCount} relationship-tree results`;
                ports.count.setAttribute("aria-label", `${resultCount} schema relationship-tree results`);
            }
