...
export class SchemaRuleInstalledPresentation {
    ...
    render() {
        ...
        list.replaceChildren(...visible.map((rule) => {
            ...
            item.append(summary);
            const action = (label, run) => { const button = list.ownerDocument.createElement("button"), listener = () => run(); button.type = "button"; button.textContent = label; button.addEventListener("click", listener); this.#disposers.push(() => button.removeEventListener("click", listener)); item.append(button); };
            action("Edit", () => { controller.edit(rule.id); });
            if (controller.syncReview(rule).schemaCount)
                action("Sync attached schemas and publish revisions", () => { controller.requestSync(rule.id); });
            action("Duplicate", () => controller.duplicate(rule.id));
            action("Export", () => controller.exportRule(rule.id));
            action(rule.enabled ? "Disable" : "Enable", () => controller.toggle(rule.id));
    ...
    openEditor() {
        ...
        if (e.attachments?.ownerDocument)
            e.attachments.replaceChildren(...this.#schemas().map((schema) => { const option = e.attachments.ownerDocument.createElement("option"); option.value = schema.id; option.textContent = `${schema.name} v${schema.version}`; return option; }));
        e.name?.focus();
    }
    populate(rule) { const e = this.#elements; if (e.name)
        e.name.value = rule.name; if (e.parameters)
        e.parameters.value = rule.parameters ?? ""; if (e.types)
        e.types.value = rule.applicableType ?? "string"; if (e.operator)
        e.operator.value = rule.operator ?? "required"; if (e.severity)
        e.severity.value = rule.severity ?? "error"; if (e.message)
        e.message.value = rule.message ?? ""; if (e.examples)
        e.examples.value = rule.examples ?? ""; }
    showRevision(previous, changes) {
        const e = this.#elements, previousParameters = previous.allowedValues?.map(String).join(",") ?? previous.parameters ?? "none";
        if (e.revisionSummary)
            e.revisionSummary.textContent = `${previous.name} v${previous.version} will become ${changes.name ?? previous.name} v${previous.version + 1}; parameters ${previousParameters} → ${changes.parameters ?? previous.parameters ?? "none"}; examples ${previous.examples ?? "none"} → ${changes.examples ?? previous.examples ?? "none"}.`;
        e.revisionReview?.showModal();
        e.confirmRevision?.focus();
    }
    showUpgrade(rule, affected) { const e = this.#elements; if (e.upgradeSummary)
        e.upgradeSummary.textContent = affected.length ? `Update pinned attachments for ${rule.name} v${rule.version}: ${affected.map(({ name }) => name).join(", ")}.` : `No pinned attachments for ${rule.name} are selected.`; if (e.confirmUpgrade)
        e.confirmUpgrade.disabled = affected.length === 0; e.upgradeReview?.showModal(); (affected.length ? e.confirmUpgrade : e.cancelUpgrade)?.focus(); }
    showSync(review) { const e = this.#elements, changes = review.schemas.map((schema) => `${schema.schemaName} revision ${schema.currentVersion} to ${schema.nextVersion}`).join("; "); if (e.syncSummary)
        e.syncSummary.textContent = review.blocked.length ? `${review.schemaCount} schemas and ${review.attachmentCount} attachments. ${review.blocked.map(({ assistance }) => assistance).join(". ")}.` : `${review.schemaCount} schemas and ${review.attachmentCount} attachments: ${changes || "no pinned revisions"}. No changes occur before confirmation.`; if (e.confirmSync)
        e.confirmSync.disabled = !review.ready; e.syncReview?.showModal(); (review.ready ? e.confirmSync : e.cancelSync)?.focus(); }
    showDeletion(rule) { const e = this.#elements; if (e.deleteSummary)
        e.deleteSummary.textContent = `${rule.name} v${rule.version} will be removed.`; e.deleteReview?.showModal(); e.confirmDelete?.focus(); }
    close(kind) { const e = this.#elements; if (kind === "editor" && e.editor)
        e.editor.hidden = true;
    else if (kind === "revision")
        e.revisionReview?.close();
    ...
    updateAttachmentPreview() { const e = this.#elements; if (e.result)
        e.result.textContent = e.updateAttachments?.checked ? "Pinned attachments will be updated" : "Existing pinned attachments remain unchanged"; }
