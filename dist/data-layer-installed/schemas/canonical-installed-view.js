...
export class SchemaCanonicalInstalledView {
    ...
    openSaved(schema) {
        ...
        const project = (canonical) => {
            ...
            const outer = p.editorDraft(stored), result = savedSchemaFromCanonical({ ...outer, name: canonical.contributorName }, canonical), { canonicalSchema: _canonical, ...projection } = result;
            return projection;
        };
    ...
    openRule(path, trigger) {
        ...
        const render = () => {
            ...
            const cancel = button("Cancel", p.closeRulePicker), review = button("Review changes", () => {
                const panel = document.createElement("section"), summary = document.createElement("p"), reviewActions = document.createElement("section"), stagedRules = working.rules.filter(({ id }) => !removedRuleIds.has(id));
                ...
                reviewActions.append(button("Cancel review", render), button("Confirm changes", () => {
                    void (async () => {
                        const current = adapter.load(), result = await c.dispatchCommand({ kind: "set", baseRevision: current.revision, propertyId: node.id, patch: { rules: structuredClone(stagedRules) } });
                        if (result)
                            p.closeRulePicker();
                    })();
                }));
