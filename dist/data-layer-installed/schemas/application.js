...
import { createInstalledHotkeyController, } from "../../utilities/hotkeys/index.js";
...
import { captureEntry, navigateSession, persistSession, restoreSession, } from "../../utilities/data-layer/capture.js";
...
export async function mountInstalledApplication(controllers) {
    ...
    if (eventTargetPrototype && nativeAddEventListener) {
        eventTargetPrototype.addEventListener = function (type, listener, options) {
            if (listener)
                listenerRecords.push({ target: this, type, listener, ...(options === undefined ? {} : { options }) });
        ...
        if (savedSessionList) {
            savedSessionList.replaceChildren(...sessions.map((session) => {
                ...
                createSequenceButton.addEventListener("click", () => {
                    replayController.createFromSession(session.id, session.name, session.events.map(({ id }) => id));
                    if (savedSessionConfirmation)
