...
export function createSchemasInstalledPublicFacade(p) {
    ...
    const focus = (path) => { p.property.selectPath(path.replace(/^\//, "").replaceAll("/", ".")); p.render(); };
    return {
        ...
        ...createSchemaAuthoringPublicOperations({ property: p.property, rule: p.rule, canonical: p.canonical, renderRulePicker: () => p.propertyWorkflow.render() }, {
            ...
            requestPropertyCopy: (path, target) => p.editorWorkflow.openCopy(path, target), confirmPropertyCopy: () => p.editorWorkflow.confirmCopy(),
            openSpecificIndex: (path, trigger) => p.editorWorkflow.openSpecificIndex(path, trigger), openManualProperty: (path, trigger) => p.editorWorkflow.openManual(path, trigger), openContextualManualProperty: (path, trigger) => p.editorWorkflow.openManual(path, trigger),
            capturePropertyReturn: (path, label) => p.propertyWorkflow.captureReturn(path, label), restorePropertyReturn: () => p.propertyWorkflow.restoreReturn(),
            closeRulePickerForCommit: () => p.propertyWorkflow.closeForCommit(), openRulePicker: (path, trigger) => p.propertyWorkflow.open(path, trigger),
            ...
            openAttachedRule: (schemaId, ruleId, path, trigger) => p.propertyWorkflow.openAttached(schemaId, ruleId, path, trigger),
            attachReusableRule: (schemaId, ruleId, path, rule) => p.rule.attach(schemaId, ruleId, path, rule), updateAttachedRule: (schemaId, ruleId, enabled) => p.rule.updateAttached(schemaId, ruleId, enabled),
        ...
        ...createCanonicalPublicOperations({ controller: p.canonical, schema: (id) => p.library.schemas.find((schema) => schema.id === id),
            ...
            show: () => { p.render(); p.relationshipTree.restoreScroll(); }, projection: (canonical) => p.canonicalPersistence.projection(canonical), facet: (canonical, node) => p.canonicalPersistence.facet(canonical, node), render: () => p.canonicalPersistence.render() })
    };
}
