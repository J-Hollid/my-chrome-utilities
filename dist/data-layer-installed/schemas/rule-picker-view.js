...
export class SchemaRulePickerView {
    ...
    render() {
        ...
        if (!c.configuration) {
            ...
            const canonical = normalize(path), attachedIds = new Set((p.active().workingDraft?.attachedRules ??
                ...
                [rule.name, rule.operator, rule.applicableType]
                    .join(" ")
                    .toLowerCase()
                    .includes(query)), reusable = reusableRulesForProperty(c.rules, propertyType, c.pickerSearch, attachedIds), create = document.createElement("section"), library = document.createElement("section");
            ...
            for (const rule of builtIns) {
                ...
                const action = () => {
                    c.setConfiguration(createRuleConfiguration(rule.name, propertyType));
                    rerender();
                };
            ...
            const close = () => p.close(), searchRules = () => {
        ...
        const configuration = workingConfiguration, editLabel = c.editingAttached
            ...
            : "Create local rule", form = document.createElement("form"), heading = document.createElement("h4"), context = document.createElement("p"), status = document.createElement("output"), parameters = document.createElement("fieldset");
        ...
        if (configuration.applyOnlyWhen) {
            ...
            configuration.conditions.forEach((predicate, index) => {
                ...
                const changeProperty = () => {
                    ...
                    configuration.conditions[index] = {
                        ...
                        ...(comparable
                            ? {
                                comparison: typedComparisonValue(sample.value),
                            }
                            : {}),
                    };
                    rerender();
            ...
            if (!applies)
                ...
                const observed = c.valueAtPath(p.capturedValue(), path), measured = configuration.ruleType === "Item count" &&
                    Array.isArray(observed.value)
                    ? observed.value.length
                    : configuration.ruleType === "Text length" &&
                        typeof observed.value === "string"
                        ? observed.value.length
                        : undefined, passed = measured === undefined
                    ? observed.exists
        ...
        const goBack = () => {
            ...
            this.render();
        }, cancelEdit = () => p.close(), submit = (event) => {
