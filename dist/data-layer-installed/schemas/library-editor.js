...
export class SchemaLibraryEditor {
    ...
    persistDraft() {
        ...
        if (!library.draft && !library.activeSchemaId)
            return;
        const schema = p.active(), name = p.root.querySelector("#schema-editor-name");
    ...
    saveDescription() {
        ...
        p.replaceActive(updateSchemaWorkingDraft(schema, { documentation }, "Update schema description"));
        const tracks = Boolean(p.canonical.editor && p.settle), settlement = tracks ? p.beginSettlement(schema.id) : undefined;
        p.persistIfStored();
        p.renderAll();
        if (tracks)
    ...
    updateTarget() {
        ...
        if (!library.draft && !library.activeSchemaId)
            return;
        const schema = p.active(), target = p.root.querySelector("#schema-editor-target")?.value === "raw input" ? "raw input" : "payload", assignments = (schema.workingDraft?.assignments ?? schema.assignments).map((assignment) => ({ ...assignment, target }));
        p.replaceActive(updateSchemaWorkingDraft(schema, { assignments }, "Update validation target"));
        p.persistIfStored();
        p.renderAll();
    }
    changeParent() {
    ...
    changeAdditionalProperties() {
        ...
        p.replaceActive(updateSchemaWorkingDraft(schema, { document: checked ? { ...document, additionalProperties: false } : document }, "Change additional-property policy"));
        const tracks = Boolean(p.canonical.editor && p.settle), settlement = tracks ? p.beginSettlement(schema.id) : undefined;
        p.persistIfStored();
        p.renderAll();
        if (tracks) {
    ...
    openRevisionReview() {
        ...
        if (!draft)
            return;
        const existing = library.schemas.find(({ id }) => id === draft.id), persisted = existing ? p.editorDraft(existing) : draft, pending = persisted.workingDraft?.pendingChanges.filter((change) => !change.startsWith("Rename schema from ")).join("; ") ?? "", proposed = persisted.workingDraft?.name ?? persisted.name, rename = existing && proposed !== existing.name ? ` Rename schema from ${existing.name} to ${proposed}.` : "", summary = p.root.querySelector("#schema-revision-review-summary"), dialog = p.root.querySelector("#schema-revision-review");
        if (summary)
            summary.textContent = existing?.published === false ? `${draft.name} draft will be published as current revision 1.` : existing ? `${existing.name} working draft will be compared with current revision ${existing.version}; confirmation publishes revision ${existing.version + 1}.${rename}${pending ? ` Pending changes: ${pending}.` : ""}` : `${draft.name} will be published as current revision 1.`;
        if (dialog)
            dialog.hidden = false;
        dialog?.showModal();
    }
    ...
    confirmRevision() {
        ...
        if (this.pendingRestoration) {
            ...
            if (p.active().id !== pending.schemaId)
    ...
    restoreRevision() {
        ...
        const summary = p.root.querySelector("#schema-revision-review-summary"), dialog = p.root.querySelector("#schema-revision-review");
        if (summary)
