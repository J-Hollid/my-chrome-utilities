import { appendObservedHistoryEntry, attachHistoryArraySnapshot } from "../../../data-layer-observer.js";
import { createObservationSourceCoordinator } from "./coordinator.js";
export function createCaptureObservationRuntime(ports) {
    ...
    const coordinator = createObservationSourceCoordinator({
        start: options => {
            if (!ports.runtime.startSource)
                throw new Error("Observation source runtime is unavailable");
    ...
    async function start(observation) {
        lastObservation = observation;
        const configuration = ports.configuration(), session = ports.session().session;
        if (ports.runtime.startSource && !configuration) {
            stop();
            return;
        }
        ...
        const current = () => ports.active() && operation === generation;
        try {
            const stopCapture = await ports.runtime.startPush({
                ...
                onSnapshot: ({ historyPath, rawValues }) => {
                    if (!current())
                        return;
                    ports.replaceObserver(attachHistoryArraySnapshot({ ...ports.observer(), sessionState: ports.session() }, { pageUrl: observation.pageUrl, ...(observation.pageLoadId ? { pageLoadId: observation.pageLoadId } : {}),
    ...
    return { start, stop, generation: () => generation,
        configurationChanged() {
            if (lastObservation && ports.session().session?.status === "active")
                void start(lastObservation);
            else if (!ports.configuration())
                stop();
        },
    };
}
