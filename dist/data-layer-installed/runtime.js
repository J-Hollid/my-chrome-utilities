import { createInstalledSidePanelShellController } from "../utility-host/installed-shell-controller.js";
export { createInstalledSidePanelShellController } from "../utility-host/installed-shell-controller.js";
...
export function createInstalledLiveInspectorCoordination(ports) {
    return {
...
export function createInstalledSchemaContributorCoordination(ports) {
    let durableProjection;
    const captureProject = (state) => {
        if (state.project.id === ports.activeProjectId())
    ...
    const currentProject = () => {
        ...
        if (!projectId)
            return undefined;
        if (durableProjection?.project.id === projectId)
    ...
    const ensureProjectContributors = async (projectId) => {
...
export async function mountInstalledDataLayerRuntime(root = document, storage = globalThis.localStorage, workspaceNavigation) {
    ...
    const showDataLayerView = (view, focus = false) => {
        if (!liveApi.dataLayerViews.includes(view))
            return;
        if (currentView === "Live" && view !== "Live")
            controllers?.capture.rememberInspectorPresentation();
        currentView = view;
    ...
    const schemaContributors = createInstalledSchemaContributorCoordination({ activeProjectId, compatibilityProject: currentProject,
        ensureProject: durable.ensureProject, loadProject: async (projectId) => {
            const loaded = await durable.repository.loadProject(projectId);
    ...
    const openSchemaContributor = (key) => {
        let contributorUi = {};
        const selectionFor = (state) => schemaApi.resolveSidePanelSchemaContributor(state, key);
        const documentFor = (state) => {
            const selection = selectionFor(state);
            if (!selection)
                throw new Error("The selected schema contributor is unavailable.");
            const base = selection.scope !== "Shared Profile" ? schemaApi.composedCanonicalSchema(state, selection.entity, selection.scope, selection.flowId)
                : (selection.entity.canonicalSchema ?? schemaApi.createCanonicalSchema({ id: `canonical:${selection.entity.id}`, contributorId: selection.entity.id, contributorName: selection.entity.name }));
        ...
        const initial = schemaContributors.currentProject(), selection = initial && selectionFor(initial);
        ...
        controllers.schemas.openCanonical({ key, label: `${selection.entity.name} · Role ${selection.scope} · scope ${selection.scope} · provenance ${selection.flowId ? `${selection.flowId} · ` : ""}${selection.entity.id}`,
            load: () => { const live = schemaContributors.currentProject(); if (!live)
                throw new Error("The active schema project is unavailable."); return documentFor(live); },
            dispatch: (command) => {
                const live = schemaContributors.currentProject();
                if (!live)
                    throw new Error("The active schema project is unavailable.");
                const selected = selectionFor(live);
                if (!selected)
                    throw new Error("The selected schema contributor is unavailable.");
                const document = documentFor(live), result = schemaApi.applyCanonicalCommand(document, command);
                if (result.status === "applied" || result.status === "rebased") {
                    const mutation = command.kind !== "select" && command.kind !== "view";
                    if (!mutation)
                    ...
                    if (mutation) {
                        ...
                        if (command.kind === "policy" && selected.scope !== "Shared Profile") {
                            ...
                            const nextSelected = selectionFor(next);
                            if (!nextSelected)
                                throw new Error("The selected schema contributor is unavailable.");
                            if (nextSelected.entity.canonicalSchema)
                                nextSelected.entity.canonicalSchema = { ...nextSelected.entity.canonicalSchema, onlyDefinedFields: result.document.onlyDefinedFields };
                            else
                                nextSelected.entity.onlyDefinedFields = result.document.onlyDefinedFields;
                        }
                        else if (selected.scope === "Shared Profile")
                            next = controllers.projects.writeUnifiedContributorCanonical(live, selected, result.document);
                        else if (selected.collectionKind === "pages" || selected.collectionKind === "propertySets")
                            next = schemaApi.saveComposedCanonicalDocument(live, selected.collectionKind, selected.entity.id, result.document);
                        else if (selected.collectionKind === "events")
                            next = schemaApi.saveComposedEventCanonicalDocument(live, selected.entity.id, result.document);
                        else if (selected.scope === "Flow Page-instance")
                            next = schemaApi.saveFlowPageInstanceCanonicalDocument(live, selected.flowId, selected.entity.id, result.document);
                        else
                            next = schemaApi.saveEventOccurrenceCanonicalDocument(live, selected.flowId, selected.entity.id, result.document);
                        const label = `${command.kind} canonical schema in ${selected.entity.name}`, committed = controllers.projects.commitUnifiedContributorState(next, label);
                        pendingRevision = committed.revision;
                        schemaContributors.captureProject(next);
                    }
                }
                return result;
            },
    ...
    const defectNavigation = (selected, action) => defectApi.createLiveDefectReportNavigation(selected.id, {
        reopenCapturedEvent: (id) => controllers.capture.openInspector(id, true), createDefectReportAction: action,
        closeToLiveFeed: () => controllers.capture.closeInspector(),
    });
    const guidedCapturedEvent = (event) => ({ id: event.id, sourceId: event.sourceId, name: event.name, payload: event.payload ?? {}, rawInput: event.rawInput ?? [],
        ...(event.pageUrl ? { pageUrl: event.pageUrl } : {}) });
    const startValidationDefectReport = (selected) => {
        const inspector = liveElements.eventInspector;
        if (!inspector)
            return;
        defectApi.renderDefectReportBuilder(inspector, selected, undefined, controllers.capture.state().observer.events, defectNavigation(selected, () => liveElements.eventInspector?.querySelector("#live-inspector-action-create-defect-report") ?? null), {
            save: async (report, options) => {
                const selectedPointers = new Set(report.evidence.validation.map(({ pointer }) => pointer));
                const issues = defectApi.currentDefectIssues(selected).filter((issue) => selectedPointers.has(issue.concretePath));
                const defect = defectApi.createValidationDefect({ id: `defect:${crypto.randomUUID()}`, now: new Date().toISOString(), report, issues });
                const result = controllers.defects.add(defect, options.saveSeparately);
                if (result.added && selected.manualFlowContext)
                    controllers["live-flow-testing"].attachDefect(selected.manualFlowContext.selectedStepId, selected.manualFlowContext.eventId, defect.id);
                if (options.copy && navigator.clipboard?.writeText)
                    await navigator.clipboard.writeText(defectApi.renderJiraReport(report).text);
                return result.added ? { feedback: options.copy ? "Defect saved and copied for Jira Cloud." : "Defect saved." } : { feedback: "A reported defect already matches the selected issue.",
                    existing: result.existing.map((candidate) => ({ id: candidate.id, label: String(candidate.report?.summary ?? candidate.id) })) };
            },
            openExisting: (id) => controllers.defects.open(id), updateExisting: (id, report) => { controllers.defects.edit(id, { report }); controllers.defects.open(id); },
        });
    };
    const startOccurrenceDefectReport = (selected, mode) => {
        const inspector = liveElements.eventInspector;
        if (!inspector)
            return;
        defectApi.renderOccurrenceDefectReportBuilder(inspector, selected, mode, controllers.schemas.schemas(), controllers.capture.state().observer.events, undefined, defectNavigation(selected, () => liveElements.eventInspector?.querySelector(`#live-inspector-action-report-${mode === "Unexpected event" ? "unexpected-event" : "wrong-event-name"}`) ?? null), {
            save: async (report, options) => {
                const defect = defectApi.createOccurrenceDefect({ id: `defect:${crypto.randomUUID()}`, now: new Date().toISOString(), report });
                const result = controllers.defects.add(defect, options.saveSeparately);
                if (options.copy && navigator.clipboard?.writeText)
                    await navigator.clipboard.writeText(defectApi.renderOccurrenceReport(report).text);
        ...
        capture: { root, storage: dataStorage,
            ...
            inspector: { splitView: () => globalThis.innerWidth >= 700, capturePresentation: () => {
                ...
                restoreReturn: (snapshot) => liveApi.restoreInspectorReturnUi(liveElements, snapshot), render: (event) => {
                    ...
                    renderObservationSourceDetails(liveElements.eventInspector, event);
                    if (liveElements.eventInspector)
        ...
        schemas: { root, storage: dataStorage, relationshipViewStorage: storage, changed: () => { }, subscribe: (listener) => durable.subscribe(({ library }) => {
            ...
            showSchemasView: () => showDataLayerView("Schemas"), scheduleFrame: (callback) => requestAnimationFrame(callback), restoreGuidedCapture: (id, path, focusAction = "validation") => {
                ...
                const restore = () => {
                    ...
                    const inspector = liveElements.eventInspector, feed = liveElements.eventFeed;
                    if (snapshot) {
                        for (const propertyPath of snapshot.expanded)
                            inspector?.querySelector(`details[data-property-path="${CSS.escape(propertyPath)}"]`)?.setAttribute("open", "");
                        if (inspector)
                    ...
                    const target = Array.from(inspector?.querySelectorAll(`button[data-action="${action}"]`) ?? [])
                        .find((button) => button.dataset.propertyPath === (path ?? snapshot?.path));
                    target?.focus({ preventScroll: true });
                };
                restore();
                if (schemaRecoveryActive) {
                    root.querySelector("#durable-storage-recovery")?.close();
                    schemaRecoveryActive = false;
                    requestAnimationFrame(() => { if (snapshot && guidedLivePropertyReturn !== snapshot)
                        return; restore(); if (guidedLivePropertyReturn === snapshot)
                        guidedLivePropertyReturn = undefined; });
                }
                else
                    queueMicrotask(() => { if (snapshot && guidedLivePropertyReturn !== snapshot)
                        return; restore(); if (guidedLivePropertyReturn === snapshot)
                        guidedLivePropertyReturn = undefined; });
            },
    ...
    return { mount() {
        ...
        dispose() {
            ...
            stopDurableCoordination?.();
            stopDurableCoordination = undefined;
            lifecycle.dispose();
            schemaPersistence.dispose();
            guidedLivePropertyReturn = undefined;
            shell.dispose();
        } };
}
