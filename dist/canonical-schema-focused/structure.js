...
export const renderCanonicalStructuralControls = (dom, context, working) => { const document = context.current(), siblings = Object.values(document.nodes).filter(({ parentId }) => parentId === working.parentId).sort((left, right) => left.order - right.order || left.id.localeCompare(right.id)), index = siblings.findIndex(({ id }) => id === working.id), earlier = button(dom, "Move earlier", () => { if (index <= 0)
    return; const afterId = index > 1 ? siblings[index - 2].id : undefined; applyStructure(context, { kind: "move", propertyId: working.id, ...(working.parentId ? { parentId: working.parentId } : {}), ...(afterId ? { afterId } : {}) }); }), later = button(dom, "Move later", () => { if (index < 0 || index >= siblings.length - 1)
