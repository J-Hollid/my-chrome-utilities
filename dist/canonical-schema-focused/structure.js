...
export const renderCanonicalStructuralControls = (dom, context, working) => { const document = context.current(), siblings = Object.values(document.nodes).filter(({ parentId }) => parentId === working.parentId).sort((left, right) => left.order - right.order || left.id.localeCompare(right.id)), reorder = renderReorderControl({ itemId: working.id, itemLabel: working.name, completeOrder: siblings.map(({ id, name }) => ({ id, label: name })), onMove: ({ itemId, toIndex }) => { const next = reorderValues(siblings, itemId, toIndex, value => value.id), position = next.findIndex(({ id }) => id === working.id), afterId = position > 0 ? next[position - 1].id : undefined; applyStructure(context, { kind: "move", propertyId: working.id, ...(working.parentId ? { parentId: working.parentId } : {}), ...(afterId ? { afterId } : {}) }); } }), toRoot = button(dom, "Move to root", () => { if (!working.parentId)
    return; applyStructure(context, { kind: "move", propertyId: working.id }); }), duplicate = button(dom, "Duplicate", () => applyStructure(context, { kind: "duplicate", propertyId: working.id, id: context.id })), remove = button(dom, "Delete property", () => applyStructure(context, { kind: "delete", propertyId: working.id })); toRoot.disabled = !working.parentId; return [reorder, toRoot, duplicate, remove]; };
