...
function renderTable(tree, context) {
    const { dom } = context, table = context.tableElement ?? dom.createElement("table"), head = dom.createElement("thead"), headRow = dom.createElement("tr"), body = dom.createElement("tbody"), filterActive = Boolean(context.query.trim()) || context.propertyFilter !== "all", propertyMenus = new Map();
    let pendingOverlay;
    ...
    for (const row of canonicalNavigatorRows(context)) {
        ...
        propertyMenus.set(row.id, propertyMenu);
    ...
    for (const row of Array.from(body.querySelectorAll("tr"))) {
        const node = context.document.nodes[row.dataset.propertyId ?? ""], identity = row.querySelector("[data-schema-table-cell='property-editor']"), actions = identity?.querySelector("[aria-label^='Property actions']"), propertyMenu = propertyMenus.get(node?.id ?? "");
        if (!node || !identity || !actions || !propertyMenu)
            continue;
