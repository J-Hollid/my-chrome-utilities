...
export function renderValuesFacet(host, context, working) {
    ...
    working.allowedValues.forEach((entry, index) => {
        ...
        value.disabled = removed || inherited && !overridden;
        ...
        const actions = ownership === "inherited" ? [button(dom, "Override here", () => nextOverride(context, entry)), button(dom, "Open source", () => { row.dataset.valueMode = "source"; })] : ownership === "overridden" ? [button(dom, "Edit", () => value.focus()), removed ? button(dom, "Restore", () => { context.removedValueIds.delete(entry.id); context.render(); }) : button(dom, "Reset to parent", () => { context.removedValueIds.add(entry.id); context.render(); })] : [removed ? button(dom, "Restore", () => { context.removedValueIds.delete(entry.id); context.render(); }) : button(dom, "Edit", () => value.focus()), removed ? Object.assign(dom.createElement("button"), { type: "button", textContent: "Removed" }) : button(dom, "Remove local", () => { context.removedValueIds.add(entry.id); context.render(); })];
        row.append(labeled(dom, `Value ${index + 1}`, value), button(dom, "View", () => { row.dataset.valueMode = "view"; const detail = dom.createElement("p"); detail.textContent = `Value ${entry.id} · ${canonicalFacetText(entry.value)} · ${ownership}`; row.append(detail); }), ...actions);
        list.append(row);
    });
