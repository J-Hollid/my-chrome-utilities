...
export function flowDocumentationSnapshotFromState(state, flowId, generatedAt = new Date().toISOString(), revision = 0) {
    ...
    const frameContexts = graph.pageFrames.flatMap((frame, index) => { const page = pageById.get(String(frame.pageId)), pageGroupId = String(frame.pageGroupId ?? ""), pageGroup = groupById.get(pageGroupId), profileId = String(page?.profileIds?.[0] ?? ""), profile = profileById.get(profileId), path = { flowId, pageFrameId: frame.id, ...(pageGroupId ? { pageGroupId } : {}), ...(page?.id ? { pageId: page.id } : {}), ...(profileId ? { profileId } : {}) }, contributors = layeredContributorsForPath(state, path), compiled = contributors.length ? compileLayeredSchema(contributors, { eventId: frame.id, eventRole: "interaction", occurrenceId: frame.id }) : emptyCompiled(), samePageIndex = graph.pageFrames.filter(({ pageId }) => pageId === frame.pageId).findIndex(({ id }) => id === frame.id) + 1; return [{ id: `context:frame:${frame.id}`, kind: "page-instance", pageFrameId: frame.id, pageName: page?.name ?? "Unresolved Page", eventName: `Page instance ${samePageIndex}`, stepLabel: `P${index + 1}`, effectiveRevision: schemaRevisionFor(state, [profile, pageGroup, page, flow, frame], frame), compiled, ...(!page ? { unresolved: [{ path: "/context/page", issue: `Unresolved Page ${String(frame.pageId)}`, repair: "Open Page reference" }] } : {}) }]; });
    const occurrenceContexts = ordered.ids.flatMap((occurrenceId, index) => {
    ...
    const contexts = [...frameContexts, ...occurrenceContexts], endpointIds = new Set([...graph.pageFrames, ...graph.occurrences].map(({ id }) => id));
    graph.relationships.forEach((relationship, index) => { const source = String(relationship.sourceEndpoint?.id ?? relationship.sourceNodeId ?? ""), target = String(relationship.targetEndpoint?.id ?? relationship.targetNodeId ?? ""), missing = [...(!endpointIds.has(source) ? [`source ${source || "(empty)"}`] : []), ...(!endpointIds.has(target) ? [`target ${target || "(empty)"}`] : [])]; if (!missing.length || !contexts.length)
        return; const frameAnchor = graph.pageFrames.find(({ id }) => id === source || id === target)?.id, occurrenceAnchor = graph.occurrences.find(({ id }) => id === source || id === target)?.id, contextId = frameAnchor ? `context:frame:${frameAnchor}` : `context:${occurrenceAnchor ?? ordered.ids[0]}`, contextIndex = Math.max(0, contexts.findIndex(({ id }) => id === contextId)), context = contexts[contextIndex]; contexts[contextIndex] = { ...context, unresolved: [...(context.unresolved ?? []), { path: `/relationships/${String(relationship.id ?? index)}`, issue: `Dangling Flow relationship: unresolved ${missing.join(" and ")}`, repair: "Open Flow relationship" }] }; });
