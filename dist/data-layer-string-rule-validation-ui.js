...
export function renderRegularExpressionTester(dom, pattern) {
    ...
    if (!patternLabel.contains(pattern))
        patternLabel.append(regularExpressionTesterCopy.patternLabel, pattern);
    host.dataset.patternTester = "true";
    host.dataset.patternTesterFieldGrid = "true";
    host.style.cssText = regularExpressionTesterGridStyle;
    patternLabel.dataset.patternTesterField = "pattern";
    patternLabel.style.cssText = "display:grid;gap:0.2rem;min-width:0;";
    pattern.style.cssText += ";box-sizing:border-box;min-width:0;width:100%;";
    sampleLabel.dataset.patternTesterField = "sample";
    sampleLabel.style.cssText = "display:grid;gap:0.2rem;min-width:0;";
    sample.type = "text";
    sample.setAttribute("aria-label", regularExpressionTesterCopy.sampleLabel);
    sample.style.cssText = "box-sizing:border-box;min-width:0;width:100%;";
    sampleLabel.append(regularExpressionTesterCopy.sampleLabel, sample);
    guidance.id = guidanceId;
    guidance.dataset.patternTesterGuidance = "true";
    guidance.style.cssText = "grid-column:1/-1;min-width:0;margin:0;";
    guidance.textContent = regularExpressionTesterCopy.guidance;
    pattern.setAttribute("aria-describedby", guidanceId);
    sample.setAttribute("aria-describedby", guidanceId);
    resultRegion.dataset.patternTestResultRegion = "true";
    resultRegion.style.cssText = "display:grid;grid-column:1/-1;min-width:0;margin:0;";
    resultLabel.id = resultLabelId;
    resultLabel.textContent = regularExpressionTesterCopy.resultLabel;
    result.setAttribute("aria-live", "polite");
    result.setAttribute("aria-labelledby", resultLabelId);
    result.style.margin = "0";
    result.dataset.patternTestResult = "true";
    resultRegion.append(resultLabel, result);
    const render = () => {
    ...
    host.append(patternLabel, sampleLabel, guidance, resultRegion);
    render();
    return host;
}
