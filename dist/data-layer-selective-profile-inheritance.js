...
export function toggleProfileInheritanceConcept(document, recipe, concept) {
    ...
    next.propertySelections = active ? next.propertySelections.filter((id) => !idSet.has(id)) : stableUnique([...next.propertySelections, ...ids]);
...
export function toggleProfileInheritanceTreeBranch(document, recipe, propertyId, operatedPropertyIds) {
    ...
    next.propertySelections = stableUnique([...next.propertySelections, ...ids]);
    return next;
}
export function profileInheritanceSelection(document, recipe) {
    const order = orderedIds(document), excluded = new Set(recipe.excludedPropertyIds), legacyFixedIds = recipe.membership !== "fixed" && recipe.sourceSnapshot ? recipe.sourceSnapshot.effectivePropertyIds : [], fixed = recipe.membership === "fixed" || Boolean(recipe.sourceSnapshot), allowlist = stableUnique([...recipe.propertySelections, ...legacyFixedIds]), missingPropertyIds = allowlist.filter((id) => !document.nodes[id]), direct = new Set();
    for (const id of order) {
        const node = document.nodes[id];
        if (excluded.has(id))
            continue;
        if (allowlist.includes(id) || (!fixed && (recipe.startingPoint === "everything" || Boolean(node.concept && recipe.conceptSelections.includes(node.concept)))))
            direct.add(id);
    }
...
export function profileInheritanceParentAdditions(document, recipe) {
    if (!recipe.sourceSnapshot)
        return [];
    const prior = new Set(recipe.sourceSnapshot.sourcePropertyIds ?? Object.keys(recipe.sourceSnapshot.propertyPaths)), selected = new Set(profileInheritanceSelection(document, recipe).effectivePropertyIds);
    return orderedIds(document).flatMap((propertyId) => { const node = document.nodes[propertyId]; if (prior.has(propertyId) || selected.has(propertyId))
...
export function includeProfileInheritanceParentAdditions(document, recipe, propertyIds) {
    const included = propertyIds.filter((propertyId) => Boolean(document.nodes[propertyId])), idSet = new Set(included), staged = { ...clone(recipe), membership: "fixed", propertySelections: stableUnique([...recipe.propertySelections, ...included]), excludedPropertyIds: recipe.excludedPropertyIds.filter((propertyId) => !idSet.has(propertyId)) }, applied = profileInheritanceRecipeApplied(document, staged), sourcePropertyIds = recipe.sourceSnapshot?.sourcePropertyIds ?? (recipe.sourceSnapshot ? Object.keys(recipe.sourceSnapshot.propertyPaths) : undefined);
...
export function profileInheritanceSourceSnapshot(document, recipe) { const selection = profileInheritanceSelection(document, recipe), ruleBearingPropertyIds = stableUnique([...selection.directPropertyIds, ...selection.ruleDependencyPropertyIds]); return { revision: document.revision, sourcePropertyIds: orderedIds(document), effectivePropertyIds: [...selection.effectivePropertyIds], propertyPaths: Object.fromEntries(selection.effectivePropertyIds.flatMap((propertyId) => document.nodes[propertyId] ? [[propertyId, canonicalPropertyPath(document, propertyId)]] : [])), ruleFingerprints: sourceRuleFingerprints(document, ruleBearingPropertyIds), definitionFingerprints: sourceDefinitionFingerprints(document, selection), missingRuleDependencyKeys: selection.missingRuleDependencies.map(missingDependencyKey) }; }
export function profileInheritanceRecipeApplied(document, recipe) { const selection = profileInheritanceSelection(document, recipe), fixed = { ...clone(recipe), membership: "fixed", propertySelections: [...selection.directPropertyIds], sourceRevision: document.revision }; delete fixed.sourceSnapshot; delete fixed.sourceImpact; const applied = { ...fixed, sourceSnapshot: profileInheritanceSourceSnapshot(document, fixed) }; return applied; }
...
export function markProfileInheritanceConsumersForSourceChange(project, profileId, before, after) { return { ...project, collections: { ...project.collections, pages: project.collections.pages.map((entity) => markProfileInheritanceTargetStale(entity, profileId, before, after)), propertySets: project.collections.propertySets.map((entity) => markProfileInheritanceTargetStale(entity, profileId, before, after)), events: project.collections.events.map((entity) => markProfileInheritanceTargetStale(entity, profileId, before, after)) } }; }
