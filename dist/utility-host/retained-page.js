import { acceptsUtilityMessage, utilityMessage, utilityPageUrl } from "./protocol.js";
export function createRetainedUtilityPage({ contribution, panel, page, selectTarget, subscribeTargetClosed }) {
    ...
    const send = (target, kind, payload) => {
        if (identity && target)
            target.postMessage(utilityMessage(identity, kind, payload), origin);
    };
    const permitDiscard = () => !dirty || page.confirm(`Discard unsaved changes in ${contribution.label}?`);
    const resetOwned = () => {
        if (!permitDiscard())
            return;
    ...
    const onMessage = (event) => {
        if (disposed || !identity || event.origin !== origin || !acceptsUtilityMessage(event.data, identity))
            return;
        ...
        if (message.kind === "ready") {
            if (owner) {
                ...
                status.textContent = unavailable ? "The bound website target is unavailable." : `${contribution.label} is ready.`;
            }
            send(owner ? frame.contentWindow : workbench, "state", lastState);
            if (unavailable)
                send(owner ? frame.contentWindow : workbench, "target-closed");
        }
        else if (owner && message.kind === "state") {
            lastState = message.payload;
            send(workbench, "state", lastState);
        }
            ...
            closeOwned();
        else if (secondary && message.kind === "stop")
            send(frame.contentWindow, "stop");
    };
    const openWorkbench = () => {
        if (!identity || unavailable || launch.disabled)
            return;
        if (workbench && !workbench.closed) {
            workbench.focus();
            return;
        }
        workbench = page.open(utilityPageUrl(contribution.page, identity, "workbench", page.location.href), "_blank");
        if (!workbench)
            status.textContent = "The browser could not open the utility page.";
    };
    const removeTargetListener = subscribeTargetClosed((id) => {
        if (!identity || identity.targetId !== id)
            return;
        unavailable = true;
        launch.disabled = true;
        status.textContent = "The bound website target is unavailable.";
        send(frame.contentWindow, "target-closed");
        send(workbench, "target-closed");
    });
    ...
    async function load() {
        ...
        starting = (async () => {
            ...
            try {
                ...
                status.textContent = `${contribution.label} could not start: ${String(error)}`;
            }
        })();
        return starting;
    }
    function dispose() {
        if (disposed)
            return;
        send(frame.contentWindow, "close");
        send(workbench, "close");
        disposed = true;
        generation += 1;
        removeTargetListener();
        page.removeEventListener("message", onMessage);
        launch.removeEventListener("click", openWorkbench);
        reset.removeEventListener("click", resetOwned);
        close.removeEventListener("click", closeOwned);
        frame.remove();
    }
    return { load, dispose };
}
//# sourceMappingURL=retained-page.js.map