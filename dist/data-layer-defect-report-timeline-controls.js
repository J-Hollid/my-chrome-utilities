...
export function appendTimelineControls(composer, entries, context, state) {
    ...
    const renderEntries = () => {
        ...
        entries.replaceChildren(...chronological.map((event) => {
            ...
            item.dataset.timelineEntryId = event.id;
            ...
            for (const [field, label] of [["includeSummary", "Summary included"], ["includePayload", "Payload included"], ["includeValidation", "Validation details included"]]) {
                if (!selection[field])
                    continue;
                const tag = document.createElement("span");
                tag.dataset.timelineEvidenceIncluded = field;
            ...
            remove.addEventListener("click", () => {
                selections = removeTimelineSelection(selections, event.id);
                updateReport();
                render();
            });
            item.append(metadata, included, adjust, remove);
            return item;
        }));
    };
    const returnToIdle = (focus = "add") => {
        const adjustedEventId = editingEventId;
        stage = "idle";
        draft = undefined;
        editingEventId = undefined;
        render();
        if (focus === "adjust" && adjustedEventId)
            adjustButtons.get(adjustedEventId)?.focus({ preventScroll: true });
        else
    ...
    const renderIdle = () => {
        ...
        addButton.textContent = "Add event to timeline";
        addButton.addEventListener("click", () => {
            filter = {};
            visibleResults = resultWindowSize;
            editingEventId = undefined;
            draft = undefined;
            stage = "select";
            renderComposer();
    ...
    const renderSelection = () => {
        ...
        stageRoot.className = "defect-timeline-composer-stage";
        stageRoot.setAttribute("aria-label", "Select one captured event");
        const explanation = document.createElement("p");
        explanation.textContent = "Select one captured event to configure its timeline evidence.";
        stageRoot.append(explanation);
        for (const [field, labelText] of [["search", "Search"], ["name", "Event name"], ["source", "Source"], ["pathname", "Pathname"], ["validation", "Validation state"]]) {
            ...
            label.append(input);
            stageRoot.append(label);
        }
        const choices = document.createElement("div");
        choices.className = "defect-timeline-event-choices";
        choices.setAttribute("aria-label", "Captured event choices");
        ...
        for (const { event, alreadyAdded } of page.choices) {
            ...
            choice.type = "radio";
            choice.name = "defect-timeline-event";
            choice.dataset.timelineEventId = event.id;
            choice.disabled = alreadyAdded;
            if (alreadyAdded)
                choice.dataset.alreadyAdded = "true";
            choice.addEventListener("change", () => {
                if (!choice.checked || alreadyAdded)
                    return;
        ...
        if (page.hasOlder) {
            ...
            older.type = "button";
            older.textContent = "Load older matches";
        ...
        stageRoot.append(cancelButton);
        composer.replaceChildren(stageRoot);
    };
    const renderConfiguration = () => {
        ...
        const stageRoot = document.createElement("section");
        stageRoot.className = "defect-timeline-composer-stage";
        stageRoot.setAttribute("aria-label", `Configure evidence for ${event.name}`);
        const title = document.createElement("p");
        title.textContent = `Selected event: ${event.name}`;
        const always = document.createElement("p");
        always.dataset.timelineAlwaysIncluded = event.id;
        ...
        if (!editingEventId) {
            ...
            back.type = "button";
            back.textContent = "Back to event selection";
            back.addEventListener("click", () => { draft = undefined; stage = "select"; renderComposer(); });
            stageRoot.append(back);
        }
        const save = document.createElement("button");
        save.type = "button";
        save.textContent = editingEventId ? "Save changes" : "Add to timeline";
        save.addEventListener("click", () => {
            selections = saveTimelineSelection(selections, draft);
            updateReport();
            returnToIdle(editingEventId ? "adjust" : "add");
        });
        const cancelButton = document.createElement("button");
        cancelButton.type = "button";
        cancelButton.textContent = "Cancel";
        cancelButton.addEventListener("click", cancel);
        stageRoot.append(save, cancelButton);
        composer.replaceChildren(stageRoot);
    };
    ...
    function render() { renderEntries(); renderComposer(); }
    render();
}
//# sourceMappingURL=data-layer-defect-report-timeline-controls.js.map