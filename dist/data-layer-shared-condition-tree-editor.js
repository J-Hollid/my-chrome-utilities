...
export const sharedConditionListboxLayout = (field, editor, viewport, contentWidth) => {
...
export function renderSharedConditionTree(host, options) {
    ...
    const render = () => {
        ...
        const propertyControl = (row, operator) => {
            ...
            property.addEventListener("input", () => { const conditionRow = operator.closest("[data-condition-kind=\"predicate\"]"), rowHeight = conditionRow?.getBoundingClientRect().height; Object.assign(row, sharedUnresolvedConditionRow(row)); delete row.value; activeIndex = -1; renderOperators(row, operator); const valueHost = conditionRow?.querySelector('[aria-label="Condition value"]'); if (valueHost) {
                valueHost.style.minHeight = `${valueHost.getBoundingClientRect().height}px`;
                valueHost.replaceChildren("Choose property and operator");
            } if (conditionRow && rowHeight)
                conditionRow.style.minHeight = `${rowHeight}px`; emit(); open(); });
