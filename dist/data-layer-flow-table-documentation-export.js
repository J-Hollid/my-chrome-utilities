...
export function writeFlowDocumentationWorkbook(snapshot, input) {
    const legend = { title: snapshot.title, headings: ["Symbol or context", "Meaning or provenance"], rows: [["M", "Mandatory"], ["O", "Optional"], ["C", "Conditional"], ["N", "Not expected"], ["—", "Not defined"], ["!", "Blocked"], ...snapshot.contexts.map((context) => [contextName(context), contextProvenance(context)])] }, diagnostics = { title: snapshot.title, headings: ["Source state", "Context", "Property", "Issue", "Repair", "Generated"], rows: snapshot.diagnostics.length ? snapshot.diagnostics.map((item) => [snapshot.incomplete ? "Draft — incomplete" : snapshot.sourceState, item.contextName, item.path, item.issue, item.repair, snapshot.generatedAt]) : [[snapshot.incomplete ? "Draft — incomplete" : snapshot.sourceState, "All selected contexts", "", "No export blockers", "", snapshot.generatedAt]] }, names = ["Flow values", "Capture matrix", "Legend and provenance", "Export diagnostics"], tables = [input.valueTable, input.matrixTable, legend, diagnostics];
