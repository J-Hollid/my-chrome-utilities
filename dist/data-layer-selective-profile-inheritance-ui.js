...
export function mountSelectiveProfileInheritance(options) {
    ...
    const renderWorkspace = () => {
        ...
        if (sources.length)
            compactControls.append(copyLabel);
        toolbar.className = "profile-inheritance-toolbar";
        toolbar.setAttribute("aria-label", "Find source properties");
        ...
        requiredFilter.append(option("any", "Any presence"), option("required", "Required"), option("optional", "Optional"));
        selectionFilter.append(option("any", "Any selection state"), option("selected", "Selected"), option("unselected", "Unselected"));
        for (const [control, key, label] of [[conceptFilter, "concept", "concept"], [typeFilter, "type", "type"], [requiredFilter, "required", "presence"], [selectionFilter, "selection", "selection state"]]) {
        ...
        for (const treeNode of shown) {
            ...
            else if (event.key === "ArrowRight" && treeNode.kind !== "property") {
                event.preventDefault();
                if (!expandedIds.has(treeNode.id)) {
                    expandedIds.add(treeNode.id);
                    renderWorkspace();
                    focusTree(treeNode.id);
                }
                else
                    moveTreeFocus(treeNode.id, 1);
            }
            else if (event.key === "ArrowLeft") {
                event.preventDefault();
                if (treeNode.kind !== "property" && expandedIds.has(treeNode.id)) {
                    expandedIds.delete(treeNode.id);
                    renderWorkspace();
                    focusTree(treeNode.id);
                }
                else if (treeNode.parentId)
                    focusTree(treeNode.parentId);
            }
            else if (event.key === " ") {
                event.preventDefault();
                checkbox.click();
            ...
            checkbox.setAttribute("aria-label", `${treeNode.label}, ${treeNode.selectedCount} of ${treeNode.totalCount} selected${treeNode.kind === "concept" && staged.conceptSelections.includes(treeNode.concept) ? ", staged source-group selection; applied membership is fixed" : ""}`);
            ...
            secondary.textContent = treeNode.kind === "concept" ? `${treeNode.selectedCount} of ${treeNode.totalCount} selected${staged.conceptSelections.includes(treeNode.concept) ? " · staged source-group selection" : ""}` : `${node.type} · ${node.presence.mode} · ${treeNode.selectedCount} of ${treeNode.totalCount} selected`;
            ...
            if (node && detailTreeNodeId === treeNode.id) {
                ...
                details.tabIndex = -1;
                details.setAttribute("aria-label", `${node.name} property details`);
        ...
        for (const dependency of closure.missingRuleDependencies) {
            ...
            for (const [control, action] of [[include, "Include dependency"], [exclude, "Exclude source rule"], [replace, "Replace source rule"]])
                control.setAttribute("aria-label", `${action} ${dependency.sourceRuleId} for ${target.name}`);
            include.addEventListener("click", () => { staged.includedDependencyPropertyIds = stable([...staged.includedDependencyPropertyIds, dependency.propertyId]); replacementDraft = undefined; renderWorkspace(); focusAfter("[data-rule-dependency] button,[aria-label='Selection summary'] button"); });
            exclude.addEventListener("click", () => { staged.excludedRuleIds = stable([...staged.excludedRuleIds, dependency.sourceRuleId]); replacementDraft = undefined; renderWorkspace(); focusAfter("[data-rule-dependency] button,[aria-label='Selection summary'] button"); });
            ...
            if (replacementDraft?.sourceRuleId === dependency.sourceRuleId) {
                ...
                if (replacementDraft.reviewed) {
                    ...
                    } replacementDraft = undefined; renderWorkspace(); focusAfter("[data-rule-dependency] button,[aria-label='Selection summary'] button"); });
        ...
        const totals = profileInheritanceSummary(document, staged), unresolved = profileInheritanceSelection(document, staged).missingRuleDependencies.length, issues = unresolved + peerIssueCount;
        dependencies.querySelector("h4").textContent = "Needs attention";
        dependencies.setAttribute("aria-label", `Needs attention, ${issues} issues`);
        if (!issues)
            dependencies.replaceChildren(Object.assign(globalThis.document.createElement("h4"), { textContent: "Needs attention" }), Object.assign(globalThis.document.createElement("p"), { textContent: "No issues need attention." }));
        sticky.className = "profile-inheritance-summary";
        sticky.setAttribute("aria-label", "Selection summary");
        const counts = globalThis.document.createElement("strong"), reviewPanel = globalThis.document.createElement("section");
        counts.textContent = `Effective properties ${totals.effective} · Exclusions ${totals.exclusions} · Issues ${issues}`;
        ...
        review.addEventListener("click", () => { reviewOpen = !reviewOpen; renderWorkspace(); focusAfter("[aria-label='Selection summary'] button"); });
        reviewPanel.hidden = !reviewOpen;
        reviewPanel.setAttribute("aria-label", "Reviewed inheritance selection");
        reviewPanel.textContent = `Source groups selected: ${staged.conceptSelections.join(", ") || "none"}. Applied membership is fixed. Fixed properties: ${staged.propertySelections.map((id) => document.nodes[id]?.name ?? id).join(", ") || "none"}. Exclusions: ${staged.excludedPropertyIds.map((id) => document.nodes[id]?.name ?? id).join(", ") || "none"}. Rule resolutions: ${[...staged.excludedRuleIds, ...staged.ruleReplacements.map(({ sourceRuleId }) => sourceRuleId), ...staged.presenceReplacements.map(({ sourceRuleId }) => sourceRuleId)].join(", ") || "none"}. Issues: ${issues}.`;
    ...
    updateCard();
    card.append(heading, summaryText, resolutionText, impactList, edit, workspace);
    host.append(card);
    return card;
}
