...
export function mountSelectiveProfileInheritance(options) {
    ...
    const renderWorkspace = () => {
        ...
        reviewPanel.textContent = `Synchronized concepts: ${staged.conceptSelections.join(", ") || "none"}. Fixed properties: ${staged.propertySelections.map((id) => document.nodes[id]?.name ?? id).join(", ") || "none"}. Exclusions: ${staged.excludedPropertyIds.map((id) => document.nodes[id]?.name ?? id).join(", ") || "none"}. Rule resolutions: ${[...staged.excludedRuleIds, ...staged.ruleReplacements.map(({ sourceRuleId }) => sourceRuleId), ...staged.presenceReplacements.map(({ sourceRuleId }) => sourceRuleId)].join(", ") || "none"}. Issues: ${issues}.`;
