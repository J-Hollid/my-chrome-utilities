...
export function unsupportedStandardSchemaRules(project) {
    const unsupported = [];
    for (const entries of Object.values(project.collections))
        for (const entity of entries ?? []) {
            const document = entity.canonicalSchema;
            if (!document)
                continue;
            for (const node of Object.values(document.nodes))
                for (const rule of node.rules) {
                    ...
                    if (reasons.length)
                        unsupported.push({ contributorId: entity.id, propertyId: node.id,
...
export async function exportExternalStandardSchema(input) {
    const production = await developerProductionSchemaExport(input.repository, input.projectId), published = await input.repository.loadCurrentPublishedProject(input.projectId);
    if (!published || published.revision !== production.projectRevision)
        throw new DOMException("The published project changed during external schema export. Retry the export.", "InvalidStateError");
...
//# sourceMappingURL=standard-schema-interoperability.js.map