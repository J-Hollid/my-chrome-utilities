...
export const pointerParts = (path) => path.split("/").slice(1).map(value => value.replaceAll("~1", "/").replaceAll("~0", "~"));
export const literalPattern = (value) => String(value ?? "").replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
/** Wrap an assertion at a payload path. Array item boundaries are explicit. */
export function assertionAt(parts, assertion, requirePath = false) {
    const [name, ...rest] = parts;
    if (name === undefined)
        return assertion;
    if (name === "*")
        return { items: assertionAt(rest, assertion, requirePath) };
    return { properties: { [name]: assertionAt(rest, assertion, requirePath) }, ...(requirePath ? { required: [name] } : {}) };
}
export function predicateSchema(document, predicate, base = []) {
    if (predicate.kind !== "predicate") {
        ...
        if (predicate.kind === "not")
            return { not: children.length === 1 ? children[0] : { allOf: children } };
    ...
    if (base.some((part, index) => full[index] !== part))
        throw new Error("This cross-array condition needs compatibility review.");
    ...
    switch (predicate.operator) {
        ...
        case "Equals":
            ...
            break;
        case "Does not equal":
            assertion = { not: { const: value } };
            break;
        case "Is one of":
            assertion = { enum: value };
            break;
        case "Starts with":
            assertion = { pattern: `^${literalPattern(value)}` };
            break;
        case "Contains":
            assertion = node.type === "array" ? { contains: { const: value } } : { pattern: literalPattern(value) };
            break;
        case "Contains any of":
            assertion = { contains: { enum: value } };
            break;
        case "Matches pattern":
            assertion = { pattern: String(value) };
            break;
        case "Greater than":
            assertion = { exclusiveMinimum: value };
            break;
        case "At least":
            assertion = { minimum: value };
            break;
        case "Less than":
            assertion = { exclusiveMaximum: value };
            break;
        case "At most":
            assertion = { maximum: value };
            break;
    }
