...
export function standardDocument(source, closeObjects) {
    ...
    for (const key of maps)
        if (value[key] && typeof value[key] === "object")
            result[key] = Object.fromEntries(Object.entries(value[key]).map(([name, child]) => [name, typeof child === "boolean" ? child : standardDocument(child, closeObjects)]));
    for (const key of singleSchemas)
        if (value[key] !== undefined)
            result[key] = typeof value[key] === "boolean" ? value[key] : standardDocument(value[key], closeObjects);
    for (const key of arraySchemas)
        if (Array.isArray(value[key]))
            result[key] = value[key].map(child => typeof child === "boolean" ? child : standardDocument(child, closeObjects));
    if (value.dependentRequired)
        result.dependentRequired = structuredClone(value.dependentRequired);
    if (source.type === "object" && closeObjects)
        result.additionalProperties = false;
    else if (source.additionalProperties !== undefined)
        result.additionalProperties = typeof source.additionalProperties === "boolean" ? source.additionalProperties : standardDocument(source.additionalProperties, closeObjects);
    if (source.forbidden?.length)
        result.not = { anyOf: source.forbidden.map(name => ({ required: [name] })) };
    return result;
}
//# sourceMappingURL=standard-document.js.map