...
import { eventFeedFilterDisplayState, inspectSavedEventFeedFilter, savedEventFeedFilterNameResult, } from "./data-layer-saved-event-feed-filters.js";
...
function renderActiveFilters(root, events, query, update) {
    ...
    for (const condition of query.conditions) {
        ...
        if (unavailable)
            text.textContent += " · Not observed in this feed";
...
function renderSavedFilters(events, query, controls) {
    const section = document.createElement("section");
    section.id = "saved-event-feed-filters";
    section.setAttribute("aria-label", "Saved event feed filters");
    const label = document.createElement("label");
    label.htmlFor = "saved-event-feed-filter-selector";
    label.textContent = "Saved filter";
    const selector = document.createElement("select");
    selector.id = "saved-event-feed-filter-selector";
    selector.append(Object.assign(document.createElement("option"), { value: "", textContent: "All events" }));
    for (const filter of controls.library.filters)
        selector.append(Object.assign(document.createElement("option"), {
            value: filter.id,
    ...
    identity.id = "saved-event-feed-filter-identity";
    identity.setAttribute("aria-live", "polite");
    identity.textContent = display.label;
    const feedback = document.createElement("output");
    feedback.id = "saved-event-feed-filter-feedback";
    feedback.setAttribute("aria-live", "polite");
    feedback.textContent = controls.feedback ?? "";
    const actions = document.createElement("details");
    actions.id = "saved-event-feed-filter-actions";
    ...
    const action = (text, run, disabled = false) => {
    ...
    nameDialog.id = "saved-event-feed-filter-name-dialog";
    const nameHeading = document.createElement("h5");
    nameHeading.tabIndex = -1;
    const nameLabel = document.createElement("label");
    nameLabel.htmlFor = "saved-event-feed-filter-name";
    nameLabel.textContent = "Saved filter name";
    const nameInput = document.createElement("input");
    nameInput.id = "saved-event-feed-filter-name";
    nameInput.autocomplete = "off";
    const nameAssistance = document.createElement("output");
    nameAssistance.id = "saved-event-feed-filter-name-assistance";
    nameAssistance.setAttribute("aria-live", "polite");
    const nameConfirm = button("Save");
    nameConfirm.disabled = true;
    const nameCancel = button("Cancel");
    let nameMode = "create";
    const refreshName = () => {
        ...
        nameAssistance.textContent = result.assistance;
        nameConfirm.disabled = !result.accepted;
    };
    const closeName = () => { nameDialog.close(); selector.focus({ preventScroll: true }); };
    const openName = (mode) => {
        nameMode = mode;
        nameHeading.textContent = mode === "rename" ? "Rename saved filter" : mode === "copy" ? "Save filter as new" : "Save current filter";
        nameConfirm.textContent = mode === "rename" ? "Rename" : "Save";
        nameInput.value = mode === "rename" ? active?.name ?? "" : "";
        refreshName();
    ...
    nameConfirm.addEventListener("click", () => {
        ...
        nameDialog.close();
        if (nameMode === "rename")
            controls.rename(result.name);
        else
            controls.create(result.name);
    });
    nameDialog.append(nameHeading, nameLabel, nameInput, nameAssistance, nameConfirm, nameCancel);
    if (display.save === "create")
        action("Save current filter", () => openName("create"));
    if (display.save === "update-or-copy") {
        action("Update", controls.update);
        action("Save as new", () => openName("copy"));
        action("Revert changes", controls.revert);
    }
    if (active) {
    ...
    deleteDialog.id = "saved-event-feed-filter-delete-dialog";
    ...
    const confirmDelete = button("Delete");
    ...
    deleteDialog.append(confirmDelete, cancelDelete);
    if (active)
        action("Delete", () => deleteDialog.showModal());
    const switchDialog = document.createElement("dialog");
    switchDialog.id = "saved-event-feed-filter-switch-dialog";
    switchDialog.append(Object.assign(document.createElement("h5"), { textContent: "Save modified filter before switching?" }));
    const saveSwitch = button("Save changes");
    const discardSwitch = button("Discard and switch");
    const cancelSwitch = button("Cancel");
    let pending = "";
    const switchToPending = () => controls.select(pending || undefined);
    saveSwitch.addEventListener("click", () => { switchDialog.close(); if (controls.update())
        switchToPending(); });
    ...
    switchDialog.addEventListener("cancel", (event) => { event.preventDefault(); cancelSwitch.click(); });
    switchDialog.append(saveSwitch, discardSwitch, cancelSwitch);
    selector.addEventListener("change", () => {
        pending = selector.value;
        if (display.modified && pending !== controls.activeFilterId) {
            selector.value = controls.activeFilterId ?? "";
...
export function renderEventFeedQueryBuilder(root, events, query, update, savedFilters) {
    ...
    root.append(...(savedFilters ? [renderSavedFilters(events, query, savedFilters)] : []), toolbar, renderActiveFilters(root, events, query, update), renderNoMatches(query, visibleCount, update), renderConditionEditor(events, query, update, add));
}
