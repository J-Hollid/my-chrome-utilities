...
function differences(actual, expected) { if (!expected)
    ...
    result.push(`issues: expected ${JSON.stringify(expected.issues)}, actual ${JSON.stringify(actual.issues)}`); if (expected.status !== undefined && status !== expected.status)
    result.push(`status: expected ${String(expected.status)}, actual ${status}`); if (expected.issueCodes !== undefined && JSON.stringify(issueCodes) !== JSON.stringify(expected.issueCodes))
