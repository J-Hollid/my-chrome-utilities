...
export function specificationPreflight(envelope) {
    ...
    for (const entity of envelope.project.collections.fixtures) {
        if (!entity.testType || entity.status === "Matched" || warnings.some(({ entityId, code }) => entityId === entity.id && code.startsWith("test-case-")))
            continue;
        const finding = guidedTestCaseFinding(entity);
        if (finding)
