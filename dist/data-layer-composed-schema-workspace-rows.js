...
function renderComposedSectionOwnership(host, row, context) {
    const local = Object.keys(row.local).some((key) => key !== "path"), inherited = Boolean(row.inherited), structureOwned = !row.inherited || Boolean(row.local.definitionId), visible = focusedSectionOwnershipActions({ local, inherited, structureOwned, overridden: row.action === "reset", invariant: row.effective.enforcement === "invariant", conflict: row.validationState === "blocked", replaceable: row.effective.enforcement === "overridable" })[context.activeSection], lifecycle = new Set(["Remove local", "Reset to parent"]);
    if (!visible.length)
        return;
