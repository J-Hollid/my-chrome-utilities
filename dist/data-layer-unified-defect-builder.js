...
function allowedValues(schema, pointer) {
    ...
    const rule = schema.attachedRules?.find((candidate) => templatePointer(candidate.propertyPath ?? "") === template && normalizedOperator(candidate.operator) === "allowed-values");
...
export function expectedPayloadFields(schema) {
    ...
    const visit = (definition, path, pointer, required) => {
        if (path)
            result.push({ path, pointer, type: definition.type ?? "value", required, schemaValues: allowedValues(schema, pointer) });
    ...
    if (schema.document.type === "object")
        visit(schema.document, "", "", true);
    else
        visit(schema.document, "value", "/value", true);
    return result;
}
...
export function expectedPayloadPresentation(eventName, payload) {
