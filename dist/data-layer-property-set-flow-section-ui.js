...
export function mountFlowSectionWorkspace({ host, state, flowId, id, onSave }) {
    ...
    for (const section of [...sections].sort((left, right) => left.order - right.order)) {
        ...
        rename.setAttribute("aria-label", `Section name ${section.name}`);
        for (const [control, label, value] of [[x, "Section x", section.bounds.x], [y, "Section y", section.bounds.y], [width, "Section width", section.bounds.width], [height, "Section height", section.bounds.height]]) {
            control.type = "number";
            control.value = String(value);
            control.setAttribute("aria-label", `${label} ${section.name}`);
        }
        for (const control of [save, move, remove, destructive])
            control.type = "button";
        save.textContent = "Save name and size";
        move.textContent = "Move Section";
        remove.textContent = "Remove Section and retain Page frames";
        destructive.textContent = "Review remove Section with contents";
        review.hidden = true;
