...
export function renderDocumentationTheme(input) {
    ...
    const sampleHost = document.createElement("section"), drawSample = (sampleTheme) => { sampleHost.replaceChildren(); sampleHost.dataset.themeSample = "true"; const table = input.sampleTable ?? { id: "theme-sample", title: "Selected section sample", headings: ["Property", "Description"], rows: [["page_name", "Page name"], ["event_name", "Observed event"]] }; if (sampleTheme.logo)
    ...
    const save = button("Save theme", () => { try {
        input.saveTheme(read(), "Save project documentation theme");
    }
    catch (error) {
        ...
        copyOutput.textContent = error instanceof Error ? error.message : String(error);
    } }), copy = button("Copy structured theme values", () => { void input.ports.writePlain(serializeProjectDocumentationTheme(read())).then(() => copyOutput.textContent = "Structured theme values copied."); }), pasteButton = button("Paste as new project-local theme", () => { try {
        const next = parseProjectDocumentationTheme(paste.value, { id: `documentation-theme:${crypto.randomUUID()}`, name: `${theme.name} copy` }), records = input.documentation(), nextSet = createProjectDocumentationSet({ ...set, themeId: next.id });
        input.persist({ sets: records.sets.map((candidate) => candidate.id === set.id ? nextSet : candidate), themes: [...records.themes, next] }, "Paste project-local documentation theme");
    }
    catch (error) {
        copyOutput.textContent = error instanceof Error ? error.message : String(error);
    } });
