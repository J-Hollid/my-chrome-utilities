...
export function mountComposedSchemaWorkspace(options) {
    ...
    const overrideRule = (sourceId) => { if (!draft)
        ...
        return; const id = `rule:${crypto.randomUUID()}`, replacement = { ...structuredClone(source), id, provenance: { source: "created", state: "overridden", sourceId } }; draft = { ...draft, rules: [...draft.rules, replacement] }; overriddenRuleIds.add(id); rerender(); };
