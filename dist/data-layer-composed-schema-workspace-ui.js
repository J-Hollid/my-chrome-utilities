...
export function composedTableQuickEditFacets(row, facet, value) {
    ...
    if (facet === "concept")
        ...
        const values = schemaTableStageAllowedValues(next.allowedValues, value, (next.type ?? row.effective.type)), reconciled = reconcileComposedAllowedValues(next, values);
...
export function mountComposedSchemaWorkspace(options) {
    ...
    const stageStructure = (command) => { if (overlayOpen && activePath) {
        pendingStructure.push(command);
        rerender();
        return;
    ...
    const beginExclusion = (row, action, focus) => { open(row, focus); focusedOpen = true; pendingAction = action; rerender(); };
    ...
    rerender();
    section.append(heading, summary, headerActions, inheritedSelectionHost, policyLabel, quickEditFeedback, filterControls, addControls, rows, panel);
