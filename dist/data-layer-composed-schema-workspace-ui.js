    ...
    const renderPanel = (mode, origin) => {
        ...
        if (mode === "local") {
            ...
            if (options.model.localChangeCount) {
                const resetAll = button("Reset all to parents", () => { const review = document.createElement("section"), description = document.createElement("p"), confirm = button("Confirm reset all", () => { workspacePanels.set(viewKey, "local"); workspacePanelFocus.set(viewKey, { mode: "local" }); options.onResetAllLocalChanges?.(); }), cancel = button("Cancel", () => { review.remove(); resetAll.focus(); }); description.textContent = `Review removal of all ${options.model.localChangeCount} local items. Current parent values will become effective.`; review.setAttribute("aria-label", "Review reset all local changes"); review.append(description, confirm, cancel); panel.append(review); confirm.focus(); });
