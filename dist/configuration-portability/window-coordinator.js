...
export function createConfigurationWindowCoordinator(input) {
    ...
    let releaseLease, leaseReady, frozen = false, activeRequest, overlay, recoveryReload;
    const previousInert = new Map();
    const reload = input.reload ?? (() => globalThis.location.reload());
    const acquireLease = () => {
        if (!locks)
            throw new DOMException("This browser cannot pause open configuration windows. Close other windows and use a supported browser.", "NotSupportedError");
        if (leaseReady)
            return leaseReady;
        leaseReady = new Promise((resolve, reject) => {
            void locks.request(LOCK_NAME, { mode: "shared" }, () => new Promise((release) => {
    ...
    const show = () => {
        ...
        for (const child of Array.from(input.document.body.children))
    ...
    const pause = async (id) => {
        ...
        show();
        if (id !== ownId)
            recoveryReload = setTimeout(reload, RECOVERY_RELOAD_MS);
        await input.settle();
        releaseLease?.();
        releaseLease = undefined;
        leaseReady = undefined;
    };
    ...
    channel?.addEventListener("message", (event) => {
        ...
        if (!message || message.id === ownId)
            return;
        if (message.type === "pause")
            void pause(message.id).catch(() => { });
        if (message.type === "resume")
            void resume(message.id).catch(() => { });
    ...
    return {
        ...
        async run(work) {
            if (!locks || !channel)
                throw new DOMException("This browser cannot pause all open configuration windows. Setup was not started.", "NotSupportedError");
            const id = ownId, timeout = new AbortController(), timer = setTimeout(() => timeout.abort(), PAUSE_TIMEOUT_MS);
            channel.postMessage({ type: "pause", id });
            try {
                await pause(id);
                let committed = false;
                ...
                if (committed) {
                    channel.postMessage({ type: "reload", id });
                    setTimeout(reload, 300);
                }
                return result;
            }
            catch (error) {
                channel.postMessage({ type: "resume", id });
                await resume(id);
                throw error;
            }
            finally {
...
//# sourceMappingURL=window-coordinator.js.map