import { createCompleteConfigurationArchive, inspectCompleteConfigurationArchive, COMPLETE_CONFIGURATION_EXCLUSIONS } from "./archive-format.js";
import { stageCompleteConfigurationSetup } from "./configuration-repository.js";
...
const element = (root, selector) => {
    ...
    if (!found)
        throw new Error(`Missing complete configuration control ${selector}`);
    return found;
};
...
export function mountCompleteConfigurationSetup(root, port, routes = {}) {
    const exportButton = element(root, "#export-complete-configuration");
    const cancelExport = element(root, "#cancel-complete-configuration-export");
    const importButton = element(root, "#import-complete-configuration");
    const file = element(root, "#import-complete-configuration-file");
    const dialog = element(root, "#complete-configuration-review");
    const summary = element(root, "#complete-configuration-review-summary");
    const policy = element(root, "#complete-configuration-conflict-policy");
    const setup = element(root, "#setup-from-configuration");
    const cancel = element(root, "#cancel-configuration-setup");
    const status = element(root, "#complete-configuration-status");
    const exportPreview = element(root, "#complete-configuration-export-preview");
    let staged;
    let legacyRecovery = false;
    let busy = false, setupInProgress = false, operation, exportOperation;
    const setBusy = (value) => { busy = value; exportButton.disabled = value; importButton.disabled = value; setup.disabled = value; };
    const onExport = () => {
        if (busy || exportOperation)
            return;
        setBusy(true);
        cancelExport.hidden = false;
        cancelExport.disabled = false;
        const controller = exportOperation = new AbortController();
        status.textContent = "Reading saved configuration…";
        void port.read({ signal: controller.signal, onProgress: (completed, total) => {
                if (!controller.signal.aborted)
                    status.textContent = `Reading saved project ${completed} of ${total}…`;
            } }).then((snapshot) => {
            controller.signal.throwIfAborted();
            exportPreview.textContent = `Included: ${Object.entries(snapshot.sections).map(([domain, records]) => `${domain} ${records.length}`).join(", ")}; binary bodies ${snapshot.bodies.length}. Excluded: ` +
            ...
            return createCompleteConfigurationArchive(snapshot, { buildIdentity: port.buildIdentity,
                signal: controller.signal, onProgress: (written) => {
                    if (!controller.signal.aborted)
            ...
            .then((archive) => {
            if (controller.signal.aborted) {
                status.textContent = "Complete configuration export cancelled; no file was created.";
                return;
            }
            downloadArchive(archive);
            status.textContent = "Exported complete configuration with manifest, digests, dependencies, and exclusions.";
        }, (error) => {
            status.textContent = controller.signal.aborted ? "Complete configuration export cancelled; no file was created." :
    ...
    const onCancelExport = () => {
        if (!exportOperation)
            return;
        exportOperation.abort();
        cancelExport.disabled = true;
        status.textContent = "Cancelling complete configuration export…";
    };
    const onImport = () => file.click();
    const showReview = async (snapshot, sourceLabel, size, excluded) => {
        ...
        const conflicts = staged.conflicts.map(({ domain, id, recipientId, reason }) => `${domain}/${id} (${reason} conflict with ${recipientId})`).join("; ");
        ...
        void inspectConfigurationImportRoute(selected).then(async (routing) => {
            if (routing.route !== "complete-configuration") {
                ...
                if (routing.route === "repository-recovery" && !routing.blockers.length) {
                    ...
                    await showReview(snapshot, "Legacy recovery JSON: projects and saved schemas only", selected.size, ["domains absent from the old recovery file", "binary assets absent by format"]);
                    return;
                }
            ...
            await showReview(inspected.snapshot, `Compatible version ${inspected.manifest.version} archive`, selected.size, inspected.manifest.exclusions.map(({ id }) => id));
    ...
    const onSetup = () => {
        ...
        const selected = (staged.conflicts.length === 0 ? "replace-all" : policy.value);
        ...
        void staged.commit(selected, { signal: operation.signal, onProgress: (completed, total) => {
                status.textContent = `Staging project ${completed} of ${total}…`;
            } }).then((result) => {
            dialog.close();
            staged = undefined;
    ...
    const onCancel = () => {
        if (setupInProgress) {
            operation?.abort();
            status.textContent = "Cancellation requested. Waiting for the storage result…";
            return;
        }
        dialog.close();
        staged = undefined;
        status.textContent = "Configuration setup cancelled; no data changed.";
    };
    exportButton.addEventListener("click", onExport);
    cancelExport.addEventListener("click", onCancelExport);
    ...
    return () => {
        ...
        cancelExport.removeEventListener("click", onCancelExport);
        importButton.removeEventListener("click", onImport);
        file.removeEventListener("change", onFile);
        setup.removeEventListener("click", onSetup);
