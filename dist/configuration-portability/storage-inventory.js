...
export function assertKnownConfigurationStorage(storage, includedKeys) {
    if (!storage)
        return;
    const known = new Set([...includedKeys, ...EXCLUDED_KEYS, ...ENVELOPES]);
    const check = (key) => {
        if (key.startsWith("my-chrome-utilities.") && !known.has(key))
            throw new DOMException(`Saved configuration key ${key} has no export rule. Update the domain inventory before export.`, "DataError");
    };
    for (let index = 0; index < storage.length; index += 1) {
        const key = storage.key(index);
        if (key && storage.getItem(key) !== null)
            check(key);
    }
    for (const envelope of ENVELOPES) {
        ...
        try {
            ...
            throw new DOMException(`Saved configuration envelope ${envelope} is unreadable.`, "DataError");
        }
        if (!values || typeof values !== "object" || Array.isArray(values))
            throw new DOMException(`Saved configuration envelope ${envelope} is invalid.`, "DataError");
