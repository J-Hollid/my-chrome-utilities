...
flowGraphBuilder = installFlowGraphBuilder({ context: flowBuilderContext, persist, id, loadVisualBody: async (projectId, assetId) => { const body = await durableProjectRuntime.repository.loadConceptVisualAssetBody(projectId, assetId); return await new Promise((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = () => reject(reader.error); reader.readAsDataURL(body); }); }, openOccurrenceSchema: (occurrenceId, path, originFocus) => layeredSchemaUi?.openGraphOccurrenceSchema(occurrenceId, path, originFocus) ?? false });
