...
import { guidedAssignmentConditionKinds, testAssignmentRouting } from "./data-layer-assignment-routing.js";
...
function renderAssignmentWorkspaceEditor(host, assignment) {
    if (!state)
        return;
    const section = document.createElement("section"), heading = document.createElement("h2"), form = document.createElement("form"), name = document.createElement("input"), region = (label) => { const fieldset = document.createElement("fieldset"), legend = document.createElement("legend"); fieldset.setAttribute("aria-label", label); legend.textContent = label; fieldset.append(legend); return fieldset; };
    ...
    form.append(labeledControl("Assignment name", name));
    ...
    renderTargets();
    schemaTarget.append(labeledControl("Contributor kind", targetKind), labeledControl("Stable contributor target", target));
    ...
    observed.append(labeledControl("Source", source), labeledControl("Event", eventSelect), labeledControl("Payload or raw-input validation target", validationTarget));
    ...
    let guidedCondition;
    const typed = (type, value) => type === "number" ? Number(value) : type === "boolean" ? value === "true" : type === "null" ? null : value;
    ...
    function renderGuidedCondition() {
        ...
        if (guided.hidden)
            return;
        const descriptor = guidedAssignmentConditionKinds.find(({ kind }) => kind === kindSelect.value) ?? guidedAssignmentConditionKinds[0], comparison = document.createElement("select"), value = document.createElement("input"), type = document.createElement("select"), parameter = document.createElement("input"), property = document.createElement("select");
        comparison.setAttribute("aria-label", `${descriptor.kind} comparison`);
        for (const option of descriptor.comparisons)
            comparison.append(new Option(option, option));
        ...
        for (const option of ["string", "number", "boolean", "null"])
            type.append(new Option(option, option));
        ...
        if (descriptor.kind === "Environment") {
            ...
            guidedControls.append(labeledControl("Configured environment", environment), labeledControl("Comparison", comparison));
            const update = () => { guidedCondition = { kind: "predicate", field: "environment", operator: comparison.value, value: environment.value }; preview.textContent = `Environment ${comparison.value} ${environment.value}.`; };
            ...
            if (descriptor.kind === "Query")
                guidedControls.append(labeledControl("Parameter name", parameter));
            if (descriptor.kind === "Context data")
                guidedControls.append(labeledControl("Schema property", property));
            guidedControls.append(labeledControl(`${descriptor.kind} comparison`, comparison));
            if (descriptor.valueKind === "typed" || descriptor.valueKind === "schema-property")
                guidedControls.append(labeledControl("Value type", type));
            guidedControls.append(labeledControl(descriptor.guidedInput, value));
            const update = () => { const field = descriptor.kind === "Query" ? `query.${parameter.value.trim()}` : descriptor.kind === "Context data" ? property.value : descriptor.field, operator = comparison.value, raw = value.value, condition = { kind: "predicate", field, operator, ...(operator === "matches pattern" ? { pattern: raw } : !["exists", "does not exist"].includes(operator) ? { value: typed(type.value, raw) } : {}) }; guidedCondition = condition; preview.textContent = `${descriptor.kind}${field ? ` ${field}` : ""} ${operator}${["exists", "does not exist"].includes(operator) ? "" : ` ${JSON.stringify(condition.value ?? condition.pattern)}`}.`; };
            for (const control of [parameter, property, comparison, type, value])
    ...
    applicability.append(labeledControl("Reusable Applicability Set or new condition", setSelect), guided);
    ...
    resolution.append(labeledControl("Priority", priority), guidance);
    const routing = region("Test assignment routing"), testSource = document.createElement("select"), testEvent = document.createElement("select"), pathname = document.createElement("input"), run = document.createElement("button"), result = document.createElement("section");
    for (const value of sources)
    ...
    run.addEventListener("click", () => { const event = state.project.collections.events.find(({ id }) => id === testEvent.value), evaluated = testAssignmentRouting(state.project, { sourceId: testSource.value, eventName: String(event?.eventName ?? event?.name ?? ""), pathname: pathname.value, payload: {} }), summary = document.createElement("p"), list = document.createElement("ul"); summary.textContent = evaluated.summary; for (const candidate of evaluated.candidates) {
        const item = document.createElement("li");
        item.dataset.assignmentCandidate = candidate.assignmentId;
        item.textContent = `${candidate.name}: ${candidate.event.evidence}; ${candidate.applicability.evidence}; ${candidate.resolution}`;
        list.append(item);
    } result.replaceChildren(summary, list); });
    routing.append(labeledControl("Observed source", testSource), labeledControl("Observed Event", testEvent), labeledControl("Observed pathname", pathname), run, result);
...
function renderSelectedEntityEditor(content, entity) {
    ...
    if (selectedKind === "assignments") {
        renderAssignmentWorkspaceEditor(content, entity);
        return;
    }
