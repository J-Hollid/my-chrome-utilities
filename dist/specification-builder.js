...
layeredSchemaUi = installLayeredSchemaUi({ context: () => { const searchActive = Boolean(q("#project-search").value.trim()), flowRouteId = !documentationOpen && !projectOverview && !creationKind && !removalReview && !searchActive && selectedKind === "flows" ? selectedId : undefined; return { ...state ? { state } : {}, kind: selectedKind, ...(selectedId ? { entityId: selectedId } : {}), ...(flowRouteId ? { flowRouteId } : {}) }; }, persist, onUndo: () => { if (state)
        void durableProjectRuntime.undo(state.project.id); }, onRedo: () => { if (state)
        void durableProjectRuntime.redo(state.project.id); } });
