import { studioCanonicalExportSource } from "./schema-context-export/studio-options.js";
import { projectContextExportSource } from "./schema-context-export/project-source.js";
...
const mountCanonicalSchemaEditor = (options) => mountCanonicalSchemaEditorBase({ ...options, contextExport: studioCanonicalExportSource(options, () => saveStatus.kind !== "idle"), conceptSuggestions: () => state ? projectCanonicalConcepts(state) : [] });
...
function renderComposedSchemaWorkspace(host, entity, kind, scope, pageGroupApplicabilitySetIds) {
    ...
    if (!model.inheritedPropertySelection.totalCount && !model.rows.some(({ inherited }) => Boolean(inherited))) {
    ...
    mountComposedSchemaWorkspace({ host: region, model, contextExport: () => projectContextExportSource(liveState(), liveEntity(), scope, undefined, saveStatus.kind !== "idle"), effectiveText: (row) => effectivePropertySummary(row.effective), conceptSuggestions: () => projectCanonicalConcepts(liveState()), schemaContributorId: entity.id, schemaContributorScope: scope, onlyDefinedFields: (canonical?.onlyDefinedFields ?? entity.onlyDefinedFields) === true, onOnlyDefinedFields: (value) => persistComposed(kind === "events" ? saveComposedEntitySchemaPolicy(liveState(), kind, entity.id, value) : saveComposedSchemaPolicy(liveState(), kind, entity.id, value)), onSave: (row, facets, structures = []) => persistComposed(saveComposedSchemaLocalFacetsAndStructures(liveState(), kind, entity.id, row.path, facets, structures, id)), onReset: (row) => persistComposed(resetComposedSchemaLocalProperty(liveState(), kind, entity.id, row.path)), ...(kind === "pages" ? { onApplyInheritedPropertySelection: (selectedPropertyIds) => persistComposed(applyComposedSchemaInheritedPropertySelection(liveState(), "pages", entity.id, selectedPropertyIds)) } : {}), onResetLocalFacet: (path, facet) => persistComposed(resetComposedSchemaLocalFacet(liveState(), kind, entity.id, path, facet)), onResetLocalRule: (path, ruleId) => persistComposed(resetComposedSchemaLocalRule(liveState(), kind, entity.id, path, ruleId)), onResetAllLocalChanges: () => persistComposed(resetComposedSchemaLocalChanges(liveState(), kind, entity.id)), onIncludeParentAdditions: (selections) => persistComposed(includeComposedSchemaParentAdditionSelections(liveState(), kind, entity.id, selections)), onStructure: () => { }, onRepair: (repair) => { const row = model.rows.find((candidate) => candidate.repairs.includes(repair) || candidate.decisions?.some(({ repairs }) => repairs.includes(repair))); if (repair.kind === "use-source" && repair.facet && row) {
            ...
            return;
        } const match = ['pages', 'propertySets', 'profiles', 'events'].find((collection) => state.project.collections[collection].some(({ id }) => id === repair.contributorId)); if (match) {
            selectedKind = match;
...
const editorFields = {
    profiles: [], pages: [],
    propertySets: [{ key: "description", label: "Description", type: "textarea" }, { key: "profileIds", label: "Shared Profile sources", collection: "profiles", multiple: true }],
...
const guidedPageEvaluatorRevision = (current, pageId) => pagePropertySetEvaluatorRevision(current.project, pageId);
    ...
    const render = () => {
        ...
        const renderDescriptor = (descriptor, concretePath = descriptor.path) => {
            ...
            if (descriptor.control === "array") {
                ...
                items.forEach((_item, index) => { const item = document.createElement("fieldset"), itemLegend = document.createElement("legend"), remove = document.createElement("button"), itemId = stableIds[index], reorder = renderReorderControl({ focusScopeId: `guided-array:${concretePath}`, itemId, itemLabel: `Item ${index + 1}`, completeOrder: items.map((_candidate, candidateIndex) => ({ id: stableIds[candidateIndex], label: `Item ${candidateIndex + 1}` })), dropTarget: item, focusScope: host, preserveTargetSemantics: true, onMove: ({ fromIndex, toIndex }) => { options.update(guidedArrayMove(options.input(), concretePath, fromIndex, toIndex)); identities.move(fromIndex, toIndex); render(); return true; } }); itemLegend.textContent = `Item ${index + 1}`; remove.type = "button"; remove.textContent = "Remove item"; remove.addEventListener("click", () => { const next = [...items]; next.splice(index, 1); identities.remove(index); update(concretePath, next); }); const contents = children.length ? children.map(child => renderDescriptor(child, child.path.replace(`${descriptor.path}/*`, `${concretePath}/${index}`))) : (() => { const itemType = String(descriptor.constraints.itemType ?? "string"), synthetic = { ...descriptor, path: `${descriptor.path}/*`, control: itemType === "number" || itemType === "integer" ? "number" : itemType === "boolean" ? "boolean" : "text", jsonTypes: [itemType], required: true }; return [renderScalar(synthetic, `${concretePath}/${index}`)]; })(); item.append(itemLegend, renderReorderableItemRow({ control: reorder, primaryContent: contents[0], trailingContent: remove, secondaryContent: contents.slice(1) })); group.append(item); });
                const add = document.createElement("button");
                add.type = "button";
...
function renderGuidedTestCaseEditor(content, entity) {
    ...
    const expectations = document.createElement("fieldset"), expectationLegend = document.createElement("legend"), winner = document.createElement("select"), propertySets = document.createElement("select"), outcome = document.createElement("select"), issueChoices = document.createElement("fieldset");
    ...
    propertySets.name = "expectedPageGroups";
    propertySets.multiple = true;
    for (const group of state.project.collections.propertySets) {
    ...
    expectations.append(expectationLegend, labeledControl("Expected winning Assignment", winner), labeledControl("Expected applicable Property Sets", propertySets), labeledControl("Expected validation outcome", outcome), issueChoices);
    ...
    for (const difference of renderedCase.differences) {
        ...
        repair.addEventListener("click", () => { const target = difference.field === "winner" ? winner : difference.field === "outcome" ? outcome : difference.field === "applicablePageGroups" ? propertySets : issueChoices.querySelector("input"); target?.focus(); });
    ...
    const readExpectations = () => { const issues = Array.from(issueChoices.querySelectorAll("input:checked"), ({ value }) => { const [path, code] = value.split("\t"); return { path: path, code: code }; }); return { ...(winner.value ? { winner: winner.value } : {}), ...(type.value === "page-context" && propertySets.selectedOptions.length ? { applicablePageGroups: Array.from(propertySets.selectedOptions, ({ value }) => value) } : {}), ...(outcome.value ? { outcome: outcome.value } : {}), ...(issues.length ? { issues } : {}) }; };
...
function renderProfileInheritanceCards(host, entity) {
    if (!state || !(selectedKind === "pages" || selectedKind === "propertySets" || selectedKind === "events"))
        return;
    const kind = selectedKind, pending = pendingProfileSource?.targetId === entity.id ? [pendingProfileSource.profileId] : [], profileIds = [...new Set([...(entity.profileIds ?? []), ...(typeof entity.profileId === "string" ? [entity.profileId] : []), ...pending])], stored = entity.profileInheritanceRecipes ?? [], scope = kind === "pages" ? "Page" : kind === "propertySets" ? "Property Set" : "Event";
    for (const profileId of profileIds) {
        ...
        if (!profile || !canonical)
            continue;
        const recipe = stored.find((candidate) => candidate.profileId === profileId) ?? createProfileInheritanceRecipe({ id: id("inheritance-recipe"), profileId, targetId: entity.id, startingPoint: "everything", sourceRevision: canonical.revision }), copySources = ['pages', 'propertySets', 'events'].flatMap((kind) => state.project.collections[kind].flatMap((candidate) => candidate.id === entity.id ? [] : (candidate.profileInheritanceRecipes ?? []).filter((candidateRecipe) => candidateRecipe.profileId === profileId).map((candidateRecipe) => ({ label: `${candidate.name} (${labels[kind]})`, recipe: candidateRecipe }))));
...
function renderSelectedEntityEditor(content, entity) {
    ...
    form.addEventListener("submit", (event) => {
        ...
        try {
            ...
            if (selectedKind === "propertySets" || selectedKind === "events") {
...
function renderCollectionGuidance(content) { if (!state)
    return; const entityName = { profiles: "Profile", applicabilitySets: "Applicability Set", pages: "Page", propertySets: "Page", events: "Event", flows: "Flow", assignments: "Assignment", fixtures: "Fixture" }, name = entityName[selectedKind], section = document.createElement("section"); section.className = "project-guidance"; if (name) {
...
function renderProjectEntityWorkspace(content, kind, entity) { if (!state)
    ...
    members.setAttribute("aria-label", "Derived Property Set members");
    ...
    renderComposedSchemaWorkspace(workspace, entity, "propertySets", "Property Set");
} if (kind === "events") {
    renderProfileInheritanceEditor(workspace, entity);
...
function renderAssuranceFindings(host, result) {
    const repairRoute = (finding) => {
        const fieldMatch = finding.field.match(/^collections\.(profiles|propertySets|pages|events|flows|assignments|fixtures)(?:\/([^/]+))?/), fieldKind = fieldMatch?.[1];
        if (fieldKind)
...
projectDocumentationWorkspaceUi = installProjectDocumentationWorkspaceContribution({ assetBodies: { storeProjectAssetBody: (identity, body) => { durableProjectRuntime.stageProjectAssetBody(identity, body); return Promise.resolve(); }, loadProjectAssetBody: identity => durableProjectRuntime.repository.loadProjectAssetBody(identity), deleteProjectAssetBody: identity => { durableProjectRuntime.discardStagedProjectAssetBody(identity); return Promise.resolve(); } }, state: () => state, revision: () => canonicalRevision, save: (documentation, label) => { if (!state)
...
flowGraphBuilder = installFlowGraphBuilder({ context: flowBuilderContext, persist, settled: () => durableProjectRuntime.settled("project"), id, repository: durableProjectRuntime.repository, openOccurrenceSchema: (occurrenceId, path, originFocus) => layeredSchemaUi?.openGraphOccurrenceSchema(occurrenceId, path, originFocus) ?? false });
...
layeredSchemaUi = installLayeredSchemaUi({ pending: () => saveStatus.kind !== "idle", context: () => { const searchActive = Boolean(q("#project-search").value.trim()), flowRouteId = !documentationOpen && !projectOverview && !creationKind && !removalReview && !searchActive && selectedKind === "flows" ? selectedId : undefined; return { ...state ? { state } : {}, kind: selectedKind, ...(selectedId ? { entityId: selectedId } : {}), ...(flowRouteId ? { flowRouteId } : {}) }; }, persist, onUndo: () => { if (state)
        void durableProjectRuntime.undo(state.project.id); }, onRedo: () => { if (state)
        void durableProjectRuntime.redo(state.project.id); } });
