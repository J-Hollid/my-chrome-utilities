...
import { createProfileInheritanceRecipe, markProfileInheritanceTargetStale, profileInheritanceRecipeApplied } from "./data-layer-selective-profile-inheritance.js";
...
const markProfileConsumersForSourceChange = (project, profileId, before, after) => ({ ...project, collections: { ...project.collections, pages: project.collections.pages.map((entity) => markProfileInheritanceTargetStale(entity, profileId, before, after)), pageGroups: project.collections.pageGroups.map((entity) => markProfileInheritanceTargetStale(entity, profileId, before, after)), events: project.collections.events.map((entity) => markProfileInheritanceTargetStale(entity, profileId, before, after)) } });
...
function renderProfileInheritanceCards(host, entity) {
    if (!state || !(selectedKind === "pages" || selectedKind === "pageGroups" || selectedKind === "events"))
        return;
    const profileIds = [...new Set([...(entity.profileIds ?? []), ...(typeof entity.profileId === "string" ? [entity.profileId] : [])])], stored = entity.profileInheritanceRecipes ?? [], scope = selectedKind === "pages" ? "Page" : selectedKind === "pageGroups" ? "Page Group" : "Event", previewModel = composedSchemaWorkspace(state, entity, scope), compositionPreview = { sources: profileIds.flatMap((profileId) => { const profile = state.project.collections.profiles.find(({ id }) => id === profileId); return profile ? [profile.name] : []; }), rows: previewModel.rows.map((row) => ({ path: row.path, source: row.source, effective: effectivePropertySummary(row.effective) })), conflicts: previewModel.conflictSummary, status: previewModel.status };
