...
export function mountComposedSchemaFacetBuilder(options) {
    ...
    const render = () => {
        ...
        options.host.setAttribute("aria-label", `${options.path} complete facet builders`);
        const common = document.createElement("fieldset"), commonLegend = document.createElement("legend"), type = document.createElement("select"), presence = document.createElement("select"), expected = document.createElement("input"), documentation = document.createElement("textarea");
        commonLegend.textContent = `Common facets for ${options.path}`;
        ...
        type.addEventListener("change", () => { draft = { ...draft, type: type.value || undefined }; });
        presence.addEventListener("change", () => { draft = { ...draft, presence: presence.value || undefined }; });
        expected.addEventListener("change", () => { try {
            draft = { ...draft, expectedValue: expected.value === "" ? undefined : typedComposedValue(type.value || options.effective.type, expected.value) };
            feedback = "";
        }
        catch (error) {
        ...
        documentation.addEventListener("input", () => { draft = { ...draft, documentation: documentation.value }; });
        common.append(commonLegend, labeled("Type", type), labeled("Presence", presence), labeled("Expected value", expected), labeled("Documentation", documentation));
        const allowed = document.createElement("fieldset"), allowedLegend = document.createElement("legend"), allowedRows = document.createElement("div");
        allowed.setAttribute("aria-label", "Composed allowed values builder");
        allowedLegend.textContent = "Allowed values";
        draft.allowedValues.forEach((entry, index) => { const row = document.createElement("div"), value = document.createElement("input"); value.value = String(entry ?? ""); value.setAttribute("aria-label", `Allowed value ${index + 1}`); value.addEventListener("change", () => { try {
            const next = clone(draft.allowedValues);
            next[index] = typedComposedValue(type.value || options.effective.type, value.value);
            draft = { ...draft, allowedValues: next };
            feedback = "";
        }
        catch (error) {
            feedback = error instanceof Error ? error.message : String(error);
            render();
        } }); row.append(labeled(`Value ${index + 1}`, value), button("Remove", () => { draft = removeComposedAllowedValue(draft, index); render(); }), button("Move up", () => { draft = moveComposedAllowedValue(draft, index, -1); render(); }), button("Move down", () => { draft = moveComposedAllowedValue(draft, index, 1); render(); })); allowedRows.append(row); });
        allowed.append(allowedLegend, allowedRows, button("Add allowed value", () => { const defaultValue = type.value === "number" || type.value === "integer" ? 0 : type.value === "boolean" ? false : type.value === "null" ? null : ""; draft = addComposedAllowedValue(draft, defaultValue); render(); }));
        const condition = document.createElement("fieldset"), conditionLegend = document.createElement("legend"), target = document.createElement("select"), property = document.createElement("select"), operator = document.createElement("select"), conditionValue = document.createElement("input"), plain = document.createElement("output"), branches = document.createElement("ul"), observation = document.createElement("textarea"), testCondition = document.createElement("button"), testResult = document.createElement("output");
        condition.setAttribute("aria-label", "Composed condition builder");
        ...
        const addGroup = (kind) => { try {
            draft = addComposedConditionGroup(draft, path(), kind);
            feedback = "";
        }
        catch (error) {
            feedback = error instanceof Error ? error.message : String(error);
        ...
        for (const value of ["pattern", "range", "cardinality", "condition", "custom"])
            ruleKind.append(option(value));
        for (const value of ["error", "warning"])
            severity.append(option(value));
        for (const [name, control] of [["Minimum", minimum], ["Maximum", maximum], ["Minimum items", minItems], ["Maximum items", maxItems]]) {
            control.type = "number";
        ...
        exampleLegend.textContent = "Example selection";
        method.append(option("blank", "Blank"), option("allowed-value", "Allowed value"), option("custom", "Custom typed value"));
        method.value = draft.exampleMethod;
        allowedExample.append(option("", "Choose allowed value"), ...draft.allowedValues.map((value, index) => option(String(index), String(value))));
        if (draft.exampleMethod === "allowed-value") {
            const index = draft.allowedValues.findIndex((value) => same(value, draft.exampleValue));
            allowedExample.value = index < 0 ? "" : String(index);
        }
        customExample.value = draft.exampleMethod === "custom" ? String(draft.exampleValue ?? "") : "";
        method.addEventListener("change", () => { draft = { ...draft, exampleMethod: method.value, ...(method.value === "blank" ? { exampleValue: undefined } : {}) }; render(); });
        allowedExample.addEventListener("change", () => { draft = { ...draft, exampleMethod: "allowed-value", exampleValue: clone(draft.allowedValues[Number(allowedExample.value)]) }; });
        customExample.addEventListener("change", () => { try {
            draft = { ...draft, exampleMethod: "custom", exampleValue: typedComposedValue(type.value || options.effective.type, customExample.value) };
            feedback = "";
        }
        catch (error) {
            feedback = error instanceof Error ? error.message : String(error);
