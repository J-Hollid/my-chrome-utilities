...
export function mountComposedSchemaFacetBuilder(options) { let draft = composedFacetDraft(options.local, options.effective), feedback = ""; const allowedValueIdentities = new StableIdentitySequence("composed-allowed-value"); if (draft.allowedValueIds?.length === draft.allowedValues.length)
    allowedValueIdentities.replace(draft.allowedValueIds); const render = () => { options.host.replaceChildren(); options.host.setAttribute("aria-label", `${options.path} complete facet builders`); const context = { options, draft: () => draft, setDraft: (next) => { draft = next; }, feedback: () => feedback, setFeedback: (value) => { feedback = value; }, render, allowedValueIdentities }, save = button("Save local facets", () => { try {
