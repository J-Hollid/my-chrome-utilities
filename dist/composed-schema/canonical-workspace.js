...
const ownershipProvenance = (row, entity, itemId, raw, localIds) => {
    ...
    return itemProvenance(raw, { source: "path-constraint", ...(origin?.contributorId ? { sourceId: origin.contributorId, contributorId: origin.contributorId } : {}), ...(origin?.contributorName ? { contributorName: origin.contributorName } : {}), ...(origin?.scope ? { scope: origin.scope } : {}), state: local ? "local" : raw && typeof raw === "object" && raw.state === "shadowed" ? "shadowed" : "inherited" });
};
...
export function composedCanonicalSchema(state, entity, scope) {
    ...
    for (const row of rows) {
        const segments = row.path.split("/").filter(Boolean), parentPath = `/${segments.slice(0, -1).join("/")}`, parentId = segments.length > 1 ? byPath.get(parentPath) : undefined, type = canonicalTypes.has(row.effective.type) ? row.effective.type : "string", id = propertyIdFor(entity, row.path, row.effective, used), localRuleIds = new Set((row.local.rules ?? []).map((rule) => String(rule.id ?? ""))), rules = (row.effective.rules ?? []).map((rule, index) => { const ruleId = String(rule.id ?? `rule:${id}:${index}`); return ruleFor(rule, ruleId, ownershipProvenance(row, entity, ruleId, rule.provenance, localRuleIds)); });
