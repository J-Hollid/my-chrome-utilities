...
export function allowedValueExpansionAvailability(evidence) {
    if (evidence.status !== "error" && evidence.status !== "warning")
        return { available: false, reason: "The rule did not fail" };
    if (normalizedOperator(evidence.operator) !== "allowed-values")
        return { available: false, reason: "The failed rule is not an Allowed values rule" };
    if (!evidence.ruleId)
        return { available: false, reason: "Stable rule identity is unavailable" };
    if (!isAllowedValue(evidence.actualValue))
        return { available: false, reason: "Only scalar JSON values can be added" };
    return { available: true };
}
function valuesForRule(rule) {
...
function expansionOrigin(input) {
    if (input.reusableRules.some(({ id, version }) => id === input.evidence.ruleId && version === input.evidence.ruleVersion))
        return "reusable";
...
function destinationsFor(origin) {
    if (origin === "inherited")
        return ["parent-schema-draft", "assigned-schema-override"];
    if (origin === "reusable")
        return ["reusable-rule-revision", "assigned-schema-override"];
    return ["assigned-schema-draft"];
}
function draftRule(schema, id) {
    return schema.workingDraft?.attachedRules?.find((rule) => rule.id === id);
}
export function reviewAllowedValueExpansion(input) {
    const availability = allowedValueExpansionAvailability(input.evidence);
    if (!availability.available)
        throw new Error(availability.reason);
    const assigned = input.schemas.find(({ id }) => id === input.assignedSchemaId);
    if (!assigned)
        throw new Error("The assigned schema no longer exists.");
    const rule = evidenceRule(input);
    const origin = expansionOrigin(input);
    const proposedValue = input.evidence.actualValue;
...
function widenedRule(rule, value) {
...
function replaceRule(rules, ruleId, update) {
    let found = false;
    ...
    if (!found)
        throw new Error("The evidenced rule is absent from the destination draft.");
    return result;
}
function updateSchemaRule(schema, ruleId, value, change) {
    ...
    if (!current)
        throw new Error("The evidenced rule is absent from the destination schema.");
    if (valuesForRule(current).some((candidate) => sameValue(candidate, value)))
        return { schema: editable, changed: false };
    return {
        schema: updateSchemaWorkingDraft(editable, { attachedRules: replaceRule(rules, ruleId, (rule) => widenedRule(rule, value)) }, change),
        changed: true,
    };
}
function assignedOverride(assigned, source, evidence, value, origin, change) {
    ...
    if (existing && valuesForRule(existing).some((candidate) => sameValue(candidate, value)))
        return { schema: editable, changed: false };
    ...
    const inheritedRuleOverrides = origin === "inherited"
        ? { ...(editable.workingDraft?.inheritedRuleOverrides ?? {}), [evidence.propertyPath]: "disabled" }
...
export function applyAllowedValueExpansion(input) {
    const review = reviewAllowedValueExpansion(input);
    if (!review.destinations.includes(input.destination))
        throw new Error("The selected destination is not valid for this rule origin.");
    ...
    const schemas = structuredClone([...input.schemas]);
    const reusableRules = structuredClone([...input.reusableRules]);
    const assignedIndex = schemas.findIndex(({ id }) => id === input.assignedSchemaId);
    if (assignedIndex < 0)
        throw new Error("The assigned schema no longer exists.");
    if (input.destination === "assigned-schema-draft") {
        const result = updateSchemaRule(schemas[assignedIndex], source.id, proposed, change);
    ...
    if (input.destination === "parent-schema-draft") {
        ...
        if (parentIndex < 0)
            throw new Error("The parent schema no longer exists.");
        const result = updateSchemaRule(schemas[parentIndex], source.id, proposed, change);
    ...
    if (input.destination === "assigned-schema-override") {
        const result = assignedOverride(schemas[assignedIndex], source, input.evidence, proposed, review.origin, change);
        schemas[assignedIndex] = result.schema;
        return { schemas, reusableRules, changed: result.changed, affectedSchemaId: input.assignedSchemaId, ...(result.changed ? { pendingChange: change } : {}) };
    }
    const reusableIndex = reusableRules.findIndex(({ id, version }) => id === source.id && version === source.version);
    if (reusableIndex < 0)
        throw new Error("The reusable rule revision no longer exists.");
    const previous = reusableRules[reusableIndex];
    if (valuesForRule(previous).some((candidate) => sameValue(candidate, proposed)))
        return { schemas, reusableRules, changed: false, affectedSchemaId: input.assignedSchemaId };
...
//# sourceMappingURL=data-layer-allowed-value-expansion.js.map