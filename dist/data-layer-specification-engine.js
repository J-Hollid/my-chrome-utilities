...
export { applyCanonicalCommand, createCanonicalProjectEnvelope, migrateCanonicalProject } from "./data-layer-specification-model.js";
...
export function compileSpecificationProject(envelope) { const diagnostics = graphDiagnostics(envelope.project); if (diagnostics.length)
    ...
    const schemaKey = assignment.id, output = layeredDocument(result.compiled), profileIds = [...new Set(result.compiled.provenance.map(({ contributorId }) => contributorId))], schemaRevision = Math.max(1, ...result.contributors.map(({ revision }) => Number(revision ?? 1)));
    schemas[schemaKey] = { schemaId: result.target.id, revision: schemaRevision, document: output.document, required: output.required, profileIds };
...
} const assignments = envelope.project.collections.assignments.map((assignment) => { const event = envelope.project.collections.events.find(({ id }) => id === assignment.eventId); return { assignmentId: assignment.id, assignmentName: assignment.name, targetId: String(assignment.targetId), targetKind: String(assignment.targetKind), schemaKey: assignment.id, ...(assignment.applicabilitySetId ? { applicabilitySetId: String(assignment.applicabilitySetId) } : {}), eventId: String(assignment.eventId), sourceId: String(assignment.sourceId ?? event?.sourceId ?? ""), validationTarget: assignment.target === "raw input" ? "raw input" : "payload", priority: Number(assignment.priority ?? 0), schemaRevision: schemas[assignment.id]?.revision ?? 1 }; }), planContent = { projectId: envelope.project.id, draftId: envelope.draftId, revision: envelope.revision, sourceProject: clone(envelope.project), pages: index(envelope.project.collections.pages), events: index(envelope.project.collections.events), applicability: index(envelope.project.collections.applicabilitySets), flows: index(envelope.project.collections.flows), schemas, assignments, provenance }, executableContent = { projectId: envelope.project.id, pages: planContent.pages, events: planContent.events, applicability: planContent.applicability, flows: planContent.flows, schemas, assignments, provenance }, plan = { ...planContent, contentIdentity: contentIdentity("plan", planContent), evaluatorContentIdentity: contentIdentity("evaluator", executableContent) }; return { status: "compiled", plan: freeze(plan), diagnostics: [] }; }
...
export function evaluateSpecificationObservation(plan, observation, priorInstances) {
    const transition = priorInstances ? flowTransition(plan, observation, priorInstances) : undefined;
    const stateTransition = transition && (transition.instances.length > 0 || transition.active !== undefined || transition.ambiguity !== undefined) ? transition : undefined, active = stateTransition?.active, effectiveObservation = { ...observation, ...(active ? { flowId: active.flowId, activeStepId: active.currentStepId } : {}) };
    const preliminary = plan.assignments.map((assignment) => { const event = plan.events[assignment.eventId], eventName = event?.eventName ?? event?.name, sourceAccepted = assignment.sourceId === String(effectiveObservation.sourceId ?? ""), eventAccepted = eventName === effectiveObservation.eventName, applicability = assignment.applicabilitySetId ? plan.applicability[assignment.applicabilitySetId] : undefined, applicabilityRejections = assignment.applicabilitySetId ? applicability ? assignmentConditionRejections(applicability.condition, effectiveObservation) : ["applicability"] : [], rejectionReasons = [...(!sourceAccepted ? ["source"] : []), ...(!eventAccepted ? ["Event"] : []), ...applicabilityRejections]; return { ...assignment, rejectionReasons, event: { accepted: sourceAccepted && eventAccepted, evidence: `Source ${sourceAccepted ? "accepted" : "rejected"}; Event ${eventAccepted ? "accepted" : "rejected"} (${event?.name ?? "missing Event"})` }, applicability: { accepted: !applicabilityRejections.length, evidence: applicabilityRejections.length ? `${applicability?.name ?? "Applicability Set"} rejected by ${applicabilityRejections.join(", ")}` : `${applicability?.name ?? "No Applicability Set"} accepted` } }; });
    const matching = stateTransition?.ambiguity ? [] : preliminary.filter(({ rejectionReasons }) => rejectionReasons.length === 0).sort((left, right) => right.priority - left.priority), highest = matching[0]?.priority, tied = matching.filter(({ priority }) => priority === highest), selected = tied.length === 1 ? tied[0] : undefined, tieIds = tied.length > 1 ? tied.map(({ assignmentId }) => assignmentId) : [], candidates = preliminary.map((candidate) => ({ ...candidate, resolution: candidate.rejectionReasons.length ? "rejected" : tieIds.includes(candidate.assignmentId) ? "tied" : selected?.assignmentId === candidate.assignmentId ? "winner" : "lower priority" })), schema = selected ? plan.schemas[selected.schemaKey] : undefined, winner = selected && schema ? { assignmentId: selected.assignmentId, schemaId: schema.schemaId, schemaRevision: selected.schemaRevision } : undefined, validationInput = selected?.validationTarget === "raw input" ? effectiveObservation.rawInput : effectiveObservation.payload, issueDetails = schema && selected ? validationIssueDetails(validationInput, schema.document, plan, selected.schemaKey) : [], issues = issueDetails.map(({ path, code, message }) => `${path}: ${code === "required" ? "required" : message}`), only = candidates.length === 1 ? candidates[0] : undefined, routingSummary = selected ? `${selected.assignmentName} is the sole winner` : tieIds.length ? `Routing is ambiguous between ${tied.map(({ assignmentName }) => assignmentName).join(" and ")}` : only?.rejectionReasons[0] ? `${only.assignmentName} is rejected by ${only.rejectionReasons[0]}` : "No Assignment matched";
    const resultContent = { planContentIdentity: plan.contentIdentity, ...(plan.releaseId ? { releaseIdentity: plan.releaseId } : {}), candidates, routingSummary, ...(winner ? { winner } : {}), ties: tieIds, activeFlowId: effectiveObservation.flowId, activeStepId: effectiveObservation.activeStepId, effectiveProfiles: schema?.profileIds ?? [], ...(schema ? { effectiveSchemaRevision: schema.revision } : {}), issues, issueDetails, provenance: selected ? Object.fromEntries(Object.entries(plan.provenance).filter(([key]) => key.startsWith(`${selected.schemaKey}:`))) : {}, ...(stateTransition ? { stateTransition } : {}) };
    const evaluatorResultContent = { ...resultContent, planContentIdentity: plan.evaluatorContentIdentity };
    return { resultIdentity: contentIdentity("result", evaluatorResultContent), ...resultContent };
}
