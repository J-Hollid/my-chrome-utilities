...
export function renderDocumentationExport(input) {
    ...
    scope.addEventListener("change", () => { input.setScope(scope.value); input.setPendingAction(undefined); input.setConfirmedIncomplete(false); input.rerender(); });
    ...
    const execute = (action, confirmed = input.confirmedIncomplete) => {
        ...
        try {
            ...
            input.setFeedback(error instanceof Error ? error.message : String(error));
            input.rerender();
        }
    };
    const request = (action) => {
        if (!input.snapshot || input.stale)
            return;
        if (input.snapshot.incomplete && !input.confirmedIncomplete) {
            input.setPendingAction(action);
            input.setFeedback("Confirm Export draft anyway to produce this output.");
            input.rerender();
            return;
        }
        execute(action);
