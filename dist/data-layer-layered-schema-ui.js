...
import { composedCanonicalSchema, composedSchemaWorkspace, includeFlowComposedSchemaParentAdditions, resetFlowComposedSchemaLocalChanges, resetFlowComposedSchemaLocalFacet, resetFlowComposedSchemaLocalProperty, resetFlowComposedSchemaLocalRule, saveComposedEventCanonicalDocument, saveFlowComposedSchemaLocalFacets, saveFlowContributorSchemaPolicy } from "./data-layer-composed-schema-workspace.js";
...
export function installLayeredSchemaUi(options) {
    ...
    const restoreFlowReturn = () => { const saved = flowReturn; if (!saved)
        ...
        const example = document.querySelector(saved.expandedExample.selector);
        if (example)
    ...
    const renderCanonicalLayerEditor = (state, entity, scope) => { const flowId = options.context().kind === "flows" ? options.context().entityId : undefined; if (scope === "Flow Page-instance" && flowId)
        return renderFlowComposedSchemaWorkspace(state, entity, flowId); const localCanonical = entity.canonicalSchema; if (!localCanonical)
        return false; const canonical = scope === "Event" ? composedCanonicalSchema(state, entity, "Event") : localCanonical, title = document.createElement("h2"), identity = document.createElement("p"), areas = document.createElement("nav"), host = document.createElement("section"), back = document.createElement("button"), compiled = compileLayeredSchema(layeredContributorsForPath(state, layeredContributorPath(state, entity, scope, flowId)), { eventId: String(entity.eventId ?? entity.id), eventRole: layeredEventRole(entity), occurrenceId: entity.id }); host.dataset.schemaContributorId = entity.id; host.dataset.schemaContributorScope = scope; title.tabIndex = -1; title.textContent = "Shared canonical schema editor"; identity.textContent = `Contributor: ${entity.name} · Scope: ${scope}`; areas.textContent = `Inherited constraints · Local contributions · Effective results · Blocking conflicts (${compiled.conflicts.length})`; back.type = "button"; back.textContent = "Return to Flow"; back.addEventListener("click", () => { editor.hidden = true; editorHost.hidden = true; workspace.hidden = false; restoreFlowReturn(); }); editor.append(title, identity, areas, host, renderRuntimeControls(state, entity, scope, compiled)); mountCanonicalSchemaEditor({ host, surface: canonicalLayerEditorSurface(options.context().kind), conceptSuggestions: canonicalLayerConceptSuggestions(() => current().state), load: () => { const live = current(); return live.state && live.entity && live.scope === "Event" ? composedCanonicalSchema(live.state, live.entity, "Event") : live.entity.canonicalSchema; }, id, dispatch: (command) => { const live = current(), document = live.state && live.entity && live.scope === "Event" ? composedCanonicalSchema(live.state, live.entity, "Event") : live.entity.canonicalSchema; if (live.scope === "Event" && command.baseRevision !== document.revision)
            ...
            if (live.scope === "Event")
                options.persist(saveComposedEventCanonicalDocument(live.state, live.entity.id, result.document));
            else {
                if (graphSelection)
    ...
    function renderSummary() { const context = options.context(), { state, entity, scope } = current(); summary.replaceChildren(); summary.dataset.contextKind = context.kind; summary.dataset.contextEntityId = context.entityId ?? ""; if (!state || !entity) {
        ...
        return;
    } editor.hidden = false; if (scope !== "Flow Page-instance")
        ensureCanonical(); renderEditor(); editorHost.hidden = false; workspace.hidden = true; editor.querySelector("h2")?.focus(); }); summary.append(title, counts, open); }
    function renderEditor() { const { state, entity, scope } = current(); editor.replaceChildren(); if (!state || !entity)
        return; const flowId = options.context().kind === "flows" ? options.context().entityId : undefined, rendered = renderCanonicalLayerEditor(state, entity, scope); if (rendered && scope === "Event-occurrence" && flowId)
        mountFlowComposedSchemaWorkspace(editor, state, entity, flowId, scope); if (!rendered) {
