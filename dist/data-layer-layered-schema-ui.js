...
export function installLayeredSchemaUi(options) {
    ...
    function renderSummary() { const context = options.context(), { state, entity, scope } = current(); summary.replaceChildren(); summary.dataset.contextKind = context.kind; summary.dataset.contextEntityId = context.entityId ?? ""; if (!state || !entity) {
        ...
        return;
    } editor.hidden = false; ensureCanonical(); renderEditor(); editorHost.hidden = false; workspace.hidden = true; editor.querySelector("h2")?.focus(); }); summary.append(title, counts, open); }
    ...
    const selectGraphContributor = (target) => { const { state, kind, entityId: flowId } = options.context(); if (!state || kind !== "flows" || !flowId)
        ...
        return; const stableSelectorId = occurrenceId ?? pageFrameId ?? pageGroupId, id = CSS.escape(stableSelectorId), selector = occurrenceId ? `[data-occurrence-id="${id}"]` : pageFrameId ? `[data-page-frame-id="${id}"]` : `[aria-label="Interactive directional Flow canvas"] [data-page-group-id="${id}"]`; captureFlowReturn(selector, pageFrameId ? "page-frame" : occurrenceId ? "occurrence" : "page-group", stableSelectorId, document.activeElement instanceof HTMLElement ? document.activeElement : undefined); renderSummary(); };
    ...
    const openGraphOccurrenceSchema = (contributorId, path, originFocus) => { const { state, kind, entityId: flowId } = options.context(), graphs = state?.project.documentationFlowGraphs, occurrence = flowId ? graphs?.[flowId]?.occurrences?.find(({ id }) => id === contributorId) : undefined, frame = flowId ? flowPageFrameContributor(state, flowId, contributorId) : undefined, contributor = occurrence ?? frame; if (!state || kind !== "flows" || !flowId || !contributor)
        return false; graphSelection = contributor; graphSelectionScope = frame ? "Flow Page-instance" : occurrence?.freePageFrame ? "Flow Page-instance" : "Event-occurrence"; returnFocus = originFocus ?? (document.activeElement instanceof HTMLElement ? document.activeElement : undefined); const stableSelectorId = contributorId, selector = frame ? `[data-page-frame-id="${CSS.escape(stableSelectorId)}"]` : occurrence ? `[data-occurrence-id="${CSS.escape(stableSelectorId)}"]` : ""; captureFlowReturn(selector, frame ? "page-frame" : "occurrence", stableSelectorId, returnFocus); editor.hidden = false; ensureCanonical(); renderEditor(); editorHost.hidden = false; workspace.hidden = true; editor.querySelector("h2")?.focus(); if (path)
