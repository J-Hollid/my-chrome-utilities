...
export function installLayeredSchemaUi(options) {
    ...
    function renderRuntimeControls(state, entity, scope, compiled) {
        ...
        saveTarget.addEventListener("click", () => { const predicates = JSON.parse(applicability.value || "[]"), eventName = predicates.find(({ field, operator }) => field === "eventName" && operator === "equals")?.value, selectedEvent = state.project.collections.events.find(({ id, name }) => id === targetEvent.value || !targetEvent.value && eventName === name); if (!selectedEvent)
            throw new Error("Choose the Event for this layered target."); const flow = state.project.collections.flows.find(({ id }) => id === path.flowId), page = state.project.collections.pages.find(({ id }) => id === path.pageId), context = { eventId: selectedEvent.id, eventRole: targetRole.value, ...(scope === "Event-occurrence" ? { occurrenceId: entity.id } : {}) }, target = { id: existing?.id ?? `layered-target:${entity.id}`, name: entity.name, contributorId: entity.id, activation: activation.value, priority: Number(priority.value), applicability: predicates, ...path, ...context, revision: (existing?.revision ?? 0) + 1, flowName: flow?.name ?? entity.name, pageName: page?.name ?? String(entity.pageName ?? entity.pageId ?? "Page"), eventName: selectedEvent.name }, next = transactProject(state, `Save layered activation for ${entity.name}`, (project) => ({ ...project, layeredSchemaTargets: [...(project.layeredSchemaTargets ?? []).filter(({ contributorId }) => contributorId !== entity.id), target] })); options.persist(next); renderEditor(); });
