...
export function installLayeredSchemaUi(options) {
    ...
    const renderCollectionComposedSchemaWorkspace = (state, entity, scope) => { const kind = scope === "Page" ? "pages" : "propertySets", identity = document.createElement("p"), back = document.createElement("button"), persistComposed = (next) => { graphSelection = next.project.collections[kind].find(({ id }) => id === entity.id); options.persist(next); queueMicrotask(renderEditor); }; identity.textContent = `Contributor: ${entity.name} · ${scope}`; editor.append(identity); mountComposedSchemaWorkspace({ host: editor, model: composedSchemaWorkspace(state, entity, scope), effectiveText: (row) => effectivePropertySummary(row.effective), conceptSuggestions: () => projectCanonicalConcepts(current().state), schemaContributorId: entity.id, schemaContributorScope: scope, onlyDefinedFields: entity.onlyDefinedFields === true, onSave: (row, facets, structures = []) => { const live = current(); if (live.state)
    ...
    const renderCanonicalLayerEditor = (state, entity, scope) => { const flowId = options.context().kind === "flows" ? options.context().entityId : undefined; if (scope === "Flow Page-instance" && flowId)
        return renderFlowComposedSchemaWorkspace(state, entity, flowId, scope); if (scope === "Event-occurrence" && flowId)
        return renderEffectiveCanonicalSchemaWorkspace(state, entity, scope, flowId); if (scope === "Page" || scope === "Property Set")
        return renderCollectionComposedSchemaWorkspace(state, entity, scope); if (scope === "Event")
        return renderEffectiveCanonicalSchemaWorkspace(state, entity, scope); const localCanonical = entity.canonicalSchema; if (!localCanonical)
    ...
    function renderSummary() { const context = options.context(), { state, entity, scope } = current(); summary.replaceChildren(); summary.dataset.contextKind = context.kind; summary.dataset.contextEntityId = context.entityId ?? ""; if (!state || !entity) {
        ...
        return;
    } editor.hidden = false; if (!schemaContributorUsesEffectiveWorkspace(scope))
