        ...
        const refreshResults = () => {
            ...
            if (document.view === "tree") {
                ...
                for (const row of rows) {
                    ...
                    documentationSummary.textContent = [row.node.documentation.displayText, row.node.documentation.comments].filter(Boolean).join(" · ");
                    documentationSummary.setAttribute("aria-label", `${row.path} documentation facets`);
                    documentationGroup.append(labeled("Documentation", documentation), documentationSummary);
                    ...
                    const cells = [choose, row.path, type, presence, valueGroup, row.node.presence.condition ? plainPredicate(row.node.presence.condition, document) : "Always", String(row.node.rules.length), documentationGroup, example, provenanceText(row.node), row.node.provenance.at(-1)?.state ?? "Local canonical definition", row.validationState, actions];
