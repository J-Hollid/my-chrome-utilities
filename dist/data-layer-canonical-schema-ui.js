...
export function mountCanonicalSchemaEditor(options) {
    ...
    const renderRules = (host, node) => {
        ...
        const addPanel = dom.createElement("fieldset"), legend = dom.createElement("legend"), kind = dom.createElement("select"), fields = dom.createElement("div"), add = button(dom, "Add rule", () => { if (!working)
            ...
            rule.reusableRuleId = reusable.value; working.rules = [...working.rules, rule]; feedback = "Staged rule addition."; render(); });
        ...
        const renderFields = () => { fields.replaceChildren(); for (const field of focusedRuleFields(kind.value)) {
            ...
            if (field === "reusableRuleId") {
                ...
                reusable.append(new Option("No reusable rule", ""), ...node.rules.filter(({ reusableRuleId }, index, array) => reusableRuleId && array.findIndex((candidate) => candidate.reusableRuleId === reusableRuleId) === index).map((candidate) => new Option(candidate.name ?? candidate.reusableRuleId, candidate.reusableRuleId)));
