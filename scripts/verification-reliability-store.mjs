...
import {
  archivedReceiptDocument, atomicReplace, defaultLegacyStoreDirectory,
...
import {receiptDocument,validatePackageReceipt} from "./verification-reliability-receipts.mjs";
import {
...
import {checkpointExcludedChangedPaths,claimableRepairCheckpointIds,
  createCheckpointResolutionOperations,repairCheckpointClaimError,terminalCheckpointIncident} from
  "./verification-policy/reliability/checkpoint-resolution-store.mjs";
...
export function createTimeoutIncidentStore({
        ...
        try {
          ...
          const excludedChangedPaths=Array.isArray(
            ...
            ?await checkpointExcludedChangedPaths(checkpointDocument,incident.id,
              {read:(id)=>this.read(id),root,isAncestor,commitDescendsFrom})
            :[];
  ...
  return Object.assign(store,
    ...
    createCheckpointResolutionOperations({ root, now, read:access.read, update:access.update,
