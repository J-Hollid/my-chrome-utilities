...
import { loadCalibrationReceiptConsumer } from
  "./verification-calibration-receipt-consumer.mjs";
import {
...
export async function runIntegrationReceiptDispositionManifest(manifestPath, {
  ...
  loadCalibrationConsumer = loadCalibrationReceiptConsumer,
} = {}) {
  ...
  const calibrationConsumerFor = await loadCalibrationConsumer({ repositoryRoot });
  const results = [];
  for (const entry of manifest.receipts) {
    ...
    const calibrationConsumer=await calibrationConsumerFor({repositoryRoot,contentIdentity,
