import {taskCheckpointShape as checkpointShape,validateTaskCheckpointRepairProof as validateCheckpointProof,
  deriveTaskCheckpointRepairProof as deriveCheckpointProof} from "./verification-reliability-checkpoint-proof.mjs";
import {observationResultRepairRequired,deriveObservationResultRepairProof,
  validateObservationResultRepairProof} from "./verification-reliability-observation-result-proof.mjs";
...
const taskCheckpointShape=incident=>checkpointShape(incident)||observationResultRepairRequired(incident);
export const taskCheckpointRepairRequired=taskCheckpointShape;
function validatedTaskCheckpointRepairProof(incident,proof) {
  return observationResultRepairRequired(incident)
    ?validateObservationResultRepairProof(incident,proof):validateCheckpointProof(incident,proof);
}
export const validateTaskCheckpointRepairProof=validatedTaskCheckpointRepairProof;
export async function deriveTaskCheckpointRepairProof(incident,options) {
  return observationResultRepairRequired(incident)
    ?deriveObservationResultRepairProof(incident,options):deriveCheckpointProof(incident,options);
}
