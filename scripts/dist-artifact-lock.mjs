...
import {
  artifactAccess,
  assertArtifactAccessPermits,
  distArtifactAccessEnvironmentKey,
  distArtifactLeaseEnvironment,
  distArtifactLockEnvironmentKey,
} from "./dist-artifact-lease.mjs";

export {distArtifactLeaseEnvironment} from "./dist-artifact-lease.mjs";
...
export async function inheritedDistArtifactLockIsHeld(
  ...
  token = process.env[distArtifactLockEnvironmentKey],
) {
...
export async function withDistArtifactLock(operation, {
  ...
  if (localAccess) {
    assertArtifactAccessPermits(localAccess, access);
    return operation();
  }
  if (await inheritedDistArtifactLockIsHeld(directory)) {
    const inheritedAccess = artifactAccess(
      process.env[distArtifactAccessEnvironmentKey] ?? "write",
    );
    assertArtifactAccessPermits(inheritedAccess, access);
    return localLockContext.run(inheritedAccess, operation);
  }
  const release = await acquireDistArtifactLock(directory, options);
  const previous = process.env[distArtifactLockEnvironmentKey];
  ...
  try {
    ...
    if (previous === undefined) delete process.env[distArtifactLockEnvironmentKey];
    else process.env[distArtifactLockEnvironmentKey] = previous;
