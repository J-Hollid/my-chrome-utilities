...
export function focusedAcceptanceOptions(args) {
  ...
  if (!options.terminalFull && !options.packIds.length && !options.changedPaths.length) {
    throw new Error("Select --pack <id>, --changed <path>, or --full");
  }
  return options;
}
...
function runCommand(command) {
  return new Promise((resolve, reject) => {
    ...
    child.once("exit", (code, signal) => {
      if (code === 0) resolve();
      else reject(new Error(`Acceptance command failed (${signal ?? code}): ${command}`));
    });
  });
}
