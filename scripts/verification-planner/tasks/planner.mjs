...
export function planVerification(
  ...
  const hasExactFeatureSlice = (registry, changedPath) => {
    if (!changedPath.endsWith(".feature")) return false;
    const pack = ownerOf(registry, changedPath);
    return Boolean(pack && verificationSliceMapping(registry, pack, changedPath).kind === "slice");
  };
  const focusedPolicyPath = (registry, changedPath) =>
    focusedFeaturePolicyPaths.has(changedPath) ||
    slicedFocusedFeaturePolicyPaths.has(changedPath) && hasExactFeatureSlice(registry, changedPath);
