# mutation-stamp: sha256=ffa65d12cbc537b998ffbfeb36fc9b49c9ae9c542cca0023ad570554653e1344
# acceptance-mutation-manifest-begin
# {"version":1,"tested_at":"2026-07-24T22:27:51.616373863Z","feature_name":"Data layer Live Flow guided testing","feature_path":"features/data-layer-live-flow-guided-testing.feature","background_hash":"3aca59d3cc4dba17ae9fb3225e4e5955c78a79278976301ec78968867bb2393c","implementation_hash":"sha256:fd6e61db293d66b1d08d1116893eb092ecc143dc4154691854ed963940b26c32","scenarios":[{"index":4,"name":"Data layer Live Flow guided testing 005","scenario_hash":"f6aefddb3f34e4b629646dc36a68d127dda18c3011e540d8ba2b861b40412f75","mutation_count":4,"result":{"Total":4,"Killed":4,"Survived":0,"Errors":0},"tested_at":"2026-07-24T22:27:51.616373863Z"},{"index":5,"name":"Data layer Live Flow guided testing 006","scenario_hash":"26aadb69fb716fbd25af5f00f5970ebbd45a61f280addcf7f35782b9fcec56cd","mutation_count":2,"result":{"Total":2,"Killed":2,"Survived":0,"Errors":0},"tested_at":"2026-07-24T22:27:51.616373863Z"},{"index":7,"name":"Data layer Live Flow guided testing 008","scenario_hash":"84fceb973c8102c1b2738f424764fdcb6c3fe22244ebfcfd61389303fe89eb87","mutation_count":4,"result":{"Total":4,"Killed":4,"Survived":0,"Errors":0},"tested_at":"2026-07-24T22:27:51.616373863Z"}]}
...
Feature: Data layer Live Flow guided testing
  ...
  Background:
    ...
    And Checkout journey contains related context-setting Page events with nested interaction Event occurrences
  ...
  Scenario: Data layer Live Flow guided testing 002
    ...
    Then its event details show context-setting Page choices in order Cart pageview, Payment pageview, Confirmation A pageview, and Confirmation B pageview
    And Cart is identified as a root Page frame
    And the repeated Confirmation Page frames remain distinct choices
    And no nested interaction Event occurrence is offered before the first Page frame is linked
  ...
  Scenario: Data layer Live Flow guided testing 003
    Given Checkout journey is the selected Flow test context
    And observed pageview event live-101 is open with no Flow-step link
    ...
    And its ordinary event validation uses Cart context-setting Page-event effective schema
  ...
  Scenario: Data layer Live Flow guided testing 004
    Given the session Page cursor identifies Cart
    And Cart contains button_click and PayPal interaction Event occurrences
    And Cart has outgoing Page relationships to Payment and Confirmation
    And the observed event is unlinked
    When the operator opens the observed event from the event feed
    Then the Flow step selector offers Cart button_click and PayPal occurrences before related Payment and Confirmation Page frames
    And no Event occurrence outside Cart and no unrelated Page frame is available
    When Cart PayPal occurrence is chosen for live-102
    Then validator compiler input combines Cart Page-instance, PayPal Event, and occurrence contributions
    And selecting the occurrence leaves the session Page cursor on Cart
    When a second feed event with no session link is opened
    Then Cart PayPal remains selectable again beside Cart's other contained Events and related Pages
    When the operator links that event to Payment Page frame
    Then selecting the Page target advances session context to Payment
  ...
  Scenario Outline: Data layer Live Flow guided testing 005
    ...
    And the session Page cursor resolves to Payment as the target's containing Page
  ...
  Scenario Outline: Data layer Live Flow guided testing 006
    Given the session is positioned at Cart Page
    And the observed event is unlinked
    And it was captured <capture_order> the event linked to Cart
    When the operator opens the observed event from the event feed
    Then the Flow step selector offers Cart's contained Events and outgoing related Pages
  ...
  Scenario: Data layer Live Flow guided testing 007
    ...
    And live-102 advanced the session Page context to Payment Page frame
    ...
    And the session Page cursor remains Payment Page frame
    When the operator switches review to another event without a recorded Flow step
    Then its available choices equal Payment contained Event occurrences plus outgoing related Pages rather than Cart choices
  ...
  Scenario Outline: Data layer Live Flow guided testing 008
    ...
    And the failed validation neither changes the linked path nor blocks valid Page-context choices
  ...
  Scenario: Data layer Live Flow guided testing 009
    Given the operator linked Cart and Payment pageview observations along one Page relationship path
    And linked repeated observations to button_click under Cart and add_payment_info under Payment without advancing either Page context
    When the Live session is saved and reopened
    Then the selected Checkout journey, current Page context, event-to-step links, schema revisions, validation results, and defect references are restored
