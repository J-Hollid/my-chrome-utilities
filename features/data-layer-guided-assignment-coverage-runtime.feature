...
Feature: Data layer guided assignment coverage runtime
  ...
  Background:
    ...
    And captured event order_complete from source event-history at https://shop.example/orders/confirmed is open in the Live event inspector
  ...
  Scenario Outline: Data layer guided assignment coverage runtime 001
    ...
    Examples:
      | schema_name     | first_property | later_properties | assignment_name  |
      | Order completed | order_id       | currency, value  | confirmed orders |
  ...
  Scenario Outline: Data layer guided assignment coverage runtime 002
    ...
    Examples:
      | schema_name     | assignment_name  | domain_condition | path_condition | property_name |
      | Order completed | shop order pages | *.example         | /orders/*      | order_id      |
  ...
  Scenario Outline: Data layer guided assignment coverage runtime 003
    ...
    Examples:
      | schema_name     | existing_assignment | property_name | later_property | new_assignment   |
      | Order completed | product pages       | order_id      | currency       | confirmed orders |

  # Data layer guided assignment coverage runtime 004
  Scenario: Data layer guided assignment coverage runtime 004
    Given schema Order completed has 2 enabled assignments that cover the captured event with one higher-priority winner
    When Add validation for currency continues in its working draft
    Then assignment configuration and assignment selection are not displayed
    And the property-rule flow can proceed to review and save
    And saving adds the currency rule without changing assignment count 2
    And no duplicate assignment identity is created

  # Data layer guided assignment coverage runtime 005
  Scenario: Data layer guided assignment coverage runtime 005
    Given Order completed has 2 equal-priority enabled assignments that cover the captured event
    When Add validation for currency continues in its working draft
    Then the currency rule can be saved without duplicating either Assignment
    And automatic-validation readiness displays Ambiguous Assignment with both human names and matching evidence
    And publication remains blocked at the named overlap repair
