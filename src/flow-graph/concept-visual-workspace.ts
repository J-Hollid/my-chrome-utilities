...
export function createFlowConceptVisualActions(options:VisualActionOptions):HTMLButtonElement[]{
  ...
  },view=()=>{const current=flowConceptVisual(options.project(),options.flowId,options.target),invoker=document.activeElement,selector=options.target.kind==="page-frame"?`g[data-page-frame-id="${CSS.escape(options.target.id)}"]:not([data-occurrence-id])`:`[data-occurrence-id="${CSS.escape(options.target.id)}"]`,fallback=document.querySelector<HTMLElement>(selector);if(current&&invoker instanceof HTMLElement)openFlowConceptVisualViewer({attachment:current.attachment,raster:current.asset},invoker,fallback??undefined);};
...
export function renderFlowConceptVisual(options:{group:SVGGElement;project:SpecificationProject;flowId:string;target:FlowConceptVisualTarget;mode:FlowVisualDisplayMode;thumbnailPixels:boolean;width:number;height:number}):void{
  ...
  const badge=svg("text");badge.dataset.flowVisualBadge=options.target.id;badge.setAttribute("x","10");badge.setAttribute("y","44");badge.textContent="▧ Visual";badge.setAttribute("aria-label",`View visual: ${visual.attachment.description}`);
