...
const availableStorageBytes=async():Promise<number|undefined>=>{try{const estimate=await navigator.storage?.estimate();return estimate?.quota===undefined?undefined:Math.max(0,estimate.quota-(estimate.usage??0));}catch{return undefined;}};
const requireValidRaster=async(raster:FlowConceptVisualRaster,file:File)=>{const available=await availableStorageBytes(),validation=validateFlowConceptVisualSource({...raster,sourceByteLength:file.size,...(available===undefined?{}:{availableStorageBytes:available})});if(!validation.valid)throw new Error(validation.diagnostic);};
...
export function createFlowConceptVisualEditor(options:{project:()=>SpecificationProject;existing?:{attachment:FlowConceptVisualAttachment;raster:FlowConceptVisualRaster};save:(value:FlowConceptVisualEditorValue)=>void;cancel:()=>void}):{root:HTMLElement;firstControl:HTMLElement}{
  ...
  choose.addEventListener("click",()=>file.click());file.addEventListener("change",()=>void stage(file.files?.[0]));target.addEventListener("paste",event=>{event.preventDefault();void stage(Array.from(event.clipboardData?.files??[])[0]);});target.addEventListener("dragover",event=>event.preventDefault());target.addEventListener("drop",event=>{event.preventDefault();void stage(Array.from(event.dataTransfer?.files??[])[0]);});description.addEventListener("input",refresh);save.addEventListener("click",()=>{if(raster&&description.value.trim())options.save({raster,description:description.value,...(caption.value?{caption:caption.value}:{}),...(source.value?{sourceReference:source.value}:{})});});cancel.addEventListener("click",options.cancel);root.append(target,choose,file,preview,storage,labelled("Description",description),labelled("Caption",caption),labelled("Source reference",source),diagnostic,save,cancel);refresh();return{root,firstControl:target};
}
