...
export function prepareFlowOutline(options: OutlineOptions): HTMLElement {
  ...
  const pageSection = (row: HTMLElement): string | undefined => {
    const id = row.dataset.pageFrameId;
    return id ? canvas.querySelector<SVGGraphicsElement>(`[data-page-frame-id="${CSS.escape(id)}"]`)?.dataset.flowSectionId : undefined;
  };
  ...
  const appendFrame = (target: HTMLOListElement, frame: HTMLLIElement): void => {
    ...
    for (const occurrence of occurrences.filter((row) => occurrenceFrame(row) === frameId)) children.append(occurrence);
  ...
  for (const section of sections) {
    ...
    for (const frame of frames.filter((row) => pageSection(row) === sectionId)) appendFrame(nested, frame);
  ...
  for (const frame of frames.filter((row) => !pageSection(row))) appendFrame(outsideRows, frame);
    ...
    for (const row of rows.filter((candidate) => candidate.textContent?.toLowerCase().includes(term))) {
      ...
      activate.addEventListener("click", () => {
        ...
        setTimeout(() => {
          ...
          const matched = liveCanvas?.querySelector<SVGGraphicsElement>(selector);
          const target = matched?.dataset.sectionDropzone ? matched.closest<SVGGraphicsElement>("g") ?? matched : matched;
          if (target) options.reveal(target);
        }, 50);
      });
