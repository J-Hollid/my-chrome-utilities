...
export const peerSetMismatch=(constraints:readonly LayerConstraint[]):boolean=>{
  ...
  const itemMatches=(value:unknown,schema:LayerItemSchema):boolean=>!schema.type||typeMatches(value,schema.type)&&(!schema.allowedValues?.length||schema.allowedValues.some((candidate)=>same(candidate,value)))&&(schema.type!=="array"||!Array.isArray(value)||!schema.items||value.every((item)=>itemMatches(item,schema.items!)));
