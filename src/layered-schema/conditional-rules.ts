...
const composeResolvedPeers=(property:EffectiveProperty,constraints:readonly LayerConstraint[]):EffectiveProperty=>{
  ...
  for(const key of ["concept","type","nullable","onlyDefinedFields","itemType","itemSchema","allowedValueIds","allowedValueProvenance","target","displayText","documentation","comments","examples","definitionId"] as const){const value=first(key);if(value!==undefined)(result[key] as unknown)=clone(value);}
