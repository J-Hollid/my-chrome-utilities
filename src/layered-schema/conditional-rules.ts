...
const conflictFacet=(facet:string):NonNullable<LayerConflict["facet"]>=>facet==="presence"?"Presence":facet==="value"?"Expected value":facet==="range"?"Range rule":facet==="cardinality"?"Cardinality rule":"Pattern rule";
const ruleValue=(facet:string,rule:Record<string,unknown>):unknown=>facet==="presence"?rule.presence:facet==="value"?(rule.allowedValues??rule.expectedValue):facet==="range"?{minimum:rule.minimum,maximum:rule.maximum}:facet==="cardinality"?{minItems:rule.minItems,maxItems:rule.maxItems}:rule.pattern;
const conflictFor=(path:string,facet:string,rules:Record<string,unknown>[]):LayerConflict=>{const source=rules[0]??{},local=rules.at(-1)??{},sourceName=named(source),localName=named(local);return{path,message:`conditional ${facet} outcomes contradict`,contributors:rules.map(named),contributorIds:rules.map((rule)=>String(rule.contributorId??rule.id??named(rule))),facet:conflictFacet(facet),section:facet==="presence"||facet==="value"?"Definition":"Rules",sourceContributor:sourceName,sourceContributorId:String(source.contributorId??source.id??sourceName),sourceValue:ruleValue(facet,source),sourceRuleId:String(source.id??""),localContributor:localName,localContributorId:String(local.contributorId??local.id??localName),localValue:ruleValue(facet,local),localRuleId:String(local.id??"")};};
...
const composeResolvedPeers=(property:EffectiveProperty,constraints:readonly LayerConstraint[]):EffectiveProperty=>{
  const result=clone(property),first=<K extends keyof LayerConstraint>(key:K):LayerConstraint[K]|undefined=>constraints.find((constraint)=>constraint[key]!==undefined)?.[key],facetKeys:(keyof LayerConstraint)[]=["concept","type","nullable","onlyDefinedFields","itemType","itemSchema","allowedValues","allowedValueIds","allowedValueProvenance","presence","protectedFacets","patterns","minimum","maximum","minItems","maxItems","expectedValue","enforcement","target","condition","displayText","documentation","comments","examples","definitionId","overrideReferences"];
  for(const key of facetKeys)delete result[key];delete result.expectedContributor;delete result.expectedContributors;
  for(const key of ["concept","type","nullable","onlyDefinedFields","itemType","itemSchema","allowedValueIds","allowedValueProvenance","protectedFacets","target","displayText","documentation","comments","examples","definitionId"] as const){const value=first(key);if(value!==undefined)(result[key] as unknown)=clone(value);}
