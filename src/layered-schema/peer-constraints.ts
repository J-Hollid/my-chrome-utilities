import type {LayerConstraint,LayerItemSchema} from "../data-layer-layered-schema.js";
...
const valueDomainMismatch=(constraints:readonly LayerConstraint[]):boolean=>{
  ...
  const type=constraints.find((constraint)=>constraint.type)?.type,finiteDomain=type==="boolean"?[true,false]:type==="null"?[null]:undefined;if(finiteDomain&&!finiteDomain.some(matches))return true;
