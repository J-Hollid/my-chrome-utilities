...
function invalidTemplateRecord(template:ProjectDocumentationTemplate):string|undefined{
  ...
  if(template.format==="excel"){
    if(template.contractVersion!==2)return`Excel documentation template ${template.id} uses unsupported contract ${template.contractVersion}.`;
