...
export function createDocumentationTemplate(input:CreateTemplateInput):ProjectDocumentationTemplate{
  const value={...clone(input),id:projectDocumentationSafeText(input.id),name:projectDocumentationSafeText(input.name),contractVersion:(input.format==="excel"?2:1) as 1|2,digest:input.body?.digest??templateDigest("rich",input.richBlocks??[])};
