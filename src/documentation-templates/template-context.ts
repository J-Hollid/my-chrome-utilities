...
export function prepareDocumentationTemplateContext(snapshot:ProjectDocumentationSnapshot,sectionId:string):TemplateContextObject{
  ...
  if(section.kind==="overview")context.overview={fields:(tableValue.rows as readonly TemplateContextObject[]).map(row=>({label:String(((row.cells as readonly TemplateContextObject[])[0]?.value)??""),value:String(((row.cells as readonly TemplateContextObject[])[1]?.value)??"")}))};
