...
function projectedOutput(prototype:ExcelTemplatePrototype,rendered:RenderedRegion,sourceBottom:number,sourceRight:number):{range?:string;backgroundFill:string;cellCount:number}|undefined{
  ...
  if(cellCount===0)return{backgroundFill:outputArea.properties.backgroundFill,cellCount};
  const range=`${address({row:source.top,column:source.left})}:${address({row:bottom,column:right})}`;
  if(bottom>1_048_576||right>16_384)throw new Error(`Generated ${outputArea.name} ${range} projects to ${rowCount} rows by ${columnCount} columns (${cellCount} cells); this exceeds the Excel worksheet limit. Reduce the Output area or the number of generated repeat items.`);
  if(cellCount>250_000)throw new Error(`Generated ${outputArea.name} ${range} projects to ${rowCount} rows by ${columnCount} columns (${cellCount} cells); the generated background exceeds the 250000-cell budget. Reduce the Output area or the number of generated repeat items.`);
...
export function renderExcelTemplateGrid(prototype:ExcelTemplatePrototype,context:TemplateContextObject):{worksheetName:string;cells:ExcelTemplateCell[];merges:string[];output?:{range?:string;backgroundFill:string;cellCount:number}}{
  const validation=validateExcelTemplatePrototype(prototype);if(!validation.valid)throw new Error(validation.findings.map(({message})=>message).join("\n"));const repeats=repeatAreas(prototype,[]),roots=repeatTree(repeats),outputArea=prototype.areas.find((area):area is Extract<ExcelTemplateArea,{type:"output"}>=>area.type==="output"),outputBounds=outputArea?rectangle(outputArea.range):undefined,sourceCells=prototype.cells.filter(cell=>!outputBounds||pointInside(coordinate(cell.address),outputBounds)).map(cell=>({...cell,sourceAddress:cell.sourceAddress??cell.address})),renderable={...prototype,cells:sourceCells},points=sourceCells.map(({address:cellAddress})=>coordinate(cellAddress)),bottom=Math.max(1,...points.map(({row})=>row),...roots.map(({item})=>item.rectangle.bottom)),right=Math.max(1,...points.map(({column})=>column),...roots.map(({item})=>item.rectangle.right)),rendered=renderContainer(renderable,{top:1,left:1,bottom,right},roots,context);
  ...
  if(!output&&(rendered.height>1_048_576||rendered.width>16_384))throw new Error("Template expansion exceeds the Excel worksheet limit.");
