...
export function validateExcelTemplatePrototype(prototype:ExcelTemplatePrototype):{valid:boolean;findings:ExcelTemplateFinding[]}{
  const findings:ExcelTemplateFinding[]=[],directives:{source:ExcelTemplateDirectiveCell;directive:ExcelTemplateDirective;rectangle?:{top:number;left:number;bottom:number;right:number}}[]=[];
  ...
  for(const cell of prototype.cells)for(const path of placeholderPaths(String(cell.value))){if(root.has(path))continue;const owner=directives.find(({directive,rectangle})=>directive.kind==="each"&&rectangle&&coordinate(cell.address).row>=rectangle.top&&coordinate(cell.address).row<=rectangle.bottom&&coordinate(cell.address).column>=rectangle.left&&coordinate(cell.address).column<=rectangle.right&&itemPaths[directive.items]?.includes(path));if(!owner)findings.push({cell:cell.address,message:`${cell.address} uses out-of-scope binding ${path}.`});}
  for(const item of directives)if(item.directive.kind==="each"){if(!itemPaths[item.directive.items]){findings.push({cell:item.source.cell,message:`${item.source.cell} uses unsupported collection ${item.directive.items}.`});continue;}const parents=directives.filter(candidate=>candidate!==item&&candidate.directive.kind==="each"&&candidate.rectangle&&item.rectangle&&candidate.rectangle.top<=item.rectangle.top&&candidate.rectangle.left<=item.rectangle.left&&candidate.rectangle.bottom>=item.rectangle.bottom&&candidate.rectangle.right>=item.rectangle.right).sort((a,b)=>{const area=(value:NonNullable<typeof a.rectangle>)=>(value.bottom-value.top+1)*(value.right-value.left+1);return area(a.rectangle!)-area(b.rectangle!);}),parent=parents[0]?.directive as Extract<ExcelTemplateDirective,{kind:"each"}>|undefined,available=parent?nestedCollections[parent.items]??[]:rootCollections[prototype.kind];if(!available.includes(item.directive.items))findings.push({cell:item.source.cell,message:`${item.source.cell} collection ${item.directive.items} is outside this repeat scope.`});}
