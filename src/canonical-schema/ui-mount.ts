...
export function mountCanonicalSchemaEditor(options:CanonicalSchemaEditorOptions):{render():void}{
  ...
  const command=(next:CanonicalCommand):CanonicalCommandResult=>{if(next.kind==="view")transientView=next.view;return dispatchFocusedCanonicalCommand(next,{current,dispatch:options.dispatch,renderAfterDispatch:options.renderAfterDispatch,host:options.host,setFeedback:(message)=>{feedback=message;},render});};
  ...
  const closeFocused=():void=>{
    const restorePath=originPath;working=undefined;removedRuleIds=new Set();removedValueIds=new Set();menuPropertyId=undefined;activePropertyId=undefined;review=undefined;
    render();
  ...
  const openProperty=(node:CanonicalPropertyNode,focus?:HTMLElement,section:FocusedPropertySection="definition"):void=>{
    const document=current();activePropertyId=node.id;activeSection=section;menuPropertyId=node.id;removedRuleIds=new Set();removedValueIds=new Set();ensureWorking(node);if(focus){originFocus=focus;originPath=canonicalPropertyPath(document,node.id);}
    render();
  };
  const finishFocusedSave=():void=>{working=undefined;removedRuleIds=new Set();removedValueIds=new Set();menuPropertyId=undefined;activePropertyId=undefined;review=undefined;render();};
  ...
  const saveFocused=(confirmedType=false):void=>{
    ...
    const result=command({kind:"set",baseRevision:document.revision,propertyId:working.id,patch,...(confirmedType?{confirmed:true}: {})});
  ...
  const showReview=():void=>{
    ...
    const changes=focusedStagedChanges(working,original,removedRuleIds,canonicalPropertyPath(current(),working.id),removedValueIds);if(!changes.length){feedback="No staged changes to review.";render();return;}
