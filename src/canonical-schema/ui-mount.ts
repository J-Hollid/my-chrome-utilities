import {canonicalArrayBoundaries,canonicalPropertyPath,type CanonicalCommand,type CanonicalCommandResult,type CanonicalIdFactory,type CanonicalPropertyNode,type CanonicalSchemaDocument,type CanonicalStructuralOperation} from "../data-layer-canonical-schema.js";
...
export interface CanonicalSchemaEditorOptions {
  ...
  renderAfterDispatch?:boolean;showOnlyDefinedFields?:boolean;conceptSuggestions?:()=>readonly string[];
  contextExport?:()=>ContextExportSource;
}
...
export function mountCanonicalSchemaEditor(options:CanonicalSchemaEditorOptions):{render():void}{
    ...
    try{
      ...
      if(result.status==="confirmation-required"){
        showImpactReview(result.impact,()=>{
          ...
          if(confirmed.status==="applied"||confirmed.status==="rebased"){review=undefined;render();}
        });
  ...
  const showReview=():void=>{
    ...
    const propertyId=working.id;const changes=focusedStagedChanges(working,original,removedRuleIds,canonicalPropertyPath(current(),propertyId),removedValueIds);if(stagedOperations.length)changes.push(...stagedOperations.map((operation)=>({label:`Structure · ${operation.kind}`,detail:`${operation.kind==="add"?`${operation.name} · ${operation.type} · `:""}${"propertyId" in operation?operation.propertyId:propertyId} staged for review`})));if(!changes.length){feedback="No staged changes to review.";render();return;}
    ...
    panel.setAttribute("aria-label","Review changes");panel.dataset.schemaOverlayLayer="review";heading.textContent="Review changes";changes.forEach(({label,detail})=>{const item=dom.createElement("li");item.textContent=`${label} · ${detail}`;list.append(item);});prospective.textContent=`Prospective effective result: ${working.type} · ${working.presence.mode} · ${working.rules.length} rules · affected consumers follow ${provenanceText(original)}.`;impact.textContent=`Impact review: ${working.type!==original.type?`type ${original.type} → ${working.type}; `:""}${working.documentation!==original.documentation?"documentation and example consumers may change; ":""}${working.rules.length!==original.rules.length?`rules ${original.rules.length} → ${working.rules.length}; `:""}Draft status and one page-scoped Undo are retained.`;actions.append(cancel,confirm);panel.append(heading,list,prospective,impact,actions);review=panel;render();
  };
  ...
  const quickEditRoot=():ParentNode=>{
