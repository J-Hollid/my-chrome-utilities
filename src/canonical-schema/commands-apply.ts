...
export function applyCanonicalAtCurrent(document:CanonicalSchemaDocument,command:CanonicalCommand):CanonicalCommandResult {
  ...
  if(command.kind==="set"){
    const descendants=orderedIds(next,command.propertyId),nextType=command.patch.type??node!.type,
      destructive=node!.type==="object"&&nextType!=="object"&&descendants.length>0,
      itemChange=node!.type==="array"&&nextType==="array"&&node!.itemType!==command.patch.itemType;
    if((destructive||itemChange)&&!command.confirmed)return{status:"confirmation-required",document,propertyId:command.propertyId,impact:destructive?"child definitions and documentation removed; destructive confirmation required":`every item changes from ${node!.itemType??"unspecified"} to ${command.patch.itemType??"unspecified"}`};
    if(destructive)for(const id of descendants)delete next.nodes[id];
    Object.assign(node!,clone(command.patch));
    if(nextType!=="array")delete node!.itemType;
    const affected=new Set<string>([command.propertyId,...(destructive?descendants:[])]);for(const operation of command.operations??[])for(const id of applyStructuralOperation(next,operation))affected.add(id);
    return{status:"applied",document:appendChange(next,command,[...affected])};
  }
  if(command.kind==="type"){const descendants=orderedIds(next,command.propertyId),destructive=node!.type==="object"&&command.type!=="object"&&descendants.length>0,itemChange=node!.type==="array"&&command.type==="array"&&node!.itemType!==command.itemType;if((destructive||itemChange)&&!command.confirmed)return{status:"confirmation-required",document,propertyId:command.propertyId,impact:destructive?"child definitions and documentation removed; destructive confirmation required":`every item changes from ${node!.itemType??"unspecified"} to ${command.itemType??"unspecified"}`};if(destructive)for(const id of descendants)delete next.nodes[id];node!.type=command.type;if(command.type==="array"&&command.itemType)node!.itemType=command.itemType;else delete node!.itemType;return{status:"applied",document:appendChange(next,command,[command.propertyId,...descendants])};}
