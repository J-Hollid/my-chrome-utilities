...
const completeDraftCommand=(document:CanonicalSchemaDocument):CanonicalSchemaDocument=>{const{changes:_legacyJournal,...currentDraft}=document;return currentDraft;};
...
export function applyCanonicalAtCurrent(document:CanonicalSchemaDocument,command:CanonicalCommand):CanonicalCommandResult {
  ...
  if(command.kind==="set"){
    ...
    if(source&&command.patch.type==="boolean")try{
      const candidate={...source,...clone(command.patch)},repair=repairCanonicalBooleanAllowedValues(candidate,"boolean");
      if(repair.repairCount)command={...command,patch:{...command.patch,allowedValues:repair.allowedValues}};
    }catch(error){return{status:"conflict",document,propertyId:command.propertyId,message:error instanceof Error?error.message:String(error)};}
  }
  const next=clone(document);
  if(command.kind==="policy"){next.onlyDefinedFields=command.onlyDefinedFields;return{status:"applied",document:completeDraftCommand(next)};}
  if(command.kind==="add"){if(command.parentId&&!next.nodes[command.parentId])throw new Error(`Parent property ${command.parentId} is unavailable.`);const propertyId=command.id("property"),node:CanonicalPropertyNode={id:propertyId,name:command.name.trim()||"property",...(command.parentId?{parentId:command.parentId}:{}),order:insertOrder(next,command.parentId,command.afterId),type:command.type,presence:{mode:"optional"},allowedValues:[],rules:[],documentation:emptyDocumentation(),provenance:[{source:"created"}],overrideReferences:[]};for(const sibling of orderWithin(next,command.parentId))if(sibling.order>=node.order)sibling.order+=1;next.nodes[propertyId]=node;if(!command.parentId)next.rootIds=orderWithin(next).map(({id})=>id);next.selectedPropertyId=propertyId;return{status:"applied",document:completeDraftCommand(next)};}
  ...
  if(command.kind==="rename"){node!.name=command.name.trim()||node!.name;return{status:"applied",document:completeDraftCommand(next)};}
  if(command.kind==="set"){
    ...
    for(const operation of command.operations??[])applyStructuralOperation(next,operation);
    return{status:"applied",document:completeDraftCommand(next)};
  }
  if(command.kind==="type"){const descendants=orderedIds(next,command.propertyId),nextItemSchema=command.type==="array"&&command.itemType?{id:`item:${node!.id}`,type:command.itemType}:undefined,change=canonicalTypeTransition(node!,command.type,command.itemType,nextItemSchema,descendants,next);if(change.confirmationRequired&&!command.confirmed)return{status:"confirmation-required",document,propertyId:command.propertyId,impact:change.impact!};let repair;try{repair=repairCanonicalBooleanAllowedValues(node!,command.type);}catch(error){return{status:"conflict",document,propertyId:command.propertyId,message:error instanceof Error?error.message:String(error)};}if(change.removeDescendants)for(const id of descendants)delete next.nodes[id];node!.type=command.type;if(repair.repairCount)node!.allowedValues=repair.allowedValues;if(command.type==="array"&&command.itemType){node!.itemType=command.itemType;node!.itemSchema=nextItemSchema!;}else{delete node!.itemType;delete node!.itemSchema;}return{status:"applied",document:completeDraftCommand(next)};}
  if(command.kind==="delete"){const descendants=[command.propertyId,...orderedIds(next,command.propertyId)],parentId=node!.parentId;for(const id of descendants)delete next.nodes[id];normalizeOrders(next,parentId);next.rootIds=orderWithin(next).map(({id})=>id);if(descendants.includes(next.selectedPropertyId??""))delete next.selectedPropertyId;return{status:"applied",document:completeDraftCommand(next)};}
  if(command.kind==="move"){if(command.parentId===command.propertyId||orderedIds(next,command.propertyId).includes(command.parentId??""))throw new Error("A property cannot move inside itself.");const oldParent=node!.parentId,oldSiblings=orderWithin(next,oldParent).filter(({id})=>id!==command.propertyId),targetSiblings=orderWithin(next,command.parentId).filter(({id})=>id!==command.propertyId),afterIndex=command.afterId?targetSiblings.findIndex(({id})=>id===command.afterId):-1,insertAt=afterIndex<0?0:afterIndex+1;oldSiblings.forEach((sibling,index)=>{sibling.order=index;});targetSiblings.splice(insertAt,0,node!);targetSiblings.forEach((sibling,index)=>{sibling.order=index;});delete node!.parentId;if(command.parentId)node!.parentId=command.parentId;next.rootIds=orderWithin(next).map(({id})=>id);return{status:"applied",document:completeDraftCommand(next)};}
  const source=node!,copies=new Map<string,string>();for(const sourceId of [command.propertyId,...orderedIds(next,command.propertyId)]){const copyId=command.id("property");copies.set(sourceId,copyId);const original=next.nodes[sourceId]!,copy=clone(original);copy.id=copyId;copy.name=sourceId===command.propertyId?`${original.name} copy`:original.name;const copyParent=sourceId===command.propertyId?original.parentId:copies.get(original.parentId!);delete copy.parentId;if(copyParent)copy.parentId=copyParent;copy.provenance=[...copy.provenance,{source:"created"}];next.nodes[copyId]=copy;}const rootCopy=copies.get(command.propertyId)!;next.nodes[rootCopy]!.order=source.order+1;normalizeOrders(next,source.parentId);next.rootIds=orderWithin(next).map(({id})=>id);next.selectedPropertyId=rootCopy;return{status:"applied",document:completeDraftCommand(next)};
}
