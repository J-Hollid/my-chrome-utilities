...
export function applyCanonicalAtCurrent(document:CanonicalSchemaDocument,command:CanonicalCommand):CanonicalCommandResult {
  ...
  if(command.kind==="set"){
    const descendants=orderedIds(next,command.propertyId),nextType=command.patch.type??node!.type,
      ...
      itemDestructive=itemChange&&descendants.length>0;
    ...
    if(destructive||itemDestructive)for(const id of descendants)delete next.nodes[id];
    Object.assign(node!,clone(command.patch));
    if(nextType!=="array"){delete node!.itemType;delete node!.itemSchema;}else if(node!.itemType)node!.itemSchema=itemSchema(node!.id,node!.itemType);
  ...
  if(command.kind==="type"){const descendants=orderedIds(next,command.propertyId),destructive=node!.type==="object"&&command.type!=="object"&&descendants.length>0,itemChange=node!.type==="array"&&command.type==="array"&&node!.itemType!==command.itemType,itemDestructive=itemChange&&descendants.length>0;if((destructive||itemChange)&&!command.confirmed)return{status:"confirmation-required",document,propertyId:command.propertyId,impact:destructive?"child definitions and documentation removed; destructive confirmation required":itemDestructive?`item fields ${descendants.map((id)=>next.nodes[id]?.name).filter(Boolean).join(", ")} removed when every item changes from ${node!.itemType??"unspecified"} to ${command.itemType??"unspecified"}`:`every item changes from ${node!.itemType??"unspecified"} to ${command.itemType??"unspecified"}`};if(destructive||itemDestructive)for(const id of descendants)delete next.nodes[id];node!.type=command.type;if(command.type==="array"&&command.itemType){node!.itemType=command.itemType;node!.itemSchema=itemSchema(node!.id,command.itemType);}else{delete node!.itemType;delete node!.itemSchema;}return{status:"applied",document:appendChange(next,command,[command.propertyId,...descendants])};}
