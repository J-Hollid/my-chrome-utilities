...
type StandardSchema = Record<string, unknown> & {
  type?: string;
  properties?: Record<string, StandardSchema>;
  items?: StandardSchema;
  required?: string[];
  allOf?: StandardSchema[];
};
...
export interface JsonSchemaCompatibilityReview {
  omitted: OmittedRule[];
  conversions: ConvertedRule[];
}

function clone<T>(value: T): T { return structuredClone(value); }
function slug(value: string): string { return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, ""); }
function pointerSegments(path: string): string[] { return canonicalNestedPath(path).split("/").filter(Boolean).map((segment) => segment.replaceAll("~1", "/").replaceAll("~0", "~")); }
function normalizedOperator(rule: AttachedSchemaRule): string { return rule.operator?.replaceAll("_", "-").replaceAll(" ", "-").toLowerCase() ?? ""; }

export function jsonSchemaResourceId(schema: Pick<SchemaDefinition, "id" | "version">): string {
  return `${RESOURCE_BASE}/${encodeURIComponent(schema.id)}/revisions/${schema.version}`;
}

export function jsonSchemaResourceFilename(schema: Pick<SchemaDefinition, "name" | "version">): string {
...
function mergeDocuments(parent: JsonSchema, child: JsonSchema): JsonSchema {
  return {
    ...clone(parent),
    ...clone(child),
...
function effectiveDocument(schema: SchemaDefinition, schemas: readonly SchemaDefinition[]): JsonSchema {
  const chain = schemaChain(schema, schemas);
  let result: JsonSchema = { type:"object" };
  for (const owner of chain) {
    ...
    if (disabled.length && result.properties) result.properties = Object.fromEntries(Object.entries(result.properties).filter(([name]) => !disabled.includes(name)));
  }
  return result;
}

function effectiveRules(schema: SchemaDefinition, schemas: readonly SchemaDefinition[]): AttachedSchemaRule[] {
  ...
  for (const owner of schemaChain(schema, schemas)) {
    ...
    for (const [id, rule] of rules) if (disabled.has(canonicalNestedPath(rule.propertyPath ?? id)) || disabled.has(canonicalNestedPath(id))) rules.delete(id);
    for (const rule of owner.attachedRules ?? []) {
      if (rule.enabled === false) rules.delete(rule.id);
      else rules.set(rule.id, clone(rule));
    }
  }
  return [...rules.values()];
}

function standardDocument(source: JsonSchema, closeObjects: boolean): StandardSchema {
  ...
  if (source.properties) result.properties = Object.fromEntries(Object.entries(source.properties).map(([name, child]) => [name, standardDocument(child, closeObjects)]));
  if (source.items) result.items = standardDocument(source.items, closeObjects);
  ...
  else if (source.additionalProperties !== undefined) result.additionalProperties = source.additionalProperties;
  if (source.forbidden?.length) result.not = { anyOf:source.forbidden.map((name) => ({ required:[name] })) };
  return result;
}

function targetAtPath(document: StandardSchema, path: string, create = true): StandardSchema | undefined {
  let current = document;
  for (const segment of pointerSegments(path)) {
    if (segment === "*" || /^\d+$/.test(segment)) {
      if (!current.items && create) current.items = {};
      current = current.items as StandardSchema;
    } else {
      if (!current.properties && create) current.properties = {};
      if (!current.properties?.[segment] && create) current.properties![segment] = {};
      current = current.properties?.[segment] as StandardSchema;
    }
...
function parentAndName(document: StandardSchema, path: string): { parent: StandardSchema; name: string } | undefined {
  const segments = pointerSegments(path); const name = segments.at(-1);
...
function appendRequired(document: StandardSchema, path: string): void {
  const target = parentAndName(document, path); if (!target) return;
  target.parent.required = [...new Set([...(target.parent.required ?? []), target.name])];
}

function appendForbidden(document: StandardSchema, path: string): void {
  const target = parentAndName(document, path); if (!target) return;
  const not = (target.parent.not ?? { anyOf:[] }) as { anyOf?: StandardSchema[] };
...
function typedParameter(rule: AttachedSchemaRule, target: StandardSchema): unknown {
  const value = rule.parameters ?? "";
  if ((rule.applicableType ?? target.type) === "number") return Number(value);
  if ((rule.applicableType ?? target.type) === "boolean") return value === "true";
  return value;
}
...
function standardAssertion(rule: AttachedSchemaRule, target: StandardSchema): StandardSchema | undefined {
  ...
  if (operator === "exact-value") return { const:typedParameter(rule, target) };
  if (operator === "allowed-values") return { enum:clone(rule.allowedValues ?? rule.parameters?.split(",").map((value) => value.trim()).filter(Boolean) ?? []) };
  if (operator === "regular-expression") return { pattern:rule.parameters ?? "" };
  if (operator === "digits-only") return { pattern:"^[0-9]+$" };
  if (operator === "non-empty-string") return { minLength:1 };
  ...
  if (operator === "text-length") return cardinalityAssertion(rule, "Length");
  if (operator === "item-count") return cardinalityAssertion(rule, "Items");
  if (operator === "value-type") return rule.parameters ?? target.type ? { type:(rule.parameters ?? target.type) as string } : {};
  return undefined;
}

function predicateAssertion(predicate: ConditionalRulePredicate): StandardSchema {
  if (predicate.operator === "Exists") { const result: StandardSchema = {}; appendRequired(result, predicate.propertyPath); return result; }
  if (predicate.operator === "Does not exist") { const result: StandardSchema = {}; appendForbidden(result, predicate.propertyPath); return result; }
  const targetAssertion: StandardSchema = predicate.operator === "Equals" ? { const:predicate.comparison?.value }
...
function applyRule(document: StandardSchema, rule: AttachedSchemaRule): boolean {
  const operator = normalizedOperator(rule); const path = rule.propertyPath ?? "";
  ...
  if (operator === "forbidden-property") { appendForbidden(document, path); return true; }
  if (operator === "allow-undeclared-properties") { const target = targetAtPath(document, path); if (target) target.additionalProperties = true; return true; }
  const target = targetAtPath(document, path); const assertion = target && standardAssertion(rule, target);
  if (!target || !assertion) return false;
  Object.assign(target, assertion); return true;
}

function decorateDocumentation(document: StandardSchema, schema: SchemaDefinition, schemas: readonly SchemaDefinition[]): void {
  const documentation = resolveEffectiveSchemaDocumentation(schema, schemas);
  if (documentation.description) document.description = documentation.description;
  for (const [path, entry] of Object.entries(documentation.properties)) {
    const target = targetAtPath(document, path, false); if (!target) continue;
    if (entry.description) target.description = entry.description;
    if (entry.example) target.examples = [clone(entry.example.value)];
  }
}
...
export function exportJsonSchemaBundle(schemas: readonly SchemaDefinition[]): { document: StandardSchema; filename: string; resourceIds: string[]; compatibility: JsonSchemaCompatibilityReview } {
...
export function createExtensionSchemaPackage<TRule extends { id: string }>(schema: SchemaDefinition, schemas: readonly SchemaDefinition[], rules: readonly TRule[]): { version: 1; schemas: SchemaDefinition[]; rules: TRule[] } {
