...
interface ImportProjectDialog {
  targetName: string;
  summary: string;
  blocked: boolean;
  commit(name: string, progress: (message: string) => void): Promise<string>;
  cancel(): void;
  dispose(): void;
  restoreFocus(): void;
}

export function openImportProjectDialog(options: ImportProjectDialog): void {
...
export function openImportErrorDialog(error: unknown, restoreFocus: () => void): void {
  const dialog = document.createElement("dialog");
  const heading = document.createElement("h4");
  const summary = document.createElement("p");
