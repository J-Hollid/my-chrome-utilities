...
export function captureLiveInspectorPresentation(
  ...
  const focusedPropertyPath = focusedInsideInspector ? focusedElement?.closest<HTMLElement>(".live-validation-property")?.dataset.propertyPath : undefined;
  return {
...
export function restoreLiveInspectorPresentation(
  ...
  if (!snapshot) return;
  const properties = inspector?.querySelector<HTMLElement>('[aria-label="Properties"]');
  ...
  if (showingNonApplicable !== snapshot.showNonApplicableProperties) {
    inspector?.querySelector<HTMLButtonElement>("#live-non-applicable-properties")?.click();
  }
