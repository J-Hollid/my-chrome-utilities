...
import {explainRule, type RuleStep} from './rule-explanation.js';

const byId = (id: string): HTMLElement => document.getElementById(id)!;
const resultText = (value: boolean): string => value ? 'True' : 'False';
const label = (rule: RuleRow): string => rule.name || `Load rule ${rule.id}`;

function stepNode(step: RuleStep): HTMLElement {
  const node = document.createElement('div');node.className = 'rule-step';
  ...
  if (step.actual !== undefined) {
    const actual = document.createElement('p');actual.textContent = `Current value: ${step.actual}`;node.append(actual);
  }
  for (const child of step.children ?? []) node.append(stepNode(child));
  return node;
}

export function renderRules(state: LiveState, select: (key: string) => void): void {
  ...
  state.rules.forEach((rule, index) => {
    let button = existing.get(rule.key) as HTMLButtonElement | undefined;
    if (!button) {
      button = document.createElement('button');button.className = 'rule';button.dataset.key = rule.key;
    ...
    button.classList.toggle('rule-true', rule.result);button.classList.toggle('rule-false', !rule.result);
  ...
  if (!explanation) conditions.textContent = 'Published condition details are unavailable for this rule.';
  else {
...
export function renderTagRules(row: TagRow | undefined, rules: RuleRow[], select: (key: string) => void): void {
  const list = byId('tag-rules');list.replaceChildren();
  if (!row) return;
  ...
  if (!linked.length) {list.textContent = 'No evaluated load rules are available for this tag.';return;}
  for (const rule of linked) {
    const button = document.createElement('button');button.className = `rule ${rule.result ? 'rule-true' : 'rule-false'}`;
    button.textContent = `${label(rule)} · ${rule.id} · ${resultText(rule.result)}`;
    button.onclick = () => select(rule.key);list.append(button);
  }
}
