...
import { createLiveOwner, type SurfaceState } from './owner.js';
...
let owner: ReturnType<typeof createLiveOwner> | undefined, state: SurfaceState | undefined;
let ownerEnded = false;
type Action = Parameters<ReturnType<typeof createLiveOwner>['action']>[0];
...
const render = (value: SurfaceState): void => {
  ...
  element<HTMLButtonElement>('access').disabled = !pageOrigin(value.live.url);
  ...
  if (frameAccess.dataset.origins !== JSON.stringify(origins)) {
    ...
    frameAccess.replaceChildren(...origins.map(origin => {
      ...
      button.onclick = async () => {
        if (await chrome.permissions.request({origins: [origin]})) action({name: 'access'});
        else element('feedback').textContent = 'Frame permission required';
      };
      return button;
    }));
  }
  if (ownerEnded) {
    for (const button of Array.from(document.querySelectorAll<HTMLButtonElement>('#start, #pause, #resume, #end, #access, #retry, #frame-access button, #names-retry, #names-access'))) {
      button.disabled = true;
    }
  }
};
const client = connectUtilityPage(window, message => {
  if (!client.ownsWork && message.kind === 'state' && message.payload) render(message.payload as SurfaceState);
  ...
  if (!client.ownsWork) return;
  if (message.kind === 'action' && message.payload) owner?.action(message.payload as Action);
  if (message.kind === 'stop') owner?.action({name: 'end'});
  if (message.kind === 'reset') owner?.action({name: 'reset'});
  if (message.kind === 'target-closed') owner?.session.closeTarget();
  if (message.kind === 'close') owner?.dispose();
});

function useTarget(tabId: number): void {
  if (owner || !client.ownsWork) return;
  owner = createLiveOwner(tabId, value => { render(value); client.send('state', value); });
}

async function listTargets(all = false): Promise<void> {
  const tabs = await chrome.tabs.query(all ? {} : {active: true, lastFocusedWindow: true});
  const select = element<HTMLSelectElement>('targets');
  select.replaceChildren(...tabs.filter(tab => tab.id !== undefined &&
    !(tab.url ?? '').startsWith(chrome.runtime.getURL(''))).map(tab =>
    new Option(tab.title ?? tab.url ?? `Website tab ${tab.id}`, String(tab.id))));
}
...
element('names-access').onclick = async () => {
  if (await chrome.permissions.request({origins: [METADATA_ORIGIN]})) action({name: 'metadata-retry'});
};
...
element('copy-source').onclick = async () => {
  ...
  try {
    ...
    await navigator.clipboard.writeText(url); action({name: 'feedback', message: 'Source URL copied'});
  }
  catch { action({name: 'feedback', message: 'The source URL could not be copied'}); }
};
element('access').onclick = async () => {
  const origin = pageOrigin(state?.live.url ?? '');
  if (!origin) { element('status').textContent = 'This page cannot grant website access'; return; }
  const granted = await chrome.permissions.request({origins: [origin]});
  if (granted) action({name: 'access'});
  else element('status').textContent = 'Permission required';
};
element('browse').onclick = async () => {
  if (await chrome.permissions.request({permissions: ['tabs']})) {
    await listTargets(true); action({name: 'access'});
  }
};
element('retry').onclick = () => action({name: 'access'});
element('use-target').onclick = () => {
  const value = element<HTMLSelectElement>('targets').value;
  if (value) useTarget(Number(value));
};
window.addEventListener('pagehide', () => owner?.dispose(), {once: true});
if (client.ownsWork) {
  if (client.identity.targetId !== null) useTarget(client.identity.targetId);
  else void listTargets();
}
