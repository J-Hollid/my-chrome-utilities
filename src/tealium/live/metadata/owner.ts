...
export function metadataOwner(current: () => LiveState, publish: () => void,
  ...
  const begin = (utid: string, rows: TagRow[]): void => {
    ...
    void (async () => {
      try {
        if (!await permission()) {entry.needsAccess = true; throw Error('Access to my.tealiumiq.com is required');}
        if (entry.controller.signal.aborted) return;
        const result = await request(utid, entry.controller.signal);
        if (disposed || entry.controller.signal.aborted || entries.get(utid) !== entry) return;
        entry.result = result;
        entry.status = 'Names loaded';
      } catch (error) {if (entry.controller.signal.aborted) return;entry.status = 'Names unavailable';entry.reason = error instanceof Error ? error.message : 'Metadata request failed';}
      if (disposed || entry.controller.signal.aborted || entries.get(utid) !== entry) return;
      apply(); if (current().status !== 'Paused') publish();
    })();
  };
  const update = (): void => {
    const live = current();
    ...
    for (const [utid, entry] of entries) {
      ...
      if ([...entry.contexts].some(value => !contexts.has(value))) {
        entry.controller.abort();delete entry.result;entry.status = 'Names unavailable';entry.reason = 'The observed document changed';
      }
    }
    if (live.status === 'Paused') return;
    for (const [utid, rows] of groups) {
      const entry = entries.get(utid);
      if (!entry) begin(utid, rows);
      else if (entry.result && !entry.controller.signal.aborted) for (const row of rows) entry.contexts.add(context(row));
    }
    apply();
  };
  return {
    ...
    retry(): void {
      if (disposed || current().status !== 'Observing') return;
      for (const entry of entries.values()) entry.controller.abort();
      const previous = new Map([...entries].map(([utid, entry]) => [utid, entry.result]));
      entries.clear();update();
      for (const [utid, entry] of entries) {const result = previous.get(utid);if (result) entry.result = result;}
      apply();publish();
    },
    dispose(): void {disposed = true;for (const entry of entries.values()) entry.controller.abort();},
  };
}
