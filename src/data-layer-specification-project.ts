...
function preserveCanonicalFacets(base:CanonicalSchemaDocument,current:CanonicalSchemaDocument,next:CanonicalSchemaDocument,nextSource:CanonicalSchemaDocument,affectedPropertyIds:readonly string[]):CanonicalSchemaDocument{
  ...
  const baseOrNextSourcePath={...baseSourcePaths,...nextSourcePaths},revision=current.revision+1,changed=[...new Set(affectedPropertyIds.map((id)=>currentBySourcePath.get(baseOrNextSourcePath[id]??"")??nextSourceId(id)))].sort();
  return{...clone(next),revision,view:current.view,nodes,rootIds:next.rootIds.map(propertyId),...(selected?{selectedPropertyId:selected}:{}),...(sourceContent?{sourceContent}:{}),changes:[...clone(current.changes),{revision,propertyIds:changed,kind:"synchronize"}]};
}
