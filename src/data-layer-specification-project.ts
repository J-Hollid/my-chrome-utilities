...
export function commitBulkProperties(state:ProjectState,profileId:string,properties:readonly {path:string;type:string}[]):{state:ProjectState;errors:{index:number;path:string;message:string}[]}{
  ...
  const migratedState:ProjectState=profile.canonicalSchema&&profile.requirements.length===0?state:{...state,project:{...state.project,collections:{...state.project.collections,profiles:state.project.collections.profiles.map((candidate)=>candidate.id===profileId?{...candidate,canonicalSchema:migratedCanonical,requirements:[]}:candidate)}}};
  let canonical=migratedCanonical;
  for(const property of properties)canonical=canonicalSchemaWithConstraint(canonical,property,(kind)=>`${kind}:${profile.id}:bulk:${++sequence}`);
...
function synchronizeSavedSchemaRules(base:readonly AttachedSchemaRule[],current:readonly AttachedSchemaRule[],next:readonly AttachedSchemaRule[]):AttachedSchemaRule[]{
  ...
  return ids.flatMap((id)=>{const oldRule=baseById.get(id),localRule=currentById.get(id),newRule=nextById.get(id);if(!oldRule)return[clone(localRule??newRule!)];if(!localRule)return[];if(!newRule)return equalJson(localRule,oldRule)?[]:[clone(localRule)];return[synchronizeDocument(oldRule,localRule,newRule) as AttachedSchemaRule];});
}
function synchronizeCanonicalRules(base:readonly CanonicalRule[],current:readonly CanonicalRule[],next:readonly CanonicalRule[]):CanonicalRule[]{
  const baseById=new Map(base.map((rule)=>[rule.id,rule])),currentById=new Map(current.map((rule)=>[rule.id,rule])),nextById=new Map(next.map((rule)=>[rule.id,rule])),ids=[...next.map(({id})=>id),...current.map(({id})=>id).filter((id)=>!nextById.has(id))];
  return ids.flatMap((id)=>{const oldRule=baseById.get(id),localRule=currentById.get(id),newRule=nextById.get(id);if(!oldRule)return[clone(localRule??newRule!)];if(!localRule)return[];if(!newRule)return equalJson(localRule,oldRule)?[]:[clone(localRule)];return[synchronizeDocument(oldRule,localRule,newRule) as CanonicalRule];});
}
function canonicalSourceChangeIds(base:CanonicalSchemaDocument,next:CanonicalSchemaDocument):string[]{
  const paths=(document:CanonicalSchemaDocument)=>document.sourceContent?.pathsByNodeId??Object.fromEntries(Object.values(document.nodes).map((node)=>[node.id,canonicalPropertyPath(document,node.id)])),basePaths=paths(base),nextPaths=paths(next),baseByPath=new Map(Object.entries(basePaths).map(([id,path])=>[path,base.nodes[id]!])),nextByPath=new Map(Object.entries(nextPaths).map(([id,path])=>[path,next.nodes[id]!]));
  const normalized=(document:CanonicalSchemaDocument,node:CanonicalPropertyNode,pathById:Record<string,string>)=>{const{id:_id,...value}=clone(node),definition=document.sourceContent?.definitionsByNodeId?.[node.id],propertyPath=(id:string)=>pathById[id]??id,predicate=(entry:CanonicalPredicate):unknown=>entry.kind==="predicate"?{...entry,propertyId:propertyPath(entry.propertyId)}:{...entry,children:entry.children.map(predicate)};if(value.parentId)value.parentId=propertyPath(value.parentId);value.provenance=value.provenance.map(({revision:_revision,...entry})=>entry);value.allowedValues=value.allowedValues.map(({id:_allowedId,...entry})=>entry) as CanonicalPropertyNode["allowedValues"];value.rules=value.rules.map((rule)=>({...rule,id:rule.id.replace(node.id,propertyPath(node.id)),...(rule.condition?{condition:predicate(rule.condition) as CanonicalPredicate}:{}),...(rule.provenance?{provenance:(({revision:_revision,...entry})=>entry)(rule.provenance)}:{})}));return{node:value,definition:clone(definition)};},sourcePaths=new Set([...baseByPath.keys(),...nextByPath.keys()]);
  return[...sourcePaths].flatMap((path)=>{const oldNode=baseByPath.get(path),newNode=nextByPath.get(path);return!oldNode||!newNode||!equalJson(normalized(base,oldNode,basePaths),normalized(next,newNode,nextPaths))?[newNode?.id??oldNode!.id]:[];}).sort();
}
function preserveCanonicalFacets(base:CanonicalSchemaDocument,current:CanonicalSchemaDocument,next:CanonicalSchemaDocument,nextSource:CanonicalSchemaDocument,affectedPropertyIds:readonly string[]):CanonicalSchemaDocument{
  const activePathById=Object.fromEntries(Object.values(current.nodes).map((node)=>[node.id,canonicalPropertyPath(current,node.id)])),currentByPath=new Map(Object.entries(activePathById).map(([id,path])=>[path,id])),currentSourcePathById=current.sourceContent?.pathsByNodeId??{},currentBySourcePath=new Map(Object.entries(currentSourcePathById).map(([id,path])=>[path,id])),nextIds=new Map(Object.values(next.nodes).map((node)=>[node.id,currentByPath.get(canonicalPropertyPath(next,node.id))??node.id]));
  const sourcePaths=(document:CanonicalSchemaDocument)=>document.sourceContent?.pathsByNodeId??Object.fromEntries(Object.values(document.nodes).map((node)=>[node.id,canonicalPropertyPath(document,node.id)])),baseSourcePaths=sourcePaths(base),baseIds=new Map(Object.entries(baseSourcePaths).map(([id,path])=>[id,currentBySourcePath.get(path)??(current.nodes[id]?id:currentByPath.get(path)??id)]));
  const remapNode=(node:CanonicalPropertyNode,ids:Map<string,string>):CanonicalPropertyNode=>{const propertyId=(id:string)=>ids.get(id)??id,stableId=propertyId(node.id),embeddedId=(id:string)=>id.replace(node.id,stableId),predicate=(value:CanonicalPredicate):CanonicalPredicate=>value.kind==="predicate"?{...clone(value),propertyId:propertyId(value.propertyId)}:{...clone(value),children:value.children.map(predicate)};return{...clone(node),id:stableId,...(node.parentId?{parentId:propertyId(node.parentId)}:{}),presence:{...clone(node.presence),...(node.presence.condition?{condition:predicate(node.presence.condition)}:{})},allowedValues:node.allowedValues.map((entry)=>({...clone(entry),id:embeddedId(entry.id)})),rules:node.rules.map((rule)=>({...clone(rule),id:embeddedId(rule.id),...(rule.condition?{condition:predicate(rule.condition)}:{})}))};},alignedBase=Object.fromEntries(Object.values(base.nodes).map((node)=>{const aligned=remapNode(node,baseIds);return[aligned.id,aligned];})),nextSourcePaths=sourcePaths(nextSource),generatedByPath=new Map(Object.values(next.nodes).map((node)=>[canonicalPropertyPath(next,node.id),nextIds.get(node.id)??node.id])),sourceIds=new Map(Object.entries(nextSourcePaths).map(([id,path])=>[id,currentBySourcePath.get(path)??currentByPath.get(path)??generatedByPath.get(path)??id])),alignedSource=Object.fromEntries(Object.values(nextSource.nodes).map((node)=>{const aligned=remapNode(node,sourceIds);return[aligned.id,aligned];})),nodes:CanonicalSchemaDocument["nodes"]={};
  const facets=(["name","parentId","order","type","itemType","presence","allowedValues","documentation","expectedValue","enforcement","target","overrideReferences","provenance"] as const);
  for(const generatedNode of Object.values(next.nodes)){
    const nextNode=remapNode(generatedNode,nextIds),currentNode=current.nodes[nextNode.id],baseNode=alignedBase[nextNode.id] as CanonicalPropertyNode|undefined,sourceNode=alignedSource[nextNode.id] as CanonicalPropertyNode|undefined;
    if(!currentNode){nodes[nextNode.id]=nextNode;continue;}
    const merged=clone(nextNode) as CanonicalPropertyNode;
    for(const facet of facets){const value=baseNode&&sourceNode?synchronizeDocument(baseNode[facet],currentNode[facet],sourceNode[facet]):clone(currentNode[facet]);if(value===undefined)delete(merged as unknown as Record<string,unknown>)[facet];else(merged as unknown as Record<string,unknown>)[facet]=value;}
...
export function adoptSavedSchema(state:ProjectState,source:SavedSchemaSource):ProjectState{
  ...
  return transactProject(state,`Adopt saved schema ${source.name}`,(project)=>{
    ...
    const preferredProfileId=`profile:${source.id}`,occupiedProfileIds=new Set(project.collections.profiles.map(({id})=>id));let profileId=preferredProfileId,suffix=0;while(occupiedProfileIds.has(profileId)){suffix+=1;profileId=`${preferredProfileId}:saved-schema-contributor${suffix===1?"":`-${suffix}`}`;}
    let canonicalSequence=0;const canonicalSchema=savedSchemaCanonicalDocument({id:source.id,name:source.name,version:source.version,document:clone(source.document),attachedRules:clone(source.rules??[]),...(source.documentation===undefined?{}:{documentation:clone(source.documentation) as NonNullable<SchemaDefinition["documentation"]>})},(kind)=>`${profileId}:${kind}:${++canonicalSequence}`,{id:`canonical:${source.id}`,contributorId:profileId,contributorName:source.name});canonicalSchema.sourceContent={...canonicalSchema.sourceContent!,document:clone(source.document),rules:clone(source.rules??[]) as unknown as readonly Record<string,unknown>[],documentation:clone(source.documentation??""),examples:clone(source.examples??[])};
...
export function stageSavedSchemaSynchronization(state:ProjectState,source:SavedSchemaSource):SavedSchemaSynchronizationReview{
  ...
  if(!adopted||!canonical||canonical.source?.identity!==source.id)throw new Error(`Saved schema ${source.name} has not been adopted.`);
  const fromRevision=Number(adopted.sourceRevision??canonical.source.revision),base=canonical.sourceContent?.document??{},projected=savedSchemaFromCanonical({id:source.id,name:adopted.name,version:fromRevision,document:clone(base),assignments:[],attachedRules:clone(canonical.sourceContent?.rules??[]) as unknown as AttachedSchemaRule[]},canonical),current=projected.document;
...
export function commitSavedSchemaSynchronization(state:ProjectState,review:SavedSchemaSynchronizationReview):ProjectState{
  return transactProject(state,`Synchronize saved schema ${review.schemaId} to revision ${review.toRevision}`,(project)=>{
    ...
    if(!adopted||!canonical||canonical.source?.identity!==review.schemaId||Number(adopted.sourceRevision??canonical.source.revision)!==review.fromRevision)throw new Error("Saved-schema synchronization review is stale.");
    const priorContent=canonical.sourceContent??{document:{},rules:[],documentation:{},examples:[]},projected=savedSchemaFromCanonical({id:review.schemaId,name:adopted.name,version:review.fromRevision,document:clone(priorContent.document),assignments:[],attachedRules:clone(priorContent.rules) as unknown as AttachedSchemaRule[],documentation:clone(priorContent.documentation) as NonNullable<SchemaDefinition["documentation"]>},canonical),document=synchronizeDocument(priorContent.document,projected.document,review.source.document) as Record<string,unknown>,rules=synchronizeSavedSchemaRules(priorContent.rules as unknown as AttachedSchemaRule[],projected.attachedRules??[],review.source.rules??[]),documentation=synchronizeDocument(priorContent.documentation,projected.documentation??{},review.source.documentation??{}),examples=synchronizeDocument(priorContent.examples,priorContent.examples,review.source.examples??[]) as readonly unknown[];
    let baseSequence=0;const base=savedSchemaCanonicalDocument({id:review.schemaId,name:adopted.name,version:review.fromRevision,document:clone(priorContent.document),attachedRules:clone(priorContent.rules) as unknown as AttachedSchemaRule[],documentation:clone(priorContent.documentation) as NonNullable<SchemaDefinition["documentation"]>},(kind)=>`${adopted.id}:base:${review.fromRevision}:${kind}:${++baseSequence}`,{id:canonical.id,contributorId:adopted.id,contributorName:adopted.name});
    let sourceSequence=0;const nextSource=savedSchemaCanonicalDocument({id:review.schemaId,name:adopted.name,version:review.toRevision,document:clone(review.source.document),attachedRules:clone(review.source.rules??[]),...(review.source.documentation===undefined?{}:{documentation:clone(review.source.documentation) as NonNullable<SchemaDefinition["documentation"]>})},(kind)=>`${adopted.id}:source:${review.toRevision}:${kind}:${++sourceSequence}`,{id:canonical.id,contributorId:adopted.id,contributorName:adopted.name}),affectedPropertyIds=canonicalSourceChangeIds(base,nextSource);
    let sequence=0;const generated=savedSchemaCanonicalDocument({id:review.schemaId,name:adopted.name,version:review.toRevision,document,attachedRules:rules,...(documentation===undefined?{}:{documentation:clone(documentation) as NonNullable<SchemaDefinition["documentation"]>})},(kind)=>`${adopted.id}:sync:${review.toRevision}:${kind}:${++sequence}`,{id:canonical.id,contributorId:adopted.id,contributorName:adopted.name});generated.sourceContent={...generated.sourceContent!,document:clone(review.source.document),rules:clone(review.source.rules??[]) as unknown as readonly Record<string,unknown>[],documentation:clone(review.source.documentation??{}),examples:clone(review.source.examples??examples)};const synchronized=preserveCanonicalFacets(base,canonical,generated,nextSource,affectedPropertyIds);
...
export type SavedSchemaReview={kind:"adopt";source:SavedSchemaSource}|{kind:"synchronize";review:SavedSchemaSynchronizationReview};
export function commitSavedSchemaReview(state:ProjectState,review:SavedSchemaReview):{state:ProjectState;profileId:string}{
  ...
  if(!profileId)throw new Error(`Saved schema ${sourceId} did not resolve its Shared Profile.`);
...
export function createFixtureFromCapturedValidation(state:ProjectState,input:CapturedValidationContinuation,id:IdFactory):ProjectState{
  ...
  const expected=assertedEvaluation(input),issues=input.evaluated.issueDetails.map(({path,code})=>({path:path??"/",code})),fixture={id:id("fixture"),name:input.name,testType:"event-validation",contributorId:input.contributorId,...(input.eventId?{eventId:input.eventId}:{}),...(input.pageId?{pageId:input.pageId}:{}),...(input.flowStepId?{flowStepId:input.flowStepId}:{}),input:clone(input.payload) as Record<string,unknown>,inputGuidance:{schemaId:input.contributorId,revision:String(input.evaluated.winner?.schemaRevision??"captured"),kind:"authoring-guidance"},sourceProvenance:{kind:"captured-live-validation",id:input.captureId,revision:input.evaluated.resultIdentity},sourceSnapshot:{sourceId:input.sourceId,eventName:input.eventName,contributorId:input.contributorId,evaluationIdentity:input.evaluated.resultIdentity,...(input.eventId?{eventId:input.eventId}:{}),...(input.pageId?{pageId:input.pageId}:{}),...(input.flowStepId?{flowStepId:input.flowStepId}:{})},reviewedExpectations:{outcome:issues.length?"Invalid":"Valid",issues},actualResult:{outcome:issues.length?"Invalid":"Valid",issues,evaluatorRevision:input.evaluated.resultIdentity,resultIdentity:input.evaluated.resultIdentity},evaluatorRevision:input.evaluated.resultIdentity,status:"Matched",differences:[],observations:[{sourceId:input.sourceId,eventName:input.eventName,payload:clone(input.payload)}],expected,assertions:[{field:"status",equals:expected.status},{field:"issueCodes",equals:clone(expected.issueCodes)}],evaluationResultIdentity:input.evaluated.resultIdentity,provenance:{kind:"captured-validation",captureId:input.captureId},evidenceStatus:"current"};
  return transactProject(state,`Create Test case from capture ${input.captureId}`,(project)=>({...project,collections:{...project.collections,fixtures:[...project.collections.fixtures,fixture]}}));
}
