import {canonicalConstraints,canonicalPropertyPath,canonicalRequirements,canonicalSchemaWithConstraint,createCanonicalSchema,migrateLegacyProfile,type CanonicalMigrationPlan,type CanonicalPredicate,type CanonicalSchemaDocument} from "./data-layer-canonical-schema.js";
...
function preserveCanonicalPropertyIds(current:CanonicalSchemaDocument,next:CanonicalSchemaDocument):CanonicalSchemaDocument{
  const currentByPath=new Map(Object.values(current.nodes).map((node)=>[canonicalPropertyPath(current,node.id),node.id])),ids=new Map(Object.values(next.nodes).map((node)=>[node.id,currentByPath.get(canonicalPropertyPath(next,node.id))??node.id])),propertyId=(id:string)=>ids.get(id)??id,predicate=(value:CanonicalPredicate):CanonicalPredicate=>value.kind==="predicate"?{...value,propertyId:propertyId(value.propertyId)}:{...value,children:value.children.map(predicate)},nodes:CanonicalSchemaDocument["nodes"]={};
  for(const node of Object.values(next.nodes)){const stableId=propertyId(node.id),embeddedId=(id:string)=>id.replace(node.id,stableId);nodes[stableId]={...clone(node),id:stableId,...(node.parentId?{parentId:propertyId(node.parentId)}:{}),presence:{...clone(node.presence),...(node.presence.condition?{condition:predicate(node.presence.condition)}:{})},allowedValues:node.allowedValues.map((entry)=>({...clone(entry),id:embeddedId(entry.id)})),rules:node.rules.map((rule)=>({...clone(rule),id:embeddedId(rule.id),...(rule.condition?{condition:predicate(rule.condition)}:{})}))};}
