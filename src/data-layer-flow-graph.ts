...
interface StoredDocumentaryFlowGraph extends FlowViewState {sections:ProjectEntity[];pageGroupIds:string[];pageFrames:DocumentaryPageFrameRecord[];occurrences:ProjectEntity[];relationships:DocumentaryRelationshipRecord[]}
...
export const saveStoredGraph=(project:SpecificationProject,flowId:string,graph:StoredDocumentaryFlowGraph):SpecificationProject=>{const{selectedItem:_selectedItem,viewport:_viewport,...semanticGraph}=graph;if(!Object.hasOwn(project.collections,"pageGroups")){delete (semanticGraph as Partial<StoredDocumentaryFlowGraph>).pageGroupIds;for(const frame of semanticGraph.pageFrames)delete (frame as DocumentaryPageFrameRecord&{pageGroupId?:string}).pageGroupId;}return{...project,documentationFlowGraphs:{...graphIndex(project),[flowId]:semanticGraph}};};
