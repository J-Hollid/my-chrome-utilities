...
export function utilityBrowserPorts(api: typeof globalThis.chrome | undefined) {
  return {
    selectTarget: async () => {
    ...
    subscribeTargetClosed: (listener: (id: number) => void) => {
...
export async function mountInstalledUtilityWorkspace(): Promise<{ mount(): void; dispose(): void }> {
  ...
  status.id = "data-layer-startup-status"; status.setAttribute("aria-live", "polite");
  status.textContent = "Opening Data Layer…";
  document.querySelector("#workspace-panel-data-layer")?.prepend(status);
  let closed = false;
  const onClose = (): void => { closed = true; workspace.dispose(); };
  ...
  try {
    ...
    if (closed) runtime.dispose();
    return {
      mount() { if (!closed) { runtime.mount(); status.remove(); } },
