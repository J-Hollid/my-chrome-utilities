...
export function renderCanonicalFocusedRules(host:HTMLElement,context:CanonicalFocusedRulesContext):void {
  ...
  working.rules.forEach((rule)=>{
    ...
    if(inherited)row.append(button(dom,"Override here",()=>{const next=context.getWorking();if(!next)return;const index=next.rules.findIndex(({id})=>id===rule.id);if(index<0)return;const replacement=clone(next.rules[index]!);replacement.id=context.id("rule");replacement.provenance={source:"created",state:"effective"};next.rules[index]=replacement;context.feedback(`Staged override of ${ruleKindLabel(rule)}.`);context.render();}),button(dom,"Open source",()=>{row.dataset.ruleMode="source";const source=dom.createElement("p");source.textContent=`Source rule ${rule.id} · ${ruleKindLabel(rule)} · inherited definition is read-only.`;row.append(source);}));else if(removed){const impact=dom.createElement("p");impact.textContent=`Impact review: ${ruleKindLabel(rule)} · effective result falls back to parent or unset.`;row.append(impact,button(dom,"Restore",()=>{context.removedRuleIds.delete(rule.id);context.render();}));}else row.append(button(dom,"Edit",()=>{row.dataset.ruleMode="edit";renderRuleEdit(row,rule,context);}),button(dom,"Remove local",()=>{context.removedRuleIds.add(rule.id);context.feedback(`Staged removal of ${ruleKindLabel(rule)}.`);context.render();}));list.append(row);
  });
