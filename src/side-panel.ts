...
function persistSchemaLibrary(): void {
  ...
  try{if(schemaValue!==previousSchemas)dataLayerStorage.setItem(SCHEMA_LIBRARY_STORAGE_KEY,schemaValue);if(nextProject&&project&&JSON.stringify(nextProject.project.collections.schemaDrafts)!==JSON.stringify(project.project.collections.schemaDrafts)){const result=commitCanonicalProjectState(projectStorage,nextProject,{expectedRevision:envelope?.revision??0,pendingLabel:"Side-panel schema edit",base:project});if(result.status==="conflict")throw new Error(`Schema edit conflicts with a newer Saved Draft.`);projectLibraryUi.captureActiveProject(nextProject,result.revision);}}
...
function recheckCapturedSchemaValidation(): void {
  ...
  liveObserverState = {
    ...
    events: events.map((event) => {
      ...
      if(evaluated?.winner){
        checked+=1;const schema=(canonical!.project.collections.schemaDrafts??[]).find(({id})=>id===evaluated.winner!.schemaId),state=evaluated.issueDetails.length?`${evaluated.issueDetails.length} issues` as const:"Valid" as const;
...
async function reviewCapturedValidationContinuation(record:SchemaValidationRecord,trigger:HTMLButtonElement):Promise<void>{
  ...
  if(!(project.project.collections.schemaDrafts??[]).some(({id})=>id===record.schemaId)){if(schemaResult)schemaResult.textContent=`Add ${record.schemaName??"the validated schema"} to ${project.project.name} before creating its Fixture.`;return;}
