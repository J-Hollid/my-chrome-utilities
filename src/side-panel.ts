...
function renderSchemaDraft(): void {
  ...
  const propertyItems = propertyRows.map((propertyRow) => {
    ...
    confirmType.addEventListener("click",()=>{if(!schemaDraft)return;try{schemaDraft=applySchemaPropertyTypeEdit(schemaDraft,{path:persistedPath,type:valueType.value as EditablePropertyType,...(valueType.value==="array"&&itemType.value?{itemType:itemType.value as Exclude<EditablePropertyType,"array">}:{}),treatment:treatment.value as TypeMismatchTreatment,removeIncompatible:false,resolutions:selectedResolutions()});persistSchemaEditorDraft(`Change ${persistedPath} type from ${schemaPropertyTypeLabel(property)} to ${valueType.value}`);renderSchemaDraft();}catch(error){typeReview.textContent=error instanceof Error?error.message:"Type change failed";}});
