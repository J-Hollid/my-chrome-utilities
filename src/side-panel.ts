...
function openContributorInUnifiedEditor(key:string):void{
  ...
  const contributorState=(live:ProjectState,selected:SidePanelContributorSelection,document:CanonicalSchemaDocument)=>selected.collectionKind==="pages"||selected.collectionKind==="pageGroups"?saveComposedCanonicalDocument(live,selected.collectionKind,selected.entity.id,document):selected.collectionKind==="events"?saveComposedEventCanonicalDocument(live,selected.entity.id,document):selected.scope==="Flow Page-instance"?saveFlowPageInstanceCanonicalDocument(live,selected.flowId!,selected.entity.id,document):selected.scope==="Event-occurrence"?saveEventOccurrenceCanonicalDocument(live,selected.flowId!,selected.entity.id,document):writeUnifiedContributorCanonical(live,selected,document),contributorPolicyState=(live:ProjectState,selected:SidePanelContributorSelection,value:boolean)=>selected.collectionKind==="pages"||selected.collectionKind==="pageGroups"||selected.collectionKind==="events"?saveComposedEntitySchemaPolicy(live,selected.collectionKind,selected.entity.id,value):selected.scope==="Flow Page-instance"?saveFlowContributorSchemaPolicy(live,selected.flowId!,"pageFrames",selected.entity.id,value):selected.scope==="Event-occurrence"?saveFlowContributorSchemaPolicy(live,selected.flowId!,"occurrences",selected.entity.id,value):live,historyKey=(projectId:string)=>compactCanonicalHistoryKey(projectId,key);
