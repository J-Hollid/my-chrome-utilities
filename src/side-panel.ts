...
const projectLibraryUi=mountProjectLibraryUi({root:document,storage:projectStorage,prepareProject:durableProjectRuntime.ensureProject,settled:durableProjectRuntime.settled,subscribe:(listener)=>durableProjectRuntime.subscribe(({library})=>listener(library)),blocked:()=>Boolean(durableProjectRuntime.failedSave()),exportProject:async(projectId)=>JSON.stringify(await durableProjectRuntime.repository.exportProject(projectId)),importProject:async(serialized,input)=>{await durableProjectRuntime.repository.importProject(JSON.parse(serialized) as Record<string,unknown>,input);},projectStorageKey:SPECIFICATION_PROJECT_STORAGE_KEY,navigationStorageKey:"my-chrome-utilities.specification-project-navigation.v1",openStudio:(url)=>{globalThis.open(url,"_blank");},onChange:renderSchemas});
...
void mountDurableProjectRepositoryUi(document,globalThis.indexedDB,durableProjectRuntime.repository).then((ui)=>{globalThis.addEventListener("durable-project-save-failed",(event)=>{const origin=document.activeElement instanceof HTMLElement?document.activeElement:undefined,failed=durableProjectRuntime.failedSave();if(!failed)return;projectLibraryUi.render();void ui.reportSaveFailure({projectId:failed.projectId,projectName:failed.projectName,command:failed.command,retry:async()=>{await durableProjectRuntime.retryFailedSave();projectLibraryUi.render();},exportUnsaved:()=>{const serialized=durableProjectRuntime.exportUnsavedDraft(),link=document.createElement("a");link.href=URL.createObjectURL(new Blob([serialized],{type:"application/json"}));link.download=`${failed.projectName.replace(/[^a-z0-9]+/gi,"-").toLowerCase()}-unsaved-draft.json`;link.click();URL.revokeObjectURL(link.href);},...(origin?{originControl:origin}:{})},(event as CustomEvent<{error?:unknown}>).detail?.error??failed.error);});}).catch((error)=>{const status=document.querySelector<HTMLOutputElement>("#durable-repository-status"),open=document.querySelector<HTMLButtonElement>("#open-storage-recovery");if(status)status.textContent=`Durable project storage unavailable: ${error instanceof Error?error.message:String(error)}`;if(open)open.disabled=true;});
...
function reviewSavedSchemaAdoption(schema:SchemaDefinition,trigger:HTMLButtonElement):void{
  ...
  confirm.addEventListener("click",()=>{try{const record=library.projects[picker.value]!;if(record.state.project.collections.schemaDrafts.some(({id})=>id===schema.id))throw new Error(`${schema.name} is already used by ${record.state.project.name}.`);projectLibraryUi.activate(record.state.project.id);const next=adoptSavedSchema(record.state,schema as unknown as Parameters<typeof adoptSavedSchema>[1]),result=commitCanonicalProjectState(projectStorage,next,{expectedRevision:record.revision,pendingLabel:`Adopt saved schema ${schema.name}`,base:record.state});if(result.status==="conflict")throw new Error(`Project changed in a newer Saved Draft; review adoption again.`);projectLibraryUi.captureActiveProject(next,result.revision);dialog.close();dialog.remove();if(schemaResult)schemaResult.textContent=`Adopted ${schema.name} revision ${schema.version} into ${record.state.project.name} with source lineage.`;renderSchemas();}catch(error){summary.textContent=error instanceof Error?error.message:String(error);}});
...
function persistSchemaLibrary(): void {
  ...
  try{if(schemaValue!==previousSchemas)dataLayerStorage.setItem(SCHEMA_LIBRARY_STORAGE_KEY,schemaValue);if(nextProject){const result=commitCanonicalProjectState(projectStorage,nextProject,{expectedRevision:envelope?.revision??0,pendingLabel:"Side-panel schema edit",...(project?{base:project}:{})});if(result.status==="conflict")throw new Error(`Schema edit conflicts with a newer Saved Draft.`);projectLibraryUi.captureActiveProject(nextProject,result.revision);}}
...
function reviewCapturedValidationContinuation(record:SchemaValidationRecord,trigger:HTMLButtonElement):void{
  ...
  confirm.addEventListener("click",()=>{try{const toProfile=destination.value==="profile";if(toProfile&&!profile.value)throw new Error("Choose a Profile for the evaluated requirements.");const next=toProfile?applyCapturedValidationToProfile(project,{captureId:record.eventId,profileId:profile.value,schemaId:record.schemaId!,evaluated:record.evaluated!}):createFixtureFromCapturedValidation(project,{name:name.value.trim(),captureId:record.eventId,sourceId:captured.sourceId,eventName:record.eventName,payload:captured.payload,schemaId:record.schemaId!,eventId:event.value,...(page.value?{pageId:page.value}:{}),...(step.value?{flowStepId:step.value}:{}),...(profile.value?{profileId:profile.value}:{}),evaluated:record.evaluated!},(kind)=>`${kind}:${crypto.randomUUID()}`),entity=toProfile?next.project.collections.profiles.find(({id})=>id===profile.value)!:next.project.collections.fixtures.at(-1)!,kind=toProfile?"profiles":"fixtures",result=commitCanonicalProjectState(projectStorage,next,{expectedRevision:envelope.revision,pendingLabel:`Continue evaluated capture ${record.eventId} as ${toProfile?"Profile requirements":"Fixture"}`,base:project});if(result.status==="conflict")throw new Error(`Project changed in a newer Saved Draft; review the continuation again.`);projectLibraryUi.captureActiveProject(next,result.revision);globalThis.localStorage.setItem("my-chrome-utilities.specification-project-navigation.v1",JSON.stringify({projectId:project.project.id,kind,id:entity.id,source:"side-panel",returnEventId:record.eventId,evaluationResultIdentity:record.evaluated!.resultIdentity}));dialog.close();dialog.remove();if(schemaResult)schemaResult.textContent=`Saved evaluated capture evidence in ${entity.name}; opening it in Specification Studio.`;globalThis.open(`specification-builder.html?project=${encodeURIComponent(project.project.id)}&kind=${kind}&entity=${encodeURIComponent(entity.id)}&source=side-panel`,"_blank");}catch(error){summary.textContent=error instanceof Error?error.message:String(error);}});
