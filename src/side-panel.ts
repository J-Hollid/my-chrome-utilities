...
interface SchemaValidationRecord { eventId: string; eventName: string; state: string; checkedAt: string; schemaId?:string; schemaName?: string; schemaVersion?: number; target?: string; issueCodes?:string[]; assignmentId?: string; assignmentName?: string; assignmentEvidence?: string; }
...
function reviewCapturedValidationContinuation(record:SchemaValidationRecord,trigger:HTMLButtonElement):void{
  ...
  if(!captured||!record.schemaId){if(schemaResult)schemaResult.textContent="The captured event or validated schema is no longer available.";return;}
  ...
  const dialog=document.createElement("dialog"),heading=document.createElement("h4"),summary=document.createElement("p"),name=document.createElement("input"),select=(labelText:string,values:readonly {id:string;name:string}[],optional=false)=>{const label=document.createElement("label"),control=document.createElement("select");label.textContent=labelText;if(optional)control.append(new Option(`No ${labelText.toLowerCase()}`,""));control.append(...values.map(({id,name:optionName})=>new Option(optionName,id)));label.append(control);dialog.append(label);return control;},confirm=document.createElement("button"),cancel=document.createElement("button");
  heading.textContent="Continue captured validation in project";summary.textContent=`${record.eventName} · ${record.state} · ${record.schemaName} revision ${record.schemaVersion} → ${project.project.name}. Review one captured observation and proposed status/issue assertions before Save.`;name.value=choices.suggestedFixtureName;name.setAttribute("aria-label","Fixture name");dialog.append(heading,summary,name);const event=select("Event",choices.events),page=select("Page",choices.pages,true),step=select("Flow step",choices.flowSteps,true),profile=select("Profile",choices.profiles,true);confirm.type=cancel.type="button";confirm.textContent="Save Fixture and open in Builder";cancel.textContent="Cancel";
  confirm.addEventListener("click",()=>{try{const next=createFixtureFromCapturedValidation(project,{name:name.value.trim(),captureId:record.eventId,sourceId:captured.sourceId,eventName:record.eventName,payload:captured.payload,schemaId:record.schemaId!,eventId:event.value,...(page.value?{pageId:page.value}:{}),...(step.value?{flowStepId:step.value}:{}),...(profile.value?{profileId:profile.value}:{}),expected:{status:/pass|valid/i.test(record.state)?"pass":"fail",issueCodes:record.issueCodes??[]}},(kind)=>`${kind}:${crypto.randomUUID()}`),fixture=next.project.collections.fixtures.at(-1)!,result=commitCanonicalProjectState(dataLayerStorage,next,{expectedRevision:envelope.revision,pendingLabel:`Continue capture ${record.eventId} as Fixture`,base:project});if(result.status==="conflict")throw new Error(`Project changed at revision ${result.revision}; review the Fixture again.`);globalThis.localStorage.setItem("my-chrome-utilities.specification-project-navigation.v1",JSON.stringify({kind:"fixtures",id:fixture.id,source:"side-panel",revision:result.revision,returnEventId:record.eventId}));dialog.close();dialog.remove();if(schemaResult)schemaResult.textContent=`Saved ${fixture.name} in ${project.project.name}; opening the canonical Fixture in Builder.`;globalThis.open(`specification-builder.html?kind=fixtures&entity=${encodeURIComponent(fixture.id)}&source=side-panel&revision=${result.revision}`,"_blank");}catch(error){summary.textContent=error instanceof Error?error.message:String(error);}});
