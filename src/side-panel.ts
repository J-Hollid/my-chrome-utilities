...
function persistSchemaLibrary(): void {
  ...
  try{if(schemaValue!==previousSchemas)dataLayerStorage.setItem(SCHEMA_LIBRARY_STORAGE_KEY,schemaValue);if(nextProject){const result=commitCanonicalProjectState(globalThis.localStorage,nextProject,{expectedRevision:envelope?.revision??0,pendingLabel:"Side-panel schema edit",...(project?{base:project}:{})});if(result.status==="conflict")throw new Error(`Schema edit conflicts with project revision ${result.revision}.`);projectLibraryUi.captureActiveProject(nextProject,result.revision);}}
...
subscribeCanonicalProjectChanges(globalThis as unknown as Parameters<typeof subscribeCanonicalProjectChanges>[0],({state:next,revision})=>{
