...
import { applySchemaPropertyTypeEdit, inspectSchemaPropertyTypeEdit, schemaPropertyTypeLabel, type EditablePropertyType, type TypeMismatchTreatment } from "./data-layer-schema-property-type-editing.js";
...
if (storedReusableSchemaRules && JSON.stringify(reusableSchemaRules) !== storedReusableSchemaRules) localStorage.setItem(SCHEMA_RULE_STORAGE_KEY, JSON.stringify(reusableSchemaRules));
...
function renderSchemaDraft(): void {
  ...
  const propertyItems = propertyRows.map((propertyRow) => {
    ...
    const typeAction = document.createElement("button"); typeAction.type="button"; typeAction.className="schema-property-edit-type";
    ...
    const typeEditor=document.createElement("fieldset");typeEditor.className="schema-property-type-editor";typeEditor.hidden=true;const typeLegend=document.createElement("legend");typeLegend.textContent=`Type for ${persistedPath}`;
    const valueTypeLabel=document.createElement("label");valueTypeLabel.textContent="Value type ";const valueType=document.createElement("select");valueType.setAttribute("aria-label",`Value type for ${persistedPath}`);(["string","number","boolean","object","array"] as const).forEach((value)=>valueType.append(Object.assign(document.createElement("option"),{value,textContent:value.charAt(0).toUpperCase()+value.slice(1)})));valueType.value=property.type??"string";valueTypeLabel.append(valueType);
    const itemTypeLabel=document.createElement("label");itemTypeLabel.textContent="Item type ";const itemType=document.createElement("select");itemType.setAttribute("aria-label",`Item type for ${persistedPath}`);([["","Any item type"],["string","String"],["number","Number"],["boolean","Boolean"],["object","Object"]] as const).forEach(([value,textContent])=>itemType.append(Object.assign(document.createElement("option"),{value,textContent})));itemType.value=property.items?.type??"";itemTypeLabel.append(itemType);itemTypeLabel.hidden=valueType.value!=="array";
    const treatmentLabel=document.createElement("label");treatmentLabel.textContent="Type mismatch treatment ";const treatment=document.createElement("select");treatment.setAttribute("aria-label",`Type mismatch treatment for ${persistedPath}`);(["error","warning","ignore"] as const).forEach((value)=>treatment.append(Object.assign(document.createElement("option"),{value,textContent:value.charAt(0).toUpperCase()+value.slice(1)})));treatment.value=property.typeMismatchTreatment??"error";treatmentLabel.append(treatment);
    const reviewType=document.createElement("button");reviewType.type="button";reviewType.textContent="Review type change";const typeReview=document.createElement("output");typeReview.className="schema-property-type-review";const impactResolutions=document.createElement("div");impactResolutions.className="schema-property-type-impact-resolutions";impactResolutions.setAttribute("aria-label",`Impact resolutions for ${persistedPath}`);const confirmType=document.createElement("button");confirmType.type="button";confirmType.textContent="Confirm type change";confirmType.disabled=true;const cancelType=document.createElement("button");cancelType.type="button";cancelType.textContent="Cancel";
    let currentImpact:ReturnType<typeof inspectSchemaPropertyTypeEdit>|undefined;
    const selectedResolutions=():Record<string,{action:"remove"|"replace";value?:string}>=>Object.fromEntries(Array.from(impactResolutions.querySelectorAll<HTMLSelectElement>("select[data-impact]"),select=>{const replacement=impactResolutions.querySelector<HTMLInputElement>(`input[data-impact-replacement="${CSS.escape(select.dataset.impact!)}"]`);return[select.dataset.impact!,{action:select.value as "remove"|"replace",...(select.value==="replace"&&replacement?{value:replacement.value}:{})}];}));
    const refreshResolutionState=()=>{if(!currentImpact)return;confirmType.disabled=currentImpact.incompatible.some((impact)=>{const select=impactResolutions.querySelector<HTMLSelectElement>(`select[data-impact="${CSS.escape(impact)}"]`),replacement=impactResolutions.querySelector<HTMLInputElement>(`input[data-impact-replacement="${CSS.escape(impact)}"]`);return !select?.value||(select.value==="replace"&&!replacement?.value.trim());});};
    const refreshTypeReview=()=>{if(!schemaDraft)return;const chosen=valueType.value as EditablePropertyType;currentImpact=inspectSchemaPropertyTypeEdit(schemaDraft,persistedPath,chosen,itemType.value?itemType.value as Exclude<EditablePropertyType,"array">:undefined);typeReview.textContent=`${currentImpact.from} changing to ${currentImpact.to}. Retained: ${currentImpact.compatible.join(", ")||"none"}. Incompatible: ${currentImpact.incompatible.join(", ")||"none"}.`;impactResolutions.replaceChildren(...currentImpact.incompatible.map((impact)=>{const row=document.createElement("label");row.textContent=`${impact} `;const select=document.createElement("select");select.dataset.impact=impact;select.setAttribute("aria-label",`Resolution for ${impact} at ${persistedPath}`);select.append(Object.assign(document.createElement("option"),{value:"",textContent:"Choose resolution"}),Object.assign(document.createElement("option"),{value:"remove",textContent:"Remove artifact"}));const replaceable=impact==="example value"||impact.startsWith("conditional dependency ");if(replaceable)select.append(Object.assign(document.createElement("option"),{value:"replace",textContent:"Replace artifact"}));const replacement=document.createElement("input");replacement.dataset.impactReplacement=impact;replacement.setAttribute("aria-label",`Replacement for ${impact} at ${persistedPath}`);replacement.placeholder="Replacement value";replacement.hidden=true;select.addEventListener("change",()=>{replacement.hidden=select.value!=="replace";refreshResolutionState();});replacement.addEventListener("input",refreshResolutionState);row.append(select,replacement);return row;}));refreshResolutionState();};
    valueType.addEventListener("change",()=>{itemTypeLabel.hidden=valueType.value!=="array";confirmType.disabled=true;typeReview.textContent="";impactResolutions.replaceChildren();});itemType.addEventListener("change",()=>{confirmType.disabled=true;typeReview.textContent="";impactResolutions.replaceChildren();});treatment.addEventListener("change",()=>{confirmType.disabled=true;});reviewType.addEventListener("click",refreshTypeReview);
    confirmType.addEventListener("click",()=>{if(!schemaDraft)return;const previousDraft=structuredClone(schemaDraft);try{schemaDraft=applySchemaPropertyTypeEdit(previousDraft,{path:persistedPath,type:valueType.value as EditablePropertyType,...(valueType.value==="array"&&itemType.value?{itemType:itemType.value as Exclude<EditablePropertyType,"array">}:{}),treatment:treatment.value as TypeMismatchTreatment,removeIncompatible:false,resolutions:selectedResolutions()});persistSchemaEditorDraft(`Change ${persistedPath} type from ${schemaPropertyTypeLabel(property)} to ${valueType.value}`);renderSchemaDraft();}catch(error){schemaDraft=previousDraft;typeReview.textContent=error instanceof Error?error.message:"Type change failed";}});
    ...
    typeAction.addEventListener("click",()=>{if(inherited){if(owningSchema){schemaDraft=schemaEditorDraft(owningSchema);renderSchemaDraft();}return;}typeEditor.hidden=false;typeAction.setAttribute("aria-expanded","true");valueType.focus({preventScroll:true});});
