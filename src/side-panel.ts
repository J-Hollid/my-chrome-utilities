...
function openCompactCanonicalPropertyActions(path:string,trigger:HTMLButtonElement):boolean {
  ...
  let ownershipSession:Parameters<typeof activateFocusedOwnershipSection>[0]={inherited:!focusedStructureOwned(original)||["inherited","overridden"].includes(focusedSourceState(original)),local:["local","overridden"].includes(focusedSourceState(original)),structureOwned:focusedStructureOwned(original),invariant:original.enforcement==="invariant",activated:[]};
  ...
  const state=focusedSourceState(original),sectionOwnership=focusedSectionOwnershipActions({inherited:!focusedStructureOwned(original)||state==="inherited"||state==="overridden",local:state==="local"||state==="overridden",structureOwned:focusedStructureOwned(original),overridden:state==="overridden",invariant:original.enforcement==="invariant",conflict:state==="conflict",replaceable:original.enforcement==="overridable"});
