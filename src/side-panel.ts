...
function dispatchCompactCanonicalCommand(command:Parameters<CompactCanonicalEditorAdapter["dispatch"]>[0]):void{
  ...
  compactCanonicalCommandFeedback=canonicalCommandOutcome(command,result,base??result.document);
...
function persistSchemaEditorDraft(change?: string): void {
  ...
  if(compactCanonicalEditor){
    ...
    for(const command of commands){const prior=canonical,result=compactCanonicalEditor.dispatch(command);if(result.status==="conflict"||result.status==="confirmation-required"){compactCanonicalPendingCommand=command;compactCanonicalPendingBase=compactCanonicalRevisionSnapshots.get(command.baseRevision);compactCanonicalReviewVisible=false;compactCanonicalCommandFeedback=result.status==="conflict"?`${result.message} Compare this command with the latest Draft, retry only this command, or reject the local edit.`:`${result.impact} Compare this command with the latest Draft before retrying or reject the local edit.`;renderCompactCanonicalContext();return;}canonical=result.document;compactCanonicalCommandFeedback=canonicalCommandOutcome(command,result,prior);}
    ...
    renderSchemas();renderCompactCanonicalEditor();
    return;
  }
...
function openContributorInUnifiedEditor(key:string):void{
  ...
  const stepHistory=(direction:"Undo"|"Redo")=>{const live=restoreCanonicalProjectState(projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY)),selected=live&&resolveSidePanelSchemaContributor(live,key);if(!live||!selected){compactCanonicalCommandFeedback=`${direction} blocked: this contributor is no longer available.`;renderCompactCanonicalContext();return;}const current=documentFor(live,selected);if(migration){migrationBlocked(current);return;}const step=direction==="Undo"?prepareCompactCanonicalUndo(compactCanonicalHistory,key,current):prepareCompactCanonicalRedo(compactCanonicalHistory,key,current);if(step.status!=="ready"){compactCanonicalCommandFeedback=step.message;renderCompactCanonicalContext();return;}commitUnifiedContributorState(contributorState(live,selected,step.document),`${direction} canonical schema edit in ${selected.entity.name}`);compactCanonicalHistory=step.history;compactCanonicalCommandFeedback=`${direction} restored ${selected.entity.name} in the Saved Draft.`;renderCompactCanonicalEditor();};
