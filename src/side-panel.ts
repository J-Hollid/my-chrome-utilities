...
function renderSchemaDraft(): void {
  ...
  const propertyItems = propertyRows.map((propertyRow) => {
    ...
    const reviewType=document.createElement("button");reviewType.type="button";reviewType.textContent="Review type change";const typeReview=document.createElement("output");typeReview.className="schema-property-type-review";const destructiveLabel=document.createElement("label");const destructive=document.createElement("input");destructive.type="checkbox";destructiveLabel.append(destructive," Remove incompatible schema data");destructiveLabel.hidden=true;const confirmType=document.createElement("button");confirmType.type="button";confirmType.textContent="Confirm type change";confirmType.disabled=true;const cancelType=document.createElement("button");cancelType.type="button";cancelType.textContent="Cancel";
    const refreshTypeReview=()=>{if(!schemaDraft)return;const chosen=valueType.value as EditablePropertyType;const impact=inspectSchemaPropertyTypeEdit(schemaDraft,persistedPath,chosen,itemType.value?itemType.value as Exclude<EditablePropertyType,"array">:undefined);typeReview.textContent=`${impact.from} changing to ${impact.to}. Retained: ${impact.compatible.join(", ")||"none"}. Incompatible: ${impact.incompatible.join(", ")||"none"}.`;destructiveLabel.hidden=!impact.incompatible.length;destructive.checked=false;confirmType.disabled=Boolean(impact.incompatible.length);};
    valueType.addEventListener("change",()=>{itemTypeLabel.hidden=valueType.value!=="array";confirmType.disabled=true;typeReview.textContent="";});itemType.addEventListener("change",()=>{confirmType.disabled=true;typeReview.textContent="";});treatment.addEventListener("change",()=>{confirmType.disabled=true;});reviewType.addEventListener("click",refreshTypeReview);destructive.addEventListener("change",()=>{confirmType.disabled=!destructive.checked;});
    confirmType.addEventListener("click",()=>{if(!schemaDraft)return;try{schemaDraft=applySchemaPropertyTypeEdit(schemaDraft,{path:persistedPath,type:valueType.value as EditablePropertyType,...(valueType.value==="array"&&itemType.value?{itemType:itemType.value as Exclude<EditablePropertyType,"array">}:{}),treatment:treatment.value as TypeMismatchTreatment,removeIncompatible:destructive.checked});persistSchemaEditorDraft(`Change ${persistedPath} type from ${schemaPropertyTypeLabel(property)} to ${valueType.value}`);renderSchemaDraft();}catch(error){typeReview.textContent=error instanceof Error?error.message:"Type change failed";}});
