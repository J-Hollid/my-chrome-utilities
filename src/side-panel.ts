...
async function settleUnifiedContributorRevision(projectId:string,revision:number):Promise<void>{
  for(let attempt=0;attempt<400;attempt+=1){const failed=durableProjectRuntime.failedSave();if(failed?.projectId===projectId)throw failed.error;const loaded=await durableProjectRuntime.repository.loadProject(projectId);if(loaded.draftSequence>=revision)return;await new Promise<void>((resolve)=>setTimeout(resolve,10));}
...
function commitUnifiedContributorState(next:ProjectState,label:string):Extract<ReturnType<typeof commitCanonicalProjectState>,{status:"committed"}>{
  const serialized=projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY),envelope=restoreCanonicalProjectEnvelope(serialized),base=restoreCanonicalProjectState(serialized);if(!envelope||!base)throw new Error("The Specification Project is unavailable.");const result=commitCanonicalProjectState(projectStorage,next,{expectedRevision:envelope.revision,pendingLabel:label,base});if(result.status==="conflict")throw new Error(`Schema contributor changed in a newer Saved Draft; review the canonical command again.`);projectLibraryUi.captureActiveProject(next,result.revision);return result;
}
