...
const compactCanonicalFocusedHost=document.createElement("section");compactCanonicalFocusedHost.hidden=true;compactCanonicalFocusedHost.dataset.schemaPresentation="compact-panel";compactCanonicalFocusedHost.className="compact-canonical-focused-editor";schemaDetail?.append(compactCanonicalFocusedHost);
...
let compactCanonicalFocusedMount:{key:string;editor:ReturnType<typeof mountCanonicalSchemaEditor>}|undefined;
...
function renderSchemas(): void {
  ...
  if (schemaEmptyState) schemaEmptyState.hidden = count > 0;
  if (schemaCount) schemaCount.textContent = `${count} schemas and contributors`;
  const heading=(name:string)=>{const item=document.createElement("li");item.dataset.schemaGroup=name;item.setAttribute("role","heading");item.textContent=name;return item;};
  if(visible.length){nodes.push(heading("Saved schemas"),...visible.map((schema) => {
    ...
    item.dataset.schemaEntryKey=`saved:${schema.id}`;item.dataset.schemaRole="Saved schema";
...
function dispatchCompactCanonicalFocusedCommand(command:Parameters<CompactCanonicalEditorAdapter["dispatch"]>[0]):ReturnType<CompactCanonicalEditorAdapter["dispatch"]>{
  const adapter=compactCanonicalEditor;if(!adapter)throw new Error("The compact canonical editor is unavailable.");
  if(compactCanonicalSettlementPending||compactCanonicalHistoryState.pending)return{status:"conflict",document:adapter.load(),message:"Resolve the current durable schema save before making another semantic change."};
  const base=compactCanonicalRevisionSnapshots.get(command.baseRevision),result=adapter.dispatch(command);
  if(result.status!=="applied"&&result.status!=="rebased")return result;
  ...
  compactCanonicalCommandFeedback=settlement?`Saving ${outcome.replace(/^(Saved|Rebased) /,"").replace(/\.$/,"")} to the ${settlementTarget}…`:outcome;compactCanonicalSettlementPending=Boolean(settlement);
  queueMicrotask(()=>{if(compactCanonicalEditor===adapter)renderCompactCanonicalEditor();});
  if(settlement&&adapter.settle)void adapter.settle().then(()=>{adapter.onSettlementCommitted?.();if(compactCanonicalEditor!==adapter||compactCanonicalSettlementSequence!==settlement)return;compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=`${outcome} ${settlementTarget.charAt(0).toUpperCase()}${settlementTarget.slice(1)} committed.`;renderCompactCanonicalEditor();},(error)=>{if(compactCanonicalEditor!==adapter||compactCanonicalSettlementSequence!==settlement)return;compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=`Not saved to the ${settlementTarget}; the exact change remains available for Retry, Reject, or export. ${error instanceof Error?error.message:String(error)}`;renderCompactCanonicalEditor();});
  return result;
}
...
function renderCompactCanonicalEditor():void{
  ...
  schemaDraft=undefined;if(schemaEditor){schemaEditor.hidden=true;delete schemaEditor.dataset.schemaPresentation;delete schemaEditor.dataset.canonicalRevision;delete schemaEditor.dataset.canonicalSchemaId;schemaEditor.removeAttribute("aria-label");}
  if(schemaDetail){schemaDetail.hidden=false;schemaDetail.setAttribute("aria-label","Side panel schema editor region");}
  compactCanonicalFocusedHost.hidden=false;compactCanonicalFocusedHost.dataset.schemaPresentation="compact-panel";compactCanonicalFocusedHost.dataset.canonicalRevision=String(canonical.revision);compactCanonicalFocusedHost.dataset.canonicalSchemaId=canonical.id;compactCanonicalFocusedHost.setAttribute("aria-busy",String(compactCanonicalSettlementPending||Boolean(compactCanonicalHistoryState.pending)));
  if(!compactCanonicalFocusedMount||compactCanonicalFocusedMount.key!==adapter.key){compactCanonicalFocusedHost.replaceChildren();compactCanonicalFocusedMount={key:adapter.key,editor:mountCanonicalSchemaEditor({host:compactCanonicalFocusedHost,surface:"Side panel",load:adapter.load,id:(kind)=>`${kind}:${crypto.randomUUID()}`,dispatch:dispatchCompactCanonicalFocusedCommand,renderAfterDispatch:false,...(adapter.onUndo?{onUndo:adapter.onUndo}:{}),...(adapter.onRedo?{onRedo:adapter.onRedo}:{}),...(compactCanonicalCommandFeedback?{initialFeedback:compactCanonicalCommandFeedback}:{})})};}else compactCanonicalFocusedMount.editor.render();
  const unavailable=compactCanonicalSettlementPending||Boolean(compactCanonicalHistoryState.pending);for(const control of Array.from(compactCanonicalFocusedHost.querySelectorAll<HTMLButtonElement|HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement>("button,input,select,textarea")))control.disabled=unavailable;
}

function openCompactCanonicalEditor(adapter:CompactCanonicalEditorAdapter):void{
  sidePanelLayeredProfileEditor?.close();
  compactCanonicalSettlementSequence+=1;compactCanonicalSettlementPending=false;compactCanonicalEditor=adapter;compactCanonicalFocusedMount=undefined;compactCanonicalPendingCommand=undefined;compactCanonicalPendingBase=undefined;compactCanonicalReviewVisible=false;compactCanonicalPresenceDraft=undefined;compactCanonicalRevisionSnapshots.clear();compactCanonicalCommandFeedback="";renderCompactCanonicalEditor();
}
...
function closeCompactCanonicalEditor():void{
  ...
  compactCanonicalFocusedMount=undefined;compactCanonicalFocusedHost.hidden=true;compactCanonicalFocusedHost.replaceChildren();compactCanonicalPendingCommand=undefined;compactCanonicalPendingBase=undefined;compactCanonicalReviewVisible=false;compactCanonicalPresenceDraft=undefined;compactCanonicalRevisionSnapshots.clear();compactCanonicalCommandFeedback="";
...
function openContributorInUnifiedEditor(key:string):void{
  ...
  let contributorUi:{selectedPropertyId?:string;view?:"tree"|"table"}={},compatibilityCanonical:CanonicalSchemaDocument|undefined,acknowledgedMigration:CanonicalSchemaDocument|undefined,migration:ReturnType<typeof migrateLegacyProfile>|undefined,migrationSavePending=false,migrationStatus="",pendingHistoryIdentity:CompactCanonicalHistoryTransitionIdentity|undefined;const isComposed=(selected:SidePanelContributorSelection):selected is SidePanelContributorSelection&{collectionKind:"pages"|"pageGroups"}=>selected.collectionKind==="pages"||selected.collectionKind==="pageGroups";const withContributorUi=(document:CanonicalSchemaDocument)=>{const selectedPropertyId=contributorUi.selectedPropertyId&&document.nodes[contributorUi.selectedPropertyId]?contributorUi.selectedPropertyId:document.selectedPropertyId;return{...document,...(selectedPropertyId?{selectedPropertyId}:{}),...(contributorUi.view?{view:contributorUi.view}:{})};};const documentFor=(live:ProjectState,selected:SidePanelContributorSelection)=>{if(!isComposed(selected)){if(migration)return withContributorUi(compatibilityCanonical??migration.document);const stored=selected.entity.canonicalSchema as CanonicalSchemaDocument|undefined;if(stored)return withContributorUi(stored);if(acknowledgedMigration)return withContributorUi(acknowledgedMigration);if(!compatibilityCanonical){if(hasLegacySchemaRepresentation(selected.entity)){migration=migrateLegacyProfile(selected.entity,{id:(kind)=>`${kind}:${crypto.randomUUID()}`});compatibilityCanonical=migration.document;}else compatibilityCanonical=createCanonicalSchema({id:`canonical:${selected.entity.id}`,contributorId:selected.entity.id,contributorName:selected.entity.name});}return withContributorUi(compatibilityCanonical);}return withContributorUi(composedCanonicalSchema(live,selected.entity,selected.collectionKind==="pages"?"Page":"Page Group"));};
