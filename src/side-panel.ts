...
async function dispatchCompactCanonicalCommand(command:Parameters<CompactCanonicalEditorAdapter["dispatch"]>[0]):Promise<boolean>{
  ...
  if(settlement&&adapter.settle){try{await adapter.settle();adapter.onSettlementCommitted?.();if(compactCanonicalEditor!==adapter||compactCanonicalSettlementSequence!==settlement)return true;compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=`${outcome} ${settlementTarget.charAt(0).toUpperCase()}${settlementTarget.slice(1)} committed.`;renderCompactCanonicalEditor();}catch(error){if(compactCanonicalEditor!==adapter||compactCanonicalSettlementSequence!==settlement)return false;compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=`Not saved to the ${settlementTarget}; the exact change remains available for Retry, Reject, or export. ${error instanceof Error?error.message:String(error)}`;renderCompactCanonicalEditor();return false;}}
  return true;
}
...
function openContributorInUnifiedEditor(key:string):void{
  ...
  const stepHistory=async(direction:"Undo"|"Redo"):Promise<void>=>{if(compactCanonicalSettlementPending||compactCanonicalHistoryState.pending){compactCanonicalCommandFeedback="Resolve the current durable schema save through Retry or Reject before changing history. Export may preserve a recovery copy first.";renderCompactCanonicalContext();return;}const adapter=compactCanonicalEditor,live=restoreCanonicalProjectState(projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY)),selected=live&&resolveSidePanelSchemaContributor(live,key);if(!adapter||!live||!selected){compactCanonicalCommandFeedback=`${direction} blocked: this contributor is no longer available.`;renderCompactCanonicalContext();return;}const current=documentFor(live,selected),scopedHistoryKey=historyKey(live.project.id);if(migration){migrationBlocked(current);return;}const step=direction==="Undo"?prepareCompactCanonicalUndo(compactCanonicalHistoryState.history,scopedHistoryKey,current):prepareCompactCanonicalRedo(compactCanonicalHistoryState.history,scopedHistoryKey,current);if(step.status!=="ready"){compactCanonicalCommandFeedback=step.message;renderCompactCanonicalContext();return;}const settlement=++compactCanonicalSettlementSequence,label=`${direction} canonical schema edit in ${selected.entity.name}`;pendingHistoryIdentity=beginCompactCanonicalPendingHistory(live.project.id,key,label,step.history);compactCanonicalSettlementPending=true;compactCanonicalCommandFeedback=`${direction} is restoring ${selected.entity.name} in the durable Saved Draft…`;renderCompactCanonicalEditor();try{commitUnifiedContributorState(contributorState(live,selected,step.document),label);await durableProjectRuntime.settled("project");completeCompactCanonicalPendingHistory(pendingHistoryIdentity);pendingHistoryIdentity=undefined;if(compactCanonicalEditor!==adapter||compactCanonicalSettlementSequence!==settlement)return;compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=`${direction} restored ${selected.entity.name} in the Saved Draft.`;renderCompactCanonicalEditor();}catch(error){if(pendingHistoryIdentity&&durableProjectRuntime.failedSave()?.command.label!==label){rejectCompactCanonicalPendingHistory(pendingHistoryIdentity);pendingHistoryIdentity=undefined;}if(compactCanonicalEditor!==adapter||compactCanonicalSettlementSequence!==settlement)return;compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=`${direction} was not saved to the durable Saved Draft; the history action remains available for Retry, Reject, or export. ${error instanceof Error?error.message:String(error)}`;renderCompactCanonicalEditor();}};
  openCompactCanonicalEditor({
    ...
    onSettlementCommitted:()=>{if(pendingHistoryIdentity){completeCompactCanonicalPendingHistory(pendingHistoryIdentity);pendingHistoryIdentity=undefined;}},
...
function openCompactCanonicalPropertyActions(path:string,trigger:HTMLButtonElement):boolean {
  ...
  if(!adapter||!documentModel||!node)return false;
  ...
  const close=()=>{if(schemaPropertyRulePicker.open)schemaPropertyRulePicker.close();schemaPropertyRulePicker.replaceChildren();trigger.focus({preventScroll:true});};
  const section=(name:"Definition"|"Structure")=>{
    ...
    host.dataset.focusedPropertyEditor="true";host.dataset.focusedSection=name.toLowerCase();host.setAttribute("aria-label",`${path} focused ${name} section`);heading.textContent=name;identity.textContent=`${path} · stable identity ${node.id} · ${focusedPropertyProvenanceSummary(node.provenance)}`;group.dataset.sectionOwnershipActions="true";group.dataset.ownershipTarget=target.label;status.setAttribute("role","status");
    for(const action of actions){const control=document.createElement("button");control.type="button";control.textContent=action;control.dataset.ownershipAction=action;control.dataset.ownershipTarget=target.label;control.setAttribute("aria-label",`${action} · ${target.label}`);control.addEventListener("click",()=>{status.textContent=`${action} targets ${target.label}.`;if(!lifecycle.has(action))return;const review=document.createElement("section"),summary=document.createElement("p"),reviewActions=document.createElement("div"),cancel=document.createElement("button"),confirm=document.createElement("button");review.setAttribute("aria-label","Review changes");summary.textContent=`Review changes · ${action} · ${target.label} · only this sparse local property will be removed; no durable write occurs before confirmation.`;cancel.type="button";cancel.textContent="Cancel review";cancel.addEventListener("click",()=>section(name));confirm.type="button";confirm.textContent="Confirm changes";confirm.addEventListener("click",()=>{void dispatchCompactCanonicalCommand({kind:"delete",baseRevision:adapter.load().revision,propertyId:node.id}).then((result)=>{if(result)close();});});reviewActions.append(cancel,confirm);review.append(summary,reviewActions);schemaPropertyRulePicker.replaceChildren(review);});group.append(control);}
    const cancel=document.createElement("button");cancel.type="button";cancel.textContent="Cancel";cancel.addEventListener("click",showMenu);group.append(status);host.append(heading,identity,group,cancel);schemaPropertyRulePicker.replaceChildren(host);
  };
  const showMenu=()=>{
    const menu=renderFocusedPropertyMenu({dom:document,path,provenance:focusedPropertyProvenanceSummary(node.provenance),close,sectionSummary:(name)=>name==="rules"?`${node.rules.length} rules`:name==="structure"?"Stable property identity":"Effective definition facets",selectSection:(name)=>{if(name==="rules"){schemaPropertyRulePicker.close();openCompactCanonicalRuleEditor(path,trigger);return;}section(name==="structure"?"Structure":"Definition");}});
    schemaPropertyRulePicker.replaceChildren(menu);
  };
  showMenu();schemaPropertyRulePicker.showModal();schemaPropertyRulePicker.querySelector<HTMLButtonElement>('[role="menuitem"]')?.focus({preventScroll:true});return true;
}
