...
function writeUnifiedContributorCanonical(state:ProjectState,selection:SidePanelContributorSelection,canonical:CanonicalSchemaDocument):ProjectState{
  const replace=(candidate:ProjectEntity):ProjectEntity=>{if(candidate.id!==selection.entity.id)return candidate;const next:ProjectEntity={...candidate,canonicalSchema:canonical,compiledTargetsStale:true,...(selection.collectionKind==="profiles"?{requirements:[]}:{})};delete next.structuredSchema;delete next.structuredDraft;delete next.schemaConstraints;return next;};
