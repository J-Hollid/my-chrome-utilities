...
async function dispatchCompactCanonicalCommand(command:Parameters<CompactCanonicalEditorAdapter["dispatch"]>[0]):Promise<boolean>{
  ...
  if(compactCanonicalSettlementPending){compactCanonicalCommandFeedback="Wait for the current durable schema save to finish before making another semantic change.";renderCompactCanonicalContext();return false;}
  ...
  if(settlement&&adapter.settle){try{await adapter.settle();if(compactCanonicalEditor!==adapter||compactCanonicalSettlementSequence!==settlement)return false;compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=`${outcome} ${settlementTarget.charAt(0).toUpperCase()}${settlementTarget.slice(1)} committed.`;renderCompactCanonicalEditor();}catch(error){if(compactCanonicalEditor!==adapter||compactCanonicalSettlementSequence!==settlement)return false;compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=`Not saved to the ${settlementTarget}; the exact change remains available for Retry or export. ${error instanceof Error?error.message:String(error)}`;renderCompactCanonicalEditor();return false;}}
  return true;
}
...
function renderCompactCanonicalEditor():void{
  ...
  renderSchemaDraft();
  if(schemaEditor){schemaEditor.setAttribute("aria-busy",String(compactCanonicalSettlementPending));for(const control of Array.from(schemaEditor.querySelectorAll<HTMLButtonElement|HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement>("button,input,select,textarea")))control.disabled=compactCanonicalSettlementPending;}
}
...
function openContributorInUnifiedEditor(key:string):void{
  ...
  const stepHistory=async(direction:"Undo"|"Redo"):Promise<void>=>{if(compactCanonicalSettlementPending){compactCanonicalCommandFeedback="Wait for the current durable schema save to finish before changing history.";renderCompactCanonicalContext();return;}const adapter=compactCanonicalEditor,live=restoreCanonicalProjectState(projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY)),selected=live&&resolveSidePanelSchemaContributor(live,key);if(!adapter||!live||!selected){compactCanonicalCommandFeedback=`${direction} blocked: this contributor is no longer available.`;renderCompactCanonicalContext();return;}const current=documentFor(live,selected);if(migration){migrationBlocked(current);return;}const step=direction==="Undo"?prepareCompactCanonicalUndo(compactCanonicalHistory,key,current):prepareCompactCanonicalRedo(compactCanonicalHistory,key,current);if(step.status!=="ready"){compactCanonicalCommandFeedback=step.message;renderCompactCanonicalContext();return;}const settlement=++compactCanonicalSettlementSequence;compactCanonicalSettlementPending=true;compactCanonicalCommandFeedback=`${direction} is restoring ${selected.entity.name} in the durable Saved Draft…`;renderCompactCanonicalEditor();try{commitUnifiedContributorState(contributorState(live,selected,step.document),`${direction} canonical schema edit in ${selected.entity.name}`);await durableProjectRuntime.settled("project");if(compactCanonicalEditor!==adapter||compactCanonicalSettlementSequence!==settlement)return;compactCanonicalHistory=step.history;compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=`${direction} restored ${selected.entity.name} in the Saved Draft.`;renderCompactCanonicalEditor();}catch(error){if(compactCanonicalEditor!==adapter||compactCanonicalSettlementSequence!==settlement)return;compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=`${direction} was not saved to the durable Saved Draft; the history action remains available after recovery. ${error instanceof Error?error.message:String(error)}`;renderCompactCanonicalEditor();}};
  openCompactCanonicalEditor({
    ...
    dispatch:(command)=>{const live=restoreCanonicalProjectState(projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY))!,selected=resolveSidePanelSchemaContributor(live,key)!,document=documentFor(live,selected);if(migration)return migrationBlocked(document);const result=applyCanonicalCommand(document,command),mutation=command.kind!=="select"&&command.kind!=="view";if(result.status==="applied"||result.status==="rebased"){if(isComposed(selected))composedUi={...(result.document.selectedPropertyId?{selectedPropertyId:result.document.selectedPropertyId}:{}),view:result.document.view};if(mutation){commitUnifiedContributorState(contributorState(live,selected,result.document),`${command.kind} ${isComposed(selected)?"sparse canonical facet":"canonical property"} in ${selected.entity.name}`);compactCanonicalHistory=recordCompactCanonicalMutation(compactCanonicalHistory,key,document,result.document);}else if(!isComposed(selected))commitUnifiedContributorState(writeUnifiedContributorCanonical(live,selected,result.document),`${command.kind} canonical property in ${selected.entity.name}`);}return result;},
