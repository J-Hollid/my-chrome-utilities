...
function openCompactCanonicalPropertyActions(path:string,trigger:HTMLButtonElement):boolean {
  ...
  let ownershipSession:Parameters<typeof activateFocusedOwnershipSection>[0]={inherited:focusedSourceState(original)==="inherited",local:["local","overridden"].includes(focusedSourceState(original)),invariant:original.enforcement==="invariant",activated:[]};
