...
function openCompactCanonicalPropertyActions(path:string,trigger:HTMLButtonElement):boolean {
  ...
  let working=structuredClone(original),activeSection:"definition"|"rules"|"structure"|undefined,feedbackText="",stagedOwnershipAction="",ownershipSession={inherited:focusedSourceState(original)==="inherited",local:["local","overridden"].includes(focusedSourceState(original)),activated:[] as readonly ("definition"|"rules"|"structure")[]};
