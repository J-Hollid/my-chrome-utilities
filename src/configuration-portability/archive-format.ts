...
interface SectionManifest {entry:string;count:number;digest:string;}
interface BodyManifest {entry:string;digest:string;contentDigest:string;mediaType:string;byteLength:number;}
export interface CompleteConfigurationManifest {
  format:typeof FORMAT;
  version:1|typeof VERSION;
  producer:{buildIdentity:string;createdAt:string};
  requiredFeatures:string[];
  activeProjectId:string|null;
  counts:Record<ConfigurationDomain,number>;
  domainVersions:Record<ConfigurationDomain,number>;
  sections:Record<ConfigurationDomain,SectionManifest>;
  bodies:BodyManifest[];
  exclusions:{id:string;reason:string}[];
}
export interface InspectedCompleteConfiguration {
  manifest:CompleteConfigurationManifest;
  snapshot:CompleteConfigurationSnapshot;
}
...
async function blobBytes(blob:Blob):Promise<Uint8Array>{return new Uint8Array(await blob.arrayBuffer());}
function bodyEntry(index:number):string{return `bodies/${String(index).padStart(6,"0")}.bin`;}

export async function createCompleteConfigurationArchive(snapshot:CompleteConfigurationSnapshot,input:{
  buildIdentity:string;createdAt?:string;requiredFeatures?:string[];signal?:AbortSignal;
  maximumBytes?:number;onProgress?:(writtenBytes:number)=>void;
}):Promise<Blob>{
  assertCompleteDomainInventory(snapshot.sections);
  if(!input.buildIdentity)throw new DOMException("The producer build identity is required.","DataError");
  const sectionBodies=new Map<ConfigurationDomain,Uint8Array>();
  const sections={} as Record<ConfigurationDomain,SectionManifest>;
  const counts={} as Record<ConfigurationDomain,number>;
  for(const domain of COMPLETE_CONFIGURATION_DOMAINS){
    const bytes=jsonBytes(snapshot.sections[domain]);
    sectionBodies.set(domain,bytes);counts[domain]=snapshot.sections[domain].length;
    sections[domain]={entry:`sections/${domain}.json`,count:counts[domain],digest:await sha256(bytes)};
  }
  const uniqueBodies:ConfigurationBody[]=[];
  const identities=new Map<string,{contentDigest:string;body:ConfigurationBody}>();
  for(const body of snapshot.bodies){
    const contentDigest=await sha256(body.bytes),prior=identities.get(body.digest);
    if(prior){if(prior.contentDigest!==contentDigest||prior.body.mediaType!==body.mediaType){
      throw new DOMException(`Configuration body ${body.digest} has conflicting content.`,"DataError");
    }continue;}
    identities.set(body.digest,{contentDigest,body});uniqueBodies.push(body);
  }
  const bodies:BodyManifest[]=[],physicalBodies=new Map<string,{entry:string;body:ConfigurationBody}>();
  for(const body of uniqueBodies){
    const contentDigest=await sha256(body.bytes);
    let physical=physicalBodies.get(contentDigest);
    if(!physical){physical={entry:bodyEntry(physicalBodies.size),body};
  ...
  const manifest:CompleteConfigurationManifest={format:FORMAT,version:VERSION,
    ...
    counts,domainVersions:Object.fromEntries(COMPLETE_CONFIGURATION_DOMAINS.map((domain)=>[domain,1])) as
      Record<ConfigurationDomain,number>,sections,bodies,
  ...
  const maximumBytes=input.maximumBytes??DEFAULT_MAXIMUM_BYTES;let written=0;
  await writeStoredZip(entries,{write:async(chunk)=>{written+=chunk.byteLength;if(written>maximumBytes){
    throw new DOMException(`The configuration archive exceeds the ${maximumBytes} byte size limit.`,"QuotaExceededError");
  }chunks.push(chunk);input.onProgress?.(written);}},
    {...(input.signal?{signal:input.signal}:{})});
  return new Blob(chunks as BlobPart[],{type:"application/zip"});
}

function parseManifest(value:unknown):CompleteConfigurationManifest{
  const manifest=value as CompleteConfigurationManifest;
  ...
  if(unknown.length)throw new DOMException(`The configuration manifest has unknown domains: ${unknown.join(", ")}.`,"DataError");
  if(!Array.isArray(manifest.requiredFeatures)||manifest.requiredFeatures.some((feature)=>
    typeof feature!=="string")||!Array.isArray(manifest.exclusions)||
      (manifest.activeProjectId!==null&&typeof manifest.activeProjectId!=="string"))throw new DOMException(
    "The configuration manifest has invalid feature, exclusion, or active-project data.","DataError");
  for(const [label,value] of [["sections",manifest.sections],["counts",manifest.counts],
    ["domainVersions",manifest.domainVersions]] as const){
    const domainKeys=Object.keys(value??{}),unknownDomains=domainKeys.filter((key)=>
      !COMPLETE_CONFIGURATION_DOMAINS.includes(key as ConfigurationDomain));
    if(unknownDomains.length)throw new DOMException(
      `The configuration manifest ${label} has unknown domains: ${unknownDomains.join(", ")}.`,"DataError");
  }
  for(const domain of COMPLETE_CONFIGURATION_DOMAINS){
    if(manifest.domainVersions[domain]!==1)throw new DOMException(
      `The ${domain} section version is unsupported or missing.`,"NotSupportedError");
  ...
  for(const [index,body] of manifest.bodies.entries())if(!/^bodies\/\d{6}\.bin$/u.test(body?.entry??"")||
...
function validateReferences(sections:CompleteConfigurationSnapshot["sections"],bodies:ConfigurationBody[]):void{
  ...
  const scan=(value:unknown,key=""):void=>{if(Array.isArray(value)){for(const item of value)scan(item,key);return;}
    if(!value||typeof value!=="object")return;for(const [name,item] of Object.entries(value)){
  ...
  for(const [domain,items] of Object.entries(sections) as [ConfigurationDomain,CompleteConfigurationSnapshot["sections"][ConfigurationDomain]][]){
    for(const item of items){for(const dependency of item.dependencies??[])if(!records.has(`${dependency.domain}/${dependency.id}`)){
      throw new DOMException(`The ${domain} record ${item.id} references missing ${dependency.domain}/${dependency.id}.`,"DataError");
    }scan(item.value);}
  }
}

export async function inspectCompleteConfigurationArchive(source:Blob,input:{signal?:AbortSignal;supportedFeatures?:readonly string[]}={}):Promise<InspectedCompleteConfiguration>{
  const entries=await readStoredZip(source,undefined,{...(input.signal?{signal:input.signal}:{})});
  const manifestEntry=entries.get("manifest.json");
  if(!manifestEntry)throw new DOMException("The archive has no configuration manifest.","DataError");
  const manifest=parseManifest(JSON.parse(decoder.decode(await blobBytes(manifestEntry))));
  const expectedEntries=new Set(["manifest.json",...COMPLETE_CONFIGURATION_DOMAINS.map((domain)=>
    manifest.sections[domain].entry),...manifest.bodies.map(({entry})=>entry)]);
  const unlisted=[...entries.keys()].filter((entry)=>!expectedEntries.has(entry));
  if(unlisted.length||expectedEntries.size!==entries.size)throw new DOMException(
    `The configuration archive has unlisted or missing entries: ${unlisted.join(", ")||"check the manifest"}.`,"DataError");
  ...
  if(unsupported.length)throw new DOMException(`The archive requires unsupported features: ${unsupported.join(", ")}.`,"NotSupportedError");
  const sections={} as Record<ConfigurationDomain,CompleteConfigurationSnapshot["sections"][ConfigurationDomain]>;
  for(const domain of COMPLETE_CONFIGURATION_DOMAINS){
    const descriptor=manifest.sections[domain],entry=descriptor&&entries.get(descriptor.entry);
    if(!entry)throw new DOMException(`The archive is missing the ${domain} section.`,"DataError");
    const bytes=await blobBytes(entry);
    if(await sha256(bytes)!==descriptor.digest)throw new DOMException(`The ${domain} section digest is invalid.`,"DataError");
  ...
  const bodies:ConfigurationBody[]=[],bodyCache=new Map<string,Uint8Array>();
  for(const descriptor of manifest.bodies){
    const entry=entries.get(descriptor.entry);
    if(!entry)throw new DOMException(`The archive is missing body ${descriptor.digest}.`,"DataError");
    let bytes=bodyCache.get(descriptor.entry);
    if(!bytes){bytes=await blobBytes(entry);bodyCache.set(descriptor.entry,bytes);}
  ...
  if(new Set(bodies.map(({digest})=>digest)).size!==bodies.length)throw new DOMException("The archive has duplicate body identities.","DataError");
  ...
  for(const record of sections.projects){
    const value=record.value as {archiveDigest?:unknown;archiveContentDigest?:unknown};
    if(typeof value.archiveDigest!=="string")continue;
