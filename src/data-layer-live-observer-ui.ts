...
function renderPropertyNode(
  ...
  if (declarationState?.alreadyDeclared) {
    const declared = document.createElement("button"); declared.type = "button"; declared.className = "live-property-declared"; declared.textContent = `Already declared in ${declarationState.destination ?? "schema"}`; declared.dataset.action = "add-property-to-schema"; declared.setAttribute("aria-disabled", "true"); declared.setAttribute("aria-label", `Add ${node.technicalPath ?? node.path} to ${declarationState.destination}`); row.append(declared);
  } else if (addToSchema) {
    const destination = declarationState?.destination ?? "schema";
    const add = document.createElement("button"); add.type = "button"; add.textContent = "Add to schema"; add.className = "live-property-add-to-schema"; add.dataset.action = "add-property-to-schema"; add.setAttribute("aria-label", `Add ${node.technicalPath ?? node.path} to ${destination}`); add.addEventListener("click", () => addToSchema(node.technicalPath ?? node.path, add)); row.append(add);
  }
