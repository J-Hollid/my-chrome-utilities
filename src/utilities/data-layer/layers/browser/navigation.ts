...
export function mountDataLayerNavigation(root:UtilityMountHost):()=>void {
  const tabs=Array.from(root.querySelectorAll<HTMLButtonElement>('[role="tab"][aria-controls^="data-layer-panel-"]'));
  ...
  const onClick=(event:Event):void=>{const tab=(event.target as Element).closest<HTMLButtonElement>('[role="tab"]');if(tab&&tabs.includes(tab))select(tab);};
