import { defineUtility, type UtilityMountHost } from "../../platform/utility-contract.js";
...
function mountHotkeyEditor(root: UtilityMountHost): void {
  const container = root.querySelector<HTMLElement>("#hotkey-editor-commands");
  const filter = root.querySelector<HTMLInputElement>("#hotkey-editor-filter");
  if (!container || !filter) return;

  const commands = listCommands().filter(({ id }) => hotkeyCommandIds.includes(
    id as typeof hotkeyCommandIds[number],
  ));
  ...
  const editor = createHotkeyEditor({
    ...
    setStatus(message) {
      const status = root.querySelector<HTMLElement>("#keymap-status");
      if (status) status.textContent = message;
    },
    setWarning(message) {
      const warning = root.querySelector<HTMLElement>("#keymap-warning");
