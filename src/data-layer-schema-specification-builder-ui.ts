...
export function renderSchemaSpecificationBuilder(
  ...
  const renderPreview = (): void => {
    ...
    headRow.replaceChildren(...columns.map((column, index) => {
      ...
      const earlier = document.createElement("button"); earlier.type = "button"; earlier.textContent = "Move earlier"; earlier.setAttribute("aria-label", `Move ${specificationColumnLabels[column]} earlier`); earlier.disabled = index === 0;
      const later = document.createElement("button"); later.type = "button"; later.textContent = "Move later"; later.setAttribute("aria-label", `Move ${specificationColumnLabels[column]} later`); later.disabled = index === columns.length - 1;
