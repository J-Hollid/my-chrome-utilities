...
export interface FlowPageFrameCardContext {
  ...
  renamePageFrame:(next:ProjectState,flowId:string,frameId:string,name:string)=>ProjectState;
  resetPageFrameName:(next:ProjectState,flowId:string,frameId:string)=>ProjectState;
}
...
export function appendFlowPageFrameCardControls(context:FlowPageFrameCardContext):void {
  ...
  name.value=frame.nameInFlow?.trim()||sourceName;name.setAttribute("aria-label",`Name in this Flow for ${sourceName}`);nameLabel.append("Name in this Flow",name);resetName.disabled=!frame.nameInFlow;
