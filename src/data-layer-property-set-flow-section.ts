...
const legacyFrameWidth=(graph:SectionGraph,frame:SectionPageFrame):number=>Math.max(190,...(graph.occurrences??[]).filter((occurrence)=>occurrence.pageFrameId===frame.id).map((occurrence)=>Number((occurrence.position as {x?:number}|undefined)?.x??24)+190));
const normalizeLegacyFramePlacement=(graph:SectionGraph,frame:SectionPageFrame,sectionId:string|undefined,laneOffset:number,namedWidth:number):SectionPageFrame=>{
  const legacy=frame as SectionPageFrame&{pageGroupId?:string;freePageRegion?:"before-lanes"|"after-lanes"},x=Number(frame.position.x??24),absoluteX=legacy.freePageRegion==="before-lanes"?x:legacy.freePageRegion==="after-lanes"?laneOffset+namedWidth+x:laneOffset+x,next:SectionPageFrame={...legacy,...(sectionId?{sectionId}:{}),position:{x:absoluteX,y:frame.position.y}};delete (next as typeof legacy).pageGroupId;delete (next as typeof legacy).freePageRegion;return next;
};
...
export function upgradePageGroupsToPropertySets(state:ProjectState,id:IdFactory):ProjectState{
  ...
  return transactProject(state,"Upgrade Page Groups to Property Sets and Flow Sections",(project)=>{
    ...
    const documentationFlowGraphs=Object.fromEntries(Object.entries(graphs(project)).map(([flowId,graph])=>{const laneIds=unique([...(graph.pageGroupIds??[]),...(graph.pageFrames??[]).flatMap((frame)=>typeof frame.pageGroupId==="string"?[frame.pageGroupId]:[])]),hasBefore=(graph.pageFrames??[]).some((frame)=>frame.freePageRegion==="before-lanes"),laneOffset=hasBefore?200:0,namedWidth=Math.max(900,...(graph.pageFrames??[]).filter((frame)=>!frame.freePageRegion).map((frame)=>Number(frame.position.x??40)+legacyFrameWidth(graph,frame)+60)),sectionByLane=new Map<string,string>(),sections=laneIds.map((groupId,order)=>{const group=legacySets.find(({id})=>id===groupId),sectionId=id("flow-section");sectionByLane.set(groupId,sectionId);return{id:sectionId,name:group?.name??groupId,order,bounds:{x:laneOffset+20,y:40+order*220,width:1000,height:200}};}),pageFrames=(graph.pageFrames??[]).map((frame)=>{const legacyFrame=frame as SectionPageFrame&{pageGroupId?:string},sectionId=typeof legacyFrame.pageGroupId==="string"?sectionByLane.get(legacyFrame.pageGroupId):undefined;return normalizeLegacyFramePlacement(graph,legacyFrame,sectionId,laneOffset,namedWidth);}),next:SectionGraph={...graph,sections,pageFrames};delete next.pageGroupIds;return[flowId,next];})),assignments=project.collections.assignments.map((assignment)=>assignment.targetKind==="Page Group"?{...assignment,targetKind:"Property Set"}:assignment),nextCollections={...project.collections,propertySets,pages,assignments} as LegacyCollections;delete nextCollections.pageGroups;const next={...project,collections:nextCollections,documentationFlowGraphs,propertySetFlowSectionVersion:1};verifyUpgrade(project,next);return next;
  });
}
