...
export type TemplateChangeReviewKind = "revision" | "push";
export interface TemplateChangeReview {
  kind: TemplateChangeReviewKind;
  proposedLabel: "Revised" | "Pushed";
  operation: string;
  resultingVersion: number;
  rows: readonly (readonly [string, string])[];
  identity: readonly (readonly [string, string, string])[];
  execution: readonly (readonly [string, string, string])[];
  changes: ReturnType<typeof createPushDraftReview>["changes"];
}

export function createTemplateChangeReview(
  ...
  const changed = (label: string, previous: string, proposed: string): readonly [string, string, string][] =>
    previous === proposed ? [] : [[label, previous, proposed]];
  return {
    kind,
    proposedLabel,
