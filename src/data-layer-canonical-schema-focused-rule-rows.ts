...
const fieldLabel=(field:string):string=>({pattern:"Regular expression",literal:"Literal value",minimum:"Minimum",maximum:"Maximum",minItems:"Minimum items",maxItems:"Maximum items",severity:"Severity",message:"Message"}[field]??field);
...
function editRule(row:HTMLElement,rule:CanonicalRule,context:CanonicalFocusedRuleRowsContext,invoker:HTMLButtonElement):void {
  ...
  const name=input(dom,"editRuleName",draft.name??""),kind=dom.createElement("select"),kindLabel=stringRuleKindOptions(context.getWorking()?.type).find(({kind:entry})=>entry===draft.kind)?.label??draft.kind;kind.name="editRuleKind";kind.disabled=true;kind.append(new Option(kindLabel,draft.kind));name.addEventListener("input",()=>{const value=name.value.trim();if(value)draft.name=value;else delete draft.name;validate();});details.append(labeled(dom,"Rule name",name),labeled(dom,"Rule type",kind));
  ...
  for(const field of focusedRuleFields(draft.kind)){
    ...
    if(field==="presence"){const control=dom.createElement("select");control.name="editRulePresence";control.append(new Option("Required","required"),new Option("Optional","optional"),new Option("Forbidden","forbidden"));control.value=draft.presence??"required";control.addEventListener("change",()=>{draft.presence=control.value as NonNullable<CanonicalRule["presence"]>;validate();});then.append(labeled(dom,"Presence",control));continue;}
    if(field==="ordinaryValue"){const control=input(dom,"editRuleOrdinaryValue",schemaTableAllowedValues(draft));control.addEventListener("input",()=>{delete draft.expectedValue;draft.allowedValues=schemaTableStageAllowedValues(draft.allowedValues??[],control.value,context.getWorking()?.type);validate();});then.append(labeled(dom,"Allowed values",control));continue;}
...
export function renderCanonicalRuleRows(host:HTMLElement,context:CanonicalFocusedRuleRowsContext):void {
  ...
  for(const rule of working.rules){const row=dom.createElement("article"),summary=dom.createElement("p"),provenanceState=rule.provenance?.state,inherited=provenanceState==="inherited"||provenanceState==="shadowed",overridden=provenanceState==="overridden",removed=removedRuleIds.has(rule.id),ownershipState=inherited?"inherited":overridden?"overridden":"local",target=focusedOwnershipActionTarget("Rules","rule",rule.id),legal=focusedOwnershipActions({inherited,overridden,local:!inherited&&!overridden,invariant:rule.enforcement==="invariant"||invariant,replaceable:rule.enforcement==="overridable"&&!invariant});row.dataset.ruleId=rule.id;row.dataset.ownership=ownershipState;row.dataset.sectionOwnershipActions="true";row.dataset.ownershipTarget=target.label;row.dataset.legalOwnershipActions=legal.join("|");summary.textContent=`${ruleKindLabel(rule)} · ${schemaTableRuleConditionSummary(rule.condition,properties)} · Then ${schemaTableRuleOutcomeSummary(rule as unknown as Record<string,unknown>)} · ${rule.severity} · ${rule.message??"No issue message"} · source ${rule.provenance?.contributorName??"local"} · ${ownershipState}${removed?" · Removed":""}`;
    const ownershipButton=(action:string,run:()=>void)=>{const control=button(dom,action,run);control.dataset.ownershipAction=action;control.dataset.ownershipTarget=target.label;control.setAttribute("aria-label",`${action} · ${target.label}`);return control;},view=()=>{row.dataset.ruleMode="view";const detail=dom.createElement("p");detail.textContent=`Rule ${rule.id} · definition ${JSON.stringify(rule)} · effective ${rule.enabled===false?"disabled":"enabled"} · source ${rule.provenance?.contributorName??"local"}`;row.append(detail);},openSource=()=>{row.dataset.ruleMode="source";const source=dom.createElement("p");source.textContent=`Source rule ${rule.id} · ${ruleKindLabel(rule)} · inherited definition is read-only.`;row.append(source);};
    row.append(summary);
    if(removed){const impact=dom.createElement("p");impact.textContent=`Impact review: ${ruleKindLabel(rule)} · effective result falls back to parent or unset.`;row.append(impact,button(dom,"Restore",()=>{removedRuleIds.delete(rule.id);context.render();}));}
    else for(const action of legal){
      if(action==="View"||action==="View conflict")row.append(ownershipButton(action,view));
      else if(action==="Replace here"||action==="Override here")row.append(ownershipButton(action,()=>replaceInheritedRule(rule,context)));
      else if(action==="Open source"||action==="Open contributing sources")row.append(ownershipButton(action,openSource));
      else if(action==="Edit"||action==="Edit local resolution"){const edit=ownershipButton(action,()=>{row.dataset.ruleMode="edit";editRule(row,rule,context,edit);});row.append(edit);}
      else if(action==="Remove local"||action==="Reset to parent")row.append(ownershipButton(action,()=>{removedRuleIds.add(rule.id);context.feedback(`Staged removal of ${ruleKindLabel(rule)}.`);context.render();}));
    }
    list.append(row);
  }
  host.append(list);
