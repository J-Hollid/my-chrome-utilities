...
function editRule(row:HTMLElement,rule:CanonicalRule,context:CanonicalFocusedRuleRowsContext):void {
  ...
  for(const field of focusedRuleFields(rule.kind)){if(field==="condition"){const tree=dom.createElement("div"),properties=context.properties?.()??[];renderSharedConditionTree(tree,{dom,...(rule.condition?{condition:rule.condition}:{}),properties:()=>properties,id:context.id,onChange:(condition)=>{const working=context.getWorking();if(!working)return;const index=working.rules.findIndex(({id})=>id===rule.id);if(index<0)return;const next=clone(working.rules[index]!);if(condition)next.condition=condition;else delete next.condition;working.rules[index]=next;context.render();}});editor.append(labeled(dom,"Shared condition tree",tree));continue;}const value=String((rule as any)[field]??""),control=input(dom,`editRule${field[0]!.toUpperCase()+field.slice(1)}`,value,["minimum","maximum","minItems","maxItems"].includes(field)?"number":"text");control.addEventListener("input",()=>{const working=context.getWorking();if(!working)return;const index=working.rules.findIndex(({id})=>id===rule.id);if(index<0)return;const next=clone(working.rules[index]!);(next as any)[field]=control.value===""?undefined:["minimum","maximum","minItems","maxItems"].includes(field)?Number(control.value):control.value;working.rules[index]=next;});editor.append(labeled(dom,field,control));}
  row.append(editor);
}
