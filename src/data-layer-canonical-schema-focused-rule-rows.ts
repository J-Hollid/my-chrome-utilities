...
function editRule(row:HTMLElement,rule:CanonicalRule,context:CanonicalFocusedRuleRowsContext):void {
  ...
  for(const field of focusedRuleFields(rule.kind)){
    if(field==="condition"){const tree=dom.createElement("div"),properties=context.properties?.()??[];renderSharedConditionTree(tree,{dom,...(rule.condition?{condition:rule.condition}:{}),properties:()=>properties,id:context.id,onChange:(condition)=>update((next)=>{if(condition)next.condition=condition;else delete next.condition;})});editor.append(labeled(dom,"When condition",tree));continue;}
