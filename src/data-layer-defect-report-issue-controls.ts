...
export function appendIssueControls(
  ...
  for (const reportIssue of state.report().issues) {
    ...
    const label = document.createElement("label"); label.htmlFor = selected.id; label.textContent = `${reportIssue.severity}: ${reportIssue.pointer} — ${reportIssue.constraint}${reportIssue.violation ? ` (${reportIssue.violation})` : ""}`;
