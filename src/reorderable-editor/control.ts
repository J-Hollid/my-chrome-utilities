...
const reorderGrip=(doc:Document):SVGSVGElement=>{
  const grip=doc.createElementNS("http://www.w3.org/2000/svg","svg");
  grip.dataset.reorderGrip="true";grip.setAttribute("data-reorder-grip","true");
  grip.setAttribute("viewBox","0 0 16 16");grip.setAttribute("aria-hidden","true");
  grip.setAttribute("focusable","false");grip.setAttribute("fill","currentColor");
  styles(grip as unknown as HTMLElement,{display:"block",width:"16px",height:"16px",flex:"0 0 16px"});
  for(const y of[3,8,13])for(const x of[5,11]){
    const dot=doc.createElementNS("http://www.w3.org/2000/svg","circle");
    dot.setAttribute("cx",String(x));dot.setAttribute("cy",String(y));dot.setAttribute("r","1.5");grip.append(dot);
  }
  return grip;
};
...
export interface ReorderableItemRowOptions {
  control?:HTMLElement;
  primaryContent:HTMLElement;
  trailingContent?:HTMLElement;
  secondaryContent?:readonly HTMLElement[];
}
...
export function renderReorderControl<T extends ReorderableItem>(options:ReorderControlOptions<T>):HTMLElement {
  ...
  if(!model.actionable){wrapper.hidden=true;return wrapper;}
  const ownedMenu=!options.existingActionsMenu,
    trigger=options.existingActionsMenu?.trigger??button(doc,""),
    handle=options.existingActionsMenu?doc.createElement("span"):trigger,
    menu=options.existingActionsMenu?.menu??doc.createElement("div"),dialog=doc.createElement("div");
  const menuId=`reorder-menu-${++identity}`,dialogId=`reorder-dialog-${identity}`;
  wrapper.className="reorderable-editor-control";styles(wrapper,{display:"inline-flex",position:"relative",maxWidth:"44px",alignItems:"center",alignSelf:"center",verticalAlign:"middle",flex:"0 0 44px",order:"-1"});
  handle.className="reorderable-editor-trigger reorderable-editor-drag-handle";handle.dataset.reorderTrigger="true";handle.dataset.reorderItemId=options.itemId;
  ...
  handle.setAttribute("data-reorder-trigger","true");handle.setAttribute("data-reorder-item-id",options.itemId);
  handle.append(reorderGrip(doc));
  if(options.existingActionsMenu)trigger.dataset.reorderMenuButton="true";
  if(ownedMenu)trigger.setAttribute("aria-label",model.accessibleName);
  trigger.setAttribute("aria-haspopup","menu");
  trigger.setAttribute("aria-expanded",String(options.existingActionsMenu?.expanded??false));trigger.setAttribute("aria-controls",menu.id||menuId);
  ...
  const closeMenu=()=>{if(!ownedMenu)return;menu.hidden=true;trigger.setAttribute("aria-expanded","false");};
  const openMenu=()=>{if(!ownedMenu)return;menu.hidden=false;trigger.setAttribute("aria-expanded","true");queueMicrotask(()=>menu.querySelector?.<HTMLButtonElement>('button:not([disabled])')?.focus());};
  ...
  for(const item of model.actions){
    const control=button(doc,item.label);control.setAttribute("role","menuitem");control.dataset.reorderMovementAction=item.id;control.disabled=item.disabled;
  ...
  handle.addEventListener("dragstart",event=>{const dragScope=currentDragScope();if(!model.canDrag||dragScope===undefined){event.preventDefault();dragSession=undefined;return;}handle.dataset.reorderDragging="true";styles(handle,{cursor:"grabbing"});dragSession={itemId:options.itemId,itemLabel:options.itemLabel,completeOrder:options.completeOrder,legalDestinationIds:new Set(options.legalDestinationIds??options.completeOrder.map(({id})=>id)),dragScope,onMove:options.onMove,...(options.localDraftUndo?{offerUndo}:{}),trigger};event.dataTransfer?.setData("application/x-reorderable-editor-item",options.itemId);});
  handle.addEventListener("dragend",()=>{delete handle.dataset.reorderDragging;styles(handle,{cursor:handle.draggable?"grab":"default"});dragSession=undefined;if(options.dropTarget)clearDropIndicator(options.dropTarget);});
  ...
  wrapper.append(handle,...(ownedMenu?[menu]:[]),dialog);return wrapper;
}
