...
export class SchemaLibraryEditor {
  ...
  updateName():void { const p=this.#ports, library=p.library; if (!library.draft && !library.activeSchemaId) return; const schema=p.active(), name=p.root.querySelector<HTMLInputElement>("#schema-editor-name")?.value ?? schema.name;
    if (p.canonical.editor) { const projection={ ...p.editorDraft(schema), name }; library.setDraft(projection); this.#refreshSaveState(projection); void p.canonical.beginProjectionPersistence(p.canonical.editor,projection,"schema name"); return; } this.persistDraft(); }
  ...
  publish(closeEditor=false):SchemaDefinition { const p=this.#ports, library=p.library, transient=library.activeIndex()<0, current=p.active(), presented=p.editorDraft(current), publishable=transient ? { ...current,id:createSchema(presented.name.trim(),1,presented.document).id,published:false } : current,
    ...
    if (p.addPublishedRules(published)) p.persistLibraries(); else p.persist(); this.#closeRevisionDialog(); if (closeEditor) { p.closeCanonical(); library.clearSelection(); } p.renderAll(); const revalidated=p.revalidate();
  ...
  closeEditor():void { const p=this.#ports; if (!p.library.draft && !p.library.activeSchemaId) return; p.library.clearSelection(); p.closeCanonical(); p.renderAll(); const result=p.root.querySelector<HTMLElement>("#schema-result"); if (result) result.textContent="Working draft retained without publishing."; }
  discardWorking():void { const p=this.#ports; if (p.library.activeIndex()>=0) { p.replaceActive(discardSchemaWorkingDraft(p.active())); p.persist(); } p.library.clearSelection(); this.#closeDialog("#close-schema-editor-review"); p.renderAll(); }
  duplicateRevision():void { const p=this.#ports, duplicate=duplicateSchemaRevision(p.active(),p.revisionVersion(),p.library.schemas); p.library.append(duplicate); p.persist(); p.renderAll(); }
