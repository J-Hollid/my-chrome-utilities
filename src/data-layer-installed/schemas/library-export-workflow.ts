...
export class SchemaLibraryExportWorkflow {
  ...
  request(): void {
    ...
    if (button) this.openChoices(button);
  }
  openStandard(scope: "library" | "schema", schema?: SchemaDefinition): void {
    ...
    conversions.append(
      ...review.conversions.map(({ ruleId, propertyPath, conversion }) =>
    ...
    omitted.setAttribute(
      "aria-label",
      "Unsupported rules omitted from standard export",
    );
    omitted.append(
      ...review.omitted.map(({ ruleName, propertyPath, behavior }) =>
        Object.assign(document.createElement("li"), {
  ...
  #confirmStandard(): void {
    ...
    if (pending.scope === "library") {
      ...
      this.#finish(
        `Exported JSON Schema Draft 2020-12 bundle · ${exported.resourceIds.length} schemas · ${omittedRuleStatus(exported.compatibility.omitted.length)}.`,
      );
    } else if (pending.schema) {
      const exported = exportJsonSchemaResource(
        pending.schema,
        this.#library.schemas,
      );
      this.#ports.download(exported.document, exported.filename);
      this.#finish(
        `Exported JSON Schema Draft 2020-12 · ${pending.schema.name} revision ${pending.schema.version} · ${omittedRuleStatus(exported.compatibility.omitted.length)}.`,
      );
    }
  }
  #exportExtension(schema?: SchemaDefinition): void {
    if (schema) {
      const archive = createExtensionSchemaPackage(
        schema,
        this.#library.schemas,
        this.#ports.rules(),
      );
      this.#ports.download(
        archive,
        `${schema.name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-extension-package-v1.json`,
      );
      this.#finish(
        `Exported Extension schema package · ${schema.name} revision ${schema.version}.`,
      );
      return;
    }
    const archive = createSchemaLibraryExport(
      this.#library.schemas,
      this.#ports.rules(),
    );
    this.#ports.download(archive, "schema-library-v1.json");
    this.#finish(
      `Exported Extension backup · ${archive.schemas.length} schemas and ${archive.rules.length} rules.`,
    );
  }
}
