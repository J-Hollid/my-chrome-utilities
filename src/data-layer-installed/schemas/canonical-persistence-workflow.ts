...
export class SchemaCanonicalPersistenceWorkflow {
  ...
  constructor(ports:CanonicalPersistenceWorkflowPorts) {
    ...
    this.#persistence=new SchemaPersistenceController({ root:ports.root,storage:ports.storage,library:ports.library,rules:ports.rules,
      ...
      renderRules:() => ports.rules.render(),renderCanonical:() => this.render(),clearCanonicalSettlement:(schemaId,settlement) => { this.clearSettlement(schemaId,settlement); },
        editorDraft:ports.editorDraft });
  }
  projection(canonical:CanonicalSchemaDocument):SchemaDefinition { return this.#ports.view.projection(canonical); }
  ...
  beginSettlement(schemaId?:string):number { const settlement=this.#ports.canonical.beginSettlement(schemaId);this.#ports.editor?.setAttribute("aria-busy","true");
    if(this.#ports.save)this.#ports.save.disabled=true;return settlement; }
  ...
  begin(kind:"guided"|"promotion",schemaId:string,previousSchemas:readonly SchemaDefinition[],previousRules:readonly ReusableSchemaRule[],nextSchemas:readonly SchemaDefinition[],
  ...
  promote(schemaId:string,previousSchemas:readonly SchemaDefinition[],previousRules:readonly ReusableSchemaRule[],nextSchemas:readonly SchemaDefinition[],
    nextRules:readonly ReusableSchemaRule[]):Promise<void> {
    const completion=this.begin("promotion",schemaId,previousSchemas,previousRules,nextSchemas,nextRules);
    persistLocalRulePromotion(this.#ports.storage,{ schemaKey:SCHEMA_LIBRARY_STORAGE_KEY,schemaValue:serializeSchemaLibrary(nextSchemas),ruleKey:SCHEMA_RULE_STORAGE_KEY,
