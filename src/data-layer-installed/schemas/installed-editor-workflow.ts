...
export class SchemaInstalledEditorWorkflow {
  ...
  editorBindings(ops: {
    ...
    return {
      ...ops,
      createSchema: () => this.openNew(),
      updateName: () => this.updateName(),
      saveDescription: () => this.saveDescription(),
      updateTarget: () => this.updateTarget(),
      changeParent: () => this.changeParent(),
      changeDeclaredOnly: () => this.changeDeclaredOnly(),
      openRevision: () => this.openRevision(),
      confirmRevision: () => this.confirmRevision(),
      cancelRevision: () => this.cancelRevision(),
      discardDraft: () => this.discardTransient(),
      keepEditing: () => this.keepEditing(),
      closeEditor: () => this.closeEditor(),
      saveAndClose: () => this.openRevision(),
      saveCloseReview: () => this.saveFromCloseReview(),
      discardWorking: () => this.discardWorking(),
      renderRevision: () => this.renderRevision(),
      duplicateRevision: () => this.duplicateRevision(),
      restoreRevision: () => this.restoreRevision(),
    };
  }
  propertyBindings(ops: {
    ...
    return {
      ...
      clearFilter: () => this.clearPropertyFilter(),
      activateSubview: (event: Event) => this.activateSubview(event),
      confirmRemoval: () => this.confirmRemoval(),
      cancelRemoval: (event?: Event) => this.cancelRemoval(event),
      undoRemoval: () => this.undoRemoval(),
      confirmDocumentationRemoval: () => this.confirmDocumentationRemoval(),
      cancelDocumentationRemoval: (event?: Event) =>
        this.cancelDocumentationRemoval(event),
      renderSpecificIndex: () => this.renderSpecificIndex(),
      submitSpecificIndex: (event: Event) => this.submitSpecificIndex(event),
      closeSpecificIndex: (event?: Event) => this.closeSpecificIndex(event),
      renderManual: () => this.renderManual(),
      submitManual: (event: Event) => this.submitManual(event),
      closeManual: (event?: Event) => this.closeManual(event),
  ...
  saveFromCloseReview(): void {
    const dialog = this.#ports.root.querySelector<HTMLDialogElement>(
      "#close-schema-editor-review",
    );
    dialog?.close();
  ...
  showSubview(subview: string): void {
    for (const tab of this.#ports.subviews) {
      const target =
          tab.dataset.schemaSubview ?? tab.getAttribute("aria-controls"),
        selected = target === subview;
  ...
  confirmRemoval(): void {
    this.#ports.property.confirmRemoval();
  }
  cancelRemoval(event?: Event): void {
    this.#ports.property.cancelRemoval(event);
  }
  undoRemoval(): void {
    this.#ports.property.undoRemoval();
  }
  ...
  confirmDocumentationRemoval(): void {
    ...
    this.#ports.schemaEditor?.setAttribute(
      "aria-busy",
      String(this.#ports.settleCanonical),
    );
  }
  ...
  openCopy(
    path: string,
    triggerOrDestination: HTMLButtonElement | string,
  ): void {
    this.#ports.property.openCopy(path, triggerOrDestination);
  }
  confirmCopy(): void {
    this.#ports.property.confirmCopy();
  }
  undoCopy(): void {
    this.#ports.property.undoCopy();
  }
  renderSpecificIndex(): void {
    this.#ports.property.renderSpecificIndex();
  }
  ...
  submitSpecificIndex(event: Event): void {
    this.#ports.property.submitSpecificIndex(event);
  }
  closeSpecificIndex(event?: Event): void {
    this.#ports.property.closeSpecificIndex(event);
  }
  renderManual(): void {
    this.#ports.property.renderManual();
  }
  ...
  submitManual(event: Event): void {
    this.#ports.property.submitManual(event);
  }
  ...
  goToExisting(): void {
    this.#ports.property.goToExisting();
  }
  ...
  openSpecification(
    ...
    this.#ports.renderSpecification(
      root,
      structuredClone(schema),
      structuredClone(this.#ports.library.schemas),
      surface,
      () => {
        root.hidden = true;
        this.#ports.editor.render();
