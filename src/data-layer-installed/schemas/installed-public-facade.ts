...
interface PublicFacadeLinks {
  library:SchemaLibraryController;property:SchemaPropertyController;rule:SchemaRuleController;assignment:{edit(schemaId:string,assignment:SchemaAssignment):void;render():void};
  ...
  relationshipTree:ReturnType<typeof createSchemaRelationshipTreeController>;projectHydration:SchemaProjectHydrationCoordinator;guidedRoot:HTMLElement|null;
...
export function createSchemasInstalledPublicFacade(p:PublicFacadeLinks) {
  ...
  const focus=(path:string):void => {p.property.selectPath(path.replace(/^\//,"").replaceAll("/", "."));p.render();};
  return {
    ...createSchemaLibraryPublicOperations({library:p.library,active,activeIndex,persist:() => p.library.persist(),render:p.render,publish:() => p.editorWorkflow.publish(),
      exportButton:p.exportButton,mounted:() => p.lifecycle.isMounted()}),
    ...createSchemaAuthoringPublicOperations({property:p.property,rule:p.rule,canonical:p.canonical,renderRulePicker:() => p.propertyWorkflow.render()},{
      openSchemaFromSource:(source:SchemaSourceDraftInput) => p.source.open(source),requestPropertyRemoval:(path:string,
        trigger?:HTMLButtonElement) => p.editorWorkflow.requestRemoval(path,trigger),requestDocumentationRemoval:(path:string,
        trigger?:HTMLElement) => p.editorWorkflow.requestDocumentationRemoval(path,trigger),
      requestPropertyCopy:(path:string,target:HTMLButtonElement|string) => p.editorWorkflow.openCopy(path,target),confirmPropertyCopy:() => p.editorWorkflow.confirmCopy(),
        openSpecificIndex:(path:string,trigger?:HTMLButtonElement) => p.editorWorkflow.openSpecificIndex(path,trigger),openManualProperty:(path?:string,
        trigger?:HTMLButtonElement) => p.editorWorkflow.openManual(path,trigger),openContextualManualProperty:(path:string,trigger?:HTMLButtonElement) =>
           p.editorWorkflow.openManual(path,
        trigger),
      capturePropertyReturn:(path:string,label:string) => p.propertyWorkflow.captureReturn(path,label),restorePropertyReturn:() => p.propertyWorkflow.restoreReturn(),
        closeRulePickerForCommit:() => p.propertyWorkflow.closeForCommit(),openRulePicker:(path:string,trigger?:HTMLButtonElement) => p.propertyWorkflow.open(path,trigger),
      openCanonicalRuleEditor:(path:string,trigger?:HTMLButtonElement) => p.canonicalView.openRule(path,trigger),openCanonicalPropertyActions:(path:string,
        trigger?:HTMLButtonElement) => p.canonicalView.openPropertyActions(path,trigger),configuredRule:() => p.propertyWorkflow.configuredRule(),
      ...
      openAttachedRule:(schemaId:string,ruleId:string,path?:string,trigger?:HTMLButtonElement) => p.propertyWorkflow.openAttached(schemaId,ruleId,path,trigger),
        attachReusableRule:(schemaId:string,ruleId:string,path?:string,rule?:ReusableSchemaRule) => p.rule.attach(schemaId,ruleId,path,rule),updateAttachedRule:(schemaId:string,
        ruleId:string,enabled:boolean) => p.rule.updateAttached(schemaId,ruleId,enabled),
      focusPropertyRule:focus,focusPropertyRow:focus,promotionRules,renderWorkflow:() => {p.rule.render();p.assignment.render();},editAssignment:(schemaId:string,
        assignment:SchemaAssignment) => p.assignment.edit(schemaId,assignment),requestLocalRulePromotion:(path:string,ruleId:string) => p.rule.openPromotion(path,ruleId)}),
    ...createGuidedPublicOperations({guided:p.guided,validation:p.validation,property:p.property,schemas:() => p.library.schemas,active,
      activeSchemaId:() => p.library.activeSchemaId,root:p.guidedRoot,generation:() => p.lifecycle.generation(),
      openValidation:(event,schema) => p.guidedWorkflow.openValidation(event,schema),closeValidation:() => p.guidedWorkflow.closeValidation(),
      ...
      persistGuidedValidation:(result:PublishedGuidedValidation) => p.guidedWorkflow.persistAndFinish(result),openGuidedEvent:(event:GuidedCapturedEvent,
    ...
    ...createCanonicalPublicOperations({controller:p.canonical,schema:(id) => p.library.schemas.find((schema) => schema.id===id),
      openSaved:(schema) => p.canonicalPersistence.openSaved(schema),open:(adapter) => p.canonicalPersistence.open(adapter),close:(clear) => p.canonicalPersistence.close(clear),
      show:() => {p.render();p.relationshipTree.restoreScroll();},projection:(canonical) => p.canonicalPersistence.projection(canonical),facet:(canonical,
      node) => p.canonicalPersistence.facet(canonical,node),render:() => p.canonicalPersistence.render()})
  };
}
