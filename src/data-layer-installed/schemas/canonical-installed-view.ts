...
export class SchemaCanonicalInstalledView {
  ...
  constructor(ports: CanonicalInstalledViewPorts) {
    this.#ports = ports;
    this.#contextTable = new SchemaCanonicalContextTableView(ports, (adapter) =>
      this.projection(adapter),
    );
  }
  projection(
    adapter: CompactCanonicalEditorAdapter,
    canonical = adapter.load(),
  ): SchemaDefinition {
    return (
      adapter.projection?.(canonical) ??
  ...
  facet(
    ...
    node: CanonicalSchemaDocument["nodes"][string],
  ): string {
  ...
  open(adapter: CompactCanonicalEditorAdapter): void {
    ...
    c.revisionSnapshots.set(
      adapter.load().revision,
      structuredClone(adapter.load()),
    );
  ...
  close(clearSelection = true): void {
    const p = this.#ports,
      c = p.controller,
      detail = p.elements.detail;
    if (c.editor && detail) c.scrollByKey.set(c.editor.key, detail.scrollTop);
    ...
    this.removeTable();
    if (p.elements.context) p.elements.context.hidden = true;
    if (p.elements.editor) p.elements.editor.hidden = true;
    if (detail) detail.hidden = false;
    if (p.elements.detailEmpty) p.elements.detailEmpty.hidden = false;
    p.renderAll();
    p.closeRoute((key) => {
      const row = Array.from(p.elements.list?.children ?? []).find(
        (candidate) =>
          (candidate as HTMLElement).dataset.schemaReferenceKey === key,
  ...
  openSaved(schema: SchemaDefinition): void {
    this.open(
      createCanonicalSavedAdapter(this.#ports, schema, () => this.close()),
    );
  }
  ...
  openPropertyActions(path: string, trigger?: HTMLButtonElement): boolean {
    const p = this.#ports,
      ...
      original =
        ...
        Object.values(model.nodes).find(
          (candidate) =>
            canonicalPropertyPath(model, candidate.id) === path ||
            candidate.id === path,
        ),
      owner = p.elements.editor,
      document = p.elements.document;
    if (!adapter || !model || !original || !owner || !document) return false;
    ...
    let working = structuredClone(original),
      activeSection: "definition" | "rules" | "structure" | undefined,
      feedbackText = "",
      stagedOwnershipAction = "";
    const ownership = focusedOwnershipState(
      focusedCanonicalOwnershipInput(original),
    );
    let ownershipSession = ownership.session;
    const removedRuleIds = new Set<string>(),
      removedValueIds = new Set<string>(),
      stagedOperations: NonNullable<
        Extract<
          Parameters<typeof applyCanonicalCommand>[1],
          {
            kind: "set";
          }
        >["operations"]
      > = [],
      state = focusedSourceState(original),
      sectionOwnership = focusedSectionOwnershipActions(ownership.input),
      close = () => {
      ...
      restoreFocus = (label: string) =>
        queueMicrotask(() =>
          Array.from(
            owner.ownerDocument.querySelectorAll<HTMLButtonElement>(
              '[data-schema-row-overlay="true"] button',
            ),
          )
            .find(
              ({ textContent, ariaLabel }) =>
                textContent?.trim() === label || ariaLabel === label,
            )
    ...
    const menu = () =>
      ...
      mount = (layers: HTMLElement[], focusLabel?: string) => {
        const sequence = focusedPropertyLayerSequence(
          activeSection,
          ...(layers.length === 3 ? ["review" as const] : []),
        );
    ...
    const sectionContext = (
      ...
      render,
      patchFor: (next: typeof working, source: typeof original) =>
        focusedPropertyPatch(next, source, removedRuleIds, removedValueIds),
      command: (command: Parameters<typeof applyCanonicalCommand>[1]) =>
        applyCanonicalCommand(model, command),
      select: () => {},
    ...
    const showReview = (
      ...
      const review = document.createElement("section"),
        ...
        patch = focusedPropertyPatch(
          working,
          original,
          removedRuleIds,
          removedValueIds,
        ),
        staged = focusedStagedChanges(
          working,
          original,
          removedRuleIds,
          path,
    ...
    const buildSection = (section: "definition" | "rules" | "structure") => {
      ...
      const target = focusedOwnershipActionTarget(
        ...
        visible = section === "rules" ? [] : sectionOwnership[section];
      if (visible.length) {
        ...
        for (const action of visible) {
          ...
          control.addEventListener("click", () => {
            ...
            ownershipSession = activateFocusedOwnershipSection(
              ownershipSession,
              section,
              action,
            );
            ...
            const operation = focusedPropertyLifecycleOperation(
              action,
              original.id,
            );
            if (operation) {
