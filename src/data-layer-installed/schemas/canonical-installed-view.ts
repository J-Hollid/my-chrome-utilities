...
export class SchemaCanonicalInstalledView {
    ...
    openRule(path: string, trigger?: HTMLButtonElement): boolean {
        ...
        const removedRuleIds = new Set<string>(), properties = () => Object.values(base.nodes).map(({ id, name, type, allowedValues }) => ({ id, name, type, allowedValues: allowedValues.map(({ value }) => value) })), button = (text: string, run: () => void): HTMLButtonElement => { const control = picker.ownerDocument.createElement("button"); control.type = "button"; control.textContent = text; control.addEventListener("click", run); return control; };
        const render = (): void => {
