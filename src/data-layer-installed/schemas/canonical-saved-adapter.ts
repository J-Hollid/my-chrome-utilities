...
export function createCanonicalSavedAdapter(
  ...
  controller.setSavedDocument(
    savedSchemaCanonicalDocument(ports.draft()!, ports.createId),
  );
  ...
  const project = (canonical: CanonicalSchemaDocument): SchemaDefinition => {
    ...
    const result = savedSchemaFromCanonical(
      { ...outer, name: canonical.contributorName },
      canonical,
    );
  ...
  const persistCanonical = (
    ...
    const source =
      ports.draft()?.id === schemaId
        ? ports.draft()!
        : ports.editorDraft(stored);
    const projection = savedSchemaFromCanonical(source, canonical);
    const updated = updateSchemaWorkingDraft(
      ports.proposeName(stored, projection.name),
      {
  ...
  return {
    ...
    dispatch: (command) => {
      ...
      if (result.status === "applied" || result.status === "rebased") {
        if (command.kind === "select" || command.kind === "view")
          controller.setSavedDocument(result.document);
        else
          persistCanonical(
            result.document,
    ...
    restoreStagedProjection: (canonical) =>
      controller.setSavedDocument(canonical),
    persistProjection,
