...
export function createSchemasInstalledController(ports: SchemasInstalledPorts) {
  ...
  const openCompactCanonicalEditor = (adapter:CompactCanonicalEditorAdapter):void => {
    if(!adapter.key.startsWith("saved:")){library.activeSchemaId=undefined;savedCanonicalDocument=undefined;library.draft=undefined;}
  ...
  const closeCompactCanonicalEditor = (clearSchemaSelection = true):void => { if (compactCanonicalEditor && schemaDetail) compactCanonicalScrollByKey.set(compactCanonicalEditor.key, schemaDetail.scrollTop);
    ...
    if (clearSchemaSelection) { library.activeSchemaId = undefined; library.draft = undefined; savedCanonicalDocument = undefined; }
  ...
  const persistSavedCanonicalResult = (schemaId:string, canonical:CanonicalSchemaDocument, change:string):void => {
    ...
    library.schemas = library.schemas.map((candidate) => candidate.id === schemaId ? updated : candidate); savedCanonicalDocument = canonical; persistSchemaLibrary();
  };
  ...
  const openSavedSchemaInUnifiedEditor = (schema:SchemaDefinition):void => {
    library.draft = schemaEditorDraft(schema); savedCanonicalDocument = savedSchemaCanonicalDocument(library.draft, (kind) => `schema:${kind}:${++compactCanonicalIdSequence}`);
  ...
  const renderSchemas = (): void => {
    ...
    const savedRow = (node: SchemaRelationshipTreeNode, level: number): HTMLLIElement | undefined => {
      const schema = library.schemas.find(({ id }) => `saved:${id}` === node.targetKey); if (!schema || !document) return;
      ...
      listen(revise, "click", () => {
        ...
        library.activeSchemaId = schema.id; library.draft = structuredClone(schema);
      ...
      listen(remove, "click", () => { const children = library.schemas.filter((candidate) => candidate.parentSchemaId === schema.id);
  ...
  const undoLastSchemaPropertyRemoval = (): void => {
    ...
    if (compactCanonicalEditor?.onUndo) {
      const path = propertyController.lastRemoval.propertyPath; propertyController.lastRemoval = undefined; compactCanonicalEditor.onUndo();
  ...
  function openSchemaPropertyCopyReview(path: string, triggerOrDestination: HTMLButtonElement | string): void {
    ...
    const reviewController = renderSchemaPropertyCopyReview(schemaPropertyCopyDialog!, { source, sources, selectedPath:path,
      ...
      onApply:(transaction) => {
        ...
        propertyController.lastCopy = transaction; propertyController.pendingCopy = undefined; propertyController.pendingCopyReview = undefined; persistSchemaLibrary(); renderSchemas(); renderSchemaRuleLibrary();
  ...
  function openCompactCanonicalPropertyActions(path:string,trigger?:HTMLButtonElement):boolean {
    ...
    compactCanonicalPropertyMenuId=original.id;propertyController.selectedPath=path.replace(/^\//,"").replaceAll("/",".");
    ...
    if (event.type === "failed" && !transactional && compactCanonicalSettlementSchemaId !== event.schemaId) {
      ...
      if (library.activeSchemaId) {
        ...
        if (activeStored) {
          ...
          savedCanonicalDocument = savedSchemaCanonicalDocument(library.draft,
  ...
  function openLocalRulePromotionReview(propertyPath: string, sourceRuleId: string): boolean {
    ...
    localRulePromotionDialog.open({ review,
      ...
      confirm:(selected: LocalRulePromotionSelection) => {
        ...
        library.schemas = structuredClone(nextSchemas); ruleController.rules = structuredClone([...nextRules]); renderSchemas(); renderSchemaRuleLibrary();
  ...
  const recheckCapturedSchemaValidation = (events:readonly GuidedCapturedEvent[] = []):readonly SchemaValidationRecord[] => {
    ...
    const records = events.map((event):SchemaValidationRecord => { const override = manualSchemaOverrides[event.id], candidates = override ? library.schemas.filter(({ id }) => id === override) : library.schemas;
  ...
  const editReusableSchemaRule = (id:string):boolean => {
    const rule = storedReusableRule(id); if (!rule) return false; ruleController.editingReusableId = id; openNewSchemaRuleEditor();
  ...
  const openNewSchemaRuleEditor = (): void => {
    ...
    if (schemaRuleAttachments?.ownerDocument) schemaRuleAttachments.replaceChildren(...library.schemas.map((schema) => {
  ...
  const beginNewReusableSchemaRule = ():void => {
    ruleController.editingReusableId = undefined; ruleController.approvedRevisionId = undefined;
    ruleController.pendingSnapshot = undefined; openNewSchemaRuleEditor();
  };
  const saveReusableSchemaRule = (): void => {
    ...
    if (previous && ruleController.approvedRevisionId !== previous.id) { captureReusableRuleSnapshot();
    ...
    if (updateSchemaRuleAttachments?.checked || rule.version === 1) library.schemas = library.schemas.map((schema) => {
    ...
    ruleController.editingReusableId = undefined; persistSchemaAndRuleLibraries(); renderSchemas(); renderSchemaRuleLibrary();
    if (schemaRuleEditor) schemaRuleEditor.hidden = true;
  };
  const captureReusableRuleSnapshot = ():void => { const previous = ruleController.editingReusableId ? storedReusableRule(ruleController.editingReusableId) : undefined;
    if (previous) ruleController.pendingSnapshot = { id:previous.id, version:previous.version, attachments:[...(previous.attachments ?? [])] }; };
  ...
  const requestSchemaRuleRevision = (id: string,
    ...
    const previous = ruleController.rules.find((rule) => rule.id === id); if (!previous) return false;
    ruleController.pendingRevision = { id, changes:structuredClone(changes) }; ruleController.approvedRevisionId = undefined;
  ...
  const confirmReusableSchemaRuleRevision = (): void => {
    const pending = ruleController.pendingRevision; if (!pending) return;
    ...
    ruleController.approvedRevisionId = pending.id; if (ruleController.editingReusableId === pending.id) { ruleController.editingReusableId = undefined; if (schemaRuleEditor) schemaRuleEditor.hidden = true; }
    ruleController.pendingRevision = undefined; persistReusableSchemaRules(); renderSchemaRuleLibrary(); schemaRuleRevisionReview?.close();
  };
  const cancelReusableSchemaRuleRevision = (): void => { ruleController.pendingRevision = undefined; schemaRuleRevisionReview?.close(); };
  const requestSchemaRuleUpgrade = (id: string, schemaIds: readonly string[]): boolean => {
    const rule = ruleController.rules.find((candidate) => candidate.id === id); if (!rule) return false;
  ...
  const confirmReusableSchemaRuleUpgrade = (): void => {
    const pending = ruleController.pendingUpgrade; if (!pending) return;
    const rule = ruleController.rules.find((candidate) => candidate.id === pending.id); if (!rule) return;
    ...
    ruleController.approvedAttachmentUpdateId = pending.id; ruleController.pendingUpgrade = undefined; persistSchemaLibrary(); schemaRuleUpgradeReview?.close();
  };
  const cancelReusableSchemaRuleUpgrade = (): void => { ruleController.pendingUpgrade = undefined; schemaRuleUpgradeReview?.close(); };
  const requestSchemaRuleSync = (id: string): boolean => {
    const rule = ruleController.rules.find((candidate) => candidate.id === id); if (!rule) return false;
    const review = reviewReusableRuleSync(library.schemas, rule); ruleController.pendingSync = { rule:structuredClone(rule), review };
  ...
  const confirmReusableSchemaRuleSync = (): void => {
    const pending = ruleController.pendingSync; if (!pending) return;
    ...
    if (JSON.stringify(settledReview) !== JSON.stringify(pending.review)) throw new Error("The attached library.schemas changed after review");
    library.schemas = publishReusableRuleSync(library.schemas, settledRule, settledReview); ruleController.pendingSync = undefined;
    persistSchemaLibrary(); renderSchemas(); schemaRuleSyncReview?.close();
  };
  const cancelReusableSchemaRuleSync = (): void => { ruleController.pendingSync = undefined; schemaRuleSyncReview?.close(); };
  ...
  const confirmReusableSchemaRuleDeletion = (): void => {
    if (!ruleController.pendingDeletionId) return;
    ruleController.rules = ruleController.rules.filter(({ id }) => id !== ruleController.pendingDeletionId);
    ruleController.pendingDeletionId = undefined; persistReusableSchemaRules(); renderSchemaRuleLibrary(); schemaRuleDeleteReview?.close();
  };
  const cancelReusableSchemaRuleDeletion = (): void => { ruleController.pendingDeletionId = undefined; schemaRuleDeleteReview?.close(); };
  ...
  const replaceSchemaLibrary = (): void => {
    if (!pendingSchemaImport) return; library.schemas = structuredClone(pendingSchemaImport.schemas);
    ruleController.rules = structuredClone(pendingSchemaImport.rules); pendingSchemaImport = undefined;
  ...
  const requestSchemaDeletion = (id: string): boolean => {
    const schema = library.schemas.find((candidate) => candidate.id === id); if (!schema) return false;
  ...
  const confirmSchemaDeletion = (): void => {
    const schema = pendingSchemaDeletion; if (!schema) return; library.schemas = library.schemas.filter(({ id }) => id !== schema.id);
    pendingSchemaDeletion = undefined; if (library.activeSchemaId === schema.id) { library.activeSchemaId = undefined; library.draft = undefined; }
  ...
  function openStandardSchemaExportReview(scope: "library" | "schema", schema?: SchemaDefinition): void {
    ...
    confirm.addEventListener("click", () => { const pending = pendingStandardSchemaExport; if (!pending) return;
      if (pending.scope === "library") { const exported = exportJsonSchemaBundle(library.schemas); downloadSchemaJson(exported.document, exported.filename);
      ...
      else if (pending.schema) { const exported = exportJsonSchemaResource(pending.schema, library.schemas); downloadSchemaJson(exported.document, exported.filename);
  ...
  function openSchemaExportChoices(trigger: HTMLButtonElement, schema?: SchemaDefinition): void {
    ...
    extension.addEventListener("click", () => { if (schema) { const archive = createExtensionSchemaPackage(schema, library.schemas, ruleController.rules);
    ...
    else { const archive = createSchemaLibraryExport(library.schemas, ruleController.rules); ports.downloadSchema(archive, "schema-library-v1.json");
  ...
  return {
    ...
    livePropertyDeclaration:(event:GuidedCapturedEvent,path:string) => { const schema = selectedGuidedContinuation(guidedContinuationSelections,event,library.schemas);
    ...
    liveValidationAvailable:(event:GuidedCapturedEvent) => { const manual=library.schemas.find(({id})=>id===manualSchemaOverrides[event.id]);
    ...
    validateLive:(event:GuidedCapturedEvent) => { const input={sourceId:event.sourceId,eventName:event.name,payload:event.payload,rawInput:event.rawInput};
      const manual=library.schemas.find(({id})=>id===manualSchemaOverrides[event.id]);return manual?validateWithSchema(input,manual,library.schemas):validateEvent(input,library.schemas,event.pageUrl); },
    ...
    guidedState:() => ({ selections:structuredClone(guidedContinuationSelections), selectedSchemaPropertyPath:propertyController.selectedPath,
