...
export function createSchemasInstalledController(ports: SchemasInstalledPorts) {
  ...
  const renderSchemaPropertyView = (): void => {
    ...
    if (schemaPropertyTree) {
      const items = propertyView.rows.flatMap((row) => { const item = schemaOwnerDocument?.createElement("li");
        ...
        const summary = schemaOwnerDocument!.createElement("strong"); summary.textContent = row.displayPath; item.append(summary);
        ...
        propertyAction("Edit rules", (button) => {
          if (!openCompactCanonicalRuleEditor(row.displayPath, button)) openSchemaPropertyRulePicker(row.canonicalPath, button);
        },
        ...
        listenProperty(disclosure, "toggle", () => { if (disclosure.open) expandedSchemaPropertyRulePaths.add(row.canonicalPath);
          else expandedSchemaPropertyRulePaths.delete(row.canonicalPath); }); disclosure.append(disclosureSummary);
        if (!attachedRules.length) disclosure.append("No rules attached to this property.");
        for (const attached of attachedRules) {
          const attachedRow = schemaOwnerDocument!.createElement("div"); attachedRow.dataset.ruleId = attached.id;
          attachedRow.textContent = `${attached.id} v${attached.version} · ${attached.operator ?? "rule"} · ${attached.enabled === false ? "disabled" : "active"} `;
          const attachedAction = (label:string, run:() => void):void => { const button = schemaOwnerDocument!.createElement("button");
            button.type = "button"; button.textContent = label; listenProperty(button, "click", run); attachedRow.append(button); };
          attachedAction("Edit", () => { editingAttachedLocalRule = attached; openSchemaPropertyRulePicker(row.canonicalPath); });
          ...
          if (attached.id.startsWith("local:")) attachedAction("Promote to reusable rule", () => { openLocalRulePromotionReview(row.canonicalPath, attached.id); });
  ...
  const confirmSchemaRevision = (): void => { publishActiveSchema(true); };
  const cancelSchemaRevision = (): void => { schemaRevisionReview?.close(); if (schemaRevisionReview) schemaRevisionReview.hidden = true; };
  ...
  const closeSchemaEditor = (): void => { if (!schemaDraft && !activeSchemaId) return; activeSchemaId = undefined; schemaDraft = undefined;
    renderSchemas(); if (schemaResult) schemaResult.textContent = "Working draft retained without publishing."; };
  ...
  function openAllowedValueExpansionReview(eventId:string, assignedSchemaId:string, evaluation:ValidationEvaluation, trigger:HTMLButtonElement):boolean {
    ...
    const disposeDialog = openAllowedValueExpansionDialog({ inspector, review, trigger,
      confirm:(destination:AllowedValueExpansionDestination) => { const applied = applyAllowedValueExpansion({ ...input, destination });
        ...
        return () => ports.scheduleFrame(() => ports.restoreGuidedCapture(eventId, evaluation.propertyPath)); },
