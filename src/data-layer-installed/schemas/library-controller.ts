...
export class SchemaLibraryController implements SchemaLibraryWorkflowPort {
  ...
  constructor(ports: SchemaLibraryPorts) {
    ...
    try {
      ...
      this.#initialProjection = Array.isArray(parsed)
        ? (parsed as SchemaDefinition[])
        : [];
    } catch {
      this.#initialProjection = [];
    }
  }
  configure(behavior: SchemaLibraryBehaviorPorts): void {
  ...
  get draft(): SchemaDefinition | undefined {
  ...
  select(id: string, draft?: SchemaDefinition): void {
    ...
    this.#draft = structuredClone(
      draft ?? this.#schemas.find((schema) => schema.id === id),
    );
  }
  setDraft(next: SchemaDefinition | undefined): void {
  ...
  append(schema: SchemaDefinition): void {
  ...
  replaceActive(schema: SchemaDefinition): void {
    ...
    this.#schemas = this.#schemas.map((candidate, candidateIndex) =>
      candidateIndex === index ? structuredClone(schema) : candidate,
    );
    this.#draft = structuredClone(schema);
  }
  reload(): void {
    this.#schemas = restoreSchemaLibrary(
      this.#ports.storage.getItem(SCHEMA_LIBRARY_STORAGE_KEY),
    );
  }
  serialize(next: readonly SchemaDefinition[] = this.#schemas): string {
    const storedById = new Map(
      this.#initialProjection.map((schema) => [schema.id, schema]),
    );
    const entries = next.map((schema) => {
      const canonical = JSON.parse(
        serializeSchemaLibrary([schema]),
  ...
  omittedStatus(count: number): string {
    return (
      this.#operations?.omittedStatus(count) ??
