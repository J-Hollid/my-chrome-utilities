...
import { inspectLocalRuleEdit, saveLocalRuleEdit, publishReusableRuleSync, reviewReusableRuleSync, type ReusableRuleSyncReview } from "../../utilities/data-layer/schemas.js";
...
import { activateFocusedOwnershipSection, applyCanonicalCommand, canonicalCommandOutcome, canonicalCommandsFromCompactProjection, canonicalMigrationDurablyAcknowledged, canonicalPredicateText, canonicalPropertyPath, clearSchemaTableOverlay, compactCanonicalCommandPolicy, compactSchemaProjection, composedCanonicalSchema, createCanonicalSchema, focusedCanonicalOwnershipInput, focusedDefinitionFieldLabels, focusedOwnershipActionTarget, focusedOwnershipSectionEditable, focusedOwnershipState, focusedPropertyLayerSequence, focusedPropertyLifecycleOperation, focusedPropertyPatch, focusedPropertyProvenanceSummary, focusedSectionOwnershipActions, focusedSourceState, focusedStagedChanges, gateFocusedOwnershipSection, hasLegacySchemaRepresentation, migrateLegacyProfile, mountCanonicalPredicateEditor, mountCanonicalSchemaEditor, mountSchemaTableOverlay, mountSidePanelLayeredProfileEditor, projectCanonicalConcepts, renderCanonicalFocusedSection, renderFocusedPropertyMenu, resolveCanonicalMigrationConflict, resolveSidePanelSchemaContributor, saveComposedCanonicalDocument, saveComposedEntitySchemaPolicy, saveComposedEventCanonicalDocument, saveEventOccurrenceCanonicalDocument, saveFlowContributorSchemaPolicy, saveFlowPageInstanceCanonicalDocument, savedSchemaCanonicalDocument, savedSchemaFromCanonical, sidePanelSchemaGroups, transactProject, type CanonicalSchemaDocument, type ProjectEntity, type ProjectState, type SidePanelContributorSelection } from "../../utilities/data-layer/schemas.js";
import {filterSchemaRelationshipTree,projectSchemaRelationshipTree,restoreSchemaRelationshipTreeView,saveSchemaRelationshipTreeView,type SchemaRelationshipCategory,type SchemaRelationshipTreeNode} from "../../utilities/data-layer/schemas.js";
import {beginCompactCanonicalHistoryTransition,compactCanonicalDocumentIdentity,compactCanonicalHistoryKey,compactCanonicalHistorySettlement,completeCompactCanonicalHistoryTransition,prepareCompactCanonicalRedo,prepareCompactCanonicalUndo,recordCompactCanonicalMutation,rejectCompactCanonicalHistoryTransition,type CompactCanonicalHistoryTransitionIdentity,type CompactCanonicalPageHistory} from "../../utilities/data-layer/schemas.js";
...
import { createManualFlowDefectEvent, mountLiveFlowTestingUi, type CompletedLiveFlowTest } from "../../utilities/data-layer/live-inspection.js";
import {
...
const scopedDataLayerStorage = createUtilityStorage(globalThis.localStorage, utilityStorageContract("data-layer"));
...
const schemaCategoryFilter = document.querySelector<HTMLSelectElement>("#schema-category-filter");
...
let pendingLocalRulePromotionPersistence: { schemaId:string; complete():void; reject(error:unknown):void } | undefined;
let pendingGuidedValidationPersistence: { schemaId:string; pause():void; complete():void; reject(error:unknown):void } | undefined;
document.querySelector<HTMLDialogElement>("#durable-storage-recovery")?.addEventListener("close",()=>{
  if(schemaPropertyInteractionReturn&&!schemaPropertyRulePicker.open&&!durableProjectRuntime.failedSchemaSave()&&!compactCanonicalProjectionRequest&&!compactCanonicalProjectionWorker)finishSchemaPropertyInteractionReturn();
});
...
const schemaRuleSyncReview = document.createElement("dialog");
schemaRuleSyncReview.id = "schema-rule-sync-review";
const schemaRuleSyncReviewSummary = document.createElement("output");
schemaRuleSyncReviewSummary.id = "schema-rule-sync-review-summary";
const confirmSchemaRuleSyncButton = document.createElement("button");
confirmSchemaRuleSyncButton.id = "confirm-schema-rule-sync"; confirmSchemaRuleSyncButton.type = "button"; confirmSchemaRuleSyncButton.textContent = "Confirm sync and publish revisions";
const cancelSchemaRuleSyncButton = document.createElement("button");
cancelSchemaRuleSyncButton.type = "button"; cancelSchemaRuleSyncButton.textContent = "Cancel";
schemaRuleSyncReview.append(Object.assign(document.createElement("h4"), { textContent:"Sync attached schemas and publish revisions" }), schemaRuleSyncReviewSummary, confirmSchemaRuleSyncButton, cancelSchemaRuleSyncButton);
document.body.append(schemaRuleSyncReview);
let pendingSchemaRuleSync: { rule:ReusableSchemaRule; review:ReusableRuleSyncReview; trigger:HTMLButtonElement } | undefined;
...
const schemaTreeScrollOwner = document.querySelector<HTMLElement>("#workspace-panel-data-layer");
const schemaPanel = document.querySelector<HTMLElement>("#data-layer-panel-schemas");
...
const restoredSavedEventFeedView = restoreSavedEventFeedWorkingView(
  dataLayerStorage.getItem(SAVED_EVENT_FEED_FILTER_WORKING_STORAGE_KEY),
...
let savedSessionLibrary: SavedSessionLibrary = restoreSavedSessionLibrary(dataLayerStorage.getItem(SAVED_SESSION_LIBRARY_STORAGE_KEY));
let defectLibrary: DefectLibrary = restoreDefectLibrary(dataLayerStorage.getItem(DEFECT_LIBRARY_STORAGE_KEY));
...
let savedSessionLiveFeed: SavedSessionLiveFeed | undefined = restoreSavedSessionLiveFeed(dataLayerStorage.getItem(SAVED_SESSION_LIVE_FEED_STORAGE_KEY), savedSessionLibrary);
if (savedSessionLiveFeed) liveObserverState = savedSessionLiveFeed.savedView;
let completedLiveFlowTests: CompletedLiveFlowTest[] = [];
...
function renderProjectEventTransport():void {
  ...
  renderHistoryPath(
    ...
    settings?.observationHistoryPath??"",
    available ? currentTargetPathStatus : "Selection required",
  );
...
const liveFlowTestingUi=mountLiveFlowTestingUi({
  ...
  onResult:(entry,event)=>{const presented=createManualFlowDefectEvent(entry,event as LiveEvent);liveObserverState={...liveObserverState,events:liveObserverState.events.map((candidate)=>candidate.id===event.id?presented:candidate)};renderLiveObserver();openLiveInspector(event.id,true);},
...
if(durableProjectRuntime.migration.status==="review-required"){const review=document.querySelector<HTMLElement>("#durable-migration-review")!,summary=document.querySelector<HTMLElement>("#durable-migration-review-summary")!,result=document.querySelector<HTMLOutputElement>("#durable-migration-review-result")!,migration=durableProjectRuntime.migration;review.hidden=false;summary.textContent=`${migration.projectId} differs between ${migration.sources.map(({source,revision,checksum})=>`${source} generation ${revision} checksum ${checksum}`).join(" and ")}. Conflicting fields: ${migration.conflictingFields.join(", ")}. No source was deleted or imported.`;document.querySelector<HTMLButtonElement>("#export-legacy-migration-sources")!.addEventListener("click",()=>{const sources=migration.sources.map((source)=>({...source,key:source.source==="library"?"my-chrome-utilities.specification-project-library.v1":"my-chrome-utilities.specification-project.v1",payload:globalThis.localStorage.getItem(source.source==="library"?"my-chrome-utilities.specification-project-library.v1":"my-chrome-utilities.specification-project.v1")})),serialized=JSON.stringify({format:"my-chrome-utilities.legacy-migration-sources",version:1,projectId:migration.projectId,sources}),link=document.createElement("a");link.href=URL.createObjectURL(new Blob([serialized],{type:"application/json"}));link.download=`${migration.projectId}-legacy-migration-sources.json`;link.click();URL.revokeObjectURL(link.href);result.textContent="Exported both complete legacy source payloads with their identities and checksums.";});for(const[selector,choice]of[["#migrate-legacy-library-source","library"],["#migrate-legacy-active-source","active"]] as const)document.querySelector<HTMLButtonElement>(selector)!.addEventListener("click",()=>{result.textContent=`Migrating the reviewed ${choice} source…`;void durableProjectRuntime.resolveMigration(choice).then(()=>location.reload(),error=>{result.textContent=`Migration was not committed; both legacy sources remain unchanged. ${error instanceof Error?error.message:String(error)}`;});});}
if(durableProjectRuntime.migration.status==="invalid-source-review"){const review=document.querySelector<HTMLElement>("#durable-migration-review")!,summary=document.querySelector<HTMLElement>("#durable-migration-review-summary")!,result=document.querySelector<HTMLOutputElement>("#durable-migration-review-result")!,migration=durableProjectRuntime.migration,schemaSource=migration.sources.find(({key})=>key==="my-chrome-utilities.schema-library.v1"),validChoice=(key:string)=>migration.sources.find((source)=>source.key===key)?.valid===true&&schemaSource?.valid!==false;review.hidden=false;summary.textContent=`Invalid legacy source recovery only; zero durable project records were written. ${migration.sources.map(({key,valid,error,bytes,checksum})=>`${key}: ${valid?"valid":"invalid"} · ${bytes} bytes · checksum ${checksum} · ${error}`).join(" | ")}. Export every preserved byte, repair the named invalid key and reload, or explicitly choose a valid project source.`;document.querySelector<HTMLButtonElement>("#export-legacy-migration-sources")!.addEventListener("click",()=>{const serialized=JSON.stringify({format:"my-chrome-utilities.invalid-legacy-migration-sources",version:1,actions:migration.actions,sources:migration.sources}),link=document.createElement("a");link.href=URL.createObjectURL(new Blob([serialized],{type:"application/json"}));link.download="invalid-legacy-migration-sources.json";link.click();URL.revokeObjectURL(link.href);result.textContent="Exported every invalid and valid legacy source byte-for-byte with errors and checksums.";});for(const[selector,choice,key]of[["#migrate-legacy-library-source","library","my-chrome-utilities.specification-project-library.v1"],["#migrate-legacy-active-source","active","my-chrome-utilities.specification-project.v1"]] as const){const control=document.querySelector<HTMLButtonElement>(selector)!;control.disabled=!validChoice(key);control.addEventListener("click",()=>{result.textContent=`Migrating only the explicitly reviewed valid ${choice} source; rejected bytes remain in backup…`;void durableProjectRuntime.resolveMigration(choice).then(()=>location.reload(),error=>{result.textContent=`Migration was not committed; every legacy source remains byte-identical. ${error instanceof Error?error.message:String(error)}`;});});}}
void mountDurableProjectRepositoryUi(document,globalThis.indexedDB,durableProjectRuntime.repository).then((ui)=>{globalThis.addEventListener("durable-project-save-failed",(event)=>{const origin=document.activeElement instanceof HTMLElement?document.activeElement:undefined,failed=durableProjectRuntime.failedSave(),failedSchema=durableProjectRuntime.failedSchemaSave();if(failed){const pendingHistory=compactCanonicalPendingHistoryFor(failed.projectId,failed.command.label);projectLibraryUi.render();void ui.reportSaveFailure({projectId:failed.projectId,projectName:failed.projectName,command:failed.command,retry:async()=>{await durableProjectRuntime.retryFailedSave();const currentEditorMatches=Boolean(pendingHistory&&compactCanonicalEditor?.key===pendingHistory.editorKey);if(pendingHistory)completeCompactCanonicalPendingHistory(pendingHistory);if(currentEditorMatches)compactCanonicalCommandFeedback=`Retried ${failed.command.label}; the exact command committed to the Saved Draft and is now in page history.`;projectLibraryUi.render();renderSchemas();if(compactCanonicalEditor)renderCompactCanonicalEditor();},reject:async()=>{await durableProjectRuntime.resolveFailedSave("reject");const currentEditorMatches=Boolean(pendingHistory&&compactCanonicalEditor?.key===pendingHistory.editorKey);if(pendingHistory)rejectCompactCanonicalPendingHistory(pendingHistory);if(currentEditorMatches)compactCanonicalCommandFeedback=`Rejected ${failed.command.label}; the last durable Saved Draft was restored and the prior Undo and Redo state is unchanged.`;projectLibraryUi.render();renderSchemas();if(compactCanonicalEditor)renderCompactCanonicalEditor();},exportUnsaved:()=>{const serialized=durableProjectRuntime.exportUnsavedDraft(),link=document.createElement("a");link.href=URL.createObjectURL(new Blob([serialized],{type:"application/json"}));link.download=`${failed.projectName.replace(/[^a-z0-9]+/gi,"-").toLowerCase()}-unsaved-draft.json`;link.click();URL.revokeObjectURL(link.href);},...(origin?{originControl:origin}:{})},(event as CustomEvent<{error?:unknown}>).detail?.error??failed.error);return;}if(!failedSchema)return;const affectedId=String(failedSchema.batch.upserts[0]?.schema.id??failedSchema.batch.deletes[0]?.schemaId??""),affectedEditorKey=`saved:${affectedId}`,promotionPersistence=pendingLocalRulePromotionPersistence?.schemaId===affectedId?pendingLocalRulePromotionPersistence:undefined,guidedPersistence=pendingGuidedValidationPersistence&&failedSchema.batch.upserts.some(({schema:candidate})=>candidate.id===pendingGuidedValidationPersistence?.schemaId)?pendingGuidedValidationPersistence:undefined,editorAtFailure=compactCanonicalEditor?.key===affectedEditorKey?compactCanonicalEditor:undefined,failedProjectionAdapter=()=>compactCanonicalProjectionRequest?.adapter.key===affectedEditorKey?compactCanonicalProjectionRequest.adapter:undefined,restoreAffectedSchema=()=>{schemas=restoreSchemaLibrary(projectStorage.getItem(SCHEMA_LIBRARY_STORAGE_KEY));const restored=schemas.find(({id})=>id===affectedId),activeAffected=compactCanonicalEditor?.key===affectedEditorKey||(!compactCanonicalEditor&&schemaDraft?.id===affectedId);if(activeAffected){if(restored){schemaDraft=schemaEditorDraft(restored);savedCanonicalDocument=savedSchemaCanonicalDocument(schemaDraft,(kind)=>`${kind}:${crypto.randomUUID()}`);}else{schemaDraft=undefined;savedCanonicalDocument=undefined;}}return restored;};guidedPersistence?.pause();restoreAffectedSchema();renderSchemas();renderSchemaWorkflowRows();renderSchemaDraft();if(compactCanonicalEditor)renderCompactCanonicalEditor();const names=failedSchema.batch.names.join(", ");void ui.reportSaveFailure({kind:"saved-schema",projectName:names,command:{label:failedSchema.batch.label},retry:async()=>{await durableProjectRuntime.retryFailedSchemaSave();restoreAffectedSchema();renderSchemas();renderSchemaWorkflowRows();const projectionAdapter=failedProjectionAdapter();if(editorAtFailure&&compactCanonicalEditor===editorAtFailure&&projectionAdapter===editorAtFailure){if(!await resumeCompactCanonicalProjectionPersistence(editorAtFailure))throw new Error("The latest queued Saved Schema metadata edit was not committed after Retry.");}else{if(projectionAdapter)discardCompactCanonicalProjectionPersistence(projectionAdapter);renderSchemaDraft();if(compactCanonicalEditor)renderCompactCanonicalEditor();}promotionPersistence?.complete();guidedPersistence?.complete();},reject:async()=>{await durableProjectRuntime.resolveFailedSchemaSave("reject");promotionPersistence?.reject(failedSchema.error);guidedPersistence?.reject(failedSchema.error);const projectionAdapter=failedProjectionAdapter();if(projectionAdapter)discardCompactCanonicalProjectionPersistence(projectionAdapter);else if(editorAtFailure&&compactCanonicalEditor===editorAtFailure)discardCompactCanonicalProjectionPersistence(editorAtFailure);restoreAffectedSchema();if(editorAtFailure&&compactCanonicalEditor===editorAtFailure)compactCanonicalCommandFeedback=`Rejected ${failedSchema.batch.label}; the latest durable Saved Schema and concurrency token were reloaded for review.`;renderSchemas();renderSchemaWorkflowRows();renderSchemaDraft();if(compactCanonicalEditor)renderCompactCanonicalEditor();},exportUnsaved:()=>{const serialized=durableProjectRuntime.exportUnsavedSchemas(),link=document.createElement("a");link.href=URL.createObjectURL(new Blob([serialized],{type:"application/json"}));link.download="unsaved-saved-schema-batch.json";link.click();URL.revokeObjectURL(link.href);},...(origin?{originControl:origin}:{})},(event as CustomEvent<{error?:unknown}>).detail?.error??failedSchema.error);});}).catch((error)=>{const status=document.querySelector<HTMLOutputElement>("#durable-repository-status"),open=document.querySelector<HTMLButtonElement>("#open-storage-recovery");if(status)status.textContent=`Durable project storage unavailable: ${error instanceof Error?error.message:String(error)}`;if(open)open.disabled=true;});
sidePanelLayeredProfileEditor=sidePanelLayeredProfileEditorHost?mountSidePanelLayeredProfileEditor({host:sidePanelLayeredProfileEditorHost,legacyEditor:schemaEditor,emptyHost:schemaDetailEmpty,openUnifiedContributor:openContributorInUnifiedEditor,load:()=>restoreCanonicalProjectState(projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY)),subscribe:(listener)=>durableProjectRuntime.subscribe(()=>listener()),onUndo:()=>{const state=restoreCanonicalProjectState(projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY));if(state)void durableProjectRuntime.undo(state.project.id);},onRedo:()=>{const state=restoreCanonicalProjectState(projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY));if(state)void durableProjectRuntime.redo(state.project.id);},persist:(next)=>{const serialized=projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY),envelope=restoreCanonicalProjectEnvelope(serialized),base=restoreCanonicalProjectState(serialized);if(!envelope||!base)throw new Error("The Specification Project is unavailable.");const result=commitCanonicalProjectState(projectStorage,next,{expectedRevision:envelope.revision,pendingLabel:"Side-panel canonical schema command",base});if(result.status==="conflict")throw new Error(`Schema contributor changed in a newer Saved Draft; review the structured edit again.`);projectLibraryUi.captureActiveProject(next,result.revision);}}):undefined;
...
type CompactCanonicalEditorAdapter={key:string;label:string;load:()=>CanonicalSchemaDocument;dispatch:(command:Parameters<typeof applyCanonicalCommand>[1])=>ReturnType<typeof applyCanonicalCommand>;stageProjectionCommand?:(command:Parameters<typeof applyCanonicalCommand>[1])=>ReturnType<typeof applyCanonicalCommand>;restoreStagedProjection?:(canonical:CanonicalSchemaDocument)=>void;projection?:(canonical:CanonicalSchemaDocument)=>SchemaDefinition;persistProjection?:(projection:SchemaDefinition,change?:string)=>boolean;settle?:()=>Promise<void>;settles?:(command:Parameters<typeof applyCanonicalCommand>[1])=>boolean;settlementTarget?:string;onSettlementCommitted?:()=>void;onUndo?:()=>void;onRedo?:()=>void;actions?:readonly {label:string;run:()=>void}[];renderContext?:(host:HTMLElement)=>void};
...
type CompactCanonicalProjectionPersistenceRequest={adapter:CompactCanonicalEditorAdapter;projection:SchemaDefinition;change?:string};
let compactCanonicalProjectionRequest:CompactCanonicalProjectionPersistenceRequest|undefined;
let compactCanonicalProjectionWorker:{adapter:CompactCanonicalEditorAdapter;promise:Promise<boolean>}|undefined;
...
let manualSchemaOverrides: Record<string, string> = (() => { try { const stored = JSON.parse(dataLayerStorage.getItem(MANUAL_SCHEMA_OVERRIDE_STORAGE_KEY) ?? "{}"); return stored && typeof stored === "object" && !Array.isArray(stored) ? stored as Record<string, string> : {}; } catch { return {}; } })();
const SCHEMA_VALIDATION_RECORD_STORAGE_KEY = "my-chrome-utilities.schema-validation-records.v1";
interface SchemaValidationRecord { eventId: string; eventName: string; state: string; checkedAt: string; schemaId?:string; schemaName?: string; schemaVersion?: number; target?: string; issueCodes?:string[]; assignmentId?: string; assignmentName?: string; assignmentEvidence?: string; evaluated?:{resultIdentity:string;winner?:{schemaId:string;schemaRevision:number};issueDetails:{code:string;path?:string}[]}; }
let schemaValidationRecords: SchemaValidationRecord[] = (() => { try { const stored = JSON.parse(dataLayerStorage.getItem(SCHEMA_VALIDATION_RECORD_STORAGE_KEY) ?? "[]"); return Array.isArray(stored) ? stored.filter((record): record is SchemaValidationRecord => !!record && typeof record.eventId === "string" && typeof record.eventName === "string" && typeof record.state === "string" && typeof record.checkedAt === "string") : []; } catch { return []; } })();
const SCHEMA_RULE_STORAGE_KEY = "my-chrome-utilities.schema-rule-library.v1";
interface ReusableSchemaRule { id: string; name: string; kind: string; version?: number; enabled?: boolean; propertyPath?: string; operator?: string; parameters?: string; allowedValues?: readonly (string | number | boolean | null)[]; comparison?: import("../../data-layer-schema-property-rule-picker.js").CardinalityComparison; limit?: number; migrationIssue?: string; description?: string; applicableType?: SchemaPropertyType; severity?: string; message?: string; examples?: string; attachments?: readonly string[]; conditionGroup?: import("../../data-layer-conditional-validation-rules.js").ConditionalRuleConditionGroup; revisionHistory?: readonly { name: string; kind: string; version: number; enabled?: boolean; propertyPath?: string; operator?: string; parameters?: string; allowedValues?: readonly (string | number | boolean | null)[]; comparison?: import("../../data-layer-schema-property-rule-picker.js").CardinalityComparison; limit?: number; migrationIssue?: string; severity?: string; message?: string; conditionGroup?: import("../../data-layer-conditional-validation-rules.js").ConditionalRuleConditionGroup }[]; }
...
if (storedReusableSchemaRules && JSON.stringify(reusableSchemaRules) !== storedReusableSchemaRules) dataLayerStorage.setItem(SCHEMA_RULE_STORAGE_KEY, JSON.stringify(reusableSchemaRules));
...
function showDataLayerView(view: DataLayerView, focus = false): void {
  ...
  if(view==="Schemas"){restorePendingSchemaTreeScroll();void hydrateActiveProjectForSchemas().catch((error)=>{if(schemaResult)schemaResult.textContent=`Schema contributors are unavailable. ${error instanceof Error?error.message:String(error)}`;});}
...
function persistSavedEventFeedWorkingView(): void {
  ...
  if (!sessionId) { dataLayerStorage.removeItem(SAVED_EVENT_FEED_FILTER_WORKING_STORAGE_KEY); return; }
  dataLayerStorage.setItem(SAVED_EVENT_FEED_FILTER_WORKING_STORAGE_KEY, serializeSavedEventFeedWorkingView(
    sessionId,
...
function commitSavedEventFeedFilters(
  ...
  const result = commitSavedEventFeedFilterLibrary(
    savedEventFeedFilterLibrary,
    proposed,
    (serialized) => dataLayerStorage.setItem(SAVED_EVENT_FEED_FILTER_STORAGE_KEY, serialized),
    failureFeedback,
  );
...
function persistSchemaAndRuleLibraries(nextSchemas: readonly SchemaDefinition[], nextRules: readonly ReusableSchemaRule[]): void {
  ...
  try {
    ...
    if (previousSchemas === null) dataLayerStorage.removeItem(SCHEMA_LIBRARY_STORAGE_KEY);
    else dataLayerStorage.setItem(SCHEMA_LIBRARY_STORAGE_KEY, previousSchemas);
    if (previousRules === null) dataLayerStorage.removeItem(SCHEMA_RULE_STORAGE_KEY);
    else dataLayerStorage.setItem(SCHEMA_RULE_STORAGE_KEY, previousRules);
    throw error;
  }
}
          ...
          }),
          {
            save:async (report, options) => {
              ...
              if (result.added) {
                ...
                if (selected.manualFlowContext) liveFlowTestingUi.attachDefect(
                  selected.manualFlowContext.selectedStepId,
                  selected.manualFlowContext.eventId,
                  defect.id,
                );
                renderDefects();
                renderLiveObserver();
...
async function reviewEventTemplateTestCaseCreation(template:EditableEventTemplate):Promise<void>{
  ...
  heading.textContent=`Create Test case from ${template.name}`;summary.textContent="Choose the destination project, then review the matching named Event and input-guidance schema. The Library template remains unchanged.";projectLabel.append("Project",projectSelect);eventLabel.append("Matching Event",eventSelect);schemaLabel.append("Input-guidance schema",schemaSelect);confirm.type=cancel.type=repair.type="button";confirm.textContent="Create Event validation Test case";cancel.textContent="Cancel";repair.textContent="Open Specification Studio to create, adopt, or select missing relationships";repair.hidden=true;
  const library=projectLibraryUi.library(),projects=Object.values(library.projects);for(const record of projects)projectSelect.append(new Option(record.state.project.name,record.state.project.id));projectSelect.value=library.activeProjectId??projects[0]?.state.project.id??"";
  const refresh=async():Promise<void>=>{confirm.disabled=true;repair.hidden=true;eventSelect.replaceChildren(new Option("Loading matching Events…",""));schemaSelect.replaceChildren(new Option("Loading schema relationships…",""));try{await durableProjectRuntime.ensureProject(projectSelect.value);await durableProjectRuntime.settled();const loaded=await durableProjectRuntime.repository.loadProject(projectSelect.value),events=loaded.state.project.collections.events.filter(({eventName,sourceId})=>eventName===template.eventName&&sourceId===template.sourceId),profiles=loaded.state.project.collections.profiles.filter((profile)=>profile.id===template.schemaId||profile.sourceIdentity===template.schemaId||(profile.canonicalSchema as {source?:{identity?:string}}|undefined)?.source?.identity===template.schemaId);eventSelect.replaceChildren(new Option(events.length?"Choose reviewed Event":"No matching Event",""),...events.map(({id,name})=>new Option(name,id)));schemaSelect.replaceChildren(new Option(profiles.length?"Choose reviewed input guidance":"No matching project schema",""),...profiles.map(({id,name,revision,canonicalSchema})=>new Option(`${name} revision ${String((canonicalSchema as {revision?:number}|undefined)?.revision??revision??1)}`,id)));if(events.length===1)eventSelect.value=events[0]!.id;if(profiles.length===1)schemaSelect.value=profiles[0]!.id;confirm.disabled=!(events.length&&profiles.length);repair.hidden=!confirm.disabled;summary.textContent=confirm.disabled?`No mapping was guessed. ${events.length?"":"Create or select a matching Event. "}${profiles.length?"":"Adopt or select the attached schema in this project."}`:`${template.name} revision ${template.version} will be copied as typed input with exact source provenance.`;}catch(error){summary.textContent=error instanceof Error?error.message:String(error);repair.hidden=false;}};
  projectSelect.addEventListener("change",()=>void refresh());eventSelect.addEventListener("change",()=>{confirm.disabled=!(eventSelect.value&&schemaSelect.value);});schemaSelect.addEventListener("change",()=>{confirm.disabled=!(eventSelect.value&&schemaSelect.value);});
  ...
  confirm.addEventListener("click",()=>{void(async()=>{confirm.disabled=true;try{const loaded=await durableProjectRuntime.repository.loadProject(projectSelect.value),profile=loaded.state.project.collections.profiles.find(({id})=>id===schemaSelect.value);if(!eventSelect.value||!profile)throw new Error("Review a matching Event and input-guidance schema before creating the Test case.");const testCase=createGuidedTestCase({name:template.name,testType:"event-validation",eventId:eventSelect.value,source:{kind:"event-library",id:template.id,revision:String(template.version),eventId:eventSelect.value,destination:template.destination,payload:structuredClone(template.payload) as Record<string,unknown>,schemaId:profile.id,schemaRevision:String((profile.canonicalSchema as {revision?:number}|undefined)?.revision??profile.revision??1)},id:(kind)=>`${kind}:${crypto.randomUUID()}`}),next=transactProject(loaded.state,`Create Test case from Event Library ${template.name}`,(project)=>({...project,collections:{...project.collections,fixtures:[...project.collections.fixtures,testCase]}})),result=commitCanonicalProjectState(projectStorage,next,{expectedRevision:loaded.draftSequence,pendingLabel:`Create Test case from ${template.name}`,base:loaded.state});if(result.status==="conflict")throw new Error("The selected project changed; review the Test case mapping again.");if(projectLibraryUi.library().activeProjectId!==projectSelect.value)projectLibraryUi.activate(projectSelect.value);projectLibraryUi.captureActiveProject(next,result.revision);const routed=recordProjectNavigation(projectLibraryUi.library(),projectSelect.value,{kind:"fixtures",id:testCase.id});projectStorage.setItem(PROJECT_LIBRARY_STORAGE_KEY,serializeProjectLibrary(routed));await durableProjectRuntime.settled();dialog.close();dialog.remove();globalThis.open(`specification-builder.html?project=${encodeURIComponent(projectSelect.value)}&kind=fixtures&entity=${encodeURIComponent(testCase.id)}&source=event-library`,"_blank");}catch(error){confirm.disabled=false;summary.textContent=error instanceof Error?error.message:String(error);}})();});
  cancel.addEventListener("click",()=>{dialog.close();dialog.remove();});dialog.append(heading,summary,projectLabel,eventLabel,schemaLabel,confirm,repair,cancel);document.body.append(dialog);dialog.showModal();heading.tabIndex=-1;heading.focus();await refresh();
}
...
function reviewSavedSchemaAdoption(schema:SchemaDefinition,trigger:HTMLButtonElement):void{
  const library=projectLibraryUi.library(),records=Object.values(library.projects);if(!records.length){if(schemaResult)schemaResult.textContent="Create a project before adopting a saved schema.";return;}
  ...
  heading.textContent="Review saved-schema adoption";picker.setAttribute("aria-label","Project for saved schema adoption");for(const record of records)picker.append(new Option(record.state.project.name,record.state.project.id));picker.value=library.activeProjectId??records[0]!.state.project.id;const describe=()=>{const record=library.projects[picker.value]!;summary.textContent=`${schema.name} revision ${schema.version} → project ${record.state.project.name}. Selection changes nothing; confirmation safely activates the chosen project and creates one project-owned canonical Draft with source lineage.`;};describe();picker.addEventListener("change",describe);confirm.type=cancel.type="button";confirm.textContent="Confirm project switch and adoption";cancel.textContent="Cancel";
  confirm.addEventListener("click",()=>{void (async()=>{confirm.disabled=true;try{const projectId=picker.value;summary.textContent=`Loading the exact current Saved Draft before adopting ${schema.name}…`;await durableProjectRuntime.settled();await durableProjectRuntime.ensureProject(projectId);await durableProjectRuntime.settled();let currentLibrary=projectLibraryUi.library();if(currentLibrary.activeProjectId!==projectId){projectLibraryUi.activate(projectId);await durableProjectRuntime.settled();await durableProjectRuntime.ensureProject(projectId);await durableProjectRuntime.settled();currentLibrary=projectLibraryUi.library();}const loaded=await durableProjectRuntime.repository.loadProject(projectId),record=currentLibrary.projects[projectId];if(!record||record.state.project.placeholder)throw new Error(`Project ${projectId} did not load its durable body.`);if(loaded.draftSequence!==record.revision)throw new Error(`Project ${record.state.project.name} changed while adoption was loading; review adoption again.`);const next=adoptSavedSchema(loaded.state,schema as unknown as Parameters<typeof adoptSavedSchema>[1]),result=commitCanonicalProjectState(projectStorage,next,{expectedRevision:loaded.draftSequence,pendingLabel:`Adopt saved schema ${schema.name}`,base:loaded.state});if(result.status==="conflict")throw new Error(`Project changed in a newer Saved Draft; review adoption again.`);projectLibraryUi.captureActiveProject(next,result.revision);await durableProjectRuntime.settled();dialog.close();dialog.remove();if(schemaResult)schemaResult.textContent=`Adopted ${schema.name} revision ${schema.version} into ${loaded.state.project.name} with source lineage.`;renderSchemas();}catch(error){confirm.disabled=false;summary.textContent=error instanceof Error?error.message:String(error);}})();});
  cancel.addEventListener("click",()=>{dialog.close();dialog.remove();trigger.focus({preventScroll:true});});dialog.append(heading,summary,picker,confirm,cancel);document.body.append(dialog);dialog.showModal();confirm.focus({preventScroll:true});
}

let schemaTreeProjectId:string|undefined,schemaTreeExpandedKeys=new Set<string>(),schemaTreeInvokingReference:string|undefined,schemaTreeRestoringScroll=false,schemaTreePendingScroll:number|undefined;
...
function restorePendingSchemaTreeScroll():void{
  ...
  queueMicrotask(()=>{schemaTreeScrollOwner.scrollTop=scrollTop;schemaTreePendingScroll=undefined;requestAnimationFrame(()=>{schemaTreeRestoringScroll=false;});});
}
function persistSchemaTreeView(projectId:string):void{
  saveSchemaRelationshipTreeView(schemaTreeStorage,projectId,{query:schemaSearch?.value??"",category:(schemaCategoryFilter?.value??"All") as SchemaRelationshipCategory,expandedKeys:[...schemaTreeExpandedKeys],scrollTop:schemaTreeScrollOwner?.scrollTop??0});
}
function renderSchemas(): void {
  const project=restoreCanonicalProjectState(projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY)),projectId=project?.project.id??"no-project",fullTree=projectSchemaRelationshipTree(project,schemas),allNodes=(nodes:readonly SchemaRelationshipTreeNode[]):SchemaRelationshipTreeNode[]=>nodes.flatMap((node)=>[node,...allNodes(node.children)]),validNodes=allNodes(fullTree),validKeys=new Set(validNodes.map(({key})=>key));
  ...
  const query=schemaSearch?.value??"",category=(schemaCategoryFilter?.value??"All") as SchemaRelationshipCategory,filtered=filterSchemaRelationshipTree(fullTree,{query,category}),rows:HTMLElement[]=[];
  const savedRow=(node:SchemaRelationshipTreeNode,level:number):HTMLLIElement|undefined=>{
    const schema=schemas.find(({id})=>`saved:${id}`===node.targetKey);if(!schema)return;
    ...
    item.dataset.schemaEntryKey=node.targetKey;item.dataset.schemaReferenceKey=node.key;item.dataset.schemaRole=node.role;item.setAttribute("role","treeitem");item.setAttribute("aria-level",String(level));item.setAttribute("aria-selected",String(compactCanonicalEditor?.key===node.targetKey));item.textContent=schema.published===false?`${schema.name} · role Saved schema · path ${node.relationshipPath} · revision ${schema.version} · Draft · ${pending} pending changes. `:`${schema.name} · current revision ${schema.version} · role Saved schema · path ${node.relationshipPath} · saved · ${pending} pending draft changes · ${history} historical revisions · ${schema.assignments.map((assignment)=>`${assignment.sourceId}/${assignment.eventName}/${assignment.target}`).join(", ")||"unassigned"}. `;
    revise.type=duplicate.type=adopt.type=build.type=exportCurrent.type=reportMissing.type=remove.type="button";revise.textContent="Edit working draft";duplicate.textContent="Duplicate";adopt.textContent="Add saved schema to project";build.textContent="Build documentation table";exportCurrent.textContent="Export";reportMissing.textContent="Report missing event";remove.textContent="Delete";revise.setAttribute("aria-label",`Edit ${schema.name}; ${node.relationshipPath}`);
    revise.addEventListener("click",()=>{schemaTreeInvokingReference=node.key;openSavedSchemaInUnifiedEditor(schema);});duplicate.addEventListener("click",()=>{schemas=[...schemas,duplicateSchemaRevision(schema,schema.version,schemas)];persistSchemaLibrary();renderSchemas();});adopt.addEventListener("click",()=>reviewSavedSchemaAdoption(schema,adopt));build.addEventListener("click",()=>openSchemaSpecification(schema,`published:${schema.version}`,build));exportCurrent.addEventListener("click",()=>openSchemaExportChoices(exportCurrent,schema));reportMissing.addEventListener("click",()=>openMissingEventBuilder("schema row actions",schema.id));remove.addEventListener("click",()=>{const children=schemas.filter((candidate)=>candidate.parentSchemaId===schema.id);if(children.length){if(schemaResult)schemaResult.textContent=`Cannot delete ${schema.name}: it is the parent of ${children.map(({name})=>name).join(", ")}.`;return;}pendingSchemaDeletion=schema;if(schemaDeleteReviewSummary)schemaDeleteReviewSummary.textContent=`${schema.name} v${schema.version} and its assignments will be removed.`;if(schemaDeleteReview){schemaDeleteReview.hidden=false;schemaDeleteReview.showModal();}});
  ...
  const visit=(node:SchemaRelationshipTreeNode,level:number):void=>{
    ...
    const item=document.createElement("li");item.dataset.schemaReferenceKey=node.key;item.setAttribute("role","treeitem");item.setAttribute("aria-level",String(level));item.style.setProperty("--schema-tree-level",String(level));
    if(node.targetKey){
      const open=document.createElement("button"),studio=document.createElement("button");item.dataset.schemaEntryKey=node.targetKey;item.dataset.schemaRole=node.role;item.setAttribute("aria-selected",String(compactCanonicalEditor?.key===node.targetKey));item.textContent=`${node.name} · role ${node.role} · path ${node.relationshipPath}. `;open.type=studio.type="button";open.textContent="Open schema";open.setAttribute("aria-label",`Open ${node.name}; ${node.relationshipPath}`);studio.textContent="Open schema in Specification Studio";studio.setAttribute("aria-label",`Open ${node.name} in Specification Studio; ${node.relationshipPath}`);open.addEventListener("click",()=>{schemaTreeInvokingReference=node.key;const retainedScroll=compactCanonicalEditor?.key===node.targetKey?schemaDetail?.scrollTop:undefined;openContributorInUnifiedEditor(node.targetKey!);if(schemaDetail&&retainedScroll!==undefined)schemaDetail.scrollTop=retainedScroll;renderSchemas();});studio.addEventListener("click",()=>{if(!project)return;const separator=node.targetKey!.indexOf(":"),kind=node.targetKey!.slice(0,separator),entityId=node.targetKey!.slice(separator+1);globalThis.open(`specification-builder.html?project=${encodeURIComponent(project.project.id)}&kind=${encodeURIComponent(kind)}&entity=${encodeURIComponent(entityId)}`,"_blank");});item.append(open,studio);
    }else{
      const toggle=document.createElement("button"),expanded=node.expanded||schemaTreeExpandedKeys.has(node.key);item.dataset.schemaGroup=node.name;item.setAttribute("aria-expanded",String(expanded));item.setAttribute("aria-selected","false");toggle.type="button";toggle.textContent=node.name;toggle.setAttribute("aria-label",`${expanded?"Collapse":"Expand"} ${node.relationshipPath}`);toggle.addEventListener("click",()=>{if(expanded)schemaTreeExpandedKeys.delete(node.key);else schemaTreeExpandedKeys.add(node.key);persistSchemaTreeView(projectId);renderSchemas();});item.append(toggle);
    }
    rows.push(item);
  ...
  if(!project){const item=document.createElement("li"),open=document.createElement("button"),create=document.createElement("button");item.setAttribute("role","status");item.textContent="No active project. Open a project to see relationship-derived contributors. ";open.type=create.type="button";open.textContent="Open project";create.textContent="Create project";open.addEventListener("click",()=>showDataLayerView("Projects",true));create.addEventListener("click",()=>{showDataLayerView("Projects",true);document.querySelector<HTMLButtonElement>("#create-library-project")?.click();});item.append(open,create);rows.push(item);}
  const resultCount=rows.filter(({dataset})=>Boolean(dataset.schemaEntryKey)).length;if(schemaEmptyState)schemaEmptyState.hidden=resultCount>0;if(schemaCount){schemaCount.textContent=`${resultCount} relationship-tree results`;schemaCount.setAttribute("aria-label",`${resultCount} schema relationship-tree results`);}schemaList?.replaceChildren(...rows);
}
...
function openLocalRulePromotionReview(propertyPath: string, sourceRuleId: string, trigger: HTMLButtonElement): void {
  ...
  localRulePromotionDialog.open({
    ...
    confirm(selected) {
      const previousRules = structuredClone(reusableSchemaRules);
      const previousRuleStorage = dataLayerStorage.getItem(SCHEMA_RULE_STORAGE_KEY);
      ...
      persistLocalRulePromotion(dataLayerStorage, {
        ...
        ...(storedSchema ? { schemaValue:serializeSchemaLibrary(nextSchemas) } : {}),
      ...
      if (!storedSchema) return afterClose;
      return new Promise<void | (() => void)>((resolve, reject) => {
        ...
        const transaction = {
          ...
          reject(error: unknown) {
            if (settled) return;
            settled = true;
            reusableSchemaRules = structuredClone(previousRules);
            if (previousRuleStorage === null) dataLayerStorage.removeItem(SCHEMA_RULE_STORAGE_KEY);
            else dataLayerStorage.setItem(SCHEMA_RULE_STORAGE_KEY, previousRuleStorage);
        ...
        void durableProjectRuntime.settled("schema").then(transaction.complete, (error) => {
          if (!durableProjectRuntime.failedSchemaSave()) transaction.reject(error);
        });
      });
    },
  });
...
function renderSchemaDraft(): void {
  ...
  const legacyPropertyRegion=schemaPropertyTree.parentElement;
  const compactTableMode=compactDocument?.view==="table";
  if(legacyPropertyRegion)legacyPropertyRegion.hidden=compactTableMode;
  if(!compactCanonicalEditor)schemaEditor?.querySelector("#compact-canonical-table-editor")?.remove();
  ...
  if(schemaPropertyInteractionReturn?.schemaId===draft.id)selectedSchemaPropertyPath=schemaPropertyInteractionReturn.path;
  ...
  const lifecycleStatus = storedSchema?.published === false
  ...
  if (status) status.textContent = compactCanonicalEditor&&compactDocument
    ? compactCanonicalEditor.key.startsWith("saved:")
      ? `${lifecycleStatus} · ${compactCanonicalEditor.label} · Schema revision ${compactDocument.revision}`
  ...
  const propertyTreeFocusLabel=document.activeElement instanceof HTMLElement&&schemaPropertyTree.contains(document.activeElement)?document.activeElement.getAttribute("aria-label"):undefined;
  const propertyItems = propertyRows.map((propertyRow) => {
    ...
    if(compactPropertyActions){compactPropertyActions.type="button";compactPropertyActions.textContent="⋯";compactPropertyActions.setAttribute("aria-label",`Property actions for ${persistedPath}`);compactPropertyActions.addEventListener("click",()=>openCompactCanonicalPropertyActions(persistedPath,compactPropertyActions));}
    const compactProvenanceNames=compactNode?.provenance.map(({contributorName})=>contributorName?.trim()).filter((name):name is string=>Boolean(name))??[];
    if(compactProvenanceNames.length)metadata.textContent+=` · ${compactProvenanceNames.join(" · ")}`;
    if(compactNode)item.dataset.propertyId=compactNode.id;
    if(compactCanonicalEditor)item.addEventListener("click",(event)=>{
      if(!compactDocument||event.target!==item&&event.target!==label)return;
      if(compactNode){void dispatchCompactCanonicalCommand({kind:"select",baseRevision:compactDocument.revision,propertyId:compactNode.id});return;}
      selectedSchemaPropertyPath=path;renderSchemaDraft();
    });
    if(compactCanonicalEditor&&!selectedRow){item.append(label,metadata,...(compactPropertyActions?[compactPropertyActions]:[]));return item;}
    ...
    const typeControls = renderSchemaPropertyTypeEditor({
      ...
      ...(inherited ? { inheritedOwner:{
        ...
        open:() => { if (owningSchema) { schemaDraft = schemaEditorDraft(owningSchema); renderSchemaDraft(); } },
      } } : {}),
      confirm:(edit) => {
    ...
    const count = document.createElement("span"); count.className="schema-property-active-rule-count"; count.textContent = ` (${attached.filter((rule) => rule.enabled !== false).length} active rules)`;
    ...
    const editCanonicalRules=compactCanonicalEditor?document.createElement("button"):undefined;
    if(editCanonicalRules){editCanonicalRules.type="button";editCanonicalRules.textContent="Edit canonical rules";editCanonicalRules.className="schema-property-canonical-rules";editCanonicalRules.setAttribute("aria-label",`Edit canonical rules for ${path}`);editCanonicalRules.addEventListener("click",()=>openCompactCanonicalRuleEditor(path,editCanonicalRules));}
    ...
    if(compactPresence&&compactCanonicalEditor){
      ...
      if(canonicalNode)mountCanonicalPredicateEditor({host:predicateControls,document:canonical,...(canonicalNode.presence.condition?{condition:canonicalNode.presence.condition}:{}),label:`Nested conditional presence for ${persistedPath}`,saveLabel:"Save conditional presence",excludePropertyId:canonicalNode.id,onSave:(condition)=>{if(!mode.value.endsWith("-when"))return;dispatchPresence({mode:mode.value as "required-when"|"forbidden-when",condition});},...(canonicalNode.presence.condition?{onClear:()=>dispatchPresence({mode:mode.value.startsWith("forbidden")?"forbidden":"required"})}:{})});
    ...
    for (const rule of attached) {
      ...
      const edit = document.createElement("button"); edit.type = "button"; edit.textContent = "Edit"; edit.className = "schema-attached-rule-edit";
      ...
      if(compactCanonicalEditor&&compactDocument&&compactNode&&canonicalRule){
        ...
        mountCanonicalPredicateEditor({host:predicateEditor,document:compactDocument,...(canonicalRule.condition?{condition:canonicalRule.condition}:{}),label:`Nested rule predicate for ${rule.id}`,saveLabel:"Save nested rule predicate",onSave:(condition)=>{const latest=compactCanonicalEditor?.load(),latestNode=latest?.nodes[compactNode.id];if(!latest||!latestNode)return;dispatchCompactCanonicalCommand({kind:"set",baseRevision:latest.revision,propertyId:latestNode.id,patch:{rules:latestNode.rules.map((candidate)=>candidate.id===canonicalRule.id?{...candidate,condition}:candidate)}});},...(canonicalRule.condition?{onClear:()=>{const latest=compactCanonicalEditor?.load(),latestNode=latest?.nodes[compactNode.id];if(!latest||!latestNode)return;dispatchCompactCanonicalCommand({kind:"set",baseRevision:latest.revision,propertyId:latestNode.id,patch:{rules:latestNode.rules.map((candidate)=>{if(candidate.id!==canonicalRule.id)return candidate;const{condition:_condition,...withoutCondition}=candidate;return withoutCondition;})}});}}:{})});
        row.append(predicateEditor);
      }
      view.append(row);
    }
    if(compactCanonicalEditor){item.append(label,metadata,...(compactPropertyActions?[compactPropertyActions]:[]));if(selectedRow){const stackedDetail=document.createElement("section"),facets=compactDocument&&compactNode?document.createElement("p"):undefined;stackedDetail.setAttribute("aria-label",`Stacked property detail for ${persistedPath}`);if(facets&&compactDocument&&compactNode){facets.className="compact-canonical-facet-summary";facets.setAttribute("aria-label",`Canonical facets for ${persistedPath}`);facets.textContent=compactCanonicalFacetText(compactDocument,compactNode);}stackedDetail.append(...(facets?[facets]:[]),typeAction,typeEditor,...(compactPresence?[compactPresence]:[]),documentationSection,...(compactLifecycle?[compactLifecycle]:[]),count,...(addContainerChild?[addContainerChild]:[]),add,...(editCanonicalRules?[editCanonicalRules]:[]),...(addSpecificIndex?[addSpecificIndex]:[]),copyProperty,removeProperty,view);item.append(stackedDetail);}}
  ...
  schemaPropertyTree.replaceChildren(...(compactTableMode?[]:roots));
  ...
  if(schemaPropertyInteractionReturn)restoreSchemaPropertyInteractionReturn();
  else if(propertyTreeFocusLabel)Array.from(schemaPropertyTree.querySelectorAll<HTMLElement>("[aria-label]")).find((control)=>control.getAttribute("aria-label")===propertyTreeFocusLabel)?.focus({preventScroll:true});
...
function openNewSchemaEditor(): void {
  if (compactCanonicalEditor) closeCompactCanonicalEditor();
...
function persistSchemaLibrary(): void {
  const previousSchemas=dataLayerStorage.getItem(SCHEMA_LIBRARY_STORAGE_KEY),schemaValue=serializeSchemaLibrary(schemas);
  ...
  try{if(schemaValue!==previousSchemas)dataLayerStorage.setItem(SCHEMA_LIBRARY_STORAGE_KEY,schemaValue);}
...
function compactCanonicalFacetText(canonical:CanonicalSchemaDocument,node:CanonicalSchemaDocument["nodes"][string]):string{
...
function beginCompactCanonicalPendingHistory(projectId:string,editorKey:string,label:string,history:CompactCanonicalPageHistory):CompactCanonicalHistoryTransitionIdentity{
  const identity={operationId:`compact-history:${crypto.randomUUID()}`,projectId,editorKey};compactCanonicalHistoryState=beginCompactCanonicalHistoryTransition(compactCanonicalHistoryState,{...identity,history});compactCanonicalPendingHistoryLabel=label;return identity;
}

function completeCompactCanonicalPendingHistory(identity:CompactCanonicalHistoryTransitionIdentity):void{
  compactCanonicalHistoryState=completeCompactCanonicalHistoryTransition(compactCanonicalHistoryState,identity);if(!compactCanonicalHistoryState.pending)compactCanonicalPendingHistoryLabel=undefined;
}

function rejectCompactCanonicalPendingHistory(identity:CompactCanonicalHistoryTransitionIdentity):void{
  compactCanonicalHistoryState=rejectCompactCanonicalHistoryTransition(compactCanonicalHistoryState,identity);if(!compactCanonicalHistoryState.pending)compactCanonicalPendingHistoryLabel=undefined;
}

function compactCanonicalPendingHistoryFor(projectId:string,label:string):CompactCanonicalHistoryTransitionIdentity|undefined{
  const pending=compactCanonicalHistoryState.pending;return pending&&pending.projectId===projectId&&compactCanonicalPendingHistoryLabel===label?{operationId:pending.operationId,projectId:pending.projectId,editorKey:pending.editorKey}:undefined;
}

function compactCanonicalSemanticUnresolved(ownedProjection?:CompactCanonicalProjectionPersistenceRequest):boolean{return Boolean(compactCanonicalSettlementPending||compactCanonicalProjectionWorker||(compactCanonicalProjectionRequest&&compactCanonicalProjectionRequest!==ownedProjection)||compactCanonicalHistoryState.pending||compactCanonicalPendingCommand||durableProjectRuntime.failedSchemaSave());}

function compactCanonicalProjectionQueueUnavailable(adapter:CompactCanonicalEditorAdapter):boolean{
  const ownedWorker=compactCanonicalProjectionWorker?.adapter===adapter;
  return Boolean(
    durableProjectRuntime.failedSchemaSave()
    ||compactCanonicalHistoryState.pending
    ||compactCanonicalPendingCommand
    ||compactCanonicalProjectionWorker&&!ownedWorker
    ||compactCanonicalProjectionRequest&&compactCanonicalProjectionRequest.adapter!==adapter
    ||compactCanonicalSettlementPending&&!ownedWorker
  );
}

type CompactCanonicalCommand=Parameters<CompactCanonicalEditorAdapter["dispatch"]>[0];
type CompactCanonicalCommandResult=ReturnType<CompactCanonicalEditorAdapter["dispatch"]>;
type CompactCanonicalDispatch={accepted:boolean;result:CompactCanonicalCommandResult;completion:Promise<boolean>};

function blockedCompactCanonicalCommand(adapter:CompactCanonicalEditorAdapter,command:CompactCanonicalCommand,message:string):CompactCanonicalCommandResult{
  return{status:"conflict",document:adapter.load(),...("propertyId"in command?{propertyId:command.propertyId}:{}),message};
}

function beginCompactCanonicalCommand(command:CompactCanonicalCommand,render:"immediate"|"deferred"="immediate",ownedProjection?:CompactCanonicalProjectionPersistenceRequest):CompactCanonicalDispatch|undefined{
  const adapter=compactCanonicalEditor;if(!adapter)return;
  const refresh=()=>{if(compactCanonicalEditor!==adapter)return;if(render==="deferred")queueMicrotask(()=>{if(compactCanonicalEditor===adapter)renderCompactCanonicalEditor();});else renderCompactCanonicalEditor();};
  const commandPolicy=compactCanonicalCommandPolicy(command.kind,compactCanonicalSemanticUnresolved(ownedProjection));
  const preservePendingPresentation= !commandPolicy.semantic&&Boolean(compactCanonicalSettlementPending||compactCanonicalHistoryState.pending||compactCanonicalPendingCommand);
  if(!commandPolicy.allowed){const message="Resolve the current durable schema save through Retry or Reject before making another semantic change. Export may preserve a recovery copy first.";compactCanonicalCommandFeedback=message;refresh();return{accepted:false,result:blockedCompactCanonicalCommand(adapter,command,message),completion:Promise.resolve(false)};}
  const base=compactCanonicalRevisionSnapshots.get(command.baseRevision);
  let result:CompactCanonicalCommandResult;
  try{result=adapter.dispatch(command);}catch(error){const message=`The canonical command was not applied; the current Draft remains unchanged. ${error instanceof Error?error.message:String(error)}`;compactCanonicalCommandFeedback=message;refresh();return{accepted:false,result:blockedCompactCanonicalCommand(adapter,command,message),completion:Promise.resolve(false)};}
  if(result.status==="conflict"){
    ...
    refresh();return{accepted:false,result,completion:Promise.resolve(false)};
  }
  ...
  const outcome=canonicalCommandOutcome(command,result,base??result.document),settlement=commandPolicy.settles&&adapter.settle&&(adapter.settles?.(command)??true)?++compactCanonicalSettlementSequence:undefined;
  if(commandPolicy.settles)compactCanonicalSettlementPending=Boolean(settlement);
  const settlementTarget=adapter.settlementTarget??"durable Saved Draft";
  const savingFeedback=settlement?`Saving ${outcome.replace(/^(Saved|Rebased) /,"").replace(/\.$/,"")} to the ${settlementTarget}…`:undefined;
  if(!preservePendingPresentation)compactCanonicalCommandFeedback=savingFeedback??outcome;
  refresh();
  const completion=settlement&&adapter.settle?(async()=>{try{await adapter.settle!();adapter.onSettlementCommitted?.();const committedFeedback=`${outcome} ${settlementTarget.charAt(0).toUpperCase()}${settlementTarget.slice(1)} committed.`;if(compactCanonicalEditor!==adapter){if(compactCanonicalSettlementSequence===settlement)compactCanonicalSettlementPending=false;if(compactCanonicalEditor)renderCompactCanonicalEditor();return true;}if(compactCanonicalSettlementSequence!==settlement){if(!compactCanonicalSettlementPending&&compactCanonicalCommandFeedback===savingFeedback){compactCanonicalCommandFeedback=committedFeedback;renderCompactCanonicalEditor();}return true;}compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=committedFeedback;renderCompactCanonicalEditor();return true;}catch(error){if(compactCanonicalEditor!==adapter){if(compactCanonicalSettlementSequence===settlement)compactCanonicalSettlementPending=false;if(compactCanonicalEditor)renderCompactCanonicalEditor();return false;}if(compactCanonicalSettlementSequence!==settlement)return false;compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=`Not saved to the ${settlementTarget}; the exact change remains available for Retry, Reject, or export. ${error instanceof Error?error.message:String(error)}`;renderCompactCanonicalEditor();return false;}})():Promise.resolve(true);
  return{accepted:true,result,completion};
}

async function dispatchCompactCanonicalCommand(command:CompactCanonicalCommand,ownedProjection?:CompactCanonicalProjectionPersistenceRequest):Promise<boolean>{
  const dispatch=beginCompactCanonicalCommand(command,"immediate",ownedProjection);return dispatch?dispatch.accepted&&await dispatch.completion:false;
}

function beginCompactCanonicalProjectionPersistence(adapter:CompactCanonicalEditorAdapter,projection:SchemaDefinition,change?:string):Promise<boolean>{
  ...
  compactCanonicalSettlementSequence+=1;compactCanonicalSettlementPending=true;
  const target=adapter.settlementTarget??"durable Saved Draft",label=change??"schema metadata";
  if(durableProjectRuntime.failedSchemaSave()){
    compactCanonicalCommandFeedback=`${label} is waiting. Resolve the failed ${target} save through Retry or Reject first.`;renderCompactCanonicalContext();if(schemaEditor)schemaEditor.setAttribute("aria-busy","true");if(saveSchemaButton)saveSchemaButton.disabled=true;return Promise.resolve(false);
  }
  compactCanonicalCommandFeedback=`Saving ${label} to the ${target}…`;renderCompactCanonicalContext();if(schemaEditor)schemaEditor.setAttribute("aria-busy","true");if(saveSchemaButton)saveSchemaButton.disabled=true;
  if(compactCanonicalProjectionWorker?.adapter===adapter)return compactCanonicalProjectionWorker.promise;
  const worker={adapter,promise:Promise.resolve(false)};compactCanonicalProjectionWorker=worker;worker.promise=(async()=>{
    let lastLabel=label,committed=false;
    try{
      while(compactCanonicalEditor===adapter){
        const request=compactCanonicalProjectionRequest;if(!request||request.adapter!==adapter)break;
        compactCanonicalProjectionRequest=undefined;lastLabel=request.change??"schema metadata";
        let changed=false;
        try{changed=adapter.persistProjection!(structuredClone(request.projection),request.change);}catch(error){
          if(durableProjectRuntime.failedSchemaSave()&&compactCanonicalEditor===adapter&&!compactCanonicalProjectionRequest)compactCanonicalProjectionRequest=request;
          if(compactCanonicalEditor===adapter){compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=`The Saved Schema metadata was not queued; the current Draft remains unchanged. ${error instanceof Error?error.message:String(error)}`;renderCompactCanonicalEditor();}return false;
        }
        if(!changed)continue;
        try{await adapter.settle?.();adapter.onSettlementCommitted?.();committed=true;}catch(error){
          if(durableProjectRuntime.failedSchemaSave()&&compactCanonicalEditor===adapter&&!compactCanonicalProjectionRequest)compactCanonicalProjectionRequest=request;
          if(compactCanonicalEditor===adapter){compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=`Not saved to the ${target}; the exact metadata edit remains available for Retry, Reject, or export. ${error instanceof Error?error.message:String(error)}`;renderCompactCanonicalEditor();}return false;
        }
        if(durableProjectRuntime.failedSchemaSave())return false;
      }
      if(compactCanonicalEditor===adapter){compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=committed?`Saved ${lastLabel} to the ${target}.`:`${lastLabel.charAt(0).toUpperCase()}${lastLabel.slice(1)} is already current in the ${target}.`;renderCompactCanonicalEditor();}
      return true;
    }finally{if(compactCanonicalProjectionWorker===worker){compactCanonicalProjectionWorker=undefined;compactCanonicalSettlementPending=false;if(compactCanonicalEditor)renderCompactCanonicalEditor();}}
  })();return worker.promise;
}

async function persistCompactCanonicalProjection(adapter:CompactCanonicalEditorAdapter,projection:SchemaDefinition,change?:string):Promise<boolean>{
  const base=adapter.load(),commands=canonicalCommandsFromCompactProjection(base,projection,(kind)=>`${kind}:${crypto.randomUUID()}`),existingRequest=compactCanonicalProjectionRequest?.adapter===adapter?compactCanonicalProjectionRequest:undefined,stagedProjection:CompactCanonicalProjectionPersistenceRequest|undefined=commands.length?(existingRequest??{adapter,projection:structuredClone(projection),...(change?{change}:{})}):undefined;
  if(stagedProjection)compactCanonicalProjectionRequest=stagedProjection;
  if(adapter.stageProjectionCommand){
    const projectionBase=structuredClone(adapter.load());
    for(const command of commands){if(compactCanonicalEditor!==adapter)return false;const base=adapter.load(),currentCommand={...command,baseRevision:base.revision} as CompactCanonicalCommand;let result:CompactCanonicalCommandResult;try{result=adapter.stageProjectionCommand(currentCommand);}catch(error){adapter.restoreStagedProjection?.(projectionBase);if(compactCanonicalProjectionRequest===stagedProjection)compactCanonicalProjectionRequest=undefined;compactCanonicalCommandFeedback=`The canonical projection was not staged; the durable Draft remains unchanged. ${error instanceof Error?error.message:String(error)}`;renderCompactCanonicalEditor();return false;}if(result.status==="conflict"||result.status==="confirmation-required"){adapter.restoreStagedProjection?.(projectionBase);compactCanonicalPendingCommand=currentCommand;compactCanonicalPendingBase=structuredClone(base);compactCanonicalReviewVisible=false;compactCanonicalCommandFeedback=result.status==="conflict"?`${result.message} Compare this command with the latest Draft, retry the complete projection, or reject the local edit.`:`${result.impact} Compare this command with the latest Draft before retrying or reject the complete projection.`;renderCompactCanonicalEditor();return false;}}
    if(compactCanonicalEditor!==adapter)return false;
    const persisted=await beginCompactCanonicalProjectionPersistence(adapter,projection,change);if(!persisted&&!durableProjectRuntime.failedSchemaSave()){adapter.restoreStagedProjection?.(projectionBase);if(compactCanonicalProjectionRequest===stagedProjection)compactCanonicalProjectionRequest=undefined;if(compactCanonicalEditor===adapter)renderCompactCanonicalEditor();}return persisted;
  }
  for(const command of commands){if(compactCanonicalEditor!==adapter)return false;const currentCommand={...command,baseRevision:adapter.load().revision} as CompactCanonicalCommand,dispatch=beginCompactCanonicalCommand(currentCommand,"immediate",stagedProjection);if(!dispatch?.accepted){if(!compactCanonicalPendingCommand&&!durableProjectRuntime.failedSchemaSave()&&compactCanonicalProjectionRequest===stagedProjection)compactCanonicalProjectionRequest=undefined;return false;}if(!await dispatch.completion){if(!compactCanonicalPendingCommand&&!durableProjectRuntime.failedSchemaSave()&&compactCanonicalProjectionRequest===stagedProjection)compactCanonicalProjectionRequest=undefined;return false;}if(compactCanonicalEditor!==adapter)return false;}
  if(compactCanonicalEditor!==adapter)return false;
  if(!adapter.persistProjection){if(compactCanonicalProjectionRequest?.adapter===adapter)compactCanonicalProjectionRequest=undefined;if(compactCanonicalEditor===adapter)renderCompactCanonicalEditor();return true;}
  return beginCompactCanonicalProjectionPersistence(adapter,projection,commands.length?undefined:change);
}

function discardCompactCanonicalProjectionPersistence(adapter?:CompactCanonicalEditorAdapter):void{
  ...
  compactCanonicalSettlementSequence+=1;compactCanonicalSettlementPending=false;
}

function resumeCompactCanonicalProjectionPersistence(adapter:CompactCanonicalEditorAdapter):Promise<boolean>{
  const request=compactCanonicalProjectionRequest;if(!request||request.adapter!==adapter){compactCanonicalSettlementPending=false;return Promise.resolve(true);}
  schemaDraft=structuredClone(request.projection);renderSchemaDraft();return persistCompactCanonicalProjection(adapter,request.projection,request.change);
}
...
function renderCompactCanonicalContext():void{
  ...
  sidePanelLayeredProfileEditorHost.replaceChildren();sidePanelLayeredProfileEditorHost.hidden=true;for(const context of Array.from(schemaEditor.querySelectorAll('[aria-label="Compact canonical schema context"]')))context.remove();
  ...
  for(const action of adapter.actions??[]){const button=document.createElement("button");button.type="button";button.textContent=action.label;button.disabled=action.label==="Publish schema"&&compactCanonicalSemanticUnresolved();button.addEventListener("click",action.run);actions.append(button);}
  feedback.setAttribute("aria-label","Compact canonical command result");feedback.textContent=compactCanonicalCommandFeedback;
  if(compactCanonicalPendingCommand){const compare=document.createElement("button"),retry=document.createElement("button"),reject=document.createElement("button");compare.type=retry.type=reject.type="button";compare.textContent="Compare latest property";retry.textContent="Retry local edit";reject.textContent="Reject local edit";compare.addEventListener("click",()=>{if(!compactCanonicalPendingCommand||!compactCanonicalEditor)return;compactCanonicalReviewVisible=true;const latest=compactCanonicalEditor.load(),scope=compactCanonicalCommandScope(compactCanonicalPendingCommand,latest),versionLabel=latest.source?.provenance==="project-composed-effective"?"composed content version":"Schema revision";compactCanonicalCommandFeedback=`Comparing ${compactCanonicalPendingCommand.kind} for ${scope}: command base ${versionLabel} ${compactCanonicalPendingCommand.baseRevision} versus latest ${versionLabel} ${latest.revision}. Overlapping saves are checked against opaque durable Draft concurrency metadata.`;renderCompactCanonicalContext();});retry.addEventListener("click",()=>{const pending=compactCanonicalPendingCommand,editor=compactCanonicalEditor,request=compactCanonicalProjectionRequest;if(!pending||!editor)return;compactCanonicalPendingCommand=undefined;if(request?.adapter===editor&&editor.stageProjectionCommand){void resumeCompactCanonicalProjectionPersistence(editor);return;}void(async()=>{if(!await dispatchCompactCanonicalCommand({...pending,baseRevision:editor.load().revision},request?.adapter===editor?request:undefined)){if(!compactCanonicalPendingCommand&&compactCanonicalEditor===editor)compactCanonicalPendingCommand=pending;renderCompactCanonicalEditor();return;}if(request?.adapter===editor&&compactCanonicalProjectionRequest===request)await resumeCompactCanonicalProjectionPersistence(editor);})();});reject.addEventListener("click",()=>{const editor=compactCanonicalEditor;if(compactCanonicalPendingCommand&&"propertyId"in compactCanonicalPendingCommand&&compactCanonicalPresenceDraft?.propertyId===compactCanonicalPendingCommand.propertyId)compactCanonicalPresenceDraft=undefined;if(editor&&compactCanonicalProjectionRequest?.adapter===editor)compactCanonicalProjectionRequest=undefined;compactCanonicalPendingCommand=undefined;compactCanonicalPendingBase=undefined;compactCanonicalReviewVisible=false;compactCanonicalCommandFeedback="Local edit rejected; the latest Draft remains unchanged.";renderCompactCanonicalEditor();});actions.append(compare,retry,reject);}
  context.append(identity,actions,feedback);
  if(compactCanonicalPendingCommand&&compactCanonicalReviewVisible){const latest=adapter.load(),comparison=document.createElement("section"),heading=document.createElement("h3"),base=document.createElement("pre"),pending=document.createElement("pre"),current=document.createElement("pre"),propertyId="propertyId"in compactCanonicalPendingCommand?compactCanonicalPendingCommand.propertyId:undefined,versionLabel=latest.source?.provenance==="project-composed-effective"?"Composed content version":"Schema revision";comparison.setAttribute("aria-label","Command-scoped canonical comparison");heading.textContent=`Command-scoped comparison · ${compactCanonicalCommandScope(compactCanonicalPendingCommand,latest)}`;base.textContent=`Base Saved Draft · ${versionLabel} ${compactCanonicalPendingCommand.baseRevision}\n${JSON.stringify(propertyId?compactCanonicalPendingBase?.nodes[propertyId]:compactCanonicalPendingBase?.rootIds??[],null,2)}`;pending.textContent=`Local ${compactCanonicalPendingCommand.kind} command\n${JSON.stringify(compactCanonicalPendingCommand,(key,value)=>key==="baseRevision"?undefined:typeof value==="function"?"[identity factory]":value,2)}`;current.textContent=`Newer Saved Draft · ${versionLabel} ${latest.revision}\n${JSON.stringify(propertyId?latest.nodes[propertyId]:latest.rootIds,null,2)}`;comparison.append(heading,base,pending,current);context.append(comparison);}
...
function renderCompactCanonicalEditor():void{
  ...
  if(schemaPropertyInteractionReturn?.schemaId===schemaDraft.id)selectedSchemaPropertyPath=schemaPropertyInteractionReturn.path;
  else if(compactCanonicalReopenSelection)selectedSchemaPropertyPath=compactCanonicalReopenSelection;
  ...
  if(schemaEditor){
    let tableHost=schemaEditor.querySelector<HTMLElement>("#compact-canonical-table-editor");
    if(!tableHost){tableHost=document.createElement("section");tableHost.id="compact-canonical-table-editor";schemaEditor.append(tableHost);}
    const dispatchMounted=(command:CompactCanonicalCommand):CompactCanonicalCommandResult=>beginCompactCanonicalCommand(command,"deferred")?.result??blockedCompactCanonicalCommand(adapter,command,"The canonical editor is no longer available.");
    mountCanonicalSchemaEditor({host:tableHost,surface:"Side panel",conceptSuggestions:()=>{const state=restoreCanonicalProjectState(projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY));return state?projectCanonicalConcepts(state):[];},load:adapter.load,id:(kind)=>`${kind}:${crypto.randomUUID()}`,dispatch:dispatchMounted,...(adapter.onUndo?{onUndo:adapter.onUndo}:{}),...(adapter.onRedo?{onRedo:adapter.onRedo}:{})});
    const tableMode=adapter.load().view==="table";
    tableHost.hidden=!tableMode;
    if(!tableMode)for(const duplicate of Array.from(tableHost.querySelectorAll("[data-property-id]")))duplicate.remove();
    const legacyPropertyRegion=schemaPropertyTree.parentElement,tableControl=Array.from(tableHost.querySelectorAll("button")).find(({textContent})=>textContent?.trim()==="Table"),treeControl=Array.from(tableHost.querySelectorAll("button")).find(({textContent})=>textContent?.trim()==="Tree");
    tableControl?.addEventListener("click",()=>{const current=adapter.load();beginCompactCanonicalCommand({kind:"view",baseRevision:current.revision,view:"table"},"deferred");tableHost!.hidden=false;if(legacyPropertyRegion)legacyPropertyRegion.hidden=true;schemaPropertyTree.replaceChildren();},{once:true});
    treeControl?.addEventListener("click",()=>{const current=adapter.load();beginCompactCanonicalCommand({kind:"view",baseRevision:current.revision,view:"tree"},"deferred");renderCompactCanonicalEditor();},{once:true});
  }
  if(schemaEditor){const unavailable=compactCanonicalSemanticUnresolved();schemaEditor.setAttribute("aria-busy",String(unavailable));if(saveSchemaButton&&adapter.key.startsWith("saved:"))saveSchemaButton.disabled=saveSchemaButton.disabled||unavailable;}
  restoreSchemaPropertyInteractionReturn();
}
...
function openCompactCanonicalEditor(adapter:CompactCanonicalEditorAdapter):void{
  ...
  discardCompactCanonicalProjectionPersistence(compactCanonicalEditor);compactCanonicalEditor=adapter;compactCanonicalPendingCommand=undefined;compactCanonicalPendingBase=undefined;compactCanonicalReviewVisible=false;compactCanonicalPresenceDraft=undefined;compactCanonicalRevisionSnapshots.clear();compactCanonicalCommandFeedback="";renderCompactCanonicalEditor();
...
function closeCompactCanonicalEditor():void{
  discardCompactCanonicalProjectionPersistence(compactCanonicalEditor);compactCanonicalEditor=undefined;schemaDraft=undefined;savedCanonicalDocument=undefined;
  ...
  renderSchemas();
  if(schemaTreeInvokingReference){document.querySelector<HTMLElement>(`#schema-list [data-schema-reference-key="${CSS.escape(schemaTreeInvokingReference)}"] button`)?.focus({preventScroll:true});schemaTreeInvokingReference=undefined;}
}
...
function persistSchemaEditorDraft(change?: string): void {
  ...
  if(compactCanonicalEditor){
    const adapter=compactCanonicalEditor;
    if(compactCanonicalProjectionQueueUnavailable(adapter)){schemaDraft=compactCanonicalProjectionRequest?.adapter===adapter?structuredClone(compactCanonicalProjectionRequest.projection):compactCanonicalProjection(adapter);compactCanonicalCommandFeedback="Wait for the current durable schema save or resolve its Retry/Reject recovery before making another semantic change.";renderSchemaDraft();renderCompactCanonicalContext();return;}
    const projection=structuredClone(schemaDraft);
    void persistCompactCanonicalProjection(adapter,projection,change).then((persisted)=>{
      if(persisted)renderSchemas();
      if(persisted||!durableProjectRuntime.failedSchemaSave())finishSchemaPropertyInteractionReturn();
    });
    return;
  }
  ...
  const changes: Partial<Pick<SchemaWorkingDraft, "document" | "assignments" | "attachedRules" | "parentSchemaId" | "inheritedRuleOverrides" | "documentation" | "canonicalSchema">> = {
    ...
    canonicalSchema:schemaDraft.canonicalSchema,
  };
...
function openContributorInUnifiedEditor(key:string):void{
  let pendingDurableRevision:number|undefined;
  schemaDraft=undefined;savedCanonicalDocument=undefined;const state=restoreCanonicalProjectState(projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY)),selection=state&&resolveSidePanelSchemaContributor(state,key);
  ...
  let contributorUi:{selectedPropertyId?:string;view?:"tree"|"table"}={},compatibilityCanonical:CanonicalSchemaDocument|undefined,acknowledgedMigration:CanonicalSchemaDocument|undefined,migration:ReturnType<typeof migrateLegacyProfile>|undefined,migrationSavePending=false,migrationStatus="",pendingMutationHistoryIdentity:CompactCanonicalHistoryTransitionIdentity|undefined,pendingStepHistoryIdentity:CompactCanonicalHistoryTransitionIdentity|undefined;const contributorSnapshots=new Map<string,{beforeIdentity:string;before:ProjectEntity;after:ProjectEntity}>(),isComposed=(selected:SidePanelContributorSelection):boolean=>selected.scope!=="Shared Profile";const withContributorUi=(document:CanonicalSchemaDocument)=>{const selectedPropertyId=contributorUi.selectedPropertyId&&document.nodes[contributorUi.selectedPropertyId]?contributorUi.selectedPropertyId:document.selectedPropertyId;return{...document,...(selectedPropertyId?{selectedPropertyId}:{}),...(contributorUi.view?{view:contributorUi.view}:{})};};const documentFor=(live:ProjectState,selected:SidePanelContributorSelection)=>{if(isComposed(selected))return withContributorUi(composedCanonicalSchema(live,selected.entity,selected.scope as "Page"|"Property Set"|"Event"|"Event-occurrence"|"Flow Page-instance",selected.flowId));if(migration)return withContributorUi(compatibilityCanonical??migration.document);const stored=selected.entity.canonicalSchema as CanonicalSchemaDocument|undefined;if(stored)return withContributorUi(stored);if(acknowledgedMigration)return withContributorUi(acknowledgedMigration);if(!compatibilityCanonical){if(hasLegacySchemaRepresentation(selected.entity)){migration=migrateLegacyProfile(selected.entity,{id:(kind)=>`${kind}:${crypto.randomUUID()}`});compatibilityCanonical=migration.document;}else compatibilityCanonical=createCanonicalSchema({id:`canonical:${selected.entity.id}`,contributorId:selected.entity.id,contributorName:selected.entity.name});}return withContributorUi(compatibilityCanonical);};
  const renderMigrationReview=(host:HTMLElement)=>{if(!migration)return;const review=document.createElement("section"),heading=document.createElement("h3"),summary=document.createElement("p"),list=document.createElement("ul"),actions=document.createElement("p"),cancel=document.createElement("button"),confirm=document.createElement("button"),status=document.createElement("p"),sourceDefinitions=Object.values(migration.document.nodes).reduce((count,node)=>count+node.provenance.length,0),deduplicated=sourceDefinitions-Object.keys(migration.document.nodes).length;review.setAttribute("aria-label","Canonical schema migration review");review.dataset.migrationState=migration.conflicts.length?"unresolved":"ready";heading.textContent="Review legacy schema migration";summary.textContent=`${Object.keys(migration.document.nodes).length} properties mapped · ${deduplicated} repeated semantic entries deduplicated with all source provenance · rules, documentation, examples, constraints, and provenance mapped · ${migration.conflicts.length} unresolved path/facet conflicts · canonical mutations are blocked and every legacy field remains unchanged until confirmation.`;for(const conflict of migration.conflicts){const item=document.createElement("li"),resolution=document.createElement("select");item.dataset.migrationPath=conflict.path;item.dataset.migrationFacet=conflict.facet;item.textContent=`Path ${conflict.path} · ${conflict.facet} facet · ${conflict.message}. `;resolution.append(new Option("Choose a source value",""),...conflict.choices.map((choice)=>new Option(choice.label,choice.id)));resolution.setAttribute("aria-label",`Resolve migration conflict ${conflict.path} ${conflict.facet} facet`);resolution.addEventListener("change",()=>{if(!resolution.value||!migration||migrationSavePending)return;migration=resolveCanonicalMigrationConflict(migration,conflict.id,resolution.value);compatibilityCanonical=migration.document;migrationStatus="";renderCompactCanonicalEditor();});item.append(resolution);list.append(item);}cancel.type="button";cancel.textContent="Cancel migration";cancel.disabled=migrationSavePending;cancel.addEventListener("click",()=>{if(migrationSavePending)return;migration=undefined;compatibilityCanonical=undefined;migrationStatus="";closeCompactCanonicalEditor();if(schemaDetailEmpty)schemaDetailEmpty.hidden=false;});confirm.type="button";confirm.textContent="Confirm canonical migration";confirm.disabled=Boolean(migration.conflicts.length);confirm.addEventListener("click",()=>{void (async()=>{if(!migration||migration.conflicts.length)return;const pendingMigration=migration;migrationSavePending=true;confirm.disabled=true;cancel.disabled=true;try{const live=restoreCanonicalProjectState(projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY))!,selected=resolveSidePanelSchemaContributor(live,key)!,label=`Migrate legacy schema for ${selected.entity.name}`,failed=durableProjectRuntime.failedSave();let acknowledged:ProjectState|undefined;if(failed){if(failed.command.label!==label)throw new Error(`A different unsaved command must be resolved first: ${failed.command.label}.`);await durableProjectRuntime.retryFailedSave();acknowledged=(await durableProjectRuntime.repository.loadProject(live.project.id)).state;}else{commitUnifiedContributorState(writeUnifiedContributorCanonical(live,selected,pendingMigration.document),label);await durableProjectRuntime.settled("project");acknowledged=restoreCanonicalProjectState(projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY))??undefined;}if(!acknowledged||!canonicalMigrationDurablyAcknowledged(acknowledged,key,pendingMigration.document))throw new Error("The canonical migration has not been acknowledged by the settled durable project projection.");migrationSavePending=false;migration=undefined;compatibilityCanonical=undefined;acknowledgedMigration=pendingMigration.document;migrationStatus="Canonical migration committed.";renderCompactCanonicalEditor();}catch(error){confirm.disabled=false;migration=pendingMigration;compatibilityCanonical=pendingMigration.document;migrationStatus=`Migration was not committed; the durable Saved Draft remains unchanged and this migration stays pending. ${error instanceof Error?error.message:String(error)}`;renderCompactCanonicalEditor();}})();});status.setAttribute("role","status");status.textContent=migrationStatus;actions.append(cancel,confirm);review.append(heading,summary,list,actions,status);host.append(review);};
  ...
  const contributorState=(live:ProjectState,selected:SidePanelContributorSelection,document:CanonicalSchemaDocument)=>selected.collectionKind==="pages"||selected.collectionKind==="propertySets"?saveComposedCanonicalDocument(live,selected.collectionKind,selected.entity.id,document):selected.collectionKind==="events"?saveComposedEventCanonicalDocument(live,selected.entity.id,document):selected.scope==="Flow Page-instance"?saveFlowPageInstanceCanonicalDocument(live,selected.flowId!,selected.entity.id,document):selected.scope==="Event-occurrence"?saveEventOccurrenceCanonicalDocument(live,selected.flowId!,selected.entity.id,document):writeUnifiedContributorCanonical(live,selected,document),contributorPolicyState=(live:ProjectState,selected:SidePanelContributorSelection,value:boolean)=>selected.collectionKind==="pages"||selected.collectionKind==="propertySets"||selected.collectionKind==="events"?saveComposedEntitySchemaPolicy(live,selected.collectionKind,selected.entity.id,value):selected.scope==="Flow Page-instance"?saveFlowContributorSchemaPolicy(live,selected.flowId!,"pageFrames",selected.entity.id,value):selected.scope==="Event-occurrence"?saveFlowContributorSchemaPolicy(live,selected.flowId!,"occurrences",selected.entity.id,value):live,historyKey=(projectId:string)=>compactCanonicalHistoryKey(projectId,key);
  const stepHistory=async(direction:"Undo"|"Redo"):Promise<void>=>{if(compactCanonicalSettlementPending||compactCanonicalHistoryState.pending){compactCanonicalCommandFeedback=`${direction} is waiting for the current durable schema save…`;renderCompactCanonicalContext();try{if(pendingDurableRevision!==undefined)await settleUnifiedContributorRevision(state.project.id,pendingDurableRevision);if(pendingMutationHistoryIdentity&&pendingDurableRevision!==undefined){completeCompactCanonicalPendingHistory(pendingMutationHistoryIdentity);pendingMutationHistoryIdentity=undefined;compactCanonicalSettlementPending=false;}}catch(error){compactCanonicalCommandFeedback=`Resolve the current durable schema save through Retry or Reject before changing history. Export may preserve a recovery copy first. ${error instanceof Error?error.message:String(error)}`;renderCompactCanonicalContext();return;}if(compactCanonicalSettlementPending||compactCanonicalHistoryState.pending){compactCanonicalCommandFeedback="Resolve the current durable schema save through Retry or Reject before changing history. Export may preserve a recovery copy first.";renderCompactCanonicalContext();return;}}const adapter=compactCanonicalEditor,durableLive=await durableProjectRuntime.repository.loadProject(state.project.id),live=durableLive.state,selected=resolveSidePanelSchemaContributor(live,key);if(!adapter||!selected){compactCanonicalCommandFeedback=`${direction} blocked: this contributor is no longer available.`;renderCompactCanonicalContext();return;}const current=documentFor(live,selected),scopedHistoryKey=historyKey(live.project.id);if(migration){migrationBlocked(current);return;}const step=direction==="Undo"?prepareCompactCanonicalUndo(compactCanonicalHistoryState.history,scopedHistoryKey,current):prepareCompactCanonicalRedo(compactCanonicalHistoryState.history,scopedHistoryKey,current);if(step.status!=="ready"){compactCanonicalCommandFeedback=step.message;renderCompactCanonicalContext();return;}++compactCanonicalSettlementSequence;const label=`${direction} canonical schema edit in ${selected.entity.name??selected.entity.id}`;pendingStepHistoryIdentity=beginCompactCanonicalPendingHistory(live.project.id,key,label,step.history);compactCanonicalSettlementPending=true;compactCanonicalCommandFeedback=`${direction} is restoring ${selected.entity.name??selected.entity.id} in the durable Saved Draft…`;renderCompactCanonicalContext();if(schemaEditor){schemaEditor.setAttribute("aria-busy","true");for(const control of Array.from(schemaEditor.querySelectorAll<HTMLButtonElement|HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement>("button,input,select,textarea")))control.disabled=true;}try{const currentIdentity=compactCanonicalDocumentIdentity(current),snapshot=direction==="Undo"?contributorSnapshots.get(currentIdentity):[...contributorSnapshots.values()].find(({beforeIdentity})=>beforeIdentity===currentIdentity),entity=direction==="Undo"?snapshot?.before:snapshot?.after;if(selected.scope==="Flow Page-instance"&&!entity)throw new Error(`${direction} cannot locate the exact reviewed Flow Page-instance contributor snapshot.`);const nextState=entity?transplantUnifiedContributorEntity(live,selected,entity,label):contributorState(live,selected,step.document);pendingDurableRevision=commitUnifiedContributorState(nextState,label).revision;await settleUnifiedContributorRevision(live.project.id,pendingDurableRevision);completeCompactCanonicalPendingHistory(pendingStepHistoryIdentity);pendingStepHistoryIdentity=undefined;compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=`${direction} restored ${selected.entity.name??selected.entity.id} in the Saved Draft.`;renderCompactCanonicalEditor();}catch(error){if(pendingStepHistoryIdentity&&durableProjectRuntime.failedSave()?.command.label!==label){rejectCompactCanonicalPendingHistory(pendingStepHistoryIdentity);pendingStepHistoryIdentity=undefined;}compactCanonicalSettlementPending=false;compactCanonicalCommandFeedback=`${direction} was not saved to the durable Saved Draft; the history action remains available for Retry, Reject, or export. ${error instanceof Error?error.message:String(error)}`;renderCompactCanonicalContext();if(schemaEditor)schemaEditor.setAttribute("aria-busy","false");}};
  openCompactCanonicalEditor({
    key,
    label:`${selection.entity.name} · Role ${selection.scope} · scope ${selection.scope} · provenance ${selection.flowId?`${selection.flowId} · `:""}${selection.entity.id}`,
    ...
    dispatch:(command)=>{const live=restoreCanonicalProjectState(projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY))!,selected=resolveSidePanelSchemaContributor(live,key)!,document=documentFor(live,selected);if(migration)return migrationBlocked(document);if(isComposed(selected)&&command.baseRevision!==document.revision)return{status:"conflict",document,message:`This contributor changed from Schema revision ${command.baseRevision} to ${document.revision}; reload the latest Draft before retrying.`};const result=applyCanonicalCommand(document,command),mutation=command.kind!=="select"&&command.kind!=="view";if(result.status==="applied"||result.status==="rebased"){if(!mutation)contributorUi={...(result.document.selectedPropertyId?{selectedPropertyId:result.document.selectedPropertyId}:{}),view:result.document.view};if(mutation){const label=`${command.kind} ${isComposed(selected)?"sparse canonical facet":"canonical property"} in ${selected.entity.name}`,nextState=command.kind==="policy"&&isComposed(selected)?contributorPolicyState(live,selected,command.onlyDefinedFields):contributorState(live,selected,result.document),nextSelection=resolveSidePanelSchemaContributor(nextState,key),historyAfter=isComposed(selected)&&nextSelection?documentFor(nextState,nextSelection):result.document,nextHistory=recordCompactCanonicalMutation(compactCanonicalHistoryState.history,historyKey(live.project.id),document,historyAfter);if(nextSelection)contributorSnapshots.set(compactCanonicalDocumentIdentity(historyAfter),{beforeIdentity:compactCanonicalDocumentIdentity(document),before:structuredClone(selected.entity),after:structuredClone(nextSelection.entity)});pendingMutationHistoryIdentity=beginCompactCanonicalPendingHistory(live.project.id,key,label,nextHistory);try{pendingDurableRevision=commitUnifiedContributorState(nextState,label).revision;}catch(error){rejectCompactCanonicalPendingHistory(pendingMutationHistoryIdentity);pendingMutationHistoryIdentity=undefined;throw error;}}}return result;},
    settle:()=>pendingDurableRevision===undefined?durableProjectRuntime.settled("project"):settleUnifiedContributorRevision(state.project.id,pendingDurableRevision),
    settles:(command)=>command.kind!=="select"&&command.kind!=="view",
    settlementTarget:"durable Saved Draft",
    onSettlementCommitted:()=>{if(pendingMutationHistoryIdentity){completeCompactCanonicalPendingHistory(pendingMutationHistoryIdentity);pendingMutationHistoryIdentity=undefined;}},
...
function writeUnifiedContributorCanonical(state:ProjectState,selection:SidePanelContributorSelection,canonical:CanonicalSchemaDocument):ProjectState{
  const replace=(candidate:ProjectEntity):ProjectEntity=>{if(candidate.id!==selection.entity.id)return candidate;const next:ProjectEntity={...candidate,canonicalSchema:canonical,compiledTargetsStale:true};if(selection.collectionKind==="profiles")next.requirements=[];else delete next.requirements;delete next.structuredSchema;delete next.structuredDraft;delete next.schemaConstraints;return next;};
...
function transplantUnifiedContributorEntity(state:ProjectState,selection:SidePanelContributorSelection,entity:ProjectEntity,label:string):ProjectState{
  return transactProject(state,label,(project)=>{if(selection.collectionKind){const collection=project.collections[selection.collectionKind] as ProjectEntity[];return{...project,collections:{...project.collections,[selection.collectionKind]:collection.map((candidate)=>candidate.id===selection.entity.id?structuredClone(entity):candidate)}} as typeof project;}const graphs=project.documentationFlowGraphs as Record<string,{pageFrames?:ProjectEntity[];occurrences?:ProjectEntity[]}>,graph=graphs[selection.flowId!]!,field=selection.scope==="Flow Page-instance"?"pageFrames":"occurrences",entries=graph[field]??[];return{...project,documentationFlowGraphs:{...graphs,[selection.flowId!]:{...graph,[field]:entries.map((candidate)=>candidate.id===selection.entity.id?structuredClone(entity):candidate)}}};});
}
...
async function settleUnifiedContributorRevision(projectId:string,revision:number):Promise<void>{
  ...
  if(!label)throw new Error(`Saved Draft command ${revision} has no durable acknowledgement identity.`);
  await durableProjectRuntime.settledProjectCommand(projectId,label);
}

function commitUnifiedContributorState(next:ProjectState,label:string):Extract<ReturnType<typeof commitCanonicalProjectState>,{status:"committed"}>{
...
function persistSavedProjectionMetadata(schemaId:string,projection:SchemaDefinition,change?:string):boolean{
  const stored=schemas.find(({id})=>id===schemaId);if(!stored)throw new Error("The saved schema is unavailable.");
  const renamed=proposeSchemaWorkingDraftName(stored,projection.name),canonical=savedCanonicalDocument??renamed.workingDraft?.canonicalSchema,updated=updateSchemaWorkingDraft(renamed,{document:projection.document,assignments:projection.assignments,attachedRules:projection.attachedRules,parentSchemaId:projection.parentSchemaId,inheritedRuleOverrides:projection.inheritedRuleOverrides,documentation:projection.documentation,...(canonical?{canonicalSchema:{...canonical,contributorName:projection.name}}:{})},change==="schema name"?undefined:change);
  if(JSON.stringify(updated)===JSON.stringify(stored))return false;
  const previousSchemas=schemas;schemas=schemas.map((candidate)=>candidate.id===updated.id?updated:candidate);
  try{persistSchemaLibrary();}catch(error){schemas=previousSchemas;throw error;}
  if(canonical)savedCanonicalDocument={...canonical,contributorName:projection.name};
  return true;
}

function savedCompactCanonicalProjection(schemaId:string,canonical:CanonicalSchemaDocument):SchemaDefinition{
  ...
  const outer=schemaEditorDraft(stored),projected=savedSchemaFromCanonical({...outer,name:canonical.contributorName},canonical),{canonicalSchema:_canonicalSchema,...projection}=projected;
  return projection;
}
...
function openSavedSchemaInUnifiedEditor(schema:SchemaDefinition):void{
  ...
  openCompactCanonicalEditor({key:`saved:${schema.id}`,label:`${schema.name} · Saved schema working draft`,load:()=>savedCanonicalDocument!,projection:(canonical)=>savedCompactCanonicalProjection(schema.id,canonical),dispatch:(command)=>{const result=applyCanonicalCommand(savedCanonicalDocument!,command);if(result.status==="applied"||result.status==="rebased"){if(command.kind==="select"||command.kind==="view")savedCanonicalDocument=result.document;else persistSavedCanonicalResult(schema.id,result.document,`${command.kind} canonical property`);}return result;},stageProjectionCommand:(command)=>{const result=applyCanonicalCommand(savedCanonicalDocument!,command);if(result.status==="applied"||result.status==="rebased")savedCanonicalDocument=result.document;return result;},restoreStagedProjection:(canonical)=>{savedCanonicalDocument=structuredClone(canonical);},persistProjection:(projection,change)=>persistSavedProjectionMetadata(schema.id,projection,change),settles:(command)=>command.kind!=="select"&&command.kind!=="view",settle:()=>durableProjectRuntime.settled("schema"),settlementTarget:"durable Saved Schema Library",actions:[{label:"Publish schema",run:()=>saveSchemaButton?.click()},{label:"Close editor",run:closeCompactCanonicalEditor}]});
}
...
function openSchemaPropertyCopyReview(path:string, trigger:HTMLButtonElement):void {
  ...
  const restoreSourcePosition=()=>{if(schemaDraft?.id!==stored.id)return;if(schemaEditor)schemaEditor.scrollTop=editorScroll;schemaPropertyTree.scrollTop=treeScroll;Array.from(schemaPropertyTree.querySelectorAll<HTMLButtonElement>("button")).find((button)=>button.getAttribute("aria-label")===`Copy ${path} to another schema`)?.focus({preventScroll:true});};
  renderSchemaPropertyCopyReview(schemaPropertyCopyDialog,{
    ...
    onApply:(transaction)=>{
      ...
      void durableProjectRuntime.settled("schema").then(restoreSourcePosition).catch(()=>{});
    },
    onClose:restoreSourcePosition,
  });
}
...
schemaManualPropertyForm.addEventListener("submit", (event) => {
  ...
  if(compactCanonicalEditor&&pendingManualPropertyCanonicalBase){
    ...
    void (async()=>{for(const[index,retained]of commands.entries()){if(!compactCanonicalEditor)return;const command=index===0?retained:{...retained,baseRevision:compactCanonicalEditor.load().revision};if(!await dispatchCompactCanonicalCommand(command))return;}selectedSchemaPropertyPath=path;closeManualPropertyForm(false);renderCompactCanonicalEditor();focusSchemaPropertyRule(path);})();return;
  }
...
function finishGuidedValidationSave(result: PublishedGuidedValidation): void {
  ...
  const recovery=document.querySelector<HTMLDialogElement>("#durable-storage-recovery");
  if(recovery?.open&&guidedPropertyReturn)recovery.addEventListener("close",()=>requestAnimationFrame(restoreGuidedPropertyReturn),{once:true});
}
...
function persistPublishedGuidedValidation(result: PublishedGuidedValidation): Promise<void> {
  ...
  const previousSchemas = dataLayerStorage.getItem(SCHEMA_LIBRARY_STORAGE_KEY);
  const previousRules = dataLayerStorage.getItem(SCHEMA_RULE_STORAGE_KEY);
  const previousReusableRules = structuredClone(reusableSchemaRules);
  let resolveCompletion!:()=>void,rejectCompletion!:(error:unknown)=>void,settled=false,paused=false;
  const completion=new Promise<void>((resolve,reject)=>{resolveCompletion=resolve;rejectCompletion=reject;});
  const restoreRules=()=>{if(previousRules===null)dataLayerStorage.removeItem(SCHEMA_RULE_STORAGE_KEY);else dataLayerStorage.setItem(SCHEMA_RULE_STORAGE_KEY,previousRules);reusableSchemaRules=structuredClone(previousReusableRules);renderSchemaWorkflowRows();};
  const transaction = {
    schemaId:schema.id,
    pause() {
    ...
    complete() {
      if(settled)return;
      settled=true;
      if(paused){dataLayerStorage.setItem(SCHEMA_RULE_STORAGE_KEY,JSON.stringify(nextRules));reusableSchemaRules=structuredClone(nextRules);}
      schemas=restoreSchemaLibrary(projectStorage.getItem(SCHEMA_LIBRARY_STORAGE_KEY));
      renderSchemas();
      renderSchemaWorkflowRows();
  ...
  try {
    ...
    transaction.reject(error);return completion;
  }
  schemas = nextSchemas;
  reusableSchemaRules = nextRules;
  renderSchemas();
  renderSchemaWorkflowRows();
  void durableProjectRuntime.settled("schema").then(transaction.complete,(error)=>{if(!durableProjectRuntime.failedSchemaSave())transaction.reject(error);});
  return completion;
}
...
function openCompactCanonicalPropertyActions(path:string,trigger:HTMLButtonElement):boolean {
  ...
  if(!adapter||!documentModel||!original||!owner)return false;
  ...
  const ownership=focusedOwnershipState(focusedCanonicalOwnershipInput(original));let ownershipSession:Parameters<typeof activateFocusedOwnershipSection>[0]=ownership.session;
  ...
  const mount=(layers:HTMLElement[],focusLabel?:string)=>{
    const sequence=focusedPropertyLayerSequence(activeSection,...(layers.length===3?["review" as const]:[]));
  ...
  const buildSection=(section:"definition"|"rules"|"structure")=>{
...
function captureSchemaPropertyInteractionReturn(path:string,trigger:HTMLButtonElement):void{
  if(!schemaDraft)return;
  schemaPropertyInteractionReturn={
    schemaId:schemaDraft.id,
    path,
    triggerLabel:trigger.getAttribute("aria-label")??`Add rule for ${path}`,
    editorScroll:schemaEditor?.scrollTop??0,
    treeScroll:schemaPropertyTree.scrollTop,
    detailScroll:schemaDetail?.scrollTop??0,
  };
}

function restoreSchemaPropertyInteractionReturn():void{
  const pending=schemaPropertyInteractionReturn;
  if(!pending||schemaDraft?.id!==pending.schemaId||schemaPropertyRulePicker.open)return;
  selectedSchemaPropertyPath=pending.path;
  if(schemaEditor)schemaEditor.scrollTop=pending.editorScroll;
  schemaPropertyTree.scrollTop=pending.treeScroll;
  if(schemaDetail)schemaDetail.scrollTop=pending.detailScroll;
  Array.from(schemaPropertyTree.querySelectorAll<HTMLButtonElement>("button"))
    .find((button)=>button.getAttribute("aria-label")===pending.triggerLabel)
    ?.focus({preventScroll:true});
}

function finishSchemaPropertyInteractionReturn():void{
  const pending=schemaPropertyInteractionReturn;
  if(!pending)return;
  restoreSchemaPropertyInteractionReturn();
  if(!compactCanonicalEditor){schemaPropertyInteractionReturn=undefined;return;}
  if(schemaEditor)schemaEditor.setAttribute("aria-busy","true");
  requestAnimationFrame(()=>{
    if(schemaPropertyInteractionReturn!==pending)return;
    if(schemaDraft?.id===pending.schemaId){selectedSchemaPropertyPath=pending.path;renderSchemaDraft();restoreSchemaPropertyInteractionReturn();}
    let stableRenderSequence=-1;
    const finishAfterStableRender=()=>{
      if(schemaPropertyInteractionReturn!==pending)return;
      restoreSchemaPropertyInteractionReturn();
      if(stableRenderSequence!==schemaPropertyRenderSequence){stableRenderSequence=schemaPropertyRenderSequence;requestAnimationFrame(finishAfterStableRender);return;}
      schemaPropertyInteractionReturn=undefined;
      if(schemaEditor)schemaEditor.setAttribute("aria-busy",String(compactCanonicalSemanticUnresolved()));
    };
    finishAfterStableRender();
  });
}

function closeSchemaPropertyRulePickerInternal(preserveInteraction:boolean): void {
  const triggerLabel=schemaRulePickerTrigger?.ariaLabel;
  if (schemaPropertyRulePicker.open) schemaPropertyRulePicker.close();
  const currentTrigger=schemaRulePickerTrigger?.isConnected
    ? schemaRulePickerTrigger
    : Array.from(schemaPropertyTree.querySelectorAll<HTMLButtonElement>("button")).find(({ariaLabel})=>ariaLabel===triggerLabel);
  ...
  if(preserveInteraction)restoreSchemaPropertyInteractionReturn();
  else{restoreSchemaPropertyInteractionReturn();schemaPropertyInteractionReturn=undefined;}
}

function closeSchemaPropertyRulePicker():void{closeSchemaPropertyRulePickerInternal(false);}
...
function configuredRuleInput(control: ReturnType<typeof ruleConfigurationControls>[number], configuration: RuleConfiguration, updateValidation: () => void): HTMLElement {
  ...
  if (input instanceof HTMLInputElement) {
    ...
    Object.assign(document.createElement("option"), { value:"", textContent:control.key === "comparison" ? "Choose a comparison" : "Choose a value" }),
    ...(control.choices ?? ["true", "false"]).map((value) => Object.assign(document.createElement("option"), { value, textContent:value })),
  );
  ...
  input.addEventListener(input instanceof HTMLSelectElement ? "change" : "input", () => {
    ...
    else if (control.key === "comparison") configuration.comparison = input.value as RuleConfiguration["comparison"];
    else if (control.key === "limit") configuration.limit = input.value;
    updateValidation();
  });
...
function createConfiguredSchemaRule(path: string, configuration: RuleConfiguration): void {
  ...
  const id = editingAttachedLocalRule?.id ?? (configuration.saveReusable ? `rule:${crypto.randomUUID()}` : `local-rule:${crypto.randomUUID()}`);
  const name = editingAttachedLocalRule?.name ?? (configuration.saveReusable ? configuration.reusableName.trim() : `${configuration.ruleType} for ${path}`);
  const rule: ReusableSchemaRule = {
    id, name, kind:configuration.ruleType, version:editingAttachedLocalRule?.version ?? 1, enabled:configuration.enabled,
  ...
  if (editingAttachedLocalRule) {
    const edited = editingAttachedLocalRule;
    editingAttachedLocalRule = undefined;
    const stored = schemas.find((candidate) => candidate.id === schemaDraft?.id);
    const nextRule = {
      id:edited.id,
      name:edited.name ?? rule.name,
      version:edited.version,
      propertyPath:canonicalRulePropertyPath(path),
      operator:rule.operator!,
      ...(rule.parameters !== undefined ? { parameters:rule.parameters } : {}),
      ...(rule.allowedValues !== undefined ? { allowedValues:structuredClone(rule.allowedValues) } : {}),
      ...
      ...(rule.conditionGroup !== undefined ? { conditionGroup:structuredClone(rule.conditionGroup) } : {}),
      enabled:rule.enabled !== false,
    };
    if (stored) {
      const updated = saveLocalRuleEdit(stored, { propertyPath:path, ruleId:edited.id, rule:nextRule });
      schemas = schemas.map((candidate) => candidate.id === updated.id ? updated : candidate);
      schemaDraft = schemaEditorDraft(updated);
      persistSchemaLibrary();
      closeSchemaPropertyRulePickerForCommit(); renderSchemas(); renderSchemaDraft(); focusSchemaPropertyRow(path);finishSchemaPropertyInteractionReturn();
    } else {
      closeSchemaPropertyRulePickerForCommit();
      updateAttachedRule(path, edited.id, () => nextRule);
    }
    return;
  }
...
function renderSchemaLocalRuleConfiguration(path: string, configuration: RuleConfiguration): void {
  const editing = Boolean(editingAttachedLocalRule);
  const heading = document.createElement("h4"); heading.id = "schema-property-rule-picker-heading"; heading.textContent = `${editing ? "Edit" : "Configure"} ${configuration.ruleType} for ${canonicalRulePropertyPath(path)}`;
  ...
  const measuredValue = configuration.ruleType === "Text length" || configuration.ruleType === "Item count"
    ...
    : "";
  context.textContent = `Local rule origin · ${canonicalRulePropertyPath(path)} · ${configuration.ruleType.toLowerCase()} operator · type ${configuration.propertyType}${measuredValue}`;
  ...
  const create = document.createElement("button"); create.type = "submit"; create.textContent = editing ? "Save changes" : "Create rule";
  ...
  const enabledLabel = document.createElement("label"); const enabled = document.createElement("input"); enabled.id = "schema-local-rule-enabled"; enabled.type = "checkbox"; enabled.checked = configuration.enabled; enabledLabel.append(enabled, " Enabled");
  enabled.addEventListener("change", () => { configuration.enabled = enabled.checked; });
  ...
  if (!editing) form.append(reusableLabel);
  if (!editing && configuration.saveReusable) {
  ...
  form.append(status, create, ...(editing ? [] : [back]), cancel); form.addEventListener("submit", (event) => { event.preventDefault(); createConfiguredSchemaRule(path, configuration); });
...
function renderSchemaPropertyRulePicker(): void {
  ...
  const library = document.createElement("section"); library.setAttribute("aria-label", "Attach from Rule Library"); library.append(Object.assign(document.createElement("h5"), { textContent:"Attach from Rule Library" }), ...reusable.map((rule) => resultButton(
    ...
    () => { closeSchemaPropertyRulePickerForCommit(); attachReusableRule(path, rule); schemaPropertyTree.querySelector<HTMLButtonElement>(`button[aria-label="Add rule for ${CSS.escape(path)}"]`)?.focus({ preventScroll:true }); },
...
function openSchemaPropertyRulePicker(path: string, trigger: HTMLButtonElement): void {
  ...
  captureSchemaPropertyInteractionReturn(path,trigger);
...
function attachReusableRule(path: string, rule: ReusableSchemaRule): void {
  ...
  if(!compactCanonicalEditor)finishSchemaPropertyInteractionReturn();
}
...
function schemaRuleTypeForAttachment(rule: NonNullable<SchemaDefinition["attachedRules"]>[number]): SchemaRuleType {
  const operator = rule.operator?.replaceAll("_", "-").toLowerCase();
  if (operator === "exact-value") return "Exact value";
  if (operator === "allowed-values") return "Allowed values";
  if (operator === "regular-expression" || operator === "regex") return "Regular expression";
  if (operator === "text-length") return "Text length";
  if (operator === "digits-only") return "Digits only";
  if (operator === "numeric-range") return "Numeric range";
  if (operator === "item-count") return "Item count";
  if (operator === "allow-undeclared-properties") return "Allow undeclared properties";
  return "Required";
}

function editReusableSchemaRule(rule: ReusableSchemaRule): void {
  editingReusableSchemaRuleId = rule.id;
  if (schemaRuleName) schemaRuleName.value = rule.name;
  if (schemaRuleTypes) schemaRuleTypes.value = rule.applicableType ?? applicablePropertyTypesForRule(rule)[0] ?? "string";
  if (schemaRuleOperator) schemaRuleOperator.value = rule.operator ?? "required";
  if (schemaRuleParameters) schemaRuleParameters.value = rule.allowedValues?.map(String).join(", ") ?? rule.parameters ?? "";
  if (schemaRuleSeverity) schemaRuleSeverity.value = rule.severity ?? "error";
  if (schemaRuleMessage) schemaRuleMessage.value = rule.message ?? "";
  if (schemaRuleExamples) schemaRuleExamples.value = rule.examples ?? "";
  if (schemaRuleAttachments) schemaRuleAttachments.replaceChildren(...schemas.map((schema) => Object.assign(document.createElement("option"), { value:schema.id, textContent:`${schema.name} v${schema.version}`, selected:rule.attachments?.includes(schema.id) ?? false })));
  if (schemaRuleEditor) schemaRuleEditor.hidden = false;
  schemaRuleName?.focus({ preventScroll:true });
}

function openAttachedSchemaRuleEditor(path: string, attached: NonNullable<SchemaDefinition["attachedRules"]>[number], trigger: HTMLButtonElement): void {
  const reusable = reusableSchemaRules.find((rule) => rule.id === attached.id);
  if (reusable) {
    showSchemaSubview("schema-rule-library");
    editReusableSchemaRule(reusable);
    return;
  }
  const source = schemas.find((schema) => schema.id === schemaDraft?.id) ?? schemaDraft;
  if (!source) return;
  const inspection = inspectLocalRuleEdit(source, path, attached.id, new Set(reusableSchemaRules.map(({ id }) => id)));
  selectedSchemaPropertyPath = schemaRulePickerPath = path;
  schemaRulePickerTrigger = trigger;
  editingAttachedLocalRule = inspection.rule;
  schemaRuleConfiguration = createRuleConfigurationFromAttachedRule(
    schemaRuleTypeForAttachment(inspection.rule),
    schemaPropertyType(path),
    inspection.rule,
  );
  renderSchemaPropertyRulePicker();
  schemaPropertyRulePicker.showModal();
  schemaPropertyRulePicker.querySelector<HTMLElement>("input, select, textarea, button")?.focus({ preventScroll:true });
}
...
function openReusableRuleSyncReview(rule: ReusableSchemaRule, trigger: HTMLButtonElement): void {
  const versioned = { ...rule, version:rule.version ?? 1 };
  const review = reviewReusableRuleSync(schemas, versioned);
  pendingSchemaRuleSync = { rule, review, trigger };
  const changes = review.schemas.map((schema) => `${schema.schemaName} revision ${schema.currentVersion} to ${schema.nextVersion} (${schema.attachmentCount} ${schema.attachmentCount === 1 ? "attachment" : "attachments"})`).join("; ");
  schemaRuleSyncReviewSummary.textContent = review.blocked.length
    ...
    : `${review.schemaCount} schemas and ${review.attachmentCount} attachments: ${changes}. No changes occur before confirmation.`;
  confirmSchemaRuleSyncButton.disabled = !review.ready;
  schemaRuleSyncReview.showModal();
  (review.ready ? confirmSchemaRuleSyncButton : cancelSchemaRuleSyncButton).focus({ preventScroll:true });
}

confirmSchemaRuleSyncButton.addEventListener("click", () => {
  const pending = pendingSchemaRuleSync;
  if (!pending) return;
  void (async()=>{
    confirmSchemaRuleSyncButton.disabled=true;cancelSchemaRuleSyncButton.disabled=true;
    try {
      await durableProjectRuntime.settled("schema");
      const settledSchemas=restoreSchemaLibrary(dataLayerStorage.getItem(SCHEMA_LIBRARY_STORAGE_KEY)),versionedRule={...pending.rule,version:pending.rule.version??1},settledReview=reviewReusableRuleSync(settledSchemas,versionedRule);
      if(JSON.stringify(settledReview)!==JSON.stringify(pending.review))throw new Error("The attached schemas changed after review. Reopen synchronization and confirm the current publication plan.");
      const nextSchemas=publishReusableRuleSync(settledSchemas,versionedRule,settledReview);
      persistSchemaAndRuleLibraries(nextSchemas,reusableSchemaRules);
      await durableProjectRuntime.settled("schema");
      schemas=restoreSchemaLibrary(dataLayerStorage.getItem(SCHEMA_LIBRARY_STORAGE_KEY));
      renderSchemas();renderSchemaWorkflowRows();
      schemaRuleSyncReview.close();pendingSchemaRuleSync=undefined;
      if(schemaResult)schemaResult.textContent=`Published ${settledReview.schemaCount} schema revisions with ${pending.rule.name} revision ${pending.rule.version??1}.`;
    }catch(error){
      schemaRuleSyncReviewSummary.textContent=error instanceof Error?`No schema revision was published. ${error.message}`:"No schema revision was published.";
      confirmSchemaRuleSyncButton.disabled=!pending.review.ready;cancelSchemaRuleSyncButton.disabled=false;
    }
  })();
});
cancelSchemaRuleSyncButton.addEventListener("click", () => {
  const trigger = pendingSchemaRuleSync?.trigger;
...
function renderSchemaWorkflowRows(): void {
  ...
  schemaRuleList?.replaceChildren(...visibleRules.map((rule) => {
    const item = document.createElement("li"); item.dataset.ruleId = rule.id; const summary = document.createElement("span"); const allowedValuesMetadata=allowedValuesRuleLibraryMetadata(rule); summary.textContent = `${rule.name} v${rule.version ?? 1} · ${rule.kind}${allowedValuesMetadata ? ` · ${allowedValuesMetadata}` : ""}${rule.operator ? ` · ${rule.operator}` : ""}${rule.attachments?.length ? ` · ${rule.attachments.length} attachments` : ""}${rule.enabled === false ? " · disabled" : ""}${rule.revisionHistory?.length ? ` · ${rule.revisionHistory.length} prior versions` : ""}`;
    const edit = document.createElement("button"); const sync = document.createElement("button"); const duplicate = document.createElement("button"); const exportRule = document.createElement("button"); const disable = document.createElement("button"); const remove = document.createElement("button"); edit.type = sync.type = duplicate.type = exportRule.type = disable.type = remove.type = "button"; edit.textContent = "Edit"; sync.textContent = "Sync attached schemas and publish revisions"; duplicate.textContent = "Duplicate"; exportRule.textContent = "Export"; disable.textContent = rule.enabled === false ? "Enable" : "Disable"; remove.textContent = "Delete";
    edit.addEventListener("click", () => editReusableSchemaRule(rule));
    const syncReview = reviewReusableRuleSync(schemas, { ...rule, version:rule.version ?? 1 });
    sync.addEventListener("click", () => openReusableRuleSyncReview(rule, sync));
    if (rule.migrationIssue) summary.textContent += ` · Migration issue: ${rule.migrationIssue}`;
    ...
    item.append(summary, edit, ...(syncReview.schemaCount ? [sync] : []), duplicate, exportRule, disable, remove); return item;
  }));
...
function recheckCapturedSchemaValidation(): void {
  ...
  const canonical=restoreCanonicalProjectEnvelope(projectStorage.getItem(SPECIFICATION_PROJECT_STORAGE_KEY)),compiled=canonical?compileSpecificationProject(canonical):undefined;
  let checked = 0;
  ...
  liveObserverState = {
    ...
    events: events.map((event) => {
      ...
      if(evaluated?.winner){
        checked+=1;const assignment=canonical!.project.collections.assignments.find(({id})=>id===evaluated.winner!.assignmentId),target=[...canonical!.project.collections.profiles,...canonical!.project.collections.propertySets,...canonical!.project.collections.pages,...canonical!.project.collections.events,...Object.values(canonical!.project.documentationFlowGraphs??{}).flatMap((graph)=>((graph as {pageFrames?:ProjectEntity[]}).pageFrames??[]))].find(({id})=>id===assignment?.targetId),state=evaluated.issueDetails.length?`${evaluated.issueDetails.length} issues` as const:"Valid" as const;
        ...
        records.push({eventId:event.id,eventName:event.name,state,checkedAt,schemaId:evaluated.winner.schemaId,schemaName:target?.name??evaluated.winner.schemaId,schemaVersion:evaluated.winner.schemaRevision,target:"payload",issueCodes:evaluated.issueDetails.map(({code})=>code),assignmentId:evaluated.winner.assignmentId,assignmentName:String(assignment?.name??evaluated.winner.assignmentId),assignmentEvidence:`Evaluated result ${evaluated.resultIdentity}`,evaluated:{resultIdentity:evaluated.resultIdentity,winner:evaluated.winner,issueDetails:evaluated.issueDetails.map(({code,path})=>({code,path}))}});
...
async function reviewCapturedValidationContinuation(record:SchemaValidationRecord,trigger:HTMLButtonElement):Promise<void>{
  try{await hydrateActiveProjectForSchemas();await durableProjectRuntime.settled();}catch(error){if(schemaResult)schemaResult.textContent=`Captured continuation could not load the active project. ${error instanceof Error?error.message:String(error)}`;return;}
  ...
  if(!project||!envelope){if(schemaResult)schemaResult.textContent="Create or open a Specification Project before continuing captured validation.";return;}
  if(!captured||!record.schemaId||!record.evaluated){if(schemaResult)schemaResult.textContent="Recheck the captured event with the project evaluator before continuing.";return;}
  if(!project.project.collections.assignments.some(({id,targetId})=>id===record.assignmentId&&targetId===record.schemaId)){if(schemaResult)schemaResult.textContent=`Add ${record.schemaName??"the validated contributor"} to ${project.project.name} before creating its Test case.`;return;}
  ...
  if(!choices.events.length){if(schemaResult)schemaResult.textContent=`Add the ${record.eventName} Event to ${project.project.name} before continuing.`;return;}
  const dialog=document.createElement("dialog"),heading=document.createElement("h4"),summary=document.createElement("p"),review=document.createElement("p"),name=document.createElement("input"),select=(labelText:string,values:readonly {id:string;name:string}[],optional=false)=>{const label=document.createElement("label"),control=document.createElement("select");label.textContent=labelText;if(optional)control.append(new Option(`No ${labelText.toLowerCase()}`,""));control.append(...values.map(({id,name:optionName})=>new Option(optionName,id)));label.append(control);dialog.append(label);return control;},confirm=document.createElement("button"),cancel=document.createElement("button");
  heading.textContent="Continue captured validation in project";summary.textContent=`${record.eventName} · ${record.state} · ${record.schemaName} revision ${record.schemaVersion} → ${project.project.name}.`;review.textContent=`Evaluated result ${record.evaluated.resultIdentity}. Proposed reviewed expectations: outcome ${record.evaluated.issueDetails.length?"Invalid":"Valid"}; issue paths and codes ${record.evaluated.issueDetails.map(({path,code})=>`${path??"/"} ${code}`).join(", ")||"none"}. Proposed Profile requirements: ${proposedRequirements.map(({path,type,required})=>`${path} (${type??"value"}${required?", required":""})`).join(", ")||"none"}. Each requirement retains this evidence identity.`;name.value=choices.suggestedFixtureName;name.setAttribute("aria-label","Test case name");dialog.append(heading,summary,review,name);const destination=select("Destination",[{id:"fixture",name:"Event validation Test case"},{id:"profile",name:"Profile requirements"}]),event=select("Event",choices.events),page=select("Page",choices.pages,true),step=select("Flow step",choices.flowSteps,true),profile=select("Profile",choices.profiles,true);confirm.type=cancel.type="button";confirm.textContent="Create Test case and open in Specification Studio";cancel.textContent="Cancel";
  ...
  confirm.addEventListener("click",()=>{void (async()=>{confirm.disabled=true;try{const toProfile=destination.value==="profile";if(toProfile&&!profile.value)throw new Error("Choose a Profile for the evaluated requirements.");await durableProjectRuntime.settled();await durableProjectRuntime.ensureProject(project.project.id);await durableProjectRuntime.settled();const loaded=await durableProjectRuntime.repository.loadProject(project.project.id),current=loaded.state,next=toProfile?applyCapturedValidationToProfile(current,{captureId:record.eventId,profileId:profile.value,contributorId:record.schemaId!,evaluated:record.evaluated!}):createFixtureFromCapturedValidation(current,{name:name.value.trim(),captureId:record.eventId,sourceId:captured.sourceId,eventName:record.eventName,payload:captured.payload,contributorId:record.schemaId!,eventId:event.value,...(page.value?{pageId:page.value}:{}),...(step.value?{flowStepId:step.value}:{}),evaluated:record.evaluated!},(kind)=>`${kind}:${crypto.randomUUID()}`),entity=toProfile?next.project.collections.profiles.find(({id})=>id===profile.value)!:next.project.collections.fixtures.at(-1)!,kind=toProfile?"profiles":"fixtures",result=commitCanonicalProjectState(projectStorage,next,{expectedRevision:loaded.draftSequence,pendingLabel:`Continue evaluated capture ${record.eventId} as ${toProfile?"Profile requirements":"Test case"}`,base:current});if(result.status==="conflict")throw new Error(`Project changed in a newer Saved Draft; review the continuation again.`);projectLibraryUi.captureActiveProject(next,result.revision);const routed=recordProjectNavigation(projectLibraryUi.library(),current.project.id,{kind,id:entity.id});projectStorage.setItem(PROJECT_LIBRARY_STORAGE_KEY,serializeProjectLibrary(routed));await durableProjectRuntime.settled();dialog.close();dialog.remove();if(schemaResult)schemaResult.textContent=`Saved evaluated capture evidence in ${entity.name}; opening it in Specification Studio.`;globalThis.open(`specification-builder.html?project=${encodeURIComponent(current.project.id)}&kind=${kind}&entity=${encodeURIComponent(entity.id)}&source=side-panel`,"_blank");}catch(error){confirm.disabled=false;summary.textContent=error instanceof Error?error.message:String(error);}})();});
  cancel.addEventListener("click",()=>{dialog.close();dialog.remove();trigger.focus({preventScroll:true});});dialog.append(confirm,cancel);document.body.append(dialog);dialog.showModal();name.focus({preventScroll:true});
}
...
function persistSavedSessionFeed(): void {
  if (savedSessionLiveFeed) dataLayerStorage.setItem(SAVED_SESSION_LIVE_FEED_STORAGE_KEY, serializeSavedSessionLiveFeed(savedSessionLiveFeed));
  else dataLayerStorage.removeItem(SAVED_SESSION_LIVE_FEED_STORAGE_KEY);
}
...
function currentSessionSaveDraft(): SessionSaveDraft {
  ...
  return createSessionSaveDraft({
    ...
    ...(completedLiveFlowTests.length ? { flowTests:structuredClone(completedLiveFlowTests) } : {}),
  });
}
...
schemaSearch?.addEventListener("input",()=>{if(schemaTreeProjectId)persistSchemaTreeView(schemaTreeProjectId);renderSchemas();});
schemaCategoryFilter?.addEventListener("change",()=>{if(schemaTreeProjectId)persistSchemaTreeView(schemaTreeProjectId);renderSchemas();});
schemaTreeScrollOwner?.addEventListener("scroll",()=>{if(schemaTreeProjectId&&!schemaTreeRestoringScroll&&schemaTreePendingScroll===undefined&&!schemaPanel?.hidden)persistSchemaTreeView(schemaTreeProjectId);},{passive:true});
schemaList?.addEventListener("keydown",(event)=>{
  const target=event.target instanceof HTMLButtonElement?event.target:undefined,controls=Array.from(schemaList.querySelectorAll<HTMLButtonElement>("li[role=treeitem] > button:first-of-type")),current=target?controls.indexOf(target):-1;if(current<0||!target)return;
  if(event.key==="ArrowDown"||event.key==="ArrowUp"||event.key==="Home"||event.key==="End"){event.preventDefault();const next=event.key==="Home"?0:event.key==="End"?controls.length-1:Math.max(0,Math.min(controls.length-1,current+(event.key==="ArrowDown"?1:-1)));controls[next]?.focus({preventScroll:false});}
  if(event.key==="ArrowRight"||event.key==="ArrowLeft"){const row=target.closest<HTMLElement>('[role="treeitem"][aria-expanded]');if(!row)return;const expanded=row.getAttribute("aria-expanded")==="true";if((event.key==="ArrowRight"&&!expanded)||(event.key==="ArrowLeft"&&expanded)){event.preventDefault();target.click();}}
});
...
schemaEditorName?.addEventListener("input", () => {
  ...
  if(compactCanonicalEditor){const adapter=compactCanonicalEditor;if(compactCanonicalProjectionQueueUnavailable(adapter)){schemaDraft=compactCanonicalProjectionRequest?.adapter===adapter?structuredClone(compactCanonicalProjectionRequest.projection):compactCanonicalProjection(adapter);compactCanonicalCommandFeedback="Wait for the current durable schema save or resolve its Retry/Reject recovery before renaming.";renderSchemaDraft();renderCompactCanonicalContext();return;}const projection=structuredClone(schemaDraft);renderSchemaDraft();void beginCompactCanonicalProjectionPersistence(adapter,projection,"schema name");return;}
...
schemaOnlyDeclaredProperties?.addEventListener("change", () => { if (schemaDraft) { const { additionalProperties: _previous, ...document } = schemaDraft.document; schemaDraft = { ...schemaDraft, document:schemaOnlyDeclaredProperties.checked ? { ...document, additionalProperties:false } : document };if(compactCanonicalEditor){const canonical=compactCanonicalEditor.load();void dispatchCompactCanonicalCommand({kind:"policy",baseRevision:canonical.revision,onlyDefinedFields:schemaOnlyDeclaredProperties.checked});return;}persistSchemaEditorDraft("Change additional-property policy"); renderSchemaDraft(); } });
...
saveSchemaButton?.addEventListener("click", () => {
  if (!schemaDraft) return;
  if(compactCanonicalEditor&&compactCanonicalSemanticUnresolved()){compactCanonicalCommandFeedback="Publish is waiting for the current canonical edit to commit. Retry after the Saved Draft is ready, or resolve the pending command first.";if(schemaResult)schemaResult.textContent=compactCanonicalCommandFeedback;renderCompactCanonicalContext();return;}
  if(saveSchemaButton.disabled)return;
  const draft = schemaDraft;
  if (schemaRevisionReview) {
    const existing = schemas.find((schema) => schema.id === draft.id);
    if (existing&&!compactCanonicalEditor) persistSchemaEditorDraft();
...
confirmSchemaRevisionButton?.addEventListener("click", () => {
  if(compactCanonicalEditor&&compactCanonicalSemanticUnresolved()){compactCanonicalCommandFeedback="Publish was not started because a canonical edit is still saving or awaiting Retry or Reject.";if(schemaResult)schemaResult.textContent=compactCanonicalCommandFeedback;renderCompactCanonicalContext();return;}
  ...
  if (!schemaDraft) return;
  const presentedDraft = schemaDraft;
  const existing = schemas.find((schema) => schema.id === presentedDraft.id);
  const draft = compactCanonicalEditor&&existing?schemaEditorDraft(existing):presentedDraft;
  const completeAssignments = draft.assignments.filter(({ sourceId, eventName }) => Boolean(sourceId.trim() && eventName.trim()));
  const candidate = { ...draft, id:existing?.id ?? createSchema(draft.name, 1, draft.document).id };
  const candidates = [...schemas.filter((schema) => schema.id !== candidate.id), candidate];
  const inheritanceError = schemaInheritanceError(candidate, candidates) ?? schemaInheritanceConflict(candidate, candidates);
  if (inheritanceError) { if (schemaResult) schemaResult.textContent = inheritanceError; return; }
  const saved = existing
    ? publishSchemaWorkingDraft(updateSchemaWorkingDraft(existing, compactCanonicalEditor?{
    ...
    : { ...draft, id:`schema:${draft.name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}:1`, published:true, assignments:completeAssignments };
  ...
  persistSchemaLibrary(); dataLayerStorage.setItem(SCHEMA_RULE_STORAGE_KEY, JSON.stringify(reusableSchemaRules));
...
discardSchemaDraftButton?.addEventListener("click", () => { if(compactCanonicalEditor)closeCompactCanonicalEditor();else{schemaDraft = undefined;renderSchemaDraft();} if (schemaCloseReview?.open) schemaCloseReview.close(); if (schemaCloseReview) schemaCloseReview.hidden = true; });
discardWorkingSchemaDraftButton?.addEventListener("click", () => {
  ...
  if(compactCanonicalEditor)closeCompactCanonicalEditor();else schemaDraft = undefined; persistSchemaLibrary(); renderSchemaDraft(); renderSchemas(); renderSchemaWorkflowRows();
...
closeSchemaEditorButton?.addEventListener("click", () => { if (!schemaDraft) return; if(compactCanonicalEditor)closeCompactCanonicalEditor();else{schemaDraft = undefined;renderSchemaDraft();} renderSchemas(); if (schemaResult) schemaResult.textContent = "Working draft retained without publishing."; });
...
saveSchemaAssignmentButton?.addEventListener("click", () => {
  ...
  schemas = schemas.map((candidate) => {
    if (candidate.id !== schema.id) return candidate;
    const editedAssignmentId = editingSchemaAssignment?.schemaId === schema.id ? editingSchemaAssignment.assignmentId : undefined;
    const isEditing = editingSchemaAssignment?.schemaId === schema.id;
    const editedIndex = isEditing ? candidate.assignments.findIndex((assignment) => assignment.id === editedAssignmentId) : -1;
    const collisionIndex = candidate.assignments.findIndex((assignment) => assignment.id === next.id);
    const replacementIndexes = new Set([editedIndex, collisionIndex].filter((index) => index >= 0));
    const insertionIndex = replacementIndexes.size ? Math.min(...replacementIndexes) : -1;
    return {
      ...candidate,
      assignments:insertionIndex >= 0
        ? candidate.assignments.flatMap((assignment, index) => index === insertionIndex ? [next] : replacementIndexes.has(index) ? [] : [assignment])
...
schemaLibraryImportFile?.addEventListener("change", async () => {
  ...
  try {
    ...
    pendingSchemaImport = { schemas:importedSchemas, rules:(archive.rules as ReusableSchemaRule[]).map(normalizeReusableSchemaRule) };
...
const applyLiveTargetPathObservation = createLiveTargetPermissionPathApplyCallback({
  coordinator:liveTargetPermissionRecoveryCoordinator,
  applyObservationEffects:(observation) => {
    dataLayerObserverState = attachHistoryArrayObserver(
      { ...dataLayerObserverState, sessionState:dataLayerSessionState },
      observation,
    );
...
const targetPathStatusController = createTargetPathStatusController({
  ...
  apply:applyLiveTargetPathObservation,
});
