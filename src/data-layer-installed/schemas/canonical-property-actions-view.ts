import { applyCanonicalCommand, canonicalPropertyPath, activateFocusedOwnershipSection, clearSchemaTableOverlay,
     focusedCanonicalOwnershipInput, focusedDefinitionFieldLabels, focusedOwnershipActionTarget, focusedOwnershipState,
     focusedPropertyLayerSequence, focusedPropertyLifecycleOperation, focusedPropertyPatch, focusedPropertyProvenanceSummary,
     focusedSectionOwnershipActions, focusedSourceState, focusedStagedChanges, gateFocusedOwnershipSection,
...
export function openCanonicalPropertyActions(ports: CanonicalInstalledViewPorts, contextTable: SchemaCanonicalContextTableView,
    ...
    let working = structuredClone(original), activeSection: "definition" | "rules" | "structure" | undefined,
         feedbackText = "", stagedOwnershipAction = "";
    ...
    const removedRuleIds = new Set<string>(), removedValueIds = new Set<string>(), stagedOperations: NonNullable<Extract<Parameters<typeof applyCanonicalCommand>[1],
         {
        kind: "set";
    }>["operations"]> = [], state = focusedSourceState(original), sectionOwnership = focusedSectionOwnershipActions(ownership.input),
         close = () => {
        clearSchemaTableOverlay(owner);
        trigger.focus({ preventScroll: true });
    }, restoreFocus = (label: string) => queueMicrotask(() => Array.from(owner.ownerDocument.querySelectorAll<HTMLButtonElement>('[data-schema-row-overlay="true"] button'))
    ...
    const menu = () => renderFocusedPropertyMenu({
        ...
        selectSection: (name) => showSection(name as "definition" | "rules" | "structure"),
    }), mount = (layers: HTMLElement[], focusLabel?: string) => {
    ...
    const sectionContext = (section: "definition" | "rules" | "structure", render: () => void) => ({
        ...
        node: original,
        getWorking: () => working,
        setWorking: (value: typeof working | undefined) => {
            if (value)
                working = value;
        },
        ...
        removedRuleIds,
        removedValueIds,
        id: (kind: string) => `${kind}:${crypto.randomUUID()}`,
        stageStructure: (operation: (typeof stagedOperations)[number]) => {
            stagedOperations.push(operation);
            render();
        },
        render,
        patchFor: (next: typeof working, source: typeof original) => focusedPropertyPatch(next, source,
             removedRuleIds, removedValueIds),
        command: (command: Parameters<typeof applyCanonicalCommand>[1]) => applyCanonicalCommand(model, command),
        select: () => { },
    ...
    const showReview = (section: "definition" | "rules" | "structure", child: HTMLElement, focusLabel?: string) => {
        const review = document.createElement("section"), heading = document.createElement("h3"), summary = document.createElement("p"),
             changes = document.createElement("ul"), actions = document.createElement("div"), cancel = document.createElement("button"),
             confirm = document.createElement("button"), patch = focusedPropertyPatch(working, original,
             removedRuleIds, removedValueIds), staged = focusedStagedChanges(working, original, removedRuleIds,
             path, removedValueIds);
    ...
    const buildSection = (section: "definition" | "rules" | "structure") => {
        const host = document.createElement("section"), heading = document.createElement("h3"), identity = document.createElement("p"),
             body = document.createElement("section"), group = document.createElement("div"), status = document.createElement("p"),
    ...
    function showSection(section: "definition" | "rules" | "structure", focusLabel?: string): void {
        activeSection = section;
