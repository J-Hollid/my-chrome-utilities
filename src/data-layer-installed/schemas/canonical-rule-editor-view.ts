...
export function openCanonicalRuleEditor(
  ...
  const node=base&&Object.values(base.nodes).find((candidate) =>
    canonicalPropertyPath(base,candidate.id)===canonicalRulePropertyPath(path)||candidate.id===path);
  ...
  const render=():void => {
    ...
    const review=button("Review changes",()=>{
      ...
      const stagedRules=working.rules.filter(({id})=>!removedRuleIds.has(id));
      ...
      reviewActions.append(
        ...
        button("Confirm changes",()=>{void (async()=>{
  ...
  render();
  picker.showModal();
  picker.querySelector<HTMLButtonElement>('[aria-label="Compact staged rule editor"] > button')?.focus({preventScroll:true});
  return true;
}
