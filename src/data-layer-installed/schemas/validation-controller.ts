...
export class SchemaValidationController {
  ...
  recordsProjection():readonly SchemaValidationRecord[] { return structuredClone(this.#records); }
  manualOverridesProjection():Readonly<Record<string,string>> { return structuredClone(this.#manualOverrides); }
  manualSchemaId(eventId:string):string|undefined { return this.#manualOverrides[eventId]; }
  ...
  render():void {
    ...
    ports.list.replaceChildren(...this.#records.map((record) => {
      ...
      summary.textContent = `${record.eventName} · ${record.state} · ${record.schemaName ? `${record.schemaName} v${record.schemaVersion} · ${
        record.target ?? "payload"}` : "No matching schema"}${record.assignmentId ? ` · assignment ${record.assignmentName ?? record.assignmentId} (${
        record.assignmentId})` : ""}${record.assignmentEvidence ? ` · ${record.assignmentEvidence}` : ""} · ${record.checkedAt}`;
      if (ports.prepare) { button.type = "button"; button.textContent = "Continue in project"; button.disabled = !record.schemaId || !record.evaluated;
        const review = ():void => { void this.reviewContinuation(record, button); }; button.addEventListener("click", review); this.ownRow(() => button.removeEventListener("click",
           review)); item.append(summary, button); }
  ...
  async reviewContinuation(record:SchemaValidationRecord, trigger:HTMLButtonElement):Promise<void> {
    const ports = this.#behavior, generation = ports?.generation(); if (!ports?.prepare || !ports.guidedRoot || !ports.document || generation === undefined || !ports.isCurrent(
      generation)) return;
    ...
    const select = (labelText:string, values:readonly {id:string;name:string}[], optional=false):HTMLSelectElement => { const label=document.createElement("label"), control=
      document.createElement("select");
      label.textContent=labelText; if(optional){const option=document.createElement("option");option.value="";option.textContent=`No ${labelText.toLowerCase(
        )}`;control.append(option);}
      for(const value of values){const option=document.createElement("option");option.value=value.id;option.textContent=value.name;control.append(option);} if(!optional&&values[0])
        control.value=values[0].id;label.append(control);dialog.append(label);return control; };
    ...
    const selectDestination = ():void => { const toProfile=destination.value==="profile";name.hidden=Boolean(toProfile);event.parentElement!.hidden=toProfile;page.parentElement!
      .hidden=toProfile;step.parentElement!.hidden=toProfile;
      confirm.textContent=toProfile?"Add requirements and open Profile":"Create Test case and open in Specification Studio"; };
    const confirmContinuation = ():void => { const toProfile=destination.value==="profile";if(toProfile&&!profile.value){summary.textContent=
      "Choose a Profile for the evaluated requirements.";return;} confirm.disabled = true;
      void continuation.commit({destination:toProfile?"profile":"fixture",name:name.value.trim(),eventId:event.value,...(page.value?{pageId:page.value}:{}),...(step.value?{
        flowStepId:step.value}:{}),...(profile.value?{profileId:profile.value}:{})})
        .then(({entityName}) => { if (ports.isCurrent(generation)) { close(false); if(ports.result)ports.result.textContent=
    ...
    const cancelContinuation=():void=>close(true); destination.addEventListener("change", selectDestination); confirm.addEventListener("click", confirmContinuation); cancel
  ...
  recheck(events:readonly CapturedEvent[] = []):readonly SchemaValidationRecord[] {
    ...
    const records = events.map((event):SchemaValidationRecord => { const override = this.#manualOverrides[event.id], schemas = ports.schemas(), candidates = override ? schemas
      ...
      return { eventId:event.id, eventName:event.name, state:result.state, checkedAt, ...(result.schema ? { schemaId:result.schema.id, schemaName:result.schema.name, schemaVersion:
        result.schema.version } : {}), issueCodes:result.issues.map((issue) => issue.rule ?? issue.schemaLocation) }; });
