...
export class SchemaRuleAttachmentWorkflow {
  ...
  pendingUpgradeState():{ id:string; schemaIds:readonly string[] }|undefined {
    return this.#pendingUpgrade ? structuredClone(this.#pendingUpgrade) : undefined;
  }
  pendingSyncState():{ rule:ReusableSchemaRule; review:ReusableRuleSyncReview }|undefined {
    return this.#pendingSync ? structuredClone(this.#pendingSync) : undefined;
  }
  approvedAttachmentUpdate():string|undefined { return this.#approvedAttachmentUpdateId; }
  ...
  requestUpgrade(id:string,schemaIds:readonly string[]):boolean { const rule=this.ports.stored(id),p=this.ports.behavior();if(!rule)return false;
    const affected=p.schemas().filter((schema) => schemaIds.includes(schema.id)&&schema.attachedRules?.some((item) => item.id===id));this.#pendingUpgrade={id,
      schemaIds:[...schemaIds]};p.presentation.showUpgrade(rule,affected);return true; }
  ...
  requestSync(id:string):boolean { const rule=this.ports.stored(id),p=this.ports.behavior();if(!rule)return false;const review=this.review(rule);
    this.#pendingSync={rule:structuredClone(rule),review};p.presentation.showSync(review);return true; }
  confirmSync():void { const pending=this.#pendingSync,p=this.ports.behavior();if(!pending)return;const rule=this.ports.stored(pending.rule.id);
    if(!rule)throw new Error("The reusable rule was removed after review");const review=this.review(rule);
    if(JSON.stringify(review)!==JSON.stringify(pending.review))throw new Error("The attached schemas changed after review");p.replaceSchemas(publishReusableRuleSync(p.schemas(),
      rule,
    review));this.#pendingSync=undefined;p.persistLibrary();p.renderAll();p.presentation.close("sync"); }
