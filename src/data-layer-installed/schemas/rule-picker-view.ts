import { builtInRulesForProperty, cardinalityComparisonPasses, comparisonValueFromInput, conditionGroupAppliesToValue, createRuleConfiguration, operatorsForConditionType, reusableRuleMetadata, reusableRulesForProperty, ruleConfigurationControls, schemaPropertyRows, typedComparisonValue, validateRuleConfiguration, type SchemaDefinition, type SchemaPropertyType, } from "../../utilities/data-layer/schemas.js";
import type { SchemaRuleController } from "./rule-controller.js";
interface RulePickerPorts {
...
export class SchemaRulePickerView {
    ...
    constructor(controller: SchemaRuleController, ports: RulePickerPorts) { this.#controller = controller; this.#ports = ports; }
    render(): void {
        ...
        c.clearPicker();
        const document = picker.ownerDocument, normalize = (value: string) => c.normalizePickerPath(value), rerender = () => { if (workingConfiguration)
            c.setConfiguration(workingConfiguration); this.render(); };
        if (!c.configuration) {
            ...
            for (const rule of builtIns) {
                ...
                const action = (): void => { c.setConfiguration(createRuleConfiguration(rule.name as NonNullable<typeof c.configuration>["ruleType"], propertyType)); rerender(); };
            ...
            for (const rule of reusable) {
                ...
                metadata.textContent = reusableRuleMetadata(rule, propertyType);
                const action = (): void => { c.attach(p.active().id, rule.id, path); p.closeForCommit(); };
                button.addEventListener("click", action);
                c.ownPicker(() => button.removeEventListener("click", action));
                article.append(button, metadata);
            ...
            if (!builtIns.length && !reusable.length) {
                ...
                const clearSearch = (): void => { c.setPickerSearch(""); rerender(); };
            ...
            const close = (): void => p.close(), searchRules = (): void => { c.setPickerSearch(search.value); rerender(); };
        ...
        const refresh = (): void => { c.setConfiguration(configuration); const validation = validateRuleConfiguration(configuration); status.textContent = validation.assistance; if (createButton)
        ...
        for (const control of ruleConfigurationControls(configuration.ruleType, configuration.propertyType)) {
            ...
            const update = (): void => { (configuration as unknown as Record<string, unknown>)[control.key] = input.value; refresh(); };
        ...
        if (allowed) {
            ...
            configuration.allowedValues.forEach((value, index) => {
                ...
                const update = (): void => { configuration.allowedValues[index] = input.value; refresh(); }, removeValue = (): void => { configuration.allowedValues.splice(index, 1); rerender(); };
            ...
            const addValue = (): void => { configuration.allowedValues.push(""); rerender(); };
        ...
        const changeConditional = (): void => { configuration.applyOnlyWhen = conditional.checked; if (conditional.checked && !configuration.conditions.length)
            configuration.conditions.push(c.conditionPredicate(path).predicates[0]!); rerender(); }, changeReusable = (): void => { configuration.saveReusable = reusable.checked; rerender(); };
        ...
        if (configuration.applyOnlyWhen) {
            ...
            const changeGroup = (): void => { configuration.conditionGroupOperator = group.value === "Any" ? "Any" : "All"; refresh(); };
            ...
            configuration.conditions.forEach((predicate, index) => {
                ...
                const changeProperty = (): void => { const sample = c.valueAtPath(p.capturedValue(), property.value), detectedType = p.propertyType(editable.document, property.value) ?? "string", comparable = sample.exists && (sample.value === null || ["string", "number", "boolean"].includes(typeof sample.value)); configuration.conditions[index] = { propertyPath: property.value, operator: comparable ? "Equals" : "Exists", detectedType, ...(comparable ? { comparison: typedComparisonValue(sample.value as string | number | boolean | null) } : {}) }; rerender(); }, changeOperator = (): void => { predicate.operator = operator.value as typeof predicate.operator; if (predicate.operator === "Exists" || predicate.operator === "Does not exist")
                    delete predicate.comparison; rerender(); }, changeComparison = (): void => { const value = comparisonValueFromInput(comparison.value, predicate.detectedType ?? "string"); if (value)
                    predicate.comparison = value;
                else
                    delete predicate.comparison; refresh(); }, removeCondition = (): void => { configuration.conditions.splice(index, 1); rerender(); };
            ...
            const addCondition = (): void => { configuration.conditions.push(c.conditionPredicate(path).predicates[0]!); rerender(); }, applies = conditionGroupAppliesToValue(p.capturedValue(), { operator: configuration.conditionGroupOperator, predicates: configuration.conditions });
            if (!applies)
        ...
        createButton = create;
        const goBack = (): void => { c.setConfiguration(undefined); this.render(); }, cancelEdit = (): void => p.close(), submit = (event: Event): void => { event.preventDefault(); if (validateRuleConfiguration(configuration).ready)
            p.createConfigured(); };
