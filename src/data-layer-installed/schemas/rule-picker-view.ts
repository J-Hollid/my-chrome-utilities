...
export class SchemaRulePickerView {
  ...
  render(): void {
    ...
    const document = picker.ownerDocument,
      normalize = (value: string) => c.normalizePickerPath(value),
      rerender = () => {
        if (workingConfiguration) c.setConfiguration(workingConfiguration);
        this.render();
      };
    if (!c.configuration) {
      const heading = document.createElement("h4"),
        ...
        propertyType = c.typeForAttachment(p.active(), path);
      ...
      const canonical = normalize(path),
        attachedIds = new Set(
          (
            p.active().workingDraft?.attachedRules ??
            p.active().attachedRules ??
            []
          )
            .filter(
              ({ propertyPath }) => normalize(propertyPath ?? "") === canonical,
            )
            .map(({ id }) => id),
        ),
        query = c.pickerSearch.trim().toLowerCase(),
        builtIns = builtInRulesForProperty(propertyType).filter(
          (rule) =>
            !query ||
        ...
        reusable = reusableRulesForProperty(
          c.rules,
          propertyType,
          c.pickerSearch,
          attachedIds,
        ),
        create = document.createElement("section"),
      ...
      for (const rule of builtIns) {
        ...
        const action = (): void => {
          c.setConfiguration(
            createRuleConfiguration(
              rule.name as NonNullable<typeof c.configuration>["ruleType"],
              propertyType,
            ),
          );
          rerender();
      ...
      c.ownPicker(
        ...
        () => search.removeEventListener("input", searchRules),
      );
      picker.replaceChildren(heading, search, results, cancel);
      return;
    }
    const configuration = workingConfiguration!,
      editLabel = c.editingAttached
    ...
    for (const control of ruleConfigurationControls(
      configuration.ruleType,
      configuration.propertyType,
    )) {
      if (control.repeatable) continue;
      const input =
      ...
      if (control.inputType === "select")
        ...
        if (control.minimum !== undefined) text.min = String(control.minimum);
        if (control.step !== undefined) text.step = String(control.step);
      }
    ...
    const allowed =
      configuration.ruleType === "Allowed values"
    ...
    if (allowed) {
      ...
      configuration.allowedValues.forEach((value, index) => {
        ...
        c.ownPicker(
          ...
          () => remove.removeEventListener("click", removeValue),
        );
    ...
    c.ownPicker(
      () => severity.removeEventListener("change", changeSeverity),
      () => message.removeEventListener("input", changeMessage),
      () => enabled.removeEventListener("change", changeEnabled),
    );
    const conditional = document.createElement("input"),
      reusable = document.createElement("input"),
      conditionalLabel = document.createElement("label"),
      reusableLabel = document.createElement("label");
    ...
    const changeConditional = (): void => {
        ...
        if (conditional.checked && !configuration.conditions.length)
          configuration.conditions.push(
            c.conditionPredicate(path).predicates[0]!,
          );
        rerender();
      },
      changeReusable = (): void => {
    ...
    c.ownPicker(
      () => conditional.removeEventListener("change", changeConditional),
      () => reusable.removeEventListener("change", changeReusable),
    );
    form.append(
      heading,
      context,
    ...
    if (configuration.applyOnlyWhen) {
      ...
      configuration.conditions.forEach((predicate, index) => {
        ...
        operator.append(
          ...operatorsForConditionType(predicate.detectedType ?? "string").map(
            (value) =>
              Object.assign(document.createElement("option"), {
                value,
                textContent: value,
        ...
        const changeProperty = (): void => {
            const sample = c.valueAtPath(p.capturedValue(), property.value),
              detectedType =
                p.propertyType(editable.document, property.value) ?? "string",
              comparable =
                sample.exists &&
                (sample.value === null ||
              ...
              ...(comparable
                ? {
                    comparison: typedComparisonValue(
                      sample.value as string | number | boolean | null,
                    ),
                  }
                : {}),
            };
          ...
          changeComparison = (): void => {
            const value = comparisonValueFromInput(
              comparison.value,
              predicate.detectedType ?? "string",
            );
        ...
        c.ownPicker(
          () => property.removeEventListener("change", changeProperty),
          () => operator.removeEventListener("change", changeOperator),
          () => comparison.removeEventListener("input", changeComparison),
          () => remove.removeEventListener("click", removeCondition),
        );
      ...
      const addCondition = (): void => {
        ...
        applies = conditionGroupAppliesToValue(p.capturedValue(), {
          operator: configuration.conditionGroupOperator,
          predicates: configuration.conditions,
        });
      if (!applies)
        ...
        const observed = c.valueAtPath(p.capturedValue(), path),
          measured =
            configuration.ruleType === "Item count" &&
    ...
    if (!c.editingAttached) form.append(reusableLabel);
    if (!c.editingAttached && configuration.saveReusable) {
      ...
      c.ownPicker(
        () => name.removeEventListener("input", changeName),
        () => description.removeEventListener("input", changeDescription),
      );
