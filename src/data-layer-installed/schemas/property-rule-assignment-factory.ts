...
import type { SchemaPropertyElements } from "./property-installed-view.js";
...
interface PropertyDomainLinks {
  root: ParentNode;
  elements: SchemaPropertyElements;
  ruleElements: RuleElements;
  library: SchemaLibraryController;
  canonical: SchemaCanonicalEditorController;
  canonicalView: SchemaCanonicalInstalledView;
  canonicalPersistence: SchemaCanonicalPersistenceWorkflow;
  editor(): SchemaInstalledEditorWorkflow;
  ...
  renderDraft(): void;
  renderProperty(): void;
  createRuleId: SchemasInstalledPorts["createRuleId"];
  download: SchemasInstalledPorts["downloadSchema"];
  capturedValue: SchemasInstalledPorts["capturedAssignmentValue"];
  promotionDialog: SchemasInstalledPorts["localRulePromotionDialog"];
  scheduleFrame: SchemasInstalledPorts["scheduleFrame"];
  settleCanonical?: SchemasInstalledPorts["settleCanonical"];
  result: HTMLElement | null;
  schemaEditor: HTMLElement | null;
  schemaDetail: HTMLElement | null;
  document: Document | undefined;
}

/** Owns complete property, rule, and assignment composition. */
export function createSchemaPropertyRuleAssignmentDomain(
  storage: Pick<Storage, "getItem" | "setItem">,
  ruleElements: RuleElements,
  schemas: () => readonly SchemaDefinition[],
  assignmentPorts: ConstructorParameters<typeof SchemaAssignmentController>[0],
) {
  ...
  let propertyView: SchemaPropertyView;
  let rulePicker: SchemaRulePickerView;
  let workflow: SchemaPropertyRuleWorkflow;
  return { property,rule,rulePresentation,assignment,
    connect(p:PropertyDomainLinks) {
      rule.configure({
        ...
        result:(message) => {if(p.result)p.result.textContent=message;},
      ...
      property.configure({
        ...
        canonicalUndo:() => {
          if(!p.canonical.editorState?.canUndo)return false;
