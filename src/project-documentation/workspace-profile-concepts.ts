export type ProfileConceptFilter="all"|"included"|"excluded"|"overrides";
export type ProfileConceptBulkAction="include-all"|"exclude-all"|"reset";
export interface ProfileConceptProperty {path:string;concept?:string|undefined}

const conceptKey=(value:string|undefined):string=>value?.trim().toLocaleLowerCase()||"ungrouped";

export function profileConceptPresentation(input:{
  concepts:readonly {name:string;included:boolean}[];
  properties:readonly ProfileConceptProperty[];
  selectedPaths:readonly string[];
  activeConcept:string|undefined;
  query:string;
  filter:ProfileConceptFilter;
}):{
  concepts:{name:string;setIncluded:boolean;total:number;included:number;matching:number}[];
  properties:{path:string;included:boolean;override:boolean}[];
} {
  const selected=new Set(input.selectedPaths),query=input.query.trim().toLocaleLowerCase(),byConcept=new Map<string,ProfileConceptProperty[]>();
  for(const property of input.properties){const key=conceptKey(property.concept),items=byConcept.get(key)??[];items.push(property);byConcept.set(key,items);}
  const visible=(included:boolean,override:boolean):boolean=>input.filter==="all"||(input.filter==="included"?included:input.filter==="excluded"?!included:override);
