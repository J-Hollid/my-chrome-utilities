...
export function createDocumentationProfileConceptRenderer(mutateSection:MutateSection):(host:HTMLElement,set:ProjectDocumentationSet,section:ProjectDocumentationSection,properties:readonly ProfileConceptProperty[])=>void {
  ...
  return function render(host,set,section,properties):void {
    ...
    const showConcepts=button("Show Profile concepts",()=>{state.mobileSurface="concepts";drawResults();}),showProperties=button("Show concept properties",()=>{state.mobileSurface="properties";drawResults();}),drawResults=():void=>{
      ...
      navigator.replaceChildren();detail.replaceChildren();navigator.dataset.profileConceptSurface=state.mobileSurface==="concepts"?"active":"inactive";detail.dataset.profileConceptSurface=state.mobileSurface==="properties"?"active":"inactive";
      for(const concept of presentation.concepts){const status=concept.setIncluded?"Included in Document settings":"Excluded in Document settings",control=button(`${concept.name} · ${concept.included} of ${concept.total} included · ${concept.matching} matching · ${status}`,()=>{state.activeConcept=concept.name;state.mobileSurface="properties";drawResults();});control.dataset.profileConcept=concept.name;control.setAttribute("aria-current",String(concept.name===state.activeConcept));navigator.append(control);}
      if(state.activeConcept){
        ...
        const choices=document.createElement("fieldset");choices.setAttribute("aria-label",`${section.name} ${state.activeConcept} property choices`);choices.append(Object.assign(document.createElement("legend"),{textContent:`${presentation.properties.length} matching properties`}));
    ...
    search.addEventListener("input",()=>{state.query=search.value;drawResults();});filter.addEventListener("change",()=>{state.filter=filter.value as ProfileConceptFilter;drawResults();});
