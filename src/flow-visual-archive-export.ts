...
import {flowVisualArchiveExtension as extension,flowVisualJsonBlob as jsonBlob,withoutEmbeddedFlowVisualBodies as withoutEmbeddedBodies,type FlowVisualArchiveManifest,type FlowVisualManifestAsset as ManifestAsset} from "./flow-visual-archive-format.js";
import {validateFlowVisualBody,validateFlowVisualMetadata} from "./flow-visual-asset-validation.js";
import {writeStoredZip,type FlowVisualZipEntry,type FlowVisualZipSink} from "./flow-visual-zip.js";
...
async function prepareArchive(input:{project:SpecificationProject;publishedProject?:SpecificationProject;assets:readonly FlowVisualArchiveAsset[]}){
  ...
  for(const asset of ordered)registerArchiveAsset(asset,uniqueBodies,assets);
  const project=withoutEmbeddedBodies(input.project),publishedProject=input.publishedProject?withoutEmbeddedBodies(input.publishedProject):undefined,manifest:FlowVisualArchiveManifest={format:"my-chrome-utilities.project-archive",version:3,requiredFeatures:["digest-addressed-visual-assets"],draftEntry:"draft.json",...(publishedProject?{publishedEntry:"published.json"}:{}),assets};
