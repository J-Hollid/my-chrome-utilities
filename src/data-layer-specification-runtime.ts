...
import {evaluateSpecificationObservation,type ExecutableSpecificationPlan,type SpecificationEvaluationResult} from "./data-layer-specification-engine.js";
...
function publishedPlan(project:SpecificationProject):ExecutableSpecificationPlan|undefined{if(!project.currentRelease)return undefined;const release=project.releases.find(({id})=>id===project.currentRelease) as (ProjectEntity&{executablePlan?:ExecutableSpecificationPlan})|undefined;return release?.executablePlan;}
...
export function recordSpecificationRuntimeObservation(storage:SpecificationRuntimeStorage,observation:SpecificationRuntimeObservation):SpecificationRuntimeResult{
  ...
  const prior=restoreInstances(storage),payload=observation.payload&&typeof observation.payload==="object"?observation.payload as Record<string,unknown>:{},url=observation.pageUrl?new URL(observation.pageUrl,globalThis.location?.href):undefined,plan=publishedPlan(project);if(!plan)return{instances:prior};const evaluation=evaluateSpecificationObservation(plan,{...observation,...payload,payload:observation.payload??{},pathname:url?.pathname??"",...(observedEventId?{eventId:observedEventId}:{}),...(observedPageId?{pageId:observedPageId}:{})},prior),transition=evaluation.stateTransition!,instances=transition.instances;storage.setItem(FLOW_INSTANCES_STORAGE_KEY,JSON.stringify(instances));
