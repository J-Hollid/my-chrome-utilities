...
export function selectLiveFlow(run:LiveFlowTestRun,state:ProjectState,flowId:string):LiveFlowTestRun{
  ...
  const frameEntities=new Map((graphs(state)[flowId]?.pageFrames??[]).map((frame)=>[frame.id,frame])),sections=new Map(current.sections.map((section)=>[section.id,section.name]));const startChoices=current.pageFrames.map((frame)=>{const page=state.project.collections.pages.find(({id})=>id===frame.pageId),lane=sections.get(frame.sectionId??"")??"Outside Sections",storedName=frameEntities.get(frame.id)?.name,instanceName=storedName===`${page?.name} frame`?page?.name:storedName;return{id:frame.id,name:instanceName||page?.name||frame.id,kind:"Page" as const,pageId:frame.pageId,lane,stableFrameId:frame.id,recommended:!incoming.has(frame.id)};}).sort((left,right)=>Number(right.recommended)-Number(left.recommended));
