...
import {compileLayeredSchema,layeredContributorPath,layeredContributorsForPath,validateLayeredObservation,type LayerContributor,type LayerScope,type ProjectEntity,type ProjectState} from "./utilities/data-layer/schemas.js";
...
export interface LiveFlowHistoryEntry {
  ...
  provenance:{contributorId:string;contributorName:string;scope:LayerScope}[];target:{id:string;name:string};status:"Valid"|"Invalid";defectId?:string;
}
...
function revision(contributors:readonly LayerContributor[]):number{return Math.max(0,...contributors.map((contributor)=>Number(contributor.revision??0)));}
...
function revisionIdentity(compiled:ReturnType<typeof compileLayeredSchema>):string{let hash=2166136261;for(const character of stableJson(compiled)){hash^=character.charCodeAt(0);hash=Math.imul(hash,16777619);}return`flow-schema:${(hash>>>0).toString(16).padStart(8,"0")}`;}
...
export function liveFlowNextSteps(run:LiveFlowTestRun,state:ProjectState):LiveFlowNextStep[]{
  if(!run.flowId||!run.currentStepId||!run.history.some(({stepId})=>stepId===run.currentStepId))return[];const source=step(state,run.flowId,run.currentStepId);
...
export function liveFlowGraphNodes(run:LiveFlowTestRun,state:ProjectState):LiveFlowGraphNode[]{
  if(!run.flowId||!run.currentStepId)return[];const current=graphs(state)[run.flowId],nextById=new Map(liveFlowNextSteps(run,state).map((next)=>[next.id,next])),nodes=[...(current?.pageFrames??[]).map((entity)=>step(state,run.flowId!,entity.id)),...(current?.occurrences??[]).map((entity)=>step(state,run.flowId!,entity.id))];
