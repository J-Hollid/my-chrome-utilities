...
const contextualMetadataValue=(property:EffectiveProperty|undefined,column:"example"|"allowedValues"):string=>{if(!property)return"";return column==="example"?property.examples?.map(String).join(" or ")??"":property.allowedValues?.map(String).join(" or ")??"";};
