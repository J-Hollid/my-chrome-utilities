...
const flowTemplateData=(snapshot:FlowDocumentationSnapshot,table:ProjectDocumentationTable,metadata:readonly FlowDocumentationMetadata[],canonicalPaths:readonly string[]):TemplateContextObject=>{
  ...
  const rows=(context:FlowDocumentationSnapshot["contexts"][number],contextIndex:number):TemplateContextObject[]=>table.rows.map((row,rowIndex)=>{
    const property=String(row[0]??""),canonicalPath=tableCanonicalPaths[rowIndex]??"",metadataValues=metadata.map((column,index)=>[column,String(row[index+1]??"")] as const),value=String(row[metadata.length+contextIndex+1]??""),cells=[{columnKey:"property",heading:"Property",value:property},...metadataValues.map(([column,item])=>({columnKey:column,heading:metadataHeading[column],value:item})),{columnKey:"value",heading:"Value",value}];
