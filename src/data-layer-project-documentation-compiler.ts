...
function profileTable(section:ProjectDocumentationSection,profile:Profile):ProjectDocumentationTable {
  const requirements=profile.canonicalSchema?canonicalRequirements(profile.canonicalSchema):profile.requirements,columns=(section.configuration?.columns?.filter((column):column is ProjectDocumentationProfileColumn=>defaultProfileColumns.includes(column as ProjectDocumentationProfileColumn))??defaultProfileColumns),paths=section.configuration?.paths??requirements.map(({path})=>path),byPath=new Map(requirements.map((item)=>[item.path,item]));
...
export function compileProjectDocumentation(input:CompileProjectDocumentationInput):ProjectDocumentationSnapshot {
  ...
  for(const section of set.sections.filter(({selected})=>selected)){
    ...
    if(section.kind==="profile"){
      ...
      revisions[profile.id]=stableRevision(profile);tables.push({...profileTable(section,profile),themeFingerprint:themeFingerprint(theme)});continue;
    }
