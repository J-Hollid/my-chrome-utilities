...
import {compileLayeredSchema,type CompiledLayeredSchema,type EffectiveProperty} from "./data-layer-layered-schema.js";
...
const contextualMetadataValue=(property:EffectiveProperty|undefined,column:FlowDocumentationMetadata):string=>{if(!property)return"";if(column==="type")return property.type??"";if(column==="allowedValues")return property.allowedValues?.map(String).join(" or ")??"";if(column==="example")return property.examples?.map(String).join(" or ")??"";if(column==="provenance")return property.origins.map(({contributorName,scope})=>`${contributorName} (${scope})`).join("; ");if(column==="description")return property.documentation??"";return property.comments??"";};
export class ProjectDocumentationVisualUnavailableError extends Error {override name="ProjectDocumentationVisualUnavailableError";}
