...
const flowTemplateData=(snapshot:FlowDocumentationSnapshot,table:ProjectDocumentationTable,metadata:readonly FlowDocumentationMetadata[],canonicalPaths:readonly string[],set:ProjectDocumentationSet):TemplateContextObject=>{
  ...
  const concepts=(items:TemplateContextObject[]):TemplateContextObject[]=>{if(!set.includeConceptSubheadings)return[];const configured=reconcileProjectDocumentationConcepts(set,items.flatMap(({concept})=>typeof concept==="string"&&concept.trim()?[concept]:[]));return configured.flatMap(({name,included})=>{if(!included)return[];const grouped=items.filter(({concept})=>String(concept??"").trim().toLocaleLowerCase()===name.toLocaleLowerCase());return grouped.length?[{name,rows:grouped}]:[];});};
