...
export function focusedRuleIssue(rule:Record<string,unknown>):string|undefined {
  if(rule.condition!==undefined&&["presence","value","pattern","range","cardinality","reusable"].includes(String(rule.kind))&&unresolvedConditionProperty(rule.condition))return"Resolve the When condition.";
