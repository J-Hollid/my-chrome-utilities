...
async function prepareArchive(input:{project:SpecificationProject;publishedProject?:SpecificationProject;assets:readonly FlowVisualArchiveAsset[]}){
  ...
  for(const asset of ordered){validateFlowVisualMetadata(asset.metadata);const entry=`assets/${asset.metadata.digest.slice(7)}.${extension(asset.metadata.mediaType)}`,prior=uniqueBodies.get(entry);if(prior&&(prior.metadata.digest!==asset.metadata.digest||prior.metadata.mediaType!==asset.metadata.mediaType||prior.metadata.byteLength!==asset.metadata.byteLength))throw new DOMException(`Digest-addressed entry ${entry} has inconsistent metadata.`,"DataError");uniqueBodies.set(entry,prior??asset);assets.push({...asset.metadata,entry});}
...
export async function writeFlowVisualArchive(input:{project:SpecificationProject;publishedProject?:SpecificationProject;assets:readonly FlowVisualArchiveAsset[]},sink:FlowVisualZipSink,options:{signal?:AbortSignal;onProgress?:(progress:FlowVisualArchiveProgress)=>void}={}):Promise<{bytesWritten:number;entryCount:number;maxChunkBytes:number}>{
  const prepared=await prepareArchive(input),entries=async function*():AsyncGenerator<FlowVisualZipEntry>{yield{name:"manifest.json",body:jsonBlob(prepared.manifest)};yield{name:"draft.json",body:jsonBlob(prepared.project)};if(prepared.publishedProject)yield{name:"published.json",body:jsonBlob(prepared.publishedProject)};let validated=0;for(const[name,asset]of prepared.uniqueBodies){if(options.signal?.aborted)throw new DOMException("Project archive work was cancelled.","AbortError");const body=typeof asset.body==="function"?await asset.body():asset.body;await validateFlowVisualBody(asset.metadata,body);options.onProgress?.({phase:"validate",entry:name,completed:++validated,total:prepared.uniqueBodies.size});yield{name,body};}}();
...
export async function createFlowVisualArchive(input:{project:SpecificationProject;publishedProject?:SpecificationProject;assets:readonly FlowVisualArchiveAsset[]}):Promise<Uint8Array>{
...
export async function estimateFlowVisualArchiveSize(input:{project:SpecificationProject;publishedProject?:SpecificationProject;assets:readonly FlowVisualArchiveAsset[]}):Promise<number>{
  const prepared=await prepareArchive(input),entries=[{name:"manifest.json",size:jsonBlob(prepared.manifest).size},{name:"draft.json",size:jsonBlob(prepared.project).size},...(prepared.publishedProject?[{name:"published.json",size:jsonBlob(prepared.publishedProject).size}]:[]),...[...prepared.uniqueBodies].map(([name,asset])=>({name,size:asset.metadata.byteLength}))];return entries.reduce((total,{name,size})=>total+size+76+2*encoder.encode(name).length,22);
}
...
function assertArchiveManifest(value:ArchiveManifest):void{if(!value||typeof value!=="object"||value.format!=="my-chrome-utilities.project-archive"||value.version!==3)throw new DOMException("Use a supported project archive version.","NotSupportedError");if(!Array.isArray(value.requiredFeatures)||(value.requiredFeatures).some(feature=>feature!=="digest-addressed-visual-assets"))throw new DOMException("The project archive requires unsupported features.","NotSupportedError");if(value.draftEntry!=="draft.json"||(value.publishedEntry!==undefined&&value.publishedEntry!=="published.json")||!Array.isArray(value.assets)||value.assets.length>10_000)throw new DOMException("The project archive manifest has an unsupported structure.","DataError");for(const asset of value.assets){validateFlowVisualMetadata(asset);if(asset.entry!==`assets/${asset.digest.slice(7)}.${extension(asset.mediaType)}`)throw new DOMException(`Visual ${asset.id} does not use its digest-addressed archive entry.`,"DataError");}}
