...
export interface FlowVisualArchiveImport {formatVersion:3;project:SpecificationProject;publishedProject?:SpecificationProject;assets:FlowVisualStoredAsset[];templateBodies:StoredDocumentationTemplateBody[];migrations:string[];}
...
export async function migrateVersion2VisualAssets(bundle:{format:string;version:number;project:SpecificationProject;publishedProject?:SpecificationProject},input:{projectId:string;id:(oldId:string)=>string}):Promise<FlowVisualArchiveImport>{
  ...
  return{formatVersion:3,project:remap(withoutEmbeddedBodies(source),mapping) as SpecificationProject,...(bundle.publishedProject?{publishedProject:remap(withoutEmbeddedBodies(bundle.publishedProject),mapping) as SpecificationProject}:{}),assets,templateBodies:[],migrations:["Embedded concept visuals moved to separate original Blob bodies"]};
}
