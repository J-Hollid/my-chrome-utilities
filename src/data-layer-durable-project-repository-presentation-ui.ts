...
export function createDurableRepositoryPresentation(root:ParentNode,callbacks:DurableRepositoryPresentationCallbacks):DurableRepositoryPresentation{
  const ownerDocument=(root as ParentNode&{ownerDocument?:Document}).ownerDocument,
    dialog=root.querySelector<HTMLDialogElement>("#durable-storage-recovery")??ownerDocument?.querySelector<HTMLDialogElement>("#durable-storage-recovery");
  if(!dialog)throw new Error("Missing durable repository control #durable-storage-recovery.");
  const status=q<HTMLOutputElement>(root,"#durable-repository-status"),open=q<HTMLButtonElement>(root,"#open-storage-recovery"),close=q<HTMLButtonElement>(dialog,"#close-storage-recovery"),retry=q<HTMLButtonElement>(dialog,"#retry-durable-save"),reject=q<HTMLButtonElement>(dialog,"#reject-durable-save"),exportUnsaved=q<HTMLButtonElement>(dialog,"#export-unsaved-draft"),exportBackup=q<HTMLButtonElement>(dialog,"#export-repository-backup"),diagnose=q<HTMLButtonElement>(dialog,"#open-storage-diagnostics"),reviewDeleteBackup=q<HTMLButtonElement>(dialog,"#review-delete-migration-backup"),deleteBackupReview=q<HTMLElement>(dialog,"#delete-migration-backup-review"),cancelDeleteBackup=q<HTMLButtonElement>(dialog,"#cancel-delete-migration-backup"),confirmDeleteBackup=q<HTMLButtonElement>(dialog,"#confirm-delete-migration-backup"),result=q<HTMLOutputElement>(dialog,"#durable-recovery-result");
  ...
  return{status(message){status.textContent=message;},result(message,focus=false){result.textContent=message;if(focus)result.focus();},
    ...
    diagnostics(input){const display=durableStorageDiagnosticsDisplay(input);q<HTMLElement>(dialog,"#durable-last-saved").textContent=display.lastSavedAt;q<HTMLElement>(dialog,"#durable-published-revision").textContent=display.publishedRevision;q<HTMLElement>(dialog,"#durable-unsaved-command").textContent=display.unsavedCommand;q<HTMLElement>(dialog,"#durable-project-size").textContent=display.projectSize;q<HTMLElement>(dialog,"#durable-release-size").textContent=display.releaseSize;q<HTMLElement>(dialog,"#durable-fixture-size").textContent=display.fixtureSize;q<HTMLElement>(dialog,"#durable-migration-backup-size").textContent=display.migrationBackupSize;q<HTMLElement>(dialog,"#durable-browser-estimate").textContent=display.browserEstimate;q<HTMLElement>(dialog,"#durable-storage-explanation").textContent=display.explanation;},
