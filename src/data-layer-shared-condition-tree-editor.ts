...
export function renderSharedConditionTree(host:HTMLElement,options:SharedConditionTreeOptions):void {
  ...
  const render=():void=>{
    ...
    const valueControl=(row:SharedFlatConditionDraft):HTMLElement=>{
      ...
      const update=()=>{try{const text=control.value;if(control instanceof HTMLSelectElement)row.value=text===""?undefined:text==="true";else if(multi)row.value=text.split(",").map((value)=>value.trim()).filter(Boolean).map((value)=>typedValue(entry.type==="array"?"string":entry.type,value));else row.value=text.trim()===""?undefined:typedValue(entry.type,text);control.setCustomValidity("");emit();}catch(error){control.setCustomValidity(error instanceof Error?error.message:String(error));emit();}};control.addEventListener("input",update);control.addEventListener("change",update);valueHost.append(control);return valueHost;
    };
