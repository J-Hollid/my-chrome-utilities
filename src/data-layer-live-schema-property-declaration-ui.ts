...
export function openLiveSchemaPropertyDeclarationDialog(
  input: LiveSchemaPropertyDeclarationDialogInput,
): void {
  ...
  const close = (restoreFocus = true) => {
    ...
    dialog.remove();
    if (restoreFocus) input.trigger.focus({ preventScroll:true });
  };
  const showReview = (schema: SchemaDefinition) => {
    const declaration = createLiveSchemaPropertyDeclaration(
      input.payload,
      input.concretePath,
      schema,
    );
  ...
  if (input.selected) showReview(input.selected);
  else showDestinations();
  dialog.showModal();
  dialog.querySelector<HTMLElement>("h5")?.focus({ preventScroll:true });
}
