...
function renderRuleEditor(row:HTMLElement,rule:Record<string,unknown>,index:number,context:ComposedFocusedSectionContext):void {
  ...
  const renderWhen=():void=>{when.replaceChildren();if(!rule.condition){when.append(button(dom,"Add When",()=>{const tree=dom.createElement("div");renderSharedConditionTree(tree,{dom,properties,id:(kind)=>`${kind}:${crypto.randomUUID()}`,onChange:(next)=>{if(next)rule.condition=next as unknown as Record<string,unknown>;else delete rule.condition;}});when.replaceChildren(tree,button(dom,"Remove When",()=>{delete rule.condition;renderWhen();}));}));return;}const tree=dom.createElement("div");renderSharedConditionTree(tree,{dom,condition:rule.condition as any,properties,id:(kind)=>`${kind}:${crypto.randomUUID()}`,onChange:(next)=>{if(next)rule.condition=next as unknown as Record<string,unknown>;else delete rule.condition;}});when.append(tree,button(dom,"Remove When",()=>{delete rule.condition;renderWhen();}));};renderWhen();editor.append(when);
  for(const field of focusedRuleFields(String(rule.kind??"reusable"))){
    ...
    if(field==="ordinaryValue"){const control=dom.createElement("input");control.name="editRuleOrdinaryValue";control.value=schemaTableAllowedValues(rule);control.addEventListener("input",()=>{const draft=context.getDraft();if(!draft)return;const next=clone(draft.rules[index] as Record<string,unknown>);delete next.expectedValue;next.allowedValues=schemaTableStageAllowedValues(Array.isArray(next.allowedValues)?next.allowedValues:[],control.value,(draft.type??context.row.effective.type) as CanonicalPropertyType|undefined);draft.rules[index]=next;});editor.append(labeled(dom,"Then allowed values",control));continue;}
...
function renderRuleInventory(host:HTMLElement,context:ComposedFocusedSectionContext):void {
  ...
  [...displayedById.values()].forEach((rule,index)=>{
    ...
    row.dataset.ruleId=id;row.dataset.ownership=local?"local":"inherited";summary.textContent=`${String(rule.name??rule.kind??"rule")} · ${schemaTableRuleConditionSummary(rule.condition as any,properties)} · Then ${schemaTableRuleOutcomeSummary(rule)} · ${String(rule.severity??"error")} · ${String(rule.message??"No issue message")} · source ${local?"local":context.row.source} · ${local?"local":"inherited"}${removed?" · Removed":""}`;
...
function renderAddPanel(host:HTMLElement,context:ComposedFocusedSectionContext):void {
  ...
  let condition:Record<string,unknown>|undefined,whenEnabled=false;
  ...
  const candidate=():Record<string,unknown>|undefined=>{
    ...
    const ordinary=fields.querySelector<HTMLInputElement>("[name=\"newRuleOrdinaryValue\"]");if(ordinary?.value)rule.allowedValues=schemaTableStageAllowedValues([],ordinary.value,(draft.type??context.row.effective.type) as CanonicalPropertyType|undefined);
  ...
  const renderFields=()=>{
    fields.replaceChildren();condition=undefined;whenEnabled=false;if(!kind.value){validate();return;}const name=dom.createElement("input");name.name="newRuleName";name.addEventListener("input",validate);fields.append(labeled(dom,"Rule name",name));const when=dom.createElement("div"),properties=()=>context.model.rows.map(({path,effective})=>({id:effective.definitionId??path,name:path.split("/").filter(Boolean).at(-1)??path,...(effective.type?{type:effective.type}:{})}));const renderWhen=()=>{when.replaceChildren();if(!whenEnabled){when.append(button(dom,"Add When",()=>{whenEnabled=true;renderWhen();validate();}));return;}const tree=dom.createElement("div");renderSharedConditionTree(tree,{dom,...(condition?{condition:condition as any}:{}),properties,id:(prefix)=>`${prefix}:${crypto.randomUUID()}`,onChange:(next)=>{condition=next as unknown as Record<string,unknown>|undefined;validate();}});when.append(tree,button(dom,"Remove When",()=>{whenEnabled=false;condition=undefined;renderWhen();validate();}));};renderWhen();fields.append(when);
