...
function renderRuleInventory(host:HTMLElement,context:ComposedFocusedSectionContext):void {
  ...
  [...displayedById.values()].forEach((rule,index)=>{
    ...
    row.dataset.ruleId=id;row.dataset.ownership=local?"local":"inherited";summary.textContent=`${String(rule.name??rule.kind??"rule")} · ${String(rule.kind??"reusable")} · ${String(rule.severity??"error")} · ${String(rule.message??"No issue message")} · ${local?"local":"inherited"}${removed?" · Removed":""}`;
...
function renderAddPanel(host:HTMLElement,context:ComposedFocusedSectionContext):void {
  ...
  legend.textContent="Add rule";status.setAttribute("role","status");kind.name="ruleKind";kind.append(new Option("Choose rule kind",""),...["pattern","range","cardinality","condition","reusable","custom"].map((entry)=>new Option(entry,entry)));
  const candidate=():Record<string,unknown>|undefined=>{
    if(!kind.value)return undefined;const rule:Record<string,unknown>={id:"staged-rule",kind:kind.value,severity:"error"};
    for(const field of ["pattern","minimum","maximum","minItems","maxItems","message"]){const control=fields.querySelector<HTMLInputElement>(`[name="newRule${field}"]`);if(control&&control.value!=="")rule[field]=numericFields.has(field)?Number(control.value):control.value;}
  ...
  const renderFields=()=>{
    ...
    for(const field of focusedRuleFields(kind.value)){
      ...
      const control=field==="severity"?dom.createElement("select"):dom.createElement("input");control.name=`newRule${field}`;if(control instanceof HTMLSelectElement)control.append(new Option("error","error"),new Option("warning","warning"));else if(numericFields.has(field))control.type="number";control.addEventListener("input",validate);fields.append(labeled(dom,field,control));
    }
    validate();
  };
