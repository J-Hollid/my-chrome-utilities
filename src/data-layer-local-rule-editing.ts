...
export interface LocalRuleEditInspection {
  origin: "Local rule";
...
export interface LocalRuleEdit {
  propertyPath: string;
  ruleId: string;
  rule: AttachedSchemaRule;
}
...
function editableRules(schema: SchemaDefinition): readonly AttachedSchemaRule[] {
  return schema.workingDraft?.attachedRules ?? schema.attachedRules ?? [];
}

function validateEditedRule(rule: AttachedSchemaRule): void {
  const operator = rule.operator?.replaceAll("_", "-").toLowerCase();
  if (operator === "regular-expression") {
    try { new RegExp(rule.parameters ?? ""); }
    catch { throw new Error("Correct the regular expression"); }
  }
...
export function inspectLocalRuleEdit(
  schema: SchemaDefinition,
  propertyPath: string,
  ruleId: string,
  reusableRuleIds: ReadonlySet<string> = new Set(),
): LocalRuleEditInspection {
  ...
  if (attachmentIndex < 0) throw new Error(`Rule ${ruleId} does not exist at ${canonicalPath}`);
  return { origin:"Local rule", canonicalPath, attachmentIndex, rule:clone(rules[attachmentIndex]!) };
}

export function saveLocalRuleEdit(schema: SchemaDefinition, edit: LocalRuleEdit): SchemaDefinition {
  const inspection = inspectLocalRuleEdit(schema, edit.propertyPath, edit.ruleId);
  validateEditedRule(edit.rule);
  if (edit.rule.id !== edit.ruleId) throw new Error("A local rule edit cannot change its stable identity");
  ...
  return updateSchemaWorkingDraft(
    withDraft,
    { attachedRules },
    `Edit ${name} at ${inspection.canonicalPath}`,
  );
}
