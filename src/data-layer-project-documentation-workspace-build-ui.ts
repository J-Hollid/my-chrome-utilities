...
export function createDocumentationSectionConfigurationRenderer(mutateSection:MutateSection):(host:HTMLElement,set:ProjectDocumentationSet,section:ProjectDocumentationSection|undefined,available:ProjectDocumentationSources)=>void {
  ...
  const renderFlow=(host:HTMLElement,set:ProjectDocumentationSet,section:ProjectDocumentationSection,available:ProjectDocumentationSources):void=>{
    ...
    if(!source){host.append("Flow unavailable.");return;}
    const contexts=source.snapshot.contexts.map(({id,pageName,eventName})=>({id,label:`${pageName} / ${eventName}`})),contextIds=checkedOrder(contexts.map(({id})=>id),section.configuration?.contextIds),paths=checkedOrder(flowDocumentationPropertyPaths(source.snapshot),section.configuration?.paths),metadata=(section.configuration?.columns??[]) as FlowDocumentationMetadata[];
    ...
    for(const context of contexts.filter(({id})=>contextIds.includes(id))){const input=controlInput(`label:${context.id}`,section.configuration?.labels?.[context.id]??"");input.setAttribute("aria-label",`Documentation label for ${context.label}`);input.addEventListener("change",()=>mutateSection(set,section.id,(value)=>({...value,configuration:{...value.configuration,labels:{...value.configuration?.labels,[context.id]:input.value}}}),`Label ${context.label}`));labels.append(labelled(context.label,input));}
    ...
    for(const [id,label] of [["description","Description"],["type","Type"],["allowedValues","Allowed values"],["example","Example"],["comments","Comments"]] as const){const check=document.createElement("input");check.type="checkbox";declareStudioChoice(check,"documentation.metadata-column");check.checked=metadata.includes(id);check.addEventListener("change",()=>mutateSection(set,section.id,(value)=>({...value,configuration:{...value.configuration,columns:setChecked(metadata,id,check.checked)}}),`Configure Flow metadata ${label}`));metadataGroup.append(labelled(label,check));}
    host.append(contextGroup,pathGroup,metadataGroup);
  };
  const renderMatrix=(host:HTMLElement,set:ProjectDocumentationSet,section:ProjectDocumentationSection,available:ProjectDocumentationSources):void=>{
    ...
    const draw=()=>{tree.replaceChildren();const matches=available.matrixContexts.filter(({searchText,label})=>`${searchText} ${label}`.toLowerCase().includes(search.value.trim().toLowerCase())),groups=new Map<string,Map<string,ProjectDocumentationMatrixContext[]>>();for(const context of matches){const parents=groups.get(context.groupLabel)??new Map<string,ProjectDocumentationMatrixContext[]>(),items=parents.get(context.parentLabel)??[];items.push(context);parents.set(context.parentLabel,items);groups.set(context.groupLabel,parents);}for(const[group,parents]of groups){const groupSet=document.createElement("fieldset");groupSet.dataset.matrixGroup=group;groupSet.append(Object.assign(document.createElement("legend"),{textContent:group}));for(const[parent,contexts]of parents){const sectionHost=document.createElement("section");sectionHost.dataset.matrixParent=parent;sectionHost.append(heading(4,parent));for(const context of contexts){const check=document.createElement("input");check.type="checkbox";declareStudioChoice(check,"documentation.matrix-context");check.checked=selected.includes(context.id);check.dataset.matrixContextId=context.id;check.dataset.matrixContextKind=context.kind;check.addEventListener("change",()=>mutateSection(set,section.id,(value)=>({...value,configuration:{...value.configuration,contextIds:setChecked(selected,context.id,check.checked)}}),`Configure matrix ${context.label}`));sectionHost.append(labelled(context.label,check));}groupSet.append(sectionHost);}tree.append(groupSet);}};
  ...
  const renderProfile=(host:HTMLElement,set:ProjectDocumentationSet,section:ProjectDocumentationSection,available:ProjectDocumentationSources):void=>{
    ...
    if(!profile){host.append("Site Profile unavailable.");return;}
    const allPaths=projectDocumentationProfilePaths(profile),paths=checkedOrder(allPaths,section.configuration?.paths),columns=checkedOrder(projectDocumentationProfileColumns(),section.configuration?.columns) as ProjectDocumentationProfileColumn[];
  ...
  return(host,set,section,available)=>{
    ...
    if(section?.kind==="flow")renderFlow(host,set,section,available);
    else if(section?.kind==="matrix")renderMatrix(host,set,section,available);
    else if(section?.kind==="profile")renderProfile(host,set,section,available);
    else host.append(heading(3,"Configure Overview"),Object.assign(document.createElement("p"),{textContent:"Overview derives the project name, purpose, and website."}));
  };
}
