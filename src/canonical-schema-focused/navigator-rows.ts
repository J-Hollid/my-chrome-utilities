...
import {schemaTableCellMetadata,schemaTableColumns,schemaTableExpectedOrAllowed,type SchemaTableEditableFacet} from "../data-layer-schema-table.js";
...
function renderTable(tree:HTMLElement,context:CanonicalSchemaRenderContext):void {
  ...
  for(const row of canonicalNavigatorRows(context)){const node=context.working?.id===row.id?context.working:row.node,tr=dom.createElement("tr"),identity=cell(0),name=dom.createElement("span"),trigger=button(dom,"⋯",()=>context.openProperty(row.node,trigger)),example=node.documentation.example.value,states=node.provenance.map(({state})=>state).filter(Boolean);tr.dataset.propertyRow="true";tr.dataset.propertyId=row.id;identity.style.position="relative";name.textContent=`${node.name} · `;trigger.setAttribute("aria-label",`Property actions for ${row.path}`);trigger.dataset.propertyActionsPath=row.path;identity.append(name,trigger);tr.append(identity,cell(1,row.path),cell(2,node.type),cell(3,node.presence.mode));
    ...
    if(context.menuPropertyId===row.id){const overlay=dom.createElement("section");overlay.dataset.schemaRowOverlay="true";overlay.setAttribute("aria-label",`${row.path} property overlay`);overlay.style.cssText="position:absolute;left:0;top:100%;z-index:10;width:min(42rem,calc(100vw - 2rem));max-width:calc(100vw - 2rem);box-sizing:border-box;overflow:auto;background:Canvas;border:1px solid ButtonBorder;padding:0.75rem;";overlay.append(context.renderMenu(row.node));if(context.focusedPropertyId===row.id){overlay.append(context.renderFocusedEditor(context.document,row.node));if(context.review)overlay.append(context.review);}identity.append(overlay);}
    body.append(tr);
  }
  table.replaceChildren(head,body);table.setAttribute("aria-label","Canonical property table");table.dataset.canonicalView="table";tree.replaceChildren(table);tree.dataset.canonicalView="table";
}
