...
export function mountCanonicalPredicateEditor(options:CanonicalPredicateEditorOptions):void {
  ...
  let draft:CanonicalPredicate|undefined = options.condition ? clone(options.condition) : undefined;
  const render = ():void => {
    ...
    save.type="button";save.textContent=options.saveLabel;save.addEventListener("click",()=>{
      if(!draft) {
        assistance.textContent="Add a condition or group.";
        tree.querySelector<HTMLElement>("button, select, input")?.focus({preventScroll:true});
        return;
      }
      ...
      if(!validation.ready) {
        ...
        const control=tree.querySelector<HTMLElement>(`[data-condition-path="${key}"] select, [data-condition-path="${key}"] input`);
