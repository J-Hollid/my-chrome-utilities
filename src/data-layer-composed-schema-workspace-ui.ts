...
import {schemaTableOverlayTarget,schemaTableOverlayTransition,schemaTableReplaceExpectedOrAllowed,schemaTableSortComparison,schemaTableSortOptions,schemaTableStageAllowedValues,type SchemaTableEditableFacet,type SchemaTableOverlayState,type SchemaTableQuickEditResult,type SchemaTableSortMode} from "./data-layer-schema-table.js";
...
export function mountComposedSchemaWorkspace(options:ComposedSchemaWorkspaceUiOptions):HTMLElement {
  ...
  filter.type="search";filter.placeholder="Filter properties";filter.setAttribute("aria-label","Filter schema properties");sort.setAttribute("aria-label","Sort schema properties");sort.append(...schemaTableSortOptions.map(({label,value})=>new Option(label,value)));filterControls.setAttribute("aria-label","Schema property navigation controls");filterControls.append(filter,sort);
  ...
  const visibleModel=():ComposedSchemaWorkspace=>{const needle=query.trim().toLowerCase(),rows=options.model.rows.filter((row)=>!needle||row.path.toLowerCase().includes(needle)||String(row.source??"").toLowerCase().includes(needle)||options.effectiveText(row).toLowerCase().includes(needle)).sort((left,right)=>schemaTableSortComparison({path:left.path,concept:left.effective.concept,source:left.source},{path:right.path,concept:right.effective.concept,source:right.source},sortMode));return{...options.model,rows};};
  const rerender=():void=>renderComposedRows(rows,{dom:document,overlayHost:section,model:visibleModel(),effectiveText:options.effectiveText,conceptSuggestions:options.conceptSuggestions,...(options.onRepair?{onRepair:options.onRepair}:{}),...(options.onStructure?{onStructure:(kind,path,name)=>{pendingStructure.push({kind,path,...(name===undefined?{}:{name})});rerender();}}:{}),...(options.rowPathDataset?{rowPathDataset:options.rowPathDataset}:{}),activePath,overlayOpen,focusedOpen,reviewOpen,activeSection,draft,removed,confirmedAction,removedRuleIds,removedValueIds,restoredRuleIds,restoredValueIds,stagedLocalValueIds,overriddenRuleIds,overrideRule,pendingAction,pendingStructure,ownershipSession,activateOwnership:(action)=>{ownershipSession=activateFocusedOwnershipSection(ownershipSession,activeSection as FocusedPropertyPrimarySection,action);rerender();},beginAction,cancelAction,confirmAction,open,commitInline,resetInline,cancelInline:()=>{},inlineDiagnostic:(message)=>{quickEditFeedback.textContent=message;},quickEditRoot:()=>options.host,quickEditScope:`composed:${options.schemaContributorId??options.model.heading}`,close,closeChild,beginReview,cancelReview,save,render:rerender,selectSection:(value)=>{activeSection=value;focusedOpen=true;reviewOpen=false;overlayState=schemaTableOverlayTransition(overlayState,{kind:"focus"});rerender();}});
  ...
  const open=(row:ComposedSchemaRow,focus?:HTMLElement,sectionName:FocusedPropertySection="definition"):void=>{if(activePath!==row.path||!draft){activePath=row.path;draft=composedFacetDraft(row.local,row.effective);removed=false;confirmedAction=undefined;removedRuleIds=new Set();removedValueIds=new Set();restoredRuleIds=new Set();restoredValueIds=new Set();stagedLocalValueIds=new Set();overriddenRuleIds=new Set();pendingStructure=[];pendingAction=undefined;ownershipSession=focusedOwnershipState(composedSchemaRowOwnershipInput(row)).session;}overlayState=schemaTableOverlayTransition(overlayState,{kind:"open",path:row.path});activeSection=sectionName;overlayOpen=true;focusedOpen=false;reviewOpen=false;if(focus){originFocus=focus;originPath=row.path;}rerender();};
