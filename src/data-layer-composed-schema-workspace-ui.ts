...
export function mountComposedSchemaWorkspace(options:ComposedSchemaWorkspaceUiOptions):HTMLElement {
  ...
  const visibleModel=():ComposedSchemaWorkspace=>{const needle=query.trim().toLowerCase(),rows=options.model.rows.filter((row)=>!needle||row.path.toLowerCase().includes(needle)||String(row.source??"").toLowerCase().includes(needle)||options.effectiveText(row).toLowerCase().includes(needle)).sort((left,right)=>sortMode==="source"?String(left.source??"").localeCompare(String(right.source??""))||left.path.localeCompare(right.path):sortMode==="validation"?left.validationState.localeCompare(right.validationState)||left.path.localeCompare(right.path):left.path.localeCompare(right.path));return{...options.model,rows};};
