...
export function mountComposedSchemaWorkspace(options:ComposedSchemaWorkspaceUiOptions):HTMLElement {
  ...
  const open=(row:ComposedSchemaRow,focus?:HTMLElement,sectionName:FocusedPropertySection="definition"):void=>{if(activePath!==row.path||!draft){activePath=row.path;draft=composedFacetDraft(row.local,row.effective);removed=false;confirmedAction=undefined;removedRuleIds=new Set();removedValueIds=new Set();restoredRuleIds=new Set();restoredValueIds=new Set();stagedLocalValueIds=new Set();overriddenRuleIds=new Set();pendingStructure=[];pendingAction=undefined;ownershipSession=focusedOwnershipState(composedSchemaRowOwnershipInput(row)).session;}overlayState=schemaTableOverlayTransition(overlayState,{kind:"open",path:row.path});activeSection=sectionName;overlayOpen=true;focusedOpen=false;reviewOpen=false;saveIssue=undefined;if(focus){originFocus=focus;originPath=row.path;}rerender();};
  ...
  const beginReview=():void=>{reviewOpen=true;saveIssue=undefined;overlayState=schemaTableOverlayTransition(overlayState,{kind:"review"});rerender();};
  ...
  const save=(row:ComposedSchemaRow):void=>{if(!draft)return;try{if(removed){options.onReset(row);close();return;}const staged=composedFacetDraftWithoutRemovedItems(draft,removedRuleIds,removedValueIds);options.onSave(row,sparseComposedFacets(staged,row.inherited??{path:row.path}),pendingStructure);close();}catch(error){saveIssue=error instanceof Error?error.message:String(error);rerender();}};
