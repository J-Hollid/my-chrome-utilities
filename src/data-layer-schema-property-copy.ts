...
export function planSchemaPropertyCopy(options:{
  ...
  const documentation=propertyPaths.flatMap((path):PlannedSchemaPropertyDocumentation[]=>{
    const entry=effectiveDocumentation.properties[path]; return entry?[{path,entry:{displayName:entry.displayName,description:entry.description,...(entry.comments?{comments:entry.comments}:{}),...(entry.example?{example:clone(entry.example)}:{})},origin:{...entry.origin}}]:[];
  });
