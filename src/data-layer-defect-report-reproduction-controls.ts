...
function appendStepPresentation(
  ...
  if(actions.style)actions.style.flexBasis="100%";if (guidance){guidance.className = "defect-reproduction-step-guidance";if(guidance.style)guidance.style.flexBasis="100%";}
...
export function appendReproductionControls<Report extends ComposableDefectReport>(
  ...
  renderSteps = () => {
    ...
    const stepIdentity=(candidate:ReproductionStep,candidateIndex:number)=>candidate.kind==="manual"?`manual:${candidate.id}`:`${candidate.kind}:${candidate.visitId}:${candidateIndex}`;
    ...
    const rendered = currentSteps.map((step, index) => {
      ...
      if (step.kind === "manual") {
        ...
        const reorder=renderReorderControl({focusScopeId:`defect-reproduction:${step.visitId}`,itemId:`manual:${step.id}`,itemLabel:step.text.replace(/^\d+\.\s*/,""),completeOrder:scopedOrder,
          ...
          scopeLabel:step.pathname,dropTarget:item,orderedContainer:steps,onMove:({itemId,fromIndex,toIndex})=>{let next=currentSteps;const direction=toIndex<fromIndex?"earlier":"later";for(let count=Math.abs(toIndex-fromIndex);count>0;count-=1)next=moveManualReproductionStep(next,itemId.replace(/^manual:/u,""),direction);updateSteps(next);return true;}});
