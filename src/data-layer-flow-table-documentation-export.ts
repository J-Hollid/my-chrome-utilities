...
export function configureFlowDocumentationTable(snapshot:FlowDocumentationSnapshot,kind:"values"|"matrix",configuration:FlowDocumentationTableConfiguration={}):FlowDocumentationTable{
  ...
  const rows=selected.flatMap((path)=>{const sourceRow=rowsByPath.get(flowDocumentationDisplayPath(path));if(!sourceRow)return[];return[[sourceRow[0]!,...metadata.map((column)=>metadataValue(snapshot,path,column)),...sourceRow.slice(1)]];});
...
export function writeFlowDocumentationWorkbook(snapshot:FlowDocumentationSnapshot,input:{valueTable:FlowDocumentationTable;matrixTable:FlowDocumentationTable}):Uint8Array{
  const legend:FlowDocumentationTable={title:snapshot.title,headings:["Symbol or context","Meaning or provenance"],rows:[["M","Mandatory"],["O","Optional"],["C","Conditional"],["N","Not expected"],["—","Not defined"],["!","Blocked"],...snapshot.contexts.map((context)=>[contextName(context),contextProvenance(context)])]},diagnostics:FlowDocumentationTable={title:snapshot.title,headings:["Source state","Context","Property","Issue","Repair","Generated"],rows:snapshot.diagnostics.length?snapshot.diagnostics.map((item)=>[snapshot.incomplete?"Draft — incomplete":snapshot.sourceState,item.contextName,item.path,item.issue,item.repair,snapshot.generatedAt]):[[snapshot.incomplete?"Draft — incomplete":snapshot.sourceState,"All selected contexts","","No export blockers","",snapshot.generatedAt]]},names=["Flow values","Capture matrix","Legend and provenance","Export diagnostics"],tables=[input.valueTable,input.matrixTable,legend,diagnostics];
