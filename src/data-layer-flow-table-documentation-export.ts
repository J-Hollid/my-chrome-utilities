...
export function documentationWorksheet(table:FlowDocumentationTable):string{const groups=new Map(table.conceptGroups?.map((group)=>[group.start,group])??[]),rows=[[table.title,...Array(Math.max(0,table.headings.length-1)).fill("")],table.headings,...table.rows.flatMap((row,index)=>{const group=groups.get(index);return[...(group?[[group.name,...Array(Math.max(0,table.headings.length-1)).fill("")]]:[]),row];})],conceptRows=new Set<number>();let offset=2;for(const [index] of table.rows.entries()){if(groups.has(index)){conceptRows.add(offset);offset+=1;}offset+=1;}const body=rows.map((row,rowIndex)=>`<row r="${rowIndex+1}"${conceptRows.has(rowIndex)?" data-concept-heading=\"true\"":""}>${row.map((value,columnIndex)=>`<c r="${columnName(columnIndex)}${rowIndex+1}" t="inlineStr"><is><t xml:space="preserve">${xml(value)}</t></is></c>`).join("")}</row>`).join(""),merges=[...conceptRows].map((row)=>`<mergeCell ref="A${row+1}:${columnName(table.headings.length-1)}${row+1}"/>`).join("");return`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData>${body}</sheetData>${merges?`<mergeCells count="${conceptRows.size}">${merges}</mergeCells>`:""}</worksheet>`;}
