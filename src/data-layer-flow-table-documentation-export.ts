...
export interface FlowDocumentationContextInput{id:string;kind:FlowDocumentationContextKind;pageFrameId:string;occurrenceId?:string;pageName:string;eventName:string;stepLabel:string;effectiveRevision:number;compiled:CompiledLayeredSchema;unresolved?:readonly FlowDocumentationUnresolvedReference[]}
...
export function orderFlowDocumentationOccurrenceIds(
  ...
  relationships:readonly {sourceNodeId?:unknown;targetNodeId?:unknown;sourceEndpoint?:{kind?:unknown;id?:unknown};targetEndpoint?:{kind?:unknown;id?:unknown};kind?:unknown}[],
  pageGroupIds:readonly string[],
):{ids:string[];labels:Record<string,string>}{
  const occurrenceIds=new Set(occurrences.map(({id})=>id)),alternativeBySource=new Map<string,string[]>(),mergeByTarget=new Map<string,string[]>();
  for(const relationship of relationships){const source=String(relationship.sourceEndpoint?.id??relationship.sourceNodeId??""),target=String(relationship.targetEndpoint?.id??relationship.targetNodeId??"");if(!occurrenceIds.has(source)||!occurrenceIds.has(target)||source===target)continue;const index=relationship.kind==="alternative"?alternativeBySource:relationship.kind==="merge"?mergeByTarget:undefined;if(index)index.set(relationship.kind==="alternative"?source:target,[...(index.get(relationship.kind==="alternative"?source:target)??[]),relationship.kind==="alternative"?target:source]);}
  const candidates=[...alternativeBySource].flatMap(([source,rawBranches])=>{const branches=[...new Set(rawBranches)];if(branches.length!==2)return[];return[...mergeByTarget].flatMap(([target,rawSources])=>{const sources=[...new Set(rawSources)];return sources.length===2&&branches.every((id)=>sources.includes(id))&&source!==target?[{source,target,branches}]:[];});});
...
const plainCell=(value:unknown)=>{const safe=String(value??"").replace(/[\t\r\n]+/gu," ");return/^[=+\-@]/u.test(safe)?`'${safe}`:safe;};
export const conceptSectionHeading=(value:string):string=>value.trim().toLocaleUpperCase();
export function renderFlowDocumentationClipboard(table:FlowDocumentationTable,options:{includeHeadings?:boolean;style?:"plain"|"bordered"|"highlighted";documentTitle?:string;conceptStyle?:string;titleStyle?:string}={}):FlowDocumentationClipboard{
  const include=options.includeHeadings!==false,titleRow=options.documentTitle?[options.documentTitle,...Array(Math.max(0,table.headings.length-1)).fill("")]:undefined,groups=new Map(table.conceptGroups?.map((group)=>[group.start,group])??[]),grouped=table.rows.flatMap((row,index)=>{const group=groups.get(index);return[...(group?[{kind:"concept" as const,name:conceptSectionHeading(group.name)}]:[]),{kind:"row" as const,row}];}),rows=[...(titleRow?[titleRow]:[]),...(include?[table.headings]:[]),...grouped.map((entry)=>entry.kind==="concept"?[entry.name,...Array(Math.max(0,table.headings.length-1)).fill("")]:entry.row),...(table.legend?[["Legend",table.legend,...Array(Math.max(0,table.headings.length-2)).fill("")]]:[])],cellStyle=options.style&&options.style!=="plain"?' style="border:1px solid #666;padding:4px"':"",headingStyle=options.style==="highlighted"?' style="border:1px solid #666;padding:4px;font-weight:bold;background:#eee"':cellStyle,conceptStyle=options.conceptStyle?` style="${escapeHtml(options.conceptStyle)}"`:headingStyle,titleStyle=options.titleStyle?` style="${escapeHtml(options.titleStyle)}"`:"";
  const body=[...grouped,...(table.legend?[{kind:"row" as const,row:["Legend",table.legend,...Array(Math.max(0,table.headings.length-2)).fill("")]}]:[])].map((entry)=>entry.kind==="concept"?`<tr data-concept-heading="${escapeHtml(entry.name)}"><th colspan="${table.headings.length}" scope="rowgroup"${conceptStyle}>${escapeHtml(entry.name)}</th></tr>`:`<tr>${entry.row.map((value)=>`<td${cellStyle}>${escapeHtml(value)}</td>`).join("")}</tr>`).join(""),html=`<table${options.style&&options.style!=="plain"?' style="border-collapse:collapse"':""}>${options.documentTitle?`<caption${titleStyle}>${escapeHtml(options.documentTitle)}</caption>`:""}${include?`<thead><tr>${table.headings.map((value)=>`<th${headingStyle}>${escapeHtml(value)}</th>`).join("")}</tr></thead>`:""}<tbody>${body}</tbody></table>`;
  return{plain:rows.map((row)=>row.map(plainCell).join("\t")).join("\n"),html};
}

export function flowDocumentationSnapshotStale(snapshot:FlowDocumentationSnapshot,current:{graphRevision:number;contextRevisions:Readonly<Record<string,number>>}):{stale:boolean;changedContexts:string[];graphChanged:boolean}{
  const changedContexts=snapshot.contexts.filter((context)=>current.contextRevisions[context.id]!==context.effectiveRevision).map(contextName),graphChanged=current.graphRevision!==snapshot.graphRevision;return{stale:graphChanged||changedContexts.length>0,changedContexts,graphChanged};
}
...
export function documentationWorksheet(table:FlowDocumentationTable):string{const groups=new Map(table.conceptGroups?.map((group)=>[group.start,group])??[]),entries=[{kind:"title" as const,row:[table.title,...Array(Math.max(0,table.headings.length-1)).fill("")]},{kind:"columns" as const,row:table.headings},...table.rows.flatMap((row,index)=>{const group=groups.get(index);return[...(group?[{kind:"concept" as const,row:[conceptSectionHeading(group.name),...Array(Math.max(0,table.headings.length-1)).fill("")]}]:[]),{kind:"row" as const,row}];})],conceptRows=new Set(entries.flatMap((entry,index)=>entry.kind==="concept"?[index]:[])),body=entries.map((entry,rowIndex)=>`<row r="${rowIndex+1}"${conceptRows.has(rowIndex)?" data-concept-heading=\"true\"":""}>${entry.row.map((value,columnIndex)=>`<c r="${columnName(columnIndex)}${rowIndex+1}" t="inlineStr"><is><t xml:space="preserve">${xml(value)}</t></is></c>`).join("")}</row>`).join(""),merges=[...conceptRows].map((row)=>`<mergeCell ref="A${row+1}:${columnName(table.headings.length-1)}${row+1}"/>`).join("");return`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData>${body}</sheetData>${merges?`<mergeCells count="${conceptRows.size}">${merges}</mergeCells>`:""}</worksheet>`;}
...
export function zipDocumentationFiles(files:readonly {name:string;content:string|Uint8Array}[]):Uint8Array{const encoder=new TextEncoder(),locals:Uint8Array[]=[],centrals:Uint8Array[]=[];let offset=0;for(const file of files){const name=encoder.encode(file.name),data=typeof file.content==="string"?encoder.encode(file.content):file.content,crc=crc32(data),local=concat(u32(0x04034b50),u16(20),u16(0),u16(0),u16(0),u16(0),u32(crc),u32(data.length),u32(data.length),u16(name.length),u16(0),name),central=concat(u32(0x02014b50),u16(20),u16(20),u16(0),u16(0),u16(0),u16(0),u32(crc),u32(data.length),u32(data.length),u16(name.length),u16(0),u16(0),u16(0),u16(0),u32(0),u32(offset),name);locals.push(local,data);centrals.push(central);offset+=local.length+data.length;}const directory=concat(...centrals);return concat(...locals,directory,u32(0x06054b50),u16(0),u16(0),u16(files.length),u16(files.length),u32(directory.length),u32(offset),u16(0));}
export function writeFlowDocumentationWorkbook(snapshot:FlowDocumentationSnapshot,input:{valueTable:FlowDocumentationTable;matrixTable:FlowDocumentationTable}):Uint8Array{
  const legend:FlowDocumentationTable={title:snapshot.title,headings:["Symbol or context","Meaning or provenance"],rows:[["M","Mandatory"],["O","Optional"],["C","Conditional"],["N","Not expected"],["—","Not defined"],["!","Blocked"],...snapshot.contexts.map((context)=>[contextName(context),`effective revision ${context.effectiveRevision}; ${context.compiled.provenance.map(({contributorName})=>contributorName).join(", ")}`])]},diagnostics:FlowDocumentationTable={title:snapshot.title,headings:["Source state","Context","Property","Issue","Repair","Generated"],rows:snapshot.diagnostics.length?snapshot.diagnostics.map((item)=>[snapshot.incomplete?"Draft — incomplete":snapshot.sourceState,item.contextName,item.path,item.issue,item.repair,snapshot.generatedAt]):[[snapshot.incomplete?"Draft — incomplete":snapshot.sourceState,"All selected contexts","","No export blockers","",snapshot.generatedAt]]},names=["Flow values","Capture matrix","Legend and provenance","Export diagnostics"],tables=[input.valueTable,input.matrixTable,legend,diagnostics];
  ...
  return zipDocumentationFiles([{name:"[Content_Types].xml",content:types},{name:"_rels/.rels",content:rootRels},{name:"xl/workbook.xml",content:workbook},{name:"xl/_rels/workbook.xml.rels",content:relationships},...tables.map((table,index)=>({name:`xl/worksheets/sheet${index+1}.xml`,content:documentationWorksheet(table)}))]);
}
