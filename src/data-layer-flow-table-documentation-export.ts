...
export interface FlowDocumentationTable{title:string;headings:readonly string[];rows:readonly (readonly string[])[];legend?:string}
...
export function documentationWorksheet(table:FlowDocumentationTable):string{const rows=[[table.title,...Array(Math.max(0,table.headings.length-1)).fill("")],table.headings,...table.rows],body=rows.map((row,rowIndex)=>`<row r="${rowIndex+1}">${row.map((value,columnIndex)=>`<c r="${columnName(columnIndex)}${rowIndex+1}" t="inlineStr"><is><t xml:space="preserve">${xml(value)}</t></is></c>`).join("")}</row>`).join("");return`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData>${body}</sheetData></worksheet>`;}
