...
import type {ContextExportSnapshot,ContextExportSource} from "./contracts.js";
export type {ContextExportSource,ContextExportSnapshot} from "./contracts.js";

export function contextExportIdentity(source:ContextExportSource):string {
  const canonical=source.canonical?structuredClone(source.canonical):undefined;
  if(canonical){delete canonical.selectedPropertyId;delete canonical.changes;canonical.view="tree";}
...
export function createContextExportSnapshot(source:ContextExportSource):ContextExportSnapshot {
  ...
  if(!result)throw new Error("The selected schema is no longer available.");
  const document:Record<string,unknown>={$schema:JSON_SCHEMA_2020_12_DIALECT,...result.document,title:frozen.name};
  if(frozen.version==="Draft")delete document.$id;
  else document.$id=jsonSchemaResourceId({id:frozen.schema?.id??frozen.canonical!.contributorId,version:frozen.version});
