import {canonicalPropertyPath,type CanonicalSchemaDocument} from "../data-layer-canonical-schema.js";
...
import type {ContextExportSource} from "./snapshot.js";

/** Resolve the same effective contribution used by the selected editor. */
export function projectContextExportSource(state:ProjectState,entity:ProjectEntity,scope:LayerScope,flowId?:string,pending=false):ContextExportSource {
  const graph=flowId?(state.project.documentationFlowGraphs as Record<string,{pageFrames?:ProjectEntity[];occurrences?:ProjectEntity[]}>)[flowId]:undefined;
  ...
  if(!live)throw new Error("The selected schema context is no longer available.");
  entity=live;
  let canonical:CanonicalSchemaDocument;
  const errors:string[]=[];
  if(scope==="Shared Profile"){
    if(!entity.canonicalSchema)throw new Error("The selected canonical schema is unavailable.");
    canonical=structuredClone(entity.canonicalSchema as CanonicalSchemaDocument);
  }else{
    const workspace=composedSchemaWorkspace(state,entity,scope,undefined,flowId);
    if(workspace.status==="blocked")errors.push(workspace.conflictSummary);
    ...
    canonical.nodes=Object.fromEntries(kept.map(node=>[node.id,node]));canonical.rootIds=canonical.rootIds.filter(id=>Boolean(canonical.nodes[id]));
  }
