...
import {
  inspectSchemaPropertyTypeEdit,
  schemaPropertyTypeImpactCanReplace,
  schemaPropertyTypeLabel,
  type EditablePropertyType,
  type SchemaPropertyTypeEdit,
  type TypeMismatchTreatment,
} from "./data-layer-schema-property-type-editing.js";
import type { SchemaDefinition } from "./data-layer-schema-verification.js";

export interface SchemaPropertyTypeEditorOptions {
  schema: SchemaDefinition;
  path: string;
  property: JsonSchema | undefined;
  inheritedOwner?: { name: string; open: () => void };
  confirm: (edit: SchemaPropertyTypeEdit) => void;
}

export interface SchemaPropertyTypeEditorControls {
  action: HTMLButtonElement;
  editor: HTMLFieldSetElement;
}

const valueTypes: readonly EditablePropertyType[] = ["string", "number", "boolean", "object", "array"];
const itemTypes: readonly [string, string][] = [["", "Any item type"], ["string", "String"], ["number", "Number"], ["boolean", "Boolean"], ["object", "Object"]];
const treatments: readonly TypeMismatchTreatment[] = ["error", "warning", "ignore"];

function capital(value: string): string {
  return value.charAt(0).toUpperCase() + value.slice(1);
}

export function renderSchemaPropertyTypeEditor(options: SchemaPropertyTypeEditorOptions): SchemaPropertyTypeEditorControls {
  ...
  for (const value of valueTypes) valueType.append(Object.assign(document.createElement("option"), { value, textContent:capital(value) }));
  ...
  for (const [value, textContent] of itemTypes) itemType.append(Object.assign(document.createElement("option"), { value, textContent }));
  ...
  for (const value of treatments) treatment.append(Object.assign(document.createElement("option"), { value, textContent:capital(value) }));
  ...
  const selectedType = (): EditablePropertyType => valueType.value as EditablePropertyType;
  const selectedItemType = (): Exclude<EditablePropertyType, "array"> | undefined => itemType.value
    ? itemType.value as Exclude<EditablePropertyType, "array">
    : undefined;
  const selectedResolutions = (): Record<string, { action: "remove" | "replace"; value?: string }> => Object.fromEntries(
    Array.from(resolutions.querySelectorAll<HTMLSelectElement>("select[data-impact]"), (select) => {
      const replacement = resolutions.querySelector<HTMLInputElement>(`input[data-impact-replacement="${CSS.escape(select.dataset.impact!)}"]`);
      return [select.dataset.impact!, {
        action:select.value as "remove" | "replace",
        ...(select.value === "replace" && replacement ? { value:replacement.value } : {}),
      }];
    }),
  );
  const refreshResolutionState = (): void => {
    if (!currentImpact) return;
    confirm.disabled = currentImpact.incompatible.some((impact) => {
      const select = resolutions.querySelector<HTMLSelectElement>(`select[data-impact="${CSS.escape(impact)}"]`);
      const replacement = resolutions.querySelector<HTMLInputElement>(`input[data-impact-replacement="${CSS.escape(impact)}"]`);
  ...
  const refreshReview = (): void => {
    ...
    resolutions.replaceChildren(...currentImpact.incompatible.map((impact) => {
      ...
      select.append(
        Object.assign(document.createElement("option"), { value:"", textContent:"Choose resolution" }),
        Object.assign(document.createElement("option"), { value:"remove", textContent:"Remove artifact" }),
      );
      const replaceable = schemaPropertyTypeImpactCanReplace(impact, selectedType());
      if (replaceable) select.append(Object.assign(document.createElement("option"), { value:"replace", textContent:"Replace artifact" }));
  ...
  confirm.addEventListener("click", () => {
    try {
      ...
      options.confirm({
        ...
        treatment:treatment.value as TypeMismatchTreatment,
